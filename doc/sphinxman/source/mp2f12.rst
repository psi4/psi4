...
*Module:* :ref:`Keywords <apdx:mp2-f12>`, :ref:`PSI Variables <apdx:mp2-f12_psivar>`, :source:`MP2-F12 <psi4/src/psi4/f12>`
...
or without DF, as specified with the ``f12_type`` keyword. The general outline of an MP2-F12 computation is as follows:
...
The results will then look something like::
   ...
   ===> MP2-F12/3C(FIX) Energies <===
    ...
    Total MP2-F12/3C(FIX) Energy:         -76.362465009228
       RHF Reference Energy:              -76.059044958341
       MP2 Correlation Energy:             -0.276482152374
       F12/3C(FIX) Correlation Energy:     -0.025998112363
       CABS Singles Correction:            -0.000939786150
...
where the last term of the :ref:`conventional residual <Residual_Conv>` couples the two residuals. Within the 
:ref:`explicitly correlated residual <Residual_F12>`, the correlation factor, :math:`f_{12}`, and the projector, :math:`\hat{Q}_{12}`, 
...
the :ref:`projector <Ansatz_3>` is known as Ansatz 3.
...
When applying the :ref:`projector <CABS_Ansatz_3>` to the F12 intermediates, the :math:`C` and :math:`B` intermediates contain terms which 
...
:ref:`conventional residual <Residual_Conv>` being minimized and the explicitly correlated amplitudes being
.. math::
   :label: Fixed_Amps
...
appropriately documented in the Appendix :ref:`apdx:mp2f12` and care should be taken to read through 
them before changing defaults. Some basic recommendations for running the module are included below:

* MP2-F12 runs much faster with density-fitted integrals, in other words, set ``F12_TYPE  DF``. 
  Unless high accuracy is needed, DF requires much less computational resources and still provides 
  quality results. If DF is chosen, the HF and MP2 modules are also set to run DF. Choose a DF basis set 
  with at least triple-zeta quality as the HF module performs poorly with lower quality DF basis sets. 
  This module utilizes robust density-fitting [Dunlap:2000:2113]_ so answers may differ from other programs.
  The default is ``F12_TYPE  DF``.
...
* MP2-F12 cannot use Psi4's Schwarz screening due to the presence of mixed basis sets. This is a work in progress but 
  siginificantly increases the computational prefactor.
