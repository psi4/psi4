...
Memory use includes shared three-index arrays, AO blocks, and one workspace
per pair worker. The Psi4 memory allowance bounds the pair workspaces; shared
arrays and other steps need additional memory. The underlying SCF and MP2
steps may still use scratch files.
