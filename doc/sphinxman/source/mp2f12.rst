...
Streamed DF execution
---------------------

Set ``f12_subtype streamed`` to build the F12 correction one occupied pair at
a time. This path uses the same DF-MP2-F12/3C(FIX) equations as ``INCORE``.
It retains shared three-index integrals and reuses each worker's pair matrices,
without constructing a complete four-index F12 integral archive. The Coulomb
metric inverse is shared with Fock construction.

AO integrals are transformed in blocks of auxiliary shells. The optional
``f12_aux_block_size`` controls the target number of auxiliary functions per
block (default 64); individual shells are never split. Pair workers follow
the Psi4 thread count, up to the number of occupied pairs. Their results are
summed in a fixed order after the parallel loop.

``STREAMED`` currently requires an all-electron, closed-shell RHF reference
in C1, ``mp2_type df``, and a nonempty CABS space. Orbital, CABS, and fitting
bases are selected through the usual options; frozen core, ``f12_beta``, and
``cabs_singles`` retain their usual meanings. ECP references and frozen virtual
orbitals are rejected. ``f12_read_ints`` must be false. Only energies are
implemented.

Before allocating shared integrals, ``STREAMED`` estimates its main arrays
for the metric, Fock, operator-transform, pair, and CABS-singles stages. It
reduces the number of pair workers to fit the remaining memory after shared
arrays, or reports insufficient memory before integral generation. The estimate
excludes the reference wavefunction, basis construction, and library workspaces,
so total process memory can be higher. SCF and MP2 may still use scratch files.
