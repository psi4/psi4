...
Using the ``dft_functional`` option in the energy procedure call, one is able to use the orbitals generated by DFT for correlated wavefunction methods::
...
Psi4 also supports easy customization and manipulation of functionals.  The values of `\alpha` and `\omega` can be adjusted with the |scf__dft_alpha|
 and |scf__dft_omega| keywords.
For example, for LRC functionals, one can control the fraction of long-range Hartree-Fock and short-range DFA by changing |scf__dft_omega|::
    molecule ch2 {
      0 3
      C
      H 1 R
