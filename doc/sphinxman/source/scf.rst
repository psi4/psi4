...
Consequently, matrices in the AO basis are spin-blocked to account for all
spin-interactions, including opposite-spin,
...
where the ERI is also in a spinor basis. To reduce computational storage, the
ERI is left in the AO basis, and instead the spin-blocks of the density matrix
are decomposed to evaluate J and K.
