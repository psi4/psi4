...
for pydir in glob.glob('../../tests/plugin_*'):
    ...
    if dirname not in []:
        ...
        pts('plugin', dirname)
        fdriver.write('   autodir_plugins/%s' % (dirname))

        fmodule = open('source/autodir_plugins/'+dirname+'.rst', 'w')
        fmodule.write('\n.. _`sec:%s`:\n' % (dirname.lower()))
            ...
            for cfile in cfiles:
                ...
                while (ii < len(contents)):
                    ...
                    if module.match(line):
                        ...
                        if kw_type == 'boolean':
                            fmodule.write('      * **Type**: :ref:`boolean <op_c_boolean>`\n')

                        elif (kw_type == 'double') and ((kw_name.lower().find('conv') > -1) or (kw_name.lower().find('tol') > -1)):
                            fmodule.write('      * **Type**: :ref:`conv double <op_c_conv>`\n')

                        elif (kw_type == 'string') and ((kw_name.lower() == 'basis') or (kw_name.lower().startswith('df_basis'))):
                            fmodule.write('      * **Type**: %s\n' % kw_type)
                            fmodule.write('      * **Possible Values**: :ref:`basis string <apdx:basisElement>`\n')
                        ...
                        if not (not kw_possible or kw_possible.isspace()):
                            ...
                            fmodule.write('      * **Possible Values**: %s\n' % (', '.join(sline)))

                        fmodule.write('      * **Default**: %s\n\n' % kw_default)
