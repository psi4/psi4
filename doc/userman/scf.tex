...
\optionname{PRINT}{GLOBALS} option. Also printed are the occupied and virtual orbital
...
physical. Spin specialization is specified by the \optionname{REFERENCE}{SCF} option,
...
\optionname{DOCC}{GLOBALS} and \optionname{SOCC}{GLOBALS} integer arrays. If these arrays are not
...
\optionname{S-TOLERANCE}{SCF}, which defaults to 1.0\e{-7}. If the smallest
...
\optionname{S-ORTHOGONALIZATION}{SCF} option to \optionval{CANONICAL}. Note that in
...
the user-specified \optionname{E-CONVERGENCE}{SCF} and
\optionname{D-CONVERGENCE}{SCF}, respectively. The maximum number of iterations is
specified by the \optionname{MAXITER}{SCF} option. It should be noted that SCF is a
...
be invoked by the \optionname{GUESS}{SCF} option. A more involved alternative to these
...
iterations $N,N+2,\ldots$, invoking \optionname{MOM-START}{SCF} $N$ can often rescue the
...
\optionname{DAMPING-PERCENTAGE}{SCF} keyword to a nonzero percent. 
...
\PSIfour for these terms. The algorithm is selected by the \optionname{SCF-TYPE}{SCF}
...
bases, otherwise the \optionname{DF-BASIS-SCF}{SCF} keyword can be used to manually
...
sieving, set the \optionname{INTS-TOLERANCE}{SCF} keyword to your desired cutoff
...
\begin{itemize}
\item For \optionname{GUESS}{SCF}, the \optionval{SAD} guess is usually your friend, even for open-shell systems (at the
...
cost.  However, SAD and \optionname{DOCC}{GLOBALS}/\optionname{SOCC}{GLOBALS} arrays do not play very well
at the moment. 
\item Do not specify the \optionname{DOCC}{GLOBALS} or \optionname{SOCC}{GLOBALS} arrays unless you are
...
\item For very large systems, you may need to relax \optionname{E-CONVERGENCE}{SCF} and
\optionname{D-CONVERGENCE}{SCF}, as the error is extensive in the size of the system.
