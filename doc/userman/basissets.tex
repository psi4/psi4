...
For built-in basis sets, the correct \optionval{spherical}/\optionval{cartesian} value is set internally from the orbital basis.
...
For basis set availability by element and the default value for keyword \optionname{PUREAM}, consult Appendix \ref{basisElement}.

%C++ option name (optionname): \optionname{pueram}

%C++ option value (optionval): \optionval{true}

%Python keyword name (pyoptionname): \pyoptionname{cast\_up}

%Python keyword value (pyoptionval): \pyoptionval{6}
...
As the first line of the file, add the word \optionval{spherical} or \optionval{cartesian} to indicate
...
As the second line of the file, add the word \optionval{spherical} or \optionval{cartesian} to indicate
whether the basis set will run in (5D/7F) or (6D/10F). 
The combination of [ basis name ], \optionname{PUREAM} value, and element basis set specifications 
...
Construct for a basis set a section like the one above that includes [ basis name ], \optionname{PUREAM}
...
The primary basis set is specified with option \optionname{BASIS}, which assigns,
