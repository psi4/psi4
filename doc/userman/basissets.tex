...
definition files at {\tt psi4/lib/basis} in the source.  For basis set availability by
...
whether the basis set will run in (5D/7F) or (6D/10F).
The combination of [ basis name ], \optionname{PUREAM}{GLOBALS} value, and element basis set specifications
...
Construct for a basis set a section like the one above that includes [ basis name ], \optionname{PUREAM}{GLOBALS}
...
The primary basis set is specified with option \optionname{BASIS}{MINTS}, which assigns,
...
More details are provided in section \ref{sec:PsithonBasisSets}.
