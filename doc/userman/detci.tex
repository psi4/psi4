...
(especially \optionname{EX\_LEVEL}) are set appropriately.
...
call, e.g., {\tt energy(\qq{mp10}\qq)} to invoke an MP10 computation, and
...
{\tt energy(\qq{mp10}\qq)} sets \optionname{mpn} = \optionval{true}.
The program uses the Wigner $(2n+1)$ rule by default (\optionname{wigner}
...
sets \optionname{MAX\_NUM\_VECS} to the required order in the wavefunction.
...
option \optionname{cc} to \optionval{true}, and set
\optionname{cc\_ex\_lvl} (note: not \optionname{ex\_lvl})
