...
void GA_Get_stats (mystat_t *mystat)
{
     GAstat_arr = (long*)&GAstat;

     mystat->numcre = GAstat_arr[0]; 
     mystat->numdes = GAstat_arr[1];
     mystat->numget = GAstat_arr[2];
     mystat->numput = GAstat_arr[3];
     mystat->numacc = GAstat_arr[4];
     mystat->numsca = GAstat_arr[5];
     mystat->numgat = GAstat_arr[6];
     mystat->numrdi = GAstat_arr[7];
     mystat->numget_procs = GAstat.numget_procs;
     mystat->numput_procs = GAstat.numput_procs;
     mystat->numacc_procs = GAstat.numacc_procs;
     mystat->numsca_procs = GAstat.numsca_procs;
     mystat->numgat_procs = GAstat.numgat_procs;
     mystat->gettot = GAbytes.gettot;
     mystat->getloc = GAbytes.getloc; 
     mystat->puttot = GAbytes.puttot;
     mystat->putloc = GAbytes.putloc;
     mystat->acctot = GAbytes.acctot;
     mystat->accloc = GAbytes.accloc;
     mystat->scatot = GAbytes.scatot;
     mystat->scaloc = GAbytes.scaloc;
     mystat->gattot = GAbytes.gattot;
     mystat->gatloc = GAbytes.gatloc;
     mystat->rditot = GAbytes.rditot;
     mystat->rdiloc = GAbytes.rdiloc;
     mystat->maxmem = GAstat.maxmem;
     mystat->curmem = GAstat.curmem;
     mystat->numser = GAstat.numser;
}


void GA_Reset_stats (void)
{
     GAstat_arr = (long*)&GAstat;
...
void GA_Put_stats (mystat_t *mystat)
{
    printf("\n                         GA Statistics\n");
    printf("                         ------------------------------\n\n");
    ...
    if (mystat->numcre < 9999)
        printf("%4ld     ",mystat->numcre);
    else
        printf("%.2e ",(double)mystat->numcre);
    if (mystat->numdes < 9999)
        printf("%4ld     ",mystat->numdes);
    else
        printf("%.2e ",(double)mystat->numdes); 
    if (mystat->numget < 9999)
        printf("%4ld     ",mystat->numget);
    else
        printf("%.2e ",(double)mystat->numget);
    if (mystat->numput < 9999)
        printf("%4ld     ",mystat->numput);
    else
        printf("%.2e ",(double)mystat->numput);
    if (mystat->numacc < 9999)
        printf("%4ld     ",mystat->numacc);
    else
        printf("%.2e ",(double)mystat->numacc);
    if (mystat->numsca < 9999)
        printf("%4ld     ",mystat->numsca);
    else
        printf("%.2e ",(double)mystat->numsca);
    if (mystat->numgat < 9999)
        printf("%4ld     ",mystat->numgat);
    else
        printf("%.2e ",(double)mystat->numgat);
    if (mystat->numrdi < 9999)
        printf("%4ld     ",mystat->numrdi);
    else
        printf("%.2e ",(double)mystat->numrdi);
    printf("\n");
 
    if(mystat->numget==0)mystat->numget=1;
    if(mystat->numput==0)mystat->numput=1;
    if(mystat->numacc==0)mystat->numacc=1;
    if(mystat->numsca==0)mystat->numsca=1;
    if(mystat->numgat==0)mystat->numgat=1;
    printf("number of processes/call %.2e %.2e %.2e %.2e %.2e\n",
                  ((double)mystat->numget_procs)/(double)mystat->numget,
                  ((double)mystat->numput_procs)/(double)mystat->numput,
                  ((double)mystat->numacc_procs)/(double)mystat->numacc,
                  ((double)mystat->numsca_procs)/(double)mystat->numsca,
                  ((double)mystat->numgat_procs)/(double)mystat->numgat);

    printf("bytes total:             %.2e %.2e %.2e %.2e %.2e %.2e\n",
                  mystat->gettot, mystat->puttot, mystat->acctot,
                  mystat->scatot, mystat->gattot, mystat->rditot);

    printf("bytes remote:            %.2e %.2e %.2e %.2e %.2e %.2e\n",
                  mystat->gettot - mystat->getloc, 
                  mystat->puttot - mystat->putloc,
                  mystat->acctot - mystat->accloc,
                  mystat->scatot - mystat->scaloc,
                  mystat->gattot - mystat->gatloc,
                  mystat->rditot - mystat->rdiloc);
