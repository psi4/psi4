...
def induction(cache, jk, do_print=True):
    ...
    # Solve
    x_B_MOA = cache["wfn_A"].cphf_solve([w_B_MOA], 1.e-8, 20, 2)[0]
    x_A_MOB = cache["wfn_B"].cphf_solve([w_A_MOB], 1.e-8, 20, 2)[0]
    ...
    W = np.dot(cache["Cocc_A"].np.T, W).dot(cache["Cvir_A"])

    indexch_ab = 2.0 * np.vdot(W, x_B_MOA)
    ...
    W = np.dot(cache["Cocc_B"].np.T, W).dot(cache["Cvir_B"])

    indexch_ba = 2.0 * np.vdot(W, x_A_MOB)
