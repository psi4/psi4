...
* user levels kwarg processed into modelchem treatment levels and the n-body levels they cover. fields nbodies_per_mc_level set and max_nbody reset
* for each modelchem treatment level, call build_tasks() below via one of four routes, depending on simple MB or layered MB(FD), MB(CBS), or MB(FD(CBS))
    ...
    * merge all the requested indices, prepend a modelchem treatment index to form `mc_(frag, bas)`
...
----------------------------------
    ...
    ----------------------
        ...
        * if multiple modelchems (multilevel):
            ...
            ----------------------------
            * separates out each modelchem calcs and calls _prepare_results on each
        ...
        * for each available derivative, call:
            ...
            * for each of cp/nocp/vmfc, apply appropriate formula to build each n-body level of cummulative total energy into body_dict
        ...
        * in struct["intermediates_{ptype}"], store dict of `task_list key = return_{ptype}` for all in task_list or results kwarg
    ...
    * form model, including copy of class with mols converted to qcsk at atres.extras["component_results"]
...
class ManyBodyComputer(BaseComputer):
    ...
    def prepare_results(
        ...
        |   |em| CP-CORRECTED TOTAL ENERGY THROUGH 1-BODY               |  |em| 1              | when cp in bsse_type                                               | weighted sum of subsystems of 1-body. summed are total energies with cp treatment                                  |
        |   |em| CP-CORRECTED TOTAL ENERGY THROUGH 2-BODY               |  |em| 1              | when cp in bsse_type & max_nbody>=2                                | weighted sum of subsystems of 2-body or fewer (cummulative); summed are total energies with cp treatment           |
        |   |em| CP-CORRECTED TOTAL ENERGY THROUGH {nb}-BODY            |  |em| 1              | when cp in bsse_type                                               | weighted sum of subsystems of {max_nbody}-body or fewer (cummulative); summed are total energies w/ cp treatment   |
        ...
        |   |em| CP-CORRECTED INTERACTION ENERGY THROUGH 2-BODY         |  |em| 1              | when cp in bsse_type & max_nbody>=2                                | 2-body total data less 1-body total data for cummulative IE; data are total energies with cp treatment             |
        |   |em| CP-CORRECTED INTERACTION ENERGY THROUGH {nb}-BODY      |  |em| 1              | when cp in bsse_type                                               | {max_nbody}-body total data less 1-body total data for cummulative IE; data are total energies with cp treatment   |
        ...
        |   |em| CP-CORRECTED 2-BODY CONTRIBUTION TO ENERGY             |  |em| 1              | when cp in bsse_type & max_nbody>=2                                | 2-body total data less (2-1)-body total data for partial IE; data are total energies w/ cp treatment               |
        |   |em| CP-CORRECTED {nb}-BODY CONTRIBUTION TO ENERGY          |  |em| 1              | when cp in bsse_type                                               | {max_nbody}-body total data less ({max_nbody}-1)-body data for partial IE; data are total energies w/ cp treatment |
        |                                                               |                      |                                                                    |                                                                                                                    |
        |   |em| NOCP-CORRECTED TOTAL ENERGY THROUGH 1-BODY             |  |em| 1              | when nocp in bsse_type                                             | weighted sum of subsystems of 1-body. summed are total energies without cp treatment                               |
        |   |em| NOCP-CORRECTED TOTAL ENERGY THROUGH 2-BODY             |  |em| 1              | when nocp in bsse_type & max_nbody>=2                              | weighted sum of subsystems of 2-body or fewer (cummulative); summed are total energies without cp treatment        |
        |   |em| NOCP-CORRECTED TOTAL ENERGY THROUGH {nb}-BODY          |  |em| 1              | when nocp in bsse_type                                             | weighted sum of subsystems of {max_nbody}-body or fewer (cummulative); summed are total energies w/o cp treatment  |
        ...
        |   |em| NOCP-CORRECTED INTERACTION ENERGY THROUGH 2-BODY       |  |em| 1              | when nocp in bsse_type & max_nbody>=2                              | 2-body total data less 1-body total data for cummulative IE; data are total energies w/o cp treatment              |
        |   |em| NOCP-CORRECTED INTERACTION ENERGY THROUGH {nb}-BODY    |  |em| 1              | when nocp in bsse_type                                             | {max_nbody}-body total data less 1-body total data for cummulative IE; data are total energies w/o cp treatment    |
        ...
        |   |em| NOCP-CORRECTED 2-BODY CONTRIBUTION TO ENERGY           |  |em| 1              | when nocp in bsse_type & max_nbody>=2                              | 2-body total data less (2-1)-body total data for partial IE; data are total energies w/o cp treatment              |
        |   |em| NOCP-CORRECTED {nb}-BODY CONTRIBUTION TO ENERGY        |  |em| 1              | when nocp in bsse_type                                             | {max_nbody}-body total data less ({max_nbody}-1)-body data for partial IE; data are total energies w/o cp treat.   |
        |                                                               |                      |                                                                    |                                                                                                                    |
        |   |em| VMFC-CORRECTED TOTAL ENERGY THROUGH 1-BODY             |  |em| 1              | when vmfc in bsse_type                                             | weighted sum of subsystems of 1-body. summed are total energies with vmfc treatment                                |
        |   |em| VMFC-CORRECTED TOTAL ENERGY THROUGH 2-BODY             |  |em| 1              | when vmfc in bsse_type & max_nbody>=2                              | weighted sum of subsystems of 2-body or fewer (cummulative); summed are total energies with vmfc treatment         |
        |   |em| VMFC-CORRECTED TOTAL ENERGY THROUGH {nb}-BODY          |  |em| 1              | when vmfc in bsse_type                                             | weighted sum of subsystems of {max_nbody}-body or fewer (cummulative); summed are total energies w/ vmfc treatment |
        ...
        |   |em| VMFC-CORRECTED INTERACTION ENERGY THROUGH 2-BODY       |  |em| 1              | when vmfc in bsse_type & max_nbody>=2                              | 2-body total data less 1-body total data for cummulative IE; data are total energies w/ vmfc treatment             |
        |   |em| VMFC-CORRECTED INTERACTION ENERGY THROUGH {nb}-BODY    |  |em| 1              | when vmfc in bsse_type                                             | {max_nbody}-body total data less 1-body total data for cummulative IE; data are total energies w/ vmfc treatment   |
        ...
        |   |em| VMFC-CORRECTED 2-BODY CONTRIBUTION TO ENERGY           |  |em| 1              | when vmfc in bsse_type & max_nbody>=2                              | 2-body total data less (2-1)-body total data for partial IE; data are total energies w/ vmfc treatment             |
        |   |em| VMFC-CORRECTED {nb}-BODY CONTRIBUTION TO ENERGY        |  |em| 1              | when vmfc in bsse_type                                             | {max_nbody}-body total data less ({max_nbody}-1)-body data for partial IE; data are total energies w/ vmfc treat.  |
        |                                                               |                      |                                                                    |                                                                                                                    |
        | energy_body_dict                                              | max_nbody            | always                                                             | total energies at each n-body level                                                                                |
        |  |em| 1                                                       |  |em| 1              | always; zeroed if cp & rtd=F                                       | cummulative through 1-body total energies w/ cp/nocp/vmfc treatment (dep. on 1st of bsse_type)                     |
        |  |em| 2                                                       |  |em| 1              | max_nbody>=2                                                       | cummulative through 2-body total energies w/ cp/nocp/vmfc treatment (dep. on 1st of bsse_type)                     |
        |  |em| {max_nbody}                                             |  |em| 1              | always                                                             | cummulative through {max_nbody}-body total energies w/ cp/nocp/vmfc treatment (dep. on 1st of bsse_type)           |
        | gradient_body_dict                                            | max_nbody            | when driver is g/h                                                 |                                                                                                                    |
        |  |em| 1                                                       |  |em| (nat, 3)       | when driver is g/h                                                 | cummulative through 1-body total gradients with cp/nocp/vmfc treatment (dep. on 1st of bsse_type)                  |
        |  |em| 2                                                       |  |em| (nat, 3)       | when driver is g/h & max_nbody>=2                                  | cummulative through 2-body total gradients with cp/nocp/vmfc treatment (dep. on 1st of bsse_type)                  |
        |  |em| {max_nbody}                                             |  |em| (nat, 3)       | when driver is g/h                                                 | cummulative through {max_nbody}-body total gradients w/ cp/nocp/vmfc treatment (dep. on 1st of bsse_type)          |
        | hessian_body_dict                                             | max_nbody            | when driver is h                                                   |                                                                                                                    |
        |  |em| 1                                                       |  |em| (nat*3, nat*3) | when driver is h                                                   | cummulative through 1-body total Hessians w/ cp/nocp/vmfc treatment (dep. on 1st of bsse_type)                     |
        |  |em| 2                                                       |  |em| (nat*3, nat*3) | when driver is h & max_nbody>=2                                    | cummulative through 2-body total Hessians w/ cp/nocp/vmfc treatment (dep. on 1st of bsse_type)                     |
        |  |em| {max_nbody}                                             |  |em| (nat*3, nat*3) | when driver is h                                                   | cummulative through {max_nbody}-body total Hessians w/ cp/nocp/vmfc treatment (dep. on 1st of bsse_type)           |
        |                                                               |                      |                                                                    |                                                                                                                    |
        | cp_energy_body_dict                                           | max_nbody            | always; zeroed if cp not in bsse_type                              | total energies at each n-body level with cp treatment                                                              |
        |   |em| 1cp                                                    |  |em| 1              | always; zeroed if cp not in bsse_type or rtd=F                     | cummulative through 1-body total energies with cp treatment                                                        |
        |   |em| 2cp                                                    |  |em| 1              | when max_nbody>=2; zeroed if cp not in bsse_type                   | cummulative through 2-body total energies with cp treatment                                                        |
        |   |em| {max_nbody}cp                                          |  |em| 1              | always; zeroed if cp not in bsse_type                              | cummulative through {max_nbody}-body total energies with cp treatment                                              |
        ...
        |   |em| 1cp                                                    |  |em| (nat, 3)       | when driver is g/h; zeroed if cp not in bsse_type or rtd=F         | cummulative through 1-body total gradients with cp treatment                                                       |
        |   |em| 2cp                                                    |  |em| (nat, 3)       | when driver is g/h & max_nbody>=2; zeroed if cp not in bsse_type   | cummulative through 2-body total gradients with cp treatment                                                       |
        |   |em| {max_nbody}cp                                          |  |em| (nat, 3)       | when driver is g/h; zeroed if cp not in bsse_type                  | cummulative through {max_nbody}-body total gradients with cp treatment                                             |
        ...
        |   |em| 1cp                                                    |  |em| (nat*3, nat*3) | when driver is h; zeroed if cp not in bsse_type or rtd=F           | cummulative through 1-body total Hessians with cp treatment                                                        |
        |   |em| 2cp                                                    |  |em| (nat*3, nat*3) | when driver is h & max_nbody>=2; zeroed if cp not in bsse_type     | cummulative through 2-body total Hessians with cp treatment                                                        |
        |   |em| {max_nbody}cp                                          |  |em| (nat*3, nat*3) | when driver is h; zeroed if cp not in bsse_type                    | cummulative through {max_nbody}-body total Hessians with cp treatment                                              |
        |                                                               |                      |                                                                    |                                                                                                                    |
        ...
        |   |em| 1nocp                                                  |  |em| 1              | always; zeroed if nocp not in bsse_type                            | cummulative through 1-body total energies with nocp treatment                                                      |
        |   |em| 2nocp                                                  |  |em| 1              | when max_nbody>=2; zeroed if nocp not in bsse_type                 | cummulative through 2-body total energies with nocp treatment                                                      |
        |   |em| {max_nbody}nocp                                        |  |em| 1              | always; zeroed if nocp not in bsse_type                            | cummulative through {max_nbody}-body total energies with nocp treatment                                            |
        ...
        |   |em| 1nocp                                                  |  |em| (nat, 3)       | when driver is g/h; zeroed if nocp not in bsse_type                | cummulative through 1-body total gradients with nocp treatment                                                     |
        |   |em| 2nocp                                                  |  |em| (nat, 3)       | when driver is g/h & max_nbody>=2; zeroed if nocp not in bsse_type | cummulative through 2-body total gradients with nocp treatment                                                     |
        |   |em| {max_nbody}nocp                                        |  |em| (nat, 3)       | when driver is g/h; zeroed if nocp not in bsse_type                | cummulative through {max_nbody}-body total gradients with nocp treatment                                           |
        ...
        |   |em| 1nocp                                                  |  |em| (nat*3, nat*3) | when driver is h; zeroed if nocp not in bsse_type                  | cummulative through 1-body total Hessians with nocp treatment                                                      |
        |   |em| 2nocp                                                  |  |em| (nat*3, nat*3) | when driver is h & max_nbody>=2; zeroed if nocp not in bsse_type   | cummulative through 2-body total Hessians with nocp treatment                                                      |
        |   |em| {max_nbody}nocp                                        |  |em| (nat*3, nat*3) | when driver is h; zeroed if nocp not in bsse_type                  | cummulative through {max_nbody}-body total Hessians with nocp treatment                                            |
        |                                                               |                      |                                                                    |                                                                                                                    |
        ...
        |   |em| 1vmfc                                                  |  |em| 1              | always; zeroed if vmfc not in bsse_type                            | cummulative through 1-body total energies with vmfc treatment                                                      |
        |   |em| 2vmfc                                                  |  |em| 1              | when max_nbody>=2; zeroed if vmfc not in bsse_type                 | cummulative through 2-body total energies with vmfc treatment                                                      |
        |   |em| {max_nbody}vmfc                                        |  |em| 1              | always; zeroed if vmfc not in bsse_type                            | cummulative through {max_nbody}-body total energies with vmfc treatment                                            |
        ...
        |   |em| 1vmfc                                                  |  |em| (nat, 3)       | when driver is g/h; zeroed if vmfc not in bsse_type                | cummulative through 1-body total gradients with vmfc treatment                                                     |
        |   |em| 2vmfc                                                  |  |em| (nat, 3)       | when driver is g/h & max_nbody>=2; zeroed if vmfc not in bsse_type | cummulative through 2-body total gradients with vmfc treatment                                                     |
        |   |em| {max_nbody}vmfc                                        |  |em| (nat, 3)       | when driver is g/h; zeroed if vmfc not in bsse_type                | cummulative through {max_nbody}-body total gradients with vmfc treatment                                           |
        ...
        |   |em| 1vmfc                                                  |  |em| (nat*3, nat*3) | when driver is h; zeroed if vmfc not in bsse_type                  | cummulative through 1-body total Hessians with vmfc treatment                                                      |
        |   |em| 2vmfc                                                  |  |em| (nat*3, nat*3) | when driver is h & max_nbody>=2; zeroed if vmfc not in bsse_type   | cummulative through 2-body total Hessians with vmfc treatment                                                      |
        |   |em| {max_nbody}vmfc                                        |  |em| (nat*3, nat*3) | when driver is h; zeroed if vmfc not in bsse_type                  | cummulative through {max_nbody}-body total Hessians with vmfc treatment                                            |
        |                                                               |                      |                                                                    |                                                                                                                    |
