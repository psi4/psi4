...
def mcscf_solver(ref_wfn):
    ...
    scf_energy = ciwfn.get_variable("HF TOTAL ENERGY")
    eold = scf_energy
    norb_iter = 1
    converged = False
    ah_step = False
    ...
    for mcscf_iter in range(1, mcscf_max_macroiteration + 1):
        ...
        ciwfn.form_opdm()
        ciwfn.form_tpdm()
        ci_grad_rms = ciwfn.get_variable("DETCI AVG DVEC NORM")
        ...
        current_energy = ciwfn.get_variable("MCSCF TOTAL ENERGY")
    ...
    # Set final energy
    ciwfn.set_variable("CURRENT ENERGY", ciwfn.get_variable("MCSCF TOTAL ENERGY"))
    ciwfn.set_energy(ciwfn.get_variable("MCSCF TOTAL ENERGY"))
