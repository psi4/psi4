...
def dftd3_c6_intermolecular_dispersion(
    atomic_numbers: np.ndarray,  # shape (n_atoms,)
    geometry: np.ndarray,  # shape (n_atoms, 3)
    C6s: np.ndarray,  # shape (n_atoms, n_atoms)
    monAs: np.ndarray,  # shape (nA,)
    ...
    for A in monAs:
        ...
        for B in monBs:
            ...
            energy += de
            pairwise_energies[A, B] = de
    raise NotImplementedError("Need to update function for D3")
    return {
        "D4 IE": energy,
        "FSAPT_EMPIRICAL_DISP": pairwise_energies,
    }
...
def sapt_dft_d4_interaction_energy(
    sapt_dimer, monomerA, monomerB, sapt_dft_functional, d4_type, data
):
...
def sapt_dft_d3_interaction_energy(
    sapt_dimer, monomerA, monomerB, sapt_dft_functional, d3_type, data
):
    if d3_type == "supermolecular":
        ...
        dimer_d3, _ = sapt_dimer.run_sdftd3(dashparam=params, dashlvl="d3bjeeqatm")
        data["D3 DIMER"] = dimer_d3
        monA_d3, _ = monomerA.run_sdftd3(dashparam=params, dashlvl="d3bjeeqatm")
        data["D3 MONOMER A"] = monA_d3
        monB_d3, _ = monomerB.run_sdftd3(dashparam=params, dashlvl="d3bjeeqatm")
        ...
        dimer_d3, _ = sapt_dimer.run_sdftd3(
            func=sapt_dft_functional, dashlvl="d3bjeeqatm"
        )
        data["D3 DIMER"] = dimer_d3
        monA_d3, _ = monomerA.run_sdftd3(func=sapt_dft_functional, dashlvl="d3bjeeqatm")
        data["D3 MONOMER A"] = monA_d3
        monB_d3, _ = monomerB.run_sdftd3(func=sapt_dft_functional, dashlvl="d3bjeeqatm")
        data["D3 MONOMER B"] = monB_d3
        data["D3 IE"] = dimer_d3 - monA_d3 - monB_d3
    elif d3_type == "intermolecular":
        dimer_d3, _ = sapt_dimer.run_sdftd3(sapt_dft_functional, property=True)
        geom, _, _, elez, _ = sapt_dimer.to_arrays()
        elez = np.array(elez, dtype=np.int32)
        monAs = np.array([i for i in range(monomerA.natom()) if monomerA.Z(i) > 0])
        monBs = np.array([i for i in range(monomerB.natom()) if monomerB.Z(i) > 0])
        if sapt_dft_functional.lower() == "pbe0":
            # SAPT_DFT_pbe0_adz_3_IE_supra
            params = [1.0, 0.89529639, -0.82033591, 0.03263695]
        elif sapt_dft_functional.lower() == "hf":
            # SAPT0_adz_3_IE_2B_BJ_inter
            params = [1.0, 0.56063068, 0.65530802, 1.06322537]
            # sadz (supermolecular damping...)
            # params = [1.0, 0.83055196, 0.70628586, 1.12379695]
        else:
            raise ValueError(
                "SAPT(DFT): DFTD3 parameters not available for functional %s"
                % sapt_dft_functional
            )
        v = dftd3_c6_intermolecular_dispersion(
            elez,
            geom,
            C6s=core.variable("DFTD3 C6 COEFFICIENTS").np,
            monAs=monAs,
            monBs=monBs,
            params=params,
        )
