...
class EmpericalDispersion(object):
    def __init__(self, alias, dtype, **kwargs):
        ...
        if self.disp_type == 'p4':
            self.disp = psi4.Dispersion.build(self.dtype, **self.dash_params)
            # self.disp = psi4.Dispersion.build(self.dtype, self.dash_params['s6'], 0, 0, 0)
        else:
            self.disp = None
    ...
    def print_out(self, level=1):
        ...
        psi4.print_out("   => %s: Empirical Dispersion <=\n\n" % self.dtype)
        psi4.print_out(self.description + "\n")

        psi4.print_out(self.citation + "\n\n")

        psi4.print_out("        S6 = %14.6E\n" % self.dash_params["s6"]);
        if "s8" in self.dash_params.keys():
            psi4.print_out("        S8 = %14.6E\n" % self.dash_params["s8"]);

        for k, v in self.dash_params.items():
            if k in ["s6", "s8"]: continue

            psi4.print_out("    %6s = %14.6E\n" % (k.upper(), v));
    ...
    def compute_energy(self, molecule):
        if self.disp_type == 'gr':
            return dftd3.run_dftd3(molecule, dashlvl=self.dtype.lower().replace('-', ''),
                                   dashparam=self.dash_params, verbose=True, dertype=0)
        else:
