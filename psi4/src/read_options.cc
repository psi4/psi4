...
int read_options(const std::string &name, Options &options, bool suppress_printing) {
    ...
    **Cfour Interface:** Keyword translates into |cfour__cfour_spherical|. -*/
    ...
    options.add_str("SCF_TYPE", "PK", "DIRECT DF MEM_DF DISK_DF PK OUT_OF_CORE CD GTFOCK DFJ+COSX DFJ+LINK");
    ...
    if (name == "CPHF" || options.read_globals()) {
        ...
        /*- SCF Type
         -*/
        options.add_str("SCF_TYPE", "DIRECT", "DIRECT DF PK OUT_OF_CORE PS INDEPENDENT GTFOCK DFJ+COSX");
