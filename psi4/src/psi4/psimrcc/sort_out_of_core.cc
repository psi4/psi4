        ...
        if (mat_it->second->is_integral() || mat_it->second->is_fock()) {
            ...
            while (mat_irrep < wfn_->moinfo()->get_nirreps() && !out_of_memory) {
                ...
                if (block_memory < ccintegrals_memory) {
                    ...
                    if (blocks_added == 0) {
                        outfile->Printf("\n    Matrix: %s irrep %d does not fit into memory",
                                        Matrix->get_label().c_str(), mat_irrep);
                        outfile->Printf("\n            memory required = %14lu bytes", (size_t)block_memory);
                    }
                    out_of_memory = true;
                }
            }
            if (!out_of_memory) mat_irrep = 0;
        }
        if (!out_of_memory) ++mat_it;
    }
        ...
        if (Matrix->is_chemist()) {
            for (size_t i = 0; i < Matrix->get_left_pairpi(h); ++i)
                for (size_t j = 0; j < Matrix->get_right_pairpi(h); ++j) {
                    ...
                    if (antisymmetric) matrix[h][i][j] -= trans->tei_block(pqrs[0], pqrs[3], pqrs[1], pqrs[2]);
                }
        } else {
            for (size_t i = 0; i < Matrix->get_left_pairpi(h); ++i)
                for (size_t j = 0; j < Matrix->get_right_pairpi(h); ++j) {
...
double CCSort::add_fock_two_out_of_core(int p, int q, int k, bool exchange) {
    ...
    if (exchange) term -= trans->tei_block(p, k, q, k);
    return (term);
}
