...
MkUpdater::MkUpdater(std::shared_ptr<PSIMRCCWfn> wfn, Options &options) : Updater(wfn, options) {}
...
void MkUpdater::update(int cycle, Hamiltonian *heff) {
    ...
    for (int mu = 0; mu < wfn_->moinfo()->get_nunique(); ++mu) {
        ...
        wfn_->blas()->scale("d'1[o][v]", mu_unique, cutoff_factor);
        wfn_->blas()->scale("d'1[O][V]", mu_unique, cutoff_factor);
        wfn_->blas()->scale("d'2[oo][vv]", mu_unique, cutoff_factor);
        wfn_->blas()->scale("d'2[oO][vV]", mu_unique, cutoff_factor);
        wfn_->blas()->scale("d'2[OO][VV]", mu_unique, cutoff_factor);
        wfn_->blas()->scale("t1_eqns[o][v]", mu_unique, c_mu);
        wfn_->blas()->scale("t1_eqns[O][V]", mu_unique, c_mu);
        wfn_->blas()->scale("t2_eqns[oo][vv]", mu_unique, c_mu);
        wfn_->blas()->scale("t2_eqns[oO][vV]", mu_unique, c_mu);
        wfn_->blas()->scale("t2_eqns[OO][VV]", mu_unique, c_mu);
    }
    ...
    for (int i = 0; i < wfn_->moinfo()->get_nunique(); i++) {
        ...
        wfn_->blas()->solve("t2[OO][VV]{" + i_str + "} += " + damp + " t2_old[OO][VV]{" + i_str + "}");
        zero_internal_amps();
        wfn_->blas()->solve("t2_old[oo][vv]{" + i_str + "} = t2[oo][vv]{" + i_str + "}");
        wfn_->blas()->solve("t2_old[oO][vV]{" + i_str + "} = t2[oO][vV]{" + i_str + "}");
        wfn_->blas()->solve("t2_old[OO][VV]{" + i_str + "} = t2[OO][VV]{" + i_str + "}");
    }
}
