...
void MatrixBase::zero() {
    ...
    if (nbites > 0) memset(&(matrix[0][0]), '\0', static_cast<size_t>(sizeof(double)) * nrows * ncols);
}
...
void MatrixBase::add(MatrixBase* A, double alpha, double beta) {
    if (nrows * ncols > 0) {
        ...
        if (alpha != 1.0) C_DSCAL(nrows * ncols, alpha, &(matrix[0][0]), 1);
...
void MatrixBase::contract(MatrixBase* A, MatrixBase* B, double const alpha, double const beta) {
    ...
    if ((max_r != 0) && (nrows != 0) && (ncols != 0))
        C_DGEMM('n', 't', nrows, ncols, max_r, alpha, &(A->get_matrix()[0][0]), max_r, &(B->get_matrix()[0][0]), max_r,
                beta, &(matrix[0][0]), ncols);
...
/**
 * Sets the block matrix B to B(pqr) = z * A(pqr) + a * M(pqr) + b * M(prq) + c * M(qpr) + d * M(qrp) + e * M(rpq) + f *
 * M(rqp)
 */
void BlockMatrix::add_permutation_1_2(double z, BlockMatrix* A, CCIndex* pqr_index, CCIndex* p_index, CCIndex* qr_index,
    ...
    for (pqr.first(); !pqr.end(); pqr.next()) {
        ...
        double value = z * blocks[p_sym]->get(p_rel, qr_rel) + a * A->blocks[p_sym]->get(p_rel, qr_rel) +
                       b * A->blocks[p_sym]->get(p_rel, rq_rel) + c * A->blocks[q_sym]->get(q_rel, pr_rel) +
                       d * A->blocks[q_sym]->get(q_rel, rp_rel) + e * A->blocks[r_sym]->get(r_rel, pq_rel) +
...
// * M(rqp)
// */
// void BlockMatrix::add_permutation_1_2_abc(double z,BlockMatrix* A, CCIndex* pqr_index,CCIndex* p_index,CCIndex*
// qr_index,
//    double a,double b,double c,double d,double e,double f)
//{
//  CCIndexIterator pqr(wfn_, pqr_index,sym);
...
void IndexMatrix::print() {
    for (BMMap::iterator iter = matrices.begin(); iter != matrices.end(); ++iter) {
        outfile->Printf("\n  Index = %4d Ref = %d", static_cast<int>(iter->first.first),
                        static_cast<int>(iter->first.second));
        iter->second->print();
    }
}
