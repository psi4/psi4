...
void MRCCSD_T::compute_restricted()
{
  ...
  if(nrefs < 100){
    for(int mu = 0; mu < nrefs; ++mu){
      ...
      outfile->Printf(" %17.12lf",closed_shell_factor * (E4ST_ooo[mu] + E4ST_ooO[mu] + E4ST_oOO[mu] + E4ST_OOO[mu]));
      outfile->Printf(" %17.12lf",closed_shell_factor * (E4DT_ooo[mu] + E4DT_ooO[mu] + E4DT_oOO[mu] + E4DT_OOO[mu]));
    }
  }
  outfile->Printf("\n   Tot ");
  double E4 = 0.0;
