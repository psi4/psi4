...
void MRCCSD_T::compute_restricted() {
    ...
    if (nrefs < 100) {
        for (int mu = 0; mu < nrefs; ++mu) {
            outfile->Printf("\n   %2d  ", mu);
            outfile->Printf(" %17.12lf", closed_shell_factor * (E4_ooo[mu] + E4_ooO[mu] + E4_oOO[mu] + E4_OOO[mu]));
            outfile->Printf(" %17.12lf", closed_shell_factor * (E4T_ooo[mu] + E4T_ooO[mu] + E4T_oOO[mu] + E4T_OOO[mu]));
            outfile->Printf(" %17.12lf",
                            closed_shell_factor * (E4ST_ooo[mu] + E4ST_ooO[mu] + E4ST_oOO[mu] + E4ST_OOO[mu]));
            outfile->Printf(" %17.12lf",
                            closed_shell_factor * (E4DT_ooo[mu] + E4DT_ooO[mu] + E4DT_oOO[mu] + E4DT_OOO[mu]));
        }
    }
    outfile->Printf("\n   Tot ");
    double E4 = 0.0;
    for (int mu = 0; mu < nrefs; ++mu)
        E4 += closed_shell_factor * (E4_ooo[mu] + E4_ooO[mu] + E4_oOO[mu] + E4_OOO[mu]) *
              h_eff->get_left_eigenvector(mu) * h_eff->get_right_eigenvector(mu);
    outfile->Printf(" %17.12lf", E4);
    double E4T = 0.0;
    for (int mu = 0; mu < nrefs; ++mu)
        E4T += closed_shell_factor * (E4T_ooo[mu] + E4T_ooO[mu] + E4T_oOO[mu] + E4T_OOO[mu]) *
               h_eff->get_left_eigenvector(mu) * h_eff->get_right_eigenvector(mu);
    outfile->Printf(" %17.12lf", E4T);
    double E4ST = 0.0;
    for (int mu = 0; mu < nrefs; ++mu)
        E4ST += closed_shell_factor * (E4ST_ooo[mu] + E4ST_ooO[mu] + E4ST_oOO[mu] + E4ST_OOO[mu]) *
                h_eff->get_left_eigenvector(mu) * h_eff->get_right_eigenvector(mu);
    outfile->Printf(" %17.12lf", E4ST);
    double E4DT = 0.0;
    for (int mu = 0; mu < nrefs; ++mu)
        E4DT += closed_shell_factor * (E4DT_ooo[mu] + E4DT_ooO[mu] + E4DT_oOO[mu] + E4DT_OOO[mu]) *
                h_eff->get_left_eigenvector(mu) * h_eff->get_right_eigenvector(mu);
    ...
    for (int mu = 0; mu < nrefs; ++mu) {
        ...
        for (int nu = 0; nu < nrefs; ++nu) {
            outfile->Printf(" %17.12lf", closed_shell_factor * d_h_eff[mu][nu]);
        }
    }
    ...
    if (!options_.get_bool("DIAGONALIZE_HEFF")) {
        ...
        for (int mu = 0; mu < nrefs; ++mu) {
            for (int nu = 0; nu < nrefs; ++nu) {
                if (mu != nu) {
                    Heff_E += h_eff->get_left_eigenvector(mu) * h_eff->get_right_eigenvector(nu) * closed_shell_factor *
                              d_h_eff[mu][nu];
                }
            }
        }
        ...
        for (int nu = 0; nu < nrefs; ++nu) {
            if (mu != nu) {
                ...
                if (options_.get_bool("DIAGONAL_CCSD_T")) {  // Option to add the off-diagonal correction
                    h_eff->add_matrix(mu, nu,
                                      closed_shell_factor * (E4_ooo[mu] + E4_ooO[mu] + E4_oOO[mu] + E4_OOO[mu]));
                }
            }
        }
    }
                        ...
                        for (abc.first(); !abc.end(); abc.next()) {
                            ...
                            if (is_avir_mu[a_abs] && is_avir_mu[b_abs] && is_avir_mu[c_abs]) {
                                ...
                                if ((i_sym == a_sym) & (jk_sym == bc_sym)) {
                                    e4ST[mu] += 0.5 * T1_ov_mu[i_sym][i_rel][a_rel] * V_oovv[jk_sym][jk_rel][bc_rel] *
                                                T[mu][ijk_sym]->get(a_sym, a_rel, bc_rel);
                                    e4DT[mu] += 0.5 * F_ov_mu[i_sym][i_rel][a_rel] *
                                                T2_oovv_mu[jk_sym][jk_rel][bc_rel] *
                                                T[mu][ijk_sym]->get(a_sym, a_rel, bc_rel);
                                }
                                if ((j_sym == a_sym) & (ik_sym == bc_sym)) {
                                    e4ST[mu] -= 0.5 * T1_ov_mu[j_sym][j_rel][a_rel] * V_oovv[ik_sym][ik_rel][bc_rel] *
                                                T[mu][ijk_sym]->get(a_sym, a_rel, bc_rel);
                                    e4DT[mu] -= 0.5 * F_ov_mu[j_sym][j_rel][a_rel] *
                                                T2_oovv_mu[ik_sym][ik_rel][bc_rel] *
                                                T[mu][ijk_sym]->get(a_sym, a_rel, bc_rel);
                                }
                                if ((k_sym == a_sym) & (ji_sym == bc_sym)) {
                                    e4ST[mu] -= 0.5 * T1_ov_mu[k_sym][k_rel][a_rel] * V_oovv[ji_sym][ji_rel][bc_rel] *
                                                T[mu][ijk_sym]->get(a_sym, a_rel, bc_rel);
                                    e4DT[mu] -= 0.5 * F_ov_mu[k_sym][k_rel][a_rel] *
                                                T2_oovv_mu[ji_sym][ji_rel][bc_rel] *
                                                T[mu][ijk_sym]->get(a_sym, a_rel, bc_rel);
                                }
                            }
                        }  // End loop over abc
                        ...
                        for (abc.first(); !abc.end(); abc.next()) {
                            ...
                            if (is_avir_mu[a_abs] && is_avir_mu[b_abs] && is_bvir_mu[c_abs]) {
                                ...
                                if ((k_sym == c_sym) & (ij_sym == ab_sym)) {
                                    e4ST[mu] += 0.5 * T1_OV_mu[k_sym][k_rel][c_rel] * V_oovv[ij_sym][ij_rel][ab_rel] *
                                                T[mu][ijk_sym]->get(a_sym, a_rel, bc_rel);
                        ...
                        for (abc.first(); !abc.end(); abc.next()) {
                            ...
                            if (is_avir_mu[a_abs] && is_bvir_mu[b_abs] && is_bvir_mu[c_abs]) {
                                ...
                                if ((j_sym == c_sym) & (ik_sym == ab_sym)) {
                                    e4ST[mu] -= T1_OV_mu[j_sym][j_rel][c_rel] * V_oOvV[ik_sym][ik_rel][ab_rel] *
                                                T[mu][ijk_sym]->get(a_sym, a_rel, bc_rel);
                                    e4DT[mu] -= F_OV_mu[j_sym][j_rel][c_rel] * T2_oOvV_mu[ik_sym][ik_rel][ab_rel] *
                                                T[mu][ijk_sym]->get(a_sym, a_rel, bc_rel);
                                }
                            }
                        }  // End loop over abc
