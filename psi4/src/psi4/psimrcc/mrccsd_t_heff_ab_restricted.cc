...
double MRCCSD_T::compute_AB_ooO_contribution_to_Heff_restricted(int u_abs, int V_abs, int x_abs, int Y_abs, int i_abs,
                                                                int j_abs, int k_abs, int mu, BlockMatrix* T3) {
    double value = 0.0;
    ...
    if ((i_abs == u_abs) && (k_abs == V_abs)) {
        CCIndexIterator e(wfn_, "[v]", j_sym);
        for (e.first(); !e.end(); e.next()) {
            int e_sym = v->get_tuple_irrep(e.ind_abs<0>());
            size_t e_rel = v->get_tuple_rel_index(e.ind_abs<0>());
            if (uv_sym == xy_sym) {
...
double MRCCSD_T::compute_AB_oOO_contribution_to_Heff_restricted(int u_abs, int V_abs, int x_abs, int Y_abs, int i_abs,
    ...
    if (i_abs == u_abs) {
        ...
        for (e.first(); !e.end(); e.next()) {
            ...
            size_t ye_rel = vv->get_tuple_rel_index(Y_abs, e.ind_abs<0>());
            if (jk_sym == ve_sym) {
                value -= T3->get(x_sym, x_rel, ye_rel) *
                         W_OOOV[mu][jk_sym][jk_rel][ve_rel];  // Could be wrong, not in the CH2 example
            }
        }
    }
