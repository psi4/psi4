...
void EFP::common_init() {
    ...
    if (!(efp_ = efp_create())) throw PsiException("EFP::common_init():efp_create()", __FILE__, __LINE__);
...
void EFP::add_fragments(std::vector<std::string> fnames) {
    ...
    for (size_t i = 1; i < fnames.size(); i++)
        if (uniq[i - 1] != uniq[i]) uniq[n_uniq++] = uniq[i];
    ...
    for (int i = 0; i < n_uniq; i++) {
        ...
        if (not_found)
            throw PsiException("EFP::add_fragments(): Fragment " + name + " not located in " + efpPath +
    ...
    for (size_t i = 0; i < fnames.size(); i++)
        if ((res = efp_add_fragment(efp_, fnames[i].c_str())))
            throw PsiException("EFP::add_fragments(): " + std::string(efp_result_to_string(res)) + " " + fnames[i],
                               __FILE__, __LINE__);
}
...
void EFP::finalize_fragments() {
    ...
    if ((res = efp_prepare(efp_)))
        throw PsiException("EFP::finalize_fragments() " + std::string(efp_result_to_string(res)), __FILE__, __LINE__);
}
...
int EFP::get_frag_multiplicity(int frag_idx) {
    ...
    if ((res = efp_get_frag_multiplicity(efp_, frag_idx, &multiplicity)))
        throw PsiException("EFP::get_frag_multiplicity(): " + std::string(efp_result_to_string(res)), __FILE__,
                           __LINE__);
...
double *EFP::get_frag_atom_Z(int frag_idx) {
    ...
    if ((res = efp_get_frag_atoms(efp_, frag_idx, frag_natom, atoms)))
        throw PsiException("EFP::get_frag_atom_Z() " + std::string(efp_result_to_string(res)), __FILE__, __LINE__);

    double *frag_atom_Z = new double[frag_natom];
    for (size_t i = 0; i < frag_natom; ++i) frag_atom_Z[i] = atoms[i].znuc;
...
double *EFP::get_frag_atom_mass(int frag_idx) {
    ...
    struct efp_atom atoms[frag_natom];
    if ((res = efp_get_frag_atoms(efp_, frag_idx, frag_natom, atoms)))
        throw PsiException("EFP::get_frag_atom_mass() " + std::string(efp_result_to_string(res)), __FILE__, __LINE__);

    double *frag_atom_mass = new double[frag_natom];
    for (size_t i = 0; i < frag_natom; ++i) frag_atom_mass[i] = atoms[i].mass;
...
std::vector<std::string> EFP::get_frag_atom_label(int frag_idx) {
    ...
    struct efp_atom atoms[frag_natom];
    if ((res = efp_get_frag_atoms(efp_, frag_idx, frag_natom, atoms)))
        throw PsiException("EFP::get_frag_atom_label() " + std::string(efp_result_to_string(res)), __FILE__, __LINE__);
...
double *EFP::get_frag_atom_coord(int frag_idx) {
    ...
    struct efp_atom atoms[frag_natom];
    if ((res = efp_get_frag_atoms(efp_, frag_idx, frag_natom, atoms)))
        throw PsiException("EFP::get_frag_atom_coord() " + std::string(efp_result_to_string(res)), __FILE__, __LINE__);
...
efp_result electron_density_field_fn(size_t n_pt, const double *xyz, double *field, void *user_data) {
    ...
    SharedMatrix Db;
    if (!wfn->same_a_b_orbs()) Db = wfn->Db();
...
void EFP::set_options() {
    ...
    if (elst_enabled_) opts.terms |= EFP_TERM_ELEC;
    if (pol_enabled_) opts.terms |= EFP_TERM_POL;
    if (disp_enabled_) opts.terms |= EFP_TERM_DISP;
    if (exch_enabled_) opts.terms |= EFP_TERM_XR;
    ...
    if ((res = efp_set_opts(efp_, &opts)))
        throw PsiException("EFP::set_options():efp_set_opts() " + std::string(efp_result_to_string(res)), __FILE__,
                           __LINE__);
    ...
    if ((efp_set_electron_density_field_fn(efp_, electron_density_field_fn)))
        throw PsiException(
            "EFP::set_options():efp_set_electron_density_field_fn() " + std::string(efp_result_to_string(res)),
            __FILE__, __LINE__);
}
...
void EFP::set_qm_atoms() {
    ...
    if ((res = efp_set_point_charges(efp_, natom, q_p, xyz_p)))
        throw PsiException("EFP::set_qm_atoms():efp_set_point_charges() " + std::string(efp_result_to_string(res)),
                           __FILE__, __LINE__);
}
...
std::shared_ptr<Matrix> EFP::modify_Fock_permanent() {
    ...
    if ((res = efp_get_multipole_count(efp_, &n_multipole)))
        throw PsiException(
            "EFP::modify_Fock_permanent():efp_get_multipole_count(): " + std::string(efp_result_to_string(res)),
            __FILE__, __LINE__);
    ...
    if ((res = efp_get_multipole_coordinates(efp_, xyz->pointer())))
        throw PsiException(
            "EFP::modify_Fock_permanent():efp_get_multipole_coordinates(): " + std::string(efp_result_to_string(res)),
            __FILE__, __LINE__);
    if ((res = efp_get_multipole_values(efp_, mult->pointer())))
        throw PsiException(
            "EFP::modify_Fock_permanent():efp_get_multipole_values(): " + std::string(efp_result_to_string(res)),
            __FILE__, __LINE__);
    ...
    for (int i = 0; i < 20; ++i) {
        mats.push_back(std::make_shared<Matrix>(nbf, nbf));
    }
    ...
    for (int frag = 0; frag < nfrag_; frag++) {
        ...
        if ((res = efp_get_frag_atom_count(efp_, frag, &natom)))
            throw PsiException(
                "EFP::modify_Fock_permanent():efp_get_frag_atom_count(): " + std::string(efp_result_to_string(res)),
                __FILE__, __LINE__);
    ...
    for (size_t n = 0; n < n_multipole; n++) {
        ...
        for (int frag = 0; frag < nfrag_; frag++) {
            ...
            if ((res = efp_get_frag_atoms(efp_, frag, natom, atoms)))
                throw PsiException(
                    "EFP::modify_Fock_permanent():efp_get_frag_atoms(): " + std::string(efp_result_to_string(res)),
                    __FILE__, __LINE__);
...
void EFP::compute() {
    ...
    if ((res = efp_compute(efp_, do_grad_ ? 1 : 0)))
        throw PsiException("EFP::compute():efp_compute() " + std::string(efp_result_to_string(res)), __FILE__,
                           __LINE__);

    if ((res = efp_get_energy(efp_, &energy)))
        throw PsiException("EFP::compute():efp_get_energy(): " + std::string(efp_result_to_string(res)), __FILE__,
                           __LINE__);
    ...
    Process::environment.globals["EFP ELST ENERGY"] =
    ...
    outfile->Printf("      %-7s                     %20.12f [Eh] %s\n", qm_pol_enabled_ ? "QM/EFP" : "EFP/EFP",
                    energy.polarization, (pol_enabled_ || qm_pol_enabled_) ? "*" : "");
    outfile->Printf("\n");
...
void EFP::print_out() {
    ...
    if (elst_enabled_) outfile->Printf("  Electrostatics damping:  %12s\n", elst_damping_.c_str());
    if (pol_enabled_) outfile->Printf("  Polarization damping:    %12s\n", pol_damping_.c_str());
    if (disp_enabled_) outfile->Printf("  Dispersion damping:      %12s\n", disp_damping_.c_str());
