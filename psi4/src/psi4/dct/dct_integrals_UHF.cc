...
void DCTSolver::transform_integrals() {
    ...
    if (options_.get_str("AO_BASIS") == "NONE" && options_.get_str("DCT_TYPE") == "CONV") sort_VVVV_integrals();
...
void DCTSolver::build_gtau() {
    dct_timer_on("DCTSolver::build_gtau()");
    ...
    dct_timer_off("DCTSolver::build_gtau()");
}
