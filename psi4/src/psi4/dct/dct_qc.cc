    ...
    while ((!orbitalsDone_ || !cumulantDone_ || !energyConverged_) && cycle++ < maxiter_) {
        ...
        if (nidp_ != 0) {
            ...
            if (options_.get_str("QC_TYPE") == "SIMULTANEOUS") {
                ...
                cycle_jacobi = run_twostep_dct_cumulant_updates();
...
void DCTSolver::compute_sigma_vector_cum_cum() {
    ...
    global_dpd_->buf4_init(&Tbb, PSIF_DCT_DPD, 0, ID("[o,v]"), ID("[o,v]"), ID("[o,v]"), ID("[o,v]"), 0,
                           "Temp (ov|ov)");
    global_dpd_->buf4_init(&D4aa, PSIF_DCT_DPD, 0, ID("[O,O]"), ID("[V,V]"), ID("[O>O]-"), ID("[V>V]-"), 0,
                           "D4 <OO|VV>");
...
void DCTSolver::compute_sigma_vector_cum_orb() {
    ...
    // DI_IJ = (IJ|KC) D_KC
    global_dpd_->file2_init(&DI, PSIF_DCT_DPD, 0, ID('O'), ID('O'), "DI <O|O>");
    global_dpd_->file2_init(&D2, PSIF_DCT_DPD, 0, ID('O'), ID('V'), "D2 <O|V>");
    ...
    // DI_AB = (AB|KC) D_KC
    global_dpd_->file2_init(&DI, PSIF_DCT_DPD, 0, ID('V'), ID('V'), "DI <V|V>");
    global_dpd_->file2_init(&D2, PSIF_DCT_DPD, 0, ID('O'), ID('V'), "D2 <O|V>");
...
int DCTSolver::iterate_nr_conjugate_gradients() {
...
int DCTSolver::iterate_nr_jacobi() {
...
void DCTSolver::check_qc_convergence() {
...
void DCTSolver::compute_orbital_rotation_nr() {
...
void DCTSolver::run_davidson() {
    ...
    outfile->Printf("\tStability analysis of DCT solution \n");
    SharedVector Evals;
    SharedMatrix Evecs;
    while (!converged) {
