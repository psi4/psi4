...
void densgrid_RHF(std::shared_ptr<Wavefunction> wfn, Options& options)
{
  double dens;
  double **D, **delta;
  double x, y, z;
...
y*pc_bohr2angstroms, z*pc_bohr2angstroms, dens/b2a3);
  ...
  // Prep .dx file
  std::shared_ptr<PsiOutStream> printer = std::make_shared<PsiOutStream>("density.dx",std::ostream::trunc);
  ...
  std::shared_ptr<PsiOutStream> printer2 = std::make_shared<PsiOutStream>("molecule.dx",std::ostream::trunc);
