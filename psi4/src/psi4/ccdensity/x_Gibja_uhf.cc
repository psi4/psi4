...
void x_Gibja_uhf(void)
{
  ...
  global_dpd_->buf4_init(&V2, PSIF_EOM_TMP, G_irr, 20, 20, 20, 20, 0, "R2L2_OVOV");
  global_dpd_->buf4_sort(&V2, PSIF_EOM_TMP0, rspq, 20, 20, "GIAJB");
  global_dpd_->buf4_close(&V2);
  global_dpd_->buf4_init(&V2, PSIF_EOM_TMP, G_irr, 30, 30, 30, 30, 0, "R2L2_ovov");
  global_dpd_->buf4_sort(&V2, PSIF_EOM_TMP0, rspq, 30, 30, "Giajb");
  global_dpd_->buf4_close(&V2);
  global_dpd_->buf4_init(&V2, PSIF_EOM_TMP, G_irr, 24, 24, 24, 24, 0, "R2L2_OvOv");
  global_dpd_->buf4_sort(&V2, PSIF_EOM_TMP0, rspq, 24, 24, "GIaJb");
  global_dpd_->buf4_close(&V2);
  global_dpd_->buf4_init(&V2, PSIF_EOM_TMP, G_irr, 27, 27, 27, 27, 0, "R2L2_oVoV");
  global_dpd_->buf4_sort(&V2, PSIF_EOM_TMP0, rspq, 27, 27, "GiAjB");
  global_dpd_->buf4_close(&V2);
  global_dpd_->buf4_init(&V2, PSIF_EOM_TMP, G_irr, 30, 20, 30, 20, 0, "R2L2_ovOV");
  global_dpd_->buf4_sort(&V2, PSIF_EOM_TMP0, rspq, 20, 30, "GIAjb");
  global_dpd_->buf4_close(&V2);
  global_dpd_->buf4_init(&V2, PSIF_EOM_TMP, G_irr, 20, 30, 20, 30, 0, "R2L2_OVov");
  global_dpd_->buf4_sort(&V2, PSIF_EOM_TMP0, rspq, 30, 20, "GiaJB");
  global_dpd_->buf4_close(&V2);

  /* term 4, G(IA,JB) <-- - L(IM,AE) T(J,E) R(M,B) */
  global_dpd_->buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 0, 21, 0, 21, 0, "Z(IM,AJ)");
  ...
  global_dpd_->buf4_init(&Z1, PSIF_EOM_TMP1, G_irr, 20, 21, 20, 21, 0, "Z(IB,AJ)");
  ...
  global_dpd_->buf4_sort(&Z1, PSIF_EOM_TMP1, prqs, 20, 21, "Z(IA,BJ)");
  global_dpd_->buf4_close(&Z1);
  global_dpd_->buf4_init(&Z1, PSIF_EOM_TMP1, G_irr, 20, 21, 20, 21, 0, "Z(IA,BJ)");
  global_dpd_->buf4_sort(&Z1, PSIF_EOM_TMP1, pqsr, 20, 20, "Z(IA,JB)");
  global_dpd_->buf4_close(&Z1);
  global_dpd_->buf4_init(&Z1, PSIF_EOM_TMP1, G_irr, 20, 20, 20, 20, 0, "Z(IA,JB)");
  global_dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 20, 20, 20, 20, 0, "GIAJB");
  global_dpd_->buf4_axpy(&Z1, &G, -1.0);
  global_dpd_->buf4_close(&Z1);
  global_dpd_->buf4_close(&G);

  /* term 4, G(ia,jb) <-- - L(im,ae) T(j,e) R(m,b) */
  global_dpd_->buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 10, 31, 10, 31, 0, "Z(im,aj)");
  ...
  global_dpd_->buf4_close(&L2);
  global_dpd_->buf4_init(&Z1, PSIF_EOM_TMP1, G_irr, 30, 31, 30, 31, 0, "Z(ib,aj)");
  ...
  global_dpd_->buf4_close(&Z);
  global_dpd_->buf4_sort(&Z1, PSIF_EOM_TMP1, prqs, 30, 31, "Z(ia,bj)");
  global_dpd_->buf4_close(&Z1);
  global_dpd_->buf4_init(&Z1, PSIF_EOM_TMP1, G_irr, 30, 31, 30, 31, 0, "Z(ia,bj)");
  global_dpd_->buf4_sort(&Z1, PSIF_EOM_TMP1, pqsr, 30, 30, "Z(ia,jb)");
  global_dpd_->buf4_close(&Z1);
  global_dpd_->buf4_init(&Z1, PSIF_EOM_TMP1, G_irr, 30, 30, 30, 30, 0, "Z(ia,jb)");
  global_dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 30, 30, 30, 30, 0, "Giajb");
  global_dpd_->buf4_axpy(&Z1, &G, -1.0);
  global_dpd_->buf4_close(&Z1);
  global_dpd_->buf4_close(&G);

  /* term 4, G(Ia,Jb) <-- - L(Im,aE) T(J,E) R(m,b) */
  global_dpd_->buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 22, 25, 22, 25, 0, "Z(Im,aJ)");
  global_dpd_->buf4_init(&L2, PSIF_CC_GL, L_irr, 22, 29, 22, 29, 0, "LIjaB");
  global_dpd_->file2_init(&T1A, PSIF_CC_OEI, 0, 0, 1, "tIA");
  global_dpd_->contract424(&L2, &T1A, &Z, 3, 1, 0, 1.0, 0.0);
  global_dpd_->file2_close(&T1A);
  global_dpd_->buf4_close(&L2);
  global_dpd_->buf4_init(&Z1, PSIF_EOM_TMP1, G_irr, 24, 25, 24, 25, 0, "Z(Ib,aJ)");
  global_dpd_->file2_init(&R1B, PSIF_CC_GR, R_irr, 2, 3, "Ria");
  global_dpd_->contract424(&Z, &R1B, &Z1, 1, 0, 1, 1.0, 0.0);
  global_dpd_->file2_close(&R1B);
  global_dpd_->buf4_close(&Z);
  global_dpd_->buf4_sort(&Z1, PSIF_EOM_TMP1, prqs, 24, 25, "Z(Ia,bJ)");
  global_dpd_->buf4_close(&Z1);
  global_dpd_->buf4_init(&Z1, PSIF_EOM_TMP1, G_irr, 24, 25, 24, 25, 0, "Z(Ia,bJ)");
  global_dpd_->buf4_sort(&Z1, PSIF_EOM_TMP1, pqsr, 24, 24, "Z(Ia,Jb)");
  global_dpd_->buf4_close(&Z1);
  global_dpd_->buf4_init(&Z1, PSIF_EOM_TMP1, G_irr, 24, 24, 24, 24, 0, "Z(Ia,Jb)");
  global_dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 24, 24, 24, 24, 0, "GIaJb");
  global_dpd_->buf4_axpy(&Z1, &G, 1.0);
  global_dpd_->buf4_close(&Z1);
  global_dpd_->buf4_close(&G);

  /* term 4, G(iA,jB) <-- - L(iM,Ae) T(j,e) R(M,B) */
  global_dpd_->buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 23, 26, 23, 26, 0, "Z(iM,Aj)");
  global_dpd_->buf4_init(&L2, PSIF_CC_GL, L_irr, 23, 28, 23, 28, 0, "LiJAb");
  global_dpd_->file2_init(&T1A, PSIF_CC_OEI, 0, 2, 3, "tia");
  global_dpd_->contract424(&L2, &T1A, &Z, 3, 1, 0, 1.0, 0.0);
  global_dpd_->file2_close(&T1A);
  global_dpd_->buf4_close(&L2);
  global_dpd_->buf4_init(&Z1, PSIF_EOM_TMP1, G_irr, 27, 26, 27, 26, 0, "Z(iB,Aj)");
  global_dpd_->file2_init(&R1A, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  global_dpd_->contract424(&Z, &R1A, &Z1, 1, 0, 1, 1.0, 0.0);
  global_dpd_->file2_close(&R1A);
  global_dpd_->buf4_close(&Z);
  global_dpd_->buf4_sort(&Z1, PSIF_EOM_TMP1, prqs, 27, 26, "Z(iA,Bj)");
  global_dpd_->buf4_close(&Z1);
  global_dpd_->buf4_init(&Z1, PSIF_EOM_TMP1, G_irr, 27, 26, 27, 26, 0, "Z(iA,Bj)");
  global_dpd_->buf4_sort(&Z1, PSIF_EOM_TMP1, pqsr, 27, 27, "Z(iA,jB)");
  global_dpd_->buf4_close(&Z1);
  global_dpd_->buf4_init(&Z1, PSIF_EOM_TMP1, G_irr, 27, 27, 27, 27, 0, "Z(iA,jB)");
  global_dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 27, 27, 27, 27, 0, "GiAjB");
  global_dpd_->buf4_axpy(&Z1, &G, 1.0);
  global_dpd_->buf4_close(&Z1);
  global_dpd_->buf4_close(&G);

  /* term 4, G(IA,jb) <-- - L2(Im,Ae) T(j,e) R(m,b) */
  global_dpd_->buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 22, 26, 22, 26, 0, "Z(Im,Aj)");
  ...
  global_dpd_->file2_close(&T1B);
  global_dpd_->file2_init(&R1B, PSIF_CC_GR, R_irr, 2, 3, "Ria");
  global_dpd_->buf4_init(&Z1, PSIF_EOM_TMP1, G_irr, 24, 26, 24, 26, 0, "Z(Ib,Aj)");
  global_dpd_->contract424(&Z, &R1B, &Z1, 1, 0, 1, 1.0, 0.0);
  global_dpd_->buf4_close(&Z);
  global_dpd_->file2_close(&R1B);
  global_dpd_->buf4_sort(&Z1, PSIF_EOM_TMP1, prqs, 20, 31, "Z(IA,bj)");
  global_dpd_->buf4_close(&Z1);
  global_dpd_->buf4_init(&Z1, PSIF_EOM_TMP1, G_irr, 20, 31, 20, 31, 0, "Z(IA,bj)");
  global_dpd_->buf4_sort(&Z1, PSIF_EOM_TMP1, pqsr, 20, 30, "Z(IA,jb)");
  global_dpd_->buf4_close(&Z1);
  global_dpd_->buf4_init(&Z1, PSIF_EOM_TMP1, G_irr, 20, 30, 20, 30, 0, "Z(IA,jb)");
  global_dpd_->buf4_init(&G, PSIF_EOM_TMP0, 0, 20, 30, 20, 30, 0, "GIAjb");
  global_dpd_->buf4_axpy(&Z1, &G, -1.0);
  global_dpd_->buf4_close(&Z1);
  global_dpd_->buf4_close(&G);

  /* term 4, G(ia,JB) <-- - L(iM,aE) T(J,E) R(M,B) */
  global_dpd_->buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 23, 25, 23, 25, 0, "Z(iM,aJ)");
  ...
  global_dpd_->file2_close(&T1A);
  global_dpd_->file2_init(&R1A, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  global_dpd_->buf4_init(&Z1, PSIF_EOM_TMP1, G_irr, 27, 25, 27, 25, 0, "Z(iB,aJ)");
  global_dpd_->contract424(&Z, &R1A, &Z1, 1, 0, 1, 1.0, 0.0);
  global_dpd_->buf4_close(&Z);
  global_dpd_->file2_close(&R1A);
  global_dpd_->buf4_sort(&Z1, PSIF_EOM_TMP1, prqs, 30, 21, "Z(ia,BJ)");
  global_dpd_->buf4_close(&Z1);
  global_dpd_->buf4_init(&Z1, PSIF_EOM_TMP1, G_irr, 30, 21, 30, 21, 0, "Z(ia,BJ)");
  global_dpd_->buf4_sort(&Z1, PSIF_EOM_TMP1, pqsr, 30, 20, "Z(ia,JB)");
  global_dpd_->buf4_close(&Z1);
  global_dpd_->buf4_init(&Z1, PSIF_EOM_TMP1, G_irr, 30, 20, 30, 20, 0, "Z(ia,JB)");
  global_dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 30, 20, 30, 20, 0, "GiaJB");
  global_dpd_->buf4_axpy(&Z1, &G, -1.0);
  global_dpd_->buf4_close(&Z1);
  global_dpd_->buf4_close(&G);
  ...
  global_dpd_->buf4_init(&Z, PSIF_EOM_TMP, G_irr, 0, 21, 2, 21, 0, "L2R1_OOVO");
  global_dpd_->buf4_init(&Z1, PSIF_EOM_TMP1, G_irr, 20, 21, 20, 21, 0, "Z(IB,AJ)");
  global_dpd_->file2_init(&T1A, PSIF_CC_OEI, 0, 0, 1, "tIA");
  ...
  /* term 5, G(ia,jb) <-- - (L(im,ae)*R(j,e)) * T(m,b) */
  global_dpd_->buf4_init(&Z, PSIF_EOM_TMP, G_irr, 10, 31, 12, 31, 0, "L2R1_oovo");
  global_dpd_->buf4_init(&Z1, PSIF_EOM_TMP1, G_irr, 30, 31, 30, 31, 0, "Z(ib,aj)");
  global_dpd_->file2_init(&T1B, PSIF_CC_OEI, 0, 2, 3, "tia");
  ...
  global_dpd_->buf4_init(&Z, PSIF_EOM_TMP, G_irr, 22, 25, 22, 25, 0, "L2R1_OovO");
  global_dpd_->buf4_init(&Z1, PSIF_EOM_TMP1, G_irr, 24, 25, 24, 25, 0, "Z(Ib,aJ)");
  global_dpd_->file2_init(&T1B, PSIF_CC_OEI, 0, 2, 3, "tia");
  global_dpd_->contract424(&Z, &T1B, &Z1, 1, 0, 1, 1.0, 0.0);
  global_dpd_->file2_close(&T1B);
  ...
  /* term 5, G(iA,jB) <-- - (L2(iM,eA)*R(j,e)) * T(M,B) */
  global_dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 23, 26, 23, 26, 0, "Z(iM,Aj)");
  global_dpd_->buf4_init(&L2, PSIF_CC_GL, L_irr, 23, 28, 23, 28, 0, "LiJAb");
  global_dpd_->file2_init(&R1A, PSIF_CC_GR, R_irr, 2, 3, "Ria");
  ...
  global_dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 22, 26, 22, 26, 0, "Z(Im,Aj)");
  global_dpd_->buf4_init(&L2, PSIF_CC_GL, L_irr, 22, 28, 22, 28, 0, "LIjAb");
  global_dpd_->file2_init(&R1B, PSIF_CC_GR, R_irr, 2, 3, "Ria");
  ...
  /* term 5, G(ia,JB) <-- - L(iM,aE) R(J,E) T(M,B) */
  global_dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 23, 25, 23, 25, 0, "Z(iM,aJ)");
  global_dpd_->buf4_init(&L2, PSIF_CC_GL, L_irr, 23, 29, 23, 29, 0, "LiJaB");
  global_dpd_->file2_init(&R1A, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  global_dpd_->contract424(&L2, &R1A, &Z, 3, 1, 0, 1.0, 0.0);
  global_dpd_->buf4_close(&L2);
  ...
  global_dpd_->file2_init(&L1B, PSIF_CC_GL, L_irr, 2, 3, "Lia");
  ...
  global_dpd_->buf4_close(&G);
    ...
    /* term 1, G(IA,jb) <-- L(I,A) R(j,b) */
  global_dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 20, 30, 20, 30, 0, "GIAjb");
  for(h=0; h < nirreps; h++) {
    global_dpd_->buf4_mat_irrep_init(&G, h);
    global_dpd_->buf4_mat_irrep_rd(&G, h);
    for(row=0; row < G.params->rowtot[h]; row++) {
  ...
  global_dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 20, 20, 20, 20, 0, "GIAJB");
  global_dpd_->buf4_sort(&G, PSIF_EOM_TMP0, psrq, 20, 20, "GIBJA");
  global_dpd_->buf4_close(&G);
  global_dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 30, 30, 30, 30, 0, "Giajb");
  global_dpd_->buf4_sort(&G, PSIF_EOM_TMP0, psrq, 30, 30, "Gibja");
  global_dpd_->buf4_close(&G);
  global_dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 24, 24, 24, 24, 0, "GIaJb");
  global_dpd_->buf4_scm(&G,-1.0);
  global_dpd_->buf4_sort(&G, PSIF_EOM_TMP0, psrq, 24, 24, "GIbJa");
  global_dpd_->buf4_close(&G);
  global_dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 27, 27, 27, 27, 0, "GiAjB");
  global_dpd_->buf4_scm(&G,-1.0);
  global_dpd_->buf4_sort(&G, PSIF_EOM_TMP0, psrq, 27, 27, "GiBjA");
  global_dpd_->buf4_close(&G);
  global_dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 20, 30, 20, 30, 0, "GIAjb");
  global_dpd_->buf4_sort(&G, PSIF_EOM_TMP0, psrq, 24, 27, "GIbjA");
  global_dpd_->buf4_close(&G);
  global_dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 30, 20, 30, 20, 0, "GiaJB");
  global_dpd_->buf4_sort(&G, PSIF_EOM_TMP0, psrq, 27, 24, "GiBJa");
  global_dpd_->buf4_close(&G);

  /* Add to ground state terms in CC_GAMMA */
  global_dpd_->buf4_init(&GIBJA, PSIF_EOM_TMP0, G_irr, 20, 20, 20, 20, 0, "GIBJA");
  ...
  global_dpd_->buf4_init(&Gibja, PSIF_EOM_TMP0, G_irr, 30, 30, 30, 30, 0, "Gibja");
  global_dpd_->buf4_init(&G, PSIF_CC_GAMMA, G_irr, 30, 30, 30, 30, 0, "Gibja");
  global_dpd_->buf4_axpy(&Gibja, &G, 1.0);
  global_dpd_->buf4_close(&G);
  global_dpd_->buf4_close(&Gibja);
  global_dpd_->buf4_init(&GIbJa, PSIF_EOM_TMP0, G_irr, 24, 24, 24, 24, 0, "GIbJa");
  global_dpd_->buf4_init(&G, PSIF_CC_GAMMA, G_irr, 24, 24, 24, 24, 0, "GIbJa");
  global_dpd_->buf4_axpy(&GIbJa, &G, 1.0);
  global_dpd_->buf4_close(&G);
  global_dpd_->buf4_close(&GIbJa);
  global_dpd_->buf4_init(&GiBjA, PSIF_EOM_TMP0, G_irr, 27, 27, 27, 27, 0, "GiBjA");
  global_dpd_->buf4_init(&G, PSIF_CC_GAMMA, G_irr, 27, 27, 27, 27, 0, "GiBjA");
  global_dpd_->buf4_axpy(&GiBjA, &G, 1.0);
  global_dpd_->buf4_close(&G);
  global_dpd_->buf4_close(&GiBjA);
  global_dpd_->buf4_init(&GIbjA, PSIF_EOM_TMP0, G_irr, 24, 27, 24, 27, 0, "GIbjA");
  global_dpd_->buf4_init(&G, PSIF_CC_GAMMA, G_irr, 24, 27, 24, 27, 0, "GIbjA");
  global_dpd_->buf4_axpy(&GIbjA, &G, 1.0);
  global_dpd_->buf4_close(&G);
  global_dpd_->buf4_close(&GIbjA);
  global_dpd_->buf4_init(&GiBJa, PSIF_EOM_TMP0, G_irr, 27, 24, 27, 24, 0, "GiBJa");
  ...
  global_dpd_->buf4_symm(&G);
  global_dpd_->buf4_close(&G);
  global_dpd_->buf4_init(&GIbjA, PSIF_CC_GAMMA, G_irr, 24, 27, 24, 27, 0, "GIbjA");
  global_dpd_->buf4_init(&GiBJa, PSIF_CC_GAMMA, G_irr, 27, 24, 27, 24, 0, "GiBJa");
  global_dpd_->buf4_symm2(&GIbjA, &GiBJa);
  global_dpd_->buf4_close(&GiBJa);
  global_dpd_->buf4_sort(&GIbjA, PSIF_CC_GAMMA, rspq, 27, 24, "GiBJa");
  global_dpd_->buf4_close(&GIbjA);
