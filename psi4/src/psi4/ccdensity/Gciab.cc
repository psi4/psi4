            ...
            for (h = 0; h < nirreps; h++) {
                ...
                for (row = 0; row < G.params->rowtot[h]; row++) {
                    ...
                    for (col = 0; col < G.params->coltot[h]; col++) {
                        ...
                        Csym = g.params->psym[c];
                        Isym = T1.params->psym[i];
                        A = g.params->colidx[a];
                        B = T1.params->colidx[b];
                        Asym = g.params->qsym[a];
                        Bsym = T1.params->qsym[b];

                        if ((Csym == Asym) && (Isym == Bsym)) value += g.matrix[Csym][C][A] * T1.matrix[Isym][I][B];
            ...
            for (h = 0; h < nirreps; h++) {
                ...
                for (row = 0; row < G.params->rowtot[h]; row++) {
                    ...
                    for (col = 0; col < G.params->coltot[h]; col++) {
                        ...
                        Bsym = g.params->qsym[b];
                        Asym = T1.params->qsym[a];

                        if ((Csym == Bsym) && (Isym == Asym)) value -= g.matrix[Csym][C][B] * T1.matrix[Isym][I][A];
