...
void DF_Helper::prepare_sparsity() {
    ...
    std::shared_ptr<IntegralFactory> rifactory = std::make_shared<IntegralFactory>(primary_, primary_, primary_, primary_);
...
void DF_Helper::prepare_AO() {
    ...
    std::shared_ptr<IntegralFactory> rifactory = std::make_shared<IntegralFactory>(aux_, zero, primary_, primary_);
...
void DF_Helper::prepare_metric_core() {
    ...
    std::shared_ptr<FittingMetric> Jinv = std::make_shared<FittingMetric>(aux_, true);
...
std::string DF_Helper::compute_metric(double pow) {
    ...
    if (std::fabs(pow - 1.0) < 1e-13)
        ...
        // get metric
        SharedMatrix metric = std::make_shared<Matrix>("met", naux_, naux_);
...
SharedMatrix DF_Helper::get_tensor(std::string name, std::vector<size_t> t0, std::vector<size_t> t1,
    ...
    SharedMatrix M = std::make_shared<Matrix>("M", A0, A1 * A2);
    double* Mp = M->pointer()[0];
