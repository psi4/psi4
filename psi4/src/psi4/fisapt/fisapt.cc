...
void FISAPT::nuclear() {
    ...
    std::shared_ptr<Matrix> Enucs = triplet(Zs, Rinv, Zs, true, false, false);
    Enucs->scale(0.5);
...
void FISAPT::unify() {
    ...
    std::shared_ptr<Matrix> D_A = doublet(Cocc_A, Cocc_A, false, true);
    std::shared_ptr<Matrix> D_B = doublet(Cocc_B, Cocc_B, false, true);
    std::shared_ptr<Matrix> D_C(D_A->clone());
    D_C->zero();
    if (Cocc_C->colspi()[0] > 0) {
        D_C = doublet(Cocc_C, Cocc_C, false, true);
    }
    ...
    // std::shared_ptr<Matrix> P_A = doublet(matrices_["Cvir0A"], matrices_["Cvir0A"], false, true);
    // std::shared_ptr<Matrix> P_B = doublet(matrices_["Cvir0B"], matrices_["Cvir0B"], false, true);

    // PA and PB are used only to define the complement of DA and DB in the DCBS
    std::shared_ptr<Matrix> P_A =
        doublet(reference_->Ca_subset("AO", "ALL"), reference_->Ca_subset("AO", "ALL"), false, true);
    P_A->subtract(D_A);
...
void FISAPT::dHF() {
    ...
    std::shared_ptr<Matrix> LD_A = doublet(LoccA, LoccA, false, true);
    std::shared_ptr<Matrix> LD_B = doublet(LoccB, LoccB, false, true);
...
void FISAPT::exch() {
    ...
    std::shared_ptr<Matrix> C_O = triplet(D_B, S, Cocc_A);
    std::vector<SharedMatrix>& Cl = jk_->C_left();
    std::vector<SharedMatrix>& Cr = jk_->C_right();
    const std::vector<SharedMatrix>& J = jk_->J();
    const std::vector<SharedMatrix>& K = jk_->K();
    ...
    Exch10_2M_terms[1] -= 2.0 * triplet(D_A, S, D_B)->vector_dot(V_A);
    Exch10_2M_terms[1] -= 4.0 * triplet(D_A, S, D_B)->vector_dot(J_A);
    Exch10_2M_terms[1] += 2.0 * triplet(D_A, S, D_B)->vector_dot(K_A);
    Exch10_2M_terms[2] -= 2.0 * triplet(D_B, S, D_A)->vector_dot(V_B);
    Exch10_2M_terms[2] -= 4.0 * triplet(D_B, S, D_A)->vector_dot(J_B);
    Exch10_2M_terms[2] += 2.0 * triplet(D_B, S, D_A)->vector_dot(K_B);
    Exch10_2M_terms[3] += 2.0 * triplet(triplet(D_B, S, D_A), S, D_B)->vector_dot(V_A);
    Exch10_2M_terms[3] += 4.0 * triplet(triplet(D_B, S, D_A), S, D_B)->vector_dot(J_A);
    Exch10_2M_terms[4] += 2.0 * triplet(triplet(D_A, S, D_B), S, D_A)->vector_dot(V_B);
    Exch10_2M_terms[4] += 4.0 * triplet(triplet(D_A, S, D_B), S, D_A)->vector_dot(J_B);
    Exch10_2M_terms[5] -= 2.0 * triplet(D_A, S, D_B)->vector_dot(K_O);
    ...
    std::shared_ptr<Matrix> C_AS = triplet(P_B, S, Cocc_A);
    Cl.clear();
    Cr.clear();
    Cl.push_back(Cocc_A);
    Cr.push_back(C_AS);
    ...
    std::shared_ptr<Matrix> Sab = triplet(matrices_["Cocc0A"], S, matrices_["Cocc0B"], true, false, false);
    ...
    std::shared_ptr<Matrix> T_A_n = doublet(matrices_["Cocc0A"], C_T_A_n, false, true);
    std::shared_ptr<Matrix> T_B_n = doublet(matrices_["Cocc0B"], C_T_B_n, false, true);
    std::shared_ptr<Matrix> T_BA_n = doublet(matrices_["Cocc0B"], C_T_BA_n, false, true);
    std::shared_ptr<Matrix> T_AB_n = doublet(matrices_["Cocc0A"], C_T_AB_n, false, true);
...
void FISAPT::ind() {
    ...
    std::shared_ptr<Matrix> C_O_A = triplet(D_B, S, matrices_["Cocc_A"]);
    std::shared_ptr<Matrix> C_P_A = triplet(triplet(D_B, S, D_A), S, matrices_["Cocc_B"]);
    std::shared_ptr<Matrix> C_P_B = triplet(triplet(D_A, S, D_B), S, matrices_["Cocc_A"]);
...
void FISAPT::disp(std::map<std::string, SharedMatrix> matrix_cache, std::map<std::string, SharedVector> vector_cache,
    ...
    std::shared_ptr<Matrix> Cr1 = triplet(D_B, S, Cvir0A);
    Cr1->scale(-1.0);
    Cr1->add(Cvir0A);
    std::shared_ptr<Matrix> Cs1 = triplet(D_A, S, Cvir0B);
    Cs1->scale(-1.0);
    Cs1->add(Cvir0B);
    std::shared_ptr<Matrix> Ca2 = triplet(D_B, S, Cocc0A);
    std::shared_ptr<Matrix> Cb2 = triplet(D_A, S, Cocc0B);
    std::shared_ptr<Matrix> Cr3 = triplet(D_B, S, Cvir0A);
    std::shared_ptr<Matrix> CrX = triplet(triplet(D_A, S, D_B), S, Cvir0A);
    Cr3->subtract(CrX);
    Cr3->scale(2.0);
    std::shared_ptr<Matrix> Cs3 = triplet(D_A, S, Cvir0B);
    std::shared_ptr<Matrix> CsX = triplet(triplet(D_B, S, D_A), S, Cvir0B);
    Cs3->subtract(CsX);
    Cs3->scale(2.0);
    std::shared_ptr<Matrix> Ca4 = triplet(triplet(D_A, S, D_B), S, Cocc0A);
    Ca4->scale(-2.0);
    std::shared_ptr<Matrix> Cb4 = triplet(triplet(D_B, S, D_A), S, Cocc0B);
    Cb4->scale(-2.0);

    // => Auxiliary V matrices <= //

    std::shared_ptr<Matrix> Jbr = triplet(Cocc0B, J_A, Cvir0A, true, false, false);
    Jbr->scale(2.0);
    std::shared_ptr<Matrix> Kbr = triplet(Cocc0B, K_A, Cvir0A, true, false, false);
    Kbr->scale(-1.0);

    std::shared_ptr<Matrix> Jas = triplet(Cocc0A, J_B, Cvir0B, true, false, false);
    Jas->scale(2.0);
    std::shared_ptr<Matrix> Kas = triplet(Cocc0A, K_B, Cvir0B, true, false, false);
    Kas->scale(-1.0);

    std::shared_ptr<Matrix> KOas = triplet(Cocc0A, K_O, Cvir0B, true, false, false);
    KOas->scale(1.0);
    std::shared_ptr<Matrix> KObr = triplet(Cocc0B, K_O, Cvir0A, true, true, false);
    KObr->scale(1.0);

    std::shared_ptr<Matrix> JBas = triplet(triplet(Cocc0A, S, D_B, true, false, false), J_A, Cvir0B);
    JBas->scale(-2.0);
    std::shared_ptr<Matrix> JAbr = triplet(triplet(Cocc0B, S, D_A, true, false, false), J_B, Cvir0A);
    JAbr->scale(-2.0);

    std::shared_ptr<Matrix> Jbs = triplet(Cocc0B, J_A, Cvir0B, true, false, false);
    Jbs->scale(4.0);
    std::shared_ptr<Matrix> Jar = triplet(Cocc0A, J_B, Cvir0A, true, false, false);
    Jar->scale(4.0);

    std::shared_ptr<Matrix> JAas = triplet(triplet(Cocc0A, J_B, D_A, true, false, false), S, Cvir0B);
    JAas->scale(-2.0);
    std::shared_ptr<Matrix> JBbr = triplet(triplet(Cocc0B, J_A, D_B, true, false, false), S, Cvir0A);
    JBbr->scale(-2.0);

    // Get your signs right Hesselmann!
    std::shared_ptr<Matrix> Vbs = triplet(Cocc0B, V_A, Cvir0B, true, false, false);
    Vbs->scale(2.0);
    std::shared_ptr<Matrix> Var = triplet(Cocc0A, V_B, Cvir0A, true, false, false);
    Var->scale(2.0);
    std::shared_ptr<Matrix> VBas = triplet(triplet(Cocc0A, S, D_B, true, false, false), V_A, Cvir0B);
    VBas->scale(-1.0);
    std::shared_ptr<Matrix> VAbr = triplet(triplet(Cocc0B, S, D_A, true, false, false), V_B, Cvir0A);
    VAbr->scale(-1.0);
    std::shared_ptr<Matrix> VRas = triplet(triplet(Cocc0A, V_B, P_A, true, false, false), S, Cvir0B);
    VRas->scale(1.0);
    std::shared_ptr<Matrix> VSbr = triplet(triplet(Cocc0B, V_A, P_B, true, false, false), S, Cvir0A);
    VSbr->scale(1.0);

    std::shared_ptr<Matrix> Sas = triplet(Cocc0A, S, Cvir0B, true, false, false);
    std::shared_ptr<Matrix> Sbr = triplet(Cocc0B, S, Cvir0A, true, false, false);
    ...
    std::shared_ptr<Matrix> SBar = triplet(triplet(Cocc0A, S, D_B, true, false, false), S, Cvir0A);
    std::shared_ptr<Matrix> SAbs = triplet(triplet(Cocc0B, S, D_A, true, false, false), S, Cvir0B);
...
void FISAPT::raw_plot(const std::string& filepath) {
    ...
    std::shared_ptr<Matrix> DFA = doublet(matrices_["LoccA"], matrices_["LoccA"], false, true);
    std::shared_ptr<Matrix> DFB = doublet(matrices_["LoccB"], matrices_["LoccB"], false, true);
...
void FISAPT::felst() {
    ...
    std::shared_ptr<Matrix> Elst10_3 = doublet(QaC, QbC, false, true);
    ...
    for (int A = 0; A < nA; A++) {
        ...
        std::shared_ptr<Matrix> Vbb =
            triplet(matrices_["Locc0B"], Vtemp2, matrices_["Locc0B"], true, false, false);
    ...
    for (int B = 0; B < nB; B++) {
        ...
        std::shared_ptr<Matrix> Vaa =
            triplet(matrices_["Locc0A"], Vtemp2, matrices_["Locc0A"], true, false, false);
...
void FISAPT::fexch() {
    ...
    std::shared_ptr<Matrix> WAbs = triplet(LoccB, W_A, CvirB, true, false, false);
    std::shared_ptr<Matrix> WBar = triplet(LoccA, W_B, CvirA, true, false, false);
    ...
    std::shared_ptr<Matrix> Sab = triplet(LoccA, S, LoccB, true, false, false);
    std::shared_ptr<Matrix> Sba = triplet(LoccB, S, LoccA, true, false, false);
    std::shared_ptr<Matrix> Sas = triplet(LoccA, S, CvirB, true, false, false);
    std::shared_ptr<Matrix> Sbr = triplet(LoccB, S, CvirA, true, false, false);
    double** Sabp = Sab->pointer();
...
void FISAPT::find() {
    ...
    for (size_t A = 0; A < nA; A++) {
        ...
        Zxyz2p[0][3] = mol->z(A);
        Vint2->compute(Vtemp2);
        std::shared_ptr<Matrix> Vbs = triplet(Cocc_B, Vtemp2, Cvir_B, true, false, false);
    ...
    for (size_t B = 0; B < nB; B++) {
        ...
        Zxyz2p[0][3] = mol->z(B);
        Vint2->compute(Vtemp2);
        std::shared_ptr<Matrix> Var = triplet(Cocc_A, Vtemp2, Cvir_A, true, false, false);
    ...
    for (size_t B = 0; B < nB + nb; B++) {
        ...
        std::shared_ptr<Matrix> x2A = doublet(Uocc_A, xA, true, false);
    ...
    for (size_t A = 0; A < nA + na; A++) {
        ...
        // Backtransform the amplitude to LO
        std::shared_ptr<Matrix> x2B = doublet(Uocc_B, xB, true, false);
...
void FISAPT::fdisp() {
    ...
    std::shared_ptr<Matrix> Cr1 = triplet(D_B, S, Cavir_A);
    Cr1->scale(-1.0);
    Cr1->add(Cavir_A);
    std::shared_ptr<Matrix> Cs1 = triplet(D_A, S, Cavir_B);
    Cs1->scale(-1.0);
    Cs1->add(Cavir_B);
    std::shared_ptr<Matrix> Ca2 = triplet(D_B, S, Caocc_A);
    std::shared_ptr<Matrix> Cb2 = triplet(D_A, S, Caocc_B);
    std::shared_ptr<Matrix> Cr3 = triplet(D_B, S, Cavir_A);
    std::shared_ptr<Matrix> CrX = triplet(triplet(D_A, S, D_B), S, Cavir_A);
    Cr3->subtract(CrX);
    Cr3->scale(2.0);
    std::shared_ptr<Matrix> Cs3 = triplet(D_A, S, Cavir_B);
    std::shared_ptr<Matrix> CsX = triplet(triplet(D_B, S, D_A), S, Cavir_B);
    Cs3->subtract(CsX);
    Cs3->scale(2.0);
    std::shared_ptr<Matrix> Ca4 = triplet(triplet(D_A, S, D_B), S, Caocc_A);
    Ca4->scale(-2.0);
    std::shared_ptr<Matrix> Cb4 = triplet(triplet(D_B, S, D_A), S, Caocc_B);
    Cb4->scale(-2.0);

    // => Auxiliary V matrices <= //

    std::shared_ptr<Matrix> Jbr = triplet(Caocc_B, J_A, Cavir_A, true, false, false);
    Jbr->scale(2.0);
    std::shared_ptr<Matrix> Kbr = triplet(Caocc_B, K_A, Cavir_A, true, false, false);
    Kbr->scale(-1.0);

    std::shared_ptr<Matrix> Jas = triplet(Caocc_A, J_B, Cavir_B, true, false, false);
    Jas->scale(2.0);
    std::shared_ptr<Matrix> Kas = triplet(Caocc_A, K_B, Cavir_B, true, false, false);
    Kas->scale(-1.0);

    std::shared_ptr<Matrix> KOas = triplet(Caocc_A, K_O, Cavir_B, true, false, false);
    KOas->scale(1.0);
    std::shared_ptr<Matrix> KObr = triplet(Caocc_B, K_O, Cavir_A, true, true, false);
    KObr->scale(1.0);

    std::shared_ptr<Matrix> JBas = triplet(triplet(Caocc_A, S, D_B, true, false, false), J_A, Cavir_B);
    JBas->scale(-2.0);
    std::shared_ptr<Matrix> JAbr = triplet(triplet(Caocc_B, S, D_A, true, false, false), J_B, Cavir_A);
    JAbr->scale(-2.0);

    std::shared_ptr<Matrix> Jbs = triplet(Caocc_B, J_A, Cavir_B, true, false, false);
    Jbs->scale(4.0);
    std::shared_ptr<Matrix> Jar = triplet(Caocc_A, J_B, Cavir_A, true, false, false);
    Jar->scale(4.0);

    std::shared_ptr<Matrix> JAas = triplet(triplet(Caocc_A, J_B, D_A, true, false, false), S, Cavir_B);
    JAas->scale(-2.0);
    std::shared_ptr<Matrix> JBbr = triplet(triplet(Caocc_B, J_A, D_B, true, false, false), S, Cavir_A);
    JBbr->scale(-2.0);

    // Get your signs right Hesselmann!
    std::shared_ptr<Matrix> Vbs = triplet(Caocc_B, V_A, Cavir_B, true, false, false);
    Vbs->scale(2.0);
    std::shared_ptr<Matrix> Var = triplet(Caocc_A, V_B, Cavir_A, true, false, false);
    Var->scale(2.0);
    std::shared_ptr<Matrix> VBas = triplet(triplet(Caocc_A, S, D_B, true, false, false), V_A, Cavir_B);
    VBas->scale(-1.0);
    std::shared_ptr<Matrix> VAbr = triplet(triplet(Caocc_B, S, D_A, true, false, false), V_B, Cavir_A);
    VAbr->scale(-1.0);
    std::shared_ptr<Matrix> VRas = triplet(triplet(Caocc_A, V_B, P_A, true, false, false), S, Cavir_B);
    VRas->scale(1.0);
    std::shared_ptr<Matrix> VSbr = triplet(triplet(Caocc_B, V_A, P_B, true, false, false), S, Cavir_A);
    VSbr->scale(1.0);

    std::shared_ptr<Matrix> Sas = triplet(Caocc_A, S, Cavir_B, true, false, false);
    std::shared_ptr<Matrix> Sbr = triplet(Caocc_B, S, Cavir_A, true, false, false);
    ...
    std::shared_ptr<Matrix> SBar = triplet(triplet(Caocc_A, S, D_B, true, false, false), S, Cavir_A);
    std::shared_ptr<Matrix> SAbs = triplet(triplet(Caocc_B, S, D_A, true, false, false), S, Cavir_B);
...
void FISAPTSCF::compute_energy() {
    ...
    for (int iter = 1; iter <= maxiter; iter++) {
        ...
        std::shared_ptr<Matrix> D = doublet(Cocc2, Cocc2, false, true);
        ...
        std::shared_ptr<Matrix> G1 = triplet(F, D, S);
        std::shared_ptr<Matrix> G2 = triplet(S, D, F);
        G1->subtract(G2);
        std::shared_ptr<Matrix> G3 = triplet(X, G1, X, true, false, false);
        double Gnorm = G3->rms();
