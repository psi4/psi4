...
void FISAPT::partition() {
    ...
    std::shared_ptr<Matrix> QF = std::make_shared<Matrix>("QF", 3, na);
...
void FISAPT::overlap() {
    ...
    int nm = primary_->nbf();
    std::shared_ptr<IntegralFactory> Tfact = std::make_shared<IntegralFactory>(primary_);
...
void FISAPT::nuclear() {
    ...
    std::shared_ptr<Matrix> Zxyz = std::make_shared<Matrix>("Zxyz", nA, 4);
    double** Zxyzp = Zxyz->pointer();

    std::shared_ptr<IntegralFactory> Vfact = std::make_shared<IntegralFactory>(primary_);
    ...
    std::shared_ptr<Matrix> Zs = std::make_shared<Matrix>("Zs", nA, 3);
    double** Zsp = Zs->pointer();

    std::shared_ptr<Matrix> Rinv = std::make_shared<Matrix>("Rinv", nA, nA);
...
void FISAPT::unify() {
    ...
    std::shared_ptr<Matrix> P_A =
        Matrix::doublet(reference_->Ca_subset("AO", "ALL"), reference_->Ca_subset("AO", "ALL"), false, true);
    P_A->subtract(D_A);
...
void FISAPT::exch() {
    ...
    std::shared_ptr<Matrix> T = std::make_shared<Matrix>("T", na + nb, na + nb);
    T->identity();
    double** Tp = T->pointer();
    for (int a = 0; a < na; a++) {
        for (int b = 0; b < nb; b++) {
    ...
    std::shared_ptr<Matrix> C_T_A_n = std::make_shared<Matrix>("C_T_A_n", nbf, na);
    std::shared_ptr<Matrix> C_T_B_n = std::make_shared<Matrix>("C_T_A_n", nbf, nb);
    std::shared_ptr<Matrix> C_T_BA_n = std::make_shared<Matrix>("C_T_BA_n", nbf, nb);
    std::shared_ptr<Matrix> C_T_AB_n = std::make_shared<Matrix>("C_T_AB_n", nbf, na);
...
void FISAPT::ind() {
    ...
    std::shared_ptr<CPHF_FISAPT> cphf = std::make_shared<CPHF_FISAPT>();
...
void FISAPT::disp(std::map<std::string, SharedMatrix> matrix_cache, std::map<std::string, SharedVector> vector_cache,
    ...
    std::shared_ptr<Matrix> Aar = std::make_shared<Matrix>("Aar", max_a * nr, nQ);
    std::shared_ptr<Matrix> Abs = std::make_shared<Matrix>("Abs", max_b * ns, nQ);
    std::shared_ptr<Matrix> Bas = std::make_shared<Matrix>("Bas", max_a * ns, nQ);
    std::shared_ptr<Matrix> Bbr = std::make_shared<Matrix>("Bbr", max_b * nr, nQ);
    std::shared_ptr<Matrix> Cas = std::make_shared<Matrix>("Cas", max_a * ns, nQ);
    std::shared_ptr<Matrix> Cbr = std::make_shared<Matrix>("Cbr", max_b * nr, nQ);
    std::shared_ptr<Matrix> Dar = std::make_shared<Matrix>("Dar", max_a * nr, nQ);
    std::shared_ptr<Matrix> Dbs = std::make_shared<Matrix>("Dbs", max_b * ns, nQ);
...
void FISAPT::plot() {
    ...
    std::shared_ptr<CubicScalarGrid> csg = std::make_shared<CubicScalarGrid>(primary_, options_);
...
void FISAPT::felst() {
    ...
    std::shared_ptr<Matrix> QaC = std::make_shared<Matrix>("QaC", na, nQ);
    double** QaCp = QaC->pointer();
    for (size_t a = 0; a < na; a++) {
        dfh->fill_tensor("Aaa", QaCp[a], {a, a + 1}, {a, a + 1});
    }

    std::shared_ptr<Matrix> QbC = std::make_shared<Matrix>("QbC", nb, nQ);
    ...
    std::shared_ptr<Matrix> Zxyz2 = std::make_shared<Matrix>("Zxyz", 1, 4);
    double** Zxyz2p = Zxyz2->pointer();
    std::shared_ptr<IntegralFactory> Vfact2 = std::make_shared<IntegralFactory>(primary_);
    std::shared_ptr<PotentialInt> Vint2(static_cast<PotentialInt*>(Vfact2->ao_potential()));
    Vint2->set_charge_field(Zxyz2);
    std::shared_ptr<Matrix> Vtemp2 = std::make_shared<Matrix>("Vtemp2", nn, nn);
    ...
    for (int A = 0; A < nA; A++) {
        ...
        std::shared_ptr<Matrix> Vbb =
            Matrix::triplet(matrices_["Locc0B"], Vtemp2, matrices_["Locc0B"], true, false, false);
    ...
    for (int B = 0; B < nB; B++) {
        ...
        std::shared_ptr<Matrix> Vaa =
            Matrix::triplet(matrices_["Locc0A"], Vtemp2, matrices_["Locc0A"], true, false, false);
...
void FISAPT::fexch() {
    ...
    std::shared_ptr<Matrix> WBab = std::make_shared<Matrix>("WBab", na, nb);
    double** WBabp = WBab->pointer();
    std::shared_ptr<Matrix> WAba = std::make_shared<Matrix>("WAba", nb, na);
    ...
    std::shared_ptr<Matrix> E_exch1 = std::make_shared<Matrix>("E_exch [a <x- b]", na, nb);
    double** E_exch1p = E_exch1->pointer();
    std::shared_ptr<Matrix> E_exch2 = std::make_shared<Matrix>("E_exch [a -x> b]", na, nb);
    ...
    size_t nQ = dfh->get_naux();
    std::shared_ptr<Matrix> TrQ = std::make_shared<Matrix>("TrQ", nr, nQ);
    double** TrQp = TrQ->pointer();
    std::shared_ptr<Matrix> TsQ = std::make_shared<Matrix>("TsQ", ns, nQ);
    double** TsQp = TsQ->pointer();
    std::shared_ptr<Matrix> TbQ = std::make_shared<Matrix>("TbQ", nb, nQ);
    double** TbQp = TbQ->pointer();
    std::shared_ptr<Matrix> TaQ = std::make_shared<Matrix>("TaQ", na, nQ);
    ...
    std::shared_ptr<Matrix> E_exch3 = std::make_shared<Matrix>("E_exch [a <x-x> b]", na, nb);
...
void FISAPT::find() {
    ...
    std::shared_ptr<Matrix> TsQ = std::make_shared<Matrix>("TsQ", ns, nQ);
    std::shared_ptr<Matrix> T1As = std::make_shared<Matrix>("T1As", na, ns);
    double** TsQp = TsQ->pointer();
    ...
    std::shared_ptr<Matrix> TrQ = std::make_shared<Matrix>("TrQ", nr, nQ);
    std::shared_ptr<Matrix> T1Br = std::make_shared<Matrix>("T1Br", nb, nr);
    double** TrQp = TrQ->pointer();
    ...
    std::shared_ptr<Matrix> xA = std::make_shared<Matrix>("xA", na, nr);
    std::shared_ptr<Matrix> xB = std::make_shared<Matrix>("xB", nb, ns);
    double** xAp = xA->pointer();
    double** xBp = xB->pointer();

    std::shared_ptr<Matrix> wB = std::make_shared<Matrix>("wB", na, nr);
    std::shared_ptr<Matrix> wA = std::make_shared<Matrix>("wA", nb, ns);
    double** wBp = wB->pointer();
    double** wAp = wA->pointer();
    ...
    std::shared_ptr<Matrix> Ind20u_AB_terms = std::make_shared<Matrix>("Ind20 [A<-B] (a x B)", na, nB + nb);
    std::shared_ptr<Matrix> Ind20u_BA_terms = std::make_shared<Matrix>("Ind20 [B<-A] (A x b)", nA + na, nb);
    ...
    std::shared_ptr<Matrix> ExchInd20u_AB_terms = std::make_shared<Matrix>("ExchInd20 [A<-B] (a x B)", na, nB + nb);
    std::shared_ptr<Matrix> ExchInd20u_BA_terms = std::make_shared<Matrix>("ExchInd20 [B<-A] (A x b)", nA + na, nb);
    ...
    std::shared_ptr<Matrix> Indu_AB_terms = std::make_shared<Matrix>("Ind [A<-B] (a x B)", na, nB + nb);
    std::shared_ptr<Matrix> Indu_BA_terms = std::make_shared<Matrix>("Ind [B<-A] (A x b)", nA + na, nb);
    ...
    std::shared_ptr<Matrix> sIndu_AB_terms = std::make_shared<Matrix>("sInd [A<-B] (a x B)", sna, snB + snb);
    std::shared_ptr<Matrix> sIndu_BA_terms = std::make_shared<Matrix>("sInd [B<-A] (A x b)", snA + sna, snb);
    ...
    if (ind_resp) {
        ...
        std::shared_ptr<Matrix> Ind20r_AB_terms = std::make_shared<Matrix>("Ind20 [A<-B] (a x B)", na, nB + nb);
        std::shared_ptr<Matrix> Ind20r_BA_terms = std::make_shared<Matrix>("Ind20 [B<-A] (A x b)", nA + na, nb);
        ...
        std::shared_ptr<Matrix> ExchInd20r_AB_terms = std::make_shared<Matrix>("ExchInd20 [A<-B] (a x B)", na, nB + nb);
        std::shared_ptr<Matrix> ExchInd20r_BA_terms = std::make_shared<Matrix>("ExchInd20 [B<-A] (A x b)", nA + na, nb);
        ...
        std::shared_ptr<Matrix> Indr_AB_terms = std::make_shared<Matrix>("Ind [A<-B] (a x B)", na, nB + nb);
        std::shared_ptr<Matrix> Indr_BA_terms = std::make_shared<Matrix>("Ind [B<-A] (A x b)", nA + na, nb);
...
void FISAPT::fdisp() {
    ...
    std::shared_ptr<Matrix> Aar = std::make_shared<Matrix>("Aar", max_r * na, nQ);
    std::shared_ptr<Matrix> Abs = std::make_shared<Matrix>("Abs", max_s * nb, nQ);
    std::shared_ptr<Matrix> Bas = std::make_shared<Matrix>("Bas", max_s * na, nQ);
    std::shared_ptr<Matrix> Bbr = std::make_shared<Matrix>("Bbr", max_r * nb, nQ);
    std::shared_ptr<Matrix> Cas = std::make_shared<Matrix>("Cas", max_s * na, nQ);
    std::shared_ptr<Matrix> Cbr = std::make_shared<Matrix>("Cbr", max_r * nb, nQ);
    std::shared_ptr<Matrix> Dar = std::make_shared<Matrix>("Dar", max_r * na, nQ);
    std::shared_ptr<Matrix> Dbs = std::make_shared<Matrix>("Dbs", max_s * nb, nQ);
...
#pragma omp parallel for schedule(dynamic) reduction(+ : Disp20, ExchDisp20, sExchDisp20)
    ...
    std::shared_ptr<Matrix> E_disp20 = std::make_shared<Matrix>("E_disp20", na, nb);
    std::shared_ptr<Matrix> E_exch_disp20 = std::make_shared<Matrix>("E_exch_disp20", na, nb);
    ...
    if (options_.get_bool("sSAPT0_SCALE")) {
        std::shared_ptr<Matrix> sE_exch_disp20 = std::make_shared<Matrix>("sE_exch_disp20", na, nb);
...
std::shared_ptr<Matrix> FISAPT::extract_columns(const std::vector<int>& cols, std::shared_ptr<Matrix> A) {
    ...
    std::shared_ptr<Matrix> A2 = std::make_shared<Matrix>("A2", nm, ni);
    double** Ap = A->pointer();
...
void FISAPTSCF::compute_energy() {
    ...
    bool diised = false;
    std::shared_ptr<Matrix> Gsize = std::make_shared<Matrix>("Gsize", nmo, nmo);
    std::shared_ptr<DIISManager> diis = std::make_shared<DIISManager>(max_diis_vectors, "FISAPT DIIS");
    ...
    for (int iter = 1; iter <= maxiter; iter++) {
        ...
        std::shared_ptr<Matrix> U2 = std::make_shared<Matrix>("C", nmo, nmo);
        std::shared_ptr<Vector> e2 = std::make_shared<Vector>("eps", nmo);
    ...
    std::shared_ptr<Vector> eps_occ = std::make_shared<Vector>("eps_occ", nocc);
    std::shared_ptr<Vector> eps_vir = std::make_shared<Vector>("eps_vir", nvir);
    ...
    std::shared_ptr<Matrix> Cocc = std::make_shared<Matrix>("Cocc", nbf, nocc);
    std::shared_ptr<Matrix> Cvir = std::make_shared<Matrix>("Cvir", nbf, nvir);
...
std::map<std::string, std::shared_ptr<Matrix> > CPHF_FISAPT::product(
    ...
    if (do_A) {
        ...
        int nv = b["A"]->ncol();
        int nso = Cvir_A_->nrow();
        std::shared_ptr<Matrix> T = std::make_shared<Matrix>("T", no, nso);
