...
void IBOLocalizer2::build_iaos() {
    ...
    std::shared_ptr<Matrix> T1 = doublet(S22_m12, S12, false, true);
    std::shared_ptr<Matrix> T2 = doublet(S11_m12, triplet(T1, T1, C, true, false, false), false, false);
    std::shared_ptr<Matrix> T3 = doublet(T2, T2, true, false);
    T3->power(-1.0 / 2.0, condition_);
    std::shared_ptr<Matrix> Ctilde = triplet(S11_m12, T2, T3, false, false, false);

    // => D and Tilde D <= //

    std::shared_ptr<Matrix> D = doublet(C, C, false, true);
    std::shared_ptr<Matrix> Dtilde = doublet(Ctilde, Ctilde, false, true);

    // => A (Before Orthogonalization) <= //

    std::shared_ptr<Matrix> DSDtilde = triplet(D, S11, Dtilde, false, false, false);
    DSDtilde->scale(2.0);

    std::shared_ptr<Matrix> L = doublet(S11_m12, S11_m12, false, false);  // TODO: Possibly Unstable
    L->add(DSDtilde);
    L->subtract(D);
    L->subtract(Dtilde);

    std::shared_ptr<Matrix> AN = doublet(L, S12, false, false);

    // => A (After Orthogonalization) <= //

    std::shared_ptr<Matrix> V = triplet(AN, S11, AN, true, false, false);
    V->power(-1.0 / 2.0, condition_);

    std::shared_ptr<Matrix> A = doublet(AN, V, false, false);
...
std::map<std::string, std::shared_ptr<Matrix> > IBOLocalizer2::localize(std::shared_ptr<Matrix> Cocc,
    ...
    std::shared_ptr<Matrix> L = triplet(Cocc, S_, A_, true, false, false);
    L->set_name("L");
    ...
    std::shared_ptr<Matrix> Focc2 = triplet(U, Focc, U, true, false, false);
    std::shared_ptr<Matrix> U2 = IBOLocalizer2::reorder_orbitals(Focc2, ranges);

    std::shared_ptr<Matrix> Uocc3 = doublet(U, U2, false, false);
    std::shared_ptr<Matrix> Focc3 = triplet(Uocc3, Focc, Uocc3, true, false, false);
    std::shared_ptr<Matrix> Locc3 = doublet(Cocc, Uocc3, false, false);

    L = doublet(U2, L, true, false);
    std::shared_ptr<Matrix> Q = orbital_charges(L);
...
void IBOLocalizer2::print_charges(double scale) {
    ...
    std::shared_ptr<Matrix> L = triplet(C_, S_, A_, true, false, false);
