...
std::string MOInfo::get_determinant_label(int i) { return references[i].get_label(); }
...
void MOInfo::build_model_space() {
  ...
  ***********************************************/
    ...
    ********************************************************/
        ...
        if (alpha_combinations.size() == 0) alpha_combinations.push_back(std::vector<int>(0));
...
std::vector<int> MOInfo::get_determinant(int i) {
    ...
    for (int p = 0; p < 2 * nall; ++p)
        if (references[i].test(p)) occupation[p] = 1;
    return occupation;
}
...
double MOInfo::get_sign_internal_excitation(int i, int j) { return (sign_internal_excitations[i][j]); }
