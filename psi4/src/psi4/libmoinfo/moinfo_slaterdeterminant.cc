...
bool MOInfo::SlaterDeterminant::is_closed_shell() {
    for (int i = 0; i < moinfo->get_nall(); i++)
        if (bits[i] != bits[i + moinfo->get_nall()]) return (false);
    return (true);
}
...
bool MOInfo::SlaterDeterminant::is_spin_flipped(SlaterDeterminant& det) {
    for (int i = 0; i < moinfo->get_nall(); i++) {
        if (bits[i] != det.test(i + moinfo->get_nall())) return (false);
        if (bits[i + moinfo->get_nall()] != det.test(i)) return (false);
    }
    return (true);
}
...
std::vector<int> MOInfo::SlaterDeterminant::get_aocc() {
    std::vector<int> aocc;
    for (int i = 0; i < moinfo->get_nall(); i++)
        if (bits[i]) aocc.push_back(moinfo->get_all_to_occ(i));
    return (aocc);
}
...
std::vector<int> MOInfo::SlaterDeterminant::get_bocc() {
    std::vector<int> bocc;
    for (int i = 0; i < moinfo->get_nall(); i++)
        if (bits[i + moinfo->get_nall()]) bocc.push_back(moinfo->get_all_to_occ(i));
    return (bocc);
}
...
std::vector<int> MOInfo::SlaterDeterminant::get_avir() {
    std::vector<int> avir;
    for (int i = 0; i < moinfo->get_nall(); i++)
        if (!bits[i]) avir.push_back(moinfo->get_all_to_vir(i));
    return (avir);
}
...
std::vector<int> MOInfo::SlaterDeterminant::get_bvir() {
    std::vector<int> bvir;
    for (int i = 0; i < moinfo->get_nall(); i++)
        if (!bits[i + moinfo->get_nall()]) bvir.push_back(moinfo->get_all_to_vir(i));
    return (bvir);
}
