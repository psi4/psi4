    ...
    ********************************************************/
    ...
    for (int i = 0; i < nocc; ++i) {
        for (int a = 0; a < nvir; ++a)
            if (occ_to_mo[i] == vir_to_mo[a]) occ_to_vir[i] = a;
    }
    ...
    for (int p = 0; p < nall; ++p) {
        for (int i = 0; i < nocc; ++i)
            if (all_to_mo[p] == occ_to_mo[i]) all_to_occ[p] = i;
        for (int a = 0; a < nvir; ++a)
            if (all_to_mo[p] == vir_to_mo[a]) all_to_vir[p] = a;
    }
    ...
    for (int u = 0; u < nactv; ++u) {
        for (int i = 0; i < nocc; ++i)
            if (actv_to_mo[u] == occ_to_mo[i]) actv_to_occ[u] = i;
        for (int a = 0; a < nvir; ++a)
            if (actv_to_mo[u] == vir_to_mo[a]) actv_to_vir[u] = a;
    }
    ...
    for (int i = 0; i < nocc; ++i) {
        for (int u = 0; u < nactv; ++u)
            if (occ_to_mo[i] == actv_to_mo[u]) occ_to_actv[i] = u;
    }
    for (int a = 0; a < nvir; ++a) {
        for (int u = 0; u < nactv; ++u)
            if (vir_to_mo[a] == actv_to_mo[u]) vir_to_actv[a] = u;
    }
