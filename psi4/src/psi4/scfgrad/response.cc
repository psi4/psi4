...
std::shared_ptr<Matrix> RSCFDeriv::hessian_response()
{
    ...
    // => Spi <= //
    {
        ...
        for (int A = 0; A < 3*natom; A++) {
            psio_->write(PSIF_HESS,"Smi^A",(char*)Smixp[0],(unsigned long) nso * nocc * sizeof(double),next_Smi,&next_Smi);
        }
        for (int A = 0; A < 3*natom; A++) {
            psio_->write(PSIF_HESS,"Sai^A",(char*)Saip[0],(unsigned long) nvir * nocc * sizeof(double),next_Sai,&next_Sai);
        }
        for (int A = 0; A < 3*natom; A++) {
            psio_->write(PSIF_HESS,"Sij^A",(char*)Sijp[0],(unsigned long) nocc * nocc * sizeof(double),next_Sij,&next_Sij);
        }
        for (int A = 0; A < 3*natom; A++) {
            psio_->write(PSIF_HESS,"Spi^A",(char*)Spip[0],(unsigned long) nmo * nocc * sizeof(double),next_Spi,&next_Spi);
        }
        next_Smi = PSIO_ZERO;
        next_Sai = PSIO_ZERO;
        next_Sij = PSIO_ZERO;
        ...
        for (int A = 0; A < natom; A++) {
            ...
            psio_->write(PSIF_HESS,"Smi^A",(char*)Smixp[0], (unsigned long) nso * nocc * sizeof(double),next_Smi,&next_Smi);
            // Smi_y
            psio_->write(PSIF_HESS,"Smi^A",(char*)Smiyp[0], (unsigned long) nso * nocc * sizeof(double),next_Smi,&next_Smi);
            // Smi_z
            psio_->write(PSIF_HESS,"Smi^A",(char*)Smizp[0], (unsigned long) nso * nocc * sizeof(double),next_Smi,&next_Smi);
    ...
    // => Tpi <= //
    {
        ...
        for (int A = 0; A < natom; A++) {
            ...
            psio_->write(PSIF_HESS,"Tpi^A",(char*)Tpip[0], (unsigned long) nmo * nocc * sizeof(double),next_Tpi,&next_Tpi);
            // Tpi_y
    ...
    // => Vpi <= //
    {
        ...
        for (int A = 0; A < natom; A++) {
            ...
            psio_->write(PSIF_HESS,"Vpi^A",(char*)Vpip[0], (unsigned long) nmo * nocc * sizeof(double),next_Vpi,&next_Vpi);
            // Vpi_y
    ...
    // => Jpi/Kpi <= //
    {
        ...
        for(int pert = 0; pert < 3*natom; ++pert){
            psio_->write(PSIF_HESS,"Gpi^A",(char*)pGpi[0], (unsigned long) nmo * nocc * sizeof(double),next_Gpi,&next_Gpi);
        }
        next_Gpi = PSIO_ZERO;
...
#pragma omp parallel for
            ...
            for (int A = 0; A < 3 * natom; A+=max_a) {
                ...
                for (int P = 0; P < nauxshell; ++P){
                    ...
                    for(int Q = 0; Q < nauxshell; ++Q){
                        ...
                        size_t stride = (size_t) Pncart * Qncart;
                ...
                for (int P = 0; P < nauxshell; ++P){
                    ...
                    for(int M = 0; M < nshell; ++M){
                        ...
                        for(int N = 0; N < nshell; ++N){
                            ...
                            size_t stride = (size_t) Pncart * Mncart * Nncart;
            ...
            for (int A = 0; A < 3 * natom; A+=max_a) {
                ...
                for (size_t index = 0L; index < npairs2; index++) {
                    ...
                    size_t stride = (size_t) Pncart * Qncart * Rncart * Sncart;
    ...
    // => J2pi/K2pi <= //
    {
        ...
        psio_address next_Gpi = PSIO_ZERO;
        for (int A = 0; A < 3 * natom; A++)
            psio_->write(PSIF_HESS,"G2pi^A",(char*)Up[0], (unsigned long) nmo*nocc*sizeof(double),next_Gpi,&next_Gpi);
        ...
        for (int A = 0; A < 3 * natom; A+=max_A) {
            ...
            for (int a = 0; a < nA; a++) {
                ...
                psio_->read(PSIF_HESS,"Sij^A",(char*)Sijp[0], (unsigned long) nocc*nocc*sizeof(double),next_Sij, &next_Sij);
    ...
    // => Fpi <= //
    {
        ...
        for (int A = 0; A < 3*natom; A++) {
            psio_->read(PSIF_HESS,"Tpi^A",(char*)Fpip[0], (unsigned long) nmo * nocc * sizeof(double),next_Tpi,&next_Tpi);
            psio_->read(PSIF_HESS,"Vpi^A",(char*)Tpip[0], (unsigned long) nmo * nocc * sizeof(double),next_Vpi,&next_Vpi);
            Fpi->add(Tpi);
            psio_->read(PSIF_HESS,"Gpi^A",(char*)Tpip[0], (unsigned long) nmo * nocc * sizeof(double),next_Jpi,&next_Jpi);
            Fpi->add(Tpi);
            psio_->write(PSIF_HESS,"Fpi^A",(char*)Fpip[0], (unsigned long) nmo * nocc * sizeof(double),next_Fpi,&next_Fpi);
        }
    }
    ...
    // => Bpi <= //
    {
        ...
        for (int A = 0; A < 3*natom; A++) {
            ...
            psio_->read(PSIF_HESS,"Fpi^A",(char*)Baip[0], (unsigned long) nvir * nocc * sizeof(double),next_Fpi,&next_Fpi);
            next_Spi = psio_get_address(PSIO_ZERO,sizeof(double)*(A * (size_t) nmo * nocc + nocc * nocc));
            psio_->read(PSIF_HESS,"Spi^A",(char*)Taip[0], (unsigned long) nvir * nocc * sizeof(double),next_Spi,&next_Spi);
            ...
            psio_->read(PSIF_HESS,"G2pi^A",(char*)Taip[0], (unsigned long) nvir * nocc * sizeof(double),next_G2pi,&next_G2pi);
            Bai->add(Tai);
            Bai->scale(-1.0);
            psio_->write(PSIF_HESS,"Bai^A",(char*)Baip[0], (unsigned long) nvir * nocc * sizeof(double),next_Bai,&next_Bai);
        }
    }
    ...
    // => CPHF (Uai) <= //
    {
        ...
        for (int A = 0; A < 3 * natom; A+=max_A) {
            ...
            for (int a = 0; a < nA; a++) {
                ...
                psio_->read(PSIF_HESS,"Bai^A",(char*)Tp[0], (unsigned long) nvir * nocc * sizeof(double),next_Bai,&next_Bai);
                double** Bp = B->pointer();
                for (int i = 0; i < nocc; i++) {
            ...
            for (int a = 0; a < nA; a++) {
                ...
                psio_->write(PSIF_HESS,"Uai^A",(char*)Tp[0], (unsigned long) nvir * nocc * sizeof(double),next_Uai,&next_Uai);
            }
    ...
    // => Upi <= //
    {
        ...
        for (int A = 0; A < 3*natom; A++) {
            psio_->read(PSIF_HESS,"Sij^A",(char*)Upqp[0], (unsigned long) nocc * nocc * sizeof(double),next_Spi,&next_Spi);
            C_DSCAL(nocc * (size_t) nocc,-0.5, Upqp[0], 1);
            psio_->read(PSIF_HESS,"Uai^A",(char*)Upqp[nocc], (unsigned long) nvir * nocc * sizeof(double),next_Uai,&next_Uai);
            psio_->write(PSIF_HESS,"Upi^A",(char*)Upqp[0], (unsigned long) nmo * nocc * sizeof(double),next_Upi,&next_Upi);
    ...
    // => Qpi <= //
    {
        ...
        for (int A = 0; A < 3 * natom; A+=max_A) {
            ...
            for (int a = 0; a < nA; a++) {
                ...
                psio_->read(PSIF_HESS,"Upi^A",(char*)Upip[0], (unsigned long) nmo*nocc*sizeof(double),next_Upi,&next_Upi);
            ...
            for (int a = 0; a < nA; a++) {
                ...
                psio_->write(PSIF_HESS,"Qpi^A",(char*)Up[0], (unsigned long) nmo*nocc*sizeof(double),next_Qpi,&next_Qpi);
            }
        }
    }
    jk.reset();
    ...
    // => Zipper <= //
    {
        ...
        for (int A = 0; A < 3 * natom; A+=max_a) {
            ...
            L->add(T);
            for (int i = 0; i < nocc; i++)
                C_DSCAL((size_t) nA*nmo,eop[i],&Lp[0][i],nocc);
            for (int B = 0; B < 3 * natom; B+=max_a) {
                int nB = (B + max_a >= 3 * natom ? 3 * natom - B : max_a);
                psio_address nextB = psio_get_address(PSIO_ZERO, B * npi * sizeof(double));
