...
std::shared_ptr<Matrix> USCFDeriv::hessian_response()
{
    ...
    std::shared_ptr<Vector> eps_a_occ = epsilon_a_subset("AO","OCC");
    std::shared_ptr<Vector> eps_b_occ = epsilon_b_subset("AO","OCC");
    std::shared_ptr<Vector> eps_a_vir = epsilon_a_subset("AO","VIR");
    std::shared_ptr<Vector> eps_b_vir = epsilon_b_subset("AO","VIR");
    ...
    double** Caop = Ca_occ->pointer();
    double** Cbop = Cb_occ->pointer();
    double** Cavp = Ca_vir->pointer();
    double** Cbvp = Cb_vir->pointer();
    double*  ea_p  = eps_a->pointer();
    double*  eb_p  = eps_b->pointer();
    double*  eaop = eps_a_occ->pointer();
    double*  ebop = eps_b_occ->pointer();
    double*  eavp = eps_a_vir->pointer();
    double*  ebvp = eps_b_vir->pointer();

    // => Sizing <= //

    int natom = molecule_->natom();
    int nso   = basisset_->nbf();
    int naocc  = eps_a_occ->dimpi()[0];
    int nbocc  = eps_b_occ->dimpi()[0];
    int navir  = eps_a_vir->dimpi()[0];
    int nbvir  = eps_b_vir->dimpi()[0];
    int nmo   = Ca->colspi()[0];

    // => Target <= //

    std::shared_ptr<Matrix> response(new Matrix("UHF Response",3*natom,3*natom));
    ...
    // => Spi <= //
    {
        ...
        // Do all alpha
        { 
            std::shared_ptr<Matrix> Smix(new Matrix("Smix",nso,naocc));
            std::shared_ptr<Matrix> Smiy(new Matrix("Smiy",nso,naocc));
            std::shared_ptr<Matrix> Smiz(new Matrix("Smiz",nso,naocc));
            double** Smixp = Smix->pointer();
            double** Smiyp = Smiy->pointer();
            double** Smizp = Smiz->pointer();

            std::shared_ptr<Matrix> Sai(new Matrix("Sai",navir,naocc));
            double** Saip = Sai->pointer();
            std::shared_ptr<Matrix> Sij(new Matrix("Sij",naocc,naocc));
            double** Sijp = Sij->pointer();
            std::shared_ptr<Matrix> Spi(new Matrix("Spi",nmo,naocc));
            double** Spip = Spi->pointer();
            ...
            psio_address next_Spi = PSIO_ZERO;
            for (int A = 0; A < 3*natom; A++) {
                psio_->write(PSIF_HESS,"Smia^A",(char*)Smixp[0],nso * naocc * sizeof(double),next_Smi,&next_Smi);
            }
            for (int A = 0; A < 3*natom; A++) {
                psio_->write(PSIF_HESS,"Saia^A",(char*)Saip[0],navir * naocc * sizeof(double),next_Sai,&next_Sai);
            }
            for (int A = 0; A < 3*natom; A++) {
                psio_->write(PSIF_HESS,"Sija^A",(char*)Sijp[0],naocc * naocc * sizeof(double),next_Sij,&next_Sij);
            }
            for (int A = 0; A < 3*natom; A++) {
                psio_->write(PSIF_HESS,"Spia^A",(char*)Spip[0],nmo * naocc * sizeof(double),next_Spi,&next_Spi);
            }
            next_Smi = PSIO_ZERO;
            next_Sai = PSIO_ZERO;
            next_Sij = PSIO_ZERO;
            ...
            for (int A = 0; A < natom; A++) {
                ...
                for (int P = 0; P < basisset_->nshell(); P++) {
                    for (int Q = 0; Q < basisset_->nshell(); Q++) {
                        ...
                        if (aP == A) {
                            ...
                            for (int p = 0; p < nP; p++) {
                                for (int q = 0; q < nQ; q++) {
                                    C_DAXPY(naocc,(*buffer2),Caop[q + oQ],1,Smixp[p + oP],1);
                                    C_DAXPY(naocc,(*buffer2++), Caop[p + oP],1,Smixp[q + oQ],1);
                                }
                            }
                            // Py
                            buffer2 = buffer + 1 * nP * nQ;
                            for (int p = 0; p < nP; p++) {
                                for (int q = 0; q < nQ; q++) {
                                    C_DAXPY(naocc,(*buffer2),Caop[q + oQ],1,Smiyp[p + oP],1);
                                    C_DAXPY(naocc,(*buffer2++), Caop[p + oP],1,Smiyp[q + oQ],1);
                                }
                            }
                            // Pz
                            buffer2 = buffer + 2 * nP * nQ;
                            for (int p = 0; p < nP; p++) {
                                for (int q = 0; q < nQ; q++) {
                                    C_DAXPY(naocc,(*buffer2),Caop[q + oQ],1,Smizp[p + oP],1);
                                    C_DAXPY(naocc,(*buffer2++), Caop[p + oP],1,Smizp[q + oQ],1);
                                }
                            }
                        }
                        if (aQ == A) {
                ...
                psio_->write(PSIF_HESS,"Smia^A",(char*)Smixp[0],nso * naocc * sizeof(double),next_Smi,&next_Smi);
                // Smi_y
                psio_->write(PSIF_HESS,"Smia^A",(char*)Smiyp[0],nso * naocc * sizeof(double),next_Smi,&next_Smi);
                // Smi_z
                psio_->write(PSIF_HESS,"Smia^A",(char*)Smizp[0],nso * naocc * sizeof(double),next_Smi,&next_Smi);

                // Sai_x
                C_DGEMM('T','N',navir,naocc,nso,0.5,Cavp[0],navir,Smixp[0],naocc,0.0,Saip[0],naocc);
                psio_->write(PSIF_HESS,"Saia^A",(char*)Saip[0],navir * naocc * sizeof(double),next_Sai,&next_Sai);
                // Sai_y
                C_DGEMM('T','N',navir,naocc,nso,0.5,Cavp[0],navir,Smiyp[0],naocc,0.0,Saip[0],naocc);
                psio_->write(PSIF_HESS,"Saia^A",(char*)Saip[0],navir * naocc * sizeof(double),next_Sai,&next_Sai);
                // Sai_z
                C_DGEMM('T','N',navir,naocc,nso,0.5,Cavp[0],navir,Smizp[0],naocc,0.0,Saip[0],naocc);
                psio_->write(PSIF_HESS,"Saia^A",(char*)Saip[0],navir * naocc * sizeof(double),next_Sai,&next_Sai);

                // Sij_x
                C_DGEMM('T','N',naocc,naocc,nso,0.5,Caop[0],naocc,Smixp[0],naocc,0.0,Sijp[0],naocc);
                psio_->write(PSIF_HESS,"Sija^A",(char*)Sijp[0],naocc * naocc * sizeof(double),next_Sij,&next_Sij);
                // Sij_y
                C_DGEMM('T','N',naocc,naocc,nso,0.5,Caop[0],naocc,Smiyp[0],naocc,0.0,Sijp[0],naocc);
                psio_->write(PSIF_HESS,"Sija^A",(char*)Sijp[0],naocc * naocc * sizeof(double),next_Sij,&next_Sij);
                // Sij_z
                C_DGEMM('T','N',naocc,naocc,nso,0.5,Caop[0],naocc,Smizp[0],naocc,0.0,Sijp[0],naocc);
                psio_->write(PSIF_HESS,"Sija^A",(char*)Sijp[0],naocc * naocc * sizeof(double),next_Sij,&next_Sij);

                // Spi_x
                C_DGEMM('T','N',nmo,naocc,nso,0.5,Cap[0],nmo,Smixp[0],naocc,0.0,Spip[0],naocc);
                psio_->write(PSIF_HESS,"Spia^A",(char*)Spip[0],nmo * naocc * sizeof(double),next_Spi,&next_Spi);
                // Spi_y
                C_DGEMM('T','N',nmo,naocc,nso,0.5,Cap[0],nmo,Smiyp[0],naocc,0.0,Spip[0],naocc);
                psio_->write(PSIF_HESS,"Spia^A",(char*)Spip[0],nmo * naocc * sizeof(double),next_Spi,&next_Spi);
                // Spi_z
                C_DGEMM('T','N',nmo,naocc,nso,0.5,Cap[0],nmo,Smizp[0],naocc,0.0,Spip[0],naocc);
                psio_->write(PSIF_HESS,"Spia^A",(char*)Spip[0],nmo * naocc * sizeof(double),next_Spi,&next_Spi);
            }
        }
        // Then all beta
        { 
            std::shared_ptr<Matrix> Smix(new Matrix("Smix",nso,nbocc));
            std::shared_ptr<Matrix> Smiy(new Matrix("Smiy",nso,nbocc));
            std::shared_ptr<Matrix> Smiz(new Matrix("Smiz",nso,nbocc));
            double** Smixp = Smix->pointer();
            double** Smiyp = Smiy->pointer();
            double** Smizp = Smiz->pointer();

            std::shared_ptr<Matrix> Sai(new Matrix("Sai",nbvir,nbocc));
            double** Saip = Sai->pointer();
            std::shared_ptr<Matrix> Sij(new Matrix("Sij",nbocc,nbocc));
            double** Sijp = Sij->pointer();
            std::shared_ptr<Matrix> Spi(new Matrix("Spi",nmo,nbocc));
            double** Spip = Spi->pointer();
            ...
            psio_address next_Spi = PSIO_ZERO;
            for (int A = 0; A < 3*natom; A++) {
                psio_->write(PSIF_HESS,"Smib^A",(char*)Smixp[0],nso * nbocc * sizeof(double),next_Smi,&next_Smi);
            }
            for (int A = 0; A < 3*natom; A++) {
                psio_->write(PSIF_HESS,"Saib^A",(char*)Saip[0],nbvir * nbocc * sizeof(double),next_Sai,&next_Sai);
            }
            for (int A = 0; A < 3*natom; A++) {
                psio_->write(PSIF_HESS,"Sijb^A",(char*)Sijp[0],nbocc * nbocc * sizeof(double),next_Sij,&next_Sij);
            }
            for (int A = 0; A < 3*natom; A++) {
                psio_->write(PSIF_HESS,"Spib^A",(char*)Spip[0],nmo * nbocc * sizeof(double),next_Spi,&next_Spi);
            }
            next_Smi = PSIO_ZERO;
            next_Sai = PSIO_ZERO;
            next_Sij = PSIO_ZERO;
            ...
            for (int A = 0; A < natom; A++) {
                ...
                for (int P = 0; P < basisset_->nshell(); P++) {
                    for (int Q = 0; Q < basisset_->nshell(); Q++) {
                        ...
                        if (aP == A) {
                            ...
                            for (int p = 0; p < nP; p++) {
                                for (int q = 0; q < nQ; q++) {
                                    C_DAXPY(nbocc,(*buffer2),Cbop[q + oQ],1,Smixp[p + oP],1);
                                    C_DAXPY(nbocc,(*buffer2++), Cbop[p + oP],1,Smixp[q + oQ],1);
                                }
                            }
                            // Py
                            buffer2 = buffer + 1 * nP * nQ;
                            for (int p = 0; p < nP; p++) {
                                for (int q = 0; q < nQ; q++) {
                                    C_DAXPY(nbocc,(*buffer2),Cbop[q + oQ],1,Smiyp[p + oP],1);
                                    C_DAXPY(nbocc,(*buffer2++), Cbop[p + oP],1,Smiyp[q + oQ],1);
                                }
                            }
                            // Pz
                            buffer2 = buffer + 2 * nP * nQ;
                            for (int p = 0; p < nP; p++) {
                                for (int q = 0; q < nQ; q++) {
                                    C_DAXPY(nbocc,(*buffer2),Cbop[q + oQ],1,Smizp[p + oP],1);
                                    C_DAXPY(nbocc,(*buffer2++), Cbop[p + oP],1,Smizp[q + oQ],1);
                                }
                            }
                        }
                        if (aQ == A) {
                ...
                psio_->write(PSIF_HESS,"Smib^A",(char*)Smixp[0],nso * nbocc * sizeof(double),next_Smi,&next_Smi);
                // Smi_y
                psio_->write(PSIF_HESS,"Smib^A",(char*)Smiyp[0],nso * nbocc * sizeof(double),next_Smi,&next_Smi);
                // Smi_z
                psio_->write(PSIF_HESS,"Smib^A",(char*)Smizp[0],nso * nbocc * sizeof(double),next_Smi,&next_Smi);

                // Sai_x
                C_DGEMM('T','N',nbvir,nbocc,nso,0.5,Cbvp[0],nbvir,Smixp[0],nbocc,0.0,Saip[0],nbocc);
                psio_->write(PSIF_HESS,"Saib^A",(char*)Saip[0],nbvir * nbocc * sizeof(double),next_Sai,&next_Sai);
                // Sai_y
                C_DGEMM('T','N',nbvir,nbocc,nso,0.5,Cbvp[0],nbvir,Smiyp[0],nbocc,0.0,Saip[0],nbocc);
                psio_->write(PSIF_HESS,"Saib^A",(char*)Saip[0],nbvir * nbocc * sizeof(double),next_Sai,&next_Sai);
                // Sai_z
                C_DGEMM('T','N',nbvir,nbocc,nso,0.5,Cbvp[0],nbvir,Smizp[0],nbocc,0.0,Saip[0],nbocc);
                psio_->write(PSIF_HESS,"Saib^A",(char*)Saip[0],nbvir * nbocc * sizeof(double),next_Sai,&next_Sai);

                // Sij_x
                C_DGEMM('T','N',nbocc,nbocc,nso,0.5,Cbop[0],nbocc,Smixp[0],nbocc,0.0,Sijp[0],nbocc);
                psio_->write(PSIF_HESS,"Sijb^A",(char*)Sijp[0],nbocc * nbocc * sizeof(double),next_Sij,&next_Sij);
                // Sij_y
                C_DGEMM('T','N',nbocc,nbocc,nso,0.5,Cbop[0],nbocc,Smiyp[0],nbocc,0.0,Sijp[0],nbocc);
                psio_->write(PSIF_HESS,"Sijb^A",(char*)Sijp[0],nbocc * nbocc * sizeof(double),next_Sij,&next_Sij);
                // Sij_z
                C_DGEMM('T','N',nbocc,nbocc,nso,0.5,Cbop[0],nbocc,Smizp[0],nbocc,0.0,Sijp[0],nbocc);
                psio_->write(PSIF_HESS,"Sijb^A",(char*)Sijp[0],nbocc * nbocc * sizeof(double),next_Sij,&next_Sij);

                // Spi_x
                C_DGEMM('T','N',nmo,nbocc,nso,0.5,Cbp[0],nmo,Smixp[0],nbocc,0.0,Spip[0],nbocc);
                psio_->write(PSIF_HESS,"Spib^A",(char*)Spip[0],nmo * nbocc * sizeof(double),next_Spi,&next_Spi);
                // Spi_y
                C_DGEMM('T','N',nmo,nbocc,nso,0.5,Cbp[0],nmo,Smiyp[0],nbocc,0.0,Spip[0],nbocc);
                psio_->write(PSIF_HESS,"Spib^A",(char*)Spip[0],nmo * nbocc * sizeof(double),next_Spi,&next_Spi);
                // Spi_z
                C_DGEMM('T','N',nmo,nbocc,nso,0.5,Cbp[0],nmo,Smizp[0],nbocc,0.0,Spip[0],nbocc);
                psio_->write(PSIF_HESS,"Spib^A",(char*)Spip[0],nmo * nbocc * sizeof(double),next_Spi,&next_Spi);
            }
        }
    }
    ...
    // => Tpi <= //
    {
        ...
        // First alpha
        {
            std::shared_ptr<Matrix> Tmix(new Matrix("Tmix",nso,naocc));
            std::shared_ptr<Matrix> Tmiy(new Matrix("Tmiy",nso,naocc));
            std::shared_ptr<Matrix> Tmiz(new Matrix("Tmiz",nso,naocc));
            double** Tmixp = Tmix->pointer();
            double** Tmiyp = Tmiy->pointer();
            double** Tmizp = Tmiz->pointer();

            std::shared_ptr<Matrix> Tpi(new Matrix("Tpi",nmo,naocc));
            double** Tpip = Tpi->pointer();
            psio_address next_Tpi = PSIO_ZERO;
            ...
            for (int A = 0; A < natom; A++) {
                ...
                for (int P = 0; P < basisset_->nshell(); P++) {
                    for (int Q = 0; Q < basisset_->nshell(); Q++) {
                        ...
                        if (aP == A) {
                            ...
                            for (int p = 0; p < nP; p++) {
                                for (int q = 0; q < nQ; q++) {
                                    C_DAXPY(naocc,(*buffer2),Caop[q + oQ],1,Tmixp[p + oP],1);
                                    C_DAXPY(naocc,(*buffer2++),Caop[p + oP],1,Tmixp[q + oQ],1);
                                }
                            }
                            // Py
                            buffer2 = buffer + 1 * nP * nQ;
                            for (int p = 0; p < nP; p++) {
                                for (int q = 0; q < nQ; q++) {
                                    C_DAXPY(naocc,(*buffer2),Caop[q + oQ],1,Tmiyp[p + oP],1);
                                    C_DAXPY(naocc,(*buffer2++),Caop[p + oP],1,Tmiyp[q + oQ],1);
                                }
                            }
                            // Pz
                            buffer2 = buffer + 2 * nP * nQ;
                            for (int p = 0; p < nP; p++) {
                                for (int q = 0; q < nQ; q++) {
                                    C_DAXPY(naocc,(*buffer2),Caop[q + oQ],1,Tmizp[p + oP],1);
                                    C_DAXPY(naocc,(*buffer2++),Caop[p + oP],1,Tmizp[q + oQ],1);
                                }
                            }
                        }
                        if (aQ == A) {
                ...
                // Tpi_x
                C_DGEMM('T','N',nmo,naocc,nso,0.5,Cap[0],nmo,Tmixp[0],naocc,0.0,Tpip[0],naocc);
                psio_->write(PSIF_HESS,"Tpia^A",(char*)Tpip[0],nmo * naocc * sizeof(double),next_Tpi,&next_Tpi);
                // Tpi_y
                C_DGEMM('T','N',nmo,naocc,nso,0.5,Cap[0],nmo,Tmiyp[0],naocc,0.0,Tpip[0],naocc);
                psio_->write(PSIF_HESS,"Tpia^A",(char*)Tpip[0],nmo * naocc * sizeof(double),next_Tpi,&next_Tpi);
                // Tpi_z
                C_DGEMM('T','N',nmo,naocc,nso,0.5,Cap[0],nmo,Tmizp[0],naocc,0.0,Tpip[0],naocc);
                psio_->write(PSIF_HESS,"Tpia^A",(char*)Tpip[0],nmo * naocc * sizeof(double),next_Tpi,&next_Tpi);
            }
        }
        ...
        // Then beta
        {
            std::shared_ptr<Matrix> Tmix(new Matrix("Tmix",nso,nbocc));
            std::shared_ptr<Matrix> Tmiy(new Matrix("Tmiy",nso,nbocc));
            std::shared_ptr<Matrix> Tmiz(new Matrix("Tmiz",nso,nbocc));
            double** Tmixp = Tmix->pointer();
            double** Tmiyp = Tmiy->pointer();
            double** Tmizp = Tmiz->pointer();

            std::shared_ptr<Matrix> Tpi(new Matrix("Tpi",nmo,nbocc));
            double** Tpip = Tpi->pointer();
            psio_address next_Tpi = PSIO_ZERO;
            ...
            for (int A = 0; A < natom; A++) {
                ...
                for (int P = 0; P < basisset_->nshell(); P++) {
                    for (int Q = 0; Q < basisset_->nshell(); Q++) {
                        ...
                        if (aP == A) {
                            ...
                            for (int p = 0; p < nP; p++) {
                                for (int q = 0; q < nQ; q++) {
                                    C_DAXPY(nbocc,(*buffer2),Cbop[q + oQ],1,Tmixp[p + oP],1);
                                    C_DAXPY(nbocc,(*buffer2++),Cbop[p + oP],1,Tmixp[q + oQ],1);
                                }
                            }
                            // Py
                            buffer2 = buffer + 1 * nP * nQ;
                            for (int p = 0; p < nP; p++) {
                                for (int q = 0; q < nQ; q++) {
                                    C_DAXPY(nbocc,(*buffer2),Cbop[q + oQ],1,Tmiyp[p + oP],1);
                                    C_DAXPY(nbocc,(*buffer2++),Cbop[p + oP],1,Tmiyp[q + oQ],1);
                                }
                            }
                            // Pz
                            buffer2 = buffer + 2 * nP * nQ;
                            for (int p = 0; p < nP; p++) {
                                for (int q = 0; q < nQ; q++) {
                                    C_DAXPY(nbocc,(*buffer2),Cbop[q + oQ],1,Tmizp[p + oP],1);
                                    C_DAXPY(nbocc,(*buffer2++),Cbop[p + oP],1,Tmizp[q + oQ],1);
                                }
                            }
                        }
                        if (aQ == A) {
                ...
                // Tpi_x
                C_DGEMM('T','N',nmo,nbocc,nso,0.5,Cbp[0],nmo,Tmixp[0],nbocc,0.0,Tpip[0],nbocc);
                psio_->write(PSIF_HESS,"Tpib^A",(char*)Tpip[0],nmo * nbocc * sizeof(double),next_Tpi,&next_Tpi);
                // Tpi_y
                C_DGEMM('T','N',nmo,nbocc,nso,0.5,Cbp[0],nmo,Tmiyp[0],nbocc,0.0,Tpip[0],nbocc);
                psio_->write(PSIF_HESS,"Tpib^A",(char*)Tpip[0],nmo * nbocc * sizeof(double),next_Tpi,&next_Tpi);
                // Tpi_z
                C_DGEMM('T','N',nmo,nbocc,nso,0.5,Cbp[0],nmo,Tmizp[0],nbocc,0.0,Tpip[0],nbocc);
                psio_->write(PSIF_HESS,"Tpib^A",(char*)Tpip[0],nmo * nbocc * sizeof(double),next_Tpi,&next_Tpi);
            }
        }   
    }
    ...
    // => Vpi <= //
    {
        ...
        // Alpha    
        {
            std::shared_ptr<Matrix> Vmix(new Matrix("Vmix",nso,naocc));
            std::shared_ptr<Matrix> Vmiy(new Matrix("Vmiy",nso,naocc));
            std::shared_ptr<Matrix> Vmiz(new Matrix("Vmiz",nso,naocc));
            double** Vmixp = Vmix->pointer();
            double** Vmiyp = Vmiy->pointer();
            double** Vmizp = Vmiz->pointer();

            std::shared_ptr<Matrix> Vpi(new Matrix("Vpi",nmo,naocc));
            double** Vpip = Vpi->pointer();
            psio_address next_Vpi = PSIO_ZERO;
            ...
            for (int A = 0; A < natom; A++) {
                ...
                for (int P = 0; P < basisset_->nshell(); P++) {
                    for (int Q = 0; Q < basisset_->nshell(); Q++) {
                        ...
                        for (int p = 0; p < nP; p++) {
                            for (int q = 0; q < nQ; q++) {
                                C_DAXPY(naocc,(*buf_x++),Caop[q + oQ],1,Vmixp[p + oP],1);
                            }
                        }
                        ...
                        for (int p = 0; p < nP; p++) {
                            for (int q = 0; q < nQ; q++) {
                                C_DAXPY(naocc,(*buf_y++),Caop[q + oQ],1,Vmiyp[p + oP],1);
                            }
                        }
                        ...
                        for (int p = 0; p < nP; p++) {
                            for (int q = 0; q < nQ; q++) {
                                C_DAXPY(naocc,(*buf_z++),Caop[q + oQ],1,Vmizp[p + oP],1);
                            }
                        }
                    }
                }
                ...
                // Vpi_x
                C_DGEMM('T','N',nmo,naocc,nso,1.0,Cap[0],nmo,Vmixp[0],naocc,0.0,Vpip[0],naocc);
                psio_->write(PSIF_HESS,"Vpia^A",(char*)Vpip[0],nmo * naocc * sizeof(double),next_Vpi,&next_Vpi);
                // Vpi_y
                C_DGEMM('T','N',nmo,naocc,nso,1.0,Cap[0],nmo,Vmiyp[0],naocc,0.0,Vpip[0],naocc);
                psio_->write(PSIF_HESS,"Vpia^A",(char*)Vpip[0],nmo * naocc * sizeof(double),next_Vpi,&next_Vpi);
                // Vpi_z
                C_DGEMM('T','N',nmo,naocc,nso,1.0,Cap[0],nmo,Vmizp[0],naocc,0.0,Vpip[0],naocc);
                psio_->write(PSIF_HESS,"Vpia^A",(char*)Vpip[0],nmo * naocc * sizeof(double),next_Vpi,&next_Vpi);
            }
        }
        // Beta    
        {
            std::shared_ptr<Matrix> Vmix(new Matrix("Vmix",nso,nbocc));
            std::shared_ptr<Matrix> Vmiy(new Matrix("Vmiy",nso,nbocc));
            std::shared_ptr<Matrix> Vmiz(new Matrix("Vmiz",nso,nbocc));
            double** Vmixp = Vmix->pointer();
            double** Vmiyp = Vmiy->pointer();
            double** Vmizp = Vmiz->pointer();

            std::shared_ptr<Matrix> Vpi(new Matrix("Vpi",nmo,nbocc));
            double** Vpip = Vpi->pointer();
            psio_address next_Vpi = PSIO_ZERO;
            ...
            for (int A = 0; A < natom; A++) {
                ...
                for (int P = 0; P < basisset_->nshell(); P++) {
                    for (int Q = 0; Q < basisset_->nshell(); Q++) {
                        ...
                        for (int p = 0; p < nP; p++) {
                            for (int q = 0; q < nQ; q++) {
                                C_DAXPY(nbocc,(*buf_x++),Cbop[q + oQ],1,Vmixp[p + oP],1);
                            }
                        }
                        ...
                        for (int p = 0; p < nP; p++) {
                            for (int q = 0; q < nQ; q++) {
                                C_DAXPY(nbocc,(*buf_y++),Cbop[q + oQ],1,Vmiyp[p + oP],1);
                            }
                        }
                        ...
                        for (int p = 0; p < nP; p++) {
                            for (int q = 0; q < nQ; q++) {
                                C_DAXPY(nbocc,(*buf_z++),Cbop[q + oQ],1,Vmizp[p + oP],1);
                            }
                        }
                    }
                }
                ...
                // Vpi_x
                C_DGEMM('T','N',nmo,nbocc,nso,1.0,Cbp[0],nmo,Vmixp[0],nbocc,0.0,Vpip[0],nbocc);
                psio_->write(PSIF_HESS,"Vpib^A",(char*)Vpip[0],nmo * nbocc * sizeof(double),next_Vpi,&next_Vpi);
                // Vpi_y
                C_DGEMM('T','N',nmo,nbocc,nso,1.0,Cbp[0],nmo,Vmiyp[0],nbocc,0.0,Vpip[0],nbocc);
                psio_->write(PSIF_HESS,"Vpib^A",(char*)Vpip[0],nmo * nbocc * sizeof(double),next_Vpi,&next_Vpi);
                // Vpi_z
                C_DGEMM('T','N',nmo,nbocc,nso,1.0,Cbp[0],nmo,Vmizp[0],nbocc,0.0,Vpip[0],nbocc);
                psio_->write(PSIF_HESS,"Vpib^A",(char*)Vpip[0],nmo * nbocc * sizeof(double),next_Vpi,&next_Vpi);
            }
        }
    }
    // => Jpi/Kpi <= //
    {
        ...
        if(options_.get_str("SCF_TYPE") == "DF"){
            ...
            // Alpha
            {
                ...
                SharedMatrix Gpi = SharedMatrix(new Matrix("MO G Deriv", nmo, naocc));
                double**pGpi = Gpi->pointer();
                psio_address next_Gpi = PSIO_ZERO;
                // Write some junk for now, to set the sizing for PSIO
                for(int pert = 0; pert < 3*natom; ++pert){
                    psio_->write(PSIF_HESS,"Gpia^A",(char*)pGpi[0],nmo * naocc * sizeof(double),next_Gpi,&next_Gpi);
                }
                next_Gpi = PSIO_ZERO;

                SharedMatrix Amn(new Matrix("(A|mn)", np, nso*nso));
                SharedMatrix Ami(new Matrix("(A|mi)", np, nso*naocc));
                SharedMatrix Aij(new Matrix("(A|ij)", np, naocc*naocc));
                ...
                for (int P = 0; P < nauxshell; ++P){
                    ...
                    for(int M = 0; M < nshell; ++M){
                        ...
                        for(int N = 0; N < nshell; ++N){
                            ...
                            C_DGEMV('N', np, nso*(size_t)nso, 1.0, Amnp[0], nso*(size_t)nso, Dap[0], 1, 0.0, cp, 1);
                            // (A|mj) = (A|mn) C[n][j]
                            C_DGEMM('N','N',np*(size_t)nso,naocc,nso,1.0,Amnp[0],nso,Caop[0],naocc,0.0,Amip[0],naocc);
                            // (A|ij) = (A|mj) C[m][i]
                            #pragma omp parallel for
                            for (int p = 0; p < np; p++) {
                                C_DGEMM('T','N',naocc,naocc,nso,1.0,Amip[p],naocc,Caop[0],naocc,0.0,&Aijp[0][p * (size_t) naocc * naocc],naocc);
                            }
                        ...
                        for(int M = 0; M < nshell; ++M){
                            ...
                            for(int N = 0; N < nshell; ++N){
                                ...
                                if(pert_incore[Pcenter]){
                                    ...
                                    // K Terms
                                    for(int p = 0; p < nP; ++p)
                                        C_DGEMM('T', 'N', nN, nso, nM, -1.0, ptr+0*stride+p*nM*nN, nN, &pTmn[oP+p][oM*nso], nso, 1.0, pdG[Px][oN], nso);
                                    for(int p = 0; p < nP; ++p)
                                        C_DGEMM('T', 'N', nN, nso, nM, -1.0, ptr+1*stride+p*nM*nN, nN, &pTmn[oP+p][oM*nso], nso, 1.0, pdG[Py][oN], nso);
                                    for(int p = 0; p < nP; ++p)
                                        C_DGEMM('T', 'N', nN, nso, nM, -1.0, ptr+2*stride+p*nM*nN, nN, &pTmn[oP+p][oM*nso], nso, 1.0, pdG[Pz][oN], nso);
                                }
                                if(pert_incore[Mcenter]){
                                    // J Terms
                                    size_t delta = 0L;
                                    ...
                                    // K Terms
                                    for(int p = 0; p < nP; ++p)
                                        C_DGEMM('T', 'N', nN, nso, nM, -1.0, ptr+3*stride+p*nM*nN, nN, &pTmn[oP+p][oM*nso], nso, 1.0, pdG[mx][oN], nso);
                                    for(int p = 0; p < nP; ++p)
                                        C_DGEMM('T', 'N', nN, nso, nM, -1.0, ptr+4*stride+p*nM*nN, nN, &pTmn[oP+p][oM*nso], nso, 1.0, pdG[my][oN], nso);
                                    for(int p = 0; p < nP; ++p)
                                        C_DGEMM('T', 'N', nN, nso, nM, -1.0, ptr+5*stride+p*nM*nN, nN, &pTmn[oP+p][oM*nso], nso, 1.0, pdG[mz][oN], nso);
                                }
                                if(pert_incore[Ncenter]){
                                    // J Terms
                                    size_t delta = 0L;
                                    ...
                                    // K Terms
                                    for(int p = 0; p < nP; ++p)
                                        C_DGEMM('T', 'N', nN, nso, nM, -1.0, ptr+6*stride+p*nM*nN, nN, &pTmn[oP+p][oM*nso], nso, 1.0, pdG[nx][oN], nso);
                                    for(int p = 0; p < nP; ++p)
                                        C_DGEMM('T', 'N', nN, nso, nM, -1.0, ptr+7*stride+p*nM*nN, nN, &pTmn[oP+p][oM*nso], nso, 1.0, pdG[ny][oN], nso);
                                    for(int p = 0; p < nP; ++p)
                                        C_DGEMM('T', 'N', nN, nso, nM, -1.0, ptr+8*stride+p*nM*nN, nN, &pTmn[oP+p][oM*nso], nso, 1.0, pdG[nz][oN], nso);
                                }
                            }
                        }
                    }
            ...
            // Beta
            {
                ...
                SharedMatrix Gpi = SharedMatrix(new Matrix("MO G Deriv", nmo, nbocc));
                double**pGpi = Gpi->pointer();
                psio_address next_Gpi = PSIO_ZERO;
                // Write some junk for now, to set the sizing for PSIO
                for(int pert = 0; pert < 3*natom; ++pert){
                    psio_->write(PSIF_HESS,"Gpib^A",(char*)pGpi[0],nmo * nbocc * sizeof(double),next_Gpi,&next_Gpi);
                }
                next_Gpi = PSIO_ZERO;

                SharedMatrix Amn(new Matrix("(A|mn)", np, nso*nso));
                SharedMatrix Ami(new Matrix("(A|mi)", np, nso*nbocc));
                SharedMatrix Aij(new Matrix("(A|ij)", np, nbocc*nbocc));
                SharedMatrix Bmn(new Matrix("Minv[B][A] (A|mn)", np, nso*nso));
                SharedMatrix Tmn(new Matrix("Tmn", np, nso*nso));
                SharedMatrix TempP(new Matrix("Temp[P]", 9, maxp));
                SharedMatrix TempPmn(new Matrix("Temp[P][mn]", maxp, nso*nso));
                ...
                for (int P = 0; P < nauxshell; ++P){
                    ...
                    for(int M = 0; M < nshell; ++M){
                        ...
                        for(int N = 0; N < nshell; ++N){
                            ...
                            // c[A] = (A|mn) D[m][n]
                            C_DGEMV('N', np, nso*(size_t)nso, 1.0, Amnp[0], nso*(size_t)nso, Dbp[0], 1, 0.0, cp, 1);
                            // (A|mj) = (A|mn) C[n][j]
                            C_DGEMM('N','N',np*(size_t)nso,nbocc,nso,1.0,Amnp[0],nso,Cbop[0],nbocc,0.0,Amip[0],nbocc);
                            // (A|ij) = (A|mj) C[m][i]
                            #pragma omp parallel for
                            for (int p = 0; p < np; p++) {
                                C_DGEMM('T','N',nbocc,nbocc,nso,1.0,Amip[p],nbocc,Cbop[0],nbocc,0.0,&Aijp[0][p * (size_t) nbocc * nbocc],nbocc);
                            }
                        }
                    }
                }
                ...
                #pragma omp parallel for
                for(int p = 0; p < np; ++p)
                    C_DGEMM('t', 'n', nso, nso, nso, 1.0, Dbp[0], nso, Bmnp[p], nso, 0.0, pTmn[p], nso);
    ...
    size_t mem = 0.9 * memory_ / 8L;
    size_t per_Aa = 3L * nso * nso + 1L * naocc * nso;
    size_t per_Ab = 3L * nso * nso + 1L * nbocc * nso;
    size_t max_Aa = (mem / 2L) / per_Aa;
    size_t max_Ab = (mem / 2L) / per_Ab;
    max_Aa = (max_Aa > 3 * natom ? 3 * natom : max_Aa);
    max_Ab = (max_Ab > 3 * natom ? 3 * natom : max_Ab);
    jk->set_memory(mem);
    jk->initialize();
     ...
     */
    std::shared_ptr<Matrix> Ca_so;
    std::shared_ptr<Matrix> Cb_so;
    std::shared_ptr<Matrix> Caocc_so;
    std::shared_ptr<Matrix> Cbocc_so;
    // C1 MO -> AO (backward transformation)
    std::shared_ptr<Matrix> Cainvao;
    std::shared_ptr<Matrix> Cbinvao;
    // C1 MO -> SO (backward transformation)
    std::shared_ptr<Matrix> Cainvso;
    std::shared_ptr<Matrix> Cbinvso;
    // C1 MO -> Symmetrized MO
    std::shared_ptr<Matrix> Camo2mosym;
    std::shared_ptr<Matrix> Cbmo2mosym;
    // SO -> C1 MO (forward transformation)
    std::shared_ptr<Matrix> Caso2mofull;
    std::shared_ptr<Matrix> Cbso2mofull;
    ...
    if(ignore_symmetry){
        ...
        // C1 MO -> AO (backward transformation)
        Cainvao = std::shared_ptr<Matrix>(new Matrix("Cainvao = Cat_ao Sao", nmo, nso));
        C_DGEMM('T', 'N', nmo, nso, nso, 1.0, Cap[0], nmo, pS[0], nso, 0.0, Cainvao->pointer()[0], nso);

        Cbinvao = std::shared_ptr<Matrix>(new Matrix("Cbinvao = Cbt_ao Sao", nmo, nso));
        C_DGEMM('T', 'N', nmo, nso, nso, 1.0, Cbp[0], nmo, pS[0], nso, 0.0, Cbinvao->pointer()[0], nso);

        // C1 MO -> SO (backward transformation)
        Cainvso = std::shared_ptr<Matrix>(new Matrix(nirrep_, nmo, nsopi_));
        Cbinvso = std::shared_ptr<Matrix>(new Matrix(nirrep_, nmo, nsopi_));
        for(int h = 0; h < nirrep_; ++h){
            if(!nsopi_[h]) continue;
            C_DGEMM('N', 'N', nmo, nsopi_[h], nso, 1.0, Cainvao->pointer()[0], nso, AO2SO_->pointer(h)[0],
                    nsopi_[h], 0.0, Cainvso->pointer(h)[0], nsopi_[h]);
            C_DGEMM('N', 'N', nmo, nsopi_[h], nso, 1.0, Cbinvao->pointer()[0], nso, AO2SO_->pointer(h)[0],
                    nsopi_[h], 0.0, Cbinvso->pointer(h)[0], nsopi_[h]);
        }

        // C1 MO -> Symmetrized MO
        Camo2mosym = std::shared_ptr<Matrix>(new Matrix(nirrep_, nmo, nmopi_));
        Cbmo2mosym = std::shared_ptr<Matrix>(new Matrix(nirrep_, nmo, nmopi_));
        for(int h = 0; h < nirrep_; ++h){
            if(!doccpi_[h] || !nsopi_[h]) continue;
            C_DGEMM('N', 'N', nmo, nmopi_[h], nsopi_[h], 1.0, Cainvso->pointer(h)[0], nsopi_[h],
                    Ca_so->pointer(h)[0], nmopi_[h], 0.0, Camo2mosym->pointer(h)[0], nmopi_[h]);
            C_DGEMM('N', 'N', nmo, nmopi_[h], nsopi_[h], 1.0, Cbinvso->pointer(h)[0], nsopi_[h],
                    Cb_so->pointer(h)[0], nmopi_[h], 0.0, Cbmo2mosym->pointer(h)[0], nmopi_[h]);
        }

        // SO -> C1 MO (forward transformation)
        Caso2mofull = std::shared_ptr<Matrix>(new Matrix(nirrep_, (int*)nsopi_, nmo));
        Cbso2mofull = std::shared_ptr<Matrix>(new Matrix(nirrep_, (int*)nsopi_, nmo));
        for(int h = 0; h < nirrep_; ++h){
            if(!nsopi_[h]) continue;
            C_DGEMM('T', 'N', nsopi_[h], nmo, nso, 1.0, AO2SO_->pointer(h)[0],
                    nsopi_[h], Ca->pointer()[0], nmo, 0.0, Caso2mofull->pointer(h)[0], nmo);
            C_DGEMM('T', 'N', nsopi_[h], nmo, nso, 1.0, AO2SO_->pointer(h)[0],
                    nsopi_[h], Cb->pointer()[0], nmo, 0.0, Cbso2mofull->pointer(h)[0], nmo);
        }
    }
    ...
    // => J2pi/K2pi <= //
    {
        ...
        //Alpha
        {
            std::shared_ptr<Matrix> Sij(new Matrix("Sij",naocc,naocc));
            double** Sijp = Sij->pointer();
            std::shared_ptr<Matrix> T(new Matrix("T",nso,naocc));
            double** Tp = T->pointer();
            std::shared_ptr<Matrix> U(new Matrix("Tempai",nmo,naocc));
            double** Up = U->pointer();

            // Write some placeholder data to PSIO, to get the sizing right
            psio_address next_Gpi = PSIO_ZERO;
            for (int A = 0; A < 3 * natom; A++)
                psio_->write(PSIF_HESS,"G2pia^A",(char*)Up[0],nmo*naocc*sizeof(double),next_Gpi,&next_Gpi);

            for (int A = 0; A < max_Aa; A++) {
                if(ignore_symmetry){
                    // Just pass C1 quantities in; this object doesn't respect symmetry anyway
                    L.push_back(Ca_occ);
                    R.push_back(std::shared_ptr<Matrix>(new Matrix("R",nso,naocc)));
                }else{
            ...
            if(!ignore_symmetry){
                ...
                for (int a = 0; a < 3*natom; a++) {
                    ...
                    psio_address next_Gpi = psio_get_address(PSIO_ZERO, a * (size_t) nmo * naocc * sizeof(double));
                    psio_->write(PSIF_HESS,"G2pia^A",(char*)Up[0],nmo*naocc*sizeof(double),next_Gpi,&next_Gpi);
                }
            }

            for (int A = 0; A < 3 * natom; A+=max_Aa) {
                int nA = max_Aa;
                if (A + max_Aa >= 3 * natom) {
                    nA = 3 * natom - A;
                    L.resize(nA);
                ...
                for (int a = 0; a < nA; a++) {
                    if(ignore_symmetry){
                        ...
                        psio_->read(PSIF_HESS,"Sija^A",(char*)Sijp[0],naocc*naocc*sizeof(double),next_Sij, &next_Sij);
                        C_DGEMM('N','N',nso,naocc,naocc,1.0,Caop[0],naocc,Sijp[0],naocc,0.0,R[a]->pointer()[0],naocc);
                    }else{
                        ...
                        std::shared_ptr<Matrix> Tmpij(new Matrix(nirrep_, naocc, (int*)doccpi_));
                        for(int comp = 0; comp < thissalc.ncomponent(); ++comp){
                            ...
                            psio_address next_Sij= psio_get_address(PSIO_ZERO,pert * (size_t) naocc * naocc * sizeof(double));
                            psio_->read(PSIF_HESS,"Sija^A",(char*)Sijp[0],naocc*naocc*sizeof(double),next_Sij, &next_Sij);

                            for(int h = 0; h < nirrep_; ++h){
                                if(!doccpi_[h]) continue;
                                C_DGEMM('N', 'N', naocc, doccpi_[h], naocc, 1.0, Sijp[0], naocc,
                                        Camo2mosym->pointer(h)[0], nmopi_[h], 0.0, Tmpij->pointer(h)[0], doccpi_[h]);
                            }
                            for(int h = 0; h < nirrep_; ++h){
                                if(!doccpi_[h^salcirrep] || !nsopi_[h]) continue;
                                C_DGEMM('N', 'N', nsopi_[h], doccpi_[h^salcirrep], naocc, coef, Caso2mofull->pointer(h)[0], nmo,
                ...
                for (int a = 0; a < nA; a++) {
                    if(ignore_symmetry){
                        // Add the 2J contribution to G
                        C_DGEMM('N','N',nso,naocc,nso,1.0,J[a]->pointer()[0],nso,Caop[0],naocc,0.0,Tp[0],naocc);
                        C_DGEMM('T','N',nmo,naocc,nso,-2.0,Cap[0],nmo,Tp[0],naocc,0.0,Up[0],naocc);

                        // Subtract the K term from G
                        C_DGEMM('N','N',nso,naocc,nso,1.0,K[a]->pointer()[0],nso,Caop[0],naocc,0.0,Tp[0],naocc);
                        C_DGEMM('T','N',nmo,naocc,nso,1.0,Cap[0],nmo,Tp[0],naocc,1.0,Up[0],naocc);

                        psio_address next_Gpi = psio_get_address(PSIO_ZERO,(A + a) * (size_t) nmo * naocc * sizeof(double));
                        psio_->write(PSIF_HESS,"G2pia^A",(char*)Up[0],nmo*naocc*sizeof(double),next_Gpi,&next_Gpi);
                    }else{
                        const CdSalc& thissalc = SALCList[a+A];
                        int salcirrep = thissalc.irrep();
                        ...
                        std::shared_ptr<Matrix> Fmat(new Matrix("F derivative", nmo, naocc));
                        std::shared_ptr<Matrix> Tmpso_occ(new Matrix(nirrep_, (int*)nsopi_, naocc));
                        for(int h = 0; h < nirrep_; ++h){
                            if(!nsopi_[h] || !nsopi_[h^salcirrep]) continue;
                            C_DGEMM('N', 'N', nsopi_[h], naocc, nsopi_[h^salcirrep], 1.0, J[a]->pointer(h)[0], nsopi_[h^salcirrep],
                                    Caso2mofull->pointer(h^salcirrep)[0], nmo, 0.0, Tmpso_occ->pointer(h)[0], naocc);
                        }
                        for(int h = 0; h < nirrep_; ++h){
                            if(!nsopi_[h]) continue;
                            C_DGEMM('T', 'N', nmo, naocc, nsopi_[h], 1.0, Caso2mofull->pointer(h)[0], nmo,
                                    Tmpso_occ->pointer(h)[0], naocc, 1.0, Fmat->pointer()[0], naocc);
                        }
                        ...
                        for(int comp = 0; comp < thissalc.ncomponent(); ++comp){
                            ...
                            double coef = salccomponent.coef;
                            int pert = 3*salcatom + salcxyz;
                            psio_address next_Gpi = psio_get_address(PSIO_ZERO,pert * (size_t) nmo * naocc * sizeof(double));
                            psio_->read(PSIF_HESS,"G2pia^A",(char*)Up[0],nmo*naocc*sizeof(double),next_Gpi,&next_Gpi);
                            C_DAXPY(nmo*naocc, coef, Fmat->pointer()[0], 1, Up[0], 1);
                            next_Gpi = psio_get_address(PSIO_ZERO,pert * (size_t) nmo * naocc * sizeof(double));
                            psio_->write(PSIF_HESS,"G2pia^A",(char*)Up[0],nmo*naocc*sizeof(double),next_Gpi,&next_Gpi);
                        }
                    }
                }
        ...
        // beta
        {
            std::shared_ptr<Matrix> Sij(new Matrix("Sij",nbocc,nbocc));
            double** Sijp = Sij->pointer();
            std::shared_ptr<Matrix> T(new Matrix("T",nso,nbocc));
            double** Tp = T->pointer();
            std::shared_ptr<Matrix> U(new Matrix("Tempai",nmo,nbocc));
            double** Up = U->pointer();

            // Write some placeholder data to PSIO, to get the sizing right
            psio_address next_Gpi = PSIO_ZERO;
            for (int A = 0; A < 3 * natom; A++)
                psio_->write(PSIF_HESS,"G2pib^A",(char*)Up[0],nmo*nbocc*sizeof(double),next_Gpi,&next_Gpi);

            for (int A = 0; A < max_Ab; A++) {
                if(ignore_symmetry){
                    // Just pass C1 quantities in; this object doesn't respect symmetry anyway
                    L.push_back(Cb_occ);
                    R.push_back(std::shared_ptr<Matrix>(new Matrix("R",nso,nbocc)));
                }else{
                    // Add symmetry back into the AO index of the quantities before calling.  The MO index doesn't
                    // matter too much here; it's contracted away during formation of the D matrices.
                    L.push_back(Cbocc_so);
                    // This is just a placeholder; to be replaced below
                    R.push_back(Cbocc_so);
                }
            }
            ...
            if(!ignore_symmetry){
                ...
                for (int a = 0; a < 3*natom; a++) {
                    // Initialize the C1 symmetry G2 contributions to zero
                    psio_address next_Gpi = psio_get_address(PSIO_ZERO, a * (size_t) nmo * nbocc * sizeof(double));
                    psio_->write(PSIF_HESS,"G2pib^A",(char*)Up[0],nmo*nbocc*sizeof(double),next_Gpi,&next_Gpi);
                }
            }

            for (int A = 0; A < 3 * natom; A+=max_Ab) {
                int nA = max_Ab;
                if (A + max_Ab >= 3 * natom) {
                    nA = 3 * natom - A;
                    L.resize(nA);
                ...
                for (int a = 0; a < nA; a++) {
                    if(ignore_symmetry){
                        ...
                        psio_->read(PSIF_HESS,"Sijb^A",(char*)Sijp[0],nbocc*nbocc*sizeof(double),next_Sij, &next_Sij);
                        C_DGEMM('N','N',nso,nbocc,nbocc,1.0,Cbop[0],nbocc,Sijp[0],nbocc,0.0,R[a]->pointer()[0],nbocc);
                    }else{
                        // Take SALCs of the C1 overlap derivatives, and add symmetry back in to the left index.
                        // Transform the right index to SOs, analogous to the C1 treatment above.
                        ...
                        int salcirrep = thissalc.irrep();
                        R[a] = std::shared_ptr<Matrix>(new Matrix("R", nsopi_, doccpi_, salcirrep));
                        std::shared_ptr<Matrix> Tmpij(new Matrix(nirrep_, nbocc, (int*)doccpi_));
                        for(int comp = 0; comp < thissalc.ncomponent(); ++comp){
                            const CdSalc::Component& salccomponent = thissalc.component(comp);
                            int salcatom = salccomponent.atom;
                            int salcxyz = salccomponent.xyz;
                            double coef = salccomponent.coef;
                            int pert = 3*salcatom + salcxyz;
                            psio_address next_Sij= psio_get_address(PSIO_ZERO,pert * (size_t) nbocc * nbocc * sizeof(double));
                            psio_->read(PSIF_HESS,"Sijb^A",(char*)Sijp[0],nbocc*nbocc*sizeof(double),next_Sij, &next_Sij);

                            for(int h = 0; h < nirrep_; ++h){
                                if(!doccpi_[h]) continue;
                                C_DGEMM('N', 'N', nbocc, doccpi_[h], nbocc, 1.0, Sijp[0], nbocc,
                                        Cbmo2mosym->pointer(h)[0], nmopi_[h], 0.0, Tmpij->pointer(h)[0], doccpi_[h]);
                            }
                            for(int h = 0; h < nirrep_; ++h){
                                if(!doccpi_[h^salcirrep] || !nsopi_[h]) continue;
                                C_DGEMM('N', 'N', nsopi_[h], doccpi_[h^salcirrep], nbocc, coef, Cbso2mofull->pointer(h)[0], nmo,
                                        Tmpij->pointer(h^salcirrep)[0], doccpi_[h^salcirrep], 1.0, R[a]->pointer(h)[0], doccpi_[h^salcirrep]);
                            }
                        }
                    }
                ...
                for (int a = 0; a < nA; a++) {
                    if(ignore_symmetry){
                        // Add the 2J contribution to G
                        C_DGEMM('N','N',nso,nbocc,nso,1.0,J[a]->pointer()[0],nso,Cbop[0],nbocc,0.0,Tp[0],nbocc);
                        C_DGEMM('T','N',nmo,nbocc,nso,-2.0,Cbp[0],nmo,Tp[0],nbocc,0.0,Up[0],nbocc);

                        // Subtract the K term from G
                        C_DGEMM('N','N',nso,nbocc,nso,1.0,K[a]->pointer()[0],nso,Cbop[0],nbocc,0.0,Tp[0],nbocc);
                        C_DGEMM('T','N',nmo,nbocc,nso,1.0,Cbp[0],nmo,Tp[0],nbocc,1.0,Up[0],nbocc);

                        psio_address next_Gpi = psio_get_address(PSIO_ZERO,(A + a) * (size_t) nmo * nbocc * sizeof(double));
                        psio_->write(PSIF_HESS,"G2pib^A",(char*)Up[0],nmo*nbocc*sizeof(double),next_Gpi,&next_Gpi);
                    }else{
                        const CdSalc& thissalc = SALCList[a+A];
                        int salcirrep = thissalc.irrep();
                        ...
                        // Transform from SO to C1 MO basis
                        std::shared_ptr<Matrix> Fmat(new Matrix("F derivative", nmo, nbocc));
                        std::shared_ptr<Matrix> Tmpso_occ(new Matrix(nirrep_, (int*)nsopi_, nbocc));
                        for(int h = 0; h < nirrep_; ++h){
                            if(!nsopi_[h] || !nsopi_[h^salcirrep]) continue;
                            C_DGEMM('N', 'N', nsopi_[h], nbocc, nsopi_[h^salcirrep], 1.0, J[a]->pointer(h)[0], nsopi_[h^salcirrep],
                                    Cbso2mofull->pointer(h^salcirrep)[0], nmo, 0.0, Tmpso_occ->pointer(h)[0], nbocc);
                        }
                        for(int h = 0; h < nirrep_; ++h){
                            if(!nsopi_[h]) continue;
                            C_DGEMM('T', 'N', nmo, nbocc, nsopi_[h], 1.0, Cbso2mofull->pointer(h)[0], nmo,
                                    Tmpso_occ->pointer(h)[0], nbocc, 1.0, Fmat->pointer()[0], nbocc);
                        }
                        ...
                        for(int comp = 0; comp < thissalc.ncomponent(); ++comp){
                            ...
                            double coef = salccomponent.coef;
                            int pert = 3*salcatom + salcxyz;
                            psio_address next_Gpi = psio_get_address(PSIO_ZERO,pert * (size_t) nmo * nbocc * sizeof(double));
                            psio_->read(PSIF_HESS,"G2pib^A",(char*)Up[0],nmo*nbocc*sizeof(double),next_Gpi,&next_Gpi);
                            C_DAXPY(nmo*nbocc, coef, Fmat->pointer()[0], 1, Up[0], 1);
                            next_Gpi = psio_get_address(PSIO_ZERO,pert * (size_t) nmo * nbocc * sizeof(double));
                            psio_->write(PSIF_HESS,"G2pib^A",(char*)Up[0],nmo*nbocc*sizeof(double),next_Gpi,&next_Gpi);
                        }
                    }
                }
    ...
    // => Fpi_a <= //
    {
        std::shared_ptr<Matrix> Tpi(new Matrix("Tpia",nmo,naocc));
        std::shared_ptr<Matrix> Fpi(new Matrix("Fpia",nmo,naocc));
        double** Tpip = Tpi->pointer();
        double** Fpip = Fpi->pointer();
        ...
        for (int A = 0; A < 3*natom; A++) {
            psio_->read(PSIF_HESS,"Tpia^A",(char*)Fpip[0],nmo * naocc * sizeof(double),next_Tpi,&next_Tpi);
            psio_->read(PSIF_HESS,"Vpia^A",(char*)Tpip[0],nmo * naocc * sizeof(double),next_Vpi,&next_Vpi);
            Fpi->add(Tpi);
            psio_->read(PSIF_HESS,"Gpia^A",(char*)Tpip[0],nmo * naocc * sizeof(double),next_Jpi,&next_Jpi);
            Fpi->add(Tpi);
            psio_->write(PSIF_HESS,"Fpia^A",(char*)Fpip[0],nmo * naocc * sizeof(double),next_Fpi,&next_Fpi);
        }
    }
    ...
    {
        std::shared_ptr<Matrix> Tpi(new Matrix("Tpi",nmo,nbocc));
        std::shared_ptr<Matrix> Fpi(new Matrix("Fpi",nmo,nbocc));
        double** Tpip = Tpi->pointer();
        double** Fpip = Fpi->pointer();
        ...
        for (int A = 0; A < 3*natom; A++) {
            psio_->read(PSIF_HESS,"Tpib^A",(char*)Fpip[0],nmo * nbocc * sizeof(double),next_Tpi,&next_Tpi);
            psio_->read(PSIF_HESS,"Vpib^A",(char*)Tpip[0],nmo * nbocc * sizeof(double),next_Vpi,&next_Vpi);
            Fpi->add(Tpi);
            psio_->read(PSIF_HESS,"Gpib^A",(char*)Tpip[0],nmo * nbocc * sizeof(double),next_Jpi,&next_Jpi);
            Fpi->add(Tpi);
            psio_->write(PSIF_HESS,"Fpib^A",(char*)Fpip[0],nmo * nbocc * sizeof(double),next_Fpi,&next_Fpi);
        }
    }

    // => Bpi_a <= //
    {
        std::shared_ptr<Matrix> Tai(new Matrix("T",navir,naocc));
        std::shared_ptr<Matrix> Bai(new Matrix("B",navir,naocc));
        double** Taip = Tai->pointer();
        double** Baip = Bai->pointer();
        ...
        for (int A = 0; A < 3*natom; A++) {
            next_Fpi = psio_get_address(PSIO_ZERO,sizeof(double)*(A * (size_t) nmo * naocc + naocc * naocc));
            psio_->read(PSIF_HESS,"Fpia^A",(char*)Baip[0],navir * naocc * sizeof(double),next_Fpi,&next_Fpi);
            next_Spi = psio_get_address(PSIO_ZERO,sizeof(double)*(A * (size_t) nmo * naocc + naocc * naocc));
            psio_->read(PSIF_HESS,"Spia^A",(char*)Taip[0],navir * naocc * sizeof(double),next_Spi,&next_Spi);
            for (int i = 0; i < naocc; i++)
                C_DAXPY(navir,-eaop[i],&Taip[0][i],naocc,&Baip[0][i],naocc);
            next_G2pi = psio_get_address(PSIO_ZERO,sizeof(double)*(A * (size_t) nmo * naocc + naocc * naocc));
            psio_->read(PSIF_HESS,"G2pia^A",(char*)Taip[0],navir * naocc * sizeof(double),next_G2pi,&next_G2pi);
            Bai->add(Tai);
            Bai->scale(-1.0);
            psio_->write(PSIF_HESS,"Baia^A",(char*)Baip[0],navir * naocc * sizeof(double),next_Bai,&next_Bai);
        }
    }

    // => Bpi_b <= //
    {
        std::shared_ptr<Matrix> Tai(new Matrix("T",nbvir,nbocc));
        std::shared_ptr<Matrix> Bai(new Matrix("B",nbvir,nbocc));
        double** Taip = Tai->pointer();
        double** Baip = Bai->pointer();
        ...
        for (int A = 0; A < 3*natom; A++) {
            next_Fpi = psio_get_address(PSIO_ZERO,sizeof(double)*(A * (size_t) nmo * nbocc + nbocc * nbocc));
            psio_->read(PSIF_HESS,"Fpib^A",(char*)Baip[0],nbvir * nbocc * sizeof(double),next_Fpi,&next_Fpi);
            next_Spi = psio_get_address(PSIO_ZERO,sizeof(double)*(A * (size_t) nmo * nbocc + nbocc * nbocc));
            psio_->read(PSIF_HESS,"Spib^A",(char*)Taip[0],nbvir * nbocc * sizeof(double),next_Spi,&next_Spi);
            for (int i = 0; i < nbocc; i++)
                C_DAXPY(nbvir,-ebop[i],&Taip[0][i],nbocc,&Baip[0][i],nbocc);
            next_G2pi = psio_get_address(PSIO_ZERO,sizeof(double)*(A * (size_t) nmo * nbocc + nbocc * nbocc));
            psio_->read(PSIF_HESS,"G2pib^A",(char*)Taip[0],nbvir * nbocc * sizeof(double),next_G2pi,&next_G2pi);
            Bai->add(Tai);
            Bai->scale(-1.0);
            psio_->write(PSIF_HESS,"Baib^A",(char*)Baip[0],nbvir * nbocc * sizeof(double),next_Bai,&next_Bai);
        }
    }
    ...
    */
        std::shared_ptr<scf::UHF> uhf = std::make_shared<scf::UHF>(wfn);

        SharedMatrix x(new Matrix("X", nmo*2, nmo*2));
