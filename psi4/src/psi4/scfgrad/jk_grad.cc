...
#pragma omp parallel for schedule(dynamic) num_threads(nthread_df)
        for (long int PMN = 0L; PMN < (long) NP * npairs; PMN++) {
...
void DFJKGrad::compute_hessian()
{
    ...
    for(int x = 0; x < 3*natoms; ++x){
        for(int y = 0; y < 3*natoms; ++y){
            ...
            KHessp[x][y] += 2.0*C_DDOT((size_t) np*na*na, dAijp[x], 1, ptmp[0], 1);
            C_DGEMM('n', 'n', np, na*na, np,  1.0, PQp[0], np, deijp[y], na*na, 0.0, ptmp[0], na*na);
            KHessp[x][y] -= 4.0*C_DDOT((size_t) np*na*na, dAijp[x], 1, ptmp[0], 1);
            KHessp[x][y] += 2.0*C_DDOT((size_t) np*na*na, deijp[x], 1, ptmp[0], 1);
        }
    }
