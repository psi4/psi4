...
void SCF::iterate_scf_equations() {
    ...
    while (!converged) {
        ...
        Dc_old = Dc;
        if (reference == tcscf) {
            for (int I = 0; I < nci; ++I) Dtc_old[I] = Dtc[I];
        }
        ...
        transform(Fc, Fc_t, C);
        if (reference == rohf) transform(Fo, Fo_t, C);
        if (reference == tcscf) {
            for (int I = 0; I < nci; ++I) transform(Ftc[I], Ftc_t[I], C);
        ...
        outfile->Printf("\n  @SCF %4d  %20.12f %20.12f %20.12f", cycle, total_energy, total_energy - old_energy,
                        rms_dens);
        if (reference == tcscf) {
            outfile->Printf("\n    ci      = [");
            for (int I = 0; I < nci; ++I) outfile->Printf("%11.8f%s", ci[I], I != nci - 1 ? "," : "");
            outfile->Printf("]");

            outfile->Printf("\n    ci_grad = [");
            for (int I = 0; I < nci; ++I) outfile->Printf("%11.8f%s", ci_grad[I], I != nci - 1 ? "," : "");
            outfile->Printf("]");
        }
        ...
        if (cycle > 15 && (std::fabs(delta_energy) < options_.get_double("E_CONVERGENCE")) &&
            ...
            if (reference == tcscf) {
                if (2.0 * std::fabs(norm_ci_grad) < options_.get_double("D_CONVERGENCE")) converged = true;
            } else {
                converged = true;
            }
        }
    ...
    if (reference == tcscf) {
        outfile->Printf("\n\n      CI coefficients  = [");
        for (int I = 0; I < nci; ++I) outfile->Printf("%12.9f%s", ci[I], I != nci - 1 ? "," : "");
        outfile->Printf("]");
    }
