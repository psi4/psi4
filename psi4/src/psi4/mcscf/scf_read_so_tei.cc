...
void SCF::read_so_tei() {
    ...
    if (nin_core != total_symmetric_block_size) out_of_core = true;
    ...
    for (int batch = 0; batch < nbatch; ++batch) {
        ...
        outfile->Printf("\n  batch %3d pq = [%8ld,%8ld] index = [%16ld,%16ld]", batch, batch_pq_min[batch],
                        batch_pq_max[batch], batch_index_min[batch], batch_index_max[batch]);
    }

    // Allocate the PK matrix
    allocate1(double, PK, nin_core);
    for (size_t i = 0; i < nin_core; i++) PK[i] = 0.0;
    outfile->Printf("\n\n  Allocated the PK matrix (%ld elements) ", (long int)nin_core);

    if (reference != rhf) {
        // Allocate the K matrix
        allocate1(double, K, nin_core);
        for (size_t i = 0; i < nin_core; i++) K[i] = 0.0;
...
void SCF::read_so_tei_form_PK() {
    ...
    for (int batch = 0; batch < nbatch; ++batch) {
        ...
        for (size_t pqrs = 0; pqrs < buffer_size; ++pqrs) PK[pqrs] = 0.0;
