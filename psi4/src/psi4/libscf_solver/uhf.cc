...
std::vector<SharedMatrix> UHF::onel_Hx(std::vector<SharedMatrix> x_vec) {
    ...
    for (size_t i = 0; i < x_vec.size() / 2; i++) {
        ...
        // Alpha
        SharedMatrix tmp1 = triplet(Ca_occ, Fa_, Ca_occ, true, false, false);
        SharedMatrix result = doublet(tmp1, x_vec[2 * i], false, false);

        SharedMatrix tmp2 = triplet(x_vec[2 * i], Ca_vir, Fa_, false, true, false);
        result->gemm(false, false, -1.0, tmp2, Ca_vir, 1.0);

        ret.push_back(result);

        // Beta
        tmp1 = triplet(Cb_occ, Fb_, Cb_occ, true, false, false);
        result = doublet(tmp1, x_vec[2 * i + 1], false, false);

        tmp2 = triplet(x_vec[2 * i + 1], Cb_vir, Fb_, false, true, false);
...
std::vector<SharedMatrix> UHF::twoel_Hx(std::vector<SharedMatrix> x_vec, bool combine, std::string return_basis) {
    ...
    for (size_t i = 0; i < nvecs; i++) {
        ...
        SharedMatrix R = doublet(Ca_vir, x_vec[2 * i], false, true);
        R->scale(-1.0);
        Cr.push_back(R);
    }
    for (size_t i = 0; i < nvecs; i++) {
        ...
        SharedMatrix R = doublet(Cb_vir, x_vec[2 * i + 1], false, true);
        R->scale(-1.0);
        Cr.push_back(R);
    }
    ...
    if (functional_->needs_xc()) {
        ...
        for (size_t i = 0; i < nvecs; i++) {
            Dx.push_back(doublet(Cl[i], Cr[i], false, true));
            Vx.push_back(std::make_shared<Matrix>("Vax Temp", Dx[i]->rowspi(), Dx[i]->colspi()));

            Dx.push_back(doublet(Cl[nvecs + i], Cr[nvecs + i], false, true));
    ...
    if (return_basis == "SO") {
        ...
        for (size_t i = 0; i < nvecs; i++) {
            ret[2 * i] = triplet(Ca_occ, ret[2 * i], Ca_vir, true, false, false);
            ret[2 * i + 1] = triplet(Cb_occ, ret[2 * i + 1], Cb_vir, true, false, false);
        }
    } else {
...
std::vector<SharedMatrix> UHF::cphf_solve(std::vector<SharedMatrix> x_vec, double conv_tol, int max_iter,
    ...
    SharedMatrix IFock_a = triplet(Ca_, Fa_, Ca_, true, false, false);
    SharedMatrix IFock_b = triplet(Cb_, Fb_, Cb_, true, false, false);
...
int UHF::soscf_update(double soscf_conv, int soscf_min_iter, int soscf_max_iter, int soscf_print) {
    ...
    SharedMatrix Gradient_a = triplet(Cocc_a, Fa_, Cvir_a, true, false, false);

    SharedMatrix Cocc_b = Cb_subset("SO", "OCC");
    SharedMatrix Cvir_b = Cb_subset("SO", "VIR");
    SharedMatrix Gradient_b = triplet(Cocc_b, Fb_, Cvir_b, true, false, false);
