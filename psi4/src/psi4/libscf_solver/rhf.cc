...
std::vector<SharedMatrix> RHF::onel_Hx(std::vector<SharedMatrix> x_vec) {
    ...
    for (size_t i = 0; i < x_vec.size(); i++) {
        ...
        SharedMatrix tmp1 = triplet(Co, F, Co, true, false, false);
        SharedMatrix result = doublet(tmp1, x_vec[i], false, false);

        SharedMatrix tmp2 = triplet(x_vec[i], Cv, F, false, true, false);
...
std::vector<SharedMatrix> RHF::twoel_Hx(std::vector<SharedMatrix> x_vec, bool combine, std::string return_basis) {
    ...
    for (size_t i = 0; i < x_vec.size(); i++) {
        ...
        Cl.push_back(Co);
        SharedMatrix R = doublet(Cv, x_vec[i], false, true);
        R->scale(-1.0);
        Cr.push_back(R);
    }
    ...
    if (return_basis == "SO") {
        ...
        for (size_t i = 0; i < ret.size(); i++) {
            if (c1_input_[i]) {
                ret[i] = triplet(Cocc_ao, ret[i], Cvir_ao, true, false, false);
            } else {
                ret[i] = triplet(Cocc_so, ret[i], Cvir_so, true, false, false);
            }
        }
    } else {
...
std::vector<SharedMatrix> RHF::cphf_solve(std::vector<SharedMatrix> x_vec, double conv_tol, int max_iter,
    ...
    if (needs_ao) {
        ...
        SharedMatrix IFock_ao = triplet(Cocc_ao, F_ao, Cocc_ao, true, false, false);
    ...
    if (needs_so) {
        ...
        SharedMatrix IFock_so = triplet(Ca_, Fa_, Ca_, true, false, false);
...
int RHF::soscf_update(double soscf_conv, int soscf_min_iter, int soscf_max_iter, int soscf_print) {
    ...
    // Gradient RHS
    SharedMatrix Gradient = triplet(Cocc, Fa_, Cvir, true, false, false);
