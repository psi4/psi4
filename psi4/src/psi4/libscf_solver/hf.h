...
class HF : public Wavefunction {
   protected:
    ...
    /** Overwrites eps with the unshifted orbital energies eps_p = (C^T F C)_pp, for orbitals C
        obtained by diagonalizing a level-shifted F */
    void remove_level_shift(const SharedMatrix& F, const SharedMatrix& C, std::shared_ptr<Vector>& eps);
