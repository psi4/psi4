...
libcppe::Molecule make_molecule(std::shared_ptr<Molecule> molecule) {
    ...
    for (int i = 0; i < molecule->natom(); ++i) {
        libcppe::Atom a(static_cast<int>(molecule->Z(i)),
                                         geom.get(i, 0),
                                         geom.get(i, 1),
                                         geom.get(i, 2));
        mol.push_back(a);
...
namespace {
    std::vector<double> mult_coeffs = {1.0, // q
                1.0, 1.0, 1.0, // mu
...
SharedMatrix PeIntegralHelper::compute_multipole_potential_integrals(Vector3 site, int order, std::vector<double>& moments) {
    ...
    for (int l = 0; l < total_components; ++l) {
        ...
        // std::cout << "coeff = " << mult_coeffs[l] << " moment = " << moments[from_alphabetical_to_psi4[l]] << std::endl;
...
SharedMatrix PeIntegralHelper::compute_field_integrals(Vector3 site, arma::vec moment) {
    ...
    auto integrals = std::make_shared<IntegralFactory>(basisset_, basisset_, basisset_, basisset_);
    std::shared_ptr<OneBodyAOInt> field_integrals_ = static_cast<std::shared_ptr<OneBodyAOInt>>(integrals->electric_field());
