...
std::string Dispersion::print_gradient(std::shared_ptr<Molecule> m) {
    ...
    for (int k = 1; k <= m->natom(); k++) {
        s << "  " << std::setw(5) << k << std::setw(20) << g[(k - 1) * 3 + 0] << std::setw(20) << g[(k - 1) * 3 + 1]
...
std::string Dispersion::print_hessian(std::shared_ptr<Molecule> m) {
    ...
    for (int k = 1; k <= m->natom(); k++) {
        for (int j = 1; j <= m->natom(); j++) {
            ...
            s << "  " << std::setw(5) << "xA" << std::setw(20) << h[(k - 1) * 3 + 0][(j - 1) * 3 + 0] << std::setw(20)
              << h[(k - 1) * 3 + 0][(j - 1) * 3 + 1] << std::setw(20) << h[(k - 1) * 3 + 0][(j - 1) * 3 + 2]
              << std::endl;
            s << "  " << std::setw(5) << "yA" << std::setw(20) << h[(k - 1) * 3 + 1][(j - 1) * 3 + 0] << std::setw(20)
              << h[(k - 1) * 3 + 1][(j - 1) * 3 + 1] << std::setw(20) << h[(k - 1) * 3 + 1][(j - 1) * 3 + 2]
              << std::endl;
            s << "  " << std::setw(5) << "zA" << std::setw(20) << h[(k - 1) * 3 + 2][(j - 1) * 3 + 0] << std::setw(20)
              << h[(k - 1) * 3 + 2][(j - 1) * 3 + 1] << std::setw(20) << h[(k - 1) * 3 + 2][(j - 1) * 3 + 2]
              << std::endl;
            s << std::endl;
        }
    }
...
SharedMatrix Dispersion::compute_gradient(std::shared_ptr<Molecule> m) {
    ...
    for (int i = 0; i < m->natom(); i++) {
        for (int j = 0; j < i; j++) {
            ...
            if (C6_type_ == C6_arit) {
                C6 = 2.0 * C6_[(int)m->Z(i, true)] * C6_[(int)m->Z(j, true)] /
                     (C6_[(int)m->Z(i, true)] + C6_[(int)m->Z(j, true)]);
                C6_R = 0.0;
            } else if (C6_type_ == C6_geom) {
