...
void amp_write_RHF(dpdfile2 *RIA, dpdbuf4 *RIjAb, int namps) {
    ...
    auto frdocc = moinfo.frdocc;
    auto clsdpi = moinfo.clsdpi;
            ...
            for (a = 0; a < R1->params->coltot[h ^ Gia]; a++) {
                ...
                for (m = 0; m < R1_stack.size(); m++) {
                    if ((std::fabs(one_R1.value) - std::fabs(R1_stack[m].value)) > 1e-12) {
                        ...
                        if (R1_stack.size() > namps) R1_stack.erase(R1_stack.end() - 1);
                        break;
                    }
                }
            }
            ...
            for (ab = 0; ab < R2->params->coltot[h ^ Gijab]; ab++) {
                ...
                for (m = 0; m < R2_stack.size(); m++) {
                    if ((std::fabs(one_R2.value) - std::fabs(R2_stack[m].value)) > 1e-12) {
                        ...
                        if (R2_stack.size() > namps) R2_stack.erase(R2_stack.end() - 1);
                        break;
                    }
                }
            }
