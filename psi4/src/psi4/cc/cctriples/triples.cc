...
void T3_UHF_AAB_abc(double ***W, double ***V, int disc, int nirreps, int I, int Gi, int J, int Gj, int K, int Gk,
                    ...
                    dpdfile2 *fia, dpdfile2 *fIJ, dpdfile2 *fij, dpdfile2 *fAB, dpdfile2 *fab, Dimension const& aoccpi,
                    int *aocc_off, Dimension const& boccpi, int *bocc_off, Dimension const& avirtpi, int *avir_off, Dimension const& bvirtpi, int *bvir_off,
                    double omega);
...
PsiReturnType cctriples(std::shared_ptr<Wavefunction> reference_wavefunction, Options &options) {
    ...
    if (params.ref == 0) { /** RHF **/
        ...
        if (params.wfn == "CCSD_T" || params.wfn == "BCCD_T") {
            ...
            reference_wavefunction->set_scalar_variable("A-(T) CORRECTION ENERGY", ET);
            reference_wavefunction->set_scalar_variable("A-CCSD(T) CORRELATION ENERGY", ET + moinfo.ecc);
            reference_wavefunction->set_scalar_variable("A-CCSD(T) TOTAL ENERGY", ET + moinfo.ecc + moinfo.eref);
        }
