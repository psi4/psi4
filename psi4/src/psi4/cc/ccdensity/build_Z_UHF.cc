...
void build_Z_UHF() {
    ...
    for (h = 0, count = 0; h < nirreps; h++)
        for (a = 0; a < X.params->rowtot[h]; a++)
            for (i = 0; i < X.params->coltot[h]; i++) Z[count++] = -X.matrix[h][a][i];
    ...
    for (ai = 0; ai < dim_A; ai++)
        for (bj = 0; bj < dim_A; bj++) A[ai][bj] = A_AA.matrix[0][ai][bj];
    ...
    for (ai = 0; ai < dim_B; ai++)
        for (bj = 0; bj < dim_B; bj++) A[ai + dim_A][bj + dim_A] = A_BB.matrix[0][ai][bj];
    global_dpd_->buf4_mat_irrep_close(&A_BB, 0);

    global_dpd_->buf4_mat_irrep_init(&A_AB, 0);
    global_dpd_->buf4_mat_irrep_rd(&A_AB, 0);
    for (ai = 0; ai < dim_A; ai++)
        for (bj = 0; bj < dim_B; bj++) A[ai][bj + dim_A] = A[bj + dim_A][ai] = A_AB.matrix[0][ai][bj];
    ...
    for (h = 0, count = 0; h < nirreps; h++)
        for (a = 0; a < D.params->rowtot[h]; a++)
            for (i = 0; i < D.params->coltot[h]; i++) {
                if (std::fabs(Z[count]) > 1e3)
                    D.matrix[h][a][i] = 0.0;
                else
                    D.matrix[h][a][i] = Z[count];
                count++;
            }
