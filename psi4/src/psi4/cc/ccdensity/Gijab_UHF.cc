...
void Gijab_UHF() {
    ...
    global_dpd_->buf4_init(&Z, PSIF_CC_TMP4, 0, 20, 20, 20, 20, 0, "Z(IA,ME)");
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (row = 0; row < Z.params->rowtot[h]; row++) {
            ...
            I = T1A.params->rowidx[i];
            Isym = T1A.params->psym[i];
            A = T1A.params->colidx[a];
            Asym = T1A.params->qsym[a];

            for (col = 0; col < Z.params->coltot[h]; col++) {
                m = Z.params->colorb[h][col][0];
                e = Z.params->colorb[h][col][1];
                M = T1A.params->rowidx[m];
                Msym = T1A.params->psym[m];
                E = T1A.params->colidx[e];
                Esym = T1A.params->qsym[e];

                if ((Isym == Esym) && (Msym == Asym))
                    Z.matrix[h][row][col] += (2 * T1A.matrix[Isym][I][E] * T1A.matrix[Msym][M][A]);
            }
        }
    ...
    global_dpd_->buf4_close(&T);
    global_dpd_->buf4_init(&Z, PSIF_CC_TMP5, 0, 30, 30, 30, 30, 0, "Z(ia,me)");
    global_dpd_->buf4_scm(&Z, -1.0);
    for (h = 0; h < nirreps; h++) {
        global_dpd_->buf4_mat_irrep_init(&Z, h);
        global_dpd_->buf4_mat_irrep_rd(&Z, h);
        for (row = 0; row < Z.params->rowtot[h]; row++) {
            i = Z.params->roworb[h][row][0];
            a = Z.params->roworb[h][row][1];
            I = T1B.params->rowidx[i];
            Isym = T1B.params->psym[i];
            A = T1B.params->colidx[a];
            Asym = T1B.params->qsym[a];

            for (col = 0; col < Z.params->coltot[h]; col++) {
                m = Z.params->colorb[h][col][0];
                e = Z.params->colorb[h][col][1];
                M = T1B.params->rowidx[m];
                Msym = T1B.params->psym[m];
                E = T1B.params->colidx[e];
                Esym = T1B.params->qsym[e];

                if ((Isym == Esym) && (Msym == Asym))
                    Z.matrix[h][row][col] += (2 * T1B.matrix[Isym][I][E] * T1B.matrix[Msym][M][A]);
            }
        }
        global_dpd_->buf4_mat_irrep_wrt(&Z, h);
        global_dpd_->buf4_mat_irrep_close(&Z, h);
    ...
    global_dpd_->buf4_close(&T);
    global_dpd_->buf4_init(&Z, PSIF_CC_TMP6, 0, 27, 24, 27, 24, 0, "Z(iA,Me)");
    for (h = 0; h < nirreps; h++) {
        global_dpd_->buf4_mat_irrep_init(&Z, h);
        global_dpd_->buf4_mat_irrep_rd(&Z, h);
        for (row = 0; row < Z.params->rowtot[h]; row++) {
            ...
            for (col = 0; col < Z.params->coltot[h]; col++) {
                ...
                if ((Isym == Esym) && (Msym == Asym))
                    Z.matrix[h][row][col] += (2 * T1B.matrix[Isym][I][E] * T1A.matrix[Msym][M][A]);
            }
        }
        global_dpd_->buf4_mat_irrep_wrt(&Z, h);
        global_dpd_->buf4_mat_irrep_close(&Z, h);
    ...
    global_dpd_->buf4_close(&T);
    global_dpd_->buf4_init(&Z, PSIF_CC_TMP7, 0, 24, 27, 24, 27, 0, "Z(Ia,mE)");
    for (h = 0; h < nirreps; h++) {
        global_dpd_->buf4_mat_irrep_init(&Z, h);
        global_dpd_->buf4_mat_irrep_rd(&Z, h);
        for (row = 0; row < Z.params->rowtot[h]; row++) {
            ...
            for (col = 0; col < Z.params->coltot[h]; col++) {
                ...
                if ((Isym == Esym) && (Msym == Asym))
                    Z.matrix[h][row][col] += (2 * T1A.matrix[Isym][I][E] * T1B.matrix[Msym][M][A]);
            }
        }
        global_dpd_->buf4_mat_irrep_wrt(&Z, h);
        global_dpd_->buf4_mat_irrep_close(&Z, h);
    ...
    if (params.wfn == "CCSD_T" && params.dertype == 1) {
        ...
        global_dpd_->buf4_init(&L, PSIF_CC_GLG, 0, 2, 7, 2, 7, 0, "LIJAB");
    ...
    global_dpd_->buf4_init(&Z1, PSIF_CC_TMP8, 0, 0, 7, 0, 7, 0, "Z1(IJ,AB)");
    global_dpd_->buf4_init(&T, PSIF_CC_TAMPS, 0, 0, 7, 2, 7, 0, "tauIJAB");
    global_dpd_->file2_init(&ZZ, PSIF_CC_TMP0, 0, 0, 0, "Z(I,M)");
    global_dpd_->contract244(&ZZ, &T, &Z1, 1, 0, 0, -1.0, 0.0);
    global_dpd_->file2_close(&ZZ);
    global_dpd_->buf4_close(&T);
    global_dpd_->buf4_sort(&Z1, PSIF_CC_TMP9, qprs, 0, 7, "Z2(JI,AB)");
    global_dpd_->buf4_init(&Z2, PSIF_CC_TMP9, 0, 0, 7, 0, 7, 0, "Z2(JI,AB)");
    ...
    global_dpd_->buf4_init(&Z1, PSIF_CC_TMP8, 0, 2, 5, 2, 5, 0, "ZZ1(IJ,AB)");
    global_dpd_->buf4_init(&T, PSIF_CC_TAMPS, 0, 2, 5, 2, 7, 0, "tauIJAB");
    global_dpd_->file2_init(&ZZ, PSIF_CC_TMP2, 0, 1, 1, "Z(E,A)");
    global_dpd_->contract424(&T, &ZZ, &Z1, 3, 0, 0, 1.0, 0.0);
    global_dpd_->file2_close(&ZZ);
    global_dpd_->buf4_close(&T);
    global_dpd_->buf4_sort(&Z1, PSIF_CC_TMP9, pqsr, 2, 5, "Z2(IJ,BA)");
    global_dpd_->buf4_init(&Z2, PSIF_CC_TMP9, 0, 2, 5, 2, 5, 0, "Z2(IJ,BA)");
    global_dpd_->buf4_axpy(&Z2, &Z1, -1.0);
    global_dpd_->buf4_close(&Z2);
    ...
    if (T2_L2_V) {
        ...
        global_dpd_->buf4_close(&V);
        global_dpd_->buf4_close(&T);
        global_dpd_->buf4_sort(&Z, PSIF_CC_TMP9, rqps, 20, 20, "Z(JA,IB)");
        global_dpd_->buf4_sort(&Z, PSIF_CC_TMP10, psrq, 20, 20, "Z(IB,JA)");
        global_dpd_->buf4_sort(&Z, PSIF_CC_TMP11, rspq, 20, 20, "Z(JB,IA)");
        global_dpd_->buf4_init(&Z1, PSIF_CC_TMP9, 0, 20, 20, 20, 20, 0, "Z(JA,IB)");
        global_dpd_->buf4_axpy(&Z1, &Z, -1.0);
        global_dpd_->buf4_close(&Z1);
        global_dpd_->buf4_init(&Z1, PSIF_CC_TMP10, 0, 20, 20, 20, 20, 0, "Z(IB,JA)");
        global_dpd_->buf4_axpy(&Z1, &Z, -1.0);
        global_dpd_->buf4_close(&Z1);
        global_dpd_->buf4_init(&Z1, PSIF_CC_TMP11, 0, 20, 20, 20, 20, 0, "Z(JB,IA)");
        global_dpd_->buf4_axpy(&Z1, &Z, 1.0);
        global_dpd_->buf4_close(&Z1);
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (row = 0; row < G.params->rowtot[h]; row++) {
            ...
            for (col = 0; col < G.params->coltot[h]; col++) {
                ...
                I = ZZ.params->rowidx[i];
                Isym = ZZ.params->psym[i];
                J = T1.params->rowidx[j];
                Jsym = T1.params->psym[j];
                A = ZZ.params->colidx[a];
                Asym = ZZ.params->qsym[a];
                B = T1.params->colidx[b];
                Bsym = T1.params->qsym[b];

                if ((Isym == Asym) && (Jsym == Bsym)) value += ZZ.matrix[Isym][I][A] * T1.matrix[Jsym][J][B];

                I = T1.params->rowidx[i];
                Isym = T1.params->psym[i];
                J = ZZ.params->rowidx[j];
                Jsym = ZZ.params->psym[j];

                if ((Jsym == Asym) && (Isym == Bsym)) value -= ZZ.matrix[Jsym][J][A] * T1.matrix[Isym][I][B];
                ...
                Asym = T1.params->qsym[a];
                B = ZZ.params->colidx[b];
                Bsym = ZZ.params->qsym[b];

                if ((Isym == Bsym) && (Jsym == Asym)) value -= ZZ.matrix[Isym][I][B] * T1.matrix[Jsym][J][A];
                ...
                if ((Isym == Asym) && (Jsym == Bsym)) value += T1.matrix[Isym][I][A] * ZZ.matrix[Jsym][J][B];
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (row = 0; row < G.params->rowtot[h]; row++) {
            ...
            for (col = 0; col < G.params->coltot[h]; col++) {
                ...
                if ((Jsym == Asym) && (Isym == Bsym)) value -= T1.matrix[Jsym][J][A] * ZZ.matrix[Isym][I][B];
                ...
                if ((Isym == Bsym) && (Jsym == Asym)) value -= T1.matrix[Isym][I][B] * ZZ.matrix[Jsym][J][A];
    ...
    if (params.wfn == "CCSD_T" && params.dertype == 1) {
        ...
        global_dpd_->buf4_init(&L, PSIF_CC_GLG, 0, 12, 17, 12, 17, 0, "Lijab");
        global_dpd_->buf4_axpy(&L, &G, 1.0);
        global_dpd_->buf4_close(&L);
        global_dpd_->buf4_close(&G);
    } else {
    ...
    global_dpd_->buf4_close(&G);
    /* - ( Z(i,m) Tau(mj,ab) - Z(j,m) Tau(mi,ab) ) */
    global_dpd_->buf4_init(&Z1, PSIF_CC_TMP8, 0, 10, 17, 10, 17, 0, "Z1(ij,ab)");
    global_dpd_->buf4_init(&T, PSIF_CC_TAMPS, 0, 10, 17, 12, 17, 0, "tauijab");
    global_dpd_->file2_init(&ZZ, PSIF_CC_TMP1, 0, 2, 2, "Z(i,m)");
    global_dpd_->contract244(&ZZ, &T, &Z1, 1, 0, 0, -1.0, 0.0);
    global_dpd_->file2_close(&ZZ);
    global_dpd_->buf4_close(&T);
    global_dpd_->buf4_sort(&Z1, PSIF_CC_TMP9, qprs, 10, 17, "Z2(ji,ab)");
    global_dpd_->buf4_init(&Z2, PSIF_CC_TMP9, 0, 10, 17, 10, 17, 0, "Z2(ji,ab)");
    global_dpd_->buf4_axpy(&Z2, &Z1, -1.0);
    global_dpd_->buf4_close(&Z2);
    ...
    global_dpd_->buf4_close(&G);
    /* - ( Z(e,a) Tau(ij,be) - Z(e,b) Tau(ij,ae) ) */
    global_dpd_->buf4_init(&Z1, PSIF_CC_TMP8, 0, 12, 15, 12, 15, 0, "ZZ1(ij,ab)");
    global_dpd_->buf4_init(&T, PSIF_CC_TAMPS, 0, 12, 15, 12, 17, 0, "tauijab");
    global_dpd_->file2_init(&ZZ, PSIF_CC_TMP3, 0, 3, 3, "Z(e,a)");
    global_dpd_->contract424(&T, &ZZ, &Z1, 3, 0, 0, 1.0, 0.0);
    global_dpd_->file2_close(&ZZ);
    global_dpd_->buf4_close(&T);
    global_dpd_->buf4_sort(&Z1, PSIF_CC_TMP9, pqsr, 12, 15, "Z2(ij,ba)");
    global_dpd_->buf4_init(&Z2, PSIF_CC_TMP9, 0, 12, 15, 12, 15, 0, "Z2(ij,ba)");
    global_dpd_->buf4_axpy(&Z2, &Z1, -1.0);
    global_dpd_->buf4_close(&Z2);
    ...
    if (T2_L2_V) {
        ...
        global_dpd_->buf4_close(&V);
        global_dpd_->buf4_close(&T);
        global_dpd_->buf4_sort(&Z, PSIF_CC_TMP9, rqps, 30, 30, "Z(ja,ib)");
        global_dpd_->buf4_sort(&Z, PSIF_CC_TMP10, psrq, 30, 30, "Z(ib,ja)");
        global_dpd_->buf4_sort(&Z, PSIF_CC_TMP11, rspq, 30, 30, "Z(jb,ia)");
        global_dpd_->buf4_init(&Z1, PSIF_CC_TMP9, 0, 30, 30, 30, 30, 0, "Z(ja,ib)");
        global_dpd_->buf4_axpy(&Z1, &Z, -1.0);
        global_dpd_->buf4_close(&Z1);
        global_dpd_->buf4_init(&Z1, PSIF_CC_TMP10, 0, 30, 30, 30, 30, 0, "Z(ib,ja)");
        global_dpd_->buf4_axpy(&Z1, &Z, -1.0);
        global_dpd_->buf4_close(&Z1);
        global_dpd_->buf4_init(&Z1, PSIF_CC_TMP11, 0, 30, 30, 30, 30, 0, "Z(jb,ia)");
        global_dpd_->buf4_axpy(&Z1, &Z, 1.0);
        global_dpd_->buf4_close(&Z1);
        global_dpd_->buf4_sort(&Z, PSIF_CC_TMP9, prqs, 10, 15, "Z(ij,ab)");
        global_dpd_->buf4_close(&Z);
        global_dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 10, 15, 12, 17, 0, "Gijab");
        global_dpd_->buf4_init(&Z, PSIF_CC_TMP9, 0, 10, 15, 10, 15, 0, "Z(ij,ab)");
        /* I don't understand this factor of 1/2 that shows up here */
        global_dpd_->buf4_axpy(&Z, &G, -0.5);
        global_dpd_->buf4_close(&Z);
        global_dpd_->buf4_close(&G);
    ...
    /* T(j,e) L(m,e) --> ZZ(j,m) */
    global_dpd_->file2_init(&ZZ, PSIF_CC_TMP8, 0, 2, 2, "Z(j,m)");
    global_dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 2, 3, "tia");
    global_dpd_->file2_init(&L1, PSIF_CC_GLG, 0, 2, 3, "Lia");
    global_dpd_->contract222(&T1, &L1, &ZZ, 0, 0, 1.0, 0.0);
    global_dpd_->file2_close(&L1);
    global_dpd_->file2_close(&T1);
    /* ZZ(j,m) T(m,b) --> ZZ2(j,b) */
    global_dpd_->file2_init(&ZZ2, PSIF_CC_TMP9, 0, 2, 3, "ZZ2(j,b)");
    global_dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 2, 3, "tia");
    global_dpd_->contract222(&ZZ, &T1, &ZZ2, 0, 1, 1.0, 0.0);
    global_dpd_->file2_close(&T1);
    global_dpd_->file2_close(&ZZ);
    ...
    /* 3 P(ij) P(ab) T(i,a) ZZ(j,b) --> G(ij,ab) */
    global_dpd_->file2_init(&ZZ, PSIF_CC_TMP9, 0, 2, 3, "ZZ2(j,b)");
    global_dpd_->file2_mat_init(&ZZ);
    global_dpd_->file2_mat_rd(&ZZ);
    global_dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 2, 3, "tia");
    global_dpd_->file2_mat_init(&T1);
    ...
    global_dpd_->buf4_init(&Z1, PSIF_CC_TMP8, 0, 22, 28, 22, 28, 0, "Z1(Ij,Ab)");
    global_dpd_->buf4_init(&T, PSIF_CC_TAMPS, 0, 22, 28, 22, 28, 0, "tauIjAb");
    global_dpd_->file2_init(&ZZ, PSIF_CC_TMP0, 0, 0, 0, "Z(I,M)");
    global_dpd_->contract244(&ZZ, &T, &Z1, 1, 0, 0, 1.0, 0.0);
    global_dpd_->file2_close(&ZZ);
    global_dpd_->buf4_close(&T);
    global_dpd_->buf4_init(&Z2, PSIF_CC_TMP9, 0, 23, 29, 23, 29, 0, "Z2(jI,bA)");
    global_dpd_->buf4_init(&T, PSIF_CC_TAMPS, 0, 23, 29, 23, 29, 0, "tauiJaB");
    global_dpd_->file2_init(&ZZ, PSIF_CC_TMP1, 0, 2, 2, "Z(i,m)");
    global_dpd_->contract244(&ZZ, &T, &Z2, 1, 0, 0, 1.0, 0.0);
    global_dpd_->file2_close(&ZZ);
    global_dpd_->buf4_close(&T);
    global_dpd_->buf4_sort(&Z2, PSIF_CC_TMP10, qprs, 22, 29, "Z2(Ij,bA)");
    global_dpd_->buf4_close(&Z2);
    global_dpd_->buf4_init(&Z2, PSIF_CC_TMP10, 0, 22, 29, 22, 29, 0, "Z2(Ij,bA)");
    global_dpd_->buf4_sort(&Z2, PSIF_CC_TMP9, pqsr, 22, 28, "Z2(Ij,Ab)");
    global_dpd_->buf4_close(&Z2);
    global_dpd_->buf4_init(&Z2, PSIF_CC_TMP9, 0, 22, 28, 22, 28, 0, "Z2(Ij,Ab)");
    global_dpd_->buf4_axpy(&Z2, &Z1, 1.0);
    global_dpd_->buf4_close(&Z2);
    global_dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 22, 28, 22, 28, 0, "GIjAb");
    global_dpd_->buf4_axpy(&Z1, &G, -1.0);
    ...
    global_dpd_->buf4_close(&G);
    /* - ( Z(E,A) Tau(Ij,bE) - Z(e,b) Tau(Ij,Ae) ) */
    global_dpd_->buf4_init(&Z1, PSIF_CC_TMP8, 0, 22, 28, 22, 28, 0, "ZZ1(Ij,Ab)");
    global_dpd_->buf4_init(&T, PSIF_CC_TAMPS, 0, 22, 28, 22, 28, 0, "tauIjAb");
    global_dpd_->file2_init(&ZZ, PSIF_CC_TMP3, 0, 3, 3, "Z(e,a)");
    global_dpd_->contract424(&T, &ZZ, &Z1, 3, 0, 0, 1.0, 0.0);
    global_dpd_->file2_close(&ZZ);
    ...
    if (T2_L2_V) {
        ...
        global_dpd_->buf4_close(&T);
        /* T'(jA,Me) V(Ib,Me) */
        global_dpd_->buf4_init(&Z1, PSIF_CC_TMP9, 0, 27, 24, 27, 24, 0, "Z(jA,Ib)");
        global_dpd_->buf4_init(&T, PSIF_CC_TMP6, 0, 27, 24, 27, 24, 0, "Z(iA,Me)");
        global_dpd_->buf4_init(&V, PSIF_CC_MISC, 0, 24, 24, 24, 24, 0, "VIaJb");
        global_dpd_->contract444(&T, &V, &Z1, 0, 0, 1.0, 0.0);
        global_dpd_->buf4_close(&V);
        global_dpd_->buf4_close(&T);
        global_dpd_->buf4_sort(&Z1, PSIF_CC_TMP10, rqps, 20, 30, "Z(IA,jb)");
        global_dpd_->buf4_close(&Z1);
        global_dpd_->buf4_init(&Z1, PSIF_CC_TMP10, 0, 20, 30, 20, 30, 0, "Z(IA,jb)");
        global_dpd_->buf4_axpy(&Z1, &Z, -1.0);
        global_dpd_->buf4_close(&Z1);
        /* T'(Ib,mE) V(jA,mE) */
        global_dpd_->buf4_init(&Z1, PSIF_CC_TMP9, 0, 24, 27, 24, 27, 0, "Z(Ib,jA)");
        global_dpd_->buf4_init(&T, PSIF_CC_TMP7, 0, 24, 27, 24, 27, 0, "Z(Ia,mE)");
        global_dpd_->buf4_init(&V, PSIF_CC_MISC, 0, 27, 27, 27, 27, 0, "ViAjB");
        global_dpd_->contract444(&T, &V, &Z1, 0, 0, 1.0, 0.0);
        global_dpd_->buf4_close(&V);
        global_dpd_->buf4_close(&T);
        global_dpd_->buf4_sort(&Z1, PSIF_CC_TMP10, psrq, 20, 30, "Z(IA,jb)");
        global_dpd_->buf4_close(&Z1);
        global_dpd_->buf4_init(&Z1, PSIF_CC_TMP10, 0, 20, 30, 20, 30, 0, "Z(IA,jb)");
        global_dpd_->buf4_axpy(&Z1, &Z, -1.0);
        global_dpd_->buf4_close(&Z1);
        /* T'(jb,ME) (T2) V(IA,ME) + T'(jb,me) (TMP5) V(IA,me) */
        global_dpd_->buf4_init(&Z1, PSIF_CC_TMP9, 0, 20, 30, 20, 30, 0, "Z(IA,jb)");
        global_dpd_->buf4_init(&T, PSIF_CC_TMP5, 0, 30, 30, 30, 30, 0, "Z(ia,me)");
        global_dpd_->buf4_init(&V, PSIF_CC_MISC, 0, 20, 30, 20, 30, 0, "VIAjb");
        ...
        global_dpd_->buf4_close(&Z1);
        /* - Z(IA,jb) --> G(Ij,Ab) */
        global_dpd_->buf4_sort(&Z, PSIF_CC_TMP9, prqs, 22, 28, "Z(Ij,Ab)");
        global_dpd_->buf4_close(&Z);
        global_dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 22, 28, 22, 28, 0, "GIjAb");
        global_dpd_->buf4_init(&Z, PSIF_CC_TMP9, 0, 22, 28, 22, 28, 0, "Z(Ij,Ab)");
        global_dpd_->buf4_axpy(&Z, &G, -0.5);
        global_dpd_->buf4_close(&Z);
        global_dpd_->buf4_close(&G);
    }
    ...
    global_dpd_->file2_init(&ZZ, PSIF_CC_TMP8, 0, 2, 3, "ZZ(j,b)");
    global_dpd_->file2_init(&L1, PSIF_CC_GLG, 0, 0, 1, "LIA");
    global_dpd_->buf4_init(&T, PSIF_CC_TAMPS, 0, 30, 20, 30, 20, 0, "tiaJB");
    global_dpd_->contract422(&T, &L1, &ZZ, 0, 0, -1.0, 0.0);
    global_dpd_->buf4_close(&T);
