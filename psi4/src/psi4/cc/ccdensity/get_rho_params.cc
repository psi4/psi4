...
void get_rho_params(Options &options) {
    ...
    if (!params.ground) {
        ...
        if (options["PROP_SYM"].has_changed()) { /* read symmetry of state for properties */
            ...
            for (i = 0; i < moinfo.nirreps; ++i) {
                if (states_per_irrep[i] > 0) prop_sym = i ^ moinfo.sym;
            }
        }
    ...
    for (i = 0; i < params.nstates; i++) {
        ...
        if (params.ref == 0) {
            if (i == 0)
                sprintf(rho_params[i].opdm_lbl, "MO-basis OPDM");
            else
                sprintf(rho_params[i].opdm_lbl, "MO-basis OPDM Root %d", i);
    ...
    for(i=0; i<params.nstates; i++) {
      outfile->Printf("\t%5s %6d %7s %4d %7s %4s %15.10lf %12.8lf\n",
    ...
    */
    ...
    for (i = 0; i < params.nstates; i++) {
        outfile->Printf("\t%5s     %d%3s %15.10lf %12.8lf\n", (rho_params[i].R_ground ? "Yes" : "No"),
                        ...
                        rho_params[i].cceom_energy, rho_params[i].R0);
    }
