...
void x_Gibja_rohf() {
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (row = 0; row < G.params->rowtot[h]; row++) {
            ...
            I = L1A.params->rowidx[i];
            Isym = L1A.params->psym[i];
            a = G.params->roworb[h][row][1];
            A = L1A.params->colidx[a];
            Asym = L1A.params->qsym[a];
    ...
    if (!params.connect_xi) {
        for (h = 0; h < nirreps; h++) {
            ...
            for (row = 0; row < G.params->rowtot[h]; row++) {
                i = G.params->roworb[h][row][0];
                I = I1A.params->rowidx[i];
                Isym = I1A.params->psym[i];
                a = G.params->roworb[h][row][1];
                A = I1A.params->colidx[a];
                Asym = I1A.params->qsym[a];
                for (col = 0; col < G.params->coltot[h ^ G_irr]; col++) {
                    j = G.params->colorb[h ^ G_irr][col][0];
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (row = 0; row < G.params->rowtot[h]; row++) {
            i = G.params->roworb[h][row][0];
            I = L1B.params->rowidx[i];
            Isym = L1B.params->psym[i];
            a = G.params->roworb[h][row][1];
            A = L1B.params->colidx[a];
            Asym = L1B.params->qsym[a];
            for (col = 0; col < G.params->coltot[h ^ G_irr]; col++) {
                j = G.params->colorb[h ^ G_irr][col][0];
    ...
    if (!params.connect_xi) {
        for (h = 0; h < nirreps; h++) {
            ...
            for (row = 0; row < G.params->rowtot[h]; row++) {
                i = G.params->roworb[h][row][0];
                I = I1B.params->rowidx[i];
                Isym = I1B.params->psym[i];
                a = G.params->roworb[h][row][1];
                A = I1B.params->colidx[a];
                Asym = I1B.params->qsym[a];
                for (col = 0; col < G.params->coltot[h ^ G_irr]; col++) {
                    j = G.params->colorb[h ^ G_irr][col][0];
