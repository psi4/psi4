...
void x_Gciab_6() {
    ...
    global_dpd_->file2_mat_init(&LR1A);
    global_dpd_->file2_mat_init(&LR1B);
    global_dpd_->file2_mat_rd(&T1A);
    global_dpd_->file2_mat_rd(&T1B);
    global_dpd_->file2_mat_rd(&LR1A);
    global_dpd_->file2_mat_rd(&LR1B);
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (row = 0; row < G.params->rowtot[h]; row++) {
            ...
            C = LR1A.params->rowidx[c];
            Csym = LR1A.params->psym[c];
            ...
            for (col = 0; col < G.params->coltot[h]; col++) {
                ...
                A = LR1A.params->colidx[a];
                Asym = LR1A.params->qsym[a];
                B = T1A.params->colidx[b];
                Bsym = T1A.params->qsym[b];
                AA = T1A.params->colidx[a];
                AAsym = T1A.params->qsym[a];
                BB = LR1A.params->colidx[b];
                BBsym = LR1A.params->qsym[b];
                if (((Csym ^ Asym) == G_irr) && (Isym == Bsym))
                    G.matrix[h][row][col] += LR1A.matrix[Csym][C][A] * T1A.matrix[Isym][I][B];
                if (((Csym ^ BBsym) == G_irr) && (Isym == AAsym))
                    G.matrix[h][row][col] -= LR1A.matrix[Csym][C][BB] * T1A.matrix[Isym][I][AA];
            }
        }
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (row = 0; row < G.params->rowtot[h]; row++) {
            c = G.params->roworb[h][row][0];
            i = G.params->roworb[h][row][1];
            C = LR1B.params->rowidx[c];
            Csym = LR1B.params->psym[c];
            I = T1B.params->rowidx[i];
            Isym = T1B.params->psym[i];
            for (col = 0; col < G.params->coltot[h ^ G_irr]; col++) {
                a = G.params->colorb[h ^ G_irr][col][0];
                b = G.params->colorb[h ^ G_irr][col][1];
                A = LR1B.params->colidx[a];
                Asym = LR1B.params->qsym[a];
                B = T1B.params->colidx[b];
                Bsym = T1B.params->qsym[b];
                AA = T1B.params->colidx[a];
                AAsym = T1B.params->qsym[a];
                BB = LR1B.params->colidx[b];
                BBsym = LR1B.params->qsym[b];
                if (((Csym ^ Asym) == G_irr) && (Isym == Bsym))
                    G.matrix[h][row][col] += LR1B.matrix[Csym][C][A] * T1B.matrix[Isym][I][B];
                if (((Csym ^ BBsym) == G_irr) && (Isym == AAsym))
                    G.matrix[h][row][col] -= LR1B.matrix[Csym][C][BB] * T1B.matrix[Isym][I][AA];
            }
        }
        global_dpd_->buf4_mat_irrep_wrt(&G, h);
        global_dpd_->buf4_mat_irrep_close(&G, h);
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (row = 0; row < G.params->rowtot[h]; row++) {
            ...
            for (col = 0; col < G.params->coltot[h ^ G_irr]; col++) {
                ...
                Bsym = T1B.params->qsym[b];
                if (((Csym ^ Asym) == G_irr) && (Isym == Bsym))
                    G.matrix[h][row][col] += LR1A.matrix[Csym][C][A] * T1B.matrix[Isym][I][B];
            }
        }
        global_dpd_->buf4_mat_irrep_wrt(&G, h);
        global_dpd_->buf4_mat_irrep_close(&G, h);
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (row = 0; row < G.params->rowtot[h]; row++) {
            ...
            for (col = 0; col < G.params->coltot[h ^ G_irr]; col++) {
                ...
                Bsym = T1A.params->qsym[b];
                if (((Csym ^ Asym) == G_irr) && (Isym == Bsym))
                    G.matrix[h][row][col] += LR1B.matrix[Csym][C][A] * T1A.matrix[Isym][I][B];
            }
        }
        global_dpd_->buf4_mat_irrep_wrt(&G, h);
        global_dpd_->buf4_mat_irrep_close(&G, h);
...
void x_Gciab_7() {
    ...
    global_dpd_->file2_mat_init(&LT1A);
    global_dpd_->file2_mat_init(&LT1B);
    global_dpd_->file2_mat_rd(&R1A);
    global_dpd_->file2_mat_rd(&R1B);
    global_dpd_->file2_mat_rd(&LT1A);
    global_dpd_->file2_mat_rd(&LT1B);
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (row = 0; row < G.params->rowtot[h]; row++) {
            c = G.params->roworb[h][row][0];
            i = G.params->roworb[h][row][1];
            C = LT1A.params->rowidx[c];
            Csym = LT1A.params->psym[c];
            I = R1A.params->rowidx[i];
            Isym = R1A.params->psym[i];
            for (col = 0; col < G.params->coltot[h ^ G_irr]; col++) {
                a = G.params->colorb[h ^ G_irr][col][0];
                b = G.params->colorb[h ^ G_irr][col][1];
                A = LT1A.params->colidx[a];
                Asym = LT1A.params->qsym[a];
                B = R1A.params->colidx[b];
                Bsym = R1A.params->qsym[b];
                AA = R1A.params->colidx[a];
                AAsym = R1A.params->qsym[a];
                BB = LT1A.params->colidx[b];
                BBsym = LT1A.params->qsym[b];
                if (((Csym ^ Asym) == L_irr) && ((Isym ^ Bsym) == R_irr))
                    G.matrix[h][row][col] += LT1A.matrix[Csym][C][A] * R1A.matrix[Isym][I][B];
                if (((Csym ^ BBsym) == L_irr) && ((Isym ^ AAsym) == R_irr))
                    G.matrix[h][row][col] -= LT1A.matrix[Csym][C][BB] * R1A.matrix[Isym][I][AA];
            }
        }
        global_dpd_->buf4_mat_irrep_wrt(&G, h);
        global_dpd_->buf4_mat_irrep_close(&G, h);
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (row = 0; row < G.params->rowtot[h]; row++) {
            c = G.params->roworb[h][row][0];
            i = G.params->roworb[h][row][1];
            C = LT1B.params->rowidx[c];
            Csym = LT1B.params->psym[c];
            I = R1B.params->rowidx[i];
            Isym = R1B.params->psym[i];
            for (col = 0; col < G.params->coltot[h ^ G_irr]; col++) {
                a = G.params->colorb[h ^ G_irr][col][0];
                b = G.params->colorb[h ^ G_irr][col][1];
                A = LT1B.params->colidx[a];
                Asym = LT1B.params->qsym[a];
                B = R1B.params->colidx[b];
                Bsym = R1B.params->qsym[b];
                AA = R1B.params->colidx[a];
                AAsym = R1B.params->qsym[a];
                BB = LT1B.params->colidx[b];
                BBsym = LT1B.params->qsym[b];
                if (((Csym ^ Asym) == L_irr) && ((Isym ^ Bsym) == R_irr))
                    G.matrix[h][row][col] += LT1B.matrix[Csym][C][A] * R1B.matrix[Isym][I][B];
                if (((Csym ^ BBsym) == L_irr) && ((Isym ^ AAsym) == R_irr))
                    G.matrix[h][row][col] -= LT1B.matrix[Csym][C][BB] * R1B.matrix[Isym][I][AA];
            }
        }
        global_dpd_->buf4_mat_irrep_wrt(&G, h);
        global_dpd_->buf4_mat_irrep_close(&G, h);
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (row = 0; row < G.params->rowtot[h]; row++) {
            ...
            for (col = 0; col < G.params->coltot[h ^ G_irr]; col++) {
                ...
                Bsym = R1B.params->qsym[b];
                if (((Csym ^ Asym) == L_irr) && ((Isym ^ Bsym) == R_irr))
                    G.matrix[h][row][col] += LT1A.matrix[Csym][C][A] * R1B.matrix[Isym][I][B];
            }
        }
        global_dpd_->buf4_mat_irrep_wrt(&G, h);
        global_dpd_->buf4_mat_irrep_close(&G, h);
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (row = 0; row < G.params->rowtot[h]; row++) {
            ...
            for (col = 0; col < G.params->coltot[h ^ G_irr]; col++) {
                ...
                Bsym = R1A.params->qsym[b];
                if (((Csym ^ Asym) == L_irr) && ((Isym ^ Bsym) == R_irr))
                    G.matrix[h][row][col] += LT1B.matrix[Csym][C][A] * R1A.matrix[Isym][I][B];
            }
        }
        global_dpd_->buf4_mat_irrep_wrt(&G, h);
        global_dpd_->buf4_mat_irrep_close(&G, h);
