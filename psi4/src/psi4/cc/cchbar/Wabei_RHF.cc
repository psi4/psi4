...
void Wabei_RHF() {
        ...
        for (Ge = 0; Ge < moinfo.nirreps; Ge++) {
            ...
            if (nrows && ncols && nlinks) {
                for (E = 0; E < moinfo.virtpi[Ge]; E++) {
                    ...
                    C_DGEMM('n', 'n', nrows, ncols, nlinks, 0.5, T1.matrix[Gi][0], nlinks, B.matrix[Gef][0], ncols, 0.0,
                            Z1.matrix[Gei][0], ncols);
    ...
    for (Gei = 0; Gei < moinfo.nirreps; Gei++) {
        ...
        if (nrows && ncols) {
            ...
            for (ei = 0; ei < W.params->rowtot[Gei]; ei++) {
                ...
                C_DGEMV('t', nrows, ncols, 1, T.matrix[Gei][0], ncols, Z.matrix[Gei][ei], 1, 1, W.matrix[Gei][0], 1);
    ...
    ** ovvv quantities on disk besides the <ia|bc> integrals and the Wabei target. */
    if (!params.wabei_lowdisk) {
        ...
        if (incore)
            global_dpd_->contract444(&Z, &F, &W, 0, 1, 1, 0);
        else
            ZFW(&Z, &F, &W, 1, 0);
        global_dpd_->buf4_close(&Z);
        global_dpd_->buf4_close(&F);
        ...
        if (incore)
            global_dpd_->contract444(&Z, &F, &W, 0, 1, -1, 1);
        else
            ZFW(&Z, &F, &W, -1, 1);
        global_dpd_->buf4_close(&Z);
        global_dpd_->buf4_close(&F);
    ...
    ** ovvv terms beyond <ia|bc> integrals and the WAbEi target, but it's very slow. */
        ...
        for (Gei = 0; Gei < moinfo.nirreps; Gei++) global_dpd_->buf4_mat_irrep_row_init(&W, Gei);
        ...
        global_dpd_->buf4_close(&Z);
        global_dpd_->buf4_close(&F);
        for (Gei = 0; Gei < moinfo.nirreps; Gei++) global_dpd_->buf4_mat_irrep_row_close(&W, Gei);
        global_dpd_->buf4_close(&W);
    }
    ...
    for (Gei = 0; Gei < moinfo.nirreps; Gei++) {
        ...
        for (ei = 0; ei < Z.params->rowtot[Gei]; ei++) {
            ...
            for (Gm = 0; Gm < moinfo.nirreps; Gm++) {
                ...
                if (nrows && ncols && nlinks)
                    C_DGEMM('t', 'n', nrows, ncols, nlinks, -1, T1.matrix[Gm][0], nrows, &(Z.matrix[Gei][ei][mb]),
                            ncols, 1, &(W.matrix[Gei][0][ab]), ncols);
            }
            global_dpd_->buf4_mat_irrep_row_wrt(&W, Gei, ei);
        }
        global_dpd_->buf4_mat_irrep_close(&Z, Gei);
    ...
    for (Gei = 0; Gei < moinfo.nirreps; Gei++) {
        ...
        for (ei = 0; ei < Z.params->rowtot[Gei]; ei++) {
            ...
            for (Gm = 0; Gm < moinfo.nirreps; Gm++) {
                ...
                ab = W.col_offset[Gei][Ga];
                if (nrows && ncols && nlinks)
                    C_DGEMM('n', 'n', nrows, ncols, nlinks, 1, &(Z.matrix[Gei][ei][am]), nlinks, T1.matrix[Gm][0],
                            ncols, 1, &(W.matrix[Gei][0][ab]), ncols);
            }
            global_dpd_->buf4_mat_irrep_row_wrt(&W, Gei, ei);
        }
...
void ZFW(dpdbuf4 *Z, dpdbuf4 *F, dpdbuf4 *W, double alpha, double beta) {
    ...
    for (Gib = 0; Gib < moinfo.nirreps; Gib++) {
        ...
        if (rows_per_bucket > F->params->rowtot[Gib]) rows_per_bucket = F->params->rowtot[Gib];
        ...
        for (m = 0; m < (rows_left ? nbuckets - 1 : nbuckets); m++) {
            ...
            if (beta == 1.0) global_dpd_->buf4_mat_irrep_rd_block(W, Gib, W_row_start, rows_per_bucket);
            ...
            for (n = 0; n < (rows_left ? nbuckets - 1 : nbuckets); n++) {
                ...
                if (nrows && ncols && nlinks)
                    C_DGEMM('n', 'n', nrows, ncols, nlinks, alpha, &(Z->matrix[Gib][W_row_start][F_row_start]),
                            Z->params->coltot[Gmf], F->matrix[Gmf][0], ncols, 1, W->matrix[Gib][0], ncols);
            }
            if (rows_left) {
                F_row_start = n * rows_per_bucket;
        ...
        if (rows_left) {
            ...
            if (beta == 1.0) global_dpd_->buf4_mat_irrep_rd_block(W, Gib, W_row_start, rows_left);
