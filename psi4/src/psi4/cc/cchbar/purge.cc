...
void purge() {
    ...
    for (h = 0; h < nirreps; h++) {
        for (m = 0; m < occpi[h]; m++)
            for (e = (virtpi[h] - openpi[h]); e < virtpi[h]; e++) FME.matrix[h][m][e] = 0.0;
    }
    ...
    for (h = 0; h < nirreps; h++) {
        for (e = 0; e < virtpi[h]; e++)
            for (m = (occpi[h] - openpi[h]); m < occpi[h]; m++) Fme.matrix[h][m][e] = 0.0;
    }
    global_dpd_->file2_mat_wrt(&Fme);
    global_dpd_->file2_mat_close(&Fme);
    global_dpd_->file2_close(&Fme);
    ...
    for (h = 0; h < nirreps; h++) {
        for (i = 0; i < occpi[h]; i++)
            for (j = (occpi[h] - openpi[h]); j < occpi[h]; j++) Fmi.matrix[h][i][j] = 0.0;

        for (i = (occpi[h] - openpi[h]); i < occpi[h]; i++)
            for (j = 0; j < occpi[h]; j++) Fmi.matrix[h][i][j] = 0.0;
    }
    ...
    for (h = 0; h < nirreps; h++) {
        for (a = 0; a < virtpi[h]; a++)
            for (b = (virtpi[h] - openpi[h]); b < virtpi[h]; b++) FAE.matrix[h][a][b] = 0.0;

        for (a = (virtpi[h] - openpi[h]); a < virtpi[h]; a++)
            for (b = 0; b < virtpi[h]; b++) FAE.matrix[h][a][b] = 0.0;
    }
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (mn = 0; mn < W.params->rowtot[h]; mn++) {
            ...
            for (ij = 0; ij < W.params->coltot[h]; ij++) {
                ...
                I = i - occ_off[isym];
                J = j - occ_off[jsym];
                if ((I >= (occpi[isym] - openpi[isym])) || (J >= (occpi[jsym] - openpi[jsym])) ||
                    (M >= (occpi[msym] - openpi[msym])) || (N >= (occpi[nsym] - openpi[nsym])))
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (mn = 0; mn < W.params->rowtot[h]; mn++) {
            ...
            for (ij = 0; ij < W.params->coltot[h]; ij++) {
                ...
                jsym = W.params->ssym[j];
                J = j - occ_off[jsym];
                if ((J >= (occpi[jsym] - openpi[jsym])) || (N >= (occpi[nsym] - openpi[nsym])))
                    W.matrix[h][mn][ij] = 0.0;
            }
        }
        global_dpd_->file4_mat_irrep_wrt(&W, h);
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (me = 0; me < W.params->rowtot[h]; me++) {
            ...
            for (jb = 0; jb < W.params->coltot[h]; jb++) {
                ...
                B = b - vir_off[bsym];
                if ((E >= (virtpi[esym] - openpi[esym])) || (B >= (virtpi[bsym] - openpi[bsym])))
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (me = 0; me < W.params->rowtot[h]; me++) {
            ...
            for (jb = 0; jb < W.params->coltot[h]; jb++) {
                ...
                J = j - occ_off[jsym];
                if ((M >= (occpi[msym] - openpi[msym])) || (J >= (occpi[jsym] - openpi[jsym])))
                    W.matrix[h][me][jb] = 0.0;
            }
        }
        global_dpd_->file4_mat_irrep_wrt(&W, h);
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (me = 0; me < W.params->rowtot[h]; me++) {
            ...
            for (jb = 0; jb < W.params->coltot[h]; jb++) {
                ...
                jsym = W.params->rsym[j];
                J = j - occ_off[jsym];
                if ((E >= (virtpi[esym] - openpi[esym])) || (J >= (occpi[jsym] - openpi[jsym])))
                    W.matrix[h][me][jb] = 0.0;
            }
        }
        global_dpd_->file4_mat_irrep_wrt(&W, h);
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (me = 0; me < W.params->rowtot[h]; me++) {
            ...
            for (jb = 0; jb < W.params->coltot[h]; jb++) {
                ...
                bsym = W.params->ssym[b];
                B = b - vir_off[bsym];
                if ((M >= (occpi[msym] - openpi[msym])) || (B >= (virtpi[bsym] - openpi[bsym])))
                    W.matrix[h][me][jb] = 0.0;
            }
        }
        global_dpd_->file4_mat_irrep_wrt(&W, h);
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (me = 0; me < W.params->rowtot[h]; me++) {
            ...
            for (jb = 0; jb < W.params->coltot[h]; jb++) {
                ...
                J = j - occ_off[jsym];
                B = b - vir_off[bsym];
                if ((M >= (occpi[msym] - openpi[msym])) || (E >= (virtpi[esym] - openpi[esym])) ||
                    (J >= (occpi[jsym] - openpi[jsym])) || (B >= (virtpi[bsym] - openpi[bsym])))
                    W.matrix[h][me][jb] = 0.0;
            }
        }
        global_dpd_->file4_mat_irrep_wrt(&W, h);
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (ma = 0; ma < W.params->rowtot[h]; ma++) {
            ...
            for (ef = 0; ef < W.params->coltot[h]; ef++) {
                ...
                E = e - vir_off[esym];
                F = f - vir_off[fsym];
                if ((A >= (virtpi[asym] - openpi[asym])) || (E >= (virtpi[esym] - openpi[esym])) ||
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (ma = 0; ma < W.params->rowtot[h]; ma++) {
            ...
            for (ef = 0; ef < W.params->coltot[h]; ef++) {
                ...
                esym = W.params->rsym[e];
                E = e - vir_off[esym];
                if ((A >= (virtpi[asym] - openpi[asym])) || (M >= (occpi[msym] - openpi[msym])) ||
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (ma = 0; ma < W.params->rowtot[h]; ma++) {
            for (ef = 0; ef < W.params->coltot[h]; ef++) {
                ...
                fsym = W.params->ssym[f];
                F = f - vir_off[fsym];
                if (F >= (virtpi[fsym] - openpi[fsym])) W.matrix[h][ma][ef] = 0.0;
            }
        }
        global_dpd_->file4_mat_irrep_wrt(&W, h);
        global_dpd_->file4_mat_irrep_close(&W, h);
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (mb = 0; mb < W.params->rowtot[h]; mb++) {
            ...
            for (ij = 0; ij < W.params->coltot[h]; ij++) {
                ...
                I = i - occ_off[isym];
                J = j - occ_off[jsym];
                if ((M >= (occpi[msym] - openpi[msym])) || (I >= (occpi[isym] - openpi[isym])) ||
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (mb = 0; mb < W.params->rowtot[h]; mb++) {
            for (ij = 0; ij < W.params->coltot[h]; ij++) {
                ...
                jsym = W.params->ssym[j];
                J = j - occ_off[jsym];
                if (J >= (occpi[jsym] - openpi[jsym])) W.matrix[h][mb][ij] = 0.0;
            }
        }
        global_dpd_->file4_mat_irrep_wrt(&W, h);
        global_dpd_->file4_mat_irrep_close(&W, h);
    ...
    for (h = 0; h < W.params->nirreps; h++) {
        ...
        for (mn = 0; mn < W.params->rowtot[h]; mn++) {
            for (ei = 0; ei < W.params->coltot[h]; ei++) {
                ...
                esym = W.params->rsym[e];
                E = e - vir_off[esym];
                if (E >= (virtpi[esym] - openpi[esym])) W.matrix[h][mn][ei] = 0.0;
            }
        }
        global_dpd_->file4_mat_irrep_wrt(&W, h);
        global_dpd_->file4_mat_irrep_close(&W, h);
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (mn = 0; mn < W.params->rowtot[h]; mn++) {
            ...
            for (ei = 0; ei < W.params->coltot[h]; ei++) {
                ...
                isym = W.params->ssym[i];
                I = i - occ_off[isym];
                if ((M >= (occpi[msym] - openpi[msym])) || (N >= (occpi[nsym] - openpi[nsym])) ||
                    (I >= (occpi[isym] - openpi[isym])))
                    W.matrix[h][mn][ei] = 0.0;
            }
        }
