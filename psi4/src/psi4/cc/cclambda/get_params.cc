...
void CCLambdaWavefunction::get_params(Options &options) {
    ...
    if (lambda_and_Ls) {
        ...
        if (prop_all) {
            for (i = 0; i < moinfo.nirreps; ++i) params.nstates += states_per_irrep[i]; /* do all L(s) */
        } else {
    ...
    for (i = 0; i < params.nstates; ++i) {
        outfile->Printf("\tLabels for eigenvector %d:\n\t%s, %s, %s, %s, %s, %s\n", i + 1, pL_params[i].L1A_lbl,
                        pL_params[i].L1B_lbl, pL_params[i].L2AA_lbl, pL_params[i].L2BB_lbl, pL_params[i].L2AB_lbl,
                        pL_params[i].L2RHF_lbl);
    }
