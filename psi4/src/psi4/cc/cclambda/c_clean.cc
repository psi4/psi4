...
void c_clean(dpdfile2 *CME, dpdfile2 *Cme, dpdbuf4 *CMNEF, dpdbuf4 *Cmnef, dpdbuf4 *CMnEf) {
    ...
    for (h = 0; h < nirreps; h++) {
        for (m = 0; m < occpi[h]; m++)
            for (e = (virtpi[h ^ C_irr] - openpi[h ^ C_irr]); e < virtpi[h ^ C_irr]; e++) CME->matrix[h][m][e] = 0.0;
    }
    ...
    for (h = 0; h < nirreps; h++) {
        for (m = (occpi[h] - openpi[h]); m < occpi[h]; m++)
            for (e = 0; e < virtpi[h ^ C_irr]; e++) Cme->matrix[h][m][e] = 0.0;
    }
    global_dpd_->file2_mat_wrt(Cme);
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (mn = 0; mn < CMNEF->params->rowtot[h]; mn++) {
            for (ef = 0; ef < CMNEF->params->coltot[h ^ C_irr]; ef++) {
                ...
                E = e - vir_off[esym];
                F = f - vir_off[fsym];
                if ((E >= (virtpi[esym] - openpi[esym])) || (F >= (virtpi[fsym] - openpi[fsym])))
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (mn = 0; mn < CMnEf->params->rowtot[h]; mn++) {
            ...
            for (ef = 0; ef < CMnEf->params->coltot[h ^ C_irr]; ef++) {
                ...
                esym = CMnEf->params->rsym[e];
                E = e - vir_off[esym];
                if ((N >= (occpi[nsym] - openpi[nsym])) || (E >= (virtpi[esym] - openpi[esym])))
                    CMnEf->matrix[h][mn][ef] = 0.0;
            }
        }
        global_dpd_->buf4_mat_irrep_wrt(CMnEf, h);
