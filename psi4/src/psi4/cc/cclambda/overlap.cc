        ...
        for (row = 0; row < L2.params->rowtot[h]; row++) {
            ...
            for (col = 0; col < L2.params->coltot[h ^ L_irr]; col++) {
                ...
                if ((Isym == Asym) && (Jsym == Bsym))
                    ST12AA += L2.matrix[h][row][col] * T1A.matrix[Isym][I][A] * T1A.matrix[Jsym][J][B];
                if ((Isym == Bsym) && (Jsym == Asym))
                    ST12AA -= L2.matrix[h][row][col] * T1A.matrix[Isym][I][B] * T1A.matrix[Jsym][J][A];
            }
        }
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (row = 0; row < L2.params->rowtot[h]; row++) {
            ...
            for (col = 0; col < L2.params->coltot[h ^ L_irr]; col++) {
                ...
                if ((Isym == Asym) && (Jsym == Bsym))
                    ST12BB += L2.matrix[h][row][col] * T1B.matrix[Isym][I][A] * T1B.matrix[Jsym][J][B];
                if ((Isym == Bsym) && (Jsym == Asym))
                    ST12BB -= L2.matrix[h][row][col] * T1B.matrix[Isym][I][B] * T1B.matrix[Jsym][J][A];
            }
        }
        global_dpd_->buf4_mat_irrep_close(&L2, h);
    }
    ...
    for (h = 0; h < nirreps; h++) {
        ...
        for (row = 0; row < L2.params->rowtot[h]; row++) {
            ...
            for (col = 0; col < L2.params->coltot[h ^ L_irr]; col++) {
                ...
                Bsym = T1B.params->qsym[b];
                if ((Isym == Asym) && (Jsym == Bsym))
                    ST12AB += L2.matrix[h][row][col] * T1A.matrix[Isym][I][A] * T1B.matrix[Jsym][J][B];
            }
        }
        global_dpd_->buf4_mat_irrep_close(&L2, h);
    }
