    ...
    for (int h = 0; h < nirreps; h++) {
        if (T1.params->rowtot[h]) {
            ...
            if (T1.params->rowtot[h] && T1.params->coltot[h]) {
                C_DGEMM('n', 't', T1.params->rowtot[h], T1.params->rowtot[h], T1.params->coltot[h], 1.0,
                        T1.matrix[h][0], T1.params->coltot[h], T1.matrix[h][0], T1.params->coltot[h], 0.0, T[0],
                        T1.params->rowtot[h]);
            }
            /*
            ...
            */
            ...
            if (DSYEV_eigval_asc(T1.params->rowtot[h], T, E) != 0){
...
static double d1diag_subblock(double **Tave, int row0, int rown, int col0, int coln) {
    ...
    if (nrow && ncol) {
        ...
        C_DGEMM('n', 't', nrow, nrow, ncol, 1.0, Tsub[0], ncol, Tsub[0], ncol, 0.0, Tsq[0], nrow);
        /* newmm(Tsub, 0, Tsub, 1, Tsq, nrow, ncol, nrow, 1.0, 0.0); */

        E = init_array(nrow);
        if (DSYEV_eigval_asc(nrow, Tsq, E) != 0){
            throw PSIEXCEPTION("DSYEV diagonalizer failed in D1 diagnostic!");
        }
