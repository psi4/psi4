...
double cc2_LHX1Y1(const char *pert_x, int irrep_x, double omega_x, const char *pert_y, int irrep_y, double omega_y) {
    ...
    for (Gam = 0; Gam < moinfo.nirreps; Gam++) {
        ...
        for (am = 0; am < W.params->rowtot[Gam]; am++) {
            ...
            if (nrows && ncols)
                C_DGEMV('n', nrows, ncols, 1.0, &Xt[W.col_offset[Gam][Ge]], ncols, X1.matrix[Gm][M], 1, 1.0,
                        Zae.matrix[Ga][A], 1);
        }
    ...
    for (Gam = 0; Gam < moinfo.nirreps; Gam++) {
        ...
        for (am = 0; am < W.params->rowtot[Gam]; am++) {
            ...
            ncols = moinfo.virtpi[Gf];
            if (nrows && ncols)
                C_DGEMV('n', nrows, ncols, 1.0, &Xt[W.col_offset[Gam][Ge]], ncols, Y1.matrix[Gm][M], 1, 1.0,
                        Zae.matrix[Ga][A], 1);
        }
        ...
        for (Ge = 0; Ge < moinfo.nirreps; Ge++) {
            ...
            if (nrows && ncols && nlinks) {
                for (E = 0; E < moinfo.virtpi[Ge]; E++) {
                    ...
                    C_DGEMM('n', 'n', nrows, ncols, nlinks, 0.5, Y1.matrix[Gi][0], nlinks, B.matrix[Gef][0], ncols, 0.0,
        ...
        for (ab = 0; ab < Z.params->rowtot[hzbuf]; ab++) {
            ...
            for (Gj = 0; Gj < moinfo.nirreps; Gj++) {
                ...
                if (rowz && colz && rowx && colx) {
                    C_DGEMM('n', 'n', rowz, colz, rowx, 1.0, &(X1.matrix[Gi][0][0]), rowx,
