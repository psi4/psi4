...
void DFOCC::mp3_pdm_3index_intr() {
    ...
    if (reference_ == "RESTRICTED") {
        ...
        T = std::make_shared<Tensor2d>("U2 <IJ|AB>", naoccA, naoccA, navirA, navirA);
        T->sort(1324, U, 1.0, 0.0);
        U.reset();
        L = std::make_shared<Tensor2d>("T2_1 <IJ|AB>", naoccA, naoccA, navirA, navirA);
        ...
        ccsd_u2_amps(U, T1);
        T = std::make_shared<Tensor2d>("U2_1 <IJ|AB>", naoccA, naoccA, navirA, navirA);
        T->sort(1324, U, 1.0, 0.0);
        U.reset();
        L = std::make_shared<Tensor2d>("T2_2 <IJ|AB>", naoccA, naoccA, navirA, navirA);
        ...
        */
        ...
        L = std::make_shared<Tensor2d>("T2_1 <Ij|Ab>", naoccA, naoccB, navirA, navirB);
        ...
        X = std::make_shared<Tensor2d>("T2_1 <Ij|bA>", naoccA, naoccB, navirB, navirA);
        X->sort(1243, L, 1.0, 0.0);
        L.reset();
        Y = std::make_shared<Tensor2d>("T2 <Ij|bA>", naoccA, naoccB, navirB, navirA);
        ...
        T->read(psio_, PSIF_DFOCC_AMPS);
        L = std::make_shared<Tensor2d>("T2_2 <Ij|Ab>", naoccA, naoccB, navirA, navirB);
        L->read(psio_, PSIF_DFOCC_AMPS);
        if (wfn_type_ == "DF-OMP3" || wfn_type_ == "CD-OMP3") {
            GijA->contract(false, true, naoccA, naoccA, naoccB * navirA * navirB, T, L, 1.0, 1.0);
        }
        ...
        X = std::make_shared<Tensor2d>("T2_2 <Ij|bA>", naoccA, naoccB, navirB, navirA);
        X->sort(1243, L, 1.0, 0.0);
        L.reset();
        Y = std::make_shared<Tensor2d>("T2_1 <Ij|bA>", naoccA, naoccB, navirB, navirA);
        Y->sort(1243, T, 1.0, 0.0);
        T.reset();
        if (wfn_type_ == "DF-OMP3" || wfn_type_ == "CD-OMP3") {
            GabA->contract(true, false, navirA, navirA, naoccA * naoccB * navirB, X, Y, -1.0, 1.0);
        ...
        T->read_anti_symm(psio_, PSIF_DFOCC_AMPS);
        L = std::make_shared<Tensor2d>("T2_1 <ij|ab>", naoccB, naoccB, navirB, navirB);
        L->read_anti_symm(psio_, PSIF_DFOCC_AMPS);
        ...
        X = std::make_shared<Tensor2d>("T2 <jI|Ab>", naoccB, naoccA, navirA, navirB);
        X->sort(2134, T, 1.0, 0.0);
        T.reset();
        Y = std::make_shared<Tensor2d>("T2_1 <jI|Ab>", naoccB, naoccA, navirA, navirB);
        ...
        X = std::make_shared<Tensor2d>("T2_1 <jI|Ab>", naoccB, naoccA, navirA, navirB);
        X->sort(2134, T, 1.0, 0.0);
        T.reset();
        Y = std::make_shared<Tensor2d>("T2_2 <jI|Ab>", naoccB, naoccA, navirA, navirB);
        Y->sort(2134, L, 1.0, 0.0);
        L.reset();
        if (wfn_type_ == "DF-OMP3" || wfn_type_ == "CD-OMP3") {
            GijB->contract(false, true, naoccB, naoccB, naoccA * navirB * navirA, X, Y, 1.0, 1.0);
        ...
        TAA = std::make_shared<Tensor2d>("T2 <IJ|AB>", naoccA, naoccA, navirA, navirA);
        ...
        TAB = std::make_shared<Tensor2d>("T2 <Ij|Ab>", naoccA, naoccB, navirA, navirB);
        ...
        U.reset();
        // T(Q,ia) <= \sum_{jb} b(Q,jb) T(jb,ia)
        TBB = std::make_shared<Tensor2d>("T2 <ij|ab>", naoccB, naoccB, navirB, navirB);
        ...
        U = std::make_shared<Tensor2d>("T2_1 <IJ|AB>", naoccA, naoccA, navirA, navirA);
        U->read_anti_symm(psio_, PSIF_DFOCC_AMPS);
        T = std::make_shared<Tensor2d>("T2_1 (IB|JA)", naoccA, navirA, naoccA, navirA);
        T->sort(1324, U, 1.0, 0.0);
        U.reset();
        X = std::make_shared<Tensor2d>("X (IB|JA)", naoccA, navirA, naoccA, navirA);
        X->gemm(false, false, T, T, 0.5, 0.0);
        ...
        U = std::make_shared<Tensor2d>("T2_1 <Ij|Ab>", naoccA, naoccB, navirA, navirB);
        U->read(psio_, PSIF_DFOCC_AMPS);
        T = std::make_shared<Tensor2d>("T2_1 (IA|jb)", naoccA, navirA, naoccB, navirB);
        T->sort(1324, U, 1.0, 0.0);
        U.reset();
        ...
        U = std::make_shared<Tensor2d>("T2_1 <ij|ab>", naoccB, naoccB, navirB, navirB);
        U->read_anti_symm(psio_, PSIF_DFOCC_AMPS);
        T = std::make_shared<Tensor2d>("T2_1 (ib|ja)", naoccB, navirB, naoccB, navirB);
        T->sort(1324, U, 1.0, 0.0);
        U.reset();
        ...
        // V_ij^Q' <= \sum_{ef} V_iejf b_ef^Q
        Vij = std::make_shared<Tensor2d>("Vp (Q|ij)", nQ, naoccB, naoccB);
        ...
        // y_ia^Q <= 2 \sum(me) V_iema b_me^Q
        // Y(me,ia) = V(ie,ma)
        Y = std::make_shared<Tensor2d>("Y (me|ia)", naoccB, navirB, naoccB, navirB);
        Y->sort(3214, V, 1.0, 0.0);
        ...
        U = std::make_shared<Tensor2d>("T2_1 <Ij|Ab>", naoccA, naoccB, navirA, navirB);
        U->read(psio_, PSIF_DFOCC_AMPS);
        T = std::make_shared<Tensor2d>("T2p_1 (Ib|jA)", naoccA, navirB, naoccB, navirA);
        ...
        X = std::make_shared<Tensor2d>("X (ef|IJ)", navirB, navirB, naoccA, naoccA);
        X->sort(2413, V, 1.0, 0.0);
        ...
        Vij = std::make_shared<Tensor2d>("Vp (Q|ij)", nQ, naoccB, naoccB);
        Vij->read(psio_, PSIF_DFOCC_AMPS);
        X = std::make_shared<Tensor2d>("X (EF|ij)", navirA, navirA, naoccB, naoccB);
        X->sort(2413, V, 1.0, 0.0);
        Vij->gemm(false, false, bQabA, X, 1.0, 1.0);
        X.reset();
        Vij->write(psio_, PSIF_DFOCC_AMPS);
        ...
        U = std::make_shared<Tensor2d>("T2_1 <Ij|Ab>", naoccA, naoccB, navirA, navirB);
        U->read(psio_, PSIF_DFOCC_AMPS);
        L = std::make_shared<Tensor2d>("T2_1 (IA|jb)", naoccA, navirA, naoccB, navirB);
        ...
        U = std::make_shared<Tensor2d>("T2_1 <ij|ab>", naoccB, naoccB, navirB, navirB);
        U->read_anti_symm(psio_, PSIF_DFOCC_AMPS);
        T = std::make_shared<Tensor2d>("T2_1 (ia|jb)", naoccB, navirB, naoccB, navirB);
        T->sort(1324, U, 1.0, 0.0);
        U.reset();
        ...
        // y_IA^Q += 2 \sum(me) V_IemA b_me^Q
        // Y(me,IA) = V(Ie,mA)
        Y = std::make_shared<Tensor2d>("Y (me|IA)", naoccB, navirB, naoccA, navirA);
        Y->sort(3214, V, 1.0, 0.0);
        Z = std::make_shared<Tensor2d>("Y (Q|IA)", nQ, naoccA, navirA);
        Z->read(psio_, PSIF_DFOCC_AMPS);
        ...
        // Y(ME,ia) = V(Ma,iE)
        Y = std::make_shared<Tensor2d>("Y (ME|ia)", naoccA, navirA, naoccB, navirB);
