...
void DFOCC::ccsd_pdm_3index_intr() {
    ...
    Y = std::make_shared<Tensor2d>("V (JI|KA)", naoccA, naoccA, naoccA, navirA);
    Y->contract(false, false, naoccA, naoccA * naoccA * navirA, navirA, t1A, X, 1.0, 0.0);
    X.reset();
    Vijka = std::make_shared<Tensor2d>("V (IJ|KA)", naoccA, naoccA, naoccA, navirA);
    ...
    Vab = std::make_shared<Tensor2d>("Vt (Q|AB)", nQ, navirA, navirA);
    U = std::make_shared<Tensor2d>("T1 (Q|IJ)", nQ, naoccA, naoccA);
    U->read(psio_, PSIF_DFOCC_AMPS);
    L = std::make_shared<Tensor2d>("T1 (Q|JI)", nQ, naoccA, naoccA);
    L->swap_3index_col(U);
    ...
    // Xeiak = Vieak
    X = std::make_shared<Tensor2d>("X (EI|AK)", navirA, naoccA, navirA, naoccA);
    X->sort(2134, V, 1.0, 0.0);
    Y = std::make_shared<Tensor2d>("V (JI|AK)", naoccA, naoccA, navirA, naoccA);
    Y->contract(false, false, naoccA, naoccA * naoccA * navirA, navirA, t1A, X, 1.0, 0.0);
    X.reset();
    Vijak = std::make_shared<Tensor2d>("V (IJ|AK)", naoccA, naoccA, navirA, naoccA);
    ...
    Tau = std::make_shared<Tensor2d>("2*Tau(ia,jb) - Tau(ib,ja)", naoccA, navirA, naoccA, navirA);
    Tau->read_symm(psio_, PSIF_DFOCC_AMPS);
    U = std::make_shared<Tensor2d>("2*Tau(ij,ab) - Tau(ji,ab)", naoccA, naoccA, navirA, navirA);
    U->sort(1324, Tau, 1.0, 0.0);
    Tau.reset();
    X = std::make_shared<Tensor2d>("Lambda <IE|AF>", naoccA, navirA, navirA, navirA);
    X->gemm(true, false, L, U, 1.0, 0.0);
    U.reset();
    Y = std::make_shared<Tensor2d>("Temp <EF|IA>", navirA, navirA, naoccA, navirA);
    Y->sort(2413, X, 1.0, 0.0);
    ...
    Y = std::make_shared<Tensor2d>("Temp <ME|AB>", naoccA, navirA, navirA, navirA);
    ...
    */
    ...
    U->sort(1324, Tau, 1.0, 0.0);
    Tau.reset();
    Ts = std::make_shared<Tensor2d>("(+)U' [I>=J|A>=B]", ntri_ijAA, ntri_abAA);
    Ta = std::make_shared<Tensor2d>("(-)U' [I>=J|A>=B]", ntri_ijAA, ntri_abAA);
    Ts->symm_row_packed4(U);
    Ta->antisymm_row_packed4(U);
    U.reset();

    // Symmetric & Anti-symmetric contributions
    Vs = std::make_shared<Tensor2d>("(+)L[I] (E, M>=N)", navirA, ntri_ijAA);
    Va = std::make_shared<Tensor2d>("(-)L[I] (E, M>=N)", navirA, ntri_ijAA);
    S = std::make_shared<Tensor2d>("S[I] (E, A>=F)", navirA, ntri_abAA);
    A = std::make_shared<Tensor2d>("A[I] (E, A>=F)", navirA, ntri_abAA);
    X = std::make_shared<Tensor2d>("X[I] (A,EF)", navirA, navirA, navirA);
    Y = std::make_shared<Tensor2d>("Eta2[I] (Q|A)", nQ, navirA);
    Z = std::make_shared<Tensor2d>("Eta2 (Q|IA)", nQ, naoccA, navirA);
    // Main loop
...
#pragma omp parallel for
    ...
    // Build tEta_ab^Q = \sum(me) L'_maeb b_me^Q
    // Symmetric & Anti-symmetric contributions
    Vs = std::make_shared<Tensor2d>("(+)L[A] (M, K>=N)", naoccA, ntri_ijAA);
    Va = std::make_shared<Tensor2d>("(-)L[A] (M, K>=N)", naoccA, ntri_ijAA);
    S = std::make_shared<Tensor2d>("S[A] (M, E>=B)", naoccA, ntri_abAA);
    A = std::make_shared<Tensor2d>("A[A] (M, E>=B)", naoccA, ntri_abAA);
    X = std::make_shared<Tensor2d>("X[A] (B,ME)", navirA, naoccA, navirA);
    Y = std::make_shared<Tensor2d>("Eta2[A] (Q|B)", nQ, navirA);
    Z = std::make_shared<Tensor2d>("Eta2 (Q|AB)", nQ, navirA, navirA);
    // Main loop
    for (int a = 0; a < navirA; ++a) {
// Form (+)L[a](m, k>=n)
