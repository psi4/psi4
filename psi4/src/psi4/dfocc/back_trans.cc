...
void DFOCC::back_trans() {
    ...
    if (reference_ == "RESTRICTED") {
        ...
        Gmo.reset();
        Gao = std::make_shared<Tensor2d>("RefSep 3-Index TPDM (Q|nn)", nQ_ref, nso_, nso_);
        ...
        bQso.reset();
        Jmhalf.reset();
        G = std::make_shared<Tensor2d>("2-Index RefSep TPDM (P|Q)", nQ_ref, nQ_ref);
        ...
        */
        ...
        Gmo.reset();
        Gao = std::make_shared<Tensor2d>("Correlation 3-Index TPDM (Q|nn)", nQ, nso_, nso_);
        ...
        bQso.reset();
        Jmhalf.reset();
        G = std::make_shared<Tensor2d>("2-Index Correlation TPDM (P|Q)", nQ, nQ);
        G->gemm(false, true, cQso, Gao, 0.5, 0.0);
        cQso.reset();
        Gao.reset();
        //G->write(psio_, PSIF_DFOCC_DENS);
