...
void DFOCC::get_moinfo() {
    ...
    /********************************************************************************************/
    // Build Hso
    Hso_ = std::make_shared<Matrix>("SO-basis One-electron Ints", nso_, nso_);
    Tso_ = std::make_shared<Matrix>("SO-basis Kinetic Energy Ints", nso_, nso_);
    Vso_ = std::make_shared<Matrix>("SO-basis Potential Energy Ints", nso_, nso_);
    Sso_ = std::make_shared<Matrix>("SO-basis Overlap Ints", nso_, nso_);
    Hso_->zero();
    Tso_->zero();
    Vso_->zero();
    Sso_->zero();
