...
//    outfile->Printf("starting REMP T2 amps\n");
    ...
    if (reference_ == "RESTRICTED") {
        ...
        // X(ia,jb) = \sum_{e} t_ij^ae F_be = \sum_{e} T(ia,je) F_be    //CSB Fock matrix contributions -> no scaling
        ...
        // t_ij^ab <= X(ia,jb) + X(jb,a) = 2Xt(ia,jb)
        // X(ia,jb) = -\sum_{m} t_mj^ab F_mi = -\sum_{m} F(m,i) T(ma,jb)  //CSB Fock matrix contributions -> no scaling
