...
void Tensor1d::print(std::string out_fname) {
    std::shared_ptr<psi::PsiOutStream> printer =
        (out_fname == "outfile" ? outfile : std::make_shared<PsiOutStream>(out_fname));
...
double Tensor1d::xay(const SharedTensor2d &a, const SharedTensor1d &y) {
    double value = 0.0;
    auto ay = std::make_shared<Tensor1d>(a->dim1_);
...
void Tensor2d::contract424(int target_x, int target_y, const SharedTensor2d &a, const SharedTensor2d &b, double alpha,
    ...
    // C(pq,rs) = \sum_{o} A(po,rs) B(o,q)
        ...
        auto temp1 = std::make_shared<Tensor2d>("temp1", a->d1_, a->d3_, a->d4_, a->d2_);
        auto temp2 = std::make_shared<Tensor2d>("temp2", d1_, d3_, d4_, d2_);
        ...
        temp1.reset();
        auto temp3 = std::make_shared<Tensor2d>("temp3", d1_, d2_, d3_, d4_);
    ...
    // C(pq,rs) = \sum_{o} A(pq,os) B(o,r)
        ...
        auto temp1 = std::make_shared<Tensor2d>("temp1", a->d1_, a->d2_, a->d4_, a->d3_);
        auto temp2 = std::make_shared<Tensor2d>("temp2", d1_, d2_, d4_, d3_);
...
void Tensor2d::contract442(int target_a, int target_b, const SharedTensor2d &a, const SharedTensor2d &b, double alpha,
    ...
    // C(p,q) = \sum_{rst} A(pr,st) B(rq,st)
        ...
        auto temp = std::make_shared<Tensor2d>("temp", b->d2_, b->d1_, b->d3_, b->d4_);
    ...
    // C(p,q) = \sum_{rst} A(pr,st) B(rs,qt)
        ...
        auto temp = std::make_shared<Tensor2d>("temp", b->d1_, b->d2_, b->d4_, b->d3_);
    ...
    // C(p,q) = \sum_{rst} A(rs,tp) B(rs,tq) = \sum_{rst} X(p,rst) B(rst,q)
        ...
        auto temp = std::make_shared<Tensor2d>("temp", a->d4_, a->d1_, a->d2_, a->d3_);
    ...
    // C(p,q) = \sum_{rst} A(rs,tp) B(rs,qt)
        ...
        auto temp1 = std::make_shared<Tensor2d>("temp", a->d4_, a->d1_, a->d2_, a->d3_);
        auto temp2 = std::make_shared<Tensor2d>("temp", b->d1_, b->d2_, b->d4_, b->d3_);
    ...
    // C(p,q) = \sum_{rst} A(rp,st) B(rq,st)
        ...
        auto temp1 = std::make_shared<Tensor2d>("temp1", a->d2_, a->d1_, a->d3_, a->d4_);
        auto temp2 = std::make_shared<Tensor2d>("temp2", b->d2_, b->d1_, b->d3_, b->d4_);
    ...
    // C(p,q) = \sum_{rst} A(rs,pt) B(rs,qt)
        ...
        auto temp1 = std::make_shared<Tensor2d>("temp1", a->d1_, a->d2_, a->d4_, a->d3_);
        auto temp2 = std::make_shared<Tensor2d>("temp2", b->d1_, b->d2_, b->d4_, b->d3_);
        temp1->sort(1243, a, 1.0, 0.0);
        temp2->sort(1243, b, 1.0, 0.0);
        if (m && n && k) {
            C_DGEMM(ta, tb, m, n, k, alpha, &(temp1->A2d_[0][0]), lda, &(temp2->A2d_[0][0]), ldb, beta, &(A2d_[0][0]),
...
void Tensor2d::add(double alpha, const SharedTensor2d &Adum) {
    auto temp = std::make_shared<Tensor2d>(Adum->dim1_, Adum->dim2_);
    temp->copy(Adum);
    temp->scale(alpha);
    add(temp);
}  //
...
SharedTensor2d Tensor2d::transpose() {
    auto temp = std::make_shared<Tensor2d>(dim2_, dim1_);
#pragma omp parallel for
    for (int i = 0; i < dim2_; ++i) {
        for (int j = 0; j < dim1_; ++j) {
...
void Tensor2d::transform(const SharedTensor2d &a, const SharedTensor2d &transformer) {
    auto temp = std::make_shared<Tensor2d>(a->dim1_, transformer->dim2_);
...
void Tensor2d::triple_gemm(const SharedTensor2d &a, const SharedTensor2d &b, const SharedTensor2d &c) {
    if (a->dim2_ == b->dim1_ && b->dim2_ == c->dim1_ && a->dim1_ == dim1_ && c->dim2_ == dim2_) {
        auto bc = std::make_shared<Tensor2d>(b->dim1_, c->dim2_);
...
void Tensor2d::write(std::shared_ptr<psi::PSIO> psio, size_t fileno, bool three_index, bool symm) {
    ...
    if (three_index && symm) {
        ...
        auto temp = std::make_shared<Tensor2d>("temp", d1_, ntri_col);
#pragma omp parallel for
        for (int R = 0; R < d1_; R++) {
            for (int p = 0; p < d2_; p++) {
                for (int q = 0; q < d3_; q++) {
...
void Tensor2d::write_symm(std::shared_ptr<psi::PSIO> psio, size_t fileno) {
    ...
    auto temp = std::make_shared<Tensor1d>("temp", ntri_col);
#pragma omp parallel for
    for (int p = 0; p < dim1_; p++) {
        for (int q = 0; q <= p; q++) {
            int pq = index2(p, q);
...
void Tensor2d::write_anti_symm(std::shared_ptr<psi::PSIO> psio, size_t fileno) {
    ...
    auto temp = std::make_shared<Tensor2d>("temp", ntri_row, ntri_col);
#pragma omp parallel for
    for (int p = 1; p < d1_; p++) {
        for (int q = 0; q < p; q++) {
            int pq = row_idx_[p][q];
...
void Tensor2d::symmetrize3(const SharedTensor2d &A) {
    auto temp = std::make_shared<Tensor2d>("temp", d1_, d3_, d2_);
...
void Tensor2d::cont444(int t_a1, int t_a2, int f_a1, int f_a2, const SharedTensor2d &A, int t_b1, int t_b2, int f_b1,
    ...
    if (t_a1 == 3 && t_a2 == 4 && t_b1 == 1 && t_b2 == 2) {
        ...
        // Sort A(..,..) to A(pq,tu)
        auto temp1 = std::make_shared<Tensor2d>("temp1", d1_, d2_, dim_t, dim_u);
#pragma omp parallel for
        for (int p = 0; p < d1_; p++) {
            for (int q = 0; q < d2_; q++) {
        ...
        // Sort B(..,..) to B(tu,rs)
        auto temp2 = std::make_shared<Tensor2d>("temp2", dim_t, dim_u, d3_, d4_);
#pragma omp parallel for
        for (int t = 0; t < dim_t; t++) {
            for (int u = 0; u < dim_u; u++) {
...
void Tensor2d::cont444(std::string idx_c, std::string idx_a, std::string idx_b, SharedTensor2d &A, SharedTensor2d &B,
    ...
    auto temp1 = std::make_shared<Tensor2d>("temp1", d1_a, d2_a, d3_a, d4_a);
    ...
    auto temp2 = std::make_shared<Tensor2d>("temp2", d1_b, d2_b, d3_b, d4_b);
...
void Tensor2d::cont343(std::string idx_c, std::string idx_a, std::string idx_b, bool delete_b, SharedTensor2d &A,
    ...
    // Sort B(..,..) to B(rs,pq)
    auto temp = std::make_shared<Tensor2d>("temp", A->d2_, A->d3_, d2_, d3_);
#pragma omp parallel for
...
void Tensor2d::cont442(std::string idx_c, std::string idx_a, std::string idx_b, bool delete_a, bool delete_b,
    ...
    // Sort A(..,..) to A(pr,st)
    auto temp1 = std::make_shared<Tensor2d>("temp1", dim1_, dim_r, dim_s, dim_t);
#pragma omp parallel for
    for (int p = 0; p < dim1_; p++) {
    ...
    // Sort B(..,..) to B(rs,tq)
    auto temp2 = std::make_shared<Tensor2d>("temp2", dim_r, dim_s, dim_t, dim2_);
#pragma omp parallel for
    for (int r = 0; r < dim_r; r++) {
        for (int s = 0; s < dim_s; s++) {
...
void Tensor2d::cont424(std::string idx_c, std::string idx_a, std::string idx_b, bool delete_a, SharedTensor2d &A,
    ...
    // Sort A(..,..) to A(pq,rt)
    auto temp = std::make_shared<Tensor2d>("temp", d1_, d2_, d3_, dim_t);
#pragma omp parallel for
    for (int p = 0; p < d1_; p++) {
        for (int q = 0; q < d2_; q++) {
...
void Tensor2d::cont244(std::string idx_c, std::string idx_a, std::string idx_b, bool delete_b, SharedTensor2d &A,
    ...
    // Sort B(..,..) to B(tq,rs)
    auto temp = std::make_shared<Tensor2d>("temp", dim_t, d2_, d3_, d4_);
#pragma omp parallel for
    for (int t = 0; t < dim_t; t++) {
        for (int q = 0; q < d2_; q++) {
...
void Tensor2d::cont323(std::string idx_c, std::string idx_a, std::string idx_b, bool delete_a, SharedTensor2d &A,
    ...
    // Sort A(Q,..) to A(Q,pr)
    auto temp = std::make_shared<Tensor2d>("temp", d1_, d2_, dim_r);
#pragma omp parallel for
    for (int Q = 0; Q < dim1_; Q++) {
        for (int p = 0; p < d2_; p++) {
            for (int r = 0; r < dim_r; r++) {
...
void Tensor2d::cont332(std::string idx_c, std::string idx_a, std::string idx_b, bool delete_a, bool delete_b,
    ...
    // Sort A(Q,..) to A(Q,rp)
    auto temp1 = std::make_shared<Tensor2d>("temp1", A->d1_, dim_r, dim1_);
#pragma omp parallel for
    ...
    // Sort B(Q,..) to B(Q,rq)
    auto temp2 = std::make_shared<Tensor2d>("temp2", B->d1_, dim_r, dim2_);
#pragma omp parallel for
...
SharedTensor2i Tensor2i::transpose() {
    auto temp = std::make_shared<Tensor2i>(dim2_, dim1_);
