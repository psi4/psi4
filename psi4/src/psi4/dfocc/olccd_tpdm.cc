...
void DFOCC::olccd_tpdm() {
    ...
    if (reference_ == "RESTRICTED") {
        ...
        T = std::make_shared<Tensor2d>("U2 (IA|JB)", naoccA, navirA, naoccA, navirA);
        ccsd_u2_amps(T, T1);
        U = std::make_shared<Tensor2d>("U2 <IJ|AB>", naoccA, naoccA, navirA, navirA);
...
#pragma omp parallel for
        ...
        T1 = std::make_shared<Tensor2d>("T2 <IJ|AB>", naoccA, naoccA, navirA, navirA);
...
#pragma omp parallel for
        ...
        T1 = std::make_shared<Tensor2d>("T2 <Ij|Ab>", naoccA, naoccB, navirA, navirB);
...
#pragma omp parallel for
        ...
        // (-)Tt(mn, ae) = 1/2 (T_mn^ae - T_nm^ae) * (2 - \delta_{mn})
        T1 = std::make_shared<Tensor2d>("T2 <ij|ab>", naoccB, naoccB, navirB, navirB);
        T1->read_anti_symm(psio_, PSIF_DFOCC_AMPS);
