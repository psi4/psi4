...
void DFOCC::semi_canonic() {
    ...
    auto UooA = std::make_shared<Tensor2d>("UooA", naoccA, naoccA);
    auto UvvA = std::make_shared<Tensor2d>("UvvA", navirA, navirA);
    auto FockooA = std::make_shared<Tensor2d>("Fock <I|J>", naoccA, naoccA);
    auto FockvvA = std::make_shared<Tensor2d>("Fock <A|B>", navirA, navirA);
...
#pragma omp parallel for
    ...
    if (reference_ == "UNRESTRICTED") {
        auto UooB = std::make_shared<Tensor2d>("UooB", naoccB, naoccB);
        auto UvvB = std::make_shared<Tensor2d>("UvvB", navirB, navirB);
        auto FockooB = std::make_shared<Tensor2d>("Fock <i|j>", naoccB, naoccB);
        auto FockvvB = std::make_shared<Tensor2d>("Fock <a|b>", navirB, navirB);
