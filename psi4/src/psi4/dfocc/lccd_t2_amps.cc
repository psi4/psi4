...
void DFOCC::lccd_t2_amps() {
    ...
    if (reference_ == "RESTRICTED") {
        ...
        if (orb_opt_ == "FALSE" || mo_optimized == 1) {
            ...
            std::shared_ptr<Matrix> RT2AA = std::make_shared<Matrix>("RT2AA", ntri_anti_ijAA, ntri_anti_abAA);
            RAA->to_matrix(RT2AA);
            RAA.reset();
            // TAA
            TAA = std::make_shared<Tensor2d>("New T2 <IJ|AB>", ntri_anti_ijAA, ntri_anti_abAA);
            TAA->read(psio_, PSIF_DFOCC_AMPS);
            std::shared_ptr<Matrix> T2AA = std::make_shared<Matrix>("T2AA", ntri_anti_ijAA, ntri_anti_abAA);
            TAA->to_matrix(T2AA);

            // RBB
            RBB = std::make_shared<Tensor2d>("RT2 <ij|ab>", ntri_anti_ijBB, ntri_anti_abBB);
            RBB->read(psio_, PSIF_DFOCC_AMPS);
            std::shared_ptr<Matrix> RT2BB = std::make_shared<Matrix>("RT2BB", ntri_anti_ijBB, ntri_anti_abBB);
            RBB->to_matrix(RT2BB);
            RBB.reset();
            // TBB
            TBB = std::make_shared<Tensor2d>("New T2 <ij|ab>", ntri_anti_ijBB, ntri_anti_abBB);
            TBB->read(psio_, PSIF_DFOCC_AMPS);
            std::shared_ptr<Matrix> T2BB = std::make_shared<Matrix>("T2BB", ntri_anti_ijBB, ntri_anti_abBB);
            TBB->to_matrix(T2BB);

            // RAB
            RAB = std::make_shared<Tensor2d>("RT2 <Ij|Ab>", naoccA * naoccB, navirA * navirB);
            RAB->read(psio_, PSIF_DFOCC_AMPS);
            std::shared_ptr<Matrix> RT2AB = std::make_shared<Matrix>("RT2AB", naoccA * naoccB, navirA * navirB);
            RAB->to_matrix(RT2AB);
            RAB.reset();
            // TAB
            TAB = std::make_shared<Tensor2d>("New T2 <Ij|Ab>", naoccA * naoccB, navirA * navirB);
            TAB->read(psio_, PSIF_DFOCC_AMPS);
            std::shared_ptr<Matrix> T2AB = std::make_shared<Matrix>("T2AB", naoccA * naoccB, navirA * navirB);
            TAB->to_matrix(T2AB);

            // add entry
            if (do_diis_ == 1)
                ccsdDiisManager->add_entry(6, RT2AA.get(), RT2BB.get(), RT2AB.get(), T2AA.get(), T2BB.get(),
                                           T2AB.get());
            RT2AA.reset();
            RT2BB.reset();
            RT2BB.reset();
            ...
            if (do_diis_ == 1) {
                if (ccsdDiisManager->subspace_size() >= cc_mindiis_)
                    ccsdDiisManager->extrapolate(3, T2AA.get(), T2BB.get(), T2AB.get());
                TAA->set2(T2AA);
                TBB->set2(T2BB);
                TAB->set2(T2AB);
            }
