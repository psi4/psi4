...
void DFOCC::t2_2nd_gen() {
    ...
    if (reference_ == "RESTRICTED") {
        ...
        // Read T2_2
        U = std::make_shared<Tensor2d>("T2_2 <IJ|AB>", naoccA, naoccA, navirA, navirA);
        ...
        // Read T2_2
        U = std::make_shared<Tensor2d>("T2_2 <ij|ab>", naoccB, naoccB, navirB, navirB);
        U->read_anti_symm(psio_, PSIF_DFOCC_AMPS);
        T = std::make_shared<Tensor2d>("T2_2 (ia|jb)", naoccB, navirB, naoccB, navirB);
        T->sort(1324, U, 1.0, 0.0);
        U.reset();
