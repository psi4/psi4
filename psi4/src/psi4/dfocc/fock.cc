...
void DFOCC::fock() {
    ...
    if (reference_ == "RESTRICTED") {
        ...
        /*
        // Diagonalize
        SharedTensor1d eigA = std::make_shared<Tensor1d>(new Tensor1d("epsilon <P|Q>", nmo_);
        SharedTensor2d UmoA = std::make_shared<Tensor2d>(new Tensor2d("UmoA", nmo_, nmo_);
        FockA->diagonalize(UmoA, eigA, cutoff);
        eigA.reset();

        // Get new MOs
        SharedTensor2d Ca_new = std::make_shared<Tensor2d>(new Tensor2d("New alpha MO coefficients", nso_, nmo_);
