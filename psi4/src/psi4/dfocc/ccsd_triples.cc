	...
	Miabd->zero();
	    ...
	    double Dij = Di + FockA->get(j + nfrzc, j + nfrzc);
	        ...
	        J1->myread(PSIF_DFOCC_IABC, (size_t)(j*navirA*navirA*navirA)*sizeof(double));
                ...
                // W[ijk](ba,c) = \sum(e) J[j](ba,e) T[ik](ec)
                V->contract(false, false, navirA*navirA, navirA, navirA, J1, T, 0, (i*naoccA*navirA*navirA) + (k*navirA*navirA), 1.0, 0.0);
                ...
                // W[ijk](bc,a) = \sum(e) J[j](bc,e) T[ki](ea)
                V->contract(false, false, navirA*navirA, navirA, navirA, J1, T, 0, (k*naoccA*navirA*navirA) + (i*navirA*navirA), 1.0, 0.0);
	        ...
	        J1->myread(PSIF_DFOCC_IABC, (size_t)(k*navirA*navirA*navirA)*sizeof(double));
                ...
                // W[ijk](ca,b) = \sum(e) J[k](ca,e) T[ij](eb)
                V->contract(false, false, navirA*navirA, navirA, navirA, J1, T, 0, (i*naoccA*navirA*navirA) + (j*navirA*navirA), 1.0, 0.0);
                ...
                // W[ijk](cb,a) = \sum(e) J[k](cb,e) T[ji](ea)
                V->contract(false, false, navirA*navirA, navirA, navirA, J1, T, 0, (j*naoccA*navirA*navirA) + (i*navirA*navirA), 1.0, 0.0);
		...
		// V[ijk](ab,c) += t_i^a <jk|bc> + t_j^b <ik|ac> + t_k^c <ij|ab>
			...
			long int ab = ab_idxAA->get(a,b);
			    ...
			    long int ac = ab_idxAA->get(a,c);
			    long int bc = ab_idxAA->get(b,c);
			    double value = V->get(ab,c) + ( t1A->get(i,a)*I2->get(jk,bc) ) + ( t1A->get(j,b)*I2->get(ik,ac) ) + ( t1A->get(k,c)*I2->get(ij,ab) );
			    V->set(ab, c, value);
			}
		    }
		}
		...
		double value3_ = 0.0;
			...
			long int ba = ab_idxAA->get(b,a);
			    ...
			    // contributions to energy
			    value_ = ( V->get(ab,c) - V->get(cb,a) ) * ( (4.0*W->get(ab,c)) + W->get(bc,a) + W->get(ca,b) ) ;
			    double Dijkabc = Dijkab - FockA->get(c + noccA, c + noccA);
			    //sum += value_ / Dijkabc;
			    sum += value_ * Dijkabc;
			    ...
			    value2_ = (4.0*W->get(ab,c)) + W->get(bc,a) + W->get(ca,b) - (3.0*W->get(cb,a)) - (2.0*W->get(ac,b)) - W->get(ba,c);
			    // overwrite J1 with X
			    //J1->set(ab, c, value2_/Dijkabc);
			    J1->set(ab, c, value2_);

			    // contributions to G_ii & G_aa
			    //value3_ = V->get(ab,c) * ( (4.0*W->get(ab,c)) + W->get(bc,a) + W->get(ca,b) - (3.0*W->get(cb,a)) - (2.0*W->get(ac,b)) - W->get(ba,c) ) ;
			    /*
		...
		// overwrite J1 with X
			...
			long int ba = ab_idxAA->get(b,a);
			    ...
			    long int ca = ab_idxAA->get(c,a);
			    long int cb = ab_idxAA->get(c,b);
			    double value = (4.0*W->get(ab,c)) + W->get(bc,a) + W->get(ca,b) - W->get(cb,a) - (2.0*W->get(ac,b)) - (3.0*W->get(ba,c));
			    J1->set(ab, c, value);
			}
		    }
		}
		...
		// overwrite J1 with X
			...
			long int ba = ab_idxAA->get(b,a);
			    ...
			    long int ca = ab_idxAA->get(c,a);
			    long int cb = ab_idxAA->get(c,b);
			    double value = (4.0*W->get(ab,c)) + W->get(bc,a) + W->get(ca,b) - (3.0*W->get(cb,a)) - (2.0*W->get(ac,b)) - W->get(ba,c);
			    J1->set(ab, c, value);
			}
		    }
		}
		...
		// overwrite J1 with X
			...
			long int ba = ab_idxAA->get(b,a);
			    ...
			    long int ca = ab_idxAA->get(c,a);
			    long int cb = ab_idxAA->get(c,b);
			    double value = (4.0*W->get(ab,c)) + W->get(bc,a) + W->get(ca,b) - (2.0*W->get(cb,a)) - (2.0*W->get(ac,b)) - (2.0*W->get(ba,c));
			    J1->set(ab, c, value);
			}
		    }
		}
		...
		// Z[ijk](l,ab) = -\sum(c) I[jk](l,c) X[ijk](ab,c)
                Z->contract(false, true, naoccA, navirA*navirA, navirA, I, V, (j*naoccA*naoccA*navirA) + (k*naoccA*navirA), 0, -1.0, 0.0);
		...
		// overwrite V with X
			...
			long int ba = ab_idxAA->get(b,a);
			    ...
			    long int ca = ab_idxAA->get(c,a);
			    long int cb = ab_idxAA->get(c,b);
			    double value = (2.0*W->get(ab,c)) + (2.0*W->get(bc,a)) + (2.0*W->get(ca,b)) - W->get(cb,a) - W->get(ac,b) - (4.0*W->get(ba,c));
			    V->set(ac, b, value);
			}
		    }
		}
		// M[ijk](a,m) = \sum(bc) X[ijk](a,cb) T[k](m,cb) 
                P3->contract(false, true, navirA, naoccA, navirA*navirA, V, T, 0, k*naoccA*navirA*navirA, 1.0, 0.0);
		...
		// Miabd += \sum(jkc) X[ijk](ab,c) T[jk](d,c)
			...
			long int ba = ab_idxAA->get(b,a);
                ...
                Miabd->contract(false, true, navirA*navirA, navirA, navirA, V, T, 0, (j*naoccA*navirA*navirA) + (k*navirA*navirA), 1.0, 1.0);
    ...
    for(long int a = 0 ; a < navirA; ++a){
		...
		syc = (a*navirA*naoccA*navirA) + (c*naoccA*navirA);
	        I3->myread(PSIF_DFOCC_ABIC, syc*sizeof(double));
                ...
                W->contract(false, false, naoccA, naoccA*naoccA, navirA, I3, U, 0, b*navirA*naoccA*naoccA, 1.0, 0.0);

                // W[abc](i,jk) -= \sum(m) T'[ac](i,m) I[b](m,jk) (1-)
                W->contract(false, false, naoccA, naoccA*naoccA, naoccA, U, I,(a*navirA*naoccA*naoccA) + (c*naoccA*naoccA), b*naoccA*naoccA*naoccA, -1.0, 1.0);
		...
		syc = (a*navirA*naoccA*navirA) + (b*naoccA*navirA);
	        I3->myread(PSIF_DFOCC_ABIC, syc*sizeof(double));
                ...
                V->contract(false, false, naoccA, naoccA*naoccA, navirA, I3, U, 0, c*navirA*naoccA*naoccA, 1.0, 0.0);

                // W[abc](i,kj) -= \sum(m) T'[ab](i,m) I[c](m,kj) (2-)
                V->contract(false, false, naoccA, naoccA*naoccA, naoccA, U, I,(a*navirA*naoccA*naoccA) + (b*naoccA*naoccA), c*naoccA*naoccA*naoccA, -1.0, 1.0);
                #pragma omp parallel for
                for(long int i = 0 ; i < naoccA; ++i){
                    for(long int j = 0 ; j < naoccA; ++j){
		        W->axpy((size_t)naoccA, i*naoccA*naoccA + j, naoccA, V, i*naoccA*naoccA + j*naoccA, 1, 1.0);
		    }
		}
		...
		syc = (b*navirA*naoccA*navirA) + (c*naoccA*navirA);
	        I3->myread(PSIF_DFOCC_ABIC, syc*sizeof(double));
                ...
                V->contract(false, false, naoccA, naoccA*naoccA, navirA, I3, U, 0, a*navirA*naoccA*naoccA, 1.0, 0.0);

                // W[abc](j,ik) -= \sum(m) T'[bc](j,m) I[a](m,ik) (3-)
                V->contract(false, false, naoccA, naoccA*naoccA, naoccA, U, I,(b*navirA*naoccA*naoccA) + (c*naoccA*naoccA), a*naoccA*naoccA*naoccA, -1.0, 1.0);
                #pragma omp parallel for
                for(long int i = 0 ; i < naoccA; ++i){
                    for(long int j = 0 ; j < naoccA; ++j){
		        W->axpy((size_t)naoccA, j*naoccA*naoccA + i*naoccA, 1, V, i*naoccA*naoccA + j*naoccA, 1, 1.0);
		    }
		}
		...
		syc = (b*navirA*naoccA*navirA) + (a*naoccA*navirA);
	        I3->myread(PSIF_DFOCC_ABIC, syc*sizeof(double));
                ...
                V->contract(false, false, naoccA, naoccA*naoccA, naoccA, U, I,(b*navirA*naoccA*naoccA) + (a*naoccA*naoccA), c*naoccA*naoccA*naoccA, -1.0, 1.0);
                #pragma omp parallel for
                for(long int i = 0 ; i < naoccA; ++i){
                    for(long int j = 0 ; j < naoccA; ++j){
		        W->axpy((size_t)naoccA, j*naoccA*naoccA + i, naoccA, V, i*naoccA*naoccA + j*naoccA, 1, 1.0);
		    }
		}
		...
		syc = (c*navirA*naoccA*navirA) + (b*naoccA*navirA);
	        I3->myread(PSIF_DFOCC_ABIC, syc*sizeof(double));
                ...
                V->contract(false, false, naoccA, naoccA*naoccA, naoccA, U, I,(c*navirA*naoccA*naoccA) + (b*naoccA*naoccA), a*naoccA*naoccA*naoccA, -1.0, 1.0);
                #pragma omp parallel for
                for(long int i = 0 ; i < naoccA; ++i){
                    for(long int j = 0 ; j < naoccA; ++j){
		        W->axpy((size_t)naoccA, i*naoccA + j, naoccA*naoccA, V, i*naoccA*naoccA + j*naoccA, 1, 1.0);
		    }
		}
		...
		syc = (c*navirA*naoccA*navirA) + (a*naoccA*navirA);
	        I3->myread(PSIF_DFOCC_ABIC, syc*sizeof(double));
                ...
                V->contract(false, false, naoccA, naoccA*naoccA, navirA, I3, U, 0, b*navirA*naoccA*naoccA, 1.0, 0.0);

                // W[abc](k,ji) -= \sum(m) T'[ca](k,m) I[b](m,ji) (6-)
                V->contract(false, false, naoccA, naoccA*naoccA, naoccA, U, I,(c*navirA*naoccA*naoccA) + (a*naoccA*naoccA), b*naoccA*naoccA*naoccA, -1.0, 1.0);
                #pragma omp parallel for
                for(long int i = 0 ; i < naoccA; ++i){
                    for(long int j = 0 ; j < naoccA; ++j){
		        W->axpy((size_t)naoccA, j*naoccA + i, naoccA*naoccA, V, i*naoccA*naoccA + j*naoccA, 1, 1.0);
		    }
		}
                ...
                for(long int i = 0 ; i < naoccA; ++i){
                    for(long int j = 0 ; j < naoccA; ++j){
                        ...
                        for(long int k = 0 ; k < naoccA; ++k){
                            long int jk = ij_idxAA->get(j,k);
                            long int ik = ij_idxAA->get(i,k);
			    double value = V->get(i,jk) + ( t1A->get(i,a)*I2->get(jk,bc) ) + ( t1A->get(j,b)*I2->get(ik,ac) ) + ( t1A->get(k,c)*I2->get(ij,ab) );
			    V->set(i, jk, value);
			}
		    }
		}
		...
		*/
		...
		// overwrite J1 with X
			    ...
			    double value = (4.0*W->get(i,jk)) + W->get(j,ki) + W->get(k,ij) - (3.0*W->get(k,ji)) - (2.0*W->get(i,kj)) - W->get(j,ik);
			    X->set(i, jk, value);
			}
		    }
		}
	...
	Miabd->zero();
		...
		// Compute Energy & (t)^L_i^a
			...
			long int ba = ab_idxAA->get(b,a);
			    ...
			    // contributions to energy
			    double value_ = ( V->get(ab,c) - V->get(cb,a) ) * ( (4.0*W->get(ab,c)) + W->get(bc,a) + W->get(ca,b) ) ;
			    double Dijkabc = Dijkab - FockA->get(c + noccA, c + noccA);
			    sum += value_ * Dijkabc;

		            // (t)^L_i^a = 1/2 \sum(jkbc) X[ijk](a,bc) I[jk](bc) 
			    double value2_ = (4.0*W->get(ab,c)) + W->get(bc,a) + W->get(ca,b) - (3.0*W->get(cb,a)) - (2.0*W->get(ac,b)) - W->get(ba,c);
			    // overwrite J1 with X
			    J1->set(ab, c, value2_);
			}
		    }
