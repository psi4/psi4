...
std::vector<SharedMatrix> fd_geoms_freq_0(std::shared_ptr<Molecule> mol, Options &options, int freq_irrep_only) {
    ...
    if (options.get_int("PRINT") > 2)
        for (int i = 0; i < salc_list.ncd(); ++i) salc_list[i].print();
