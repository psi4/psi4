...
void CIWavefunction::form_opdm() {
    ...
    Da_ = triplet(Ca_, MO_Da, Ca_, false, false, true);
    Db_ = triplet(Cb_, MO_Db, Cb_, false, false, true);
...
void CIWavefunction::ci_nat_orbs() {
    ...
    SharedMatrix Cnat = doublet(Cactv, NO_vecs);
    ...
    if (options_.get_str("WFN") == "CASSCF") {
        if (!somcscf_) {
            ...
            SharedMatrix Cocc_semi = doublet(Cocc, evecs_o);
            ...
            SharedMatrix Cvir_semi = doublet(Cvir, evecs_v);
