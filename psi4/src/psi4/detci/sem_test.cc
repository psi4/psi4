...
** Returns: none
*/
void CIWavefunction::sem_test(double **A, int N, int M, int L, double **evecs, double *evals, double **b, double conv_e,
    ...
    for (I = 0; I < N; I++) A[I][I] -= CalcInfo_->edrc;
    ...
    for (i = 0; i < M; i++) Mmatrix[i] = init_matrix(maxnvect, maxnvect);
    ...
    while (!converged && iter <= maxiter) {
        ...
        if (lse_do) {
            ...
            for (k = 0; k < M; k++) {
                ...
                for (i = 0; i < L; i++) {
                    for (j = i; j < L; j++) {
                        Mmatrix[k][i][j] = Mmatrix[k][j][i] = sigma_overlap[i][j] - 2.0 * lambda[k] * G[i][j];
            ...
            for (k = 0; k < M; k++) {
                if (DSYEV_eigvec_asc(L, Mmatrix[k], m_lambda[k], m_alpha[k]) != 0){
                    throw PSIEXCEPTION("DSYEV diagonalizer failed in DETCI SEM_TEST!");
                }
                if (print_ > 2) {
        ...
        if ((Parameters_->collapse_size > 0) && (iter2 - Parameters_->collapse_size + 1 > 0) &&
            (Lvec[iter2 - Parameters_->collapse_size + 1] + M * Parameters_->collapse_size > maxnvect) &&
            iter != maxiter) {
            collapse_num++;
            ...
            if (lse_do)
                for (k = 0; k < M; k++)
                    for (i = 0; i < L; i++)
                        for (I = 0; I < N; I++) d[k][I] += m_alpha[k][i][0] * b[i][I];

            else
                for (k = 0; k < M; k++)
                    for (i = 0; i < L; i++)
                        for (I = 0; I < N; I++) d[k][I] += alpha[i][k] * b[i][I];

            /* copy d matrix to end of b matrix */
            for (i = 0; i < M; i++)
                for (I = 0; I < N; I++) b[maxnvect - 1 - i][I] = d[i][I];
        ...
        for (i = 0; i < M; i++) {
            ...
            tval = sqrt(tval);
            dvecnorm[i] = tval;
            if (dvecnorm[i] <= conv_rms && std::fabs(lambda[i] - lastroot[i]) <= conv_e)
                converged_root[i] = 1;
            else {
                converged_root[i] = 0;
                converged = 0;
            }
            outfile->Printf("Iter %3d  Root %d = %13.9lf", iter - 1, i + 1, (lambda[i] + offst));
        ...
        if (converged || iter == maxiter) {
            for (i = 0; i < M; i++) {
                ...
                for (j = 0; j < L; j++) {
                    tval = alpha[j][i];
                    for (I = 0; I < N; I++) evecs[i][I] += tval * b[j][I];
                }
            }
            break;
        } else {
