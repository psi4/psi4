 ...
 */
std::vector<SharedMatrix> CIWavefunction::tpdm(SharedCIVector Ivec, SharedCIVector Jvec, int Iroot, int Jroot) {
    std::vector<std::tuple<int, int, double> > states_vec;
            ...
            for (Ibuf = 0; Ibuf < Ivec->buf_per_vect_; Ibuf++) {
                ...
                for (Jbuf = 0; Jbuf < Jvec->buf_per_vect_; Jbuf++) {
                    ...
                    if (Jvec->buf_offdiag_[Jbuf] &&
                        (s1_contrib_[Iblock][Jblock2] || s2_contrib_[Iblock][Jblock2] || s3_contrib_[Iblock][Jblock2]))
                        do_Jblock2 = 1;
                    ...
                    if (do_Jblock) {
                        tpdm_block(alplist_, betlist_, CalcInfo_->num_ci_orbs, Ivec->num_alpcodes_, Ivec->num_betcodes_,
                                   tpdm_aap, tpdm_bbp, tpdm_abp, Jvec->blocks_[Jblock], Ivec->blocks_[Iblock], Jac, Jbc,
                    ...
                    if (do_Jblock2) {
                        ...
                        tpdm_block(alplist_, betlist_, CalcInfo_->num_ci_orbs, Ivec->num_alpcodes_, Ivec->num_betcodes_,
                                   tpdm_aap, tpdm_bbp, tpdm_abp, transp_tmp, Ivec->blocks_[Iblock], Jbc, Jac, Jnbs,
                                   Jnas, Iac, Ibc, Inas, Inbs, weight);
                    }
                ...
                if (Ivec->buf_offdiag_[Ibuf]) { /* need to get contrib of transpose */
                    ...
                    for (Jbuf = 0; Jbuf < Jvec->buf_per_vect_; Jbuf++) {
                        ...
                        if (do_Jblock) {
                            tpdm_block(alplist_, betlist_, CalcInfo_->num_ci_orbs, Ivec->num_alpcodes_,
                                       Ivec->num_betcodes_, tpdm_aap, tpdm_bbp, tpdm_abp, Jvec->blocks_[Jblock],
                                       transp_tmp2, Jac, Jbc, Jnas, Jnbs, Iac, Ibc, Inas, Inbs, weight);
                        }

                        if (do_Jblock2) {
                            Jvec->transp_block(Jblock, transp_tmp);
                            tpdm_block(alplist_, betlist_, CalcInfo_->num_ci_orbs, Ivec->num_alpcodes_,
                                       Ivec->num_betcodes_, tpdm_aap, tpdm_bbp, tpdm_abp, transp_tmp, transp_tmp2, Jbc,
                                       Jac, Jnbs, Jnas, Iac, Ibc, Inas, Inbs, weight);
                        }
                    } /* end loop over Jbuf */
                    ...
                    for (Iblock = Ivec->first_ablk_[Iairr]; Iblock <= Ivec->last_ablk_[Iairr]; Iblock++) {
                        ...
                        for (Jblock = Jvec->first_ablk_[Jairr]; Jblock <= Jvec->last_ablk_[Jairr]; Jblock++) {
                            ...
                            if (s1_contrib_[Iblock][Jblock] || s2_contrib_[Iblock][Jblock] ||
                                ...
                                tpdm_block(alplist_, betlist_, CalcInfo_->num_ci_orbs, Ivec->num_alpcodes_,
                                           Ivec->num_betcodes_, tpdm_aap, tpdm_bbp, tpdm_abp, Jvec->blocks_[Jblock],
                                           Ivec->blocks_[Iblock], Jac, Jbc, Jnas, Jnbs, Iac, Ibc, Inas, Inbs, weight);
                            ...
                            if (Jvec->buf_offdiag_[Jbuf]) {
                                ...
                                if (s1_contrib_[Iblock][Jblock2] || s2_contrib_[Iblock][Jblock2] ||
                                    ...
                                    Jvec->transp_block(Jblock, transp_tmp);
                                    tpdm_block(alplist_, betlist_, CalcInfo_->num_ci_orbs, Ivec->num_alpcodes_,
                                               Ivec->num_betcodes_, tpdm_aap, tpdm_bbp, tpdm_abp, transp_tmp,
                            ...
                            for (Jblock = Jvec->first_ablk_[Jairr]; Jblock <= Jvec->last_ablk_[Jairr]; Jblock++) {
                                ...
                                if (Jvec->buf_offdiag_[Jbuf]) {
                                    ...
                                    if (s1_contrib_[Iblock][Jblock2] || s2_contrib_[Iblock][Jblock2] ||
                                        ...
                                        tpdm_block(alplist_, betlist_, CalcInfo_->num_ci_orbs, Ivec->num_alpcodes_,
                                                   Ivec->num_betcodes_, tpdm_aap, tpdm_bbp, tpdm_abp, transp_tmp,
                                                   transp_tmp2, Jbc, Jac, Jnbs, Jnas, Iac, Ibc, Inas, Inbs, weight);
                                    }
                                }
...
void CIWavefunction::tpdm_block(struct stringwr **alplist, struct stringwr **betlist, int nbf, int nalplists,
                                int nbetlists, double *twopdm_aa, double *twopdm_bb, double *twopdm_ab, double **CJ,
                                double **CI, int Ja_list, int Jb_list, int Jnas, int Jnbs, int Ia_list, int Ib_list,
