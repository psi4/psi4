...
void stringset_translate_addr(StringSet *sset, int new_nel, int new_ndrc, int *pitz2corr, struct olsen_graph *Graph,
                              int *new_list, int *new_idx);
...
** August 2003
*/
void CIWavefunction::parse_import_vector(SlaterDetSet *sdset, int *ialplist, int *ialpidx, int *ibetlist, int *ibetidx,
                                         int *blknums) {
    int i, j;
    ...
    stringset_translate_addr(alphastrings, CalcInfo_->num_alp_expl, CalcInfo_->num_drc_orbs, CalcInfo_->reorder.data(),
                             AlphaG_, new_alphastr_list, new_alphastr_idx);
    stringset_translate_addr(betastrings, CalcInfo_->num_bet_expl, CalcInfo_->num_drc_orbs, CalcInfo_->reorder.data(),
                             BetaG_, new_betastr_list, new_betastr_idx);
    ...
    */
    ...
    free(new_alphastr_list);
    free(new_alphastr_idx);
    free(new_betastr_list);
    free(new_betastr_idx);
}
...
void stringset_translate_addr(StringSet *sset, int new_nel, int new_ndrc, int *pitz2corr, struct olsen_graph *Graph,
    ...
    */
    for (s = 0; s < sset->size; s++) {
        ...
        for (i = 0, l = 0; i < num_former_drc; i++) tmpocc[l++] = former_drc_occ[i]; /* these are already translated */
