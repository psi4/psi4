...
int og_lex_addr(struct olsen_graph *Graph, int *occs, int nel, int *listnum) {
    ...
    for (i = 0; i < nel; i++) {
        ...
        if (j <= Graph->ras1_lvl)
            inras1++;
        else if (j >= Graph->ras3_lvl && j < Graph->ras4_lvl)
            inras3++;
        else if (j >= Graph->ras4_lvl)
            inras4++;
        else
            inras2++;
    }
...
void str_abs2rel(int absidx, int *relidx, int *listnum, struct olsen_graph *Graph) {
    ...
    for (irrep = 0; irrep < Graph->nirreps; irrep++) {
        if (tot + Graph->str_per_irrep[irrep] > absidx)
            break;
        else
            tot += Graph->str_per_irrep[irrep];
    }

    for (code = 0; code < Graph->subgr_per_irrep; code++) {
        if (tot + Graph->sg[irrep][code].num_strings > absidx)
            break;
        else
            tot += Graph->sg[irrep][code].num_strings;
    }
    *relidx = absidx - tot;
    *listnum = irrep * Graph->subgr_per_irrep + code;
}
