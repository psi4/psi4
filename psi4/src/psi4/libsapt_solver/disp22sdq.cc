...
double SAPT2p::disp220d_2(int ampfile, const char *glabel, const char *thetalabel, int intfile, const char *BSlabel,
    ...
    if (trans == 'n' || trans == 'N') {
        ...
        psio_->read_entry(ampfile, glabel, (char *)yARBS[0], sizeof(double) * aoccA * nvirA * aoccB * nvirB);

        C_DGEMM('N', 'T', aoccA * nvirA, aoccB * nvirB, ndf_ + 3, 1.0, T_p_AR[0], ndf_ + 3, B_p_BS[0], ndf_ + 3, 1.0,
                yARBS[0], aoccB * nvirB);
        ...
        psio_->read_entry(ampfile, glabel, (char *)yBSAR[0], sizeof(double) * aoccA * nvirA * aoccB * nvirB);

        C_DGEMM('N', 'T', aoccB * nvirB, aoccA * nvirA, ndf_ + 3, 1.0, B_p_BS[0], ndf_ + 3, T_p_AR[0], ndf_ + 3, 1.0,
                yBSAR[0], aoccA * nvirA);
...
double SAPT2p::disp220q_1(int ampfile, const char *tlabel, const char *Tlabel, const char *thetalabel, size_t aoccA,
    ...
    psio_->read_entry(ampfile, tlabel, (char *)thetaARAR[0], sizeof(double) * aoccA * nvirA * aoccA * nvirA);
    ...
    psio_->read_entry(ampfile, thetalabel, (char *)theta_p_AR[0], sizeof(double) * aoccA * nvirA * (ndf_ + 3));

    double **xARAR = block_matrix(aoccA * nvirA, aoccA * nvirA);

    C_DGEMM('N', 'T', aoccA * nvirA, aoccA * nvirA, ndf_ + 3, 1.0, T_p_AR[0], ndf_ + 3, theta_p_AR[0], ndf_ + 3, 0.0,
            xARAR[0], aoccA * nvirA);

    energy = 4.0 * C_DDOT((long int)aoccA * nvirA * aoccA * nvirA, xARAR[0], 1, thetaARAR[0], 1);
...
double SAPT2p::disp220q_2(int ampfile, const char *pAAlabel, const char *pRRlabel, const char *Tlabel, int intfile,
    ...
    C_DGEMM('N', 'T', aoccA, aoccA, nvirA * (ndf_ + 3), 1.0, B_p_AR[0], nvirA * (ndf_ + 3), T_p_AR[0],
            nvirA * (ndf_ + 3), 0.0, qAA[0], aoccA);

    for (int a = 0; a < aoccA; a++) {
        C_DGEMM('N', 'T', nvirA, nvirA, ndf_ + 3, 1.0, B_p_AR[a * nvirA], ndf_ + 3, T_p_AR[a * nvirA], ndf_ + 3, 1.0,
                qRR[0], nvirA);
    }
...
double SAPT2p::disp220q_3(int ampfile, const char *tARARlabel, const char *tARBSlabel, const char trans, int intfile,
    ...
    if (trans == 'n' || trans == 'N') {
        ...
        psio_->read_entry(ampfile, tARBSlabel, (char *)tARBS[0], sizeof(double) * aoccA * nvirA * aoccB * nvirB);

        C_DGEMM('N', 'T', aoccA * nvirA, aoccA * nvirA, aoccB * nvirB, 1.0, tARBS[0], aoccB * nvirB, tARBS[0],
                aoccB * nvirB, 0.0, xARAR[0], aoccA * nvirA);

        free_block(tARBS);
    } else if (trans == 't' || trans == 'T') {
        double **tBSAR = block_matrix(aoccB * nvirB, aoccA * nvirA);
        psio_->read_entry(ampfile, tARBSlabel, (char *)tBSAR[0], sizeof(double) * aoccA * nvirA * aoccB * nvirB);

        C_DGEMM('T', 'N', aoccA * nvirA, aoccA * nvirA, aoccB * nvirB, 1.0, tBSAR[0], aoccA * nvirA, tBSAR[0],
                aoccA * nvirA, 0.0, xARAR[0], aoccA * nvirA);
    ...
    psio_->read_entry(ampfile, tARARlabel, (char *)tARAR[0], sizeof(double) * aoccA * nvirA * aoccA * nvirA);
    antisym(tARAR, aoccA, nvirA);

    double **yARAR = block_matrix(aoccA * nvirA, aoccA * nvirA);

    C_DGEMM('N', 'T', aoccA * nvirA, aoccA * nvirA, aoccA * nvirA, 1.0, xARAR[0], aoccA * nvirA, tARAR[0],
            aoccA * nvirA, 0.0, yARAR[0], aoccA * nvirA);
    ...
    double energy = 4.0 * C_DDOT((long int)aoccA * nvirA * aoccA * nvirA, xARAR[0], 1, yARAR[0], 1);
...
double SAPT2p::disp220q_4(int ampfile, const char *tARARlabel, const char *tARBSlabel, const char trans, int intfile,
    ...
    C_DGEMM('N', 'T', aoccA, aoccA, nvirA * aoccA * nvirA, 1.0, tARAR[0], nvirA * aoccA * nvirA, gARAR[0],
            nvirA * aoccA * nvirA, 0.0, rAA[0], aoccA);

    C_DGEMM('T', 'N', nvirA, nvirA, aoccA * nvirA * aoccA, 1.0, tARAR[0], nvirA, gARAR[0], nvirA, 0.0, rRR[0], nvirA);
    ...
    if (trans == 'n' || trans == 'N') {
        ...
        C_DGEMM('N', 'T', aoccA, aoccA, nvirA * aoccB * nvirB, 1.0, tARBS[0], nvirA * aoccB * nvirB, tARBS[0],
                nvirA * aoccB * nvirB, 0.0, sAA[0], aoccA);

        for (int a = 0; a < aoccA; a++) {
            C_DGEMM('N', 'T', nvirA, nvirA, aoccB * nvirB, 1.0, tARBS[a * nvirA], aoccB * nvirB, tARBS[a * nvirA],
                    aoccB * nvirB, 1.0, sRR[0], nvirA);
        }
        ...
        for (int b = 0, bs = 0; b < aoccB; b++) {
            for (int s = 0; s < nvirB; s++, bs++) {
                C_DGEMM('N', 'T', aoccA, aoccA, nvirA, 1.0, tBSAR[bs], nvirA, tBSAR[bs], nvirA, 1.0, sAA[0], aoccA);
            }
        }

        C_DGEMM('T', 'N', nvirA, nvirA, aoccA * aoccB * nvirB, 1.0, tBSAR[0], nvirA, tBSAR[0], nvirA, 0.0, sRR[0],
                nvirA);
