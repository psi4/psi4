...
double SAPT2p::disp21_1(int ampfile, const char *glabel, const char *tlabel, size_t aoccA, size_t nvirA, size_t aoccB,
    ...
    psio_->read_entry(ampfile, glabel, (char *)gARBS[0], sizeof(double) * aoccA * nvirA * aoccB * nvirB);

    double energy = 4.0 * C_DDOT((long int)aoccA * nvirA * aoccB * nvirB, tARBS[0], 1, gARBS[0], 1);
...
double SAPT2p::disp21_2(int ampfile, const char *tlabel, const char *thetalabel, size_t aoccA, size_t nvirA) {
    ...
    double energy = 8.0 * C_DDOT((long int)aoccA * nvirA * (ndf_ + 3), T_p_AR[0], 1, theta_p_AR[0], 1);
