...
SAPT2p::SAPT2p(SharedWavefunction Dimer, SharedWavefunction MonomerA, SharedWavefunction MonomerB, Options& options,
...
void SAPT2p::print_results() {
    ...
    for (scal_it = Xscal.begin(); scal_it != Xscal.end(); ++scal_it) {
        ...
        double tot_disp = 0.0;
        if (nat_orbs_t3_)
            tot_disp = e_disp20_ + e_disp21_ + e_disp22sdq_ + e_est_disp22t_ + *scal_it * e_exch_disp20_;
        else
            tot_disp = e_disp20_ + e_disp21_ + e_disp22sdq_ + e_disp22t_ + *scal_it * e_exch_disp20_;
