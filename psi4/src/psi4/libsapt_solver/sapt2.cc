...
SAPT2::SAPT2(SharedWavefunction Dimer, SharedWavefunction MonomerA, SharedWavefunction MonomerB, Options &options,
...
void SAPT2::print_results() {
    ...
    for (scal_it = Xscal.begin(); scal_it != Xscal.end(); ++scal_it) {
        ...
        double tot_ind = e_ind20_ + dHF2 + e_ind22_ + *scal_it * (e_exch_ind20_ + e_exch_ind22_);
        double tot_ct = e_ind20_ + e_ind22_ + *scal_it * (e_exch_ind20_ + e_exch_ind22_);
...
#pragma omp for private(mn) schedule(guided)
        ...
        for (int P = 0; P < numPshell; ++P) {
            ...
            C_DGEMM('N', 'N', nmoA_, nmoA_, nso_, 1.0, halftrans, nso_, CA_[0], nmoA_, 0.0, MO_RI[P], nmoA_);
        }
    ...
    if (temp_size > nmoA_ * nmoA_) temp_size = nmoA_ * nmoA_;
    ...
    for (int i = 0, oP = 0; oP < nmoA_ * nmoA_; ++i, oP += numP) {
        ...
        for (int P = 0; P < ndf_; ++P) {
            psio_->read(PSIF_SAPT_TEMP, "MO AA RI Integrals", (char *)&(temp[P][0]), sizeof(double) * numP, next_DF_MO,
                        &next_DF_MO);
            next_DF_MO = psio_get_address(next_DF_MO, sizeof(double) * (nmoA_ * nmoA_ - numP));
        }

        temp_J = block_matrix(numP, ndf_ + 3);

        C_DGEMM('T', 'N', numP, ndf_, ndf_, 1.0, temp[0], numP, J_mhalf[0], ndf_, 0.0, temp_J[0], ndf_ + 3);
        ...
        for (int ij = 0; ij < numP; ij++) {
            ...
            if (i < noccA_ && j < noccA_) {
                psio_->write(PSIF_SAPT_AA_DF_INTS, "AA RI Integrals", (char *)&(temp_J[ij][0]),
                             sizeof(double) * (ndf_ + 3), next_DF_AA, &next_DF_AA);
            }
        }
        ...
        for (int ij = 0; ij < numP; ij++) {
            ...
            int j = (ij + oP) % nmoA_;
            if (i < noccA_ && j >= noccA_) {
                psio_->write(PSIF_SAPT_AA_DF_INTS, "AR RI Integrals", (char *)&(temp_J[ij][0]),
                             sizeof(double) * (ndf_ + 3), next_DF_AR, &next_DF_AR);
            }
        }
        ...
        for (int ij = 0; ij < numP; ij++) {
            ...
            int j = (ij + oP) % nmoA_;
            if (i >= noccA_ && j >= noccA_) {
                psio_->write(PSIF_SAPT_AA_DF_INTS, "RR RI Integrals", (char *)&(temp_J[ij][0]),
...
#pragma omp for private(mn) schedule(guided)
        ...
        for (int P = 0; P < numPshell; ++P) {
            ...
            C_DGEMM('N', 'N', nmoB_, nmoB_, nso_, 1.0, halftrans, nso_, CB_[0], nmoB_, 0.0, MO_RI[P], nmoB_);
        }
    ...
    if (temp_size > nmoB_ * nmoB_) temp_size = nmoB_ * nmoB_;
    ...
    for (int i = 0, oP = 0; oP < nmoB_ * nmoB_; ++i, oP += numP) {
        ...
        next_DF_MO = psio_get_address(PSIO_ZERO, sizeof(double) * oP);
        for (int P = 0; P < ndf_; ++P) {
            psio_->read(PSIF_SAPT_TEMP, "MO BB RI Integrals", (char *)&(temp[P][0]), sizeof(double) * numP, next_DF_MO,
                        &next_DF_MO);
            next_DF_MO = psio_get_address(next_DF_MO, sizeof(double) * (nmoB_ * nmoB_ - numP));
        }
        ...
        for (int ij = 0; ij < numP; ij++) {
            ...
            if (i < noccB_ && j < noccB_) {
                psio_->write(PSIF_SAPT_BB_DF_INTS, "BB RI Integrals", (char *)&(temp_J[ij][0]),
                             sizeof(double) * (ndf_ + 3), next_DF_BB, &next_DF_BB);
            }
        }
        ...
        for (int ij = 0; ij < numP; ij++) {
            ...
            int j = (ij + oP) % nmoB_;
            if (i < noccB_ && j >= noccB_) {
                psio_->write(PSIF_SAPT_BB_DF_INTS, "BS RI Integrals", (char *)&(temp_J[ij][0]),
                             sizeof(double) * (ndf_ + 3), next_DF_BS, &next_DF_BS);
            }
        }
        ...
        for (int ij = 0; ij < numP; ij++) {
            ...
            int j = (ij + oP) % nmoB_;
            if (i >= noccB_ && j >= noccB_) {
                psio_->write(PSIF_SAPT_BB_DF_INTS, "SS RI Integrals", (char *)&(temp_J[ij][0]),
                             sizeof(double) * (ndf_ + 3), next_DF_SS, &next_DF_SS);
            }
        }
...
#pragma omp for private(mn) schedule(guided)
        ...
        for (int P = 0; P < numPshell; ++P) {
            C_DGEMM('T', 'N', nmoA_, nso_, nso_, 1.0, CA_[0], nmoA_, AO_RI[P], nso_, 0.0, halftrans, nso_);
            C_DGEMM('N', 'N', nmoA_, nmoB_, nso_, 1.0, halftrans, nso_, CB_[0], nmoB_, 0.0, MO_RI[P], nmoB_);
        }
    ...
    if (temp_size > nmoA_ * nmoB_) temp_size = nmoA_ * nmoB_;
    ...
    for (int i = 0, oP = 0; oP < nmoA_ * nmoB_; ++i, oP += numP) {
        ...
        next_DF_MO = psio_get_address(PSIO_ZERO, sizeof(double) * oP);
        for (int P = 0; P < ndf_; ++P) {
            psio_->read(PSIF_SAPT_TEMP, "MO AB RI Integrals", (char *)&(temp[P][0]), sizeof(double) * numP, next_DF_MO,
                        &next_DF_MO);
            next_DF_MO = psio_get_address(next_DF_MO, sizeof(double) * (nmoA_ * nmoB_ - numP));
        }
        ...
        for (int ij = 0; ij < numP; ij++) {
            ...
            if (i < noccA_ && j < noccB_) {
                psio_->write(PSIF_SAPT_AB_DF_INTS, "AB RI Integrals", (char *)&(temp_J[ij][0]),
                             sizeof(double) * (ndf_ + 3), next_DF_AB, &next_DF_AB);
            }
        }
        ...
        for (int ij = 0; ij < numP; ij++) {
            ...
            int j = (ij + oP) % nmoB_;
            if (i < noccA_ && j >= noccB_) {
                psio_->write(PSIF_SAPT_AB_DF_INTS, "AS RI Integrals", (char *)&(temp_J[ij][0]),
                             sizeof(double) * (ndf_ + 3), next_DF_AS, &next_DF_AS);
            }
        }
        ...
        for (int ij = 0; ij < numP; ij++) {
            ...
            int j = (ij + oP) % nmoB_;
            if (i >= noccA_ && j < noccB_) {
                psio_->write(PSIF_SAPT_AB_DF_INTS, "RB RI Integrals", (char *)&(temp_J[ij][0]),
                             sizeof(double) * (ndf_ + 3), next_DF_RB, &next_DF_RB);
            }
        }
...
void SAPT2::w_integrals() {
    ...
    C_DGEMV('n', noccA_ * nvirA_, ndf_, 2.0, &(B_p_AR[0][0]), ndf_ + 3, diagBB_, 1, 1.0, &(wBAR_[0][0]), 1);
    ...
    C_DGEMV('n', noccB_ * nvirB_, ndf_, 2.0, &(B_p_BS[0][0]), ndf_ + 3, diagAA_, 1, 1.0, &(wABS_[0][0]), 1);
    ...
    C_DGEMV('n', noccA_ * noccA_, ndf_, 2.0, &(B_p_AA[0][0]), ndf_ + 3, diagBB_, 1, 1.0, &(wBAA_[0][0]), 1);
    ...
    C_DGEMV('n', noccB_ * noccB_, ndf_, 2.0, &(B_p_BB[0][0]), ndf_ + 3, diagAA_, 1, 1.0, &(wABB_[0][0]), 1);
    ...
    C_DGEMV('n', nvirA_ * nvirA_, ndf_, 2.0, &(B_p_RR[0][0]), ndf_ + 3, diagBB_, 1, 1.0, &(wBRR_[0][0]), 1);
    ...
    C_DGEMV('n', nvirB_ * nvirB_, ndf_, 2.0, &(B_p_SS[0][0]), ndf_ + 3, diagAA_, 1, 1.0, &(wASS_[0][0]), 1);
...
void SAPT2::natural_orbitalify(int ampfile, const char *VV_opdm, double *evals, int foccA, int noccA, size_t nvirA,
    ...
    psio_->read_entry(ampfile, VV_opdm, (char *)P[0], sizeof(double) * nvirA * nvirA);
    ...
    C_DGEMM('T', 'N', num_no_vir, nvirA, nvirA, 1.0, &(nat_orbs_MO[0][0]), nvirA, &(Fock_MO[0][0]), nvirA, 0.0,
            &(tempmat[0][0]), nvirA);
    C_DGEMM('N', 'N', num_no_vir, num_no_vir, nvirA, 1.0, &(tempmat[0][0]), nvirA, &(nat_orbs_MO[0][0]), nvirA, 0.0,
            &(Fock_NO[0][0]), num_no_vir);
    ...
    C_DGEMM('N', 'N', nvirA, num_no_vir, num_no_vir, 1.0, &(nat_orbs_MO[0][0]), nvirA, &(X[0][0]), num_no_vir, 0.0,
            &(MO_MVO[0][0]), num_no_vir);
    ...
    if (monomer == 'A') {
        ...
        C_DCOPY(nvirA * num_no_vir, &(MO_MVO[0][0]), 1, &(no_CA_[0][0]), 1);
    ...
    if (monomer == 'B') {
        ...
        C_DCOPY(nvirA * num_no_vir, &(MO_MVO[0][0]), 1, &(no_CB_[0][0]), 1);
...
void SAPT2::natural_orbitalify_df_ints() {
    double **B_p_AR = get_DF_ints(PSIF_SAPT_AA_DF_INTS, "AR RI Integrals", 0, noccA_, 0, nvirA_);
    double **C_p_AR = block_matrix(noccA_ * no_nvirA_, ndf_ + 3);

    for (int a = 0; a < noccA_; a++) {
        C_DGEMM('T', 'N', no_nvirA_, ndf_ + 3, nvirA_, 1.0, no_CA_[0], no_nvirA_, B_p_AR[a * nvirA_], ndf_ + 3, 0.0,
                C_p_AR[a * no_nvirA_], ndf_ + 3);
    }

    psio_->write_entry(PSIF_SAPT_AA_DF_INTS, "AR NO RI Integrals", (char *)C_p_AR[0],
                       sizeof(double) * noccA_ * no_nvirA_ * (ndf_ + 3));

    free_block(B_p_AR);
    free_block(C_p_AR);

    double **B_p_BS = get_DF_ints(PSIF_SAPT_BB_DF_INTS, "BS RI Integrals", 0, noccB_, 0, nvirB_);
    double **C_p_BS = block_matrix(noccB_ * no_nvirB_, ndf_ + 3);

    for (int b = 0; b < noccB_; b++) {
        C_DGEMM('T', 'N', no_nvirB_, ndf_ + 3, nvirB_, 1.0, no_CB_[0], no_nvirB_, B_p_BS[b * nvirB_], ndf_ + 3, 0.0,
                C_p_BS[b * no_nvirB_], ndf_ + 3);
    }

    psio_->write_entry(PSIF_SAPT_BB_DF_INTS, "BS NO RI Integrals", (char *)C_p_BS[0],
                       sizeof(double) * noccB_ * no_nvirB_ * (ndf_ + 3));
    ...
    C_DGEMM('T', 'N', no_nvirA_, nvirA_ * (ndf_ + 3), nvirA_, 1.0, no_CA_[0], no_nvirA_, B_p_RR[0], nvirA_ * (ndf_ + 3),
            0.0, C_p_RR[0], nvirA_ * (ndf_ + 3));
    ...
    for (int r = 0; r < no_nvirA_; r++) {
        C_DGEMM('T', 'N', no_nvirA_, ndf_ + 3, nvirA_, 1.0, no_CA_[0], no_nvirA_, C_p_RR[r * nvirA_], ndf_ + 3, 0.0,
                D_p_RR[r * no_nvirA_], ndf_ + 3);
    }

    psio_->write_entry(PSIF_SAPT_AA_DF_INTS, "RR NO RI Integrals", (char *)D_p_RR[0],
                       sizeof(double) * no_nvirA_ * no_nvirA_ * (ndf_ + 3));
    ...
    C_DGEMM('T', 'N', no_nvirB_, nvirB_ * (ndf_ + 3), nvirB_, 1.0, no_CB_[0], no_nvirB_, B_p_SS[0], nvirB_ * (ndf_ + 3),
            0.0, C_p_SS[0], nvirB_ * (ndf_ + 3));
    ...
    for (int s = 0; s < no_nvirB_; s++) {
        C_DGEMM('T', 'N', no_nvirB_, ndf_ + 3, nvirB_, 1.0, no_CB_[0], no_nvirB_, C_p_SS[s * nvirB_], ndf_ + 3, 0.0,
                D_p_SS[s * no_nvirB_], ndf_ + 3);
    }

    psio_->write_entry(PSIF_SAPT_BB_DF_INTS, "SS NO RI Integrals", (char *)D_p_SS[0],
                       sizeof(double) * no_nvirB_ * no_nvirB_ * (ndf_ + 3));
