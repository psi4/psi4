...
void USAPT0::fock_terms() {
    ...
    std::shared_ptr<Matrix> Da_A = doublet(Cocca_A_, Cocca_A_, false, true);
    std::shared_ptr<Matrix> Db_A = doublet(Coccb_A_, Coccb_A_, false, true);
    std::shared_ptr<Matrix> Da_B = doublet(Cocca_B_, Cocca_B_, false, true);
    std::shared_ptr<Matrix> Db_B = doublet(Coccb_B_, Coccb_B_, false, true);

    // => Compute the P matrices <= //

    std::shared_ptr<Matrix> Pa_A = doublet(Cvira_A_, Cvira_A_, false, true);
    std::shared_ptr<Matrix> Pb_A = doublet(Cvirb_A_, Cvirb_A_, false, true);
    std::shared_ptr<Matrix> Pa_B = doublet(Cvira_B_, Cvira_B_, false, true);
    std::shared_ptr<Matrix> Pb_B = doublet(Cvirb_B_, Cvirb_B_, false, true);
    ...
    std::shared_ptr<Matrix> S_CA = doublet(S, Cocca_A_);
    std::shared_ptr<Matrix> Ca_AS = doublet(Pa_B, S_CA);
    //    Next one for DM-based formula (debug purposes for now)
    std::shared_ptr<Matrix> Ca_AB = doublet(Da_B, S_CA);
    S_CA = doublet(S, Coccb_A_);
    std::shared_ptr<Matrix> Cb_AS = doublet(Pb_B, S_CA);
    //    Next one for DM-based formula (debug purposes for now)
    std::shared_ptr<Matrix> Cb_AB = doublet(Db_B, S_CA);
    S_CA.reset();
    ...
    std::shared_ptr<Matrix> Ta_A_n = doublet(C_Ta_A_n, Cocca_A_, false, true);
    std::shared_ptr<Matrix> Ta_B_n = doublet(C_Ta_B_n, Cocca_B_, false, true);
    std::shared_ptr<Matrix> Ta_BA_n = doublet(C_Ta_BA_n, Cocca_A_, false, true);

    std::shared_ptr<Matrix> Tb_A_n = doublet(C_Tb_A_n, Coccb_A_, false, true);
    std::shared_ptr<Matrix> Tb_B_n = doublet(C_Tb_B_n, Coccb_B_, false, true);
    std::shared_ptr<Matrix> Tb_BA_n = doublet(C_Tb_BA_n, Coccb_A_, false, true);
    ...
    inter_a = std::shared_ptr<Matrix>(triplet(El_pot_B, Da_A, S));
    inter_a->add(Ka_AS);
    inter_b = std::shared_ptr<Matrix>(triplet(El_pot_B, Db_A, S));
    inter_b->add(Kb_AS);

    double Exch10_2 = 0.0;
    std::vector<double> Exch10_2_terms;
    /*    Exch10_2_terms.resize(3);
    Exch10_2_terms[0] -= triplet(triplet(Da_A,S,Da_B),S,Pa_A)->vector_dot(V_B);
    Exch10_2_terms[0] -= triplet(triplet(Da_A,S,Da_B),S,Pa_A)->vector_dot(Ja_B);
    Exch10_2_terms[0] -= triplet(triplet(Da_A,S,Da_B),S,Pa_A)->vector_dot(Jb_B);
    Exch10_2_terms[1] -= triplet(triplet(Da_B,S,Da_A),S,Pa_B)->vector_dot(V_A);
    Exch10_2_terms[1] -= triplet(triplet(Da_B,S,Da_A),S,Pa_B)->vector_dot(Ja_A);
    Exch10_2_terms[1] -= triplet(triplet(Da_B,S,Da_A),S,Pa_B)->vector_dot(Jb_A);
    Exch10_2_terms[2] -= triplet(Pa_A,S,Da_B)->vector_dot(Ka_AS);
    Exch10_2_terms[0] -= triplet(triplet(Db_A,S,Db_B),S,Pb_A)->vector_dot(V_B);
    Exch10_2_terms[0] -= triplet(triplet(Db_A,S,Db_B),S,Pb_A)->vector_dot(Ja_B);
    Exch10_2_terms[0] -= triplet(triplet(Db_A,S,Db_B),S,Pb_A)->vector_dot(Jb_B);
    Exch10_2_terms[1] -= triplet(triplet(Db_B,S,Db_A),S,Pb_B)->vector_dot(V_A);
    Exch10_2_terms[1] -= triplet(triplet(Db_B,S,Db_A),S,Pb_B)->vector_dot(Ja_A);
    Exch10_2_terms[1] -= triplet(triplet(Db_B,S,Db_A),S,Pb_B)->vector_dot(Jb_A);
    Exch10_2_terms[2] -= triplet(Pb_A,S,Db_B)->vector_dot(Kb_AS);
*/ Exch10_2_terms.resize(4);
    Exch10_2_terms[0] -= triplet(Pa_A, S, Da_B)->vector_dot(inter_a);
    Exch10_2_terms[1] -= triplet(Pa_B, S, Da_A)->vector_dot(triplet(El_pot_A, Da_B, S));
    Exch10_2_terms[2] -= triplet(Pb_A, S, Db_B)->vector_dot(inter_b);
    Exch10_2_terms[3] -= triplet(Pb_B, S, Db_A)->vector_dot(triplet(El_pot_A, Db_B, S));
    ...
    inter_a = std::shared_ptr<Matrix>(triplet(S, Da_B, El_pot_A));
    inter_a->scale(-1.0);
    inter_a->subtract(triplet(El_pot_B, Da_A, S));
    inter_a->add(Ka_AB);
    inter_a->add(ha_A);
    inter_a->add(ha_B);

    inter_b = std::shared_ptr<Matrix>(triplet(S, Db_B, El_pot_A));
    inter_b->scale(-1.0);
    inter_b->subtract(triplet(El_pot_B, Db_A, S));
    inter_b->add(Kb_AB);
    inter_b->add(hb_A);
    inter_b->add(hb_B);
    ...
    Exch10_2dm_terms[2] -= triplet(Da_A, S, Da_B)->vector_dot(inter_a);
    Exch10_2dm_terms[3] -= triplet(Db_A, S, Db_B)->vector_dot(inter_b);

    /*    double Exch10_2dm = 0.0;
    std::vector<double> Exch10_2dm_terms;
    Exch10_2dm_terms.resize(6);
    Exch10_2dm_terms[0] += triplet(Da_A,S,Da_B)->vector_dot(Ka_A);
    Exch10_2dm_terms[0] -= triplet(Da_A,S,Da_B)->vector_dot(Ja_A);
    Exch10_2dm_terms[0] -= triplet(Da_A,S,Da_B)->vector_dot(Jb_A);
    Exch10_2dm_terms[0] -= triplet(Da_A,S,Da_B)->vector_dot(V_A);
    Exch10_2dm_terms[0] -= triplet(Da_A,S,Da_B)->vector_dot(Ka_AB);
    Exch10_2dm_terms[0] += triplet(Da_A,S,triplet(Da_B,Ja_A,Da_B))->vector_dot(S);
    Exch10_2dm_terms[0] += triplet(Da_A,S,triplet(Da_B,Jb_A,Da_B))->vector_dot(S);
    Exch10_2dm_terms[0] += triplet(Da_A,S,triplet(Da_B,V_A,Da_B))->vector_dot(S);
    Exch10_2dm_terms[0] += triplet(Da_A,S,triplet(Da_B,S,Da_A))->vector_dot(Ja_B);
    Exch10_2dm_terms[0] += triplet(Da_A,S,triplet(Da_B,S,Da_A))->vector_dot(Jb_B);
    Exch10_2dm_terms[0] += triplet(Da_A,S,triplet(Da_B,S,Da_A))->vector_dot(V_B);
    Exch10_2dm_terms[1] += triplet(Db_A,S,Db_B)->vector_dot(Kb_A);
    Exch10_2dm_terms[1] -= triplet(Db_A,S,Db_B)->vector_dot(Jb_A);
    Exch10_2dm_terms[1] -= triplet(Db_A,S,Db_B)->vector_dot(Ja_A);
    Exch10_2dm_terms[1] -= triplet(Db_A,S,Db_B)->vector_dot(V_A);
    Exch10_2dm_terms[1] -= triplet(Db_A,S,Db_B)->vector_dot(Kb_AB);
    Exch10_2dm_terms[1] += triplet(Db_A,S,triplet(Db_B,Jb_A,Db_B))->vector_dot(S);
    Exch10_2dm_terms[1] += triplet(Db_A,S,triplet(Db_B,Ja_A,Db_B))->vector_dot(S);
    Exch10_2dm_terms[1] += triplet(Db_A,S,triplet(Db_B,V_A,Db_B))->vector_dot(S);
    Exch10_2dm_terms[1] += triplet(Db_A,S,triplet(Db_B,S,Db_A))->vector_dot(Jb_B);
    Exch10_2dm_terms[1] += triplet(Db_A,S,triplet(Db_B,S,Db_A))->vector_dot(Ja_B);
    Exch10_2dm_terms[1] += triplet(Db_A,S,triplet(Db_B,S,Db_A))->vector_dot(V_B);
    Exch10_2dm_terms[2] += triplet(Da_B,S,Da_A)->vector_dot(Ka_B);
    Exch10_2dm_terms[2] -= triplet(Da_B,S,Da_A)->vector_dot(Jb_B);
    Exch10_2dm_terms[2] -= triplet(Da_B,S,Da_A)->vector_dot(Ja_B);
    Exch10_2dm_terms[2] -= triplet(Da_B,S,Da_A)->vector_dot(V_B);
    Exch10_2dm_terms[3] += triplet(Db_B,S,Db_A)->vector_dot(Kb_B);
    Exch10_2dm_terms[3] -= triplet(Db_B,S,Db_A)->vector_dot(Ja_B);
    Exch10_2dm_terms[3] -= triplet(Db_B,S,Db_A)->vector_dot(Jb_B);
    Exch10_2dm_terms[3] -= triplet(Db_B,S,Db_A)->vector_dot(V_B);
...
*/ for (int k = 0; k < Exch10_2dm_terms.size(); k++) {
    ...
    std::shared_ptr<Matrix> C_Oa_B = triplet(Da_A, S, Cocca_B_);
    std::shared_ptr<Matrix> C_Ob_B = triplet(Db_A, S, Coccb_B_);
    std::shared_ptr<Matrix> C_Pa_B = triplet(triplet(Da_B, S, Da_A), S, Cocca_B_);
    std::shared_ptr<Matrix> C_Pb_B = triplet(triplet(Db_B, S, Db_A), S, Coccb_B_);
    std::shared_ptr<Matrix> C_Pa_A = triplet(triplet(Da_A, S, Da_B), S, Cocca_A_);
    std::shared_ptr<Matrix> C_Pb_A = triplet(triplet(Db_A, S, Db_B), S, Coccb_A_);
...
std::shared_ptr<Matrix> USAPT0::build_ind_pot(std::map<std::string, std::shared_ptr<Matrix> >& vars) {
    ...
    return triplet(Ca, El_pot, Cr, true, false, false);
}
std::shared_ptr<Matrix> USAPT0::build_exch_ind_pot(std::map<std::string, std::shared_ptr<Matrix> >& vars) {
    ...
    T = h_A->clone();
    T->scale(-1.0);
    T->add(triplet(S, D_A, El_pot_B));
    T->add(triplet(El_pot_A, D_B, S));
    T->subtract(K_O->transpose());

    W->add(triplet(S, D_B, T));

    // 8 Use again T for intermediate

    T = h_B->clone();
    T->scale(-1.0);
    T->add(triplet(El_pot_B, D_A, S));
    T->subtract(K_O);

    W->add(triplet(T, D_B, S));
...
void USAPT0::mp2_terms() {
    ...
    std::shared_ptr<Matrix> Ca_a2 = doublet(Da_B, S);
    std::shared_ptr<Matrix> Cb_a2 = doublet(Db_B, S);
    std::shared_ptr<Matrix> Ca_b2 = doublet(Da_A, S);
    std::shared_ptr<Matrix> Cb_b2 = doublet(Db_A, S);

    std::shared_ptr<Matrix> Ca_s1 = doublet(Ca_b2, Cavira_B_);
    std::shared_ptr<Matrix> Cb_s1 = doublet(Cb_b2, Cavirb_B_);
    std::shared_ptr<Matrix> Ca_r1 = doublet(Ca_a2, Cavira_A_);
    std::shared_ptr<Matrix> Cb_r1 = doublet(Cb_a2, Cavirb_A_);

    std::shared_ptr<Matrix> Ca_s3 = triplet(Da_B, S, Ca_s1);
    std::shared_ptr<Matrix> Cb_s3 = triplet(Db_B, S, Cb_s1);
    std::shared_ptr<Matrix> Ca_r3 = triplet(Da_A, S, Ca_r1);
    std::shared_ptr<Matrix> Cb_r3 = triplet(Db_A, S, Cb_r1);
    ...
    Ca_a2 = doublet(Ca_a2, Caocca_A_);
    Cb_a2 = doublet(Cb_a2, Caoccb_A_);
    Ca_b2 = doublet(Ca_b2, Caocca_B_);
    Cb_b2 = doublet(Cb_b2, Caoccb_B_);

    std::shared_ptr<Matrix> Ca_a4 = triplet(Da_A, S, Ca_a2);
    std::shared_ptr<Matrix> Cb_a4 = triplet(Db_A, S, Cb_a2);
    std::shared_ptr<Matrix> Ca_b4 = triplet(Da_B, S, Ca_b2);
    std::shared_ptr<Matrix> Cb_b4 = triplet(Db_B, S, Cb_b2);

    // => Auxiliary Fock-derived matrices <= //
    // We build them to maximize intermediate reuse and minimize the number
    // of matrix multiplications

    std::shared_ptr<Matrix> Da_BS = doublet(Da_B, S);
    std::shared_ptr<Matrix> Db_BS = doublet(Db_B, S);
    std::shared_ptr<Matrix> Da_AS = doublet(Da_A, S);
    std::shared_ptr<Matrix> Db_AS = doublet(Db_A, S);

    std::shared_ptr<Matrix> Ta_as = doublet(El_pot_B, Da_AS);
    std::shared_ptr<Matrix> Tb_as = doublet(El_pot_B, Db_AS);
    std::shared_ptr<Matrix> Ta_br = doublet(El_pot_A, Da_BS);
    std::shared_ptr<Matrix> Tb_br = doublet(El_pot_A, Db_BS);

    std::shared_ptr<Matrix> Sa_Bar =
        triplet(Caocca_A_, S, doublet(Da_BS, Cavira_A_), true, false, false);
    std::shared_ptr<Matrix> Sb_Bar =
        triplet(Caoccb_A_, S, doublet(Db_BS, Cavirb_A_), true, false, false);
    std::shared_ptr<Matrix> Sa_Abs =
        triplet(Caocca_B_, S, doublet(Da_AS, Cavira_B_), true, false, false);
    std::shared_ptr<Matrix> Sb_Abs =
        triplet(Caoccb_B_, S, doublet(Db_AS, Cavirb_B_), true, false, false);
    ...
    Ta_as->add(triplet(S, Da_B, El_pot_A));
    Tb_as->add(triplet(S, Db_B, El_pot_A));
    Ta_br->add(triplet(S, Da_A, El_pot_B));
    Tb_br->add(triplet(S, Db_A, El_pot_B));
    ...
    std::shared_ptr<Matrix> Sa_as = triplet(Caocca_A_, S, Cavira_B_, true, false, false);
    std::shared_ptr<Matrix> Sb_as = triplet(Caoccb_A_, S, Cavirb_B_, true, false, false);
    std::shared_ptr<Matrix> Sa_br = triplet(Caocca_B_, S, Cavira_A_, true, false, false);
    std::shared_ptr<Matrix> Sb_br = triplet(Caoccb_B_, S, Cavirb_A_, true, false, false);

    std::shared_ptr<Matrix> Qa_as = triplet(Caocca_A_, Ta_as, Cavira_B_, true, false, false);
    std::shared_ptr<Matrix> Qb_as = triplet(Caoccb_A_, Tb_as, Cavirb_B_, true, false, false);
    std::shared_ptr<Matrix> Qa_br = triplet(Caocca_B_, Ta_br, Cavira_A_, true, false, false);
    std::shared_ptr<Matrix> Qb_br = triplet(Caoccb_B_, Tb_br, Cavirb_A_, true, false, false);

    std::shared_ptr<Matrix> Qa_ar = triplet(Caocca_A_, El_pot_B, Cavira_A_, true, false, false);
    std::shared_ptr<Matrix> Qb_ar = triplet(Caoccb_A_, El_pot_B, Cavirb_A_, true, false, false);
    std::shared_ptr<Matrix> Qa_bs = triplet(Caocca_B_, El_pot_A, Cavira_B_, true, false, false);
    std::shared_ptr<Matrix> Qb_bs = triplet(Caoccb_B_, El_pot_A, Cavirb_B_, true, false, false);
