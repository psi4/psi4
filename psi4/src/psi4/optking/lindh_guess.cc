...
double **FRAG::Lindh_guess(void) {
    ...
    for (int A = 0; A < natom; ++A)
        for (int B = 0; B <= A; ++B) R[B][A] = R[A][B] = v3d_dist(geom[A], geom[B]);
    ...
    for (int A = 0; A < natom; ++A)
        for (int B = 0; B < natom; ++B)
            if (R[A][B] < dist_limit) close[A][B] = true;
    ...
    */
    ...
    for (int i = 0; i < Nintco; ++i)
        for (int k = 0; k < 3 * natom; ++k)
            for (int j = 0; j < Nintco; ++j) G[i][j] += B[i][k] * B[j][k];
    free_matrix(B);
    ...
    for (std::size_t i = 0; i < coords.simples.size(); ++i) {  // loop over coords.simples
        ...
        if (q->g_type() == stre_type) {
            ...
            int b = q->g_atom(1);
            int c = q->g_atom(2);
            Lindh_k = k_phi * Lindh_rho(a, b, R[a][b]) * Lindh_rho(b, c, R[b][c]);
        } else if (q->g_type() == tors_type) {
            int a = q->g_atom(0);
            int b = q->g_atom(1);
            int c = q->g_atom(2);
            int d = q->g_atom(3);
            Lindh_k = k_tau * Lindh_rho(a, b, R[a][b]) * Lindh_rho(b, c, R[b][c]) * Lindh_rho(c, d, R[c][d]);
