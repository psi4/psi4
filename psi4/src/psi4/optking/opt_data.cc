...
// save rest of stuff
void STEP_DATA::save_step_info(double DE_predicted_in, double *unit_step_in, double dq_norm_in, double dq_gradient_in,
...
OPT_DATA::~OPT_DATA() {
    ...
    for (std::size_t i = 0; i < steps.size(); ++i) delete steps[i];
    steps.clear();
}
...
bool OPT_DATA::conv_check(opt::MOLECULE &mol) const {
    ...
    if (Opt_params.opt_type == OPT_PARAMS::IRC) {
        ...
        for (int i = 0; i < Nintco; ++i) f[i] -= overlap * Ginv_p[i];
        free_array(Ginv_p);
    ...
    if (Opt_params.opt_type != OPT_PARAMS::IRC) {
        ...
        Opt_params.i_max_DE ? oprintf_out("  %10.2e %1s", Opt_params.conv_max_DE, "*")
                            : oprintf_out("             %1s", "o");
        Opt_params.i_max_force ? oprintf_out("  %10.2e %1s", Opt_params.conv_max_force, "*")
                               : oprintf_out("             %1s", "o");
        Opt_params.i_rms_force ? oprintf_out("  %10.2e %1s", Opt_params.conv_rms_force, "*")
                               : oprintf_out("             %1s", "o");
        Opt_params.i_max_disp ? oprintf_out("  %10.2e %1s", Opt_params.conv_max_disp, "*")
                              : oprintf_out("             %1s", "o");
        Opt_params.i_rms_disp ? oprintf_out("  %10.2e %1s", Opt_params.conv_rms_disp, "*")
                              : oprintf_out("             %1s", "o");
        ...
        oprintf_out(
            "   %4d %16.8f  %10.2e %1s  %10.2e %1s  %10.2e %1s  %10.2e %1s  %10.2e %1s  ~\n", iteration, g_energy(), DE,
            (Opt_params.i_max_DE ? ((std::fabs(DE) < Opt_params.conv_max_DE) ? "*" : "") : "o"), max_force,
            (Opt_params.i_max_force ? ((std::fabs(max_force) < Opt_params.conv_max_force) ? "*" : "") : "o"), rms_force,
            (Opt_params.i_rms_force ? ((std::fabs(rms_force) < Opt_params.conv_rms_force) ? "*" : "") : "o"), max_disp,
            (Opt_params.i_max_disp ? ((std::fabs(max_disp) < Opt_params.conv_max_disp) ? "*" : "") : "o"), rms_disp,
    ...
    if ((  // For un-modified Q-Chem or Molpro (Baker) criteria, if forces and either energy change or displacement met,
           // convergence!
...
void OPT_DATA::summary(void) const {
    ...
    oprintf_out(
        ...
        "- ~\n");
    oprintf_out(
        "   Step         Total Energy             Delta E       MAX Force       RMS Force        MAX Disp        RMS "
        "Disp  ~\n");
    oprintf_out(
        "  "
        "--------------------------------------------------------------------------------------------------------------"
...
void OPT_DATA::H_update(opt::MOLECULE &mol) {
    ...
    if (steps_since_last_H) oprintf_out("\tPrevious computed or guess Hessian on step %d.\n", check_start + 1);
    ...
    for (int i_step = ((int)steps.size()) - 2; i_step >= check_start; --i_step) {
        ...
        if (max_change > Opt_params.H_update_dq_tol) {
            oprintf_out("\tChange in internal coordinate of %5.2e exceeds limit of %5.2e.\n", max_change,
                        Opt_params.H_update_dq_tol);
            oprintf_out("\t Skipping Hessian update for step %d.\n", i_step + 1);
            continue;
        }
        use_steps.push_back(i_step);
        if ((int)use_steps.size() == Opt_params.H_update_use_last) break;
    }

    oprintf_out("\tSteps to be used in Hessian update:");
    for (std::size_t i = 0; i < use_steps.size(); ++i) oprintf_out(" %d", use_steps[i] + 1);
    oprintf_out("\n");
    ...
    for (std::size_t s = 0; s < use_steps.size(); ++s) {
        ...
        if (Opt_params.H_update == OPT_PARAMS::BFGS) {
            for (int i = 0; i < Nintco; ++i)
                for (int j = 0; j < Nintco; ++j) H_new[i][j] = H[i][j] + dg[i] * dg[j] / gq;
            ...
            for (int i = 0; i < Nintco; ++i)
                for (int j = 0; j < Nintco; ++j) H_new[i][j] -= Hdq[i] * Hdq[j] / qHq;
            ...
            for (int i = 0; i < Nintco; ++i) Z[i] = dg[i] - Z[i];

            qz = array_dot(dq, Z, Nintco);

            for (int i = 0; i < Nintco; ++i)
                for (int j = 0; j < Nintco; ++j) H_new[i][j] = H[i][j] + Z[i] * Z[j] / qz;
            ...
            for (int i = 0; i < Nintco; ++i)  // (1-phi)*MS
                for (int j = 0; j < Nintco; ++j) H_new[i][j] = H[i][j] + (1.0 - phi) * Z[i] * Z[j] / qz;
        ...
        if (Opt_params.H_update_limit) {  // limit changes in H
            ...
            for (int i = 0; i < Nintco; ++i)
                for (int j = 0; j < Nintco; ++j) H_new[i][j] -= H[i][j];
            ...
            for (int i = 0; i < Nintco; ++i)
                for (int j = 0; j < Nintco; ++j) H[i][j] = H_new[i][j];
        }
...
OPT_DATA::OPT_DATA(int Nintco_in, int Ncart_in) {
    ...
    if (!data_file_present) {
        ...
        if (Nintco_old != Nintco) oprintf_out("\tThe number of coordinates has changed.  Ignoring old data.\n");
        if (Ncart_old != Ncart) oprintf_out("\tThe number of atoms has changed.  Ignoring old data.\n");
...
void OPT_DATA::write(void) {
    ...
    for (std::size_t i = 0; i < steps.size(); ++i) steps[i]->write(i + 1, Nintco, Ncart);
    opt_io_close(1);
...
bool OPT_DATA::previous_step_report(void) const {
    ...
    if (Opt_params.print_lvl >= 2) oprintf_out("\tEnergy ratio = %10.5lf\n", Energy_ratio);
