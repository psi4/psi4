...
extern int F_DGEEV(char *, char *, int *N1, double *A, int *N2, double *w1, double *w2, double *vl, int *N3, double *vr,
                   int *N4, double *work, int *lwork, int *info);
...
bool opt_asymm_matrix_eig(double **A, int dim, double *evals) {
    ...
    for (int i = 0; i < dim; ++i)
        for (int j = 0; j < dim; ++j) At[j][i] = A[i][j];
    ...
    F_DGEEV(&noLeft, &doRight, &dim, A[0], &dim, evals, evals_i, Levects[0], &dim, Revects[0], &dim, work, &lwork,
            &rval);
    ...
    F_DGEEV(&noLeft, &doRight, &dim, At[0], &dim, evals, evals_i, Levects[0], &dim, Revects[0], &dim, work, &lwork,
            &rval);
    ...
    for (int i = 0; i < dim; ++i) {
        ...
        for (int j = 0; j < dim; ++j) A[cnt][j] = Revects[small][j];
        cnt++;
    }

    for (int j = 0; j < dim; ++j) evals[j] = evals_i[j];
...
double **symm_matrix_inv(double **A, int dim, bool redundant) {
    ...
    if (redundant) {
        for (i = 0; i < dim; ++i)
            if (std::fabs(evals[i]) > Opt_params.redundant_eval_tol) A_inv[i][i] = 1.0 / evals[i];
    } else {
        for (i = 0; i < dim; ++i) A_inv[i][i] = 1.0 / evals[i];
    }
...
double **matrix_return_copy(double **A, int nr, int nc) {
    ...
    int i, j;
    for (i = 0; i < nr; ++i)
        for (j = 0; j < nc; ++j) A_new[i][j] = A[i][j];
    return A_new;
}
...
void array_copy(double *v_from, double *v_to, int n) {
    for (int i = 0; i < n; ++i) v_to[i] = v_from[i];
}

void matrix_copy(double **from, double **to, int nr, int nc) {
    double *from1 = from[0];
    double *to1 = to[0];
    for (int i = 0; i < nr * nc; ++i) to1[i] = from1[i];
}
...
double array_norm(double *v1, int n) { return sqrt(array_dot(v1, v1, n)); }

void array_scm(double *v1, double a, int n) {
    for (int i = 0; i < n; ++i) v1[i] *= a;
}
...
void matrix_root(double **A, int dim, bool inverse) {
    ...
    if (inverse) {
        for (int k = 0; k < dim; k++)
            if (std::fabs(A_evals[k]) > Opt_params.redundant_eval_tol) A_evals[k] = 1.0 / A_evals[k];
    }
    ...
    for (int i = 0; i < dim; i++)
        for (int j = 0; j < dim; j++)
            for (int k = 0; k < dim; k++) A[i][j] += V[k][i] * A_evals[k] * V[k][j];
