...
double *COMBO_COORDINATES::values(GeomType geom) const {
    ...
    for (std::size_t cc = 0; cc < index.size(); ++cc) q[cc] = value(geom, cc);
            ...
            for (int b = 0; b < natom_simple; ++b) {
                ...
                for (int xyz_a = 0; xyz_a < 3; ++xyz_a)
                    for (int xyz_b = 0; xyz_b < 3; ++xyz_b)
                        dq2dx2[3 * atom_a + xyz_a][3 * atom_b + xyz_b] +=
                            coeff.at(lookup).at(s) * dq2dx2_simple[3 * a + xyz_a][3 * b + xyz_b];
            }
        }
    }
    return true;
...
void COMBO_COORDINATES::clear_combos(void) {
    for (std::size_t i = 0; i < index.size(); ++i) index[i].clear();
    for (std::size_t i = 0; i < coeff.size(); ++i) coeff[i].clear();
    index.clear();
    coeff.clear();
    return;
}
...
void COMBO_COORDINATES::print_disp(std::string psi_fp, FILE *qc_fp, int lookup, const double q_orig, const double f_q,
    ...
    if (index[lookup].size() == 1)  // Simple printout style
        ...
        oprintf(psi_fp, qc_fp, "%-15s = %13.6lf%13.6lf%13.6lf%13.6lf\n", iss.str().c_str(), q_orig * _bohr2angstroms,
                f_q * _hartree2aJ / _bohr2angstroms, dq * _bohr2angstroms, new_q * _bohr2angstroms);
    }
    return;
}
