...
class IRC_POINT {
   ...
   public:
    IRC_POINT(int coord_in, double *q_p_in, double *x_p_in, double *q_in, double *x_in, double *f_q_in, double *f_x_in,
              double E_in, double step, double arc, double line) {
        coord_step = coord_in;
        q_pivot = q_p_in;
        x_pivot = x_p_in;
        q = q_in;
...
class IRC_DATA {
   ...
   public:
    ...
    // free memory
    ~IRC_DATA() {
        for (size_t i = 0; i < steps.size(); ++i) delete steps[i];
        steps.clear();
    }
    ...
    double *g_q_pivot(void) const { return steps[steps.size() - 1]->g_q_pivot(); }
    double *g_x_pivot(void) const { return steps[steps.size() - 1]->g_x_pivot(); }
    double *g_q(void) const { return steps[steps.size() - 1]->g_q(); }
    // double g_q(int i) const
    //{
    // return g_q(i);
    //}
    double *g_x(void) const { return steps[steps.size() - 1]->g_x(); }
    double *g_f_q(void) const { return steps[steps.size() - 1]->g_f_q(); }
    double *g_f_x(void) const { return steps[steps.size() - 1]->g_f_x(); }
    double g_line_dist(int index) const { return steps[index]->g_line_dist(); }
    ...
    // double *f_x_in, double E_in, double step, double arc, double line)
    {
        ...
        IRC_POINT *onepoint =
            new IRC_POINT(coord_in, q_p_in, x_p_in, q_in, x_in, f_q_in, f_x_in, E_in, step_dist, arc_dist, line_dist);
