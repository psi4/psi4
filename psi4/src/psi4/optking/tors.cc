...
double **TORS::DqDx(GeomType geom) const {
    ...
    for (int a = 0; a < 4; ++a)        // atoms
        for (int i = 0; i < 3; ++i) {  // i=a_xyz
            ...
            if ((a == 0) || (a == 1)) tval1 = zeta(a, 0, 1) * uXw[i] / (Lu * sin_u * sin_u);

            if ((a == 2) || (a == 3)) tval2 = zeta(a, 2, 3) * vXw[i] / (Lv * sin_v * sin_v);

            if ((a == 1) || (a == 2)) tval3 = zeta(a, 1, 2) * uXw[i] * cos_u / (Lw * sin_u * sin_u);
            ...
            for (int i = 0; i < 3; ++i) {      // i = a_xyz
                for (int j = 0; j < 3; ++j) {  // j=b_xyz
                    ...
                    if ((a != b) && (i != j)) {
                        ...
                        if (a == 1 && b == 1)
                            tval += zeta(a, 0, 1) * zeta(b, 1, 2) * (j - i) * pow(-0.5, std::fabs(j - i)) *
                                    (+w[k] * cos_u - u[k]) / (Lu * Lw * sin_u * sin_u);

                        if ((a == 3 && b == 2) || (a == 3 && b == 1) || (a == 2 && b == 2) || (a == 2 && b == 1))
                            tval += zeta(a, 3, 2) * zeta(b, 2, 1) * (j - i) * pow(-0.5, std::fabs(j - i)) *
                                    (-w[k] * cos_v - v[k]) / (Lv * Lw * sin_v * sin_v);

                        if ((a == 2 && b == 1) || (a == 2 && b == 0) || (a == 1 && b == 1) || (a == 1 && b == 0))
                            tval += zeta(a, 2, 1) * zeta(b, 1, 0) * (j - i) * pow(-0.5, std::fabs(j - i)) *
                                    (-w[k] * cos_u + u[k]) / (Lu * Lw * sin_u * sin_u);

                        if (a == 2 && b == 2)
                            tval += zeta(a, 1, 2) * zeta(b, 2, 3) * (j - i) * pow(-0.5, std::fabs(j - i)) *
                                    (+w[k] * cos_v + v[k]) / (Lv * Lw * sin_v * sin_v);
                    }
...
void TORS::print(std::string psi_fp, FILE *qc_fp, GeomType geom, int off) const {
...
void TORS::print_disp(std::string psi_fp, FILE *qc_fp, const double q_old, const double f_q, const double dq,
                      const double q_new, int atom_offset) const {
    ostringstream iss(ostringstream::out);  // create stream; allow output to it
    if (s_frozen) iss << "*";
    iss << "D(" << s_atom[0] + atom_offset + 1 << "," << s_atom[1] + atom_offset + 1 << ","
        << s_atom[2] + atom_offset + 1 << "," << s_atom[3] + atom_offset + 1 << ")" << std::flush;
    oprintf(psi_fp, qc_fp, "%-15s = %13.6lf%13.6lf%13.6lf%13.6lf\n", iss.str().c_str(), q_old / _pi * 180.0,
            f_q * _pi / 180.0, dq / _pi * 180.0, q_new / _pi * 180.0);
}

void TORS::print_intco_dat(std::string psi_fp, FILE *qc_fp, int off) const {
    if (s_frozen)
        oprintf(psi_fp, qc_fp, "D*%6d%6d%6d%6d", s_atom[0] + 1 + off, s_atom[1] + 1 + off, s_atom[2] + 1 + off,
                s_atom[3] + 1 + off);
    else
        oprintf(psi_fp, qc_fp, "D %6d%6d%6d%6d", s_atom[0] + 1 + off, s_atom[1] + 1 + off, s_atom[2] + 1 + off,
                s_atom[3] + 1 + off);

    if (s_has_fixed_eq_val) oprintf(psi_fp, qc_fp, "%10.5lf", s_fixed_eq_val);
    oprintf(psi_fp, qc_fp, "\n");
}

void TORS::print_s(std::string psi_fp, FILE *qc_fp, GeomType geom) const {
    oprintf(psi_fp, qc_fp, "S vector for tors, D(%d %d %d %d): \n", s_atom[0] + 1, s_atom[1] + 1, s_atom[2] + 1,
            s_atom[3] + 1);
...
bool TORS::operator==(const SIMPLE_COORDINATE &s2) const {
    if (tors_type != s2.g_type()) return false;

    if (this->s_atom[0] == s2.g_atom(0) && this->s_atom[1] == s2.g_atom(1) && this->s_atom[2] == s2.g_atom(2) &&
        this->s_atom[3] == s2.g_atom(3))
        return true;
    else if (this->s_atom[0] == s2.g_atom(3) && this->s_atom[1] == s2.g_atom(2) && this->s_atom[2] == s2.g_atom(1) &&
             this->s_atom[3] == s2.g_atom(0))
        return true;
    else
        return false;
}
