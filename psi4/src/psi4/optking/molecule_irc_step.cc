...
void IRC_DATA::progress_report(opt::MOLECULE &mol) {
    ...
    if (Opt_params.IRC_direction == OPT_PARAMS::BACKWARD) sign = -1;
    ...
    for (std::size_t i = 0; i < steps.size(); ++i) {
        if (i == 0)
            DE = g_step(i).g_energy();
        else
            DE = g_step(i).g_energy() - g_step(i - 1).g_energy();
    ...
    for (int j = 0; j < dim / blocks; j++) {
        ...
        oprintf_out("\n");
        for (std::size_t i = 0; i < steps.size(); ++i) {
            oprintf_out("@IRC  %3d %9.2lf %9.5lf  %9.5lf   ", i, sign * g_step(i).g_step_dist(),
                        sign * g_step(i).g_arc_dist(), sign * g_step(i).g_line_dist());
            for (int k = (j * blocks); k < ((j + 1) * blocks); k++) oprintf_out("%13.8f", g_step(i).g_q()[k]);
            oprintf_out("\n");
        }
        oprintf_out("@IRC --------------------------------------");
        for (int i = (j * blocks); i < ((j + 1) * blocks); i++) {
    ...
    if (dim % blocks != 0) {
        ...
        for (std::size_t i = 0; i < steps.size(); ++i) {
            oprintf_out("@IRC  %3d %9.2lf %9.5lf  %9.5lf   ", i, sign * g_step(i).g_step_dist(),
                        sign * g_step(i).g_arc_dist(), sign * g_step(i).g_line_dist());
            for (int k = (dim - (dim % blocks)); k < dim; k++) oprintf_out("%13.8f", g_step(i).g_q()[k]);
            oprintf_out("\n");
        }
        oprintf_out("@IRC --------------------------------------");
        for (int i = (dim - (dim % blocks)); i < dim; i++) {
...
void MOLECULE::irc_step(void) {
    ...
    if (p_irc_data->sphere_step == 1 && p_irc_data->size() > 3) {
        ...
        if (u_f_q_dot < -0.7 || (std::fabs(p_irc_data->g_line_dist(p_irc_data->size() - 1) -
                                           p_irc_data->g_line_dist(p_irc_data->size() - 2)) < s * 10e-03)) {
    ...
    if (at_FS) {
        ...
        if (at_TS) {
            ...
            v = init_array(Nintco);
            for (int i = 0; i < Nintco; i++) v[i] = -f_q[i];
        }
        ...
        int point_number =
            Opt_params.IRC_direction == OPT_PARAMS::FORWARD ? p_irc_data->size() - 1 : -(p_irc_data->size() - 1);
        set_geom_array(x);
        oprintf_out("\n@IRC\n");
        ...
        dq_h = 0;
        for (int i = 0; i < Nintco; i++) dq_h += dq_u[i] * array_dot(H[i], dq_u, Nintco);
    ...
    for (int i = 0; i < Nintco; i++) p[i] = q[i] - q_pivot[i];
    for (int i = 0; i < Nintco; i++) p_0[i] = p[i] - dq_0[i];
    for (int i = 0; i < Nintco; i++) p_m[i] = array_dot(rootG_inv[i], p, Nintco);
    for (int i = 0; i < Nintco; i++) p_m0[i] = array_dot(rootG_inv[i], p_0, Nintco);
    ...
    for (int i = 0; i < Nintco; i++) g[i] = -f_q[i];
    for (int i = 0; i < Nintco; i++) g_0[i] = -f_q_0[i];
    for (int i = 0; i < Nintco; i++) g_m[i] = array_dot(rootG_reg[i], g, Nintco);
    for (int i = 0; i < Nintco; i++) g_m0[i] = array_dot(rootG_reg[i], g_0, Nintco);
    ...
    for (int i = 0; i < Nintco; i++) p_h[i] = array_dot(p_m, V[i], Nintco);
    double *g_h = init_array(Nintco);  // in the basis of H_m
    for (int i = 0; i < Nintco; i++) g_h[i] = array_dot(g_m, V[i], Nintco);
    ...
    while (std::fabs(lambda - old_lambda) > 1e-16) {
        ...
        if (lagrangian * old_lagrangian < 0) {
            ...
            old_lambda = lambda;
            lambda +=
                h_f * (1 + 0.5 * h_f * df[2] / df[1]) / (1 + (df[2] / df[1]) * h_f + (df[3] / (6 * df[1])) * h_f * h_f);
        } else {
            old_lambda = lambda;
            lambda += h_f * (24 * df[1] + 24 * df[2] * h_f + 4 * df[3] * h_f * h_f) /
                      (24 * df[1] + 36 * h_f * df[2] + 6 * (df[2] * df[2] / df[1]) * h_f * h_f + 8 * df[3] * h_f * h_f +
                       df[4] * h_f * h_f * h_f);
        }
        ...
        if (lag_iter > 200) throw(INTCO_EXCEPT("Could not converge lagrangian for constrained minimization"));
    }
    ...
    for (int i = 0; i < Nintco; i++) H_m[i][i] -= lambda;
    double **H_m_lag_inv = symm_matrix_inv(H_m, Nintco, 1);

    double *g_lag_p = init_array(Nintco);
    for (int i = 0; i < Nintco; i++) g_lag_p[i] = lambda * p_m[i] - g_m[i];
    ...
    /* START PRINTING */
    {
        ...
        for (int i = 0; i < Nintco; i++) Ginv_p[i] = array_dot(Ginv[i], p, Nintco);
        free_matrix(Ginv);
        ...
        for (int i = 0; i < Nintco; ++i) f_tan[i] -= overlap * Ginv_p[i];
        free_array(Ginv_p);
        ...
        if (p_irc_data->go) {
            if (p_irc_data->sphere_step == 1) {
                oprintf_out("\n@IRC");
                oprintf_out(
                    "\n@IRC Point   Sphere Step       Energy          DE         MAX Force     RMS Force      MAX Disp "
                    "     RMS Disp   \n");
                oprintf_out(
                    "@IRC "
                    "--------------------------------------------------------------------------------------------------"
                ...
                oprintf_out(
                    "\n     Point   Sphere Step       Energy          DE         MAX Force     RMS Force      MAX Disp "
                    "     RMS Disp   \n");
                oprintf_out(
                    "     "
                    "--------------------------------------------------------------------------------------------------"
            ...
            oprintf_out(
                "@IRC %3d   %8d      %16.8f %10.2e %1s  %10.2e %1s  %10.2e %1s  %10.2e %1s  %10.2e %1s  ~\n", point,
                p_irc_data->sphere_step, E, DE,
                (Opt_params.i_max_DE ? ((std::fabs(DE) < Opt_params.conv_max_DE) ? "*" : "") : "o"), max_force,
                (Opt_params.i_max_force ? ((std::fabs(max_force) < Opt_params.conv_max_force) ? "*" : "") : "o"),
                rms_force,
                (Opt_params.i_rms_force ? ((std::fabs(rms_force) < Opt_params.conv_rms_force) ? "*" : "") : "o"),
                max_disp, (Opt_params.i_max_disp ? ((std::fabs(max_disp) < Opt_params.conv_max_disp) ? "*" : "") : "o"),
                rms_disp,
                (Opt_params.i_rms_disp ? ((std::fabs(rms_disp) < Opt_params.conv_rms_disp) ? "*" : "") : "o"));
            oprintf_out(
                "     "
                "------------------------------------------------------------------------------------------------------"
                "-----\n\n");
...
double step_N_factor(double **G, double *g, int Nintco) {
    ...
    for (int i = 0; i < Nintco; i++) {
        double Ggsum = 0;
        for (int j = i + 1; j < Nintco; j++) Ggsum += G[i][j] * g[j];
...
double *lowest_evector(double **H, int Nintco) {
    ...
    for (int i = 0; i < Nintco; i++)
        if (std::fabs(V[0][i]) > std::fabs(max_element)) max_element = V[0][i];

    int sign;
    (max_element == std::fabs(max_element)) ? sign = 1 : sign = -1;

    double *min_evector = init_array(Nintco);
    for (int i = 0; i < Nintco; i++) min_evector[i] = sign * V[0][i];
...
void interpolation(double *p, double *p_0, double *g, double *g_0, int dim) {
    ...
    for (int i = 0; i < dim; i++) pPer[i] = p_0[i] - cosTh * p[i];
