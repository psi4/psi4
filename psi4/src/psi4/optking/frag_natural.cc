...
int FRAG::form_natural_coord_combinations(void) {
    ...
    for (int i = 0; i < natom; ++i) {
        ...
        for (int j = 0; j < natom; ++j)
            if (connectivity[i][j]) one.push_back(j);
        bond.push_back(one);
    }

    // Make list of coordination numbers.
    int *CN = init_int_array(natom);
    for (int i = 0; i < natom; ++i) CN[i] = (int)bond[i].size();

    if (Opt_params.print_lvl > 1) {
        oprintf_out("  -Atom-  -Coord. number-\n");
        for (int i = 0; i < natom; ++i) oprintf_out("%8d%17d\n", i + 1, CN[i]);
    }
    ...
    for (std::size_t i = 0; i < rings_full.size(); ++i) {
        ...
        if (rings_full[i][1] > *rings_full[i].end()) std::reverse(rings_full[i].begin(), rings_full[i].end());
    }
    ...
    for (std::size_t i = 0; i < rings.size(); ++i) {
        ...
        if (!match) rings.push_back(rings_full[i]);
    }

    oprintf_out(" %d rings were detected\n", rings.size());
    for (std::size_t i = 0; i < rings.size(); ++i) {
        oprintf_out(" Ring %d : \n", i + 1);
        for (std::size_t j = 0; j < rings.size(); ++j) oprintf_out(" %d ", rings[i][j]);
    }

    // Array of ring / non-ring atoms.
    bool *inR = init_bool_array(natom);
    for (int i = 0; i < natom; ++i) inR[i] = false;

    for (std::size_t i = 0; i < rings.size(); ++i)
        for (std::size_t j = 0; j < rings[i].size(); ++j) inR[rings[i][j]] = true;
    ...
    */
    ...
    for (std::size_t i = 0; i < coords.simples.size(); ++i)
        if (coords.simples[i]->g_type() == stre_type) add_trivial_coord_combination(i);
    ...
    for (int i = 0; i < natom; ++i) {
        ...
        if (CN[i] == 2) {
            ...
            if (v3d_angle(geom[bond_to_T[i][0]], geom[i], geom[bond_to_T[i][1]], val)) {  // computable
                ...
                if (val > Opt_params.linear_bend_threshold)  // ~175 degrees
                    pA->make_lb_normal();
                int A = find(pA);
                cc_index.push_back(A);
                cc_coeff.push_back(1.0);
        ...
        // *** Coordination == 3 cases ***
            ...
            if (ok && std::fabs(val) < _pi / 9) {  // less than 20 degrees?
                ...
                cc_index.push_back(X1);
                cc_coeff.push_back(1.0);
                cc_index.push_back(X2);
                cc_coeff.push_back(1.0);
                cc_index.push_back(X3);
                cc_coeff.push_back(1.0);
                coords.index.push_back(cc_index);
                coords.coeff.push_back(cc_coeff);
                cc_index.clear();
