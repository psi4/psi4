...
class INTERFRAG {
   ...
   public:
    ...
    INTERFRAG(FRAG *A_in, FRAG *B_in, int A_index_in, int B_index_in, double **weightA_in, double **weightB_in,
              int ndA_in = 3, int ndB_in = 3, bool use_principal_axes = false);

    ~INTERFRAG() { delete inter_frag; }

    // update location of reference points using fragment member geometries
    void update_reference_points(void) { update_reference_points(A->geom, B->geom); }
    ...
    */
    ...
    void compute_derivative_B(int coord_index, GeomType new_geom_A, GeomType new_geom_B, double **Bprim, int A_offset,
                              int B_offset);
