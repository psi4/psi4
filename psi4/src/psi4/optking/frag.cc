...
FRAG::~FRAG() {
    ...
    coords.clear_combos();
    for (size_t i = 0; i < coords.simples.size(); ++i) delete coords.simples[i];
...
void FRAG::set_masses(void) {
    int i;
    for (i = 0; i < natom; ++i) mass[i] = Z_to_mass[(int)Z[i]];
    return;
}
...
void FRAG::update_connectivity_by_distances(void) {
    ...
    for (i = 0; i < natom; ++i)
        for (j = 0; j < natom; ++j) connectivity[i][j] = false;
...
void FRAG::update_connectivity_by_bonds(void) {
    for (int i = 0; i < natom; ++i)
        for (int j = 0; j < natom; ++j) connectivity[i][j] = false;
...
int FRAG::add_auxiliary_bonds(void) {
    ...
    for (int i = 0; i < natom; ++i) Zint[i] = (int)Z[i];
        ...
        for (int b = a + 1; b < natom; ++b) {
            ...
            if (R < Rcov * Opt_params.auxiliary_bond_factor) {
                ...
                for (int c = 0; c < natom; ++c)
                    if (c != a && c != b)
                        if (connectivity[a][c] && connectivity[b][c]) omit = true;
                    ...
                    if (c != a && c != b)
                        if (connectivity[c][a])
                            for (int d = 0; d < natom; ++d)
                                if (d != a && d != b && d != c)
                                    if (connectivity[d][c] && connectivity[d][b]) omit = true;
...
bool FRAG::present(const SIMPLE_COORDINATE *one) const {
    for (size_t k = 0; k < coords.simples.size(); ++k) {
        if (*one == *(coords.simples[k])) return true;
    }
    return false;
}
...
int FRAG::form_trivial_coord_combinations(void) {
    coords.clear_combos();
    for (size_t s = 0; s < coords.simples.size(); ++s) add_trivial_coord_combination(s);
...
bool FRAG::is_noncart_present(void) const {
    for (size_t k = 0; k < coords.simples.size(); ++k) {
        if (coords.simples[k]->g_type() != INTCO_TYPE::cart_type) return true;
    }
...
int FRAG::find(const SIMPLE_COORDINATE *one) const {
    for (size_t k = 0; k < coords.simples.size(); ++k) {
        if (*one == *(coords.simples[k])) return k;
    }
    return coords.simples.size();
}

// Return values of all coordinates.
double *FRAG::coord_values(void) const { return coords.values(geom); }
double *FRAG::coord_values(GeomType new_geom) const { return coords.values(new_geom); }

// return the value of a single internal
double FRAG::coord_value(int coord_index) const { return coords.value(geom, coord_index); }
double FRAG::coord_value(GeomType new_geom, int coord_index) const { return coords.value(new_geom, coord_index); }
...
void FRAG::compute_B(double **B, int coord_offset, int atom_offset) const {
    for (int cc = 0; cc < Ncoord(); ++cc)
        for (int x = 0; x < 3 * natom; ++x) B[coord_offset + cc][3 * atom_offset + x] = 0.0;

    for (int cc = 0; cc < Ncoord(); ++cc) coords.DqDx(geom, cc, B[coord_offset + cc], atom_offset);
}
...
void FRAG::compute_derivative_B(GeomType new_geom, int coord_index, double **Bprime, int frag_atom_offset) const {
...
double **FRAG::compute_constraints(void) const {
    ...
    for (size_t i = 0; i < coords.simples.size(); ++i)
        if (coords.simples[i]->is_frozen()) C[i][i] = 1.0;
...
void FRAG::freeze_coords(void) {
    for (size_t i = 0; i < coords.simples.size(); ++i) coords.simples[i]->freeze();
}
...
void FRAG::compute_G(double **G, bool use_masses) const {
    ...
    if (use_masses) {
        for (int i = 0; i < Ncoord(); ++i)
            for (int a = 0; a < natom; ++a)
                for (int xyz = 0; xyz < 3; ++xyz) B[i][3 * a + xyz] /= sqrt(mass[a]);
    }
...
void FRAG::fix_tors_near_180(void) {
    for (size_t i = 0; i < coords.simples.size(); ++i)
        if (coords.simples[i]->g_type() == tors_type) coords.simples[i]->fix_tors_near_180(geom);
}
...
void FRAG::fix_oofp_near_180(void) {
    for (size_t i = 0; i < coords.simples.size(); ++i)
        if (coords.simples[i]->g_type() == oofp_type) coords.simples[i]->fix_oofp_near_180(geom);
}

void FRAG::set_geom_array(double *geom_array_in) {
    int xyz, i, cnt = 0;
    for (i = 0; i < natom; ++i)
        for (xyz = 0; xyz < 3; ++xyz) geom[i][xyz] = geom_array_in[cnt++];
}

void FRAG::set_geom(double **geom_in) {
    for (int i = 0; i < natom; ++i)
        for (int xyz = 0; xyz < 3; ++xyz) geom[i][xyz] = geom_in[i][xyz];
}
...
double *FRAG::g_geom_array(void) {
    ...
    for (int i = 0; i < natom; ++i)
        for (int xyz = 0; xyz < 3; ++xyz) geom_array[cnt++] = geom[i][xyz];
    return geom_array;
}
...
double *FRAG::g_grad_array(void) {
    ...
    for (i = 0; i < natom; ++i)
        for (xyz = 0; xyz < 3; ++xyz) grad_array[cnt++] = grad[i][xyz];
    return grad_array;
}
...
bool **FRAG::g_connectivity(void) const {
    ...
    for (int i = 0; i < natom; ++i)
        for (int j = 0; j <= i; ++j) c[i][j] = c[j][i] = connectivity[i][j];
    return c;
}

const bool *const *FRAG::g_connectivity_pointer(void) const { return connectivity; }
...
double *FRAG::com(GeomType in_geom) {
    ...
    for (int i = 0; i < g_natom(); ++i) {
        sum += mass[i];
        for (int xyz = 0; xyz < 3; ++xyz) center[xyz] += mass[i] * in_geom[i][xyz];
    }
    for (int xyz = 0; xyz < 3; ++xyz) center[xyz] /= sum;
    return center;
}
    ...
    for (int atom = 0; atom < g_natom(); ++atom) {
        ...
        for (xyz = 0; xyz < 3; ++xyz) {
            for (xyz2 = 0; xyz2 < 3; ++xyz2) {
                if (xyz == xyz2) tval += (in_geom[atom][xyz] - center[xyz]) * (in_geom[atom][xyz] - center[xyz]);
...
INTCO_TYPE FRAG::get_simple_type(int simple_index) { return coords.simples.at(simple_index)->g_type(); }
