...
bool FRAG::apply_frozen_constraints(string R_string, string B_string, string D_string, string C_string) {
    ...
    if (R_atoms.size() % 2 != 0) throw(INTCO_EXCEPT("Frozen distance string should contain even number of atoms."));
    if (B_atoms.size() % 3 != 0) throw(INTCO_EXCEPT("Frozen bend string should contain 3*(whole number) of atoms."));
    if (D_atoms.size() % 4 != 0)
    ...
    if (D_atoms.size()) {
        ...
        for (std::size_t i = 0; i < D_atoms.size(); i += 4)
            oprintf_out("\t %5d %5d %5d %5d\n", D_atoms[i] + 1, D_atoms[i + 1] + 1, D_atoms[i + 2] + 1,
                        D_atoms[i + 3] + 1);
    }

    if (!R_atoms.size() && !B_atoms.size() && !D_atoms.size() && !Carts.size()) return false;
    ...
    for (std::size_t i = 0; i < R_atoms.size(); i += 2) {
        ...
        if (a >= natom || b >= natom) throw(INTCO_EXCEPT("Impossibly large index for atom in frozen distance string."));
    ...
    for (std::size_t i = 0; i < Carts.size(); ++i) {
        ...
        if (atom >= natom) throw(INTCO_EXCEPT("Impossibly large index for atom in frozen cartesian string."));
...
bool FRAG::apply_fixed_constraints(string R_string, string B_string, string D_string) {
    ...
    if (!R.size() && !B.size() && !D.size()) return false;

    if (R.size()) {
        oprintf_out("\tFixed distance atom list: \n");
        for (std::size_t i = 0; i < R.size(); ++i) oprintf_out("\t %5d %5d\n", R[i].atoms[0] + 1, R[i].atoms[1] + 1);
    }
    ...
    if (D.size()) {
        ...
        for (std::size_t i = 0; i < D.size(); ++i)
            oprintf_out("\t %5d %5d %5d %5d\n", D[i].atoms[0] + 1, D[i].atoms[i + 1] + 1, D[i].atoms[2] + 1,
                        D[i].atoms[3] + 1);
    }
    ...
    for (std::size_t i = 0; i < R.size(); ++i) {
        ...
        if (a >= natom || b >= natom) throw(INTCO_EXCEPT("Impossibly large index for atom in fixed distance string."));
        ...
        if ((std::size_t)index == coords.simples.size())
            ...
            coords.simples[index]->set_fixed_eq_val(R[i].eq_val /
                                                    _bohr2angstroms);  // it's there already, add the fixed value
            delete one_stre;
        }
    }
    // do fixed bends
    for (std::size_t i = 0; i < B.size(); ++i) {
        ...
        if ((std::size_t)index == coords.simples.size())
            ...
            coords.simples[index]->set_fixed_eq_val(B[i].eq_val / 180.0 *
                                                    _pi);  // it's there already, add the fixed value
            delete one_bend;
        }
    }
    // do fixed dihedrals
...
std::vector<frozen_cart> split_to_frozen_cart(string &str) {
    ...
    while (std::getline(ss, item, delim)) {
        if (item.find_first_not_of(" ") != string::npos) {  // Ignore empty entries.
            ...
            if (new_entry) {
                ...
                if (len > 3)
                    throw(INTCO_EXCEPT(
                        "Frozen cartesian specification (X, XY, ...) should have no more than 3 letters."));
    ...
    if (!new_entry) throw(INTCO_EXCEPT("In frozen cartesian specification did not find pairs of valid entries."));
    return C;
}
