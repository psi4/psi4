...
void MOLECULE::forces(void) {
    ...
    if (Opt_params.print_lvl > 3) oprint_array_out_precise(f_x, Ncart);
    ...
    for (int i = 0; i < Nintco; ++i)
        for (int k = 0; k < Ncart; ++k)
            for (int j = 0; j < Nintco; ++j) G[i][j] += B[i][k] * /* u[k] * */ B[j][k];
    free_matrix(B);
    ...
    for (std::size_t f = 0; f < fb_fragments.size(); ++f) {
        ...
        for (int i = 0; i < fb_fragments[f]->Ncoord(); ++i) f_q[g_fb_fragment_coord_offset(f) + i] = fb_force[i];
    }
...
bool MOLECULE::has_fixed_eq_vals(void) {
    for (std::size_t f = 0; f < fragments.size(); ++f)
        for (int i = 0; i < fragments[f]->Ncoord(); ++i)
            if (fragments[f]->coord_has_fixed_eq_val(i)) return true;
...
bool MOLECULE::is_noncart_present(void) const {
    ...
    for (std::size_t f = 0; f < fragments.size(); ++f)
        if (fragments[f]->is_noncart_present()) return true;
...
void MOLECULE::project_f_and_H(void) {
    ...
    for (int i = 0; i < Nintco; ++i)
        if (C[i][i] != 0) constraints_present = true;
    ...
    if (constraints_present) {
        ...
        free_matrix(T3);
        for (int i = 0; i < Nintco; ++i)
            for (int j = 0; j < Nintco; ++j) P[i][j] -= T2[i][j];
        free_matrix(T2);
    }
    free_matrix(C);
    ...
    if (Opt_params.print_lvl >= 3) {
        ...
        if (Opt_params.fb_fragments) oprintf_out("\tFB external coordinates are not projected.\n");
...
void MOLECULE::apply_intrafragment_step_limit(double *&dq) {
    ...
    for (std::size_t f = 0; f < fragments.size(); ++f)
        for (int i = 0; i < fragments[f]->Ncoord(); ++i)
            if ((scale * sqrt(array_dot(dq, dq, dim))) > limit) scale = limit / sqrt(array_dot(dq, dq, dim));
    ...
    if (scale != 1.0) {
        ...
        for (std::size_t f = 0; f < fragments.size(); ++f)
            for (int i = 0; i < fragments[f]->Ncoord(); ++i) dq[g_coord_offset(f) + i] *= scale;
    }
}
...
std::vector<int> MOLECULE::validate_angles(double const *const dq) {
    ...
    for (std::size_t f = 0; f < fragments.size(); ++f) {
        ...
        for (std::size_t i = 0; i < frag_angle.size(); ++i) lin_angle.push_back(frag_angle[i]);
...
void MOLECULE::H_guess(void) const {
    ...
    if (Opt_params.intrafragment_H == OPT_PARAMS::SCHLEGEL || Opt_params.intrafragment_H == OPT_PARAMS::FISCHER ||
        Opt_params.intrafragment_H == OPT_PARAMS::LINDH_SIMPLE || Opt_params.intrafragment_H == OPT_PARAMS::SIMPLE) {
        for (std::size_t f = 0; f < fragments.size(); ++f) {
        ...
        for (std::size_t I = 0; I < interfragments.size(); ++I) {
            ...
            for (int i = 0; i < interfragments[I]->Ncoord(); ++i)
                for (int j = 0; j < interfragments[I]->Ncoord(); ++j)
                    H[g_interfragment_coord_offset(I) + i][g_interfragment_coord_offset(I) + j] = H_interfrag[i][j];
...
bool MOLECULE::cartesian_H_to_internals(double **H_cart) const {
    ...
    for (int q = 0; q < Nintco; ++q) {
        ...
        for (int i = 0; i < Ncart; ++i)
            for (int j = 0; j < Ncart; ++j) H_cart[i][j] -= grad_q[q] * dq2dx2[i][j];
...
double *MOLECULE::g_masses(void) const {
    ...
    int cnt = 0;
    for (std::size_t f = 0; f < fragments.size(); ++f)
        for (int i = 0; i < fragments[f]->g_natom(); ++i) u[cnt++] = fragments[f]->g_mass(i);
    return u;
}
...
double *MOLECULE::g_Z(void) const {
    ...
    for (std::size_t f = 0; f < fragments.size(); ++f) {
        ...
        for (int i = 0; i < fragments[f]->g_natom(); ++i) Zs[cnt++] = frag_Z[i];
    }
    return Zs;
}
...
double **MOLECULE::compute_B(void) const {
    ...
    for (std::size_t f = 0; f < fragments.size(); ++f) fragments[f]->compute_B(B, g_coord_offset(f), g_atom_offset(f));
...
double **MOLECULE::compute_derivative_B(int intco_index) const {
    ...
    if (fragment_index == -1) throw(INTCO_EXCEPT("MOLECULE::compute_derivative_B() could not find intco_index"));
            ...
            for (int b = 0; b < frag_natom; ++b) {
                ...
                /* old way before more complicated combinations
                    ...
                    for (int a=0; a<natom_intco; ++a) {
                      atom_a = g_atom_offset(fragment_index) +
                   fragments[fragment_index]->g_simple_atom(coordinate_index, a); for (int b=0; b<natom_intco; ++b) {
                        atom_b = g_atom_offset(fragment_index) +
                   fragments[fragment_index]->g_simple_atom(coordinate_index, b); for (int xyz_a=0; xyz_a<3; ++xyz_a)
                        for (int xyz_b=0; xyz_b<3; ++xyz_b)
                          dq2dx2[3*atom_a + xyz_a][3*atom_b + xyz_b] = dq2dx2_frag[3*a+xyz_a][3*b+xyz_b];
                      }
                    }
    ...
    if (use_masses) {
        ...
        for (int i = 0; i < Nintco; ++i)
            for (int a = 0; a < g_natom(); ++a)
                for (int xyz = 0; xyz < 3; ++xyz) B[i][3 * a + xyz] /= sqrt(u[a]);
...
bool MOLECULE::apply_input_constraints(void) {
    ...
    if (!Opt_params.frozen_distance_str.empty() || !Opt_params.frozen_bend_str.empty() ||
        !Opt_params.frozen_dihedral_str.empty() || !Opt_params.frozen_cartesian_str.empty()) {
        oprintf_out("\tAssuming in current code that numbering for constraints corresponds to unified fragment.\n");
        frozen_present =
            fragments[0]->apply_frozen_constraints(Opt_params.frozen_distance_str, Opt_params.frozen_bend_str,
                                                   Opt_params.frozen_dihedral_str, Opt_params.frozen_cartesian_str);
    }

    if (!Opt_params.fixed_distance_str.empty() || !Opt_params.fixed_bend_str.empty() ||
        !Opt_params.fixed_dihedral_str.empty()) {
        oprintf_out("\tAssuming in current code that numbering for constraints corresponds to unified fragment.\n");
        fixed_present = fragments[0]->apply_fixed_constraints(Opt_params.fixed_distance_str, Opt_params.fixed_bend_str,
                                                              Opt_params.fixed_dihedral_str);
    }
...
int MOLECULE::add_cartesians(void) {
    int nadded = 0;
    for (std::size_t f = 0; f < fragments.size(); ++f) nadded += fragments[f]->add_cartesians();
    return nadded;
}
...
void MOLECULE::freeze_intrafragments(void) {
    ...
    for (std::size_t f = 0; f < fragments.size(); ++f) fragments[f]->freeze();
}
...
void MOLECULE::freeze_intrafragment_coords(void) {
    ...
    for (std::size_t f = 0; f < fragments.size(); ++f) fragments[f]->freeze_coords();
}

// Determine trivial coordinate combinations, i.e., don't combine..
int MOLECULE::form_trivial_coord_combinations(void) {
    int nadded = 0;
    for (std::size_t f = 0; f < fragments.size(); ++f) nadded += fragments[f]->form_trivial_coord_combinations();
    for (std::size_t I = 0; I < interfragments.size(); ++I)
...
int MOLECULE::form_delocalized_coord_combinations(void) {
    int nadded = 0;
    for (std::size_t f = 0; f < fragments.size(); ++f) nadded += fragments[f]->form_delocalized_coord_combinations();
    ...
    if (g_nfragment() == 2 && g_nfb_fragment() == 0) {  // there is only 1 fragment
        ...
        for (int cc = 0; cc < Ncoord(); ++cc) {
            ...
            for (int a = 0; a < Natom; ++a)
                for (int xyz = 0; xyz < 3; ++xyz) displaced_geom[a][xyz] += 0.1 * Bs[cc][3 * a + xyz];
            ...
            if (!symm_rfo_step) asymm_coord.push_back(cc);
        }
        ...
        for (std::size_t i = 0; i < asymm_coord.size(); ++i) oprintf_out(" %d", asymm_coord[i] + 1);
        oprintf_out("\n");

        for (std::size_t i = 0; i < asymm_coord.size(); ++i) {
            fragments[0]->erase_combo_coord(asymm_coord[i]);
            for (std::size_t j = i; j < asymm_coord.size(); ++j) asymm_coord[j] -= 1;
        }
        free_matrix(Bs);
...
int MOLECULE::form_natural_coord_combinations(void) {
    int nadded = 0;
    for (std::size_t f = 0; f < fragments.size(); ++f) nadded += fragments[f]->form_natural_coord_combinations();
    return nadded;
}
...
double **MOLECULE::compute_constraints(void) {
    ...
    for (std::size_t f = 0; f < fragments.size(); ++f) {
        ...
        for (i = 0; i < fragments[f]->Ncoord(); ++i)
            for (j = 0; j < fragments[f]->Ncoord(); ++j) C[g_coord_offset(f) + i][g_coord_offset(f) + j] = C_frag[i][j];
...
bool MOLECULE::is_coord_fixed(int coord_index) {
    ...
    for (std::size_t f = 0; f < fragments.size(); ++f)
        for (int i = 0; i < fragments[f]->Ncoord(); ++i) {
            if (cnt == coord_index) return fragments[f]->coord_has_fixed_eq_val(i);
            ++cnt;
        }
    return false;
}
...
void MOLECULE::update_fb_values(void) {
    for (std::size_t i = 0; i < fb_fragments.size(); ++i) {
        ...
        for (int iter = 0; iter < p_Opt_data->g_iteration(); ++iter) {
            ...
            for (int coord = 0; coord < 6; ++coord) vals[coord] += dq[g_fb_fragment_coord_offset(i) + coord];
        }
