...
INTERFRAG::INTERFRAG(FRAG *A_in, FRAG *B_in, int A_index_in, int B_index_in, double **weightA_in, double **weightB_in,
                     int ndA_in, int ndB_in, bool use_principal_axes) {
    A = A_in;
    B = B_in;
    A_index = A_index_in;
    B_index = B_index_in;
...
void INTERFRAG::add_coordinates_of_reference_pts(void) {
    ...
    for (int a = 0; a < A->g_natom(); ++a)
        if (A->Z[a] == 7 || A->Z[a] == 8 || A->Z[a] == 9 || A->Z[a] == 17) is_XA[a] = true;

    bool *is_XB = init_bool_array(g_natom());
    for (int b = 0; b < B->g_natom(); ++b)
        if (B->Z[b] == 7 || B->Z[b] == 8 || B->Z[b] == 9 || B->Z[b] == 17) is_XB[b] = true;
                    ...
                    for (int y = 0; y < B->natom; ++y) {
                        if (weightB[0][y] != 0.0 && is_XB[y]) {  // electronegative Y atom is part of B[0]
                            if (v3d_angle(A->geom[x], A->geom[h], B->geom[y], ang)) {  // check angle
                                if (ang < _pi / 2) one_stre->set_hbond(true);
                            }
                        }
                    }
                }
    ...
    if (one_stre->is_hbond()) oprintf_out("Detected H-bonding interfragment coordinate.\n");
...
void INTERFRAG::update_reference_points(GeomType new_geom_A, GeomType new_geom_B) {
    ...
    if (!principal_axes) {  // use fixed weights
        ...
        for (int xyz = 0; xyz < 3; ++xyz) inter_frag->geom[2][xyz] = fragment_com[xyz];
        ...
        for (i = 0; i < ndA - 1; ++i)  // i can only be 0 or 1
            for (int xyz = 0; xyz < 3; ++xyz) inter_frag->geom[1 - i][xyz] = fragment_com[xyz] + axes[i][xyz];
        ...
        fragment_com = B->com();
        for (int xyz = 0; xyz < 3; ++xyz) inter_frag->geom[3][xyz] = fragment_com[xyz];
        ...
        for (i = 0; i < ndB - 1; ++i)  // i can only be 0 or 1
            for (int xyz = 0; xyz < 3; ++xyz) inter_frag->geom[4 + i][xyz] = fragment_com[xyz] + axes[i][xyz];
        ...
        if (Opt_params.print_lvl >= 3) {
            ...
            for (int i = 2; i > 2 - ndA; --i)
                oprintf_out("%15.10lf %15.10lf %15.10lf\n", inter_frag->geom[i][0], inter_frag->geom[i][1],
                            inter_frag->geom[i][2]);
            for (int i = 0; i < ndB; ++i)
                oprintf_out("%15.10lf %15.10lf %15.10lf\n", inter_frag->geom[3 + i][0], inter_frag->geom[3 + i][1],
                            inter_frag->geom[3 + i][2]);
        }
    }
}
...
void INTERFRAG::freeze(void) { inter_frag->freeze(); }

// is coordinate J frozen?  J runs over only active coordinates.
bool INTERFRAG::is_frozen(int J) {
    if (J < 0 || J >= Ncoord()) throw(INTCO_EXCEPT("INTERFRAG::is_frozen() index J runs only over active coordinates"));
    return inter_frag->coords.simples[J]->is_frozen();
}

// are all interfragment modes frozen?
bool INTERFRAG::is_frozen(void) { return inter_frag->is_frozen(); }
...
double *INTERFRAG::coord_values(GeomType new_geom_A, GeomType new_geom_B) {
    ...
    for (int i = 0; i < Ncoord(); ++i) q[i] = inter_frag->coords.simples.at(i)->value(inter_frag->geom);
...
void INTERFRAG::compute_B(GeomType new_geom_A, GeomType new_geom_B, double **Bin, int coord_offset, int A_off,
                          int B_off) {
    update_reference_points(new_geom_A, new_geom_B);
    int natomA = A->natom;
    int natomB = B->natom;
    ...
    for (int i = 2; i >= 0; --i, ++cnt) {
        if (i < ndA) {
            ...
            for (int j = 0; j < A->g_natom(); ++j)
                if (weightA[i][j] != 0.0) oprintf(psi_fp, qc_fp, " %d/%5.3f", off_A + j + 1, weightA[i][j]);
    ...
    for (int i = 0; i < 3; ++i, ++cnt) {
        if (i < ndB) {
            ...
            for (int j = 0; j < B->g_natom(); ++j)
                if (weightB[i][j] != 0.0) oprintf(psi_fp, qc_fp, " %d/%5.3f", off_B + j + 1, weightB[i][j]);
            oprintf(psi_fp, qc_fp, "\n");
        }
    }
...
std::string INTERFRAG::get_coord_definition(int coord_index, int off_A, int off_B) const {
    ...
    for (int i = 0; i < ndA; ++i) {
        iss << "A" << i + 1;
        for (int j = 0; j < A->g_natom(); ++j)
            if (weightA[i][j] != 0.0) iss << j + 1 + off_A;
        iss << "\n";
    }
    for (int i = 0; i < ndB; ++i) {
        iss << "B" << i + 1;
        for (int j = 0; j < B->g_natom(); ++j)
            if (weightB[i][j] != 0.0) iss << j + 1 + off_B;
        iss << "\n";
    }
    return iss.str();
}
...
double **INTERFRAG::H_guess(void) {
    ...
    if (Opt_params.interfragment_H == OPT_PARAMS::FISCHER_LIKE) {
        ...
        if (Opt_params.interfragment_distance_inverse) rAB = inter_frag->coords.simples[0]->value(inter_frag->geom);
...
double **INTERFRAG::compute_constraints(void) const {
    ...
    for (int i = 0; i < 6; ++i) {
        if (D_on[i]) {
            if (inter_frag->coords.simples[cnt++]->is_frozen()) C[i][i] = 1.0;
        }
    }
    return C;
}
