    ...
    try {      // STEP2 ; do other exceptions
        try {  // STEP1 ; check bad_step exception first
            ...
            if (if_intco.is_open()) {  // old internal coordinates are present
                ...
                if (mol1->Ncoord() == 0) mol1->form_trivial_coord_combinations();
                ...
                if (Opt_params.coordinates != OPT_PARAMS::CARTESIAN) mol1->add_intrafragment_hbonds();
                ...
                if (Opt_params.coordinates == OPT_PARAMS::REDUNDANT ||
                    ...
                    Opt_params.coordinates == OPT_PARAMS::NATURAL || Opt_params.coordinates == OPT_PARAMS::BOTH) {
                ...
                if (Opt_params.coordinates == OPT_PARAMS::CARTESIAN) mol1->add_cartesians();
                ...
                if (Opt_params.coordinates == OPT_PARAMS::BOTH) mol1->add_cartesians();  // also adds trivial combos
            ...
            if (Opt_params.fb_fragments_only) mol1 = new MOLECULE(0);  // construct an empty molecule
            if (Opt_params.fb_fragments) mol1->add_fb_fragments();     // add EFP fragments
            ...
            if (p_Opt_data->g_iteration() == 1)
                if (Opt_params.print_trajectory_xyz_file && Opt_params.opt_type != OPT_PARAMS::IRC) mol1->print_xyz(-1);
            ...
            if (Opt_params.test_B) mol1->test_B();

            if (Opt_params.test_derivative_B) mol1->test_derivative_B();
...
#if defined(OPTKING_PACKAGE_QCHEM)
            rem_write((int)converged,
                      REM_GEOM_OPT_CONVERGED);  // tell QChem if converged (return value ignored for now)
            ...
            if (converged) {
                if (Opt_params.opt_type == OPT_PARAMS::IRC) {
                    if (!p_irc_data->go) {
                        ...
                        delete p_Opt_data;
                        INTCO_EXCEPT::dynamic_level =
                            options.get_int("DYNAMIC_LEVEL");  // reset for future optimizations
            ...
            if (Opt_params.print_trajectory_xyz_file && Opt_params.opt_type != OPT_PARAMS::IRC) mol1->print_xyz();
            ...
            if ((!Opt_params.dynamic ||
                 (p_Opt_data->g_consecutive_backsteps() < Opt_params.consecutive_backsteps_allowed)) &&
