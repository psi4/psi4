...
inline double v3d_norm(const double *A) { return sqrt(A[0] * A[0] + A[1] * A[1] + A[2] * A[2]); }

// take dot product of two vectors
inline double v3d_dot(const double *A, const double *B) { return (A[0] * B[0] + A[1] * B[1] + A[2] * B[2]); }
...
inline bool v3d_eAB(const double *A, const double *B, double *eAB, const double min_norm = 1.0e-8,
