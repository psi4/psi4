...
bool myline(std::ifstream &fin, std::vector<std::string> &tokens, int &line_num) {
    ...
    while (read_next && !fin.eof()) {
        ...
        if (line_present) {
            ++line_num;
            streamline << sline;
            while (streamline >> sline) tokens.push_back(sline);
...
bool MOLECULE::read_coords(std::ifstream &fintco) {
    ...
    while (line_present) {
        if ((vline[0] == "F") || (vline[0] == "F*")) {  // read first and last atom for fragment
            ...
            // Normalize
            double tval = 0.0;
            for (std::size_t j = 0; j < cc_coeff.size(); ++j) tval += cc_coeff[j] * cc_coeff[j];
            tval = 1.0 / sqrt(tval);
            for (std::size_t j = 0; j < cc_coeff.size(); ++j) cc_coeff[j] *= tval;
            ...
            if (vline.size() != 3) {
                error << "Format of interfragment line is \"I integer(1st_frag) integer(2nd_frag)\" in line "
                      << line_num << ".\n";
                throw(INTCO_EXCEPT(error.str().c_str()));
            }
            ...
            line_present = myline(fintco, vline, line_num);
            if (!line_present) {
                error << "Missing line " << line_num + 1
                      << " to indicate with six 1/0's which coordinates are active.\n";
                throw(INTCO_EXCEPT(error.str().c_str()));
            }
            if (vline.size() != 6) {
            ...
            while (line_present && !end_of_if) {
                if (vline[0] == "A1" || vline[0] == "A2" || vline[0] == "A3" || vline[0] == "B1" || vline[0] == "B2" ||
                    vline[0] == "B3") {
                    if (vline.size() < 2) {
                    ...
                    for (std::size_t i = 1; i < vline.size(); ++i) {
                        ...
                        if (vline[0] == "A1")
                            A1.push_back(a - g_atom_offset(first_frag));
                        else if (vline[0] == "A2")
                            A2.push_back(a - g_atom_offset(first_frag));
                        else if (vline[0] == "A3")
                            A3.push_back(a - g_atom_offset(first_frag));
                        else if (vline[0] == "B1")
                            B1.push_back(a - g_atom_offset(second_frag));
                        else if (vline[0] == "B2")
                            B2.push_back(a - g_atom_offset(second_frag));
                        else if (vline[0] == "B3")
                            B3.push_back(a - g_atom_offset(second_frag));
                    }
            ...
            if (ndA > 0)
                for (std::size_t i = 0; i < A1.size(); ++i) weightA[0][A1[i]] = 1.0 / A1.size();

            if (ndA > 1)
                for (std::size_t i = 0; i < A2.size(); ++i) weightA[1][A2[i]] = 1.0 / A2.size();

            if (ndA > 2)
                for (std::size_t i = 0; i < A3.size(); ++i) weightA[2][A3[i]] = 1.0 / A3.size();

            if (ndB > 0)
                for (std::size_t i = 0; i < B1.size(); ++i) weightB[0][B1[i]] = 1.0 / B1.size();

            if (ndB > 1)
                for (std::size_t i = 0; i < B2.size(); ++i) weightB[1][B2[i]] = 1.0 / B2.size();

            if (ndB > 2)
                for (std::size_t i = 0; i < B3.size(); ++i) weightB[2][B3[i]] = 1.0 / B3.size();

            INTERFRAG *one_IF = new INTERFRAG(fragments[first_frag], fragments[second_frag], first_frag, second_frag,
                                              weightA, weightB, ndA, ndB);

            // freeze coordinates, need to convert to the index of active coordinates
            int if_index = 0;
            for (int i = 0; i < 6; ++i) {
                if (D_on[i]) {
                    if (D_frozen[i]) one_IF->freeze(if_index++);
                }
            }
...
bool FRAG::read_coord(std::vector<std::string> &s, int offset) {
    ...
    if ((s[0] == "R") || (s[0] == "H")) {
        if (s.size() != 3 && s.size() != 4) throw(INTCO_EXCEPT("Format of stretch entry is \"R atom_1 atom_2\""));
        if (s.size() == 4) {
            if (stof(s[3], &eq_val))
                has_eq_val = true;
            else
        ...
        if (!stoi(s[1], &a) || !stoi(s[2], &b)) throw(INTCO_EXCEPT("Format of stretch entry is \"R atom_1 atom_2\""));
        --a;
        --b;
    ...
    if (s[0] == "X") {  // read cartesian coordinate
        if (s.size() != 3 && s.size() != 4) throw(INTCO_EXCEPT("Format of cartesian entry is \"X atom_1 xyz\""));
        if (s.size() == 4) {
            if (stof(s[3], &eq_val))
                has_eq_val = true;
            else
