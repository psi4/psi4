...
bool INTERFRAG::orient_fragment(double *dq, double *fq) {
    ...
    for (i = 0; i < Ncoord(); ++i) {
        if (D_on[i]) q_target[i] = q_orig[i] + dq[i];
    }
    ...
    if (ndA < 3)
        for (xyz = 0; xyz < 3; ++xyz) ref_A[2][xyz] = (xyz + 1);

    if (ndA < 2)
        for (xyz = 0; xyz < 3; ++xyz) ref_A[1][xyz] = (xyz + 2);

    // compute current location of reference points on A and B
    for (pts = 0; pts < ndA; ++pts)
        for (i = 0; i < A->natom; ++i)
            for (xyz = 0; xyz < 3; ++xyz) ref_A[pts][xyz] += weightA[pts][i] * A->geom[i][xyz];

    for (pts = 0; pts < ndB; ++pts)
        for (i = 0; i < B->natom; ++i)
            for (xyz = 0; xyz < 3; ++xyz) ref_B[pts][xyz] += weightB[pts][i] * B_geom[i][xyz];

    // compute B1-B2 distance, B2-B3 distance, and B1-B2-B3 angle
    if (ndB > 1) R_B1B2 = v3d_dist(ref_B[1], ref_B[0]);
    ...
    if (ndB > 1) zmat_point(ref_A[1], ref_A[0], ref_B_final[0], R_B1B2, theta_B, tau, ref_B_final[1]);
    if (ndB > 2) zmat_point(ref_A[0], ref_B_final[0], ref_B_final[1], R_B2B3, B_angle, phi_B, ref_B_final[2]);
    ...
    for (xyz = 0; xyz < 3; ++xyz) {
        ...
        for (i = 0; i < B->natom; ++i) B_geom[i][xyz] += tval;
    }
    ...
    if (ndB > 1) { /* move fragment B to place reference point B2 in correct location */
        ...
        if (std::fabs(B_angle) > 1.0e-7) {
            ...
            for (xyz = 0; xyz < 3; ++xyz)
                for (i = 0; i < B->natom; ++i) B_geom[i][xyz] -= ref_B[0][xyz];
            ...
            for (pts = 0; pts < ndB; ++pts)
                for (xyz = 0; xyz < 3; ++xyz)
                    for (i = 0; i < B->natom; ++i) ref_B[pts][xyz] += weightB[pts][i] * B_geom[i][xyz];
        }
    ...
    if (ndB == 3) {  // move fragment B to place reference point B3 in correct location
        ...
        if (std::fabs(B_angle) > 1.0e-10) {
            ...
            for (xyz = 0; xyz < 3; ++xyz)
                for (i = 0; i < B->natom; ++i) B_geom[i][xyz] -= ref_B[1][xyz];
...
void zmat_point(double *A, double *B, double *C, double R_CD, double theta_BCD, double phi_ABCD, double *D) {
    ...
    for (int xyz = 0; xyz < 3; xyz++) eY[xyz] /= sinABC;
    v3d_cross_product(eY, eBC, eX);
    for (int xyz = 0; xyz < 3; xyz++)
        D[xyz] = C[xyz] + R_CD * (-eBC[xyz] * cos(theta_BCD) + eX[xyz] * sin(theta_BCD) * cos(phi_ABCD) +
...
void rotate_vecs(double *w, double phi, double **v, int num_v) {
    ...
    for (int i = 0; i < num_v; ++i)
        for (int j = 0; j < 3; ++j) v[i][j] = v_new[i][j];
