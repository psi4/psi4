...
double **OOFP::DqDx(GeomType geom) const {
    ...
    for (int j = 0; j < 3; ++j) dqdx[0][j] = (tmp[j] - tmp2[j]) / rBA;
    ...
    for (int j = 0; j < 3; ++j) dqdx[2][j] = (tmp[j] - tmp3[j]) / rBC;
    ...
    for (int j = 0; j < 3; ++j) dqdx[3][j] = (tmp[j] - tmp3[j]) / rBD;

    // S vector for D
    for (int j = 0; j < 3; ++j) dqdx[1][j] = -1.0 * dqdx[0][j] - dqdx[2][j] - dqdx[3][j];
...
void OOFP::print_disp(std::string psi_fp, FILE *qc_fp, const double q_old, const double f_q, const double dq,
...
void OOFP::print_intco_dat(std::string psi_fp, FILE *qc_fp, int off) const {
    if (s_frozen)
        oprintf(psi_fp, qc_fp, "O*%6d%6d%6d%6d", s_atom[0] + 1 + off, s_atom[1] + 1 + off, s_atom[2] + 1 + off,
                s_atom[3] + 1 + off);
    else
        oprintf(psi_fp, qc_fp, "O %6d%6d%6d%6d", s_atom[0] + 1 + off, s_atom[1] + 1 + off, s_atom[2] + 1 + off,
                s_atom[3] + 1 + off);
...
bool OOFP::operator==(const SIMPLE_COORDINATE &s2) const {
    if (oofp_type != s2.g_type()) return false;
