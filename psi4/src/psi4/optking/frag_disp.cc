...
void FRAG::displace(double *dq, double *fq, int atom_offset) {
    ...
    if (ensure_convergence) {  // change step as necessary to get convergence
        ...
        while (!conv) {
            ...
            if (cnt > 0) {
                ...
                for (int i = 0; i < Nints; ++i) dq[i] = dq_orig[i] / (2 * cnt);
            }
    ...
    for (int i = 0; i < Nints; ++i)
        if (coords.simples[i]->is_frozen()) frag_constraints_present = true;
    ...
    if (frag_constraints_present) {
        ...
        for (int i = 0; i < Nints; ++i)
            if (coords.simples[i]->is_frozen()) dq_adjust_frozen[i] = q_orig[i] - q_before_adjustment[i];
    ...
    for (int i = 0; i < Nints; ++i) dq[i] = q_final[i] - q_orig[i];  // calculate dq from _target_

    for (int i = 0; i < Nints; ++i) {
        // passed through pi, but don't think this code is necessary; given the way values are computed
        if (coords.simples[i]->g_type() == tors_type || coords.simples[i]->g_type() == oofp_type) {
            if (dq[i] > _pi) {
                dq[i] = dq[i] - (2 * _pi);
...
bool FRAG::displace_util(double *dq, bool focus_on_constraints) {
    ...
    for (i = 0; i < Nints; ++i) q_target[i] = q_orig[i] + dq[i];

    if (Opt_params.print_lvl >= 2) {
        oprintf_out("\t In displace_util \n");
        oprintf_out("\t       Original         Target           Dq\n");
        for (i = 0; i < Nints; ++i) oprintf_out("\t%15.10lf%15.10lf%15.10lf\n", q_orig[i], q_target[i], dq[i]);
    }
    ...
    while (!bt_iter_done) {
        ...
        */
        ...
        for (i = 0; i < Ncarts; ++i) new_geom[i] += dx[i];
        ...
        if (focus_on_constraints) {
            ...
            for (i = 0; i < Nints; ++i)
                if (!coords.simples[i]->is_frozen()) q_target[i] = new_q[i];
        }

        for (i = 0; i < Nints; ++i) dq[i] = q_target[i] - new_q[i];

        free_array(new_q);

        // save first try in case doesn't converge
        if (bmat_iter_cnt == 0) {
            for (i = 0; i < Ncarts; ++i) first_geom[i] = new_geom[i];
    ...
    if (Opt_params.print_lvl >= 2) oprintf_out("\t---------------------------------------------------\n");

    if (Opt_params.print_lvl >= 2) {
        oprintf_out("\n\tReport of back-transformation:\n");
        oprintf_out("\t  int       q_target          Error\n");
        oprintf_out("\t-----------------------------------\n");
        for (i = 0; i < Nints; ++i) oprintf_out("\t%5d%15.10lf%15.10lf\n", i + 1, q_target[i], -dq[i]);
        oprintf_out("\n");
    }
    ...
    if (bt_converged) {
        ...
        rval = true;  // not converged and only for constraint fixing
