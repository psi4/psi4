...
class STEP_DATA {
   ...
   public:
    ...
    // save rest of stuff
    void save_step_info(double DE_predicted_in, double *unit_step_in, double dq_norm_in, double dq_gradient_in,
                        double dq_hessian_in, int Nintco);
...
class OPT_DATA {
   ...
   public:
    ...
    void save_step_info(double DE_predicted_in, double *unit_step_in, double dq_norm_in, double dq_gradient_in,
        ...
        steps[steps.size() - 1]->save_step_info(DE_predicted_in, unit_step_in, dq_norm_in, dq_gradient_in,
                                                dq_hessian_in, Nintco);
    }
    ...
    void set_rfo_eigenvector(double *evect_in) {
        for (int i = 0; i < Nintco; ++i) rfo_eigenvector[i] = evect_in[i];
    }

    // step data
    double *g_forces_pointer(void) const { return steps[steps.size() - 1]->g_forces_pointer(); }
    double *g_dq_pointer(void) const { return steps[steps.size() - 1]->g_dq_pointer(); }
    ...
    double g_energy(int i) const { return steps[i]->g_energy(); }
    double *g_forces_pointer(int i) const { return steps.at(i)->g_forces_pointer(); }
    ...
    double *g_geom_const_pointer(int i) const { return steps.at(i)->g_geom_const_pointer(); }
    double *g_dq_pointer(int i) const { return steps.at(i)->g_dq_pointer(); }
    double g_dq_norm(int i) const { return steps.at(i)->g_dq_norm(); }
    ...
    double g_dq_gradient(int i) const { return steps.at(i)->g_dq_gradient(); }
    ...
    double g_dq_hessian(int i) const { return steps.at(i)->g_dq_hessian(); }
    ...
    void reset_iteration_to_size(void) { iteration = steps.size() + 1; }
