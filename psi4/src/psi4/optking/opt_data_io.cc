...
bool opt_io_is_present(void) {
    ...
    if (psio_rd_toclen(PSI_OPTDATA_FILE_NUM) > 0) file_present = true;
    ...
    if (opt_data_stream.is_open()) {
        if (opt_data_stream.good()) file_present = true;
...
void opt_clean(void) {
    ...
    if (!Opt_params.keep_intcos) opt_intco_dat_remove();  // remove intco.dat
...
void opt_io_open(OPT_IO_FILE_STATUS status) {
    ...
    if (opt_data_stream.is_open() && (status == OPT_IO_OPEN_OLD)) return;

    if (opt_data_stream.is_open() && (status == OPT_IO_OPEN_NEW)) opt_data_stream.close();

    if (status == OPT_IO_OPEN_NEW) remove(QCHEM_OPTDATA_FILENAME);
