...
void MOLECULE::fragmentize(void) {
    ...
    for (i = 0; i < nfrag; ++i) {
        ...
        for (j = 0; j < natom; ++j)
            if (frag_atoms[i][j]) fatoms[i].push_back(j);
    }
    ...
    if (nfrag == 1) {
        ...
        if (Opt_params.fragment_mode == OPT_PARAMS::SINGLE) {
            ...
            for (int f1 = 0; f1 < nfrag; ++f1) frag_connectivity[f1][f1] = true;
            ...
            while (!all_connected) {
                for (int f2 = 0; f2 < nfrag; ++f2) {
                    for (int f1 = 0; f1 < f2; ++f1) {
                        ...
                        if (Rij > scale_dist * (R_i + R_j)) continue;
                ...
                for (int i = 0; i < nfrag; ++i) set_label[i] = i;
                ...
                for (int i = 0; i < nfrag; ++i)
                    for (int j = 0; j < nfrag; ++j) {
                        ...
                        if (Opt_params.print_lvl >= 2) {
                            ...
                            for (int k = 0; k < nfrag; ++k) oprintf_out(" %d", set_label[k]);
                            oprintf_out("\n");
                        }
                    }
                ...
                for (int i = 1; i < nfrag; ++i)
                    if (set_label[i] != set_label[0]) all_connected = false;
    ...
    for (int i = 0; i < nfrag; ++i) fatoms[i].clear();
    fatoms.clear();
