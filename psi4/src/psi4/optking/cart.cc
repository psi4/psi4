...
double CART::value(GeomType geom) const { return geom[s_atom[0]][xyz]; }
...
void CART::print_intco_dat(std::string psi_fp, FILE *qc_fp, int off) const {
    ...
    if (xyz == 0)
        ...
        oprintf(psi_fp, qc_fp, "     Y");
    else if (xyz == 2)
        oprintf(psi_fp, qc_fp, "     Z");
...
// print displacement
void CART::print_disp(std::string psi_fp, FILE *qc_fp, const double q_orig, const double f_q, const double dq,
...
bool CART::operator==(const SIMPLE_COORDINATE &s2) const {
    if (cart_type != s2.g_type()) return false;
