...
static inline double r_ref_table(int perA, int perB) {
    if (perA == 1) {
        if (perB == 1)
            return 1.35;  // Lindh 1.35
        else if (perB == 2)
            return 2.1;  // Lindh 2.1
        else
            return 2.53;
    } else if (perA == 2) {
...
static inline double Rcov(double ZA, double ZB) { return (cov_radii[(int)ZA] + cov_radii[(int)ZB]) / _bohr2angstroms; }

// This function generates various diagonal Hessian guesses.
double **FRAG::H_guess(void) {
    double **R = init_matrix(natom, natom);
    for (int i = 0; i < natom; ++i)
        for (int j = i + 1; j < natom; ++j) R[i][j] = R[j][i] = v3d_dist(geom[i], geom[j]);
        ...
        for (std::size_t i = 0; i < coords.simples.size(); ++i) {
            ...
            switch (q->g_type()) {
                ...
                case (bend_type):
                    ...
                    C = 0.44;
                    D = -0.42;
                    f[cnt++] = A + B / (pow(rABcov * rBCcov, D)) * exp(-C * (R[a][b] + R[b][c] - rABcov - rBCcov));
                    break;
                ...
                case (oofp_type):
                    ...
                    if (!v3d_oofp(geom[a], geom[b], geom[c], geom[d], phi)) phi = _pi / 4;
    ...
    for (std::size_t i = 0; i < coords.simples.size(); ++i) H_simple[i][i] = f[i];
    free_array(f);
