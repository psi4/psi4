...
void MOLECULE::print_geom_out(void) {
    ...
    for (std::size_t i = 0; i < fragments.size(); ++i) fragments[i]->print_geom(psi_outfile, qc_outfile);
}
...
void MOLECULE::print_geom_out_irc(void) {
    ...
    for (std::size_t i = 0; i < fragments.size(); ++i) fragments[i]->print_geom_irc(psi_outfile, qc_outfile);
}
...
void MOLECULE::print_xyz(int iter_shift) {
    ...
    for (std::size_t i = 0; i < fragments.size(); ++i) fragments[i]->print_geom("geoms.xyz", qc_fp);
#if defined(OPTKING_PACKAGE_QCHEM)
    fclose(qc_fp);
#endif
    return;
...
void MOLECULE::print_xyz_irc(int point, bool forward) {
    ...
    if (forward) {
        ...
        for (std::size_t i = 0; i < fragments.size(); ++i) fragments[i]->print_geom("irc_forward.xyz", qc_fp);
#if defined(OPTKING_PACKAGE_QCHEM)
        fclose(qc_fp);
#endif
    } else {
        ...
        for (std::size_t i = 0; i < fragments.size(); ++i) fragments[i]->print_geom("irc_backward.xyz", qc_fp);
#if defined(OPTKING_PACKAGE_QCHEM)
        fclose(qc_fp);
#endif
    }
...
void MOLECULE::print_intco_dat(std::string psi_fp, FILE *qc_fp) {
    ...
    for (std::size_t I = 0; I < interfragments.size(); ++I) {
        ...
        for (int i = 0; i < 6; ++i) oprintf(psi_fp, qc_fp, " %d", (int)interfragments[I]->coordinate_on(i));
...
std::string MOLECULE::get_coord_definition_from_global_index(int index) const {
    ...
    if (index < Nintra) {
        ...
        for (f = 0; f < fragments.size(); ++f)
            if (index < g_coord_offset(f)) break;
        --f;
    ...
    if (index < Nintra + Ninter) {
        ...
        for (f = 0; f < interfragments.size(); ++f)
            if (index < g_interfragment_coord_offset(f)) break;
        --f;
    ...
    if (index < Nintra + Ninter + Nfb) {
        for (std::size_t f = 0; f < fb_fragments.size(); ++f)
            if (index < g_fb_fragment_coord_offset(f)) break;
        --f;
...
void MOLECULE::print_coords(std::string psi_fp, FILE *qc_fp) const {
    for (std::size_t i = 0; i < fragments.size(); ++i) {
        ...
        if (Opt_params.coordinates == OPT_PARAMS::DELOCALIZED) {
            ...
            if ((Opt_params.print_lvl > 1 && p_Opt_data->g_iteration() == 1) || Opt_params.print_lvl > 3)
