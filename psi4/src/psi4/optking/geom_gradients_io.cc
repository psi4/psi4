...
void MOLECULE::read_geom_grad(void) {
  ...
  try {
    ...
    if (Opt_params.fb_fragments) {
        ...
        for (int i = 0; i < FBfrag; ++i) FBatom += AtomsinFB[i];
    ...
    if (QNATOMS_real != (QNATOMS - FBatom)) QCrash("Number of computed real atoms is inconsistent.");
    ...
    for (int f = 0; f < FBfrag; ++f) {
        ...
        for (int i = 0; i < 6; ++i) efp_f[i] = -1 * QGrad[3 * QNATOMS_real + 6 * f + i];
    ...
    for (size_t i = 0; i < interfragments.size(); ++i) interfragments[i]->update_reference_points();
...
double **OPT_DATA::read_cartesian_H(void) const {
    ...
    try {
        std::string hess_fname =
            psi::get_writer_file_prefix(psi::Process::environment.legacy_molecule()->name()) + ".hess";
        ...
        for (int i = 0; i < Ncart; ++i)
            for (int j = 0; j < Ncart; ++j) if_Hcart >> H_cart[i][j];
    ...
    int cnt = 0;
    for (int i = 0; i < Ncart; ++i)
        for (int j = 0; j < Ncart; ++j) H_cart[i][j] = Hess[cnt++];
