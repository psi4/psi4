...
double **STRE::DqDx(GeomType geom) const {
    ...
    if (inverse_stre) val = value(geom);  // val = (1/R)

    for (int a = 0; a < 2; ++a)
        for (int a_xyz = 0; a_xyz < 3; ++a_xyz) {
            dqdx[a][a_xyz] = eAB[a_xyz];
            if (a == 0) dqdx[a][a_xyz] *= -1;
            if (inverse_stre) dqdx[a][a_xyz] *= -1.0 * val * val;  // -(1/R)^2 * (dR/da)
        }
...
// print displacement
void STRE::print_disp(std::string psi_fp, FILE *qc_fp, const double q_orig, const double f_q, const double dq,
...
bool STRE::operator==(const SIMPLE_COORDINATE &s2) const {
    if (stre_type != s2.g_type()) return false;
