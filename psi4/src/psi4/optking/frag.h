...
class FRAG {
   ...
   public:
    ...
    void connect(int i, int j) { connectivity[i][j] = connectivity[j][i] = true; }
    ...
    int g_simple_natom(const int coord_index) const { return coords.simples.at(coord_index)->g_natom(); }
    ...
    void simple_add(SIMPLE_COORDINATE *i) { coords.simples.push_back(i); }
     ...
     */
    int principal_axes(double **axes, double *evals) { return (principal_axes(geom, axes, evals)); }

    // These functions are needed for the forces() function
    // to apply user-defined equilibrium extra forces.
    bool coord_has_fixed_eq_val(int coord_index) const { return coords.simples[coord_index]->has_fixed_eq_val(); }
    double coord_fixed_eq_val(int coord_index) const { return coords.simples[coord_index]->fixed_eq_val(); }
     ...
     */
    bool apply_frozen_constraints(std::string R_string, std::string B_string, std::string D_string,
                                  std::string C_string);
