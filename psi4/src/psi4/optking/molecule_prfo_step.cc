...
void MOLECULE::prfo_step(void) {
    ...
    for (int i = 0; i < Nintco; ++i) H_diag[i][i] = lambda[i];
    ...
    if (p_Opt_data->g_iteration() == 1 || !Opt_params.rfo_follow_root) {
        ...
        oprintf_out(
            "\tMaximizing along Hessian eigenvalue %d whose \
    ...
    for (int i = 0; i < mu + 1; ++i) {
        ...
        if (std::fabs(tval) < Opt_params.rfo_normalization_max) {
            for (int j = 0; j < mu + 1; ++j) rfo_max[i][j] /= rfo_max[i][mu];
        }
    }
    if (Opt_params.print_lvl >= 3) {
    ...
    for (int i = 0; i < Nintco - mu + 1; ++i) {
        ...
        if (std::fabs(tval) < Opt_params.rfo_normalization_max) {
            for (int j = 0; j < Nintco - mu + 1; ++j) rfo_min[i][j] /= rfo_min[i][Nintco - mu];
        }
    }
    if (Opt_params.print_lvl >= 3) {
    ...
    double rfo_h = 0;
    for (int i = 0; i < Nintco; ++i) rfo_h += rfo_u[i] * array_dot(Horig[i], rfo_u, Nintco);
