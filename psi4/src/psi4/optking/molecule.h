...
class MOLECULE {
   ...
   public:
    ...
    ~MOLECULE() {
        for (std::size_t i = 0; i < fragments.size(); ++i) delete fragments[i];
        fragments.clear();
        for (std::size_t i = 0; i < interfragments.size(); ++i) delete interfragments[i];
        interfragments.clear();
        for (std::size_t i = 0; i < fb_fragments.size(); ++i) delete fb_fragments[i];
        fb_fragments.clear();
    }
    ...
    int g_natom(void) const {  // excludes atoms in fb fragments
        int n = 0;
        for (std::size_t f = 0; f < fragments.size(); ++f) n += fragments[f]->g_natom();
        return n;
    }

    int Ncoord(void) const {
        int n = 0;
        for (std::size_t f = 0; f < fragments.size(); ++f) n += fragments[f]->Ncoord();
        for (std::size_t i = 0; i < interfragments.size(); ++i) n += interfragments[i]->Ncoord();
        for (std::size_t e = 0; e < fb_fragments.size(); ++e) n += fb_fragments[e]->Ncoord();
        return n;
    }
    ...
    int Ncoord_interfragment(void) const {
        int n = 0;
        for (std::size_t f = 0; f < interfragments.size(); ++f) n += interfragments[f]->Ncoord();
        return n;
    }

    int Ncoord_fb_fragment(void) const {
        int n = 0;
        for (std::size_t f = 0; f < fb_fragments.size(); ++f) n += fb_fragments[f]->Ncoord();
        return n;
    }
    ...
    int g_atom_offset(int index) const {
        int n = 0;
        for (int f = 1; f <= index; ++f) n += fragments[f - 1]->g_natom();
        return n;
    }
    ...
    int g_coord_offset(int index) const {
        int n = 0;
        for (int f = 0; f < index; ++f) n += fragments[f]->Ncoord();
        return n;
    }
    ...
    int g_interfragment_coord_offset(int index) const {
        ...
        for (int f = 0; f < index; ++f) n += interfragments[f]->Ncoord();
        return n;
    }
    ...
    int g_fb_fragment_coord_offset(int index) const {
        ...
        for (int f = 0; f < index; ++f) n += fb_fragments[f]->Ncoord();
        return n;
    }
    ...
    void update_connectivity_by_distances(void) {
        for (std::size_t i = 0; i < fragments.size(); ++i) fragments[i]->update_connectivity_by_distances();
    }

    void update_connectivity_by_bonds(void) {
        for (std::size_t i = 0; i < fragments.size(); ++i) fragments[i]->update_connectivity_by_bonds();
    }
    ...
    void print_geom(std::string psi_fp, FILE *qc_fp, bool print_mass = false) {
        for (std::size_t i = 0; i < fragments.size(); ++i) fragments[i]->print_geom(psi_fp, qc_fp, i, print_mass);
    }
    ...
    void print_geom_grad(std::string psi_fp, FILE *qc_fp, bool print_mass = false) {
        for (std::size_t i = 0; i < fragments.size(); ++i) fragments[i]->print_geom_grad(psi_fp, qc_fp, i, print_mass);
    }
    ...
    int add_intrafragment_simples_by_connectivity(void) {
        int n = 0;
        for (std::size_t i = 0; i < fragments.size(); ++i) n += fragments[i]->add_simples_by_connectivity();
        return n;
    }

    int add_intrafragment_hbonds(void) {
        int n = 0;
        for (std::size_t i = 0; i < fragments.size(); ++i) n += fragments[i]->add_hbonds();
        return n;
    }

    int add_intrafragment_auxiliary_bonds(void) {
        int n = 0;
        for (std::size_t i = 0; i < fragments.size(); ++i) n += fragments[i]->add_auxiliary_bonds();
        return n;
    }
    ...
    double *coord_values(GeomType new_geom) const {
        ...
        for (std::size_t f = 0; f < fragments.size(); ++f) {
            ...
            for (int i = 0; i < fragments[f]->Ncoord(); ++i) q[g_coord_offset(f) + i] = q_frag[i];
        ...
        for (std::size_t I = 0; I < interfragments.size(); ++I) {
            ...
            for (int i = 0; i < interfragments[I]->Ncoord(); ++i) q[g_interfragment_coord_offset(I) + i] = q_IF[i];
    ...
    double *g_grad_array(void) const {
        ...
        for (std::size_t f = 0; f < fragments.size(); ++f) {
            ...
            for (int i = 0; i < 3 * fragments[f]->g_natom(); ++i) g[3 * g_atom_offset(f) + i] = g_frag[i];
            free_array(g_frag);
        }
        return g;
    }
    ...
    double **g_geom_2D(void) const {
        ...
        for (std::size_t f = 0; f < fragments.size(); ++f) {
            ...
            for (int i = 0; i < fragments[f]->g_natom(); ++i)
                for (int xyz = 0; xyz < 3; ++xyz) g[g_atom_offset(f) + i][xyz] = g_frag[i][xyz];
            free_matrix(g_frag);
        }
        return g;
    }
    ...
    double **g_grad_2D(void) const {
        ...
        for (std::size_t f = 0; f < fragments.size(); ++f) {
            ...
            int cnt = 0;
            for (int i = 0; i < fragments[f]->g_natom(); ++i)
                for (int xyz = 0; xyz < 3; ++xyz) g[g_atom_offset(f) + i][xyz] = g_frag[cnt++];
            free_array(g_frag);
        }
        return g;
    }
    ...
    void fix_tors_near_180(void) {
        for (std::size_t f = 0; f < fragments.size(); ++f) fragments[f]->fix_tors_near_180();
        for (std::size_t I = 0; I < interfragments.size(); ++I) interfragments[I]->fix_tors_near_180();
    }

    void fix_bend_axes(void) {
        for (std::size_t f = 0; f < fragments.size(); ++f) fragments[f]->fix_bend_axes();
        for (std::size_t I = 0; I < interfragments.size(); ++I) interfragments[I]->fix_bend_axes();
    }
    void unfix_bend_axes(void) {
        for (std::size_t f = 0; f < fragments.size(); ++f) fragments[f]->unfix_bend_axes();
        for (std::size_t I = 0; I < interfragments.size(); ++I) interfragments[I]->unfix_bend_axes();
    }

    void fix_oofp_near_180(void) {
        for (std::size_t f = 0; f < fragments.size(); ++f) fragments[f]->fix_oofp_near_180();
        for (std::size_t I = 0; I < interfragments.size(); ++I) interfragments[I]->fix_oofp_near_180();
    }
    ...
    */
    ...
    void set_masses(void) {
        for (std::size_t f = 0; f < fragments.size(); ++f) fragments[f]->set_masses();
    }
