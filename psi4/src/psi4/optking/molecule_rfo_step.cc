...
void MOLECULE::rfo_step(void) {
    ...
    for (i = 0; i < dim; ++i)
        for (j = 0; j < dim; ++j) RFO[i][j] = H[i][j];

    for (i = 0; i < dim; ++i) RFO[i][dim] = RFO[dim][i] = -fq[i];
    ...
    while (!converged && iter < max_projected_rfo_iter) {
        ...
        for (i = 0; i <= dim; i++) {
            for (j = 0; j < dim; j++) SRFO[j][i] = RFO[j][i] / alpha;
        ...
        for (i = 0; i < dim + 1; ++i) {
            ...
            if (std::fabs(tval) < Opt_params.rfo_normalization_max) {     // same check occurs below for acceptability
                for (j = 0; j < dim + 1; ++j) SRFO[i][j] /= SRFO[i][dim];
            }
        }
        if (Opt_params.print_lvl >= 4) {
        ...
        if ((!rfo_follow_root) || (p_Opt_data->g_iteration() == 1)) {
            ...
            if (iter == 0) oprintf_out("\tGoing to follow RFO solution %d.\n", rfo_root + 1);
        ...
        if (iter == 0) oprintf_out("\tUsing RFO vector %d.\n", rfo_root + 1);
        ...
        for (j = 0; j < dim; ++j) dq[j] = SRFO[rfo_root][j];  // leave out last column
        ...
        for (std::size_t f = 0; f < fragments.size(); ++f) {
            if (fragments[f]->is_frozen()) {
                if (iter == 0) oprintf_out("\tZero'ing out displacements for frozen fragment %d\n", f + 1);
                for (i = 0; i < fragments[f]->Ncoord(); ++i) dq[g_coord_offset(f) + i] = 0.0;
            }
        }

        for (std::size_t I = 0; I < interfragments.size(); ++I) {
            if (interfragments[I]->is_frozen()) {
                if (iter == 0) oprintf_out("\tZero'ing out displacements for frozen interfragment %d\n", I + 1);
                for (int i = 0; i < interfragments[I]->Ncoord(); ++i) dq[g_interfragment_coord_offset(I) + i] = 0;
            }
        }
        // Set dq of frozen interfragment coordinates to hard zero.
        for (std::size_t I = 0; I < interfragments.size(); ++I) {
            for (int i = 0; i < interfragments[I]->Ncoord(); ++i)
                if (interfragments[I]->is_frozen(i)) dq[g_interfragment_coord_offset(I) + i] = 0.0;
        }
        ...
        sum = 0;
        for (i = 0; i < dim; i++) sum += (pow(array_dot(Hevects[i], fq, dim), 2)) / (pow((h[i] - lambda * alpha), 3));
    ...
    if (!converged) apply_intrafragment_step_limit(dq);
    ...
    rfo_h = 0;
    for (i = 0; i < dim; ++i) rfo_h += rfo_u[i] * array_dot(Hevects[i], rfo_u, dim);
    ...
    if (!lin_angles.empty()) throw(INTCO_EXCEPT("New linear angles", lin_angles));
