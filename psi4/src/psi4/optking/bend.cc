...
void BEND::compute_axes(GeomType geom) const {
    ...
    tv1[0] = 1;
    tv1[1] = 0;
    tv1[2] = 0;  // more likely not to create 2 bends
    tv2[0] = 0;
    tv2[1] = 0;
    tv2[2] = 1;  // that both break a symmetry plane
    v3d_normalize(tv1);
    v3d_normalize(tv2);
...
double BEND::value(GeomType geom) const {
    ...
    if (!axes_fixed) compute_axes(geom);
    ...
    */
    ...
    if (!v3d_angle(u, origin, x, phi)) throw(INTCO_EXCEPT("BEND::value: could not compute linear bend", true));

    if (!v3d_angle(x, origin, v, tval)) throw(INTCO_EXCEPT("BEND::value: could not compute linear bend", true));
    phi += tval;
    free_array(origin);
    // }
    return phi;
...
double **BEND::DqDx(GeomType geom) const {
    ...
    for (int a = 0; a < 3; ++a)
        for (int i = 0; i < 3; ++i) dqdx[a][i] = zeta(a, 0, 1) * uXw[i] / Lu + zeta(a, 2, 1) * wXv[i] / Lv;
...
void BEND::print_disp(std::string psi_fp, FILE *qc_fp, const double q_old, const double f_q, const double dq,
    ...
    oprintf(psi_fp, qc_fp, "%-15s = %13.6lf%13.6lf%13.6lf%13.6lf\n", iss.str().c_str(), q_old / _pi * 180.0,
            f_q * _hartree2aJ * _pi / 180.0, dq / _pi * 180.0, q_new / _pi * 180.0);
}
...
bool BEND::operator==(const SIMPLE_COORDINATE &s2) const {
    ...
    if (this->s_atom[1] != s2.g_atom(1)) return false;

    if (this->_bend_type != s2.g_bend_type()) return false;
