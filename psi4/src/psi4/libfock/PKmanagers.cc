...
void PKManager::integrals_buffering(const double* buffer, size_t P, size_t Q, size_t R, size_t S) {
    int thread = 0;
#ifdef _OPENMP
    thread = omp_get_thread_num();
#endif
    ...
    for (bfiter.first(); bfiter.is_done() == false; bfiter.next()) {
        ...
        if (std::fabs(val) > cutoff_) {
            ...
            // DEBUG              outfile->Printf("Integral <%d %d|%d %d>,<%d|%d> =
...
void PKManager::integrals_buffering_wK(const double* buffer, size_t P, size_t Q, size_t R, size_t S) {
    int thread = 0;
#ifdef _OPENMP
    thread = omp_get_thread_num();
#endif
...
PKMgrDisk::PKMgrDisk(std::shared_ptr<PSIO> psio, std::shared_ptr<BasisSet> primary, size_t memory, Options& options)
    : PKManager(primary, memory, options) {
    psio_ = psio;
    AIO_ = std::make_shared<AIOHandler>(psio_);
...
void PKMgrDisk::print_batches() {
    ...
    for (int batch = 0; batch < batch_pq_min_.size(); ++batch) {
        outfile->Printf("\tBatch %3d pq = [%8zu,%8zu] index = [%14zu,%zu] size = %12zu\n", batch + 1,
                        batch_pq_min_[batch], batch_pq_max_[batch], batch_index_min_[batch], batch_index_max_[batch],
                        batch_index_max_[batch] - batch_index_min_[batch]);
    }
}

void PKMgrDisk::write() { get_buffer()->write(batch_index_min_, batch_index_max_, pk_pairs()); }

void PKMgrDisk::write_wK() { get_buffer()->write_wK(batch_index_min_, batch_index_max_, pk_pairs()); }

void PKMgrDisk::open_PK_file() { psio_->open(pk_file_, PSIO_OPEN_OLD); }

void PKMgrDisk::close_PK_file(bool keep) { psio_->close(pk_file_, keep); }

void PKMgrDisk::prepare_JK(std::vector<SharedMatrix> D, std::vector<SharedMatrix> Cl, std::vector<SharedMatrix> Cr) {
    if (writing()) {
        finalize_PK();
        set_writing(false);
    } else {
...
void PKMgrDisk::form_J(std::vector<SharedMatrix> J, std::string exch, std::vector<SharedMatrix> K) {
    make_J_vec(J);
...
PKMgrReorder::PKMgrReorder(std::shared_ptr<PSIO> psio, std::shared_ptr<BasisSet> primary, size_t memory,
                           Options& options)
...
PKMgrYoshimine::PKMgrYoshimine(std::shared_ptr<PSIO> psio, std::shared_ptr<BasisSet> primary, size_t memory,
                               Options& options)
    : PKMgrDisk(psio, primary, memory, options) {
...
void PKMgrYoshimine::close_iwl_buckets_wK() { psio()->close(iwl_file_wK_, 1); }
...
void PKMgrInCore::write() { get_buffer()->finalize_ints(pk_pairs()); }

void PKMgrInCore::write_wK() { get_buffer()->finalize_ints_wK(pk_pairs()); }
...
void PKMgrInCore::prepare_JK(std::vector<SharedMatrix> D, std::vector<SharedMatrix> Cl, std::vector<SharedMatrix> Cr) {
    form_D_vec(D, Cl, Cr);
}
                    ...
                    for (int q = 0; q <= p; ++q) {
                        ...
                        for (int r = 0; r <= p; ++r) {
                            ...
                            for (int s = 0; s <= maxs; ++s) {
                                ...
                                // DEBUG                       bool prt = (p == 0 && r == 0) || (q == 0 && r == 0) || (p
                                // == 0 && s == 0) || (s == 0 && q == 0);
                                // DEBUG                       if(exch == "wK" && prt) {
                                // DEBUG                         outfile->Printf("PK int (%d %d|%d %d) =
                                // %20.16f\n",p,q,r,s,*j_ptr);
                                // DEBUG                       }
                                K_vec[p][r] += (*j_ptr) * fac * Dmat[q][s];
                                K_vec[r][p] += (*j_ptr) * fac * Dmat[s][q];
