...
CISRHamiltonian::CISRHamiltonian(std::shared_ptr<JK> jk, SharedMatrix Caocc, SharedMatrix Cavir,
...
std::shared_ptr<Vector> CISRHamiltonian::diagonal() {
    ...
    for (int symm = 0; symm < nirrep; ++symm) {
        ...
        for (int h = 0; h < nirrep; ++h) {
            ...
            offset += static_cast<long> (nocc) * nvir;
        }
    }
    ...
    for (int symm = 0; symm < nirrep; ++symm) {
        for (size_t N = 0; N < x.size(); ++N) {
            ...
            for (int h = 0; h < Caocc_->nirrep(); ++h) {
                ...
                for (int i = 0; i < nocc; i++) {
                    for (int a = 0; a < nvir; a++) {
                        double local_x = xp[i * nvir + a + offset];
            ...
            if (delta_vector) {
                ...
                C_DCOPY(Cavir_->rowspi()[delta_h ^ symm], &Cvp[0][delta_a], Cavir_->colspi()[delta_h ^ symm], Crp[0],
                        1);
                ...
                auto Cr =
                    std::make_shared<Matrix>(ss.str(), Caocc_->nirrep(), Caocc_->rowspi(), Caocc_->colspi(), symm);
...
TDHFRHamiltonian::TDHFRHamiltonian(std::shared_ptr<JK> jk, SharedMatrix Caocc, SharedMatrix Cavir,
                                   std::shared_ptr<Vector> eps_aocc, std::shared_ptr<Vector> eps_avir,
                                   std::shared_ptr<VBase> v)
    : RHamiltonian(jk, v), singlet_(true), Caocc_(Caocc), Cavir_(Cavir), eps_aocc_(eps_aocc), eps_avir_(eps_avir) {}
...
TDARHamiltonian::TDARHamiltonian(std::shared_ptr<JK> jk, std::shared_ptr<VBase> v, SharedMatrix Cocc,
                                 SharedMatrix Caocc, SharedMatrix Cavir, std::shared_ptr<Vector> eps_aocc,
...
TDDFTRHamiltonian::TDDFTRHamiltonian(std::shared_ptr<JK> jk, std::shared_ptr<VBase> v, SharedMatrix Cocc,
                                     SharedMatrix Caocc, SharedMatrix Cavir, std::shared_ptr<Vector> eps_aocc,
                                     std::shared_ptr<Vector> eps_avir)
...
CPKSRHamiltonian::CPKSRHamiltonian(std::shared_ptr<JK> jk, std::shared_ptr<VBase> v, SharedMatrix Cocc,
                                   SharedMatrix Caocc, SharedMatrix Cavir, std::shared_ptr<Vector> eps_aocc,
                                   std::shared_ptr<Vector> eps_avir)
...
std::vector<std::pair<SharedMatrix, SharedMatrix> > USTABHamiltonian::unpack_paired(
    ...
    for (int symm = 0; symm < nirrep; ++symm) {
        ...
        for (int h = 0; h < nirrep; ++h) {
            ...
            offseta += static_cast<long> (nocca) * nvira;
        }
        for (int h = 0; h < nirrep; ++h) {
            ...
            offsetb += static_cast<long> (noccb) * nvirb;
        }
