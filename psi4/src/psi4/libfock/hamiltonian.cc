...
SharedMatrix RHamiltonian::explicit_hamiltonian()
{
    ...
    SharedMatrix H = std::make_shared<Matrix>("Explicit Hamiltonian", diag->nirrep(), diag->dimpi(), diag->dimpi());
...
{
    std::shared_ptr<Vector> diag = std::make_shared<Vector>("Matrix Diagonal", M_->rowspi());
...
{
    std::shared_ptr<Vector> diaga = std::make_shared<Vector>("Alpha Matrix Diagonal", M_.first->rowspi());
    std::shared_ptr<Vector> diagb = std::make_shared<Vector>("Beta Matrix Diagonal", M_.first->rowspi());
...
std::shared_ptr<Vector> CISRHamiltonian::diagonal()
{
    ...
    std::shared_ptr<Vector> diag = std::make_shared<Vector>("CIS Diagonal", nov);
    ...
    for (int symm = 0; symm < nirrep; ++symm) {
        for (size_t N = 0; N < x.size(); ++N) {
            ...
            if (delta_vector) {
                ...
                SharedMatrix Cl = std::make_shared<Matrix>(ss.str(), Caocc_->nirrep(), Caocc_->rowspi(), rank);
                ...
                SharedMatrix Cr = std::make_shared<Matrix>(ss2.str(), Caocc_->nirrep(), Caocc_->rowspi(), rank, symm);
                ...
                SharedMatrix Cr = std::make_shared<Matrix>(ss.str(), Caocc_->nirrep(), Caocc_->rowspi(), Caocc_->colspi(), symm);
...
std::vector<SharedMatrix > CISRHamiltonian::unpack(const std::shared_ptr<Vector>& eig)
{
    ...
    for (int symm = 0; symm < nirrep; ++symm) {
        SharedMatrix t = std::make_shared<Matrix>("T",Caocc_->nirrep(), Caocc_->colspi(), Cavir_->colspi(), symm);
        long int offset = 0L;
        for (int h = 0; h < nirrep; ++h) {
            int nocc = Caocc_->colspi()[h];
            int nvir = Cavir_->colspi()[h^symm];
...
std::shared_ptr<Vector> TDHFRHamiltonian::diagonal()
{
    ...
    std::shared_ptr<Vector> diag = std::make_shared<Vector>("TDHF Diagonal", nov);
                                     ...
                                     std::vector<std::shared_ptr<Vector> >& b)
{
    ...
    for (int symm = 0; symm < nirrep; ++symm) {
        for (size_t N = 0; N < x.size(); ++N) {
            ...
            SharedMatrix CXr = std::make_shared<Matrix>(ss.str(), Caocc_->nirrep(), Caocc_->rowspi(), Caocc_->colspi(), symm);
            ...
            SharedMatrix CYr = std::make_shared<Matrix>(ss.str(), Caocc_->nirrep(), Caocc_->rowspi(), Caocc_->colspi(), symm);
...
std::shared_ptr<Vector> CPHFRHamiltonian::diagonal()
{
    ...
    std::shared_ptr<Vector> diag = std::make_shared<Vector>("CPHF Diagonal", nov);
...
std::map<std::string, SharedVector> CPHFRHamiltonian::pack(const std::map<std::string, SharedMatrix>& x)
{
    ...
    for (std::map<std::string, SharedMatrix>::const_iterator it = x.begin();
        ...
        std::shared_ptr<Vector> v = std::make_shared<Vector>("X", nov);
...
std::vector<SharedMatrix > CPHFRHamiltonian::unpack(const std::vector<std::shared_ptr<Vector> >& x)
{
    ...
    for (size_t i = 0; i < x.size(); i++) {
        for (int symm = 0; symm < nirrep; ++symm) {
            SharedMatrix t = std::make_shared<Matrix>("X",Caocc_->nirrep(), Caocc_->colspi(), Cavir_->colspi(), symm);
            long int offset = 0L;
            for (int h = 0; h < nirrep; ++h) {
                int nocc = Caocc_->colspi()[h];
                int nvir = Cavir_->colspi()[h^symm];
                                     ...
                                     std::vector<std::shared_ptr<Vector> >& b)
{
    ...
    for (int symm = 0; symm < nirrep; ++symm) {
        for (size_t N = 0; N < x.size(); ++N) {
            ...
            std::stringstream ss2;
            ss2 << "P, h = " << symm << ", N = " << N;
            SharedMatrix P2 = std::make_shared<Matrix>(ss2.str(), Caocc_->colspi(), Cavir_->colspi(), symm);
...
std::pair<std::shared_ptr<Vector>, std::shared_ptr<Vector> > USTABHamiltonian::diagonal()
{
    ...
    std::shared_ptr<Vector> diaga = std::make_shared<Vector>("UStab Alpha Diagonal", nova);
    std::shared_ptr<Vector> diagb = std::make_shared<Vector>("UStab Beta Diagonal", novb);
    ...
    for (int symm = 0; symm < nirrepa; ++symm) {
        for (size_t N = 0; N < x.size(); ++N) {
            ...
            if (delta_vector) {
                ...
                SharedMatrix Cl = std::make_shared<Matrix>(ss.str(), Cocca_->nirrep(), Cocca_->rowspi(), rank);
                ...
                SharedMatrix Cr = std::make_shared<Matrix>(ss2.str(), Cocca_->nirrep(), Cocca_->rowspi(), rank, symm);
                ...
                SharedMatrix Cr = std::make_shared<Matrix>(ss.str(), Cocca_->nirrep(), Cocca_->rowspi(), Cocca_->colspi(), symm);
    ...
    for (int symm = 0; symm < nirrepb; ++symm) {
        for (size_t N = 0; N < x.size(); ++N) {
            ...
            if (delta_vector) {
                ...
                SharedMatrix Cl = std::make_shared<Matrix>(ss.str(), Coccb_->nirrep(), Coccb_->rowspi(), rank);
                ...
                SharedMatrix Cr = std::make_shared<Matrix>(ss2.str(), Coccb_->nirrep(), Coccb_->rowspi(), rank, symm);
                ...
                SharedMatrix Cr = std::make_shared<Matrix>(ss.str(), Coccb_->nirrep(), Coccb_->rowspi(), Coccb_->colspi(), symm);
        ...
        const std::shared_ptr<Vector>& eig)
{
    ...
    for (int symm = 0; symm < nirrep; ++symm) {
        SharedMatrix talpha = std::make_shared<Matrix>("T",Cocca_->nirrep(), Cocca_->colspi(), Cvira_->colspi(), symm);
        SharedMatrix tbeta = std::make_shared<Matrix>("T",Coccb_->nirrep(), Coccb_->colspi(), Cvirb_->colspi(), symm);
        long int offseta = 0L;
        long int offsetb = 0L;
        for (int h = 0; h < nirrep; ++h) {
