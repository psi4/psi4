...
double GetBSRadius(unsigned Z) {
    ...
    static const double BSRadii[55] = {
        1.000, 1.001, 1.012, 0.825, 1.408, 1.485, 1.452, 1.397, 1.342, 1.287, 1.243, 1.144, 1.364, 1.639,
        1.716, 1.705, 1.683, 1.639, 1.595, 1.485, 1.474, 1.562, 1.562, 1.562, 1.562, 1.562, 1.562, 1.562,
        1.562, 1.562, 1.562, 1.650, 1.727, 1.760, 1.771, 1.749, 1.727, 1.628, 1.606, 1.639, 1.639, 1.639,
        1.639, 1.639, 1.639, 1.639, 1.639, 1.639, 1.639, 1.672, 1.804, 1.881, 1.892, 1.892, 1.881,
    };
...
// subdiagonal since it's a symmetric tridiagonal matrix). These values are destroyed as we go along; after the routine
// runs, `a' will contain the eigenvalues of the matrix and `b' will be an array of zeroes. `q' is the top row of the
// eigenvector matrix. It corresponds to `w' in the Golub-Welsch paper's microfiche implementation. Note: This routine
// expects b[-1] to be a valid memory location, but doesn't require that it be initialized.
...
RadialGridMgr::SchemeTable RadialGridMgr::radialschemes[] = {
    ...
    {"MURA", getTrapezoidalRoots, mura5_r,
     mura5_dr},  // Also called "Knowles" or "Mura-Knowles". Molpro calls this "LOG"
...
// stored in several different places that must all get updated at once if someone adds a new radial grid type. With the
// current system, you only have to add your new scheme to the RadialGridMgr::radialschemes table and you're done.
...
void StandardGridMgr::Initialize_SG0() {
    ...
    PruneSpec SG0specs[18] = {
        {nullptr, 0, 0, 0},       {H__grp, 23, 1406, 1.30}, {nullptr, 0, 0, 0},       {Li_grp, 23, 1406, 1.95},
        {Be_grp, 23, 1390, 2.20}, {B__grp, 23, 1426, 1.45}, {C__grp, 23, 1390, 1.20}, {N__grp, 23, 1414, 1.10},
        {O__grp, 23, 1154, 1.10}, {F__grp, 23, 1494, 1.20}, {nullptr, 0, 0, 0},       {Na_grp, 26, 1328, 2.30},
        {Mg_grp, 26, 1468, 2.20},  // Warning: The original paper has 1492 points instead of 1468...
        {Al_grp, 26, 1496, 2.10}, {Si_grp, 26, 1496, 1.30}, {P__grp, 26, 1496, 1.30}, {S__grp, 26, 1456, 1.10},
        {Cl_grp, 26, 1480, 1.45}};
...
void StandardGridMgr::Initialize_SG1() {
    ...
    static const double SG1radii[] = {0,      1.0000, 0.5882, 3.0769, 2.0513, 1.5385, 1.2308, 1.0256, 0.8791, 0.7692,
                                      0.6838, 4.0909, 3.1579, 2.5714, 2.1687, 1.8750, 1.6514, 1.4754, 1.3333};
    ...
    static const short rows[] = {
        0, 1, 1, 2, 2, 2, 2, 2, 2, 2, 2, 3, 3, 3, 3, 3, 3, 3, 3,
    };
...
double NuclearWeightMgr::GetStratmannCutoff(int A) const {
    ...
    double mucutoff = (aij == 0) ? -0.64  // Then nu = mu, so to get nu < -0.64 we need mu < -0.64
                                 : (aij >= 1 / 6.56)
                                       ? -1  // If aij > 1/6.56 = 1/(4*(1+0.64)), then there's no solution.
...
void DFTGrid::buildGridFromOptions(std::map<std::string, int> int_opts_map,
    ...
    std::vector<std::string> int_keys = {"DFT_BLOCK_MAX_POINTS", "DFT_BLOCK_MIN_POINTS", "DFT_SPHERICAL_POINTS",
                                         "DFT_RADIAL_POINTS"};
...
void OctreeGridBlocker::block() {
    ...
    for (size_t A = 0; A < completed_tree.size(); A++) {
        ...
        blocks_.push_back(
            std::make_shared<BlockOPoints>(block.size(), &x_[index], &y_[index], &z_[index], &w_[index], extents_));
