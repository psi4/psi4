...
RCIS::RCIS(SharedWavefunction ref_wfn, Options& options) : RBase(ref_wfn, options) {}
RCIS::~RCIS() {}
...
void RCIS::print_amplitudes() {
    ...
    for (size_t i = 0; i < states_.size(); i++) {
        ...
        if (amps.size()) {
            ...
            outfile->Printf("  %-5zu %1s%-5d(%3s) %5d%-3s -> %5d%-3s %11.3E\n", i + 1, (m == 1 ? "S" : "T"), j + 1,
                            labels[h].c_str(), std::get<1>(amps[0]) + 1, labels[std::get<2>(amps[0])].c_str(),
                            std::get<3>(amps[0]) + 1, labels[std::get<4>(amps[0])].c_str(), std::get<0>(amps[0]));
            for (size_t index = 1; index < amps.size(); index++) {
                outfile->Printf("                    %5d%-3s -> %5d%-3s %11.3E\n", std::get<1>(amps[index]) + 1,
                                labels[std::get<2>(amps[index])].c_str(), std::get<3>(amps[index]) + 1,
...
void RCIS::print_densities() {
    ...
    for (size_t i = 0; i < options_["CIS_NO_STATES"].size(); i++) {
        ...
        std::pair<std::shared_ptr<Matrix>, std::shared_ptr<Vector> > stuff =
            Nao((singlet ? singlets_[state - 1] : triplets_[state - 1]), false);
        std::stringstream s;
    ...
    for (size_t i = 0; i < options_["CIS_AD_STATES"].size(); i++) {
        ...
        std::pair<std::shared_ptr<Matrix>, std::shared_ptr<Matrix> > stuff =
            ADao((singlet ? singlets_[state - 1] : triplets_[state - 1]));
        std::stringstream s;
...
RTDHF::RTDHF(SharedWavefunction ref_wfn, Options& options) : RBase(ref_wfn, options) {}
RTDHF::~RTDHF() {}
...
RCPKS::RCPKS(SharedWavefunction ref_wfn, Options& options) : RCPHF(ref_wfn, options) {}
RCPKS::~RCPKS() {}
...
RTDDFT::RTDDFT(SharedWavefunction ref_wfn, Options& options) : RTDHF(ref_wfn, options) {}
RTDDFT::~RTDDFT() {}
