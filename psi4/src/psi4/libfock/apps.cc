...
void RCPHF::add_polarizability()
{
    ...
    for (size_t i = 0; i < dipole.size(); i++) {
        ...
        SharedMatrix B = std::make_shared<Matrix>(ss.str(), Caocc_->colspi(), Cavir_->colspi(), dipole[i]->symmetry());
...
void RCPHF::analyze_polarizability()
{
    ...
    // Analysis
    SharedMatrix polarizability = std::make_shared<Matrix>("CPHF Polarizability", 3, 3);
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
...
double RCPHF::compute_energy()
{
    ...
    std::shared_ptr<CPHFRHamiltonian> H = std::make_shared<CPHFRHamiltonian>(jk_, Caocc_,Cavir_,eps_aocc_,eps_avir_);
...
void RCIS::print_transitions()
{
    ...
    std::shared_ptr<IntegralFactory> fact = std::make_shared<IntegralFactory>(basisset_,basisset_,basisset_,basisset_);
...
{
    SharedMatrix D = std::make_shared<Matrix>("TDso", T1->nirrep(), C_->rowspi(), C_->rowspi(), T1->symmetry());
...
SharedMatrix RCIS::TDao(SharedMatrix T1, bool singlet)
{
    ...
    SharedMatrix D2 = std::make_shared<Matrix>("TDao", AO2SO_->rowspi()[0], AO2SO_->rowspi()[0]);
...
{
    SharedMatrix D = std::make_shared<Matrix>("Dmo", reference_wavefunction_->nmopi(), reference_wavefunction_->nmopi());
...
SharedMatrix RCIS::Dso(SharedMatrix T1, bool diff)
{
    ...
    SharedMatrix D2 = std::make_shared<Matrix>("Dso", C_->nirrep(), C_->rowspi(), C_->rowspi());
...
SharedMatrix RCIS::Dao(SharedMatrix T1, bool diff)
{
    ...
    SharedMatrix D2 = std::make_shared<Matrix>("Dao", AO2SO_->rowspi()[0], AO2SO_->rowspi()[0]);
...
{
    SharedMatrix D = Dmo(T1, diff);
    SharedMatrix C = std::make_shared<Matrix>("Nmo", D->nirrep(), D->rowspi(), D->rowspi());
    std::shared_ptr<Vector> O = std::make_shared<Vector>("Occupation", D->rowspi());
...
std::pair<SharedMatrix, std::shared_ptr<Vector> > RCIS::Nso(SharedMatrix T1, bool diff)
{
    ...
    SharedMatrix N2 = std::make_shared<Matrix>("Nso", C_->nirrep(), C_->rowspi(), C_->colspi());
...
std::pair<SharedMatrix, std::shared_ptr<Vector> > RCIS::Nao(SharedMatrix T1, bool diff)
{
    ...
    SharedMatrix N2 = std::make_shared<Matrix>("Nso", C_->nrow(), C_->ncol());
    SharedMatrix N3 = std::make_shared<Matrix>("Nso", C_->nrow(), C_->ncol());
    std::shared_ptr<Vector> O2 = std::make_shared<Vector>("Occupation", C_->ncol());
...
std::pair<SharedMatrix, SharedMatrix> RCIS::ADmo(SharedMatrix T1)
{
    ...
    SharedMatrix A = std::make_shared<Matrix>("A", N->rowspi(), N->rowspi());
    SharedMatrix D = std::make_shared<Matrix>("D", N->rowspi(), N->rowspi());
    for (int h = 0; h < N->nirrep(); h++) {
        int nrow = N->rowspi()[h];
        int ncol = N->colspi()[h];
...
double RCIS::compute_energy()
{
    ...
    // Construct components
    std::shared_ptr<CISRHamiltonian> H = std::make_shared<CISRHamiltonian>(jk_, Caocc_,Cavir_,eps_aocc_,eps_avir_);
...
double RTDHF::compute_energy()
{
    ...
    // Construct components
    std::shared_ptr<TDHFRHamiltonian> H = std::make_shared<TDHFRHamiltonian>(jk_, Caocc_,Cavir_,eps_aocc_,eps_avir_);
...
double RCPKS::compute_energy()
{
    ...
    // Construct components
    std::shared_ptr<CPKSRHamiltonian> H = std::make_shared<CPKSRHamiltonian>(jk_,v_,Cocc_,Caocc_,Cavir_,eps_aocc_,eps_avir_);
    std::shared_ptr<CGRSolver> solver = CGRSolver::build_solver(options_,H);
...
double RTDA::compute_energy()
{
    ...
    // Construct components
    std::shared_ptr<TDARHamiltonian> H = std::make_shared<TDARHamiltonian>(jk_,v_,Cocc_,Caocc_,Cavir_,eps_aocc_,eps_avir_);
...
double RTDDFT::compute_energy()
{
    ...
    // Construct components
    std::shared_ptr<TDDFTRHamiltonian> H = std::make_shared<TDDFTRHamiltonian>(jk_,v_,Cocc_,Caocc_,Cavir_,eps_aocc_,eps_avir_);
    std::shared_ptr<DLRXSolver> solver = DLRXSolver::build_solver(options_,H);
