...
SharedMatrix SOMCSCF::H_approx_diag() {
    ...
    SharedVector dI = std::make_shared<Vector>("dI", nactpi_);
    SharedVector DIF = std::make_shared<Vector>("IF * OPDM", nactpi_);
    for (int h = 0; h < nirrep_; h++) {
    ...
    SharedMatrix H = std::make_shared<Matrix>("Approximate diag hessian", nirrep_, noapi_, navpi_);
    int offset_act = 0;
    for (int h = 0; h < nirrep_; h++) {
        if (!noapi_[h] || !navpi_[h]) continue;
...
SharedMatrix SOMCSCF::Hk(SharedMatrix x) {
    ...
    SharedMatrix U = std::make_shared<Matrix>("U", nirrep_, nmopi_, nmopi_);
    SharedMatrix Uocc = std::make_shared<Matrix>("Uocc", nirrep_, noccpi_, nmopi_);
    SharedMatrix Uact = std::make_shared<Matrix>("Uact", nirrep_, nactpi_, nmopi_);
    for (int h = 0; h < nirrep_; h++) {
        if (!noapi_[h] || !navpi_[h]) continue;
        double** Up = U->pointer(h);
        double** xp = x->pointer(h);
    ...
    // => Hessian <= //
    SharedMatrix hessx = std::make_shared<Matrix>("Hessian x", nirrep_, noapi_, navpi_);
...
void DFSOMCSCF::transform(bool approx_only) {
    ...
    SharedMatrix AO_C = std::make_shared<Matrix>("AO_C", nao_, aoc_rowdim);
    ...
    // not ideal FIXME
    SharedMatrix AO_R = std::make_shared<Matrix>("AO_R", nao_, nrot);
    SharedMatrix AO_a = std::make_shared<Matrix>("AO_a", nao_, aoc_rowdim - nrot);
...
void DFSOMCSCF::set_act_MO() {
    ...
    SharedMatrix aaQ = std::make_shared<Matrix>("aaQ", nact_ * nact_, nQ);
...
SharedMatrix DFSOMCSCF::compute_Qk(SharedMatrix TPDM, SharedMatrix U, SharedMatrix Uact) {
    ...
    // Read NaQ
    SharedMatrix NaQ = std::make_shared<Matrix>("RaQ", nmo_ * nact_, nQ);
    double* NaQp = NaQ->pointer()[0];
    dfh_->fill_tensor("RaQ", NaQ);

    SharedMatrix xyQ = std::make_shared<Matrix>("xyQ", nact_ * nact_, nQ);
    ...
    SharedMatrix dQk = std::make_shared<Matrix>("dQk", nact_, nmo_);
    ...
    SharedMatrix wnQ = std::make_shared<Matrix>("nwQ", nact_ * nmo_, nQ);
    double** wnQp = wnQ->pointer();
    SharedMatrix NNQ = std::make_shared<Matrix>("RRQ", chunk_size * nmo_, nQ);
    ...
    SharedMatrix Gleft = std::make_shared<Matrix>("Gnwxy", nmo_, nact3);
...
SharedMatrix DiskSOMCSCF::compute_Q(SharedMatrix TPDMmat) {
    ...
    SharedMatrix Qmat = std::make_shared<Matrix>(&Q);
...
SharedMatrix DiskSOMCSCF::compute_Qk(SharedMatrix TPDMmat, SharedMatrix U, SharedMatrix Uact) {
    ...
    SharedMatrix Qkmat = std::make_shared<Matrix>(&Qk);
