...
double SOMCSCF::rhf_energy(SharedMatrix C) {
    ...
    SharedMatrix D = doublet(C, C, false, true);
...
void SOMCSCF::update(SharedMatrix Cocc, SharedMatrix Cact, SharedMatrix Cvir, SharedMatrix OPDM, SharedMatrix TPDM) {
    ...
    // For active Fock
    SharedMatrix CL_COPDM = doublet(matrices_["Cact"], matrices_["OPDM"]);
    ...
    K[0]->scale(0.5);
    J[0]->subtract(K[0]);
    matrices_["AFock"] = triplet(matrices_["C"], J[0], matrices_["C"], true, false, false);
    ...
    if (compute_IFock_) {
        ...
        matrices_["IFock"] = triplet(matrices_["C"], J[1], matrices_["C"], true, false, false);
        matrices_["IFock"]->set_name("IFock");
    } else {
        matrices_["IFock"] = triplet(matrices_["C"], matrices_["AO_IFock"], matrices_["C"], true, false, false);
        matrices_["IFock"]->set_name("IFock");
    }
    ...
    SharedMatrix tmpD = doublet(matrices_["Cocc"], matrices_["Cocc"], false, true);
    if (has_fzc_) {
        SharedMatrix tmpDf = doublet(matrices_["Cfzc"], matrices_["Cfzc"], false, true);
        tmpD->add(tmpDf);
    }
...
SharedMatrix SOMCSCF::compute_AFock(SharedMatrix OPDM) {
    ...
    // For active Fock
    SharedMatrix CL_COPDM = doublet(matrices_["Cact"], OPDM);
    Cl.push_back(CL_COPDM);
    Cr.push_back(matrices_["Cact"]);
    ...
    K[0]->scale(0.5);
    J[0]->subtract(K[0]);
    SharedMatrix AFock = triplet(matrices_["C"], J[0], matrices_["C"], true, false, false);
...
SharedMatrix SOMCSCF::Hk(SharedMatrix x) {
    ...
    SharedMatrix CLUocc = doublet(matrices_["C"], Uocc, false, true);
    Cl.push_back(CLUocc);
    Cr.push_back(matrices_["Cocc"]);

    // For active Fock
    SharedMatrix CLUact = triplet(matrices_["C"], Uact, matrices_["OPDM"], false, true, true);
    Cr.push_back(CLUact);
    ...
    SharedMatrix IFk = doublet(matrices_["IFock"], U, false, true);
    IFk->gemm(false, false, 1.0, U, matrices_["IFock"], 1.0);

    J[0]->scale(4.0);
    J[0]->subtract(K[0]);
    J[0]->subtract(K[0]->transpose());
    SharedMatrix trans_half = doublet(J[0], matrices_["C"]);
    IFk->gemm(true, false, 1.0, matrices_["C"], trans_half, 1.0);

    // Rotated active fock
    SharedMatrix ret = doublet(matrices_["AFock"], U, false, true);
    ...
    trans_half = doublet(J[1], matrices_["C"]);
...
void DFSOMCSCF::set_act_MO() {
    ...
    matrices_["actMO"] = doublet(aaQ, aaQ, false, true);
    aaQ.reset();
}
...
SharedMatrix DFSOMCSCF::compute_Qk(SharedMatrix TPDM, SharedMatrix U, SharedMatrix Uact) {
    ...
    SharedMatrix Gnwxy = doublet(NaQ, xyQ, false, true);
    ...
    SharedMatrix Qk = doublet(tQ, U, false, true);
    // dQk->print();
