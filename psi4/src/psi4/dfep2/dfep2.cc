...
std::vector<std::vector<std::pair<double, double>>> DFEP2Wavefunction::compute(
    ...
    SharedMatrix AO_CE = std::make_shared<Matrix>("Solve orbitals", AO_C_->rowspi()[0], nE);
    ...
    std::shared_ptr<AIOHandler> aio = std::make_shared<AIOHandler>(psio);
    ...
    SharedMatrix aEQ = std::make_shared<Matrix>("aEQ", nE * nvir, nQ);
    double* aEQp = aEQ->pointer()[0];
    dfh_->fill_tensor("aEQ", aEQ);

    SharedMatrix iEQ = std::make_shared<Matrix>("iEQ", nE * nocc, nQ);
    double* iEQp = iEQ->pointer()[0];
    dfh_->fill_tensor("iEQ", iEQ);

    // Allocate temps
    SharedMatrix block_iaQ = std::make_shared<Matrix>(block_size * nvir, nQ);
    SharedMatrix temp_ovvE = std::make_shared<Matrix>(block_size * nvir, nvir * nE);
    SharedMatrix temp_ovoE = std::make_shared<Matrix>(block_size * nvir, nocc * nE);
    ...
    for (size_t iter = 0; iter < max_iter_; iter++) {
        ...
        SharedMatrix I_ovvE = std::make_shared<Matrix>(aaE_size * nvir, nvir * nE);
...
#pragma omp parallel for private(rank) schedule(dynamic, 1) collapse(2) num_threads(num_threads_)
        ...
        SharedMatrix I_vooE = std::make_shared<Matrix>(ooE_size * nocc, nocc * nE);
