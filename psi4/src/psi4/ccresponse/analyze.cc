...
void analyze(const char *pert, int irrep, double omega)
{
  ...
  std::shared_ptr<PsiOutStream> printer = std::make_shared<PsiOutStream>(lbl,std::ostream::app);
  //ffile(&efile, lbl, 1);
  ...
  std::shared_ptr<PsiOutStream> printer2 = std::make_shared<PsiOutStream>(lbl,std::ostream::app);
  //ffile(&efile, lbl, 1);
  amp_array = init_array(num_div);
