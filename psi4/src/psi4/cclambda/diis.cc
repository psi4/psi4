...
void CCLambdaWavefunction::diis(int iter, int L_irr) {
    ...
    if (params.ref == 0) { /** RHF **/
        ...
        for (h = 0; h < nirreps; h++)
            for (row = 0; row < L1a.params->rowtot[h]; row++)
                for (col = 0; col < L1a.params->coltot[h ^ L_irr]; col++) error[0][word++] = L1a.matrix[h][row][col];
        global_dpd_->file2_mat_close(&L1a);
        global_dpd_->file2_close(&L1a);
        ...
        for (h = 0; h < nirreps; h++) {
            ...
            global_dpd_->buf4_mat_irrep_rd(&L2a, h);
            for (row = 0; row < L2a.params->rowtot[h]; row++)
                for (col = 0; col < L2a.params->coltot[h ^ L_irr]; col++) error[0][word++] = L2a.matrix[h][row][col];
            global_dpd_->buf4_mat_irrep_close(&L2a, h);
        }
        global_dpd_->buf4_close(&L2a);
        ...
        */
        ...
        for (h = 0; h < nirreps; h++)
            for (row = 0; row < L1a.params->rowtot[h]; row++)
                for (col = 0; col < L1a.params->coltot[h ^ L_irr]; col++) L1a.matrix[h][row][col] = error[0][word++];
        ...
        for (h = 0; h < nirreps; h++) {
            global_dpd_->buf4_mat_irrep_init(&L2a, h);
            for (row = 0; row < L2a.params->rowtot[h]; row++)
                for (col = 0; col < L2a.params->coltot[h ^ L_irr]; col++) L2a.matrix[h][row][col] = error[0][word++];
        ...
        for (q = 0; q < vector_length; q++) {
            error[0][q] = 0.0;
            for (p = 0; p < nvector; p++) error[0][q] += C[p] * vector[p][q];
        }
