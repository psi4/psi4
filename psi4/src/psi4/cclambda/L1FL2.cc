        ...
        for (h = 0; h < nirreps; h++) {
            ...
            for (row = 0; row < newL2.params->rowtot[h]; row++) {
                ...
                for (col = 0; col < newL2.params->coltot[h ^ L_irr]; col++) {
                    ...
                    I = L.params->rowidx[i];
                    Isym = L.params->psym[i];
                    J = F.params->rowidx[j];
                    Jsym = F.params->psym[j];
                    A = L.params->colidx[a];
                    Asym = L.params->qsym[a];
                    B = F.params->colidx[b];
        ...
        for (h = 0; h < nirreps; h++) {
            ...
            for (row = 0; row < newL2.params->rowtot[h]; row++) {
                ...
                for (col = 0; col < newL2.params->coltot[h ^ L_irr]; col++) {
                    ...
                    I = LIA.params->rowidx[i];
                    Isym = LIA.params->psym[i];
                    J = FJB.params->rowidx[j];
                    Jsym = FJB.params->psym[j];
                    A = LIA.params->colidx[a];
                    Asym = LIA.params->qsym[a];
                    B = FJB.params->colidx[b];
                    Bsym = FJB.params->qsym[b];

                    if (((Isym ^ Asym) == L_irr) && (Jsym == Bsym))
                        newL2.matrix[h][row][col] += (LIA.matrix[Isym][I][A] * FJB.matrix[Jsym][J][B]);

                    J = LIA.params->rowidx[j];
                    Jsym = LIA.params->psym[j];
                    I = FJB.params->rowidx[i];
                    Isym = FJB.params->psym[i];

                    if ((Isym == Asym) && ((Jsym ^ Bsym) == L_irr))
                        newL2.matrix[h][row][col] += (LIA.matrix[Jsym][J][B] * FJB.matrix[Isym][I][A]);

                    I = LIA.params->rowidx[i];
                    Isym = LIA.params->psym[i];
                    J = FJB.params->rowidx[j];
                    Jsym = FJB.params->psym[j];
                    B = LIA.params->colidx[b];
                    Bsym = LIA.params->qsym[b];
                    A = FJB.params->colidx[a];
                    Asym = FJB.params->qsym[a];

                    if (((Jsym ^ Asym) == L_irr) && (Isym == Bsym))
                        newL2.matrix[h][row][col] -= (LIA.matrix[Jsym][J][A] * FJB.matrix[Isym][I][B]);
                    ...
                    if ((Jsym == Asym) && ((Isym ^ Bsym) == L_irr))
                        newL2.matrix[h][row][col] -= (LIA.matrix[Isym][I][B] * FJB.matrix[Jsym][J][A]);
                }
            }
