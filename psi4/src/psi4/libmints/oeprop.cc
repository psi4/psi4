...
void Prop::add(const std::string& prop) { tasks_.insert(prop); }
void Prop::add(std::vector<std::string> props) {
    for (int i = 0; i < (int)props.size(); i++) {
        tasks_.insert(props[i]);
    }
}
void Prop::clear() { tasks_.clear(); }
...
OEProp::OEProp(std::shared_ptr<Wavefunction> wfn) : Prop(wfn) { common_init(); }
OEProp::~OEProp() {}
...
void OEProp::compute_no_occupations() {
    ...
    if (!same_dens_) {
        ...
        for (int index = start_occ_a; index < stop_vir_a; index++) {
            if (index < offset_a) {
                outfile->Printf("  HONO-%-2d: %4d%3s %8.3f\n", offset_a - index - 1, std::get<1>(metric_a[index]) + 1,
        ...
        for (int index = start_occ_b; index < stop_vir_b; index++) {
            if (index < offset_b) {
                outfile->Printf("  HONO-%-2d: %4d%3s %8.3f\n", offset_b - index - 1, std::get<1>(metric_b[index]) + 1,
