...
OrbitalSpace OrbitalSpace::transform(const OrbitalSpace &A, const std::shared_ptr<BasisSet> &B) {
    ...
    // 2. Form T
    SharedMatrix I = create("I = SAB SBB SBA", SBA->colspi(), SBA->colspi());
    I->transform(SBB, SBA);

    SharedMatrix T = create("T", A.dim(), A.dim());
    T->transform(I, A.C());
    ...
    // 4a. Ca T^{-1/2}
    SharedMatrix CaT = create("Ca*T^{-1/2}", A.C()->rowspi(), A.C()->colspi());
    CaT->gemm(false, false, 1.0, A.C(), T, 0.0);

    // 4b. Sba * 4a
    SharedMatrix SbaCaT = create("SbaCaT", SBB->rowspi(), A.C()->colspi());
    SbaCaT->gemm(false, false, 1.0, SBA, CaT, 0.0);

    // 4c. [Sbb]^-1 * 4b
    SharedMatrix Cb = create("Cb", SBB->rowspi(), A.C()->colspi());
...
OrbitalSpace orthogonalize(const std::string &id, const std::string &name, const std::shared_ptr<BasisSet> &bs,
    ...
    SharedMatrix evecs = create("evecs", SODIM, SODIM);
    SharedMatrix sqrtm = create("evecs", SODIM, SODIM);
    SharedVector evals = create("evals", SODIM);
...
OrbitalSpace orthogonal_compliment(const OrbitalSpace &space1, const OrbitalSpace &space2, const std::string &id,
    ...
    SharedMatrix C12 = create("C12", space1.C()->colspi(), space2.C()->colspi());
    ...
    SharedMatrix D11 = create("D11", C12->colspi(), C12->colspi());
    D11->gemm(true, false, 1.0, C12, C12, 0.0);
    //        D11->print();

    SharedMatrix V11 = create("V11", D11->rowspi(), D11->colspi());
    SharedVector E1 = create("E1", D11->colspi());
    ...
    SharedMatrix newC = create("Transformation matrix", space2.C()->rowspi(), zeros);
