...
void DPD::cc3_sigma_UHF_AAA(dpdbuf4 *CMNEF, dpdbuf4 *WABEI, dpdbuf4 *WMBIJ, int do_singles, dpdbuf4 *Dints_anti,
                            ...
                            dpdbuf4 *SIJAB, Dimension const& aoccpi, int *aocc_off, Dimension const& avirtpi, int *avir_off, double omega,
                            std::string out) {
    ...
    const auto& occpi = aoccpi;
    occ_off = aocc_off;
    const auto& virtpi = avirtpi;
    vir_off = avir_off;
                ...
                for (i = 0; i < occpi[Gi]; i++) {
                    ...
                    for (j = 0; j < occpi[Gj]; j++) {
                        ...
                        for (k = 0; k < occpi[Gk]; k++) {
                            ...
                            T3_AAA(W1, nirreps, I, Gi, J, Gj, K, Gk, CMNEF, WABEI, WMBIJ, &fIJ, &fAB, occpi, occ_off,
                            ...
                            if (do_doubles) {
                                ...
                                if (nrows && ncols)
                                    C_DGEMV('n', nrows, ncols, 1.0, W1[Gab][0], ncols, FME->matrix[Gk][k], 1, 1.0,
                                            SIJAB->matrix[Gij][ij], 1);
    ...
    if (do_singles) {
        ...
        for (h = 0; h < nirreps; h++) buf4_mat_irrep_close(Dints_anti, h);
    }

    if (do_doubles) {
        file2_mat_close(FME);

        for (h = 0; h < nirreps; h++) buf4_mat_irrep_close(WMNIE, h);
...
void DPD::cc3_sigma_UHF_BBB(dpdbuf4 *Cmnef, dpdbuf4 *Wabei, dpdbuf4 *Wmbij, int do_singles, dpdbuf4 *Dijab_anti,
                            dpdfile2 *Sia, int do_doubles, dpdfile2 *Fme, dpdbuf4 *Wmafe, dpdbuf4 *Wmnie,
                            dpdbuf4 *Sijab, Dimension const& boccpi, int *bocc_off, Dimension const& bvirtpi, int *bvir_off, double omega,
                            std::string out) {
    std::shared_ptr<psi::PsiOutStream> printer = (out == "outfile" ? outfile : std::make_shared<PsiOutStream>(out));
    int h, nirreps;
    int *occ_off, *vir_off;
    ...
    nirreps = Cmnef->params->nirreps;
    const auto& occpi = boccpi;
    occ_off = bocc_off;
    const auto& virtpi = bvirtpi;
    vir_off = bvir_off;
    ...
    if (do_singles) {
        ...
        for (h = 0; h < nirreps; h++) buf4_mat_irrep_close(Dijab_anti, h);
    }
...
void DPD::cc3_sigma_UHF_AAB(dpdbuf4 *C2AA, dpdbuf4 *C2AB, dpdbuf4 *C2BA, dpdbuf4 *FAA, dpdbuf4 *FAB, dpdbuf4 *FBA,
                            ...
                            Dimension const& avirtpi, int *avir_off, Dimension const& bvirtpi, int *bvir_off, double omega, std::string out) {
    std::shared_ptr<psi::PsiOutStream> printer = (out == "outfile" ? outfile : std::make_shared<PsiOutStream>(out));
    int h, nirreps;
    int Gi, Gj, Gk, Gijk;
    int Ga, Gb, Gc, Gab;
                ...
                for (i = 0; i < aoccpi[Gi]; i++) {
                    ...
                    for (j = 0; j < aoccpi[Gj]; j++) {
                        ...
                        for (k = 0; k < boccpi[Gk]; k++) {
                            ...
                            T3_AAB(W1, nirreps, I, Gi, J, Gj, K, Gk, C2AA, C2AB, C2BA, FAA, FAB, FBA, EAA, EAB, EBA,
                                   &fIJ, &fij, &fAB, &fab, aoccpi, aocc_off, boccpi, bocc_off, avirtpi, avir_off,
                                   bvirtpi, bvir_off, omega);
                            ...
                            if (do_singles) {
                                ...
                                if (nrows && ncols)
                                    C_DGEMV('t', nrows, ncols, 0.25, W1[Gab][0], ncols, DAA->matrix[Gij][ij], 1, 1.0,
                                            Sia->matrix[Gk][k], 1);
                                ...
                                for (Gab = 0; Gab < nirreps; Gab++) {
                                    ...
                                    if (nrows && ncols)
                                        C_DGEMV('n', nrows, ncols, 1.0, W1[Gab][ab], ncols, &(DAB->matrix[Gjk][jk][bc]),
                                                1, 1.0, SIA->matrix[Gi][i], 1);
                                }
                            }
                            ...
                            if (do_doubles) {
                                ...
                                for (Gab = 0; Gab < nirreps; Gab++) {
                                    ...
                                    if (nrows && ncols)
                                        C_DGEMV('t', nrows, ncols, 1.0, W1[Gab][ab], ncols, FME->matrix[Gi][i], 1, 1.0,
                                                &(SIjAb->matrix[Gjk][jk][bc]), 1);
                                }
                ...
                for (i = 0; i < boccpi[Gi]; i++) {
                    ...
                    for (j = 0; j < boccpi[Gj]; j++) {
                        ...
                        for (k = 0; k < aoccpi[Gk]; k++) {
                            ...
                            T3_AAB(W1, nirreps, I, Gi, J, Gj, K, Gk, C2BB, C2BA, C2AB, FBB, FBA, FAB, EBB, EBA, EAB,
                                   &fij, &fIJ, &fab, &fAB, boccpi, bocc_off, aoccpi, aocc_off, bvirtpi, bvir_off,
                                   avirtpi, avir_off, omega);
                            ...
                            if (do_singles) {
                                ...
                                if (nrows && ncols)
                                    C_DGEMV('t', nrows, ncols, 0.25, W1[Gab][0], ncols, DBB->matrix[Gij][ij], 1, 1.0,
                                            SIA->matrix[Gk][k], 1);
                                ...
                                for (Gab = 0; Gab < nirreps; Gab++) {
                                    ...
                                    if (nrows && ncols)
                                        C_DGEMV('n', nrows, ncols, 1.0, W1[Gab][ab], ncols, &(DBA->matrix[Gjk][jk][bc]),
                                                1, 1.0, Sia->matrix[Gi][i], 1);
                                }
                            } /* end do_singles */
