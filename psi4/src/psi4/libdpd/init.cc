...
extern void dpd_init(int dpd_num, int nirreps, long int memory, int cachetype, int *cachefiles, int **cachelist,
                    dpd_file4_cache_entry *priority, int num_subspaces, const std::vector<int *> &spaceArrays) {
    if (dpd_list[dpd_num])
...
DPD::DPD(int dpd_num, int nirreps, long int memory, int cachetype, int *cachefiles, int **cachelist,
         dpd_file4_cache_entry *priority, int num_subspaces, const std::vector<int *> &spaceArrays_in) {
    /* Grab the irrep population and orbital symmetry arrays from the arg list */
    std::vector<std::vector<int>> orbspi_ctor(num_subspaces);
    std::vector<std::vector<int>> orbsym_ctor(num_subspaces);
    std::vector<int> numorbs_ctor(num_subspaces);
    for (size_t i = 0; i < num_subspaces; i++) {
        orbspi_ctor.at(i) = std::vector<int>(nirreps);
        for (size_t j = 0; j < nirreps; j++) orbspi_ctor.at(i).at(j) = spaceArrays_in.at(2 * i)[j];

        /* Compute the number of orbitals in this subspace */
        numorbs_ctor.at(i) = 0;
        for (size_t h = 0; h < nirreps; h++) numorbs_ctor.at(i) += orbspi_ctor.at(i).at(h);

        orbsym_ctor.at(i) = std::vector<int>(numorbs_ctor.at(i));
        for (size_t j = 0; j < numorbs_ctor.at(i); j++) orbsym_ctor.at(i).at(j) = spaceArrays_in.at(2 * i + 1)[j];
    }
    if(num_subspaces*2 != spaceArrays_in.size()) throw PSIEXCEPTION(" ");
    std::vector<std::vector<int>> spaceArrays(num_subspaces*2);
    for (size_t i = 0; i < num_subspaces; i++) {
        for (size_t j = 0; j < nirreps; j++) spaceArrays.at(2*i).push_back(spaceArrays_in.at(2 * i)[j]);
        for (size_t j = 0; j < numorbs_ctor.at(i); j++) spaceArrays.at(2*i + 1).push_back(spaceArrays_in.at(2 * i + 1)[j]);
    }
    init(dpd_num, nirreps, memory, cachetype, cachefiles, cachelist, priority, num_subspaces, spaceArrays);
}
...
DPD::DPD(int dpd_num, int nirreps, long int memory, int cachetype, int *cachefiles, int **cachelist,
    ...
    for (int i = 0; i < num_subspaces; i++) {
        {
        std::vector<int> tmparray(nirreps);
        ...
        std::vector<int> tmparray(spaces[i].nOrb());
...
void DPD::init(int dpd_num_in, int nirreps_in, long int memory_in, int cachetype_in, int *cachefiles_in,
              ...
              const std::vector<int*> &spaceArrays_in) {
...
//    int *tmparray;
    ...
    std::vector<int> numorbs(num_subspaces);
    for (i = 0; i < num_subspaces; i++) {
        orbspi[i] = (int *)malloc(sizeof(int) * nirreps);
        //tmparray = spaceArrays_in.at(2 * i);
        for (j = 0; j < nirreps; j++) orbspi[i][j] = spaceArrays_in.at(2 * i)[j];
...
//        tmparray = spaceArrays_in.at(2 * i + 1);
        for (j = 0; j < numorbs[i]; j++) orbsym[i][j] = spaceArrays_in.at(2 * i + 1)[j];
    }
...
//    int *tmparray;
    ...
    for (i = 0; i < num_subspaces; i++) {
        orbspi[i] = (int *)malloc(sizeof(int) * nirreps);
        //tmparray = spaceArrays_in.at(2 * i);
        for (j = 0; j < nirreps; j++) orbspi[i][j] = spaceArrays_in.at(2 * i).at(j);
        ...
        orbsym[i] = (int *)malloc(sizeof(int) * numorbs[i]);
//        tmparray = spaceArrays_in.at(2 * i + 1);
        for (j = 0; j < numorbs[i]; j++) orbsym[i][j] = spaceArrays_in.at(2 * i + 1).at(j);
    }
