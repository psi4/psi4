...
void IntegralTransform::initialize() {
    ...
    if (transformationType_ == TransformationType::Restricted) {
        reorder_qt(clsdpi_.blocks().data(), openpi_.blocks().data(), frzcpi_, frzvpi_, aQT_, mopi_, nirreps_);
        bQT_ = aQT_;
    } else {
        bQT_ = init_int_array(nmo_);
        reorder_qt_uhf(clsdpi_.blocks().data(), openpi_.blocks().data(), frzcpi_, frzvpi_, aQT_, bQT_, mopi_, nirreps_);
    }
