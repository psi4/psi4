...
PCM::PCM(Options &options, std::shared_ptr<PSIO> /* psio */, int nirrep, std::shared_ptr<BasisSet> basisset) {
    ...
    if (nirrep > 1) throw PSIEXCEPTION("You must add\n\n\tsymmetry c1\n\nto the molecule{} block to run the PCM code.");

    basisset_ = basisset;

    std::shared_ptr<IntegralFactory> integrals(new IntegralFactory(basisset, basisset, basisset, basisset));
    ...
    if (PSI4_provides_input) {
        ...
        context_ = pcmsolver_new(PCMSOLVER_READER_HOST, molecule->natom(), charges, coordinates, symmetry_info,
                                 &host_input, host_writer);
    } else {
        context_ = pcmsolver_new(PCMSOLVER_READER_OWN, molecule->natom(), charges, coordinates, symmetry_info,
                                 &host_input, host_writer);
    }
    ...
    for (int tess = 1; tess <= ntess_; ++tess) pcmsolver_get_center(context_, tess, &(ptess_Zxyz[tess - 1][1]));
