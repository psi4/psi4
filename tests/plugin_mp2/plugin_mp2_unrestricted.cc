...
double plugin_mp2_unrestricted(Options &options, shared_ptr<Chkpt> chkpt)
{
    ...
    IntegralTransform ints(chkpt, spaces,
    ...
    if(ROHF){
        ...
        for(int h = 0, offset = 0; h < nirreps; ++h){
            for(int i = 0; i < aOccOrbsPI[h]; ++i){
                ...
                for(int a = aOccOrbsPI[h] ; a < mopi[h]; ++a){
                    int aa = a + offset;
        ...
        for(int h = 0, offset = 0; h < nirreps; ++h){
            ...
            for(int a = bOccOrbsPI[h]; a < mopi[h]; ++a){
                int aa = a + offset;
    ...
    dpd_buf4_init(&K, PSIF_LIBTRANS_DPD, 0, ID("[o,v]"), ID("[o,v]"),
                  ID("[o,v]"), ID("[o,v]"), 0, "MO Ints (ov|ov)");
    dpd_buf4_sort(&K, PSIF_LIBTRANS_DPD , prqs, ID("[o,o]"), ID("[v,v]"), 
                  "MO Ints <oo|vv>");
    dpd_buf4_close(&K);
    dpd_buf4_init(&K, PSIF_LIBTRANS_DPD, 0, ID("[o>o]-"), ID("[v>v]-"), 
                  ID("[o,o]"), ID("[v,v]"), 1, "MO Ints <oo|vv>");
