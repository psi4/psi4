...
plugin_mp2(Options &options)
{
    ...
    eSCF = Process::environment.reference_wavefunction()->reference_energy();

    nirreps             = Process::environment.reference_wavefunction()->nirrep();
    SharedVector aEvals = Process::environment.reference_wavefunction()->epsilon_a();
    SharedVector bEvals = Process::environment.reference_wavefunction()->epsilon_b();
    ...
    mopi                = Process::environment.reference_wavefunction()->nmopi();
    nmo                 = Process::environment.reference_wavefunction()->nmo();
    clsdpi              = Process::environment.reference_wavefunction()->doccpi();
    openpi              = Process::environment.reference_wavefunction()->soccpi();
    frzcpi              = Process::environment.reference_wavefunction()->frzcpi();
    frzvpi              = Process::environment.reference_wavefunction()->frzvpi();
    ...
    for(int h = 0; h < nirreps; ++h){
        for(int a = 0; a < aOccOrbsPI[h]; ++a) aOccEvals[aOccCount++] = aEvals->get(h, a);
        for(int b = 0; b < bOccOrbsPI[h]; ++b) bOccEvals[bOccCount++] = bEvals->get(h, b);
        for(int a = aOccOrbsPI[h]; a < mopi[h]; ++a) aVirEvals[aVirCount++] = aEvals->get(h, a);
        for(int b = bOccOrbsPI[h]; b < mopi[h]; ++b) bVirEvals[bVirCount++] = bEvals->get(h, b);
    }
