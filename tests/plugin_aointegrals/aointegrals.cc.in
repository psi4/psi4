...
{
    if (name == "PLUGIN_AOINTEGRALS"|| options.read_globals()) {
...
plugin_aointegrals(Options &options)
{
    ...
    shared_ptr<SimpleMatrix> sMat(factory->create_simple_matrix("Overlap"));
    shared_ptr<SimpleMatrix> tMat(factory->create_simple_matrix("Kinetic"));
    shared_ptr<SimpleMatrix> vMat(factory->create_simple_matrix("Potential"));
    shared_ptr<SimpleMatrix> hMat(factory->create_simple_matrix("One Electron Ints"));
