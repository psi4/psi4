...
void GPUCoupledCluster::DIIS(double*c,long int nvec,long int n){
  long int i,j,k;
  doublereal sum,dum;
  integer*ipiv,nvar;
  nvar = nvec+1;
  ...
  lda = ldb = nvar;
  info = 0;
  F_DGESV(nvar,nrhs,A,lda,ipiv,B,ldb,info);
...
void GPUCoupledCluster::DIISOldVector(long int iter,int diis_iter,int replace_diis_iter){
...
double GPUCoupledCluster::DIISErrorVector(int diis_iter,int replace_diis_iter,int iter){
  double nrm;
...
void GPUCoupledCluster::DIISNewAmplitudes(int diis_iter){
  long int o = ndoccact;
  long int arraysize,v = nvirt;
  arraysize=o*o*v*v;
