...
@pytest.mark.parametrize("inp", [
    ...
    # Listed first so they run first: wb97m-v is the only case that puts VV10
    # nonlocal correlation and wK exchange in the same gradient, which makes it
    # the most likely to break while the cuEST wK gradient route is new. b97m-v
    # below is the same VV10 kernel without range separation, so the pair
    # localizes a failure to one side or the other. The rest of the list is in
    # rung order.
