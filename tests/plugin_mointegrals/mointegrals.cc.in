...
{
    if (name == "PLUGIN_MOINTEGRALS"|| options.read_globals()) {
...
plugin_mointegrals(Options &options)
{
    /*
    ...
    int nirrep  = Process::environment.reference_wavefunction()->nirrep();
    int *soccpi = Process::environment.reference_wavefunction()->soccpi();
    int nopen   = 0;
    for(int h = 0; h < nirrep; ++h) nopen += soccpi[h];
    if(nopen) throw PSIEXCEPTION("This code is for closed shell molecules only!");

    // For nww, we'll just transform for closed shells and generate all integrals.  For more elaborate use of the
    ...
    dpd_buf4_init(&K, PSIF_LIBTRANS_DPD, 0, ID("[A,A]"), ID("[A,A]"),
