...
void MRCCSD_T::compute_restricted()
{
  ...
  if(nrefs < 100){
    for(int mu = 0; mu < nrefs; ++mu){
      psi::fprintf(outfile,"\n   %2d  ",mu);
      psi::fprintf(outfile," %17.12lf",closed_shell_factor * (E4_ooo[mu] + E4_ooO[mu] + E4_oOO[mu] + E4_OOO[mu]));
      psi::fprintf(outfile," %17.12lf",closed_shell_factor * (E4T_ooo[mu] + E4T_ooO[mu] + E4T_oOO[mu] + E4T_OOO[mu]));
      psi::fprintf(outfile," %17.12lf",closed_shell_factor * (E4ST_ooo[mu] + E4ST_ooO[mu] + E4ST_oOO[mu] + E4ST_OOO[mu]));
      psi::fprintf(outfile," %17.12lf",closed_shell_factor * (E4DT_ooo[mu] + E4DT_ooO[mu] + E4DT_oOO[mu] + E4DT_OOO[mu]));
    }
  }
  psi::fprintf(outfile,"\n   Tot ");
  double E4 = 0.0;
  ...
  for(int mu = 0; mu < nrefs; ++mu){
    ...
    for(int nu = 0; nu < nrefs; ++nu){
      psi::fprintf(outfile," %17.12lf",closed_shell_factor * d_h_eff[mu][nu]);
    }
  }
