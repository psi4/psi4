...
void Wmbej_build(void) {
  ...
  if(params.ref == 0) { /** RHF **/
    ...
    dpd_->contract424(&F, &tIA, &WMbEj, 3, 1, 0, 1, 1); /* should run OOC, if needed */
    ...
    */
    ...
    dpd_->contract444(&D, &t2, &W, 0, 0, 1, 1);
    dpd_->buf4_close(&D);
    dpd_->buf4_close(&t2);
    ...
    dpd_->contract444(&D, &tiajb, &Wmbej, 0, 0, 0.5, 1);
    dpd_->buf4_close(&D);
    ...
    dpd_->contract444(&D, &tjAIb, &WmBEj, 0, 0, 0.5, 1);
    dpd_->buf4_close(&D);
    ...
    dpd_->contract444(&D, &tIbjA, &WMbeJ, 0, 0, 0.5, 1);
    dpd_->buf4_close(&D);
    ...
    dpd_->buf4_init(&D, PSIF_CC_DINTS, 0, 30, 30, 30, 30, 0, "D <ij||ab> (ia,jb)");
    dpd_->buf4_init(&tiajb, PSIF_CC_TAMPS, 0, 30, 30, 30, 30, 0, "tiajb");
    dpd_->contract444(&D, &tiajb, &Wmbej, 0, 0, 0.5, 1);
    dpd_->buf4_close(&D);
    dpd_->buf4_close(&tiajb);

    /* D(me,NF) * T2(jb,NF) --> W(me,jb) */
    dpd_->buf4_init(&tiaJB, PSIF_CC_TAMPS, 0, 30, 20, 30, 20, 0, "tiaJB");
    dpd_->buf4_init(&D, PSIF_CC_DINTS, 0, 30, 20, 30, 20, 0, "D <Ij|Ab> (ia,JB)");
    dpd_->contract444(&D, &tiaJB, &Wmbej, 0, 0, 0.5, 1);
    dpd_->buf4_close(&D);
    dpd_->buf4_close(&tiaJB);
    ...
    dpd_->buf4_init(&D, PSIF_CC_DINTS, 0, 20, 20, 20, 20, 0, "D <IJ||AB> (IA,JB)");
    dpd_->buf4_init(&tIAJB, PSIF_CC_TAMPS, 0, 20, 20, 20, 20, 0, "tIAJB");
    dpd_->contract444(&D, &tIAJB, &WMBEJ, 0, 0, 0.5, 1);
    dpd_->buf4_close(&D);
    dpd_->buf4_close(&tIAJB);

    /* D(ME,nf) * T2(JB,nf) --> W(ME,JB) */
    dpd_->buf4_init(&tIAjb, PSIF_CC_TAMPS, 0, 20, 30, 20, 30, 0, "tIAjb");
    dpd_->buf4_init(&D, PSIF_CC_DINTS, 0, 20, 30, 20, 30, 0, "D <Ij|Ab> (IA,jb)");
    dpd_->contract444(&D, &tIAjb, &WMBEJ, 0, 0, 0.5, 1);
    dpd_->buf4_close(&D);
    dpd_->buf4_close(&tIAjb);
    ...
    /* D(mE,Nf) * T2(jB,Nf) --> W(mE,jB) */
    dpd_->buf4_init(&WmBEj, PSIF_CC_HBAR, 0, 27, 27, 27, 27, 0, "WmBEj");

    dpd_->buf4_init(&D, PSIF_CC_DINTS, 0, 27, 24, 27, 24, 0, "D <iJ|aB> (iB,Ja)");
    dpd_->buf4_init(&tjAIb, PSIF_CC_TAMPS, 0, 27, 24, 27, 24, 0, "tjAIb");
    dpd_->contract444(&D, &tjAIb, &WmBEj, 0, 0, 0.5, 1);
    dpd_->buf4_close(&D);
    dpd_->buf4_close(&tjAIb);
    ...
    /* D(Me,nF) * T2(Jb,nF) --> W(Me,Jb) */
    dpd_->buf4_init(&WMbeJ, PSIF_CC_HBAR, 0, 24, 24, 24, 24, 0, "WMbeJ");

    dpd_->buf4_init(&D, PSIF_CC_DINTS, 0, 24, 27, 24, 27, 0, "D <Ij|Ab> (Ib,jA)");
    dpd_->buf4_init(&tIbjA, PSIF_CC_TAMPS, 0, 24, 27, 24, 27, 0, "tIbjA");
    dpd_->contract444(&D, &tIbjA, &WMbeJ, 0, 0, 0.5, 1);
    dpd_->buf4_close(&D);
    dpd_->buf4_close(&tIbjA);
