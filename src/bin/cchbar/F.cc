...
void F_build(void) {
  ...
  if(params.ref == 0) {
    ...
    */
    ...
    dpd_->file2_init(&tIA, PSIF_CC_OEI, 0, 0, 1, "tIA");
    dpd_->file2_init(&FME, PSIF_CC_OEI, 0, 0, 1, "FME");
    dpd_->contract222(&tIA, &FME, &FAE, 1, 1, -0.5, 1);
    dpd_->file2_close(&tIA);
    dpd_->file2_close(&FME);
    ...
    dpd_->file2_init(&tIA, PSIF_CC_OEI, 0, 0, 1, "tIA");
    dpd_->file2_init(&FME, PSIF_CC_OEI, 0, 0, 1, "FME");
    dpd_->contract222(&FME, &tIA, &FMI, 0, 0, 0.5, 1);
    dpd_->file2_close(&FME);
    dpd_->file2_close(&tIA);
    ...
    dpd_->file2_mat_init(&FAEt);
    dpd_->file2_mat_rd(&FAEt);

    for(h=0; h < moinfo.nirreps; h++)
      for(a=0; a < FAEt.params->rowtot[h]; a++)
	FAEt.matrix[h][a][a] = 0.0;

    dpd_->file2_mat_wrt(&FAEt);
    dpd_->file2_mat_close(&FAEt);
    dpd_->file2_close(&FAEt);

    dpd_->file2_init(&FMIt, PSIF_CC_OEI, 0, 0, 0, "FMIt");
    dpd_->file2_mat_init(&FMIt);
    dpd_->file2_mat_rd(&FMIt);

    for(h=0; h < moinfo.nirreps; h++)
      for(a=0; a < FMIt.params->rowtot[h]; a++)
	FMIt.matrix[h][a][a] = 0.0;

    dpd_->file2_mat_wrt(&FMIt);
    dpd_->file2_mat_close(&FMIt);
