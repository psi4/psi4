...
void Wabei_RHF(void)
{
  ...
  dpd_->buf4_sort(&F, PSIF_CC_HBAR, qpsr, 11, 5, "WAbEi (Ei,Ab)");
  dpd_->buf4_close(&F);
  ...
  for(Gei=0; Gei < moinfo.nirreps; Gei++) {
    ...
    for(Ge=0; Ge < moinfo.nirreps; Ge++) {
      ...
      W.matrix[Gei] = dpd_->dpd_block_matrix(moinfo.occpi[Gi], W.params->coltot[Gei]);
	...
	for(E=0; E < moinfo.virtpi[Ge]; E++) {
	  ...
	  dpd_->buf4_mat_irrep_rd_block(&W, Gei, W.row_offset[Gei][e], moinfo.occpi[Gi]);

	  C_DGEMV('t',nrows,ncols,-1.0,&T2.matrix[Gmi][row][0],ncols,&Fme.matrix[Gm][0][E],
		  moinfo.virtpi[Ge],1.0,W.matrix[Gei][0],1);

	  dpd_->buf4_mat_irrep_wrt_block(&W, Gei, W.row_offset[Gei][e], moinfo.occpi[Gi]);
	}
      }

      row += moinfo.occpi[Gm] * moinfo.occpi[Gi];
      dpd_->free_dpd_block(W.matrix[Gei], moinfo.occpi[Gi], W.params->coltot[Gei]);
    }
  ...
  dpd_->buf4_init(&Z, PSIF_CC_HBAR, 0, 0, 11, 0, 11, 0, "WMnIe (nM,eI)");
  dpd_->buf4_sort(&Z, PSIF_CC_HBAR, rspq, 11, 0, "WMnIe (eI,nM)");
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&W, PSIF_CC_HBAR, 0, 11, 5, 11, 5, 0, "WAbEi (Ei,Ab)");
  dpd_->buf4_init(&Z, PSIF_CC_HBAR, 0, 11, 0, 11, 0, 0, "WMnIe (eI,nM)");
  ...
  for(Gei=0; Gei < moinfo.nirreps; Gei++) {
    ...
    if(nrows && ncols) {
      dpd_->buf4_mat_irrep_init(&Z, Gei);
      dpd_->buf4_mat_irrep_rd(&Z, Gei);
      dpd_->buf4_mat_irrep_init(&T, Gnm);
      dpd_->buf4_mat_irrep_rd(&T, Gnm);
	...
	C_DGEMV('t',nrows,ncols,1,T.matrix[Gei][0],ncols,Z.matrix[Gei][ei],1,
		1,W.matrix[Gei][0],1);
	dpd_->buf4_mat_irrep_row_wrt(&W, Gei, ei);
      }
      dpd_->buf4_mat_irrep_row_close(&W, Gei);
      dpd_->buf4_mat_irrep_close(&T, Gnm);
      dpd_->buf4_mat_irrep_close(&Z, Gei);
    }
  }
  dpd_->buf4_close(&T);
  dpd_->buf4_close(&Z);
  ...
  ** ovvv quantities on disk besides the <ia|bc> integrals and the Wabei target. */
  if(!params.wabei_lowdisk) {
    ...
    dpd_->buf4_init(&F, PSIF_CC_FINTS, 0, 10, 5, 10, 5, 0, "F <ia|bc>");
    dpd_->buf4_sort(&F, PSIF_CC_TMP0, prsq, 10, 5, "F <ia|bc> (ib,ca)");
    dpd_->buf4_close(&F);
    ...
    dpd_->buf4_init(&W, PSIF_CC_TMP0, 0, 10, 5, 10, 5, 0, "W(ib,ea)");
    dpd_->buf4_init(&F, PSIF_CC_TMP0, 0, 10, 5, 10, 5, 0, "F <ia|bc> (ib,ca)");
    dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 10, 10, 10, 10, 0, "Z1(ib,mf)");
    if(incore) dpd_->contract444(&Z, &F, &W, 0, 1, 1, 0);
    else ZFW(&Z, &F, &W, 1, 0);
    dpd_->buf4_close(&Z);
    dpd_->buf4_close(&F);
    dpd_->buf4_init(&F, PSIF_CC_FINTS, 0, 10, 5, 10, 5, 0, "F <ia|bc>");
    dpd_->buf4_init(&Z, PSIF_CC_TAMPS, 0, 10, 10, 10, 10, 0, "tIAjb");
    if(incore) dpd_->contract444(&Z, &F, &W, 0, 1, -1, 1);
    else ZFW(&Z, &F, &W, -1, 1);
    dpd_->buf4_close(&Z);
    dpd_->buf4_close(&F);
    dpd_->buf4_sort_axpy(&W, PSIF_CC_HBAR, rpsq, 11, 5, "WAbEi (Ei,Ab)", 1);
    dpd_->buf4_close(&W);
    psio_close(PSIF_CC_TMP0, 0); /* delete the extra ovvv quantities on disk */
    psio_open(PSIF_CC_TMP0, PSIO_OPEN_NEW);
    dpd_->buf4_init(&W, PSIF_CC_TMP0, 0, 10, 5, 10, 5, 0, "W(ia,eb)");
    dpd_->buf4_init(&F, PSIF_CC_FINTS, 0, 10, 5, 10, 5, 0, "F <ia|bc>");
    dpd_->buf4_init(&Z, PSIF_CC_TAMPS, 0, 10, 10, 10, 10, 0, "tauIjAb (Ib,jA)");
    if(incore) dpd_->contract444(&Z, &F, &W, 0, 1, -1, 0);
    else ZFW(&Z, &F, &W, -1, 0);
    dpd_->buf4_close(&Z);
    dpd_->buf4_close(&F);
    dpd_->buf4_sort_axpy(&W, PSIF_CC_HBAR, rpqs, 11, 5, "WAbEi (Ei,Ab)", 1);
    dpd_->buf4_close(&W);
  ...
  ** ovvv terms beyond <ia|bc> integrals and the WAbEi target, but it's very slow. */
    ...
    dpd_->buf4_init(&F, PSIF_CC_FINTS, 0, 10, 5, 10, 5, 0, "F <ia|bc>");
    dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 10, 10, 10, 10, 0, "Z1(ib,mf)");
    dpd_->buf4_sort(&Z, PSIF_CC_TMP0, rpsq, 0, 5, "Z1(mi,fb)");
    dpd_->buf4_close(&Z);
    dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 0, 5, 0, 5, 0, "Z1(mi,fb)");
    dpd_->buf4_init(&T2, PSIF_CC_TAMPS, 0, 0, 5, 0, 5, 0, "tIjAb");
	...
	for(Gi=0; Gi < moinfo.nirreps; Gi++) {
	  Gmi = Gm ^ Gi;
	  dpd_->buf4_mat_irrep_row_init(&Z, Gmi);
	  dpd_->buf4_mat_irrep_row_init(&T2, Gmi);
	  dpd_->buf4_mat_irrep_row_init(&Tau, Gmi);
	  ...
	  for(Gf=0; Gf < moinfo.nirreps; Gf++) {
	    ...
	    WW1[Gb] = dpd_->dpd_block_matrix(moinfo.virtpi[Gb], moinfo.virtpi[Ge]);
	    WW2[Gb] = dpd_->dpd_block_matrix(moinfo.virtpi[Gb], moinfo.virtpi[Ge]);
	  }
	  for(I=0; I < moinfo.occpi[Gi]; I++) {
	    i = moinfo.occ_off[Gi] + I;
	    mi = Z.params->rowidx[m][i];
	    dpd_->buf4_mat_irrep_row_rd(&Z, Gmi, mi);
	    dpd_->buf4_mat_irrep_row_rd(&T2, Gmi, mi);
	    dpd_->buf4_mat_irrep_row_rd(&Tau, Gmi, mi);
	    for(Gf=0; Gf < moinfo.nirreps; Gf++) {
	      Gb = Gf ^ Gmi; /* Z is totally symmetric */
	      Ge = Gf ^ Gma; /* F is totally symmetric */
	      Gei = Ge ^ Gi;
	  ...
	  dpd_->buf4_mat_irrep_row_close(&Z, Gmi);
	  dpd_->buf4_mat_irrep_row_close(&T2, Gmi);
	  dpd_->buf4_mat_irrep_row_close(&Tau, Gmi);
	} /* Gi */
	  /* free W1 target */
	for(Gf=0; Gf < moinfo.nirreps; Gf++) {
	  Gb = Gf ^ Gmi; /* Z is totally symmetric */
	  Ge = Gf ^ Gma; /* F is totally symmetric */
	  dpd_->free_dpd_block(WW1[Gb],moinfo.virtpi[Gb], moinfo.virtpi[Ge]);
	  dpd_->free_dpd_block(WW2[Gb],moinfo.virtpi[Gb], moinfo.virtpi[Ge]);
	}
      } /* ma */
  ...
  dpd_->buf4_close(&D);
  dpd_->buf4_close(&T2);
  dpd_->buf4_sort(&Z, PSIF_CC_TMP0, qrps, 11, 10, "Z(Ei,Mb)");
  dpd_->buf4_close(&Z);

  /** - t_M^A ( <Ei|Mb> + Z(Ei,Mb) ) --> W(Ei,Ab) **/
  dpd_->buf4_init(&D, PSIF_CC_DINTS, 0, 0, 5, 0, 5, 0, "D <ij|ab>");
  dpd_->buf4_sort_axpy(&D, PSIF_CC_TMP0, spqr, 11, 10, "Z(Ei,Mb)", 1);
  dpd_->buf4_close(&D);
  ...
  dpd_->buf4_init(&T2, PSIF_CC_TAMPS, 0, 10, 10, 10, 10, 0, "tIbjA");
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 10, 10, 10, 10, 0, "Z(mE,iA)");
  dpd_->contract444(&D, &T2, &Z, 0, 0, 1, 0);
  dpd_->buf4_close(&T2);
  dpd_->buf4_close(&D);
  dpd_->buf4_sort(&Z, PSIF_CC_TMP0, qrsp, 11, 11, "Z(Ei,Am)");
  dpd_->buf4_close(&Z);

  /** t_m^b ( - <mA|iE> + Z(mA,iE) ) --> Z2(Ab,Ei) **/
  dpd_->buf4_init(&C, PSIF_CC_CINTS, 0, 11, 11, 11, 11, 0, "C <ai|bj>");
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 11, 11, 11, 11, 0, "Z(Ei,Am)");
...
void build_Z1(void)
{
  ...
  dpd_->buf4_copy(&T2, PSIF_CC_TMP0, "Z1(ib,mf)");
  dpd_->buf4_close(&T2);
  ...
  dpd_->buf4_init(&Z1, PSIF_CC_TMP0, 0, 10, 10, 10, 10, 0, "Z1(ib,mf)");
  for(h=0; h < moinfo.nirreps; h++) {
    dpd_->buf4_mat_irrep_init(&Z1, h);
    dpd_->buf4_mat_irrep_rd(&Z1, h);
    ...
    dpd_->buf4_mat_irrep_wrt(&Z1, h);
    dpd_->buf4_mat_irrep_close(&Z1, h);
  }
  dpd_->file2_mat_close(&T1);
  dpd_->file2_close(&T1);
...
void ZFW(dpdbuf4 *Z, dpdbuf4 *F, dpdbuf4 *W, double alpha, double beta)
{
  ...
  for(Gib=0; Gib < moinfo.nirreps; Gib++) {
    ...
    dpd_->buf4_mat_irrep_init(Z, Gib);
    ...
    dpd_->buf4_mat_irrep_init_block(F, Gib, rows_per_bucket);
    dpd_->buf4_mat_irrep_init_block(W, Gib, rows_per_bucket);
    ...
    for(m=0; m < (rows_left ? nbuckets-1 : nbuckets); m++) {
      ...
      if(beta == 1.0)
	dpd_->buf4_mat_irrep_rd_block(W, Gib, W_row_start, rows_per_bucket);

      for(n=0; n < (rows_left ? nbuckets-1 : nbuckets); n++) {
	F_row_start = n*rows_per_bucket;
	dpd_->buf4_mat_irrep_rd_block(F, Gib, F_row_start, rows_per_bucket);
	...
	if(nrows && ncols && nlinks)
      ...
      if(rows_left) {
	F_row_start = n*rows_per_bucket;
	dpd_->buf4_mat_irrep_rd_block(F, Gib, F_row_start, rows_left);
	...
	if(nrows && ncols && nlinks)
      ...
      dpd_->buf4_mat_irrep_wrt_block(W, Gib, W_row_start, rows_per_bucket);
    }
    if(rows_left) {
      W_row_start = m*rows_per_bucket;
      zero_arr(W->matrix[Gib][0], rows_per_bucket*ncols);
      ...
      if(beta == 1.0)
	dpd_->buf4_mat_irrep_rd_block(W, Gib, W_row_start, rows_left);
	...
	if(nrows && ncols && nlinks)
      ...
      dpd_->buf4_mat_irrep_wrt_block(W, Gib, W_row_start, rows_left);
    } /* m */

    dpd_->buf4_mat_irrep_close_block(F, Gib, rows_per_bucket);
    dpd_->buf4_mat_irrep_close_block(W, Gib, rows_per_bucket);

    dpd_->buf4_mat_irrep_close(Z, Gib);
