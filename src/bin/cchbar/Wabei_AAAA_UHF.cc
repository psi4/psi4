...
void WABEI_UHF(void)
{
  ...
  /** W'(AB,EI) <--- <AB||EF> t_I^F **/
  dpd_->buf4_init(&W, PSIF_CC_TMP0, 0, 7, 21, 7, 21, 0, "W'(AB,EI)");
  ...
  **/
  ...
  dpd_->file2_close(&T1);
  dpd_->buf4_close(&Z);
  dpd_->buf4_sort(&Z1, PSIF_CC_TMP0, qprs, 5, 21, "Z2(BA,EI)");
  dpd_->buf4_close(&Z1);

  /** Z1(AB,EI) - Z2(BA,EI) --> W'(AB,EI) **/
  dpd_->buf4_init(&Z1, PSIF_CC_TMP0, 0, 5, 21, 5, 21, 0, "Z1(AB,EI)");
  dpd_->buf4_init(&Z2, PSIF_CC_TMP0, 0, 5, 21, 5, 21, 0, "Z2(BA,EI)");
  dpd_->buf4_axpy(&Z2, &Z1, -1.0);
  dpd_->buf4_close(&Z2);

  dpd_->buf4_init(&W, PSIF_CC_TMP0, 0, 5, 21, 7, 21, 0, "W'(AB,EI)");
  ...
  **/
  ...
  /** Z(MN,EI) <-- <MN||EF> t_I^F **/
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 2, 21, 2, 21, 0, "Z(MN,EI)");
  ...
  **/
  ...
  /** <BM||EF> --> F(BE,MF) **/
  dpd_->buf4_init(&F, PSIF_CC_FINTS, 0, 21, 5, 21, 5, 1, "F <AI|BC>");
  dpd_->buf4_sort(&F, PSIF_CC_FINTS, prqs, 5, 20, "F <AI||BC> (AB,IC)");
  dpd_->buf4_close(&F);

  /** <Bm|Ef> --> (BE,mf) **/
  dpd_->buf4_init(&F, PSIF_CC_FINTS, 0, 26, 28, 26, 28, 0, "F <Ai|Bc>");
  dpd_->buf4_sort(&F, PSIF_CC_FINTS, prqs, 5, 30, "F <Ai|Bc> (AB,ic)");
  dpd_->buf4_close(&F);

  /** <BM||EF> t_IM^AF --> Z(BE,IA) **/
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 5, 20, 5, 20, 0, "Z(BE,IA)");
  dpd_->buf4_init(&F, PSIF_CC_FINTS, 0, 5, 20, 5, 20, 0, "F <AI||BC> (AB,IC)");
  ...
  /** <Bm|Ef> t_Im^Af --> Z(BE,IA) **/
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 5, 20, 5, 20, 0, "Z(BE,IA)");
  dpd_->buf4_init(&F, PSIF_CC_FINTS, 0, 5, 30, 5, 30, 0, "F <Ai|Bc> (AB,ic)");
  ...
  /** Z(BE,IA) --> Z'(EI,AB) **/
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 5, 20, 5, 20, 0, "Z(BE,IA)");
  dpd_->buf4_sort(&Z, PSIF_CC_TMP0, qrsp, 21, 5, "Z'(EI,AB)");
  dpd_->buf4_close(&Z);

  /** Z'(EI,AB) --> Z''(EI,BA) **/
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 21, 5, 21, 5, 0, "Z'(EI,AB)");
  dpd_->buf4_sort(&Z, PSIF_CC_TMP0, pqsr, 21, 5, "Z''(EI,BA)");
  dpd_->buf4_close(&Z);

  /** Z'(EI,AB) = Z'(EI,AB) - Z''(EI,BA) **/
  dpd_->buf4_init(&Z1, PSIF_CC_TMP0, 0, 21, 5, 21, 5, 0, "Z'(EI,AB)");
  dpd_->buf4_init(&Z2, PSIF_CC_TMP0, 0, 21, 5, 21, 5, 0, "Z''(EI,BA)");
  dpd_->buf4_axpy(&Z2, &Z1, -1);
  dpd_->buf4_close(&Z2);
  dpd_->buf4_close(&Z1);

  /** W(EI,AB) <-- Z'(EI,AB) **/
  dpd_->buf4_init(&W, PSIF_CC_HBAR, 0, 21, 5, 21, 7, 0, "WEIAB");
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 21, 5, 21, 5, 0, "Z'(EI,AB)");
  ...
  **/
  ...
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 20, 20, 20, 20, 0, "Z(ME,IB)");
  ...
  /** Z(ME,IB) --> Z(MB,EI) **/
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 20, 20, 20, 20, 0, "Z(ME,IB)");
  dpd_->buf4_sort_axpy(&Z, PSIF_CC_TMP0, psqr, 20, 21, "Z(MB,EI)", 1);
  dpd_->buf4_close(&Z);

  /** Z(AB,EI) <-- -t_M^A Z(MB,EI) **/
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 5, 21, 5, 21, 0, "Z(AB,EI)");
  dpd_->buf4_init(&Z1, PSIF_CC_TMP0, 0, 20, 21, 20, 21, 0, "Z(MB,EI)");
  dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tIA");
  ...
  /** Z(AB,EI) --> Z'(BA,EI) **/
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 5, 21, 5, 21, 0, "Z(AB,EI)");
  dpd_->buf4_sort(&Z, PSIF_CC_TMP0, qprs, 5, 21, "Z'(BA,EI)");
  dpd_->buf4_close(&Z);

  /** Z(AB,EI) = Z(AB,EI) - Z'(BA,EI) **/
  dpd_->buf4_init(&Z1, PSIF_CC_TMP0, 0, 5, 21, 5, 21, 0, "Z(AB,EI)");
  dpd_->buf4_init(&Z2, PSIF_CC_TMP0, 0, 5, 21, 5, 21, 0, "Z'(BA,EI)");
  dpd_->buf4_axpy(&Z2, &Z1, -1);
  dpd_->buf4_close(&Z2);
  dpd_->buf4_close(&Z1);
  ...
  dpd_->buf4_init(&W, PSIF_CC_TMP0, 0, 7, 21, 7, 21, 0, "W'(AB,EI)");
  dpd_->buf4_sort_axpy(&W, PSIF_CC_HBAR, rspq, 21, 7, "WEIAB", 1);
  dpd_->buf4_close(&W);
}
