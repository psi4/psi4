...
void cc2_Wabei_build(void)
{
  ...
  if(params.ref == 0) { /** RHF **/
    ...
    dpd_buf4_copy(&F, PSIF_CC_TMP0, "CC2 WAbEi (Ei,Ab)");
    dpd_buf4_close(&F);
    ...
    dpd_buf4_sort(&F, PSIF_CC_TMP2, rspq, 7, 11, "CC2 WABEI (A>B,EI)");
    dpd_buf4_sort(&F, PSIF_CC_TMP2, rspq, 7, 11, "CC2 Wabei (a>b,ei)");
    dpd_buf4_close(&F);
    ...
    /* W(aB,eI) <--- <aB|eI> */
    dpd_buf4_init(&F, PSIF_CC_FINTS, 0, 11, 5, 11, 5, 0, "F <ai|bc>");
    dpd_buf4_sort(&F, PSIF_CC_TMP2, rspq, 5, 11, "CC2 WAbEi (Ab,Ei)");
    dpd_buf4_sort(&F, PSIF_CC_TMP2, rspq, 5, 11, "CC2 WaBeI (aB,eI)");
    dpd_buf4_close(&F);
  }
    ...
    dpd_buf4_sort(&F, PSIF_CC_TMP0, rspq, 7, 21, "CC2 WABEI (A>B,EI)");
    dpd_buf4_close(&F);

    /* W(a>b,ei) <--- <ab||ei> */
    dpd_buf4_init(&F, PSIF_CC_FINTS, 0, 31, 17, 31,15, 1, "F <ai|bc>");
    dpd_buf4_sort(&F, PSIF_CC_TMP0, rspq, 17, 31, "CC2 Wabei (a>b,ei)");
    dpd_buf4_close(&F);

    /* W(Ab,Ei) <--- <Ab|Ei> */
    dpd_buf4_init(&F, PSIF_CC_FINTS, 0, 28, 26, 28, 26, 0, "F <Ab|Ci>");
    dpd_buf4_copy(&F, PSIF_CC_TMP0, "CC2 WAbEi (Ab,Ei)");
    dpd_buf4_close(&F);

    /* W(aB,eI) <--- <aB|eI> */
    dpd_buf4_init(&F, PSIF_CC_FINTS, 0, 25, 29, 25, 29, 0, "F <aI|bC>");
    dpd_buf4_sort(&F, PSIF_CC_TMP0, psrq, 29, 25, "CC2 WaBeI (aB,eI)");
    dpd_buf4_close(&F);
  }
  timer_off("F->Wabei");
  ...
  if(params.ref == 0) { /** RHF **/
    ...
    dpd_buf4_init(&W, PSIF_CC_TMP0, 0, 11, 5, 11, 5, 0, "CC2 WAbEi (Ei,Ab)");
    ...
    /** W(a>b,ei) <--- <ab||ef> * t1[i][f] **/
    dpd_buf4_init(&W, PSIF_CC_TMP2, 0, 7, 11, 7, 11, 0, "CC2 WABEI (A>B,EI)");
    ...
    dpd_buf4_init(&W, PSIF_CC_TMP2, 0, 5, 11, 5, 11, 0, "CC2 WAbEi (Ab,Ei)");
    ...
    /** W(A>B,EI) <--- <AB||EF> * t1[I][F] **/
    dpd_buf4_init(&W, PSIF_CC_TMP0, 0, 7, 21, 7, 21, 0, "CC2 WABEI (A>B,EI)");
    dpd_buf4_init(&B, PSIF_CC_BINTS, 0, 7, 5, 5, 5, 1, "B <AB|CD>");
    ...
    /** W(a>b,ei) <--- <ab||ef> * t1[i][f] **/
    dpd_buf4_init(&W, PSIF_CC_TMP0, 0, 17, 31, 17, 31, 0, "CC2 Wabei (a>b,ei)");
    dpd_buf4_init(&B, PSIF_CC_BINTS, 0, 17, 15, 15, 15, 1, "B <ab|cd>");
    dpd_contract424(&B, &tia, &W, 3, 1, 0, 1, 1);
    dpd_buf4_close(&B);
    dpd_buf4_close(&W);

    /** W(Ab,Ei) <--- <Ab|Ef> * t1[i][f] **/
    dpd_buf4_init(&W, PSIF_CC_TMP0, 0, 28, 26, 28, 26, 0, "CC2 WAbEi (Ab,Ei)");
    ...
    /** W(aB,eI) <--- t1[I][F] * <aB|eF>  **/
    dpd_buf4_init(&Z, PSIF_CC_TMP0, 0, 28, 24, 28, 24, 0, "CC2 ZBaIe (Ba,Ie)");
    dpd_buf4_init(&B, PSIF_CC_BINTS, 0, 28, 28, 28, 28, 0, "B <Ab|Cd>");
    dpd_contract244(&tIA, &B, &Z, 1, 2, 1, 1, 0);
    dpd_buf4_close(&B);
    dpd_buf4_sort_axpy(&Z, PSIF_CC_TMP0, qpsr, 29, 25, "CC2 WaBeI (aB,eI)", 1);
    dpd_buf4_close(&Z);
  ...
  if(params.ref == 0) { /** RHF **/
    ...
    dpd_buf4_init(&W, PSIF_CC_TMP0, 0, 11, 5, 11, 5, 0, "CC2 WAbEi (Ei,Ab)");
    dpd_buf4_init(&Z, PSIF_CC_TMP0, 0, 11, 11, 11, 11, 0, "CC2 ZMbeJ (eJ,bM)");
    ...
    /** AAA ***/
    dpd_buf4_init(&Z1, PSIF_CC_TMP0, 0, 5, 21, 5, 21, 0, "CC2 ZABEI (AB,EI)");
    dpd_buf4_init(&Z, PSIF_CC_TMP0, 0, 20, 21, 20, 21, 0, "CC2 ZMBEJ");
    dpd_contract244(&tIA, &Z, &Z1, 0, 0, 0, -1, 0);
    dpd_buf4_close(&Z);
   
    dpd_buf4_sort(&Z1, PSIF_CC_TMP0, qprs, 5, 21, "CC2 ZABEI (BA,EI)");
    dpd_buf4_init(&Z, PSIF_CC_TMP0, 0, 5, 21, 5, 21, 0, "CC2 ZABEI (BA,EI)");
    dpd_buf4_axpy(&Z, &Z1, -1);
    dpd_buf4_close(&Z);
    dpd_buf4_init(&W, PSIF_CC_TMP0, 0, 5, 21, 7, 21, 0, "CC2 WABEI (A>B,EI)");
    dpd_buf4_axpy(&Z1, &W, 1);
    dpd_buf4_close(&Z1);
    dpd_buf4_close(&W);

    /** ABAB **/
    dpd_buf4_init(&Z1, PSIF_CC_TMP0, 0, 28, 26, 28, 26, 0, "CC2 ZAbEi (Ab,Ei)");
    dpd_buf4_init(&Z, PSIF_CC_TMP0, 0, 24, 26, 24, 26, 0, "CC2 ZMbEj");
    dpd_contract244(&tIA, &Z, &Z1, 0, 0, 0, -1, 0);
    dpd_buf4_close(&Z);
    dpd_buf4_close(&Z1);

    dpd_buf4_init(&Z1, PSIF_CC_TMP0, 0, 29, 26, 29, 26, 0, "CC2 ZAbEi (bA,Ei)");
    dpd_buf4_init(&Z, PSIF_CC_TMP0, 0, 27, 26, 27, 26, 0, "CC2 ZmBEj");
    dpd_contract244(&tia, &Z, &Z1, 0, 0, 0, -1, 0);
    dpd_buf4_close(&Z);
    dpd_buf4_sort_axpy(&Z1, PSIF_CC_TMP0, qprs, 28, 26, "CC2 ZAbEi (Ab,Ei)", 1);
    dpd_buf4_close(&Z1);

    dpd_buf4_init(&Z, PSIF_CC_TMP0, 0, 28, 26, 28, 26, 0, "CC2 ZAbEi (Ab,Ei)");
    dpd_buf4_init(&W, PSIF_CC_TMP0, 0, 28, 26, 28, 26, 0, "CC2 WAbEi (Ab,Ei)");
    dpd_buf4_axpy(&Z, &W, -1);
    dpd_buf4_close(&W);
    dpd_buf4_close(&Z);
    ...
    /** BABA **/
    dpd_buf4_init(&Z1, PSIF_CC_TMP0, 0, 29, 25, 29, 25, 0, "CC2 ZaBeI (aB,eI)");
    dpd_buf4_init(&Z, PSIF_CC_TMP0, 0, 27, 25, 27, 25, 0, "CC2 ZmBeJ");
    dpd_contract244(&tia, &Z, &Z1, 0, 0, 0, -1, 0);
    dpd_buf4_close(&Z);
    dpd_buf4_close(&Z1);

    dpd_buf4_init(&Z1, PSIF_CC_TMP0, 0, 28, 25, 28, 25, 0, "CC2 ZaBeI (Ba,eI)");
    dpd_buf4_init(&Z, PSIF_CC_TMP0, 0, 24, 25, 24, 25, 0, "CC2 ZMbeJ");
    dpd_contract244(&tIA, &Z, &Z1, 0, 0, 0, -1, 0);
    dpd_buf4_close(&Z);
    dpd_buf4_sort_axpy(&Z1, PSIF_CC_TMP0, qprs, 29, 25, "CC2 ZaBeI (aB,eI)", 1);
    dpd_buf4_close(&Z1);

    dpd_buf4_init(&Z, PSIF_CC_TMP0, 0, 29, 25, 29, 25, 0, "CC2 ZaBeI (aB,eI)");
    dpd_buf4_init(&W, PSIF_CC_TMP0, 0, 29, 25, 29, 25, 0, "CC2 WaBeI (aB,eI)");
    dpd_buf4_axpy(&Z, &W, -1);
    dpd_buf4_close(&W);
    dpd_buf4_close(&Z);

    /** BBBB **/
    dpd_buf4_init(&Z1, PSIF_CC_TMP0, 0, 15, 31, 15, 31, 0, "CC2 Zabei (ab,ei)");
    dpd_buf4_init(&Z, PSIF_CC_TMP0, 0, 30, 31, 30, 31, 0, "CC2 Zmbej");
    dpd_contract244(&tia, &Z, &Z1, 0, 0, 0, -1, 0);
    dpd_buf4_close(&Z);
    dpd_buf4_sort(&Z1, PSIF_CC_TMP0, qprs, 15, 31, "CC2 Zabei (ba,ei)");
    dpd_buf4_init(&Z, PSIF_CC_TMP0, 0, 15, 31, 15, 31, 0, "CC2 Zabei (ba,ei)");
    dpd_buf4_axpy(&Z, &Z1, -1);
    dpd_buf4_close(&Z);
    dpd_buf4_init(&W, PSIF_CC_TMP0, 0, 15, 31, 17, 31, 0, "CC2 Wabei (a>b,ei)");
    dpd_buf4_axpy(&Z1, &W, 1);
    dpd_buf4_close(&Z1);
    dpd_buf4_close(&W);
  ...
  if (params.ref == 0) { /* RHF */
    ...
    dpd_buf4_init(&W, PSIF_CC2_HET1, 0, 5, 11, 5, 11, 0, "CC2 WAbEi");
    dpd_buf4_scm(&W, 0);
    dpd_buf4_close(&W);

    dpd_buf4_init(&W, PSIF_CC_TMP0, 0, 11, 5, 11, 5, 0, "CC2 WAbEi (Ei,Ab)");
    dpd_buf4_sort_axpy(&W, PSIF_CC2_HET1, rspq, 5, 11, "CC2 WAbEi", 1);
    dpd_buf4_close(&W);

    if(params.wfn == "EOM_CC2") {
      dpd_buf4_init(&W, PSIF_CC_TMP0, 0, 11, 5, 11, 5, 0, "CC2 WAbEi (Ei,Ab)");
      dpd_buf4_copy(&W, PSIF_CC2_HET1, "CC2 WAbEi (Ei,Ab)");
      dpd_buf4_close(&W);
    }
  ...
  if (params.ref == 1) { /* ROHF */
    ...
    /* sort to Wabei (ei,ab) */
    dpd_buf4_init(&W, PSIF_CC_TMP2, 0, 7, 11, 7, 11, 0, "CC2 WABEI (A>B,EI)");
    dpd_buf4_sort(&W, PSIF_CC2_HET1, rspq, 11, 7, "CC2 WABEI (EI,A>B)");
    dpd_buf4_close(&W);
    dpd_buf4_init(&W, PSIF_CC_TMP2, 0, 7, 11, 7, 11, 0, "CC2 Wabei (a>b,ei)");
    dpd_buf4_sort(&W, PSIF_CC2_HET1, rspq, 11, 7, "CC2 Wabei (ei,a>b)");
    dpd_buf4_close(&W);
    dpd_buf4_init(&W, PSIF_CC_TMP2, 0, 5, 11, 5, 11, 0, "CC2 WAbEi (Ab,Ei)");
    dpd_buf4_sort(&W, PSIF_CC2_HET1, rspq, 11, 5, "CC2 WAbEi (Ei,Ab)");
    dpd_buf4_close(&W);
    dpd_buf4_init(&W, PSIF_CC_TMP2, 0, 5, 11, 5, 11, 0, "CC2 WaBeI (aB,eI)");
    dpd_buf4_sort(&W, PSIF_CC2_HET1, rspq, 11, 5, "CC2 WaBeI (eI,aB)");
    dpd_buf4_close(&W);
    ...
    /* sort to Wabei (ei,ab) */
    dpd_buf4_init(&W, PSIF_CC_TMP0, 0, 7, 21, 7, 21, 0, "CC2 WABEI (A>B,EI)");
    dpd_buf4_sort(&W, PSIF_CC2_HET1, rspq, 21, 7, "CC2 WABEI (EI,A>B)");
    dpd_buf4_close(&W);
    dpd_buf4_init(&W, PSIF_CC_TMP0, 0, 17, 31, 17, 31, 0, "CC2 Wabei (a>b,ei)");
    dpd_buf4_sort(&W, PSIF_CC2_HET1, rspq, 31, 17, "CC2 Wabei (ei,a>b)");
    dpd_buf4_close(&W);
    dpd_buf4_init(&W, PSIF_CC_TMP0, 0, 28, 26, 28, 26, 0, "CC2 WAbEi (Ab,Ei)");
    dpd_buf4_sort(&W, PSIF_CC2_HET1, rspq, 26, 28, "CC2 WAbEi (Ei,Ab)");
    dpd_buf4_close(&W);
    dpd_buf4_init(&W, PSIF_CC_TMP0, 0, 29, 25, 29, 25, 0, "CC2 WaBeI (aB,eI)");
    dpd_buf4_sort(&W, PSIF_CC2_HET1, rspq, 25, 29, "CC2 WaBeI (eI,aB)");
    dpd_buf4_close(&W);
  }
...
void purge_cc2_Wabei(void) {
  ...
  dpd_file4_init(&W, PSIF_CC_TMP2, 0, 11, 7,"CC2 WABEI (EI,A>B)");
  for(h=0; h < nirreps; h++) {
  ...
  dpd_file4_init(&W, PSIF_CC_TMP2, 0, 11, 5,"CC2 WAbEi (Ei,Ab)");
  for(h=0; h < nirreps; h++) {
    dpd_file4_mat_irrep_init(&W, h);
    dpd_file4_mat_irrep_rd(&W, h);
    for(ei=0; ei<W.params->rowtot[h]; ei++) {
