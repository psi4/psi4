...
void Wamef_build(void) {
  ...
  if(params.ref == 0) { 
    ...
    dpd_->buf4_sort(&F, PSIF_CC_HBAR, qpsr, 11, 5, "WAmEf");
    dpd_->buf4_close(&F);
	...
	W.matrix[h] = dpd_->dpd_block_matrix(moinfo.occpi[Gm], W.params->coltot[h]);

	for(A=0; A < moinfo.virtpi[Ga]; A++) {
	  a = moinfo.vir_off[Ga] + A;

	  dpd_->buf4_mat_irrep_rd_block(&W, h, W.row_offset[h][a], moinfo.occpi[Gm]);
	  ...
	  dpd_->buf4_mat_irrep_wrt_block(&W, h, W.row_offset[h][a], moinfo.occpi[Gm]);
	}

	row += moinfo.occpi[Gn] * moinfo.occpi[Gm];

	dpd_->free_dpd_block(W.matrix[h], moinfo.occpi[Gm], W.params->coltot[h]);
      }
    ...
    dpd_->buf4_copy(&F, PSIF_CC_HBAR, "WAMEF");
    dpd_->buf4_copy(&F, PSIF_CC_HBAR, "Wamef");
    dpd_->buf4_close(&F);

    /* T(N,A) <NM||EF> --> W(AM,E>F) */
    dpd_->buf4_init(&WAMEF, PSIF_CC_HBAR, 0, 11, 7, 11, 7, 0, "WAMEF");
    dpd_->buf4_init(&D, PSIF_CC_DINTS, 0, 0, 7, 0, 7, 0, "D <ij||ab> (ij,a>b)");
    dpd_->contract244(&tIA, &D, &WAMEF, 0, 0, 0, -1, 1);
    dpd_->buf4_close(&D);
    ...
    /* T(N,A) <NM||EF> --> W(AM,E>F) */
    dpd_->buf4_init(&WAMEF, PSIF_CC_HBAR, 0, 21, 7, 21, 7, 0, "WAMEF");
    dpd_->buf4_init(&D, PSIF_CC_DINTS, 0, 0, 7, 0, 7, 0, "D <IJ||AB> (IJ,A>B)");
    dpd_->contract244(&tIA, &D, &WAMEF, 0, 0, 0, -1, 1);
    dpd_->buf4_close(&D);
    dpd_->buf4_close(&WAMEF);  
    ...
    /* T(n,a) <nm||ef> --> W(am,e>f) */
    dpd_->buf4_init(&Wamef, PSIF_CC_HBAR, 0, 31, 17, 31, 17, 0, "Wamef");
    ...
    /* T(N,A) <Nm|Ef> --> W(Am,Ef) */
    dpd_->buf4_init(&WAmEf, PSIF_CC_HBAR, 0, 26, 28, 26, 28, 0, "WAmEf");
    ...
    /* T(n,a) <nM|eF> --> W(aM,eF) */
    dpd_->buf4_init(&WaMeF, PSIF_CC_HBAR, 0, 25, 29, 25, 29, 0, "WaMeF");
