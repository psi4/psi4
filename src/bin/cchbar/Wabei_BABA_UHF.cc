...
void WaBeI_UHF(void)
{
  ...
  dpd_->contract244(&Fme, &T2, &W, 0, 0, 0, -1, 1);
  dpd_->buf4_close(&W);
  ...
  /** Z(Ie,Ba) <--- t_I^F <Fe|Ba> **/
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 24, 28, 24, 28, 0, "Z(Ie,Ba)");
  dpd_->buf4_init(&B, PSIF_CC_BINTS, 0, 28, 28, 28, 28, 0, "B <Ab|Cd>");
  dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tIA");
  dpd_->contract244(&T1, &B, &Z, 1, 0, 0, 1, 0);
  dpd_->file2_close(&T1);
  dpd_->buf4_close(&B);
  /** Z(Ie,Ba) --> W'(aB,eI) **/
  dpd_->buf4_sort(&Z, PSIF_CC_TMP0, srqp, 29, 25, "W'(aB,eI)");
  dpd_->buf4_close(&Z);
  ...
  **/
  ...
  /** Z(aM,eI) <-- - <aM|eF> t_I^F **/
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 25, 25, 25, 25, 0, "Z(aM,eI)");
  dpd_->buf4_init(&F, PSIF_CC_FINTS, 0, 25, 29, 25, 29, 0, "F <aI|bC>");
  dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tIA");
  ...
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 25, 25, 25, 25, 0, "Z(aM,eI)");
  dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tIA");
  dpd_->contract424(&Z, &T1, &W, 1, 0, 0, 1, 1);
  dpd_->file2_close(&T1);
  dpd_->buf4_close(&Z);
  dpd_->buf4_close(&W);

  /** Z(mB,eI) <-- <mB|eF> t_I^F **/
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 27, 25, 27, 25, 0, "Z(mB,eI)");
  ...
  dpd_->buf4_init(&W, PSIF_CC_TMP0, 0, 29, 25, 29, 25, 0, "W'(aB,eI)");
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 27, 25, 27, 25, 0, "Z(mB,eI)");
  dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 2, 3, "tia");
  dpd_->contract244(&T1, &Z, &W, 0, 0, 0, -1, 1);
  dpd_->file2_close(&T1);
  dpd_->buf4_close(&Z);
  dpd_->buf4_close(&W);
  ...
  **/
  ...
  /** Z(mN,eI) <-- <mN|eF> t_I^F **/
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 23, 25, 23, 25, 0, "Z(mN,eI)");
  ...
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 23, 25, 23, 25, 0, "Z(mN,eI)");
  dpd_->buf4_init(&W, PSIF_CC_TMP0, 0, 29, 25, 29, 25, 0, "W'(aB,eI)");
  dpd_->buf4_init(&T2, PSIF_CC_TAMPS, 0, 23, 29, 23, 29, 0, "tauiJaB");
  ...
  **/
  ...
  /** <Bm|Fe> --> F(Be,mF) **/
  dpd_->buf4_init(&F, PSIF_CC_FINTS, 0, 26, 28, 26, 28, 0, "F <Ai|Bc>");
  dpd_->buf4_sort(&F, PSIF_CC_FINTS, psqr, 28, 27, "F <Ai|Bc> (Ac,iB)");
  dpd_->buf4_close(&F);

  /** <Bm|Fe> t_Im^Fa --> Z(Be,Ia) **/
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 28, 24, 28, 24, 0, "Z(Be,Ia)");
  dpd_->buf4_init(&F, PSIF_CC_FINTS, 0, 28, 27, 28, 27, 0, "F <Ai|Bc> (Ac,iB)");
  ...
  /** Z(Be,Ia) --> W(eI,aB) **/
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 28, 24, 28, 24, 0, "Z(Be,Ia)");
  dpd_->buf4_sort_axpy(&Z, PSIF_CC_HBAR, qrsp, 25, 29, "WeIaB", 1);
  dpd_->buf4_close(&Z);

  /** Z''(ae,IB) <-- - <am||ef> t_Im^Bf **/
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 15, 20, 15, 20, 0, "Z(ae,IB)");
  ...
  /** Z''(ai,IB) --> W(eI,aB) **/
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 15, 20, 15, 20, 0, "Z(ae,IB)");
  dpd_->buf4_sort_axpy(&Z, PSIF_CC_HBAR, qrps, 25, 29, "WeIaB", 1);
  dpd_->buf4_close(&Z);
  ...
  **/
  ...
  dpd_->buf4_copy(&D, PSIF_CC_TMP0, "Z(mB,eI)");
  dpd_->buf4_close(&D);

  /** <mn||ef> t_In^Bf --> Z(me,IB) **/
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 30, 20, 30, 20, 0, "Z(me,IB)");
  ...
  /** Z(me,IB) --> Z(mB,eI) **/
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 30, 20, 30, 20, 0, "Z(me,IB)");
  dpd_->buf4_sort_axpy(&Z, PSIF_CC_TMP0, psqr, 27, 25, "Z(mB,eI)", 1);
  dpd_->buf4_close(&Z);
  ...
  dpd_->buf4_sort(&C, PSIF_CC_TMP0, qpsr, 25, 25, "Z(aM,eI)");
  dpd_->buf4_close(&C);
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 25, 25, 25, 25, 0, "Z(aM,eI)");
  dpd_->buf4_scm(&Z, -1);
  dpd_->buf4_close(&Z);
  ...
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 24, 24, 24, 24, 0, "Z(Me,Ia)");
  ...
  /** Z(Me,Ia) --> Z(aM,eI) **/
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 24, 24, 24, 24, 0, "Z(Me,Ia)");
  dpd_->buf4_sort_axpy(&Z, PSIF_CC_TMP0, spqr, 25, 25, "Z(aM,eI)", 1);
  dpd_->buf4_close(&Z);
  ...
  dpd_->buf4_init(&W, PSIF_CC_TMP0, 0, 29, 25, 29, 25, 0, "W'(aB,eI)");
  dpd_->buf4_sort_axpy(&W, PSIF_CC_HBAR, rspq, 25, 29, "WeIaB", 1);
  dpd_->buf4_close(&W);
}
