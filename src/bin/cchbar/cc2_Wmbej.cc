...
void cc2_Wmbej_build(void) {
  ...
  if(params.ref == 0) { /** RHF **/
    ...
    dpd_->buf4_scmcopy(&C, PSIF_CC_TMP0, "CC2 WMbeJ (Mb,Je)", -1);
    dpd_->buf4_close(&C);

    dpd_->buf4_init(&D, PSIF_CC_DINTS, 0, 10, 11, 10, 11, 0, "D <ij|ab> (ib,aj)");
    dpd_->buf4_copy(&D, PSIF_CC_TMP0, "CC2 WMbEj");
    dpd_->buf4_close(&D);
    ...
    dpd_->buf4_init(&WMbEj, PSIF_CC_TMP0, 0, 10, 11, 10, 11, 0, "CC2 WMbEj");
    ...
    dpd_->buf4_init(&WMbeJ, PSIF_CC_TMP0, 0, 10, 10, 10, 10, 0, "CC2 WMbeJ (Mb,Je)");
    ...
    /* Sort to (ME,JB) */
    dpd_->buf4_init(&WMbEj, PSIF_CC_TMP0, 0, 10, 11, 10, 11, 0, "CC2 WMbEj");
    dpd_->buf4_sort(&WMbEj, PSIF_CC2_HET1, prsq, 10, 10, "CC2 WMbEj (ME,jb)");
    dpd_->buf4_close(&WMbEj);

    dpd_->buf4_init(&WMbeJ, PSIF_CC_TMP0, 0, 10, 10, 10, 10, 0, "CC2 WMbeJ (Mb,Je)");
    dpd_->buf4_sort(&WMbeJ, PSIF_CC2_HET1, psrq, 10, 10, "CC2 WMbeJ (Me,Jb)");
    dpd_->buf4_close(&WMbeJ);
    ...
    dpd_->buf4_close(&D);
    dpd_->buf4_init(&W, PSIF_CC2_HET1, 0, 10, 10, 10, 10, 0, "CC2 WMbEj (ME,jb)");
    ...
    dpd_->buf4_scmcopy(&C, PSIF_CC_TMP0, "CC2 WMBEJ", -1);
    dpd_->buf4_scmcopy(&C, PSIF_CC_TMP0, "CC2 Wmbej", -1);
    dpd_->buf4_close(&C);
    ...
    dpd_->buf4_init(&WMbeJ, PSIF_CC_TMP0, 0, 10, 10, 10, 10, 0, "CC2 WMbeJ");
    ...
    dpd_->buf4_init(&WMBEJ, PSIF_CC_TMP0, 0, 10, 11, 10, 11, 0, "CC2 WMBEJ");
    dpd_->buf4_sort(&WMBEJ, PSIF_CC2_HET1, prsq, 10, 10, "CC2 WMBEJ");
    dpd_->buf4_close(&WMBEJ);
    ...
    dpd_->buf4_init(&WMbeJ, PSIF_CC_TMP0, 0, 10, 10, 10, 10, 0, "CC2 WMbeJ");
    dpd_->buf4_sort(&WMbeJ, PSIF_CC2_HET1, psrq, 10, 10, "CC2 WMbeJ");
    dpd_->buf4_close(&WMbeJ);
    ...
    dpd_->buf4_init(&W, PSIF_CC2_HET1, 0, 10, 10, 10, 10, 0, "CC2 WMBEJ");
    ...
    dpd_->buf4_init(&Wmbej, PSIF_CC2_HET1, 0, 10, 10, 10, 10, 0, "CC2 Wmbej");
    ...
    dpd_->buf4_scmcopy(&D, PSIF_CC_TMP0, "CC2 WMbEj", -1);
    dpd_->buf4_close(&D);
    ...
    dpd_->buf4_init(&W, PSIF_CC_TMP0, 0, 20, 21, 20, 21, 0, "CC2 WMBEJ");
    dpd_->buf4_init(&F, PSIF_CC_FINTS, 0, 20, 5, 20, 5, 1, "F <IA|BC>");
    dpd_->contract424(&F, &tIA, &W, 3, 1, 0, 1, 1);
    dpd_->buf4_close(&F);
    dpd_->buf4_close(&W);

    dpd_->buf4_init(&W, PSIF_CC_TMP0, 0, 30, 31, 30, 31, 0, "CC2 Wmbej");
    dpd_->buf4_init(&F, PSIF_CC_FINTS, 0, 30, 15, 30, 15, 1, "F <ia|bc>");
    dpd_->contract424(&F, &tia, &W, 3, 1, 0, 1, 1);
    dpd_->buf4_close(&F);
    dpd_->buf4_close(&W);

    dpd_->buf4_init(&W, PSIF_CC_TMP0, 0, 24, 26, 24, 26, 0, "CC2 WMbEj");
    dpd_->buf4_init(&F, PSIF_CC_FINTS, 0, 24, 28, 24, 28, 0, "F <Ia|Bc>");
    dpd_->contract424(&F, &tia, &W, 3, 1, 0, -1, 1);
    dpd_->buf4_close(&F);
    dpd_->buf4_close(&W);

    dpd_->buf4_init(&W, PSIF_CC_TMP0, 0, 27, 25, 27, 25, 0, "CC2 WmBeJ");
    ...
    dpd_->buf4_init(&W, PSIF_CC_TMP0, 0, 20, 21, 20, 21, 0, "CC2 WMBEJ");
    dpd_->contract424(&Y, &tIA, &W, 1, 0, 1, -1, 1);
    dpd_->buf4_close(&Y);
    dpd_->buf4_close(&W);
    ...
    /** Sort **/
    dpd_->buf4_init(&W, PSIF_CC_TMP0, 0, 20, 21, 20, 21, 0, "CC2 WMBEJ");
    dpd_->buf4_sort(&W, PSIF_CC2_HET1, prqs, 20, 21, "CC2 WMBEJ (ME,BJ)");
    dpd_->buf4_close(&W);

    dpd_->buf4_init(&W, PSIF_CC_TMP0, 0, 30, 31, 30, 31, 0, "CC2 Wmbej");
    dpd_->buf4_sort(&W, PSIF_CC2_HET1, prqs, 30, 31, "CC2 Wmbej (me,bj)");
    dpd_->buf4_close(&W);

    dpd_->buf4_init(&W, PSIF_CC_TMP0, 0, 24, 26, 24, 26, 0, "CC2 WMbEj");
    dpd_->buf4_sort(&W, PSIF_CC2_HET1, prqs, 20, 31, "CC2 WMbEj (ME,bj)");
    dpd_->buf4_close(&W);

    dpd_->buf4_init(&W, PSIF_CC_TMP0, 0, 27, 25, 27, 25, 0, "CC2 WmBeJ");
    dpd_->buf4_sort(&W, PSIF_CC2_HET1, prqs, 30, 21, "CC2 WmBeJ (me,BJ)");
    dpd_->buf4_close(&W);
