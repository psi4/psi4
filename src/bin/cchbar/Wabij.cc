...
void Wabij_build(void)
{
  ...
  if(params.ref == 0) { /** RHF **/
    dpd_->buf4_init(&newtIjAb, PSIF_CC_HBAR, 0, 0, 5, 0, 5, 0, "WAbIj residual");
    dotval = dpd_->buf4_dot_self(&newtIjAb);
    fprintf(outfile,"Norm squared of <Phi^IJ_AB|Hbar|0>: %20.15lf\n",dotval);
    dpd_->buf4_close(&newtIjAb);
  }
  else if (params.ref == 1) {
    dpd_->buf4_init(&newtIJAB, PSIF_CC_HBAR, 0, 2, 7, 2, 7, 0, "WABIJ residual");
    dpd_->buf4_print(&newtIJAB,outfile,1);
    dpd_->buf4_close(&newtIJAB);
    dpd_->buf4_init(&newtijab, PSIF_CC_HBAR, 0, 2, 7, 2, 7, 0, "Wabij residual");
    dpd_->buf4_print(&newtijab,outfile,1);
    dpd_->buf4_close(&newtijab);
    ...
    dpd_->buf4_print(&newtIJAB,outfile,1);
    dpd_->buf4_close(&newtIJAB);
    dpd_->buf4_init(&newtijab, PSIF_CC_HBAR, 0, 12, 17, 12, 17, 0, "Wabij residual");
    dpd_->buf4_print(&newtijab,outfile,1);
    dpd_->buf4_close(&newtijab);
    dpd_->buf4_init(&newtIjAb, PSIF_CC_HBAR, 0, 22, 28, 22, 28, 0, "WAbIj residual");
    dpd_->buf4_print(&newtIjAb,outfile,1);
    dpd_->buf4_close(&newtIjAb);
  }
}
...
void DT2(void)
{
  ...
  if(params.ref == 0) { /*** RHF ***/
    ...
    dpd_->buf4_copy(&D, PSIF_CC_HBAR, "WAbIj residual");
    dpd_->buf4_close(&D);
  }
...
void FaetT2(void)
{
  ...
  if(params.ref == 0) { /** RHF **/
    ...
    dpd_->buf4_init(&newtIJAB, PSIF_CC_HBAR, 0, 2, 5, 2, 7, 0, "WABIJ residual");
    dpd_->buf4_init(&newtijab, PSIF_CC_HBAR, 0, 2, 5, 2, 7, 0, "Wabij residual");
    dpd_->buf4_init(&newtIjAb, PSIF_CC_HBAR, 0, 0, 5, 0, 5, 0, "WAbIj residual");
    ...
    dpd_->buf4_init(&newtIJAB, PSIF_CC_HBAR, 0, 2, 5, 2, 7, 0, "WABIJ residual");
    dpd_->buf4_init(&newtijab, PSIF_CC_HBAR, 0, 12, 15, 12, 17, 0, "Wabij residual");
    dpd_->buf4_init(&newtIjAb, PSIF_CC_HBAR, 0, 22, 28, 22, 28, 0, "WAbIj residual");
    dpd_->buf4_init(&tIJAB, PSIF_CC_TAMPS, 0, 2, 5, 2, 7, 0, "tIJAB");
...
void FmitT2(void)
{
  ...
  if(params.ref == 0) { /** RHF **/
    ...
    dpd_->buf4_init(&newtIJAB, PSIF_CC_HBAR, 0, 0, 7, 2, 7, 0, "WABIJ residual");
    dpd_->buf4_init(&newtijab, PSIF_CC_HBAR, 0, 0, 7, 2, 7, 0, "Wabij residual");
    dpd_->buf4_init(&newtIjAb, PSIF_CC_HBAR, 0, 0, 5, 0, 5, 0, "WAbIj residual");
    ...
    dpd_->buf4_init(&newtIJAB, PSIF_CC_HBAR, 0, 0, 7, 2, 7, 0, "WABIJ residual");
    dpd_->buf4_init(&newtijab, PSIF_CC_HBAR, 0, 10, 17, 12, 17, 0, "Wabij residual");
    dpd_->buf4_init(&newtIjAb, PSIF_CC_HBAR, 0, 22, 28, 22, 28, 0, "WAbIj residual");
    dpd_->buf4_init(&tIJAB, PSIF_CC_TAMPS, 0, 0, 7, 2, 7, 0, "tIJAB");
...
void ZT2(void)
{
  ...
  if(params.ref == 0) { /** RHF **/
    ...
    dpd_->buf4_close(&Z); 
    dpd_->file2_close(&T1); 
    dpd_->buf4_sort_axpy(&X, PSIF_CC_HBAR, rspq, 0, 5, "WAbIj residual", 1);
    dpd_->buf4_sort_axpy(&X, PSIF_CC_HBAR, srqp, 0, 5, "WAbIj residual", 1);
    dpd_->buf4_close(&X);
  }
  else if(params.ref == 1) { /** ROHF **/
...
/*     dpd_contract444(&F, &tau, &Z, 0, 0, 1, 0); */
    for(Gmb=0; Gmb < moinfo.nirreps; Gmb++) {
      ...
      dpd_->buf4_mat_irrep_init(&tau, Gij);
      dpd_->buf4_mat_irrep_rd(&tau, Gij);
      dpd_->buf4_mat_irrep_init(&Z, Gmb);
      dpd_->buf4_mat_irrep_row_init(&F, Gmb);
      for(mb=0; mb < F.params->rowtot[Gmb]; mb++) {
	dpd_->buf4_mat_irrep_row_rd(&F, Gmb, mb);
	...
	if(nrows && ncols)
      ...
      dpd_->buf4_mat_irrep_row_close(&F, Gmb);
      dpd_->buf4_mat_irrep_wrt(&Z, Gmb);
      dpd_->buf4_mat_irrep_close(&Z, Gmb);
      dpd_->buf4_mat_irrep_close(&tau, Gij);
    }
    dpd_->buf4_close(&tau);
    dpd_->buf4_close(&F);
    dpd_->buf4_close(&Z);  
...
void FT2(void)
{
  ...
  if(params.ref == 0) { /** RHF **/
    ...
    dpd_->buf4_close(&t2);
    dpd_->buf4_sort_axpy(&X, PSIF_CC_HBAR, qpsr, 0, 5, "WAbIj residual", 1);
    dpd_->buf4_close(&X);
  }
  else if(params.ref == 1) { /** ROHF **/
    dpd_->buf4_init(&newtIJAB, PSIF_CC_HBAR, 0, 0, 7, 2, 7, 0, "WABIJ residual");
...
void ET2(void)
{
  ...
  if(params.ref == 0) { /** RHF **/
    ...
    dpd_->contract424(&E, &tIA, &newtIjAb, 1, 0, 0, -1, 1);
    dpd_->buf4_close(&E);
    dpd_->buf4_init(&E, PSIF_CC_EINTS, 0, 10, 0, 10, 0, 0, "E <ia|jk>");
    dpd_->contract244(&tIA, &E, &newtIjAb, 0, 0, 1, -1, 1);
    dpd_->buf4_close(&E);
    dpd_->file2_close(&tIA);
    dpd_->buf4_close(&newtIjAb);
  }
    ...
    dpd_->file2_init(&tia, PSIF_CC_OEI, 0, 0, 1, "tia");
    /*** AA ***/
    dpd_->buf4_init(&E, PSIF_CC_EINTS, 0, 11, 2, 11, 0, 1, "E <ai|jk>");
    dpd_->buf4_init(&t2, PSIF_CC_TMP0, 0, 2, 5, 2, 5, 0, "T (I>J,AB)");
    dpd_->contract424(&E, &tIA, &t2, 1, 0, 0, -1, 0);
    ...
    dpd_->contract424(&E, &tia, &newtIjAb, 3, 0, 0, -1, 1);
    dpd_->buf4_close(&E);
...
void WmbejT2(void)
{
  ...
  if(params.ref == 0) { /** RHF **/
    ...
    dpd_->buf4_init(&T2new, PSIF_CC_HBAR, 0, 0, 5, 0, 5, 0, "WAbIj residual");
    ...
    dpd_->buf4_init(&T2, PSIF_CC_HBAR, 0, 0, 5, 2, 7, 0, "WABIJ residual");
    ...
    dpd_->buf4_close(&T2);
    /* T2(ij,ab) --> New T2(ij,ab) */
    dpd_->buf4_init(&T2, PSIF_CC_HBAR, 0, 10, 15, 12, 17, 0, "Wabij residual");
    dpd_->buf4_axpy(&T2new, &T2, 1);
    dpd_->buf4_close(&T2);
    dpd_->buf4_close(&T2new);
    ...
    dpd_->buf4_close(&T2new);
    /* T2(Ij,Ab) (part 1) + T2(Ij,Ab) (part 2) --> New T2(Ij,Ab) */
    dpd_->buf4_init(&T2new, PSIF_CC_HBAR, 0, 22, 28, 22, 28, 0, "WAbIj residual");
...
void CT2(void)
{
  ...
  if(params.ref == 0) { /** RHF **/
    ...
    dpd_->buf4_init(&Y, PSIF_CC_TMP0, 0, 10, 0, 10, 0, 0, "Y (mA,jI)");
    dpd_->buf4_init(&C, PSIF_CC_CINTS, 0, 10, 10, 10, 10, 0, "C <ia|jb>");
    dpd_->contract424(&C, &tIA, &Y, 3, 1, 0, 1, 0);
    dpd_->buf4_close(&C);
    /* T(m,b) * Y(mA,jI) --> T2(bA,jI) */
    dpd_->buf4_init(&T2new, PSIF_CC_TMP0, 0, 5, 0, 5, 0, 0, "X(5,0)");
    dpd_->contract244(&tIA, &Y, &T2new, 0, 0, 0, 1, 0);
    dpd_->buf4_close(&Y);
    /* T(bA,jI) --> Tnew(Ij,Ab) */
    dpd_->buf4_sort(&T2new, PSIF_CC_TMP0, srqp, 0, 5, "X(0,5) 1");
    dpd_->buf4_close(&T2new);
    dpd_->buf4_init(&T2, PSIF_CC_TMP0, 0, 0, 5, 0, 5, 0, "X(0,5) 1");
    dpd_->buf4_init(&T2new, PSIF_CC_HBAR, 0, 0, 5, 0, 5, 0, "WAbIj residual");
    dpd_->buf4_axpy(&T2, &T2new, -1);
    dpd_->buf4_close(&T2);
    dpd_->buf4_close(&T2new);
    /* C(Mb|Ie) * T(j,e) --> Y(Mb,Ij) */
    dpd_->buf4_init(&Y, PSIF_CC_TMP0, 0, 10, 0, 10, 0, 0, "Y (Mb,Ij)");
    dpd_->buf4_init(&C, PSIF_CC_CINTS, 0, 10, 10, 10, 10, 0, "C <ia|jb>");
    dpd_->contract424(&C, &tIA, &Y, 3, 1, 0, 1, 0);
    dpd_->buf4_close(&C);
    ...
    dpd_->buf4_close(&Y);
    /* T(Ab,Ij) --> Tnew(Ij,Ab) */
    dpd_->buf4_sort(&T2new, PSIF_CC_TMP0, rspq, 0, 5, "X(0,5) 1");
    dpd_->buf4_close(&T2new);
    dpd_->buf4_init(&T2, PSIF_CC_TMP0, 0, 0, 5, 0, 5, 0, "X(0,5) 1");
    dpd_->buf4_init(&T2new, PSIF_CC_HBAR, 0, 0, 5, 0, 5, 0, "WAbIj residual");
    dpd_->buf4_axpy(&T2, &T2new, -1);
    ...
    dpd_->buf4_init(&Y, PSIF_CC_TMP0, 0, 10, 0, 10, 0, 0, "Y(Mb,jI)");
    dpd_->buf4_init(&D, PSIF_CC_DINTS, 0, 10, 10, 10, 10, 0, "D <ij|ab> (ib,ja)");
    dpd_->contract424(&D, &tIA, &Y, 3, 1, 0, 1, 0);
    dpd_->buf4_close(&D);
    ...
    dpd_->buf4_close(&Y);
    /* T2(Ab,jI) --> Tnew(Ij,Ab) */
    dpd_->buf4_sort(&T2new, PSIF_CC_TMP0, srpq, 0, 5, "X(0,5) 1");
    dpd_->buf4_close(&T2new);
    dpd_->buf4_init(&T2, PSIF_CC_TMP0, 0, 0, 5, 0, 5, 0, "X(0,5) 1");
    dpd_->buf4_init(&T2new, PSIF_CC_HBAR, 0, 0, 5, 0, 5, 0, "WAbIj residual");
    dpd_->buf4_axpy(&T2, &T2new, -1);
    ...
    dpd_->buf4_init(&Y, PSIF_CC_TMP0, 0, 10, 0, 10, 0, 0, "Y(mA,Ij)");
    dpd_->buf4_init(&D, PSIF_CC_DINTS, 0, 10, 10, 10, 10, 0, "D <ij|ab> (ib,ja)");
    dpd_->contract424(&D, &tIA, &Y, 3, 1, 0, 1, 0);
    dpd_->buf4_close(&D);
    ...
    dpd_->buf4_close(&Y);
    /* T2(bA,Ij) --> Tnew(Ij,Ab) */
    dpd_->buf4_sort(&T2new, PSIF_CC_TMP0, rsqp, 0, 5, "X(0,5) 1");
    dpd_->buf4_close(&T2new);
    dpd_->buf4_init(&T2, PSIF_CC_TMP0, 0, 0, 5, 0, 5, 0, "X(0,5) 1");
    dpd_->buf4_init(&T2new, PSIF_CC_HBAR, 0, 0, 5, 0, 5, 0, "WAbIj residual");
    dpd_->buf4_axpy(&T2, &T2new, -1);
    ...
    /*** AA ***/
    /* C(MB||JE) * T(I,E) --> Y(MB,JI) */
    dpd_->buf4_init(&Y, PSIF_CC_TMP0, 0, 20, 0, 20, 0, 0, "Y (MB,JI)");
    ...
    /*** BB ***/
    /* C(mb||je) * T(i,e) --> Y(mb,ji) */
    dpd_->buf4_init(&Y, PSIF_CC_TMP0, 0, 30, 10, 30, 10, 0, "Y (mb,ji)");
    dpd_->buf4_init(&C, PSIF_CC_CINTS, 0, 30, 30, 30, 30, 0, "C <ia||jb>");
    dpd_->contract424(&C, &tia, &Y, 3, 1, 0, 1, 0);
    dpd_->buf4_close(&C);
    /* T(m,a) * Y(mb,ji) --> T(ab,ji) */
    dpd_->buf4_init(&T2new, PSIF_CC_TMP0, 0, 15, 10, 15, 10, 0, "X(15,10)");
    dpd_->contract244(&tia, &Y, &T2new, 0, 0, 0, 1, 0);
    dpd_->buf4_close(&Y);
    /* T(ab,ji) --> T(ij,ab) */
    dpd_->buf4_sort(&T2new, PSIF_CC_TMP0, srpq, 10, 15, "X(10,15) 1");
    dpd_->buf4_close(&T2new);
    /* P(ij) P(ab) T2(ij,ab) */
    dpd_->buf4_init(&T2new, PSIF_CC_TMP0, 0, 10, 15, 10, 15, 0, "X(10,15) 1");
    dpd_->buf4_sort(&T2new, PSIF_CC_TMP0, qprs, 10, 15, "X(10,15) 2");
    ...
    /*** AB ***/
    /* C(mA|jE) * T(I,E) --> Y(mA,jI) */
    dpd_->buf4_init(&Y, PSIF_CC_TMP0, 0, 27, 23, 27, 23, 0, "Y (mA,jI)");
    dpd_->buf4_init(&C, PSIF_CC_CINTS, 0, 27, 27, 27, 27, 0, "C <iA|jB>");
    dpd_->contract424(&C, &tIA, &Y, 3, 1, 0, 1, 0);
    dpd_->buf4_close(&C);
    /* T(m,b) * Y(mA,jI) --> T2(bA,jI) */
    dpd_->buf4_init(&T2new, PSIF_CC_TMP0, 0, 29, 23, 29, 23, 0, "X(29,23)");
    dpd_->contract244(&tia, &Y, &T2new, 0, 0, 0, 1, 0);
    dpd_->buf4_close(&Y);
    /* T(bA,jI) --> Tnew(Ij,Ab) */
    dpd_->buf4_sort(&T2new, PSIF_CC_TMP0, srqp, 22, 28, "X(22,28) 1");
    dpd_->buf4_close(&T2new);
    dpd_->buf4_init(&T2, PSIF_CC_TMP0, 0, 22, 28, 22, 28, 0, "X(22,28) 1");
    dpd_->buf4_init(&T2new, PSIF_CC_HBAR, 0, 22, 28, 22, 28, 0, "WAbIj residual");
    dpd_->buf4_axpy(&T2, &T2new, -1);
    dpd_->buf4_close(&T2);
    dpd_->buf4_close(&T2new);
    /* C(Mb|Ie) * T(j,e) --> Y(Mb,Ij) */
    dpd_->buf4_init(&Y, PSIF_CC_TMP0, 0, 24, 22, 24, 22, 0, "Y (Mb,Ij)");
    dpd_->buf4_init(&C, PSIF_CC_CINTS, 0, 24, 24, 24, 24, 0, "C <Ia|Jb>");
    dpd_->contract424(&C, &tia, &Y, 3, 1, 0, 1, 0);
    dpd_->buf4_close(&C);
    /* T(M,A) * Y(Mb,Ij) --> T2(Ab,Ij) */
    dpd_->buf4_init(&T2new, PSIF_CC_TMP0, 0, 28, 22, 28, 22, 0, "X(28,22)");
    dpd_->contract244(&tIA, &Y, &T2new, 0, 0, 0, 1, 0);
    dpd_->buf4_close(&Y);
    /* T(Ab,Ij) --> Tnew(Ij,Ab) */
    dpd_->buf4_sort(&T2new, PSIF_CC_TMP0, rspq, 22, 28, "X(22,28) 1");
    dpd_->buf4_close(&T2new);
    dpd_->buf4_init(&T2, PSIF_CC_TMP0, 0, 22, 28, 22, 28, 0, "X(22,28) 1");
    dpd_->buf4_init(&T2new, PSIF_CC_HBAR, 0, 22, 28, 22, 28, 0, "WAbIj residual");
    dpd_->buf4_axpy(&T2, &T2new, -1);
    ...
    dpd_->buf4_close(&T2new);
    /* D(Mb,jE) * T(I,E) --> Y(Mb,jI) */
    dpd_->buf4_init(&Y, PSIF_CC_TMP0, 0, 24, 23, 24, 23, 0, "Y(Mb,jI)");
    dpd_->buf4_init(&D, PSIF_CC_DINTS, 0, 24, 27, 24, 27, 0, "D <Ij|Ab> (Ib,jA)");
    dpd_->contract424(&D, &tIA, &Y, 3, 1, 0, 1, 0);
    dpd_->buf4_close(&D);
    /* T(M,A) * Y(Mb,jI) --> T2(Ab,jI) */
    dpd_->buf4_init(&T2new, PSIF_CC_TMP0, 0, 28, 23, 28, 23, 0, "X(28,23)");
    dpd_->contract244(&tIA, &Y, &T2new, 0, 0, 0, 1, 0);
    dpd_->buf4_close(&Y);
    /* T2(Ab,jI) --> Tnew(Ij,Ab) */
    dpd_->buf4_sort(&T2new, PSIF_CC_TMP0, srpq, 22, 28, "X(22,28) 1");
    dpd_->buf4_close(&T2new);
    dpd_->buf4_init(&T2, PSIF_CC_TMP0, 0, 22, 28, 22, 28, 0, "X(22,28) 1");
    dpd_->buf4_init(&T2new, PSIF_CC_HBAR, 0, 22, 28, 22, 28, 0, "WAbIj residual");
    dpd_->buf4_axpy(&T2, &T2new, -1);
    ...
    dpd_->buf4_close(&T2new);
    /* D(mA,Ie) * T(j,e) --> Y(mA,Ij) */
    dpd_->buf4_init(&Y, PSIF_CC_TMP0, 0, 27, 22, 27, 22, 0, "Y(mA,Ij)");
    dpd_->buf4_init(&D, PSIF_CC_DINTS, 0, 27, 24, 27, 24, 0, "D <iJ|aB> (iB,Ja)");
    dpd_->contract424(&D, &tia, &Y, 3, 1, 0, 1, 0);
    dpd_->buf4_close(&D);
    /* T(m,b) * Y(mA,Ij) --> T2(bA,Ij) */
    dpd_->buf4_init(&T2new, PSIF_CC_TMP0, 0, 29, 22, 29, 22, 0, "X(29,22)");
    dpd_->contract244(&tia, &Y, &T2new, 0, 0, 0, 1, 0);
    dpd_->buf4_close(&Y);
    /* T2(bA,Ij) --> Tnew(Ij,Ab) */
    dpd_->buf4_sort(&T2new, PSIF_CC_TMP0, rsqp, 22, 28, "X(22,28) 1");
    dpd_->buf4_close(&T2new);
    dpd_->buf4_init(&T2, PSIF_CC_TMP0, 0, 22, 28, 22, 28, 0, "X(22,28) 1");
    dpd_->buf4_init(&T2new, PSIF_CC_HBAR, 0, 22, 28, 22, 28, 0, "WAbIj residual");
    dpd_->buf4_axpy(&T2, &T2new, -1);
...
void Wmbej_for_Wabij(void)
{
  ...
  if(params.ref == 0) { /** RHF **/
    ...
    dpd_->contract424(&F, &tIA, &WMbEj, 3, 1, 0, 1, 1); /* should be OOC-capable in libdpd */
    ...
    */
    ...
    dpd_->buf4_init(&F, PSIF_CC_FINTS, 0, 10, 5, 10, 5, 0, "F <ia|bc>");
    dpd_->buf4_init(&W, PSIF_CC_TMP0, 0, 10, 10, 10, 10, 0, "WMbeJ");
    dpd_->file2_mat_init(&tIA);
    dpd_->file2_mat_rd(&tIA);
    for(Gmb=0; Gmb < moinfo.nirreps; Gmb++) {
      dpd_->buf4_mat_irrep_row_init(&W, Gmb);
      dpd_->buf4_mat_irrep_row_init(&F, Gmb);
      for(mb=0; mb < F.params->rowtot[Gmb]; mb++) {
	dpd_->buf4_mat_irrep_row_rd(&W, Gmb, mb);
	dpd_->buf4_mat_irrep_row_rd(&F, Gmb, mb);
	...
	dpd_->buf4_mat_irrep_row_wrt(&W, Gmb, mb);
      }
      dpd_->buf4_mat_irrep_row_close(&F, Gmb);
      dpd_->buf4_mat_irrep_row_close(&W, Gmb);
    }
  ...
  if(params.ref == 0) { /** RHF **/
    dpd_->buf4_init(&WMbEj, PSIF_CC_TMP0, 0, 10, 11, 10, 11, 0, "WMbEj");
    dpd_->buf4_sort(&WMbEj, PSIF_CC_HBAR, prsq, 10, 10, "WMbEj");
    dpd_->buf4_close(&WMbEj);
    dpd_->buf4_init(&WMbeJ, PSIF_CC_TMP0, 0, 10, 10, 10, 10, 0, "WMbeJ");
    dpd_->buf4_sort(&WMbeJ, PSIF_CC_HBAR, psrq, 10, 10, "WMbeJ");
    dpd_->buf4_close(&WMbeJ);
  }
  else if(params.ref == 1) { /** ROHF **/
    dpd_->buf4_init(&WMBEJ, PSIF_CC_TMP0, 0, 10, 11, 10, 11, 0, "WMBEJ");
    ...
    dpd_->buf4_init(&W, PSIF_CC_TMP0, 0, 20, 21, 20, 21, 0, "WMBEJ");
    dpd_->buf4_sort(&W, PSIF_CC_HBAR, prsq, 20, 20, "WMBEJ");
    dpd_->buf4_close(&W);
    dpd_->buf4_init(&W, PSIF_CC_TMP0, 0, 30, 31, 30, 31, 0, "Wmbej");
    dpd_->buf4_sort(&W, PSIF_CC_HBAR, prsq, 30, 30, "Wmbej");
    dpd_->buf4_close(&W);
    dpd_->buf4_init(&W, PSIF_CC_TMP0, 0, 24, 26, 24, 26, 0, "WMbEj");
    dpd_->buf4_sort(&W, PSIF_CC_HBAR, prsq, 20, 30, "WMbEj");
    dpd_->buf4_close(&W);
    dpd_->buf4_init(&W, PSIF_CC_TMP0, 0, 27, 25, 27, 25, 0, "WmBeJ");
    dpd_->buf4_sort(&W, PSIF_CC_HBAR, prsq, 30, 20, "WmBeJ");
    dpd_->buf4_close(&W);
    dpd_->buf4_init(&W, PSIF_CC_TMP0, 0, 24, 24, 24, 24, 0, "WMbeJ");
    dpd_->buf4_sort(&W, PSIF_CC_HBAR, psrq, 24, 24, "WMbeJ");
    dpd_->buf4_close(&W);
    dpd_->buf4_init(&W, PSIF_CC_TMP0, 0, 27, 27, 27, 27, 0, "WmBEj");
    dpd_->buf4_sort(&W, PSIF_CC_HBAR, psrq, 27, 27, "WmBEj");
    dpd_->buf4_close(&W);
  }
  ...
  if(params.ref == 0) { /** RHF **/
    ...
    dpd_->contract444(&D, &t2, &W, 0, 0, 0.5, 1);
    dpd_->buf4_close(&D);
    dpd_->buf4_close(&t2);
    dpd_->buf4_init(&t2, PSIF_CC_TAMPS, 0, 10, 10, 10, 10, 0, "tjAIb");
...
void purge_Wabij(void) {
  ...
  dpd_->file4_init(&W, PSIF_CC_HBAR, 0, 2, 7, "WABIJ residual");
  for(h=0; h < nirreps; h++) {
  ...
  /* Purge Wabij (ij,ab) matrix elements */
  dpd_->file4_init(&W, PSIF_CC_HBAR, 0, 0, 5, "WAbIj residual");
  for(h=0; h < nirreps; h++) {
    dpd_->file4_mat_irrep_init(&W, h);
    dpd_->file4_mat_irrep_rd(&W, h);
    for(mn=0; mn < W.params->rowtot[h]; mn++) {
