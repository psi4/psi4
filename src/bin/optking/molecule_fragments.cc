...
void MOLECULE::fragmentize(void) {
  ...
  bool **connectivity = fragments[0]->g_connectivity();
...
printf("closest atoms between fragments is A %d, B %d\n", A1, B1);
    ...
    for (int iA=0; iA < nA; ++iA) {
      if (cA[iA][A1]) {
        if (v3d_angle(B[B1],A[A1],A[iA], tval)) {
          if (tval > Opt_params.collinear_lim*_pi && tval < (1-Opt_params.collinear_lim)*_pi) {
            A2 = iA;
            ++ndA;
            break;
          }
    ...
    // don't error check for missing A3's for now; a linear triatomic need not have it, e.g.
