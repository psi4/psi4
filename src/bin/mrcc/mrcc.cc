...
{
    psi::fprintf(outfile, "        %15s [ ", name.c_str());
    for (int h=0; h<dim.n(); ++h) {
        psi::fprintf(outfile, "%3d", dim[h]);
        if (h != dim.n()-1)
            psi::fprintf(outfile, ",");
    }
    psi::fprintf(outfile, "]\n");
        ...
        while ((readin = fread(batch, line_length, 1000, ccdensities_))  ) {
            ...
            for (size_t i=0; i<readin; ++i) {
                if (sscanf(batch+offset, "%lE %d %d %d %d\n", &value, &p, &q, &r, &s) != 5) {
                    ...
                    psi::fprintf(stderr, "Malformed line: %s\n", line.c_str());
...
void load_restricted(FILE* ccdensities, double tolerance, const Dimension& active_mopi, IntegralTransform& ints)
{
    ...
    /****START OF HACK****/
    psi::fprintf(outfile, "    Beginning integral transformation.\n");
    // This transforms everything (OEI and TEI)
    ints.transform_tei(MOSpace::all, MOSpace::all, MOSpace::all, MOSpace::all);
    // Use the IntegralTransform object's DPD instance, for convenience
    dpd_set_default(ints.get_dpd_id());
    psi::fprintf(outfile, "    Transformation complete.\n\n");
    /****END HACK****/
    ...
    psi::fprintf(outfile, "\tEnergies recomputed from MRCC's density matrices:\n");
...
PsiReturnType mrcc_load_ccdensities(Options& options, const boost::python::dict& level)
{
    ...
    psi::fprintf(outfile, "  PSI4 interface to MRCC:\n");
    ...
    psi::fprintf(outfile, "    Loading gradient data for %s.\n\n", fullname.c_str());
...
PsiReturnType mrcc_generate_input(Options& options, const boost::python::dict& level)
{
    ...
    psi::fprintf(outfile, "    Generating inputs for %s.\n\n", fullname.c_str());

    psi::fprintf(outfile, "    Automatically determined settings:\n");
    psi::fprintf(outfile, "        method %d\n        exlevel %d\n        fullname %s\n\n",
    ...
    if (!_default_psio_lib_->exists(PSIF_SO_TEI)) {
        psi::fprintf(outfile, "\n");
        psi::fprintf(outfile, "  WARNING: Integrals were not found on disk. Computing them now.\n");
        psi::fprintf(outfile, "           If you modified any of the integrals those modifications\n");
        psi::fprintf(outfile, "           will be lost. If you need them file an issue on GitHub:\n");
        psi::fprintf(outfile, "               https://github.com/psi4/psi4public/issues\n\n");
    ...
    psi::fprintf(outfile, "    Transformation complete.\n\n");
    psi::fprintf(outfile, "  Generating fort.55 integral file...");
    ...
    psi::fprintf(fort56, "%6d%6d%6d%6d%6d      0     0%6d     0%6d%6d     1%6d      0      0%6d     0     0    0.00    0%6lu\n",
            exlevel,                                         // # 1
            nsing,                                           // # 2
            ntrip,                                           // # 3
    ...
    psi::fprintf(fort56, "ex.lev,nsing,ntrip, rest,CC/CI, dens,conver, symm, diag,   CS,spatial, HF,ndoub, nacto, nactv,  tol,maxex, sacc,   freq,dboc,  mem\n");
