...
PsiReturnType DFCoupledCluster::triples() {
  ...
  if (memory<0){
      fprintf(outfile,"        Error: not enough memory for (t).  Try reducing number of threads.\n");
      fprintf(outfile,"\n");
      fprintf(outfile,"\n");
      fflush(outfile);
      return Failure;
  ...
  double * tempt  = (double*)malloc(o*o*v*v*sizeof(double));
  double * E2ijak = (double*)malloc(o*o*o*v*sizeof(double));
  ...
  F_DGEMM('n','t',o*v,o*v,nQ,1.0,Qov,o*v,Qov,o*v,0.0,E2klcd,o*v);

  free(Qoo);
  free(Qov);

  double ** E2abci = (double**)malloc(nthreads*sizeof(double*));
  // some v^3 intermediates
  double **Z  = (double**)malloc(nthreads*sizeof(double*));
  double **Z2 = (double**)malloc(nthreads*sizeof(double*));
