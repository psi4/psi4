...
void projections(struct L_Params *pL_params) {
  ...
  for (i=1; i<params.nstates;++i) {
    ...
    if (params.ref == 0) {
      dpd_->file2_init(&L1A, PSIF_CC_LAMPS, IRR, 0, 1, L1A_lbl);
    }
    else if (params.ref == 1) {
      dpd_->file2_init(&L1A, PSIF_CC_LAMPS, IRR, 0, 1, L1A_lbl);
      dpd_->file2_init(&L1B, PSIF_CC_LAMPS, IRR, 0, 1, L1B_lbl);
    }
    else if (params.ref == 2) {
      dpd_->file2_init(&L1A, PSIF_CC_LAMPS, IRR, 0, 1, L1A_lbl);
      dpd_->file2_init(&L1B, PSIF_CC_LAMPS, IRR, 2, 3, L1B_lbl);
    }

    if (IRR == 0) {
      /* dot_L1T1 = <0|Lme Tme|0>, assuming L0=0 (excited state) */
      if (params.ref == 0) {
        dot_L1T1 = 2.0 * dpd_->file2_dot(&L1A, &T1A);
      }
      else if (params.ref >= 1) {
        dot_L1T1 = dpd_->file2_dot(&L1A, &T1A);
        dot_L1T1 += dpd_->file2_dot(&L1B, &T1B);
      }

      /* dot_L2T2 = <0|Lmnef Tmnef|0> */
      if (params.ref == 0) {
        dpd_->buf4_init(&L2, PSIF_CC_LAMPS, IRR, 0, 5, 0, 5, 0, L2AB_lbl);
        dpd_->buf4_init(&T2, PSIF_CC_TAMPS, 0, 0, 5, 0, 5, 0, "2 tIjAb - tIjBa");
        dot_L2T2 = dpd_->buf4_dot(&L2, &T2);
        dpd_->buf4_close(&T2);
        dpd_->buf4_close(&L2);
      }
      else if (params.ref == 1) {
        dpd_->buf4_init(&L2, PSIF_CC_LAMPS, IRR, 2, 7, 2, 7, 0, L2AA_lbl);
        dpd_->buf4_init(&T2, PSIF_CC_TAMPS, 0, 2, 7, 2, 7, 0, "tIJAB");
        dot_L2T2 = dpd_->buf4_dot(&L2, &T2);
        dpd_->buf4_close(&T2);
        dpd_->buf4_close(&L2);
        dpd_->buf4_init(&L2, PSIF_CC_LAMPS, IRR, 2, 7, 2, 7, 0, L2BB_lbl);
        dpd_->buf4_init(&T2, PSIF_CC_TAMPS, 0, 2, 7, 2, 7, 0, "tijab");
        dot_L2T2 += dpd_->buf4_dot(&L2, &T2);
        dpd_->buf4_close(&T2);
        dpd_->buf4_close(&L2);
       ...
       dpd_->buf4_close(&T2);
       dpd_->buf4_close(&L2);
       dpd_->buf4_init(&L2, PSIF_CC_LAMPS, IRR, 12, 17, 12, 17, 0, L2BB_lbl);
       dpd_->buf4_init(&T2, PSIF_CC_TAMPS, 0, 12, 17, 12, 17, 0, "tijab");
       dot_L2T2 += dpd_->buf4_dot(&L2, &T2);
       dpd_->buf4_close(&T2);
       dpd_->buf4_close(&L2);
       dpd_->buf4_init(&L2, PSIF_CC_LAMPS, IRR, 22, 28, 22, 28, 0, L2AB_lbl);
     ...
     if (params.ref == 0) {
        dpd_->file2_init(&I1, PSIF_CC_TMP, IRR, 0, 1, "X(N,F)");
        dpd_->buf4_init(&L2, PSIF_CC_LAMPS, IRR, 0, 5, 0, 5, 0, L2AB_lbl2);
        dpd_->dot13(&T1A, &L2, &I1, 0, 0, 1.0, 0.0);
        dpd_->buf4_close(&L2);
        dot_L2T1T1 = 2.0 * dpd_->file2_dot(&T1A, &I1);
        dpd_->file2_close(&I1);
     }
     else if (params.ref == 1) {
        dpd_->file2_init(&I1, PSIF_CC_TMP, IRR, 0, 1, "X(N,F)");
        dpd_->buf4_init(&L2, PSIF_CC_LAMPS, IRR, 0, 5, 2, 7, 0, L2AA_lbl);
        dpd_->dot13(&T1A, &L2, &I1, 0, 0, 1.0, 0.0);
        dpd_->buf4_close(&L2);
        dot_L2T1T1 = dpd_->file2_dot(&T1A, &I1);
        dpd_->file2_close(&I1);
        dpd_->file2_init(&I1, PSIF_CC_TMP, IRR, 0, 1, "X(n,f)");
        dpd_->buf4_init(&L2, PSIF_CC_LAMPS, IRR, 0, 5, 2, 7, 0, L2BB_lbl);
        dpd_->dot13(&T1B, &L2, &I1, 0, 0, 1.0, 0.0);
        dpd_->buf4_close(&L2);
        dot_L2T1T1 += dpd_->file2_dot(&T1B, &I1);
        dpd_->file2_close(&I1);
        dpd_->file2_init(&I1, PSIF_CC_TMP, IRR, 0, 1, "X(n,f)");
        dpd_->buf4_init(&L2, PSIF_CC_LAMPS, IRR, 0, 5, 0, 5, 0, L2AB_lbl);
        dpd_->dot13(&T1A, &L2, &I1, 0, 0, 1.0, 0.0);
        dpd_->buf4_close(&L2);
        dot_L2T1T1 += 2.0 * dpd_->file2_dot(&T1B, &I1);
        dpd_->file2_close(&I1);
      }
      else if (params.ref == 2) {
        dpd_->file2_init(&I1, PSIF_CC_TMP, IRR, 0, 1, "X(N,F)");
        ...
        dot_L2T1T1 = dpd_->file2_dot(&T1A, &I1);
        dpd_->file2_close(&I1);
        dpd_->file2_init(&I1, PSIF_CC_TMP, IRR, 2, 3, "X(n,f)");
        dpd_->buf4_init(&L2, PSIF_CC_LAMPS, IRR, 10, 15, 12, 17, 0, L2BB_lbl);
        dpd_->dot13(&T1B, &L2, &I1, 0, 0, 1.0, 0.0);
        dpd_->buf4_close(&L2);
        dot_L2T1T1 += dpd_->file2_dot(&T1B, &I1);
        dpd_->file2_close(&I1);
    ...
    /* open R files */
    if (params.ref == 0) {
      dpd_->file2_init(&R1A, PSIF_CC_RAMPS, IRR, 0, 1, R1A_lbl);
    }
    else if (params.ref == 1) {
      dpd_->file2_init(&R1A, PSIF_CC_RAMPS, IRR, 0, 1, R1A_lbl);
      dpd_->file2_init(&R1B, PSIF_CC_RAMPS, IRR, 0, 1, R1B_lbl);
    }
    else if (params.ref == 2) {
      dpd_->file2_init(&R1A, PSIF_CC_RAMPS, IRR, 0, 1, R1A_lbl);
      dpd_->file2_init(&R1B, PSIF_CC_RAMPS, IRR, 2, 3, R1B_lbl);
    }

    /* dot_L1R1 = Lme Rme */
    if (params.ref == 0) {
      dot_L1R1 = 2.0 * dpd_->file2_dot(&L1A,&R1A);
    }
    else if (params.ref >= 1) {
      dot_L1R1 = dpd_->file2_dot(&L1A,&R1A);
      dot_L1R1 += dpd_->file2_dot(&L1B,&R1B);
    }

    /* dot_L2R2 = <0|Lmnef Rmnef|0> */
    if (params.ref == 0) {
      dpd_->buf4_init(&L2, PSIF_CC_LAMPS, IRR, 0, 5, 0, 5, 0, L2AB_lbl2);
      dpd_->buf4_init(&R2, PSIF_CC_RAMPS, IRR, 0, 5, 0, 5, 0, R2AB_lbl);
      dot_L2R2 = dpd_->buf4_dot(&L2, &R2);
      dpd_->buf4_close(&R2);
      dpd_->buf4_close(&L2);
    }
    else if (params.ref == 1) {
      dpd_->buf4_init(&L2, PSIF_CC_LAMPS, IRR, 2, 7, 2, 7, 0, L2AA_lbl);
      dpd_->buf4_init(&R2, PSIF_CC_RAMPS, IRR, 2, 7, 2, 7, 0, R2AA_lbl);
      dot_L2R2 = dpd_->buf4_dot(&L2, &R2);
      dpd_->buf4_close(&R2);
      dpd_->buf4_close(&L2);
      dpd_->buf4_init(&L2, PSIF_CC_LAMPS, IRR, 2, 7, 2, 7, 0, L2BB_lbl);
      dpd_->buf4_init(&R2, PSIF_CC_RAMPS, IRR, 2, 7, 2, 7, 0, R2BB_lbl);
      dot_L2R2 += dpd_->buf4_dot(&L2, &R2);
      dpd_->buf4_close(&R2);
      dpd_->buf4_close(&L2);
      dpd_->buf4_init(&L2, PSIF_CC_LAMPS, IRR, 0, 5, 0, 5, 0, L2AB_lbl);
      ...
      dpd_->buf4_close(&L2);
      dpd_->buf4_init(&L2, PSIF_CC_LAMPS, IRR, 12, 17, 12, 17, 0, L2BB_lbl);
      dpd_->buf4_init(&R2, PSIF_CC_RAMPS, IRR, 12, 17, 12, 17, 0, R2BB_lbl);
      dot_L2R2 += dpd_->buf4_dot(&L2, &R2);
      dpd_->buf4_close(&R2);
      dpd_->buf4_close(&L2);
      dpd_->buf4_init(&L2, PSIF_CC_LAMPS, IRR, 22, 28, 22, 28, 0, L2AB_lbl);
      dpd_->buf4_init(&R2, PSIF_CC_RAMPS, IRR, 22, 28, 22, 28, 0, R2AB_lbl);
      dot_L2R2 += dpd_->buf4_dot(&L2, &R2);
      dpd_->buf4_close(&R2);
      dpd_->buf4_close(&L2);
    }
    ...
    if (params.ref == 0) {
      dpd_->buf4_init(&L2, PSIF_CC_LAMPS, IRR, 0, 5, 0, 5, 0, L2AB_lbl2);
      dpd_->file2_init(&I1, PSIF_CC_TMP, IRR, 0, 1, "X2(N,F)");
      dpd_->dot13(&T1A, &L2, &I1, 0, 0, 1.0, 0.0);
      dpd_->buf4_close(&L2);
      dot_L2T1R1 = 2.0 * dpd_->file2_dot(&R1A, &I1);
      dpd_->file2_close(&I1);
    }
    else if (params.ref == 1) {
      dpd_->file2_init(&I1, PSIF_CC_TMP, IRR, 0, 1, "X2(N,F)");
      ...
      dpd_->dot13(&T1A, &L2, &I1, 0, 0, 1.0, 0.0);
      dpd_->buf4_close(&L2);
      dot_L2T1R1 = dpd_->file2_dot(&R1A, &I1);
      dpd_->file2_close(&I1);
      dpd_->file2_init(&I1, PSIF_CC_TMP, IRR, 0, 1, "X2(n,f)");
      dpd_->buf4_init(&L2, PSIF_CC_LAMPS, IRR, 0, 5, 2, 7, 0, L2BB_lbl);
      dpd_->dot13(&T1B, &L2, &I1, 0, 0, 1.0, 0.0);
      dpd_->buf4_close(&L2);
      dot_L2T1R1 += dpd_->file2_dot(&R1B, &I1);
      dpd_->file2_close(&I1);
      dpd_->buf4_init(&L2, PSIF_CC_LAMPS, IRR, 0, 5, 0, 5, 0, L2AB_lbl);
      dpd_->file2_init(&I1, PSIF_CC_TMP, IRR, 0, 1, "X2(N,F)");
      dpd_->dot24(&T1B, &L2, &I1, 0, 0, 1.0, 0.0);
      dpd_->buf4_close(&L2);
      dot_L2T1R1 += dpd_->file2_dot(&R1A, &I1);
      dpd_->file2_close(&I1);
      dpd_->file2_init(&I1, PSIF_CC_TMP, IRR, 0, 1, "X2(n,f)");
      ...
      dot_L2T1R1 = dpd_->file2_dot(&R1A, &I1);
      dpd_->file2_close(&I1);
      dpd_->file2_init(&I1, PSIF_CC_TMP, IRR, 2, 3, "X2(n,f)");
      dpd_->buf4_init(&L2, PSIF_CC_LAMPS, IRR, 10, 15, 12, 17, 0, L2BB_lbl);
      dpd_->dot13(&T1B, &L2, &I1, 0, 0, 1.0, 0.0);
      dpd_->buf4_close(&L2);
      dot_L2T1R1 += dpd_->file2_dot(&R1B, &I1);
    ...
    /* close L1, R1 */
    if (params.ref == 0) {
      dpd_->file2_close(&R1A); dpd_->file2_close(&L1A);
    }
    else if (params.ref >= 1) {
      dpd_->file2_close(&R1A); dpd_->file2_close(&R1B);
      dpd_->file2_close(&L1A); dpd_->file2_close(&L1B);
    }
