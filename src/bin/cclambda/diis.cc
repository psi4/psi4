...
void diis(int iter, int L_irr)
{
  ...
  if(params.ref == 0) { /** RHF **/
    ...
    word=0;
    dpd_->file2_init(&L1a, PSIF_CC_LAMBDA, L_irr, 0, 1, "New LIA");
    /*dpd_file2_print(&L1a,outfile);*/
    dpd_->file2_mat_init(&L1a);
    dpd_->file2_mat_rd(&L1a);
    dpd_->file2_init(&L1b, PSIF_CC_LAMBDA, L_irr, 0, 1, "LIA");
    ...
    dpd_->buf4_init(&L2a, PSIF_CC_LAMBDA, L_irr, 0, 5, 0, 5, 0, "New LIjAb");
    /*dpd_buf4_print(&L2a,outfile,1);*/
    dpd_->buf4_init(&L2b, PSIF_CC_LAMBDA, L_irr, 0, 5, 0, 5, 0, "LIjAb");
    /*dpd_buf4_print(&L2b,outfile,1);*/
    for(h=0; h < nirreps; h++) {
      dpd_->buf4_mat_irrep_init(&L2a, h);
      dpd_->buf4_mat_irrep_rd(&L2a, h);
      dpd_->buf4_mat_irrep_init(&L2b, h);
      dpd_->buf4_mat_irrep_rd(&L2b, h);
...
/* fprintf(outfile,"%15.10lf\n", error[0][word-1]); */
      ...
      dpd_->buf4_mat_irrep_close(&L2a, h);
      dpd_->buf4_mat_irrep_close(&L2b, h);
    }
    dpd_->buf4_close(&L2a);
    dpd_->buf4_close(&L2b);
    ...
    */
    ...
    dpd_->file2_mat_wrt(&L1a);
    dpd_->file2_mat_close(&L1a);
    dpd_->file2_copy(&L1a, PSIF_CC_LAMBDA, "New Lia"); /* to be removed after spin-adaptation */
    /*dpd_file2_print(&L1a,outfile);*/
    dpd_->file2_close(&L1a);
    ...
    for(h=0; h < nirreps; h++) {
      ...
      dpd_->buf4_mat_irrep_wrt(&L2a, h);
      dpd_->buf4_mat_irrep_close(&L2a, h);
    }
    dpd_->buf4_close(&L2a);

    /* to be removed after spin-adaptation */
    dpd_->buf4_init(&L2a, PSIF_CC_LAMBDA, L_irr, 2, 7, 0, 5, 1, "New LIjAb");
    dpd_->buf4_copy(&L2a, PSIF_CC_LAMBDA, "New LIJAB");
    dpd_->buf4_copy(&L2a, PSIF_CC_LAMBDA, "New Lijab");
    dpd_->buf4_close(&L2a);
    ...
    dpd_->file2_init(&L1, PSIF_CC_LAMBDA, L_irr, 0, 1, "LIA");
    dpd_->buf4_init(&L2a, PSIF_CC_LAMBDA, L_irr, 2, 7, 2, 7, 0, "LIJAB");
    dpd_->buf4_init(&L2b, PSIF_CC_LAMBDA, L_irr, 0, 5, 0, 5, 0, "LIjAb");
    for(h=0; h < nirreps; h++) {
    ...
    dpd_->buf4_init(&L2a, PSIF_CC_LAMBDA, L_irr, 2, 7, 2, 7, 0, "New LIJAB");
    dpd_->buf4_init(&L2b, PSIF_CC_LAMBDA, L_irr, 2, 7, 2, 7, 0, "LIJAB");
    for(h=0; h < nirreps; h++) {
      dpd_->buf4_mat_irrep_init(&L2a, h);
      dpd_->buf4_mat_irrep_rd(&L2a, h);
      dpd_->buf4_mat_irrep_init(&L2b, h);
    ...
    /* Compute the length of a single error vector */
    dpd_->file2_init(&L1a, PSIF_CC_TMP0, L_irr, 0, 1, "LIA");
    dpd_->file2_init(&L1b, PSIF_CC_TMP0, L_irr, 2, 3, "Lia");
    dpd_->buf4_init(&L2a, PSIF_CC_TMP0, L_irr, 2, 7, 2, 7, 0, "LIJAB");
    dpd_->buf4_init(&L2b, PSIF_CC_TMP0, L_irr, 12, 17, 12, 17, 0, "Lijab");
    dpd_->buf4_init(&L2c, PSIF_CC_TMP0, L_irr, 22, 28, 22, 28, 0, "LIjAb");
    for(h=0; h < nirreps; h++) {
    ...
    dpd_->file2_init(&L1a, PSIF_CC_LAMBDA, L_irr, 2, 3, "New Lia");
    dpd_->file2_mat_init(&L1a);
    dpd_->file2_mat_rd(&L1a);
    dpd_->file2_init(&L1b, PSIF_CC_LAMBDA, L_irr, 2, 3, "Lia");
    dpd_->file2_mat_init(&L1b);
    dpd_->file2_mat_rd(&L1b);
    for(h=0; h < nirreps; h++)
      for(row=0; row < L1a.params->rowtot[h]; row++)
    ...
    dpd_->buf4_init(&L2a, PSIF_CC_LAMBDA, L_irr, 12, 17, 12, 17, 0, "New Lijab");
    dpd_->buf4_init(&L2b, PSIF_CC_LAMBDA, L_irr, 12, 17, 12, 17, 0, "Lijab");
    for(h=0; h < nirreps; h++) {
      dpd_->buf4_mat_irrep_init(&L2a, h);
      dpd_->buf4_mat_irrep_rd(&L2a, h);
      dpd_->buf4_mat_irrep_init(&L2b, h);
    ...
    dpd_->buf4_init(&L2a, PSIF_CC_LAMBDA, L_irr, 22, 28, 22, 28, 0, "New LIjAb");
    dpd_->buf4_init(&L2b, PSIF_CC_LAMBDA, L_irr, 22, 28, 22, 28, 0, "LIjAb");
    for(h=0; h < nirreps; h++) {
      dpd_->buf4_mat_irrep_init(&L2a, h);
      dpd_->buf4_mat_irrep_rd(&L2a, h);
      dpd_->buf4_mat_irrep_init(&L2b, h);
