...
int converged(int L_irr)
{
  ...
  dpd_->file2_init(&L1old, PSIF_CC_LAMBDA, L_irr, 0, 1, "LIA");
  dpd_->file2_mat_init(&L1old);
  dpd_->file2_mat_rd(&L1old);
  ...
  dpd_->file2_close(&L1);
  dpd_->file2_mat_close(&L1old);
  dpd_->file2_close(&L1old);
  ...
  if(params.ref == 1) { /** ROHF **/
    ...
    dpd_->file2_mat_init(&L1);
    dpd_->file2_mat_rd(&L1);
    dpd_->file2_init(&L1old, PSIF_CC_LAMBDA, L_irr, 2, 3, "Lia");
    dpd_->file2_mat_init(&L1old);
    dpd_->file2_mat_rd(&L1old);
  ...
  if(params.ref == 1 || params.ref == 2) { 
    ...
    dpd_->buf4_init(&L2old, PSIF_CC_LAMBDA, L_irr, 2, 7, 2, 7, 0, "LIJAB");
    for(h=0; h < nirreps; h++) {
      dpd_->buf4_mat_irrep_init(&L2, h);
      dpd_->buf4_mat_irrep_rd(&L2, h);
      dpd_->buf4_mat_irrep_init(&L2old, h);
      dpd_->buf4_mat_irrep_rd(&L2old, h);
	...
	for(col=0; col < L2.params->coltot[h^L_irr]; col++)
      ...
      dpd_->buf4_mat_irrep_close(&L2old, h);
    }
    dpd_->buf4_close(&L2old);
    dpd_->buf4_close(&L2);
  }
  ...
  if(params.ref == 1) { /** ROHF **/
    ...
    dpd_->buf4_init(&L2old, PSIF_CC_LAMBDA, L_irr, 12, 17, 12, 17, 0, "Lijab");
  }
  ...
  if(params.ref == 0 || params.ref == 1) { /** RHF/ROHF **/
    ...
    dpd_->buf4_init(&L2old, PSIF_CC_LAMBDA, L_irr, 0, 5, 0, 5, 0, "LIjAb");
  }
  else if(params.ref == 2) { /** UHF **/
    dpd_->buf4_init(&L2, PSIF_CC_LAMBDA, L_irr, 22, 28, 22, 28, 0, "New LIjAb");
    dpd_->buf4_init(&L2old, PSIF_CC_LAMBDA, L_irr, 22, 28, 22, 28, 0, "LIjAb");
  }
