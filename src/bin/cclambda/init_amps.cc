...
void init_amps(struct L_Params L_params)
{
  ...
  if (params.zeta) {
    if (params.ref == 0) { /* RHF */
      dpd_file2_init(&XIA, PSIF_EOM_XI, L_irr, 0, 1, "XIA");
      dpd_file2_copy(&XIA, PSIF_CC_LAMBDA, "LIA");
      dpd_file2_close(&XIA);
      ...
      dpd_buf4_init(&XIjAb, PSIF_EOM_XI, L_irr, 0, 5, 0, 5, 0, "XIjAb");
      dpd_buf4_copy(&XIjAb, PSIF_CC_LAMBDA, "LIjAb");
      ...
      dpd_buf4_init(&XIJAB, PSIF_EOM_XI, L_irr, 2, 7, 2, 7, 0, "XIJAB");
      dpd_buf4_copy(&XIJAB, PSIF_CC_LAMBDA, "LIJAB");
      dpd_buf4_close(&XIJAB);
      dpd_buf4_init(&LIJAB, PSIF_CC_LAMBDA, L_irr, 2, 7, 2, 7, 0, "LIJAB");
      dpd_buf4_init(&dIJAB, PSIF_CC_DENOM, L_irr, 2, 7, 2, 7, 0, "dIJAB");
      dpd_buf4_dirprd(&dIJAB, &LIJAB);
      dpd_buf4_close(&dIJAB);
      dpd_buf4_close(&LIJAB);
      ...
      dpd_file2_init(&Xia, PSIF_EOM_XI, L_irr, 2, 3, "Xia");
      dpd_file2_copy(&Xia, PSIF_CC_LAMBDA, "Lia");
      dpd_file2_close(&Xia);
      ...
      dpd_buf4_init(&Xijab, PSIF_EOM_XI, L_irr, 12, 17, 12, 17, 0, "Xijab");
      dpd_buf4_copy(&Xijab, PSIF_CC_LAMBDA, "Lijab");
      dpd_buf4_close(&Xijab);
      dpd_buf4_init(&Lijab, PSIF_CC_LAMBDA, L_irr, 12, 17, 12, 17, 0, "Lijab");
      dpd_buf4_init(&dijab, PSIF_CC_DENOM, L_irr, 12, 17, 12, 17, 0, "dijab");
      dpd_buf4_dirprd(&dijab, &Lijab);
      dpd_buf4_close(&dijab);
      dpd_buf4_close(&Lijab);

      dpd_buf4_init(&XIjAb, PSIF_EOM_XI, L_irr, 22, 28, 22, 28, 0, "XIjAb");
      dpd_buf4_copy(&XIjAb, PSIF_CC_LAMBDA, "LIjAb");
      dpd_buf4_close(&XIjAb);
      dpd_buf4_init(&LIjAb, PSIF_CC_LAMBDA, L_irr, 22, 28, 22, 28, 0, "LIjAb");
      dpd_buf4_init(&dIjAb, PSIF_CC_DENOM, L_irr, 22, 28, 22, 28, 0, "dIjAb");
      dpd_buf4_dirprd(&dIjAb, &LIjAb);
      dpd_buf4_close(&dIjAb);
      dpd_buf4_close(&LIjAb);
    }
