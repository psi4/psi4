...
void cc3_t3z_RHF_AAA(void)
{
  ...
  dpd_->buf4_init(&ZIFLN, PSIF_CC3_MISC, 0, 10, 0, 10, 0, 0, "CC3 ZIFLN");
  for(h=0; h < nirreps; h++) {
    dpd_->buf4_mat_irrep_init(&ZIFLN, h);
  }

  dpd_->buf4_init(&ZDFAN, PSIF_CC3_MISC, 0, 10, 5, 10, 5, 0, "CC3 ZDFAN (NA,FD)");
  dpd_->buf4_scm(&ZDFAN, 0.0);
  ...
  dpd_->buf4_init(&F, PSIF_CC3_HET1, 0, 10, 5, 10, 7, 0, "CC3 WABEI (IE,B>A)");
  dpd_->buf4_init(&E, PSIF_CC3_HET1, 0, 0, 10, 2, 10, 0, "CC3 WMBIJ (I>J,MB)");
  ...
  for(Gi=0; Gi < nirreps; Gi++) {
	...
	for(Gb=0; Gb < nirreps; Gb++) {
	  ...
	  W2[Gb] = dpd_->dpd_block_matrix(virtpi[Gb], F.params->coltot[Gca]);
	}
	...
	for(i=0; i < occpi[Gi]; i++) {
	  ...
	  for(j=0; j < occpi[Gj]; j++) {
	    ...
	    for(k=0; k < occpi[Gk]; k++) {
	      ...
	      dpd_->T3_AAA(W1, nirreps, I, Gi, J, Gj, K, Gk, &T2, &F, &E, &fIJ, &fAB, 
		...
		ZDFAN.matrix[Gke] = dpd_->dpd_block_matrix(nrows, ncols);
		dpd_->buf4_mat_irrep_rd_block(&ZDFAN, Gke, ke, nrows);

		if(nrows && ncols && nlinks)
		  C_DGEMM('n', 'n', nrows, ncols, nlinks, -0.5, &(Dints.matrix[Gij][ij][eb]), nlinks,
			  W2[Gb][0], ncols, 1.0, ZDFAN.matrix[Gke][0], ncols);

		dpd_->buf4_mat_irrep_wrt_block(&ZDFAN, Gke, ke, nrows);
		dpd_->free_dpd_block(ZDFAN.matrix[Gke], nrows, ncols);
	      }
	...
	for(Gb=0; Gb < nirreps; Gb++) {
	  Gca = Gb ^ Gijk; /* totally symmtric */
	  dpd_->free_dpd_block(W2[Gb], virtpi[Gb], F.params->coltot[Gca]);
	}
  ...
  for(h=0; h < nirreps; h++) {
    dpd_->buf4_mat_irrep_wrt(&ZIFLN, h);
    dpd_->buf4_mat_irrep_close(&ZIFLN, h);
  }
  dpd_->buf4_close(&ZIFLN);

  dpd_->buf4_close(&ZDFAN);

  for(h=0; h < nirreps; h++) dpd_->buf4_mat_irrep_close(&Dints, h);
...
void cc3_t3z_RHF_AAB(void)
{
  ...
  for(h=0; h < nirreps; h++) {
    dpd_->buf4_mat_irrep_init(&ZIFLN, h);
    dpd_->buf4_mat_irrep_rd(&ZIFLN, h);
  ...
  dpd_->buf4_init(&T2BA, PSIF_CC_TAMPS, 0, 0, 5, 0, 5, 0, "tiJaB");
  dpd_->buf4_init(&FAA, PSIF_CC3_HET1, 0, 10, 5, 10, 7, 0, "CC3 WABEI (IE,B>A)");
  dpd_->buf4_init(&FAB, PSIF_CC3_HET1, 0, 10, 5, 10, 5, 0, "CC3 WaBeI (Ie,Ba)");
  dpd_->buf4_init(&FBA, PSIF_CC3_HET1, 0, 10, 5, 10, 5, 0, "CC3 WAbEi (iE,bA)");
  dpd_->buf4_init(&EAA, PSIF_CC3_HET1, 0, 0, 10, 2, 10, 0, "CC3 WMBIJ (I>J,MB)");
  dpd_->buf4_init(&EAB, PSIF_CC3_HET1, 0, 0, 10, 0, 10, 0, "CC3 WMbIj (Ij,Mb)");
  dpd_->buf4_init(&EBA, PSIF_CC3_HET1, 0, 0, 10, 0, 10, 0, "CC3 WmBiJ (iJ,mB)");
  ...
  for(Gi=0; Gi < nirreps; Gi++) {
	...
	for(Ga=0; Ga < nirreps; Ga++) {
	  ...
	  W2[Ga] = dpd_->dpd_block_matrix(virtpi[Ga], FAA.params->coltot[Gbc]);
	}
	...
	for(i=0; i < occpi[Gi]; i++) {
	  ...
	  for(j=0; j < occpi[Gj]; j++) {
	    ...
	    for(k=0; k < occpi[Gk]; k++) {
	      ...
	      dpd_->T3_AAB(W1, nirreps, I, Gi, J, Gj, K, Gk, &T2AA, &T2AB, &T2BA, 
		...
		ZDFAN.matrix[Gje] = dpd_->dpd_block_matrix(nrows, ncols);
		dpd_->buf4_mat_irrep_rd_block(&ZDFAN, Gje, je, nrows);

		if(nrows && ncols && nlinks)
		  C_DGEMM('n', 'n', nrows, ncols, nlinks, -1.0, &(Dints.matrix[Gik][ik][ec]), nlinks,
			  W2[Gc][0], ncols, 1.0, ZDFAN.matrix[Gje][0], ncols);

		dpd_->buf4_mat_irrep_wrt_block(&ZDFAN, Gje, je, nrows);
		dpd_->free_dpd_block(ZDFAN.matrix[Gje], nrows, ncols);
	      }
	...
	for(Ga=0; Ga < nirreps; Ga++) {
	  Gbc = Ga ^ Gijk; /* totally symmtric */
	  dpd_->free_dpd_block(W2[Ga], virtpi[Ga], FAA.params->coltot[Gbc]);
	}
  ...
  dpd_->buf4_sort(&ZDFAN, PSIF_CC3_MISC, qpsr, 11, 5, "CC3 ZDFAN (AN,DF)");
  dpd_->buf4_close(&ZDFAN);
  dpd_->buf4_sort(&ZDfAn, PSIF_CC3_MISC, qpsr, 11, 5, "CC3 ZDfAn (An,Df)");
  dpd_->buf4_close(&ZDfAn);
