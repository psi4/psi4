...
void cc2_Gai_build(int L_irr) {
  ...
  dpdbuf4 LIJAB, Lijab, LIjAb, LiJaB;
    ...
    if(params.ref == 0) {
      ...
      dpd_file2_init(&G, PSIF_CC_TMP0, L_irr, 1, 0, "GAI");
      dpd_file2_init(&G, PSIF_CC_TMP0, L_irr, 4, 3, "Gai");
      ...
      dpd_file2_init(&GAI, PSIF_CC_TMP0, L_irr, 1, 0, "CC2 GAI");
      dpd_file2_init(&Gai, PSIF_CC_TMP0, L_irr, 3, 2, "CC2 Gai");

      /** AA **/
      dpd_buf4_init(&tIJAB, PSIF_CC_TAMPS, 0, 20, 20, 20, 20, 0, "tIAJB");
      dpd_file2_init(&LIA, PSIF_CC_LAMBDA, L_irr, 0, 1, "LIA");
      dpd_contract422(&tIJAB, &LIA, &GAI, 0, 1, 1, 0);
      dpd_file2_close(&LIA);
      dpd_buf4_close(&tIJAB);

      dpd_buf4_init(&tIjAb, PSIF_CC_TAMPS, 0, 20, 30, 20, 30, 0, "tIAjb");
      ...
      /** BB **/
      dpd_buf4_init(&tijab, PSIF_CC_TAMPS, 0, 30, 30, 30, 30, 0, "tiajb");
      dpd_file2_init(&Lia, PSIF_CC_LAMBDA, L_irr, 2, 3, "Lia");
      dpd_contract422(&tijab, &Lia, &Gai, 0, 1, 1, 0);
      dpd_file2_close(&Lia);
      dpd_buf4_close(&tijab);

      dpd_buf4_init(&tiJaB, PSIF_CC_TAMPS, 0, 30, 20, 30, 20, 0, "tiaJB");
      dpd_file2_init(&LIA, PSIF_CC_LAMBDA, L_irr, 0, 1, "LIA");
      dpd_contract422(&tiJaB, &LIA, &Gai, 0, 1, 1, 1);
      dpd_file2_close(&LIA);
      dpd_buf4_close(&tiJaB);
