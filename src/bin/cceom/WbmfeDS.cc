...
/*     dpd_file2_close(&CME); */
    ...
    dpd_->file2_init(&XBF, PSIF_EOM_TMP, C_irr, 1, 1, "XBF");
    dpd_->file2_scm(&XBF, 0.0);
    dpd_->file2_mat_init(&XBF);
    dpd_->file2_mat_rd(&XBF);
    dpd_->file2_init(&C, PSIF_EOM_CME, C_irr, 0, 1, CME_lbl);
    dpd_->file2_mat_init(&C);
    dpd_->file2_mat_rd(&C);
    ...
    dpd_->buf4_close(&W);
    dpd_->file2_close(&C);
    dpd_->file2_mat_wrt(&XBF);
    dpd_->file2_mat_close(&XBF);

    dpd_->buf4_init(&Z, PSIF_EOM_TMP, C_irr, 0, 5, 0, 5, 0, "WbmfeDS Z(Ij,Ab)");
    dpd_->buf4_init(&TIjAb, PSIF_CC_TAMPS, H_IRR, 0, 5, 0, 5, 0, "tIjAb");
    dpd_->contract424(&TIjAb, &XBF, &Z, 3, 1, 0, 1.0, 0.0);
    ...
    dpd_->buf4_init(&WAMEF, PSIF_CC_HBAR, H_IRR, 11, 5, 11, 7, 0, "WAMEF");
    dpd_->dot24(&CME, &WAMEF, &XBF, 0, 0, 1.0, 1.0);
    dpd_->buf4_close(&WAMEF);
    dpd_->file2_close(&CME);
    dpd_->file2_init(&Cme, PSIF_EOM_Cme, C_irr, 0, 1, Cme_lbl);
    dpd_->buf4_init(&WAmEf, PSIF_CC_HBAR, H_IRR, 11, 5, 11, 5, 0, "WAmEf");
    dpd_->dot24(&Cme, &WAmEf, &XBF, 0, 0, 1.0, 1.0);
    dpd_->buf4_close(&WAmEf);
    dpd_->file2_close(&Cme);
    dpd_->file2_close(&XBF);
    ...
    dpd_->buf4_init(&WP, PSIF_EOM_TMP, C_irr, 2, 5, 2, 5, 0, "WbmfeDS_P");
    dpd_->file2_init(&XBF, PSIF_EOM_TMP, C_irr, 1, 1, "XBF");
    dpd_->buf4_init(&TIJAB, PSIF_CC_TAMPS, H_IRR, 2, 5, 2, 7, 0, "tIJAB");
    dpd_->contract424(&TIJAB, &XBF, &WP, 3, 1, 0, 1.0, 0.0);
    dpd_->buf4_close(&TIJAB);
    dpd_->file2_close(&XBF);
    dpd_->buf4_sort(&WP, PSIF_EOM_TMP, pqsr, 2, 5, "WbmfeDS_M"); 
    dpd_->buf4_init(&SIJAB, PSIF_EOM_SIJAB, C_irr, 2, 5, 2, 7, 0, SIJAB_lbl);
    dpd_->buf4_axpy(&WP, &SIJAB, 1.0);
    dpd_->buf4_close(&WP);
    dpd_->buf4_init(&WM, PSIF_EOM_TMP, C_irr, 2, 5, 2, 5, 0, "WbmfeDS_M");
    ...
    dpd_->buf4_init(&TIjAb, PSIF_CC_TAMPS, H_IRR, 0, 5, 0, 5, 0, "tIjAb");
    dpd_->file2_init(&Xbf, PSIF_EOM_TMP, C_irr, 1, 1, "Xbf");
    dpd_->contract424(&TIjAb, &Xbf, &SIjAb, 3, 1, 0, 1.0, 1.0);
    dpd_->file2_close(&Xbf);
    dpd_->file2_init(&XBF, PSIF_EOM_TMP, C_irr, 1, 1, "XBF");
    dpd_->contract244(&XBF, &TIjAb, &SIjAb, 1, 2, 1, 1.0, 1.0);
    dpd_->file2_close(&XBF);
    dpd_->buf4_close(&TIjAb);
    dpd_->buf4_close(&SIjAb);
  }
    ...
    dpd_->file2_scm(&XBF, 0.0);
    dpd_->file2_init(&CME, PSIF_EOM_CME, C_irr, 0, 1, CME_lbl);
    dpd_->buf4_init(&WAMEF, PSIF_CC_HBAR, H_IRR, 21, 5, 21, 7, 0, "WAMEF");
    dpd_->dot24(&CME, &WAMEF, &XBF, 0, 0, 1.0, 1.0);
    dpd_->buf4_close(&WAMEF);
    dpd_->file2_close(&CME);
    dpd_->file2_init(&Cme, PSIF_EOM_Cme, C_irr, 2, 3, Cme_lbl);
    dpd_->buf4_init(&WAmEf, PSIF_CC_HBAR, H_IRR, 26, 28, 26, 28, 0, "WAmEf");
    dpd_->dot24(&Cme, &WAmEf, &XBF, 0, 0, 1.0, 1.0);
    dpd_->buf4_close(&WAmEf);
    dpd_->file2_close(&Cme);
/*
...
*/
    ...
    /* Xbf = Cme * Wbmfe + CME * WbMfE */
    dpd_->file2_init(&Xbf, PSIF_EOM_TMP, C_irr, 3, 3, "Xbf");
    dpd_->file2_scm(&Xbf, 0.0);
    dpd_->file2_init(&Cme, PSIF_EOM_Cme, C_irr, 2, 3, Cme_lbl);
    dpd_->buf4_init(&Wamef, PSIF_CC_HBAR, H_IRR, 31, 15, 31, 17, 0, "Wamef");
    dpd_->dot24(&Cme, &Wamef, &Xbf, 0, 0, 1.0, 1.0);
    dpd_->buf4_close(&Wamef);
    dpd_->file2_close(&Cme);
    dpd_->file2_init(&CME, PSIF_EOM_CME, C_irr, 0, 1, CME_lbl);
    dpd_->buf4_init(&WaMeF, PSIF_CC_HBAR, H_IRR, 25, 29, 25, 29, 0, "WaMeF");
    dpd_->dot24(&CME, &WaMeF, &Xbf, 0, 0, 1.0, 1.0);
    dpd_->buf4_close(&WaMeF);
    dpd_->file2_close(&CME);
/*
...
*/
    ...
    /* Sijab += Xbf * Tijaf - Xaf * Tijbf */
    dpd_->buf4_init(&WP, PSIF_EOM_TMP, C_irr, 12, 15, 12, 15, 0, "WbmfeDS_PB");
    dpd_->file2_init(&Xbf, PSIF_EOM_TMP, C_irr, 3, 3, "Xbf");
    dpd_->buf4_init(&Tijab, PSIF_CC_TAMPS, H_IRR, 12, 15, 12, 17, 0, "tijab");
    dpd_->contract424(&Tijab, &Xbf, &WP, 3, 1, 0, 1.0, 0.0);
    dpd_->buf4_close(&Tijab);
    dpd_->file2_close(&Xbf);
    dpd_->buf4_sort(&WP, PSIF_EOM_TMP, pqsr, 12, 15, "WbmfeDS_MB");
    dpd_->buf4_init(&Sijab, PSIF_EOM_Sijab, C_irr, 12, 15, 12, 17, 0, Sijab_lbl);
    dpd_->buf4_axpy(&WP, &Sijab, 1.0);
    dpd_->buf4_close(&WP);
    dpd_->buf4_init(&WM, PSIF_EOM_TMP, C_irr, 12, 15, 12, 15, 0, "WbmfeDS_MB");
    dpd_->buf4_axpy(&WM, &Sijab, -1.0);
    dpd_->buf4_close(&WM);
    dpd_->buf4_close(&Sijab);
