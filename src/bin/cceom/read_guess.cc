...
void read_guess_init(void)
{
  ...
  for(k=0; k < num_vectors; k++) {
    ...
    for(l=0; l < vector_len; l++) {
      ...
      if ((spin != 0) && (params.eom_ref == 0)) {
        psi::fprintf(outfile,"only alpha guesses allowed for EOM_REF = RHF\n");
        exit(1);
      }
  ...
  psi::fprintf(outfile,"EOM_GUESS_VECTORS implies roots_per_irrep: \n\t");
  for (i=0;i<moinfo.nirreps;++i)
    psi::fprintf(outfile,"%s %d, ",moinfo.irr_labs[i], eom_params.states_per_irrep[i]);
  psi::fprintf(outfile,"\n");
  psi::fprintf(outfile,"and Rs_per_irrep: \n\t");
  for (i=0;i<moinfo.nirreps;++i)
    psi::fprintf(outfile,"%s %d, ",moinfo.irr_labs[i], eom_params.cs_per_irrep[i]);
  psi::fprintf(outfile,"These numbers should match those given by the roots_per_irrep keyword\n");
  psi::fprintf(outfile,"\n\n");
  fflush(outfile);
  return;
}
