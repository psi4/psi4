  ...
  for(i=0; i < eom_params.cs_per_irrep[C_irr]; i++) {
    ...
    if(params.wfn == "EOM_CC2") {
      ...
      if(psio_tocscan(PSIF_CC_INFO, E_lbl) == NULL) {
        psi::fprintf(outfile,"No EOM CC2 Energy found in CC_INFO.  Not normalizing R.\n");
        return;
      }
      ...
      if(psio_tocscan(PSIF_CC_INFO, E_lbl) == NULL) {
        psi::fprintf(outfile,"No EOM CCSD Energy found in CC_INFO.  Not normalizing R.\n");
        return;
      }
      psio_read_entry(PSIF_CC_INFO, E_lbl, (char *) &(energy), sizeof(double));
    }
      ...
      if(psio_tocscan(PSIF_CC_INFO, E_lbl) == NULL) {
        psi::fprintf(outfile,"No EOM CC3 Energy found in CC_INFO.  Not normalizing R.\n");
        return;
      }
      psio_read_entry(PSIF_CC_INFO, E_lbl, (char *) &(energy), sizeof(double));
    }
    ...
    */
    ...
    norm = dot_C(&RIA, &Ria, &fRIJAB, &fRijab, &fRIjAb);
    norm += rzero * rzero;
    psi::fprintf(outfile,"<R|R> = %20.16lf\n",norm);
...
/* end debugging stuff */ 
    ...
    if (eom_params.dot_with_L) {
      ...
      if (C_irr == L_irr ) {
        ...
        psi::fprintf(outfile,"<LIA|RIA>          = %15.10lf\n", dot_IA);
        dot_ia = global_dpd_->file2_dot(&Lia, &Ria);
        psi::fprintf(outfile,"<Lia|Ria>          = %15.10lf\n", dot_ia);
        dot_IJAB = global_dpd_->buf4_dot(&LIJAB, &RIJAB);
        psi::fprintf(outfile,"<LIJAB|RIJAB>      = %15.10lf\n", dot_IJAB);
        dot_ijab = global_dpd_->buf4_dot(&Lijab, &Rijab);
        psi::fprintf(outfile,"<Lijab|Rijab>      = %15.10lf\n", dot_ijab);
        dot_IjAb = global_dpd_->buf4_dot(&LIjAb, &RIjAb);
        psi::fprintf(outfile,"<LIjAb|RIjAb>      = %15.10lf\n", dot_IjAb);
...
/* end debugging stuff */ 
...
#ifdef EOM_DEBUG
    psi::fprintf(outfile,"Norm with produced ROHF-like quantities = %15.10lf\n", norm);
    ...
    if (eom_params.dot_with_L) {
      if (C_irr == L_irr) {
        ...
        psi::fprintf(outfile,"Performing RHF orthogonality test\n");
        psi::fprintf(outfile,"<L0|R0>              = %15.10lf\n", eom_params.L0 * rzero);
        psi::fprintf(outfile,"2*<LIA|RIA>          = %15.10lf\n", r1);
        psi::fprintf(outfile,"<LIjAb|2RIjAb-RIjbA> = %15.10lf\n", r2);
        psi::fprintf(outfile,"<L|R>                = %15.10lf\n", dotval);
      }
      else {
        psi::fprintf(outfile,"<L|R> zero by symmetry\n");
      }
      if (C_irr == L_irr ) {
