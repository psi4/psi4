...
void get_largest_R1_amps(dpdfile2 *R1, int namps, vector<R1_amp> & R1_stack) {
  ...
  Gia = R1->my_irrep;
  dpd_->file2_mat_init(R1);
  dpd_->file2_mat_rd(R1);
...
void get_largest_R2_amps(dpdbuf4 *R2, int namps, vector<R2_amp> & R2_stack) {
  ...
  for(h=0; h < nirreps; h++) {
    dpd_->buf4_mat_irrep_init(R2, h);
    dpd_->buf4_mat_irrep_rd(R2, h);
    ...
    dpd_->buf4_mat_irrep_close(R2, h);
  }
}
