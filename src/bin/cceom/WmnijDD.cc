...
void WmnijDD(int i, int C_irr) {
  ...
  if (params.eom_ref == 0) { /* RHF */
    ...
    dpd_->buf4_init(&WMnIj, PSIF_CC_HBAR, H_IRR, 0, 0, 0, 0, 0, "WMnIj");
    dpd_->contract444(&WMnIj, &CMnEf, &SIjAb, 1, 1, 1.0, 1.0);
    ...
    dpd_->buf4_init(&WMNIJ, PSIF_CC_HBAR, H_IRR, 2, 2, 2, 2, 0, "WMNIJ");
    dpd_->contract444(&WMNIJ, &CMNEF, &SIJAB, 1, 1, 1.0, 1.0);
    dpd_->buf4_close(&WMNIJ);
    dpd_->buf4_close(&SIJAB);
    dpd_->buf4_close(&CMNEF);
    ...
    dpd_->buf4_init(&Wmnij, PSIF_CC_HBAR, H_IRR, 12, 12, 12, 12, 0, "Wmnij");
    dpd_->contract444(&Wmnij, &Cmnef, &Sijab, 1, 1, 1.0, 1.0);
    dpd_->buf4_close(&Wmnij);
    dpd_->buf4_close(&Sijab);
    dpd_->buf4_close(&Cmnef);
    ...
    dpd_->buf4_init(&WMnIj, PSIF_CC_HBAR, H_IRR, 22, 22, 22, 22, 0, "WMnIj");
    dpd_->contract444(&WMnIj, &CMnEf, &SIjAb, 1, 1, 1.0, 1.0);
    dpd_->buf4_close(&WMnIj);
    dpd_->buf4_close(&SIjAb);
    dpd_->buf4_close(&CMnEf);
