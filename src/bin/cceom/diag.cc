...
//    read_guess_init();
  ...
  psi::fprintf(outfile,"Symmetry of ground state: %s\n", moinfo.irr_labs[moinfo.sym]);
...
timer_on("INIT GUESS");
#endif
    psi::fprintf(outfile,"Symmetry of excited state: %s\n", moinfo.irr_labs[moinfo.sym ^ C_irr]);
    psi::fprintf(outfile,"Symmetry of right eigenvector: %s\n",moinfo.irr_labs[C_irr]);
    if (params.eom_ref == 0)
      psi::fprintf(outfile,"Seeking states with multiplicity of %d\n", eom_params.mult);
	...
	psi::fprintf(outfile,"Obtaining initial guess from singles-singles block of Hbar...");
	diagSS(C_irr);
        if (!eom_params.print_singles) psi::fprintf(outfile,"Done.\n\n");
      }
//      else if(eom_params.guess == "INPUT") {
//        read_guess(C_irr);
//      }
      else if(eom_params.guess == "DISK" && params.ref == 0) {
        psi::fprintf(outfile, "Using C1 vectors on disk as initial guesses.\n");
        ...
        psi::fprintf(outfile,"Checking overlap of orthogonalized initial guesses\n");
        for(i=0; i < eom_params.cs_per_irrep[C_irr]; i++) {
          ...
          for(j=0; j < eom_params.cs_per_irrep[C_irr]; j++) {
            ...
            psi::fprintf(outfile,"C[%d][%d] = %15.10lf\n",i,j, 2.0 * dpd_file2_dot(&CME, &CME2));
        ...
        psi::fprintf(outfile, "Invalid initial guess method.\n");
...
timer_off("INIT GUESS");
    ...
    while ((keep_going == 1) && (iter < eom_params.max_iter)) {
      psi::fprintf(outfile,"Iter=%-4d L=%-4d", iter+1, L); fflush(outfile);
      keep_going = 0;
      ...
      for (i=0;i<L;++i) {
        ...
        for (j=0;j<L;++j) {
				  if (i<already_sigma && j<already_sigma)
					  continue;
           /* psi::fprintf(outfile,"Computing G[%d][%d].\n",i,j); */
          ...
          if(params.eom_ref == 0) {
            ...
            global_dpd_->buf4_close(&SIjAb);
            /* psi::fprintf(outfile,"r2aa %12.7lf r2bb %12.7lf r2ab %12.7lf\n", r2aa, r2bb, r2ab); */
          }
          else if (params.eom_ref == 2) {
            sprintf(lbl, "%s %d", "SIA", j);
            global_dpd_->file2_init(&SIA, PSIF_EOM_SIA, C_irr, 0, 1, lbl);
...
#endif /* timing */
#ifdef EOM_DEBUG
      psi::fprintf(outfile,"The G Matrix\n");
      ...
      psi::fprintf(outfile,"  Root    EOM Energy     Delta E   Res. Norm    Conv?\n");
      ...
      if (params.eom_ref > 0) {
        ...
        psi::fprintf(outfile,"Norm of residual vector %d  before precondition %18.13lf\n",k,norm);
#endif
#ifdef TIME_CCEOM
      timer_off("CALC RES");
#endif /* timing */
	...
	if(params.eom_ref == 0) precondition_RHF(&RIA, &RIjAb, lambda[k]);
...
#ifdef EOM_DEBUG
        psi::fprintf(outfile,"Norm of residual vector %d  after precondition %18.13lf\n",k,norm);
#endif

        psi::fprintf(outfile,"%22d%15.10lf%11.2e%12.2e",k+1,lambda[k], lambda[k]-lambda_old[k], norm);
	  ...
	  if(params.eom_ref == 0) {
...
#ifdef EOM_DEBUG
          psi::fprintf(outfile,"Norm of residual vector af preconditioning %18.13lf\n",norm);
#endif
        ...
        if ( (params.wfn == "EOM_CC3") && (cc3_stage>0) ) {
          cc3_index = k;
          cc3_eval = lambda[k];
          /* psi::fprintf(outfile,"Setting CC3 eigenvalue to %15.10lf\n",cc3_eval); */
        }
      }
      ...
      if (L >= vectors_per_root * eom_params.cs_per_irrep[C_irr]) {
        if ( (params.wfn == "EOM_CC3") && (cc3_stage>0) ) {
          ...
          psi::fprintf(outfile,"Collapsing to %d vector(s).\n",L);
          already_sigma = 0;
          ignore_G_old = 1;
        }
        else {
        ...
        if ( (params.wfn == "EOM_CC3") && (cc3_stage>0) ) {
          psi::fprintf(outfile,"Change in CC3 energy from last iterated value %15.10lf\n",
      ...
      if (eom_params.restart_eom_cc3 && (cc3_stage==0)) {
        psi::fprintf(outfile,"Jumping to EOM CC3 iterations\n");
        keep_going = 0;
      }
      if ((keep_going == 0) && (iter < eom_params.max_iter)) {
        ...
        if ( (params.wfn == "EOM_CC3") && (cc3_stage == 0) ) {
          psi::fprintf(outfile, "Completed EOM_CCSD\n");
          psi::fprintf(outfile,"Collapsing to only %d vector(s).\n", eom_params.cs_per_irrep[C_irr]);
          if (!eom_params.restart_eom_cc3) {
          ...
          psi::fprintf(outfile,"Setting initial CC3 eigenvalue to %15.10lf\n",cc3_eval);
          ...
          if (cc3_stage == 1) psi::fprintf(outfile, "Forcing one restart with sigma recomputation.\n");
          else psi::fprintf(outfile,"Forcing restart to make sure new sigma vectors give same eigenvalue.\n");
          psi::fprintf(outfile,"Collapsing to only %d vector(s).\n", cc3_index+1);
          ...
          if (cc3_stage == 1)
            ...
            psi::fprintf(outfile,"Change in CC3 energy from last iterated value %15.10lf\n",
              cc3_eval-cc3_last_converged_eval);
          cc3_last_converged_eval = cc3_eval;
          psi::fprintf(outfile,"Setting old CC3 eigenvalue to %15.10lf\n",cc3_eval);
          keep_going = 1;
          already_sigma = 0;
          ignore_G_old = 1;
          L_old = L;
    ...
    if (num_converged == eom_params.cs_per_irrep[C_irr]) { }
      ...
      psi::fprintf(outfile,"so not all roots converged!\n\n");
    }
    else if ( (params.wfn == "EOM_CC3") && (num_converged == 1) ) { }
    else {
      psi::fprintf(outfile,"\nAlgorithm failure: No vectors could be added, ");
      psi::fprintf(outfile,"though not all roots converged!\n\n");
    }
    ...
    if (num_converged > 0) {
      psi::fprintf(outfile,"\nFinal Energetic Summary for Converged Roots of Irrep %s\n",
	      moinfo.irr_labs[moinfo.sym^C_irr]);
      psi::fprintf(outfile,"                     Excitation Energy              Total Energy\n");
      psi::fprintf(outfile,"                (eV)     (cm^-1)     (au)             (au)\n");
      for (i=0;i<eom_params.cs_per_irrep[C_irr];++i) {
        if (converged[i] == 1) {
          ...
          psi::fprintf(outfile,"EOM State %d %10.3lf %10.1lf %14.10lf  %17.12lf\n", ++num_converged_index,
          ...
          if (C_irr == eom_params.prop_sym && i == eom_params.prop_root) {
            ...
            psi::fprintf(outfile,"\tPutting into environment CURRENT ENERGY:             %15.10lf\n", totalE);
//            Process::environment.globals["CURRENT CORRELATION ENERGY"] = lambda_old[i]+moinfo.ecc;
//            psi::fprintf(outfile,"\tPutting into environment CURRENT CORRELATION ENERGY: %15.10lf\n", lambda_old[i]+moinfo.ecc);
          }
  ...
  psi::fprintf(outfile,"\tTotal # of sigma evaluations: %d\n",nsigma_evaluations);
  return;
}
