...
timer_on("INIT GUESS");
    ...
    if (!first_irrep) {
      dpd_->file2_cache_close();
      dpd_->file2_cache_init();
      dpd_->file4_cache_close();
      dpd_->file4_cache_init();
    }
...
//      }
        ...
        for(i=1; i < eom_params.cs_per_irrep[C_irr]; i++) {
          ...
          for(j=0; j < i; j++) {
            ...
            dpd_->file2_init(&CME2, PSIF_EOM_CME, C_irr, 0, 1, lbl);
            norm = 2.0 * dpd_->file2_dot(&CME, &CME2);
            dpd_->file2_axpy(&CME2, &CME, -1.0*norm, 0);
            dpd_->file2_close(&CME2);
          }
          norm = norm_C1_rhf(&CME);
          dpd_->file2_scm(&CME, 1.0/norm);
          dpd_->file2_close(&CME);
...
timer_off("INIT GUESS");
    ...
    while ((keep_going == 1) && (iter < eom_params.max_iter)) {
      ...
      for (i=already_sigma;i<L;++i) {
	   ...
	   so this may only be necessary for debugging */
        ...
        if (params.eom_ref == 1) {
          ...
          dpd_->buf4_init(&SIJAB, PSIF_EOM_SIJAB, C_irr, 2, 7, 2, 7, 0, lbl);
          sprintf(lbl, "%s %d", "Sijab", i);
          dpd_->buf4_init(&Sijab, PSIF_EOM_Sijab, C_irr, 2, 7, 2, 7, 0, lbl);
          sprintf(lbl, "%s %d", "SIjAb", i);
      ...
      for (i=0;i<L;++i) {
        ...
        if(params.eom_ref == 0) {
          ...
          dpd_->buf4_copy(&CMnEf, PSIF_EOM_TMP, "CMnEf");
          dpd_->buf4_sort(&CMnEf, PSIF_EOM_TMP, pqsr, 0, 5, "CMnfE");
          dpd_->buf4_close(&CMnEf);

          dpd_->buf4_init(&CMnEf1, PSIF_EOM_TMP, C_irr, 0, 5, 0, 5, 0, "CMnEf");
          dpd_->buf4_scm(&CMnEf1, 2.0);
          dpd_->buf4_init(&CMnfE1, PSIF_EOM_TMP, C_irr, 0, 5, 0, 5, 0, "CMnfE");
          dpd_->buf4_axpy(&CMnfE1, &CMnEf1, -1.0);
          dpd_->buf4_close(&CMnfE1);
          dpd_->buf4_close(&CMnEf1);
        ...
        for (j=0;j<L;++j) {
          ...
          if(params.eom_ref == 0) {
            ...
            dpd_->file2_init(&SIA, PSIF_EOM_SIA, C_irr, 0, 1, lbl);
            tval = 2.0 * dpd_->file2_dot(&CME, &SIA);
            sprintf(lbl, "%s %d", "SIjAb", j);
            dpd_->buf4_init(&SIjAb, PSIF_EOM_SIjAb, C_irr, 0, 5, 0, 5, 0, lbl);
            tval += dpd_->buf4_dot(&CMnEf, &SIjAb);
            dpd_->file2_close(&SIA);
						if (params.full_matrix) {
            ...
            sprintf(lbl, "%s %d", "SIJAB", j);
            dpd_->buf4_init(&SIJAB, PSIF_EOM_SIJAB, C_irr, 2, 7, 2, 7, 0, lbl);
            tval += r2aa = dpd_->buf4_dot(&CMNEF, &SIJAB);
            dpd_->buf4_close(&SIJAB);
            sprintf(lbl, "%s %d", "Sijab", j);
            dpd_->buf4_init(&Sijab, PSIF_EOM_Sijab, C_irr, 2, 7, 2, 7, 0, lbl);
            tval += r2bb = dpd_->buf4_dot(&Cmnef, &Sijab);
            dpd_->buf4_close(&Sijab);
            sprintf(lbl, "%s %d", "SIjAb", j);
            dpd_->buf4_init(&SIjAb, PSIF_EOM_SIjAb, C_irr, 0, 5, 0, 5, 0, lbl);
            tval += r2ab = dpd_->buf4_dot(&CMnEf, &SIjAb);
            dpd_->buf4_close(&SIjAb);
            ...
            dpd_->buf4_close(&SIJAB);
            sprintf(lbl, "%s %d", "Sijab", j);
            dpd_->buf4_init(&Sijab, PSIF_EOM_Sijab, C_irr, 12, 17, 12, 17, 0, lbl);
            tval += dpd_->buf4_dot(&Cmnef, &Sijab);
            dpd_->buf4_close(&Sijab);
            sprintf(lbl, "%s %d", "SIjAb", j);
      ...
      if (params.eom_ref == 0) {
        dpd_->file2_init(&RIA, PSIF_EOM_R, C_irr, 0, 1, "RIA");
        dpd_->buf4_init(&RIjAb, PSIF_EOM_R, C_irr, 0, 5, 0, 5, 0, "RIjAb");
      }
      else if (params.eom_ref == 1) {
        dpd_->file2_init(&RIA, PSIF_EOM_R, C_irr, 0, 1, "RIA");
        dpd_->file2_init(&Ria, PSIF_EOM_R, C_irr, 0, 1, "Ria");
        dpd_->buf4_init(&RIJAB, PSIF_EOM_R, C_irr, 2, 7, 2, 7, 0, "RIJAB");
        dpd_->buf4_init(&Rijab, PSIF_EOM_R, C_irr, 2, 7, 2, 7, 0, "Rijab");
        dpd_->buf4_init(&RIjAb, PSIF_EOM_R, C_irr, 0, 5, 0, 5, 0, "RIjAb");
      }
      else if (params.eom_ref == 2) {
        dpd_->file2_init(&RIA, PSIF_EOM_R, C_irr, 0, 1, "RIA");
        dpd_->file2_init(&Ria, PSIF_EOM_R, C_irr, 2, 3, "Ria");
        dpd_->buf4_init(&RIJAB, PSIF_EOM_R, C_irr, 2, 7, 2, 7, 0, "RIJAB");
        dpd_->buf4_init(&Rijab, PSIF_EOM_R, C_irr, 12, 17, 12, 17, 0, "Rijab");
        dpd_->buf4_init(&RIjAb, PSIF_EOM_R, C_irr, 22, 28, 22, 28, 0, "RIjAb");
      }
      ...
      dpd_->file2_scm(&RIA, 0.0);
      dpd_->buf4_scm(&RIjAb, 0.0);
      if (params.eom_ref > 0) {
        dpd_->file2_scm(&Ria, 0.0);
        dpd_->buf4_scm(&RIJAB, 0.0);
        dpd_->buf4_scm(&Rijab, 0.0);
        ...
        for (i=0;i<L;++i) {
          if (params.eom_ref == 0) { /* RHF residual */
            ...
            sprintf(lbl, "%s %d", "CME", i);
            dpd_->file2_init(&CME, PSIF_EOM_CME, C_irr, 0, 1, lbl);
            dpd_->file2_axpbycz(&CME, &SIA, &RIA, -1.0*lambda[k]*alpha[i][k], alpha[i][k], 1.0);
            dpd_->file2_close(&CME);
            dpd_->file2_close(&SIA);
            ...
            sprintf(lbl, "%s %d", "SIjAb", i);
            dpd_->buf4_init(&SIjAb, PSIF_EOM_SIjAb, C_irr, 0, 5, 0, 5, 0, lbl);
            dpd_->buf4_axpbycz(&CMnEf, &SIjAb, &RIjAb, -1.0*lambda[k]*alpha[i][k], alpha[i][k], 1.0);
            dpd_->buf4_close(&CMnEf);
            dpd_->buf4_close(&SIjAb);
	...
	if(params.eom_ref == 0) precondition_RHF(&RIA, &RIjAb, lambda[k]);
        ...
        if (params.eom_ref == 0) {
          ...
          dpd_->buf4_init(&RIjbA, PSIF_EOM_TMP, C_irr, 0, 5, 0, 5, 0, "RIjbA");
      ...
      for (i=0;i<eom_params.cs_per_irrep[C_irr];++i) {
        if (converged[i] == 1) {
          ...
          if(params.eom_ref == 0) {
            ...
            dpd_->file2_init(&CME, PSIF_CC_RAMPS, C_irr, 0, 1, lbl);
            sprintf(lbl, "%s %d %d", "RIjAb", C_irr, i);
            dpd_->buf4_init(&CMnEf, PSIF_CC_RAMPS, C_irr, 0, 5, 0, 5, 0, lbl);
            ...
            dpd_->file2_init(&Cme, PSIF_CC_RAMPS, C_irr, 0, 1, lbl);
            sprintf(lbl, "%s %d %d", "RIJAB", C_irr, i);
            dpd_->buf4_init(&CMNEF, PSIF_CC_RAMPS, C_irr, 2, 7, 2, 7, 0, lbl);
            sprintf(lbl, "%s %d %d", "Rijab", C_irr, i);
            dpd_->buf4_init(&Cmnef, PSIF_CC_RAMPS, C_irr, 2, 7, 2, 7, 0, lbl);
            sprintf(lbl, "%s %d %d", "RIjAb", C_irr, i);
            dpd_->buf4_init(&CMnEf, PSIF_CC_RAMPS, C_irr, 0, 5, 0, 5, 0, lbl);
            ...
            dpd_->file2_init(&CME, PSIF_CC_RAMPS, C_irr, 0, 1, lbl);
            sprintf(lbl, "%s %d %d", "Ria", C_irr, i);
            dpd_->file2_init(&Cme, PSIF_CC_RAMPS, C_irr, 2, 3, lbl);
            sprintf(lbl, "%s %d %d", "RIJAB", C_irr, i);
            dpd_->buf4_init(&CMNEF, PSIF_CC_RAMPS, C_irr, 2, 7, 2, 7, 0, lbl);
            sprintf(lbl, "%s %d %d", "Rijab", C_irr, i);
            dpd_->buf4_init(&Cmnef, PSIF_CC_RAMPS, C_irr, 12, 17, 12, 17, 0, lbl);
            sprintf(lbl, "%s %d %d", "RIjAb", C_irr, i);
            dpd_->buf4_init(&CMnEf, PSIF_CC_RAMPS, C_irr, 22, 28, 22, 28, 0, lbl);
...
void init_C1(int i, int C_irr ){
  ...
  if (params.eom_ref == 0) {
    ...
    dpd_->file2_init(&CME, PSIF_EOM_CME, C_irr, 0, 1, lbl);
    sprintf(lbl, "%s %d", "Cme", i);
    if (params.eom_ref == 1) dpd_->file2_init(&Cme, PSIF_EOM_Cme, C_irr, 0, 1, lbl);
    else if (params.eom_ref == 2) dpd_->file2_init(&Cme, PSIF_EOM_Cme, C_irr, 2, 3, lbl);
    scm_C1(&CME, &Cme, 0.0);
    dpd_->file2_close(&CME);
    dpd_->file2_close(&Cme);
  }
...
void init_S1(int i, int C_irr) {
  ...
  if (params.eom_ref == 0) {
    ...
    dpd_->file2_init(&SIA, PSIF_EOM_SIA, C_irr, 0, 1, lbl);
    sprintf(lbl, "%s %d", "Sia", i);
    if (params.eom_ref == 1) dpd_->file2_init(&Sia, PSIF_EOM_Sia, C_irr, 0, 1, lbl);
    else if (params.eom_ref == 2) dpd_->file2_init(&Sia, PSIF_EOM_Sia, C_irr, 2, 3, lbl);
    scm_C1(&SIA, &Sia, 0.0);
    dpd_->file2_close(&SIA);
    dpd_->file2_close(&Sia);
  }
