...
void write_Rs(int C_irr, double *evals, int *converged) {
  ...
  for (i=0; i<eom_params.cs_per_irrep[C_irr]; ++i) {
    ...
    dpd_->file2_init(&CME, PSIF_EOM_CME, C_irr, 0, 1, C_lbl);
    dpd_->file2_copy(&CME, PSIF_CC_RAMPS, R_lbl);
    ...
    if (params.eom_ref <= 1)
      dpd_->buf4_init(&CMnEf, PSIF_EOM_CMnEf, C_irr, 0, 5, 0, 5, 0, C_lbl);
    else if (params.eom_ref == 2)
      dpd_->buf4_init(&CMnEf, PSIF_EOM_CMnEf, C_irr, 22, 28, 22, 28, 0, C_lbl);
    dpd_->buf4_copy(&CMnEf, PSIF_CC_RAMPS, R_lbl);
    ...
    if(params.eom_ref > 0) {
      ...
      if (params.eom_ref == 1)
         ...
         dpd_->file2_init(&Cme, PSIF_EOM_Cme, C_irr, 2, 3, C_lbl);
      dpd_->file2_copy(&Cme, PSIF_CC_RAMPS, R_lbl);
      dpd_->file2_close(&Cme);
      ...
      dpd_->buf4_init(&CMNEF, PSIF_EOM_CMNEF, C_irr, 2, 7, 2, 7, 0, C_lbl);
      dpd_->buf4_copy(&CMNEF, PSIF_CC_RAMPS, R_lbl);
      dpd_->buf4_close(&CMNEF);
      ...
      if (params.eom_ref == 1)
        ...
        dpd_->buf4_init(&Cmnef, PSIF_EOM_Cmnef, C_irr, 12, 17, 12, 17, 0, C_lbl);
        dpd_->buf4_copy(&Cmnef, PSIF_CC_RAMPS, R_lbl);
        dpd_->buf4_close(&Cmnef);
    }
  }
