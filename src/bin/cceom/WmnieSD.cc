...
void WmnieSD(int i, int C_irr) {
  ...
  if (params.eom_ref == 0) { /* RHF */
    ...
    dpd_->buf4_init(&WMNIE, PSIF_CC_HBAR, H_IRR, 2, 11, 2, 11, 0, "WMNIE (M>N,EI)");
    sprintf(lbl, "%s %d", "CMNEF", i);
    dpd_->buf4_init(&CMNEF, PSIF_EOM_CMNEF, C_irr, 2, 5, 2, 7, 0, lbl);
    dpd_->contract442(&WMNIE, &CMNEF, &SIA, 3, 3, 1.0, 1.0);
    dpd_->buf4_close(&CMNEF);
    dpd_->buf4_close(&WMNIE);
    ...
    /* SIA += 0.5 WMNIE * CMNAE + WMnIe * CMnAe */
    dpd_->buf4_init(&WMNIE, PSIF_CC_HBAR, H_IRR, 2, 21, 2, 21, 0, "WMNIE (M>N,EI)");
    sprintf(lbl, "%s %d", "CMNEF", i);
    dpd_->buf4_init(&CMNEF, PSIF_EOM_CMNEF, C_irr, 2, 5, 2, 7, 0, lbl);
    dpd_->contract442(&WMNIE, &CMNEF, &SIA, 3, 3, 1.0, 1.0);
    dpd_->buf4_close(&CMNEF);
    ...
    /* Sia += 0.5 Wmnie * Cmnae + Wmnie * Cmnae */
    dpd_->buf4_init(&Wmnie, PSIF_CC_HBAR, H_IRR, 12, 31, 12, 31, 0, "Wmnie (m>n,ei)");
    sprintf(lbl, "%s %d", "Cmnef", i);
    dpd_->buf4_init(&Cmnef, PSIF_EOM_Cmnef, C_irr, 12, 15, 12, 17, 0, lbl);
    dpd_->contract442(&Wmnie, &Cmnef, &Sia, 3, 3, 1.0, 1.0);
    dpd_->buf4_close(&Cmnef);
    dpd_->buf4_close(&Wmnie);
