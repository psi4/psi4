...
/*     dpd_file2_close(&SIA); */
    ...
    dpd_->buf4_init(&WAMEF, PSIF_CC_HBAR, H_IRR, 11, 7, 11, 7, 0, "WAMEF");
    sprintf(lbl, "%s %d", "CMNEF", i);
    dpd_->buf4_init(&CMNEF, PSIF_EOM_CMNEF, C_irr, 0, 7, 2, 7, 0, lbl);
    dpd_->contract442(&CMNEF, &WAMEF, &SIA, 0, 0, 1.0, 1.0);
    ...
    /* SIA += 0.5 CIMEF* WAMEF + CImEf * WAmEf */
    dpd_->buf4_init(&WAMEF, PSIF_CC_HBAR, H_IRR, 21, 7, 21, 7, 0, "WAMEF");
    sprintf(lbl, "%s %d", "CMNEF", i);
    dpd_->buf4_init(&CMNEF, PSIF_EOM_CMNEF, C_irr, 0, 7, 2, 7, 0, lbl);
    dpd_->contract442(&CMNEF, &WAMEF, &SIA, 0, 0, 1.0, 1.0);
    dpd_->buf4_close(&CMNEF);
    ...
    /* Sia += 0.5 Cimef * Wamef + CiMeF * WaMeF */
    dpd_->buf4_init(&Wamef, PSIF_CC_HBAR, H_IRR, 31, 17, 31, 17, 0, "Wamef");
    sprintf(lbl, "%s %d", "Cmnef", i);
    dpd_->buf4_init(&Cmnef, PSIF_EOM_Cmnef, C_irr, 10, 17, 12, 17, 0, lbl);
    dpd_->contract442(&Cmnef, &Wamef, &Sia, 0, 0, 1.0, 1.0);
    dpd_->buf4_close(&Cmnef);
    dpd_->buf4_close(&Wamef);
