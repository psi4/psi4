  ...
  dpdbuf4 *RIJAB, dpdbuf4 *Rijab, dpdbuf4 *RIjAb, int *numCs, int irrep)
{
  ...
  for (i=0; i<*numCs; i++) {
    ...
    dotval += global_dpd_->file2_dot(Ria, &Cme);
  //psi::fprintf(outfile, "OE Dotval for vector %d = %20.14f\n", i, dotval);
    dotval += global_dpd_->buf4_dot(RIJAB, &CMNEF);
    dotval += global_dpd_->buf4_dot(Rijab, &Cmnef);
    dotval += global_dpd_->buf4_dot(RIjAb, &CMnEf);

  //psi::fprintf(outfile, "Dotval for vector %d = %20.14f\n", i, dotval);
  ...
  //psi::fprintf(outfile, "Norm of residual (TDC) = %20.14f\n", norm);
...
void schmidt_add_RHF(dpdfile2 *RIA, dpdbuf4 *RIjAb, int *numCs, int irrep)
{
 ...
 //psi::fprintf(outfile, "Dotval for vector %d = %20.14f\n", i, dotval);
  ...
  if (norm < eom_params.schmidt_add_residual_tol) {
    ...
    norm = sqrt(norm);
    psi::fprintf(outfile,"Norm of final new C in schmidt_add(): %20.15lf\n", norm);
    dpd_buf4_close(&R2b);
#endif
