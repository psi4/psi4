...
void diagSS(int C_irr) {
  ...
  */
  ...
  if (params.eom_ref > 0) {
    if (params.eom_ref == 1) dpd_->file2_init(&Ria, PSIF_EOM_R, C_irr, 0, 1, "Ria");
    else if (params.eom_ref == 2) dpd_->file2_init(&Ria, PSIF_EOM_R, C_irr, 2, 3, "Ria");
    dpd_->file2_mat_init(&Ria);
    dpd_->file2_mat_wrt(&Ria);
    dpd_->file2_close(&Ria);
  }
  ...
  while ((keep_going == 1) && (iter < eom_params.max_iter_SS)) {
    ...
    for (i=0;i<L;++i) {
      ...
      dpd_->file2_close(&CME);
      if (params.eom_ref > 0) dpd_->file2_close(&Cme);
    }
    ...
    for (k=0;k<num_roots;++k) {
      ...
      if (params.eom_ref > 0) dpd_->file2_scm(&Ria, 0.0);
      converged[k] = 0;
      for (i=0;i<L;++i) { 
        sprintf(lbl, "%s %d", "SIA", i);
        dpd_->file2_init(&SIA, PSIF_EOM_SIA, C_irr, 0, 1, lbl);
	...
	*/
    ...
    dpd_->file2_close(&RIA);
    if (params.eom_ref > 0) dpd_->file2_close(&Ria);
...
void precondition_SS(dpdfile2 *RIA, dpdfile2 *Ria, double eval)
{
  ...
  if (params.eom_ref == 1) dpd_->file2_init(&Dia, PSIF_EOM_D, C_irr, 0, 1, "Dia");
  else if (params.eom_ref == 2) dpd_->file2_init(&Dia, PSIF_EOM_D, C_irr, 2, 3, "Dia");
  dpd_->file2_mat_init(&Dia);
  dpd_->file2_mat_rd(&Dia);
  for(h=0; h < nirreps; h++)
    for(i=0; i < Ria->params->rowtot[h]; i++)
...
void schmidt_add_SS(dpdfile2 *RIA, dpdfile2 *Ria, int C_irr, int *numCs)
{
  ...
  for (i=0; i<*numCs; i++) {
    ...
    sprintf(Cme_lbl, "%s %d", "Cme", i);
    if (params.eom_ref == 1) dpd_->file2_init(&Cme, PSIF_EOM_Cme, C_irr, 0, 1, Cme_lbl);
    else if (params.eom_ref == 2) dpd_->file2_init(&Cme, PSIF_EOM_Cme, C_irr, 2, 3, Cme_lbl);
...
void restart_SS(double **alpha, int L, int num, int C_irr) {
  ...
  for (i=0; i<num; ++i) {
    ...
    dpd_->file2_init(&C1, PSIF_EOM_CME, C_irr, 0, 1, lbl);
    dpd_->file2_scm(&C1, 0.0);
    for (j=0;j<L;++j) {
      sprintf(lbl, "%s %d", "CME", j);
      dpd_->file2_init(&CME, PSIF_EOM_CME, C_irr, 0, 1, lbl);
      dpd_->file2_axpy(&CME, &C1, alpha[j][i], 0);
      dpd_->file2_close(&CME);
    }
    dpd_->file2_close(&C1);
    ...
    if (params.eom_ref > 0) {
      sprintf(lbl, "%s %d", "Cme", L+i);
      if (params.eom_ref == 1) dpd_->file2_init(&C1, PSIF_EOM_Cme, C_irr, 0, 1, lbl);
      else if (params.eom_ref == 2) dpd_->file2_init(&C1, PSIF_EOM_Cme, C_irr, 2, 3, lbl);
      dpd_->file2_scm(&C1, 0.0);
      for (j=0;j<L;++j) {
        sprintf(lbl, "%s %d", "Cme", j);
        if (params.eom_ref == 1) dpd_->file2_init(&Cme, PSIF_EOM_Cme, C_irr, 0, 1, lbl);
