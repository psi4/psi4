...
void norm_HC1(int i, int C_irr) {
  ...
  if (params.eom_ref == 0) { /* RHF */
    ...
    /** FME **/
    dpd_->file2_init(&F, PSIF_CC3_HC1, 0, 0, 1, "HC1 FME");
    dot = dpd_->file2_dot_self(&F);
    dpd_->file2_close(&F);
    fprintf(outfile,"<FME|FME> = %15.10lf\n", dot); 

    /** WAMEF **/
    dpd_->buf4_init(&W, PSIF_CC3_HC1, 0, 11, 5, 11, 5, 0, "HC1 WAmEf (Am,Ef)");
    dot = dpd_->buf4_dot_self(&W);
    dpd_->buf4_close(&W);
    fprintf(outfile,"<WAmEf (Am,Ef)|WAmEf> = %15.10lf\n", dot); 

    /** WMNIE **/
    dpd_->buf4_init(&W, PSIF_CC3_HC1, 0, 0, 10, 0, 10, 0, "HC1 WMnIe (Mn,Ie)");
    dot = dpd_->buf4_dot_self(&W);
    dpd_->buf4_close(&W);
    fprintf(outfile,"<WMnIe (Mn,Ie)|WMnIe> = %15.10lf\n", dot); 

    dpd_->buf4_init(&W, PSIF_CC3_HC1, 0, 0, 10, 0, 10, 0, "HC1 2WMnIe - WnMIe (Mn,Ie)");
    dot = dpd_->buf4_dot_self(&W);
    dpd_->buf4_close(&W);
...
*/
    ...
    dpd_->buf4_init(&W, PSIF_CC3_HC1, 0, 10, 5, 10, 5, 0, "CC3 WAbEi (Ie,Ab)");
    dot = dpd_->buf4_dot_self(&W);
    dpd_->buf4_close(&W);
    ...
    /** WAMEF **/
    dpd_->buf4_init(&W, PSIF_CC3_HC1, 0, 11, 5, 11, 7, 0, "HC1 WAMEF (AM,E>F)");
    dot = dpd_->buf4_dot_self(&W);
    dpd_->buf4_close(&W);
    ...
    dpd_->buf4_init(&W, PSIF_CC3_HC1, 0, 0, 10, 2, 10, 0, "HC1 WMNIE (M>N,IE)");
    dot = dpd_->buf4_dot_self(&W);
    dpd_->buf4_close(&W);
    ...
    dpd_->buf4_init(&W, PSIF_CC3_HC1, 0, 0, 11, 2, 11, 0, "HC1 WMNIE (M>N,EI)");
    dot = dpd_->buf4_dot_self(&W);
    dpd_->buf4_close(&W);
    ...
    dpd_->buf4_init(&W, PSIF_CC3_HC1, 0, 0, 11, 0, 11, 0, "HC1 WMnIe (Mn,eI)");
    dot = dpd_->buf4_dot_self(&W);
    dpd_->buf4_close(&W);
...
*/
    ...
    dpd_->buf4_init(&W, PSIF_CC3_HC1, 0, 10, 5, 10, 7, 0, "HC1 WABEI (IE,A>B)");
    dot = dpd_->buf4_dot_self(&W);
    dpd_->buf4_close(&W);
    ...
    dpd_->buf4_init(&W, PSIF_CC3_HC1, 0, 10, 5, 10, 5, 0, "HC1 WAbEi (iE,Ab)");
    dot = dpd_->buf4_dot_self(&W);
    dpd_->buf4_close(&W);
    ...
    /** Fme **/
    dpd_->file2_init(&F, PSIF_CC3_HC1, 0, 2, 3, "HC1 Fme");
    dot = dpd_->file2_dot_self(&F);
    dpd_->file2_close(&F);
    fprintf(outfile,"\t<Fme|Fme> = %15.10lf\n", dot); 
    ...
    fprintf(outfile,"Wamef terms\n");
    /** WAMEF **/
    dpd_->buf4_init(&W, PSIF_CC3_HC1, 0, 20, 5, 20, 7, 0, "HC1 WAMEF (MA,F>E)");
    dot = dpd_->buf4_dot_self(&W);
    dpd_->buf4_close(&W);
    fprintf(outfile,"\t<WAMEF (MA,F>E)|WAMEF> = %15.10lf\n", dot); 

    /** Wamef **/
    dpd_->buf4_init(&W, PSIF_CC3_HC1, 0, 30, 15, 30, 17, 0, "HC1 Wamef (ma,f>e)");
    dot = dpd_->buf4_dot_self(&W);
    dpd_->buf4_close(&W);
    fprintf(outfile,"\t<Wamef (ma,f>e)|Wamef> = %15.10lf\n", dot); 

    /** WAmEf **/
    dpd_->buf4_init(&W, PSIF_CC3_HC1, 0, 27, 29, 27, 28, 0, "HC1 WAmEf (mA,fE)");
    dot = dpd_->buf4_dot_self(&W);
    dpd_->buf4_close(&W);
    fprintf(outfile,"\t<WAmEf (mA,fE)|WAmEf> = %15.10lf\n", dot); 

    /** WaMeF **/
    dpd_->buf4_init(&W, PSIF_CC3_HC1, 0, 24, 28, 24, 28, 0, "HC1 WaMeF (Ma,Fe)");
    dot = dpd_->buf4_dot_self(&W);
    dpd_->buf4_close(&W);
    fprintf(outfile,"\t<WaMeF (Ma,Fe)|WaMeF> = %15.10lf\n", dot); 
    ...
    dpd_->buf4_init(&W, PSIF_CC3_HC1, 0, 0, 20, 2, 20, 0, "HC1 WMNIE (M>N,IE)");
    dot = dpd_->buf4_dot_self(&W);
    dpd_->buf4_close(&W);
    fprintf(outfile,"\t<WMNIE (M>N,IE)|WMNIE> = %15.10lf\n", dot); 

    dpd_->buf4_init(&W, PSIF_CC3_HC1, 0, 10, 30, 12, 30, 0, "HC1 Wmnie (m>n,ie)");
    dot = dpd_->buf4_dot_self(&W);
    dpd_->buf4_close(&W);
    fprintf(outfile,"\t<Wmnie (m>n,ie)|Wmnie> = %15.10lf\n", dot); 

    dpd_->buf4_init(&W, PSIF_CC3_HC1, 0, 22, 24, 22, 24, 0, "HC1 WMnIe (Mn,Ie)");
    dot = dpd_->buf4_dot_self(&W);
    dpd_->buf4_close(&W);
    fprintf(outfile,"\t<WMnIe (Mn,Ie)|WMnIe> = %15.10lf\n", dot); 

    dpd_->buf4_init(&W, PSIF_CC3_HC1, 0, 23, 27, 23, 27, 0, "HC1 WmNiE (mN,iE)");
    dot = dpd_->buf4_dot_self(&W);
    dpd_->buf4_close(&W);
    fprintf(outfile,"\t<WmNiE (mN,iE)|WmNiE> = %15.10lf\n", dot); 
...
*/
    ...
    dpd_->buf4_init(&W, PSIF_CC3_HC1, 0, 20, 5, 20, 7, 0, "HC1 WABEI (IE,B>A)");
    dot = dpd_->buf4_dot_self(&W);
    dpd_->buf4_close(&W);
    fprintf(outfile,"\t<WABEI (IE,A>B)|WABEI> = %15.10lf\n", dot);

    dpd_->buf4_init(&W, PSIF_CC3_HC1, 0, 30, 15, 30, 17, 0, "HC1 Wabei (ie,b>a)");
    dot = dpd_->buf4_dot_self(&W);
    dpd_->buf4_close(&W);
    fprintf(outfile,"\t<Wabei (ie,a>b)|Wabei> = %15.10lf\n", dot);

    dpd_->buf4_init(&W, PSIF_CC3_HC1, 0, 27, 29, 27, 29, 0, "HC1 WAbEi (iE,bA)");
    dot = dpd_->buf4_dot_self(&W);
    dpd_->buf4_close(&W);
    fprintf(outfile,"\t<WAbEi (iE,Ab)|WAbEi> = %15.10lf\n", dot);

    dpd_->buf4_init(&W, PSIF_CC3_HC1, 0, 24, 28, 24, 28, 0, "HC1 WaBeI (Ie,Ba)");
    dot = dpd_->buf4_dot_self(&W);
    dpd_->buf4_close(&W);
    fprintf(outfile,"\t<WaBeI (Ie,aB)|WaBeI> = %15.10lf\n", dot);
  }
