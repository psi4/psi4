...
void norm_HC1(int i, int C_irr) {
  ...
  if (params.eom_ref == 0) { /* RHF */
    ...
    dot = global_dpd_->buf4_dot_self(&W);
    global_dpd_->buf4_close(&W);
    psi::fprintf(outfile,"<2WMnIe - WnMIe (Mn,Ie)|2WMnIe - WnMIe> = %15.10lf\n", dot); 
...
*/
    ...
    dot = global_dpd_->buf4_dot_self(&W);
    global_dpd_->buf4_close(&W);
    psi::fprintf(outfile,"<WAbEi (Ie,Ab) | WAbEi (Ie,Ab)> = %15.10lf\n", dot);
    ...
    dot = global_dpd_->buf4_dot_self(&W);
    global_dpd_->buf4_close(&W);
    psi::fprintf(outfile,"\t<WAMEF (AM,E>F)|WAMEF> = %15.10lf\n", dot); 
...
*/
    ...
    dot = global_dpd_->buf4_dot_self(&W);
    global_dpd_->buf4_close(&W);
    psi::fprintf(outfile,"\t<WABEI (IE,A>B)|WABEI> = %15.10lf\n", dot);
    ...
    dot = global_dpd_->buf4_dot_self(&W);
    global_dpd_->buf4_close(&W);
    psi::fprintf(outfile,"\t<WAbEi (iE,Ab)|WAbEi> = %15.10lf\n", dot);
    ...
    psi::fprintf(outfile,"***** Norms of HC1 *****\n");
    /** FME **/
    global_dpd_->file2_init(&F, PSIF_CC3_HC1, 0, 0, 1, "HC1 FME");
    dot = global_dpd_->file2_dot_self(&F);
    global_dpd_->file2_close(&F);
    ...
    dot = global_dpd_->buf4_dot_self(&W);
    global_dpd_->buf4_close(&W);
    psi::fprintf(outfile,"\t<WAMEF (MA,F>E)|WAMEF> = %15.10lf\n", dot); 
    ...
    dot = global_dpd_->buf4_dot_self(&W);
    global_dpd_->buf4_close(&W);
    psi::fprintf(outfile,"\t<WAmEf (mA,fE)|WAmEf> = %15.10lf\n", dot); 
