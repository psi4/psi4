...
void BT2_AO(void)
{
  ...
  if(params.ref == 0) { /** RHF **/
    ...
    if(params.aobasis == "DISK") {
      ...
      dpd_->buf4_init(&tau1_AO, PSIF_CC_TMP0, 0, 5, 0, 5, 0, 0, "tauPqIj (1)");
      dpd_->buf4_init(&tau2_AO, PSIF_CC_TMP0, 0, 5, 0, 5, 0, 0, "tauPqIj (2)");
      dpd_->buf4_scm(&tau2_AO, 0.0);
        
      for(h=0; h < nirreps; h++) {
	dpd_->buf4_mat_irrep_init(&tau1_AO, h);
	dpd_->buf4_mat_irrep_rd(&tau1_AO, h);
	dpd_->buf4_mat_irrep_init(&tau2_AO, h);
      }
	...
	counter += AO_contribute(&InBuf, &tau1_AO, &tau2_AO);
      ...
      for(h=0; h < nirreps; h++) {
	dpd_->buf4_mat_irrep_wrt(&tau2_AO, h);
	dpd_->buf4_mat_irrep_close(&tau2_AO, h);
	dpd_->buf4_mat_irrep_close(&tau1_AO, h);
      }
      dpd_->buf4_close(&tau1_AO);
      ...
      dpd_->file4_init(&T, PSIF_CC_TAMPS, 0, 0, 5, "tauIjAb");
      dpd_->file4_cache_del(&T);
      dpd_->file4_close(&T);

      dpd_->file4_init(&T, PSIF_CC_TAMPS, 0, 0, 5, "New tIjAb");
      dpd_->file4_cache_del(&T);
      dpd_->file4_close(&T);
    ...
    dpd_->buf4_init(&tau1_AO, PSIF_CC_TMP0, 0, 5, 2, 5, 2, 0, "tauPQIJ (1)");
    ...
    dpd_->buf4_init(&tau1_AO, PSIF_CC_TMP0, 0, 15, 12, 15, 12, 0, "taupqij (1)");
    ...
    dpd_->buf4_init(&tau1_AO, PSIF_CC_TMP0, 0, 28, 22, 28, 22, 0, "tauPqIj (1)");
