...
void print_pair_energies(double* emp2_aa, double* emp2_ab, double* ecc_aa, double* ecc_ab)
{
  if(params.ref == 0) { /** RHF **/
    ...
    if (!params.spinadapt_energies) {
      ...
      psi::fprintf(outfile, "\tAlpha-alpha pair energies\n");
      psi::fprintf(outfile, "\t    i       j         MP2             %s\n",params.wfn.c_str());
      psi::fprintf(outfile, "\t  -----   -----   ------------   ------------\n");
      if (naa) {
        ij = 0;
        for(i=0; i<nocc_act; i++)
          for(j=0; j<i; j++,ij++) {
            psi::fprintf(outfile, "\t  %3d     %3d     %12.9lf   %12.9lf\n", i+1, j+1, emp2_aa[ij], ecc_aa[ij]);
            emp2_aa_tot += emp2_aa[ij];
            ecc_aa_tot += ecc_aa[ij];
          }
      }
      psi::fprintf(outfile, "\t  -------------   ------------   ------------\n");
      psi::fprintf(outfile, "\t      Total       %12.9lf   %12.9lf\n\n", emp2_aa_tot, ecc_aa_tot);

      psi::fprintf(outfile, "\tAlpha-beta pair energies\n");
      psi::fprintf(outfile, "\t    i       j         MP2             %s\n",params.wfn.c_str());
      psi::fprintf(outfile, "\t  -----   -----   ------------   ------------\n");
      if (nab) {
        ij = 0;
      ...
      psi::fprintf(outfile, "\tSinglet pair energies\n");
      psi::fprintf(outfile, "\t    i       j         MP2             %s\n",params.wfn.c_str());
      psi::fprintf(outfile, "\t  -----   -----   ------------   ------------\n");
      ij = 0;
      for(i=0; i<nocc_act; i++)
      ...
      psi::fprintf(outfile, "\tTriplet pair energies\n");
      psi::fprintf(outfile, "\t    i       j         MP2             %s\n",params.wfn.c_str());
      psi::fprintf(outfile, "\t  -----   -----   ------------   ------------\n");
      if (naa) {
        ij = 0;
