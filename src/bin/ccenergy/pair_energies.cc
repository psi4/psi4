...
void pair_energies(double** epair_aa, double** epair_ab)
{
  ...
  if(params.ref == 0) { /** RHF **/
    ...
    if (naa) {
      ...
      dpd_->buf4_init(&tau, PSIF_CC_TAMPS, 0, 2, 5, 0, 5, 1, "tauIjAb");
      dpd_->buf4_init(&E, PSIF_CC_TMP0, 0, 2, 2, 2, 2, 0, "E <ij|kl>");
      dpd_->contract444(&D, &tau, &E, 0, 0, 1.0, 0.0);
      ...
      for(irrep=0; irrep<moinfo.nirreps; irrep++) {
        ...
        dpd_->buf4_mat_irrep_init(&E, irrep);
        dpd_->buf4_mat_irrep_rd(&E, irrep);
	...
	dpd_->buf4_mat_irrep_close(&E, irrep);
      }
    ...
    if (nab) {
      ...
      dpd_->buf4_init(&E, PSIF_CC_TMP0, 0, 0, 0, 0, 0, 0, "E <ij|kl>");
      dpd_->contract444(&D, &tau, &E, 0, 0, 1.0, 0.0);
