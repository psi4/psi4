...
int rotate(void)
{
  ...
  if(fabs(max) <= params.bconv) {
    psi::fprintf(outfile, "\tBrueckner orbitals converged.  Maximum T1 = %15.12f\n",
	    fabs(max));
    chkpt_close();
    return(1);
  }
  else 
    psi::fprintf(outfile, "\tRotating orbitals.  Maximum T1 = %15.12f\n", fabs(max));
  ...
  if(params.ref == 0) { /* RHF */
    ...
    if((stat = C_DSYEV('v','u', nmo,&(MO_S[0][0]),nmo,evals,work,nmo*3))) {
      psi::fprintf(outfile, "rotate(): Error in overlap diagonalization. stat = %d\n", stat);
    ...
    */
	...
	if((stat = C_DSYEV('v','u', moinfo.occpi[h], &(Foo[h][0][0]), 
	  ...
	  psi::fprintf(outfile, "rotate(): Error in Foo[%1d] diagonalization. stat = %d\n", 
		  h, stat);
	...
	*/
	...
	if((stat = C_DSYEV('v','u', moinfo.virtpi[h], &(Fvv[h][0][0]), moinfo.virtpi[h], 
	  ...
	  psi::fprintf(outfile, "rotate(): Error in Fvv[%1d] diagonalization. stat = %d\n", 
		  h, stat);
      throw PsiException("rotate(): Error in Foo diagonalization.", __FILE__, __LINE__);
	}
	free(evals);
