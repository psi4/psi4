...
void sort_amps(void)
{
  ...
  if(params.ref == 0) { /** RHF **/
    ...
    /* 2 tIjAb - tIjBa */
    dpd_buf4_init(&t2, PSIF_CC_TAMPS, 0, 0, 5, 0, 5, 0, "tIjAb");
    dpd_buf4_scmcopy(&t2, PSIF_CC_TAMPS, "2 tIjAb - tIjBa", 2);
    dpd_buf4_sort_axpy(&t2, PSIF_CC_TAMPS, pqsr, 0, 5, "2 tIjAb - tIjBa", -1);
    dpd_buf4_close(&t2);

    /* 2 T(IA,jb) - t(IB,ja) */
    dpd_buf4_init(&t2, PSIF_CC_TAMPS, 0, 10, 10, 10, 10, 0, "tIAjb");
    dpd_buf4_scmcopy(&t2, PSIF_CC_TAMPS, "2 tIAjb - tIBja", 2);
    dpd_buf4_close(&t2);
    dpd_buf4_init(&t2, PSIF_CC_TAMPS, 0, 10, 10, 10, 10, 0, "2 tIAjb - tIBja");
    dpd_buf4_init(&t2B, PSIF_CC_TAMPS, 0, 10, 10, 10, 10, 0, "tIbjA");
    ...
    dpd_buf4_sort(&t2, PSIF_CC_TAMPS, rspq, 27, 24, "tiBJa");
    dpd_buf4_close(&t2);
