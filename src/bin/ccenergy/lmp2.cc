...
void lmp2(void)
{
  ...
  dpd_->file2_copy(&fij, PSIF_CC_OEI, "fIJ (non-diagonal)");
  dpd_->file2_close(&fij);

  dpd_->file2_init(&fij, PSIF_CC_OEI, 0, 0, 0, "fIJ (non-diagonal)");
  ...
  dpd_->buf4_copy(&D, PSIF_CC_TAMPS, "LMP2 tIjAb");
  dpd_->buf4_close(&D);

  dpd_->buf4_init(&T2, PSIF_CC_TAMPS, 0, 0, 5, 0, 5, 0, "LMP2 tIjAb");
  if(params.local) {
    local_filter_T2(&T2);
  }
  else {
  ...
  for(iter=1; iter < lmp2_maxiter; iter++) {
    ...
    dpd_->buf4_init(&D, PSIF_CC_DINTS, 0, 0, 5, 0, 5, 0, "D <ij|ab>");
    dpd_->buf4_copy(&D, PSIF_CC_TAMPS, "New LMP2 tIjAb Increment");
    dpd_->buf4_close(&D);

    dpd_->buf4_init(&newT2, PSIF_CC_TAMPS, 0, 0, 5, 0, 5, 0, "New LMP2 tIjAb Increment");
    dpd_->buf4_init(&T2, PSIF_CC_TAMPS, 0, 0, 5, 0, 5, 0, "LMP2 tIjAb");

    dpd_->file2_init(&fij, PSIF_CC_OEI, 0, 0, 0, "fIJ");
    dpd_->contract424(&T2, &fij, &newT2, 1, 0, 1, -1, 1);
    dpd_->contract244(&fij, &T2, &newT2, 0, 0, 0, -1, 1);
    dpd_->file2_close(&fij);

    dpd_->file2_init(&fab, PSIF_CC_OEI, 0, 1, 1, "fAB");
    dpd_->contract244(&fab, &T2, &newT2, 1, 2, 1, 1, 1);
    dpd_->contract424(&T2, &fab, &newT2, 3, 1, 0, 1, 1);
    dpd_->file2_close(&fab);

    dpd_->buf4_copy(&T2, PSIF_CC_TAMPS, "New LMP2 tIjAb");
    dpd_->buf4_close(&T2);
    ...
    dpd_->buf4_init(&newT2, PSIF_CC_TAMPS, 0, 0, 5, 0, 5, 0, "New LMP2 tIjAb");
    dpd_->buf4_init(&T2, PSIF_CC_TAMPS, 0, 0, 5, 0, 5, 0, "New LMP2 tIjAb Increment");
    ...
    dpd_->buf4_init(&newT2, PSIF_CC_TAMPS, 0, 0, 5, 0, 5, 0, "New LMP2 tIjAb");
    dpd_->buf4_mat_irrep_init(&newT2, 0);
    dpd_->buf4_mat_irrep_rd(&newT2, 0);
    ...
    dpd_->buf4_mat_irrep_close(&newT2, 0);
    dpd_->buf4_close(&T2);
    dpd_->buf4_close(&newT2);
    ...
    if(rms < params.convergence) {
      ...
      dpd_->buf4_init(&T2, PSIF_CC_TAMPS, 0, 0, 5, 0, 5, 0, "New LMP2 tIjAb");
      dpd_->buf4_copy(&T2, PSIF_CC_TAMPS, "LMP2 tIjAb");
      dpd_->buf4_close(&T2);
    }
  }
  ...
  dpd_->buf4_init(&D, PSIF_CC_DINTS, 0, 0, 5, 0, 5, 0, "D 2<ij|ab> - <ij|ba>");
  dpd_->buf4_mat_irrep_init(&D, 0);
  dpd_->buf4_mat_irrep_rd(&D, 0);
  dpd_->buf4_init(&T2, PSIF_CC_TAMPS, 0, 0, 5, 0, 5, 0, "New LMP2 tIjAb");
  dpd_->buf4_mat_irrep_init(&T2, 0);
  dpd_->buf4_mat_irrep_rd(&T2, 0);
