...
ADC::rhf_construct_sigma(int irrep, int root)
{
    ...
    dpd_->buf4_init(&A, PSIF_ADC_SEM, 0, ID("[O,V]"), ID("[O,V]"), ID("[O,V]"), ID("[O,V]"), 0, "A3h3p1234");
    dpd_->contract422(&A, &B, &S, 0, 0, 1, 0);
    dpd_->buf4_close(&A);
    
    // Evaluation of the remaining one 3h-3p diagram
    dpd_->buf4_init(&K, PSIF_ADC,          0, ID("[O,O]"), ID("[V,V]"), ID("[O,O]"), ID("[V,V]"), 0, "2 K1234 - K1243");
    dpd_->buf4_init(&V, PSIF_LIBTRANS_DPD, 0, ID("[O,O]"), ID("[V,V]"), ID("[O,O]"), ID("[V,V]"), 0, "MO Ints 2 V1234 - V1243");
    
    sprintf(lbl, "DOV_[%d]12", irrep);
    dpd_->file2_init(&D, PSIF_ADC_SEM, irrep, ID('O'), ID('V'), lbl);
    // D_{ia} <-- \sum_{jb} (2 <ij|ab> - <ij|ba>) b_{jb}
    dpd_->dot24(&B, &V, &D, 0, 0, 1, 0);
    // \sigma_{ia} <-- 0.5 \sum_{jb} (2 K_{ijab} - K_{ijba}) D_{jb}
    dpd_->dot24(&D, &K, &S, 0, 0, 0.5, 1);
    dpd_->file2_close(&D);
    
    sprintf(lbl, "EOV_[%d]12", irrep);
    dpd_->file2_init(&E, PSIF_ADC_SEM, irrep, ID('O'), ID('V'), lbl);
    // E_{ia} <-- \sum_{jb} (2 K_{ijab} - K_{ijba}) b_{jb}
    dpd_->dot24(&B, &K, &E, 0, 0, 1, 0);
    // \sigma_{ia} <-- \sum_{jb} (2 <ij|ab> - <ij|ba>) E_{jb}
    dpd_->dot24(&E, &V, &S, 0, 0, 0.5, 1);
    dpd_->file2_close(&E);
    ...
    dpd_->buf4_init(&Z, PSIF_ADC_SEM, irrep, ID("[O,O]"), ID("[V,V]"), ID("[O,O]"), ID("[V,V]"), 0, lbl);
    // ZOVOV_{jiab} <--  \sum_{c} <jc|ab> b_{ic}
    dpd_->contract424(&V, &B, &Z, 1, 1, 1,  1, 0);
    dpd_->buf4_close(&V);
    
    dpd_->buf4_init(&V, PSIF_LIBTRANS_DPD, 0, ID("[O,O]"), ID("[V,O]"), ID("[O,O]"), ID("[V,O]"), 0, "MO Ints <OO|VO>");
    // ZOVOV_{ijab} <-- - \sum_{k} <ij|ak> b_{kb}
    dpd_->contract424(&V, &B, &Z, 3, 0, 0, -1, 1);
    dpd_->buf4_close(&V);
    
    // B_{iajb} <-- (2Z_{ijab}-Z_{ijba}+2Z_{jiab}-Z_{jiba}) / (\omega+e_i-e_a+e_j-e_b)
    sprintf(lbl, "BOOVV_[%d]1234", irrep);
    dpd_->buf4_scmcopy(&Z, PSIF_ADC_SEM, lbl, 2.0);
    dpd_->buf4_sort_axpy(&Z, PSIF_ADC_SEM, pqsr, ID("[O,O]"), ID("[V,V]"), lbl, -1.0);
    dpd_->buf4_sort_axpy(&Z, PSIF_ADC_SEM, qprs, ID("[O,O]"), ID("[V,V]"), lbl, -1.0);
    dpd_->buf4_sort_axpy(&Z, PSIF_ADC_SEM, qpsr, ID("[O,O]"), ID("[V,V]"), lbl,  2.0);
    dpd_->buf4_close(&Z);
    
    dpd_->buf4_init(&Z, PSIF_ADC_SEM, irrep, ID("[O,O]"), ID("[V,V]"), ID("[O,O]"), ID("[V,V]"), 0, lbl);
    sprintf(lbl, "D_[%d]1234", irrep);
    dpd_->buf4_init(&A, PSIF_ADC_SEM, irrep, ID("[O,O]"), ID("[V,V]"), ID("[O,O]"), ID("[V,V]"), 0, lbl);
    dpd_->buf4_dirprd(&A, &Z);
    dpd_->buf4_close(&A);
 
    dpd_->buf4_init(&V, PSIF_LIBTRANS_DPD, 0, ID("[O,V]"), ID("[V,V]"), ID("[O,V]"), ID("[V,V]"), 0, "MO Ints <OV|VV>");
    // \sigma_{ia} <-- \sum_{jbc} B_{jicb} <ja|cb>
    dpd_->contract442(&Z, &V, &S, 1, 1, 1, 1);
    dpd_->buf4_close(&V);
    
    dpd_->buf4_init(&V, PSIF_LIBTRANS_DPD, 0, ID("[O,O]"), ID("[V,O]"), ID("[O,O]"), ID("[V,O]"), 0, "MO Ints <OO|VO>");
    // \sigma_{ia} <-- - \sum_{jkb} <kj|bi> B_{jkab}
    dpd_->contract442(&V, &Z, &S, 3, 3, -1, 1); //This is genuine
    dpd_->buf4_close(&V);
    dpd_->buf4_close(&Z);
