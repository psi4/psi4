...
ADC::rhf_init_tensors()
{
    ...
    dpd_->buf4_sort(&V, PSIF_LIBTRANS_DPD, prqs, ID("[O,O]"), ID("[V,V]"), "MO Ints <OO|VV>");
    dpd_->buf4_close(&V);
    dpd_->buf4_init(&V, PSIF_LIBTRANS_DPD, 0, ID("[O,O]"), ID("[V,V]"), ID("[O,O]"), ID("[V,V]"), 0, "MO Ints <OO|VV>");
    dpd_->buf4_sort(&V, PSIF_LIBTRANS_DPD, pqsr, ID("[O,O]"), ID("[V,V]"), "MO Ints V1243");
    dpd_->buf4_scmcopy(&V, PSIF_LIBTRANS_DPD, "MO Ints 2 V1234 - V1243", 2.0);
    dpd_->buf4_copy(&V, PSIF_ADC, "K1234");
    dpd_->buf4_init(&K, PSIF_LIBTRANS_DPD, 0, ID("[O,O]"), ID("[V,V]"), ID("[O,O]"), ID("[V,V]"), 0, "MO Ints 2 V1234 - V1243");
    dpd_->buf4_init(&V, PSIF_LIBTRANS_DPD, 0, ID("[O,O]"), ID("[V,V]"), ID("[O,O]"), ID("[V,V]"), 0, "MO Ints V1243)");
    dpd_->buf4_axpy(&V, &K, -1);
    dpd_->buf4_close(&V);
    dpd_->buf4_close(&K);

    dpd_->buf4_init(&K, PSIF_ADC, 0, ID("[O,O]"), ID("[V,V]"), ID("[O,O]"), ID("[V,V]"), 0, "K1234");
    ...
    for(int h = 0;h < nirrep_;h++){
        ...
        dpd_->buf4_mat_irrep_wrt(&K, h);
        dpd_->buf4_mat_irrep_close(&K, h);
    }
    dpd_->buf4_sort(&K, PSIF_ADC, pqsr, ID("[O,O]"), ID("[V,V]"), "K1243");
    dpd_->buf4_scmcopy(&K, PSIF_ADC, "2 K1234 - K1243", 2.0);
    dpd_->buf4_close(&K);
    dpd_->buf4_init(&K, PSIF_ADC, 0, ID("[O,O]"), ID("[V,V]"), ID("[O,O]"), ID("[V,V]"), 0, "2 K1234 - K1243");
    dpd_->buf4_init(&V, PSIF_ADC, 0, ID("[O,O]"), ID("[V,V]"), ID("[O,O]"), ID("[V,V]"), 0, "K1243");
    dpd_->buf4_axpy(&V, &K, -1);
    dpd_->buf4_close(&V);
    dpd_->buf4_init(&V, PSIF_LIBTRANS_DPD, 0, ID("[O,O]"), ID("[V,V]"), ID("[O,O]"), ID("[V,V]"), 0, "MO Ints <OO|VV>");
    energy = dpd_->buf4_dot(&V, &K);
    dpd_->buf4_close(&V);
    dpd_->buf4_init(&V, PSIF_ADC, 0, ID("[O,O]"), ID("[V,V]"), ID("[O,O]"), ID("[V,V]"), 0, "K1234");
    sq_norm = 1 + dpd_->buf4_dot(&V, &K);
    ...
    dpd_->file2_init(&A, PSIF_ADC, 0, ID('O'), ID('O'), "RHO_OO");
    dpd_->contract442(&V, &K, &A, 0, 0, 1, 0);
    dpd_->buf4_close(&K);
    fflush(outfile);
    
    dpd_->file2_mat_init(&A);
    dpd_->file2_mat_rd(&A);
    dpd_->buf4_init(&K, PSIF_ADC, 0, ID("[O,O]"), ID("[V,V]"), ID("[O,O]"), ID("[V,V]"), 0, "tilde K1234");
    
    for(int h = 0;h < nirrep_;h++){
        dpd_->buf4_mat_irrep_init(&V, h);
        dpd_->buf4_mat_irrep_init(&K, h);
        dpd_->buf4_mat_irrep_rd(&V, h);
        ...
        dpd_->buf4_mat_irrep_wrt(&K, h);
        dpd_->buf4_mat_irrep_close(&K, h);
        dpd_->buf4_mat_irrep_close(&V, h);
    }
    dpd_->buf4_close(&V);
    dpd_->file2_mat_close(&A);
    dpd_->file2_close(&A);

    dpd_->buf4_sort(&K, PSIF_ADC, pqsr, ID("[O,O]"), ID("[V,V]"), "tilde K1243");
    dpd_->buf4_scmcopy(&K, PSIF_ADC, "tilde 2 K1234 - K1243", 2.0);
    dpd_->buf4_close(&K);
    dpd_->buf4_init(&K, PSIF_ADC, 0, ID("[O,O]"), ID("[V,V]"), ID("[O,O]"), ID("[V,V]"), 0, "tilde 2 K1234 - K1243");
    dpd_->buf4_init(&V, PSIF_ADC, 0, ID("[O,O]"), ID("[V,V]"), ID("[O,O]"), ID("[V,V]"), 0, "tilde K1243");
    dpd_->buf4_axpy(&V, &K, -1.0);
    dpd_->buf4_close(&V);
    dpd_->buf4_init(&V, PSIF_LIBTRANS_DPD, 0, ID("[O,O]"), ID("[V,V]"), ID("[O,O]"), ID("[V,V]"), 0, "MO Ints <OO|VV>");
    ePR2 = dpd_->buf4_dot(&V, &K);
    dpd_->buf4_close(&V);
    dpd_->buf4_init(&V, PSIF_ADC, 0, ID("[O,O]"), ID("[V,V]"), ID("[O,O]"), ID("[V,V]"), 0, "tilde K1234");
    sq_norm = 1 + dpd_->buf4_dot(&V, &K);
    dpd_->buf4_close(&K);
    dpd_->buf4_close(&V);
    ...
    dpd_->buf4_init(&V, PSIF_LIBTRANS_DPD, 0, ID("[O,O]"), ID("[V,V]"), ID("[O>=O]+"), ID("[V>=V]+"), 0, "MO Ints (OO|VV)");
    dpd_->buf4_sort(&V, PSIF_LIBTRANS_DPD, prqs, ID("[O,V]"), ID("[O,V]"), "MO Ints <OV|OV>");
    dpd_->buf4_close(&V);
    
    // Sort(pqrs): <OO|VO> <-- (OV|OO)
    dpd_->buf4_init(&V, PSIF_LIBTRANS_DPD, 0, ID("[O,V]"), ID("[O,O]"), ID("[O,V]"), ID("[O>=O]+"), 0, "MO Ints (OV|OO)");
    dpd_->buf4_sort(&V, PSIF_LIBTRANS_DPD, prqs, ID("[O,O]"), ID("[V,O]"), "MO Ints <OO|VO>");
    dpd_->buf4_close(&V);

    // Sort(prqs): <OV|VV> <-- (OV|VV)
    dpd_->buf4_init(&V, PSIF_LIBTRANS_DPD, 0, ID("[O,V]"), ID("[V,V]"), ID("[O,V]"), ID("[V>=V]+"), 0, "MO Ints (OV|VV)");
    dpd_->buf4_sort(&V, PSIF_LIBTRANS_DPD, prqs, ID("[O,V]"), ID("[V,V]"), "MO Ints <OV|VV>");
    dpd_->buf4_close(&V);
