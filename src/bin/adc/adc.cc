...
{   
    ...
    psi::fprintf(outfile,     "\t*****************************************************\n");
    for(int h = 0; h < nirrep_; h++){
        psi::fprintf(outfile, "\t %3s   %3d   %3d   %3d   %3d   %3d   %3d   %3d   %3d\n",
  ...
  // Setting up dimensions for each irrep block and totoal dimension of S manifold.
    ...
    psi::fprintf(outfile, "\tNEWTON_CONV = %3g, NORM_TOL = %3g\n", conv_, norm_tol_);
    psi::fprintf(outfile, "\tPOLE_MAX    = %3d, SEM_MAX  = %3d\n\n", pole_max_, sem_max_);
    
    psi::fprintf(outfile, "\tNXS           = %d\n", nxs_);
//    psi::fprintf(outfile, "\tIRREP_XYZ     = [");
//    for(int i = 0;i < 3;i++)
//        psi::fprintf(outfile, " %3s ", irreps_[irrep_axis_[i]]);
//    psi::fprintf(outfile, "]\n");
    psi::fprintf(outfile, "\tNXS_PER_IRREP = [");
    for(int i = 0;i < nirrep_;i++){
        psi::fprintf(outfile, " %d ", nxspi_[i]);
    }
    psi::fprintf(outfile, "]\n");

    if(DEBUG_){
        psi::fprintf(outfile, "Debagging mode...\n");
        psi::fprintf(outfile, "\tNMO   = %3d, NXS = %3d\n", nmo_, nxs_);
        psi::fprintf(outfile, "\tNOPEN = %3d\n", nopen_);
        psi::fprintf(outfile, "\tROOTS_PER_IRREP = [");
        for(int i = 0;i < nirrep_;i++) psi::fprintf(outfile, "%3d", rpi_[i]);
        psi::fprintf(outfile, " ]\n");
    }
