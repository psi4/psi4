...
namespace psi { namespace adc {
    ...
    */
	...
	exit(PSI_RETURN_FAILURE);
      ...
      nocc = moinfo.clsdpi[0]; /* active doubly occupied orbitals */
      nfzc = moinfo.frocc[0];  /* frozen doubly occupied orbitals */
      nocc_all = nocc + nfzc; /* all doubly occupied orbitals */
      nvir = moinfo.virpi[0]; /* active virtual orbitals */
    ...
    */
    ...
    /* Initially T2-array ought to be given in Dirac notation <ij|ab> */
    void local_filter_T2(double **T2)
    {
      int ij, i, j, a, b, ab;
      int nso, nocc, nvir, nij, nab;
	...
	for(j=0; j < nocc; j++,ij++) {
	  ...
	  C_DGEMM('n', 't', nvir, nvir, pairdom_len[ij], 1.0, &(X2[0][0]), nso,
		  &(V[ij][0][0]), pairdom_len[ij], 0.0, &(T2[ij][0]), nvir);
	}
      ...
      //	psio_write_entry(PSIF_PPA, "T2", (char *)T2[0], nab*nij*sizeof(double));
      //	free_block(T2);
    }
    
    void local_filter_U2(double **T2, double lambda)
    {
      int ij, i, j, a, b, ab;
      int nso, nocc, nvir;
      int *pairdom_len, *pairdom_nrlen, *weak_pairs;
    ...
    void make_fock(double **C, double **F)
    {
      int i, j, k, mu, nu, nocc, nso, noei, ntei;
      int ij, munu, imu, jnu, ijmunu, imujnu;
      double **Temp, **D, *V, *T, *Vtw, tmp = 0;
      ...
      /* Usage of BLAS is not quite perfect, containing some redundant step */
      D = block_matrix(nso,nso);
      C_DGEMM('n', 't', nso, nso, nocc, 1, &(C[0][0]), nso, &(C[0][0]), 
	      nso, 1, &(D[0][0]), nso);
      
      T = init_array(noei);
      V = init_array(noei);
      iwl_rdone(PSIF_OEI, PSIF_SO_T, T, noei, 0, 0, outfile);
      iwl_rdone(PSIF_OEI, PSIF_SO_V, V, noei, 0, 0, outfile);
      for(i = 0, ij = 0;i < nso;i++){
	for(j = 0;j <= i;j++, ij++){
	  if(i==j)
	    F[i][j] = T[ij] + V[ij];
	  else 
	    F[i][j] = F[j][i] = T[ij] + V[ij];
	}
      }
      free(T);
      free(V);
      ...
      Vtw = init_array(ntei);
      iwl_rdtwo(PSIF_SO_TEI, Vtw, ioff, nso, 0, 0, 0, outfile);
      for(i = 0, ij = 0;i < nso;i++){
	for(j = 0;j <= i;j++, ij++){
	  for(mu = 0;mu < nso;mu++){
	    for(nu = 0;nu < nso;nu++){
	      ...
	      jnu = INDEX(j, nu);
	      munu = INDEX(mu, nu);
	      ijmunu = INDEX(ij, munu);
	      imujnu = INDEX(imu, jnu);
	      tmp += D[mu][nu] * (2*Vtw[ijmunu]-Vtw[imujnu]);
	    }
	  }
	  if(i==j)
	    F[i][j] += tmp;
      ...
      C_DGEMM('t', 'n', nso, nso, nso, 1, &(C[0][0]), nso, &(F[0][0]), nso, 
	      1, &(Temp[0][0]), nso);
      free_block(F);
      F = block_matrix(nso, nso);
      C_DGEMM('n', 'n', nso, nso, nso, 1, &(Temp[0][0]), nso, &(C[0][0]), nso, 
	      1, &(F[0][0]), nso);
      free_block(Temp);
      if(params.print >= 2)
	print_mat(F, nso, nso, outfile);
