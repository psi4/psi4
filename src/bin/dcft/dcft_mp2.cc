...
DCFTSolver::mp2_guess()
{
    ...
    if (guess == "MP2") {
        ...
        */
        ...
        dpd_->buf4_copy(&I, PSIF_DCFT_DPD, "Lambda <OO|VV>");
        dpd_->buf4_close(&I);
        dpd_->buf4_init(&I, PSIF_DCFT_DPD, 0, ID("[O,O]"), ID("[V,V]"),
                      ID("[O>O]-"), ID("[V>V]-"), 0, "Lambda <OO|VV>");
        dpd_->buf4_init(&D, PSIF_LIBTRANS_DPD, 0, ID("[O,O]"), ID("[V,V]"),
                      ID("[O>=O]+"), ID("[V>=V]+"), 0, "D <OO|VV>");
        dpd_->buf4_dirprd(&D, &I);
        dpd_->buf4_close(&I);
        dpd_->buf4_close(&D);
     ...
     */
        ...
        dpd_->buf4_init(&T2, PSIF_CC_TAMPS, 0, ID("[O>O]-"), ID("[V>V]-"),
                      ID("[O>O]-"), ID("[V>V]-"), 0, "tIJAB");
        dpd_->buf4_copy(&T2, PSIF_DCFT_DPD, "Lambda <OO|VV>");
        dpd_->buf4_close(&T2);
