...
void
DCFTSolver::compute_density_VVVV()
{
    psio_->open(PSIF_DCFT_DENSITY, PSIO_OPEN_OLD);
...
DCFTSolver::compute_ewdm()
{
    ...
    // We need to restrict it by scaling Gamma and switching bk_pack in global_dpd_->buf4_dump to 1
    // If restriction is used it seems that one needs to do a seperate global_dpd_->buf4_sort to the chemists' notation
    // before calling global_dpd_->buf4_dump - check that with swap23 = 0 (AYS)
    global_dpd_->buf4_init(&G, PSIF_DCFT_DENSITY, 0, ID("[V,V]"), ID("[V,V]"),
              ID("[V>V]-"), ID("[V>V]-"), 0, "Gamma <VV|VV>");
