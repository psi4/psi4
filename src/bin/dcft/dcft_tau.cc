...
DCFTSolver::build_tau()
{
    ...
    dpd_->buf4_init(&L1, PSIF_DCFT_DPD, 0,
                  _ints->DPD_ID("[O,O]"), _ints->DPD_ID("[V,V]"),
                  ID("[O>O]-"), ID("[V>V]-"), 0, "Lambda <OO|VV>");
    dpd_->buf4_init(&L2, PSIF_DCFT_DPD, 0,
                  _ints->DPD_ID("[O,O]"), _ints->DPD_ID("[V,V]"),
                  ID("[O>O]-"), ID("[V>V]-"), 0, "Lambda <OO|VV>");
    /*
     * Tau_IJ = -1/2 Lambda_IKAB Lambda_JKAB
     */
    dpd_->contract442(&L1, &L2, &T_OO, 0, 0, -0.5, 0.0);
    /*
     * Tau_AB = +1/2 Lambda_IJAC Lambda_IJBC
     */
    dpd_->contract442(&L1, &L2, &T_VV, 2, 2, 0.5, 0.0);
    dpd_->buf4_close(&L1);
    dpd_->buf4_close(&L2);
...
DCFTSolver::refine_tau() {
    ...
    dpd_->file2_mat_wrt(&T_OO);
    dpd_->file2_mat_wrt(&T_oo);
    dpd_->file2_mat_wrt(&T_VV);
    dpd_->file2_mat_wrt(&T_vv);
