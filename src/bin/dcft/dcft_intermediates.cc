...
DCFTSolver::build_cumulant_intermediates()
{
    ...
    if(options_.get_str("AO_BASIS") == "NONE"){
        ...
        dpd_->contract444(&L, &I, &G, 0, 0, 1.0, 1.0);
        dpd_->buf4_close(&I);
        dpd_->buf4_close(&L);
        dpd_->buf4_close(&G);
    ...
    dpd_->buf4_init(&G, PSIF_DCFT_DPD, 0, ID("[O>O]-"), ID("[V>V]-"),
                  ID("[O>O]-"), ID("[V>V]-"), 0, "G <OO|VV>");
    dpd_->contract444(&I, &L, &G, 0, 1, 1.0, 1.0);
    dpd_->buf4_close(&I);
    dpd_->buf4_close(&L);
    dpd_->buf4_close(&G);
    ...
    dpd_->buf4_init(&Laa, PSIF_DCFT_DPD, 0, ID("[O,O]"), ID("[V,V]"),
                  ID("[O>O]-"), ID("[V>V]-"), 0, "Lambda <OO|VV>");
    dpd_->buf4_sort(&Laa, PSIF_DCFT_DPD, prqs, ID("[O,V]"), ID("[O,V]"), "Lambda (OV|OV)");
    dpd_->buf4_close(&Laa);
    ...
    dpd_->buf4_init(&Lab, PSIF_DCFT_DPD, 0, ID("[O,o]"), ID("[V,v]"),
                  ID("[O,o]"), ID("[V,v]"), 0, "Lambda <Oo|Vv>");
    dpd_->buf4_sort(&Lab, PSIF_DCFT_DPD, psqr, ID("[O,v]"), ID("[o,V]"), "Lambda (Ov|oV)");
    dpd_->buf4_close(&Lab);
    ...
    dpd_->buf4_init(&Lbb, PSIF_DCFT_DPD, 0, ID("[o,o]"), ID("[v,v]"),
                  ID("[o>o]-"), ID("[v>v]-"), 0, "Lambda <oo|vv>");
    dpd_->buf4_sort(&Lbb, PSIF_DCFT_DPD, prqs, ID("[o,v]"),ID("[o,v]"), "Lambda (ov|ov)");
    dpd_->buf4_close(&Lbb);
    ...
    dpd_->contract444(&Lab, &I, &Tab, 0, 0, -1.0, 0.0);
    ...
    dpd_->contract444(&I, &Lab, &Tab, 0, 1, -1.0, 1.0);
    ...
    dpd_->buf4_sort(&Lab, PSIF_DCFT_DPD, psrq, ID("[O,V]"),ID("[o,v]"), "Lambda (OV|ov)");
    dpd_->buf4_close(&Lab);
    dpd_->buf4_init(&Lab, PSIF_DCFT_DPD, 0, ID("[O,V]"), ID("[o,v]"),
                  ID("[O,V]"), ID("[o,v]"), 0, "Lambda (OV|ov)");
    ...
    dpd_->contract444(&Lab, &I, &Taa, 0, 0, 1.0, 0.0);

    // T_iajb = Sum_KC lambda_KCia g_KCjb
    dpd_->contract444(&Lab, &I, &Tbb, 1, 1, 1.0, 0.0);

    // T_IAjb += Sum_kc g_IAkc lambda_jbkc
    dpd_->contract444(&I, &Lbb, &Tab, 0, 0, 1.0, 1.0);

    // T_IAjb += Sum_KC lambda_IAKC g_KCjb
    dpd_->contract444(&Laa, &I, &Tab, 0, 1, 1.0, 1.0);
    ...
    // T_IAJB -= Sum_KC lambda_IAKC g_JBKC
    dpd_->contract444(&Laa, &I, &Taa, 0, 0, -1.0, 1.0);
    ...
    // T_iajb -= Sum_kc lambda_iakc g_jbkc
    dpd_->contract444(&Lbb, &I, &Tbb, 0, 0, -1.0, 1.0);

    // T_IAjb -= Sum_KC lambda_IAkc g_jbkc
    dpd_->contract444(&Lab, &I, &Tab, 0, 0, -1.0, 1.0);
    dpd_->buf4_close(&I);
...
DCFTSolver::compute_F_intermediate() {
    ...
    */
    ...
    dpd_->contract244(&F_VV, &Laa, &T, 1, 2, 1, 1.0, 0.0);
    ...
    dpd_->contract244(&F_OO, &Laa, &T, 1, 0, 0, -1.0, 0.0);
    ...
    dpd_->file2_init(&F_VV, PSIF_LIBTRANS_DPD, 0, ID('V'), ID('V'), "F <V|V>");
    dpd_->contract244(&F_VV, &Lab, &F, 1, 2, 1, 1.0, 0.0);
    dpd_->file2_close(&F_VV);
    // F_IjAb += lambda_IjAc F_bc
    dpd_->file2_init(&F_vv, PSIF_LIBTRANS_DPD, 0, ID('v'), ID('v'), "F <v|v>");
    dpd_->contract424(&Lab, &F_vv, &F, 3, 1, 0, 1.0, 1.0);
    dpd_->file2_close(&F_vv);
    // F_IjAb -= lambda_KjAb F_IK
    dpd_->file2_init(&F_OO, PSIF_LIBTRANS_DPD, 0, ID('O'), ID('O'), "F <O|O>");
    dpd_->contract244(&F_OO, &Lab, &F, 1, 0, 0, -1.0, 1.0);
    dpd_->file2_close(&F_OO);
    // F_IjAb -= lambda_IkAb F_jk
    dpd_->file2_init(&F_oo, PSIF_LIBTRANS_DPD, 0, ID('o'), ID('o'), "F <o|o>");
    dpd_->contract424(&Lab, &F_oo, &F, 1, 1, 1, -1.0, 1.0);
    dpd_->file2_close(&F_oo);
    dpd_->buf4_close(&Lab);
    dpd_->buf4_close(&F);
    ...
    // Temp_ijab = lambda_ijcb F_ac
    dpd_->file2_init(&F_vv, PSIF_LIBTRANS_DPD, 0, ID('v'), ID('v'), "F <v|v>");
    dpd_->contract244(&F_vv, &Lbb, &T, 1, 2, 1, 1.0, 0.0);
    dpd_->file2_close(&F_vv);
    dpd_->buf4_close(&Lbb);
    dpd_->buf4_close(&T);
    dpd_->buf4_init(&T, PSIF_DCFT_DPD, 0, ID("[o>o]-"), ID("[v>v]-"),
    ...
    dpd_->file2_init(&F_oo, PSIF_LIBTRANS_DPD, 0, ID('o'), ID('o'), "F <o|o>");
    dpd_->contract244(&F_oo, &Lbb, &T, 1, 0, 0, -1.0, 0.0);
    dpd_->file2_close(&F_oo);
    dpd_->buf4_close(&Lbb);
    // F_ijab += Temp_ijab
    dpd_buf4_add(&F, &T, 1.0);
...
DCFTSolver::form_density_weighted_fock() {
    ...
    dpd_->file2_mat_init(&F_OO);
    dpd_->file2_mat_init(&F_oo);
    dpd_->file2_mat_init(&F_VV);
    dpd_->file2_mat_init(&F_vv);
    ...
    dpd_->file2_mat_wrt(&F_OO);
    dpd_->file2_mat_wrt(&F_oo);
    dpd_->file2_mat_wrt(&F_VV);
    dpd_->file2_mat_wrt(&F_vv);
