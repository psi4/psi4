...
DCFTSolver::compute_orbital_gradient_OV() {
    ...
    dpd_->contract222(&H, &T, &X, 0, 1, 1.0, 0.0);
    dpd_->file2_close(&T);
    dpd_->file2_close(&H);
    dpd_->file2_close(&X);
    ...
    // X_IA -= Y2_IC Tau_CA
    dpd_->file2_init(&X, PSIF_DCFT_DPD, 0, ID('O'), ID('V'), "X <O|V>");
    dpd_->contract222(&Y2_OV, &T_VV, &X, 0, 1, -1.0, 1.0);
    dpd_->file2_close(&X);
    ...
    dpd_->buf4_init(&W, PSIF_DCFT_DPD, 0, ID("[O,V]"), ID("[O,O]"),
                  ID("[O,V]"), ID("[O>O]-"), 0, "W <OV|OO>");
    dpd_->buf4_init(&LL, PSIF_DCFT_DPD, 0, ID("[O,O]"), ID("[V,V]"),
                  ID("[O>O]-"), ID("[V>V]-"), 0, "Lambda <OO|VV>");

    dpd_->contract442(&W, &LL, &X, 0, 2, 0.25, 1.0);
    dpd_->buf4_close(&W);
    dpd_->buf4_close(&LL);
    dpd_->file2_close(&X);
    ...
    dpd_->contract442(&W, &LL, &X, 2, 2, 0.5, 1.0);
    dpd_->buf4_close(&W);
    dpd_->buf4_close(&LL);
    dpd_->file2_close(&X);
    ...
    dpd_->contract442(&W, &LL, &X, 3, 3, 0.5, 1.0);
    dpd_->buf4_close(&W);
    dpd_->buf4_close(&LL);
    dpd_->file2_close(&X);
