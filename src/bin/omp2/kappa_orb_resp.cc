...
if (reference_ == "RESTRICTED") {
    ...
    dpd_file2_init(&P, PSIF_OMP2_DPD, 0, ID('V'), ID('O'), "P <V|O>");  
    ...
    dpd_file2_init(&S, PSIF_OMP2_DPD, 0, ID('V'), ID('O'), "Sigma <V|O>");  
    dpd_file2_init(&P, PSIF_OMP2_DPD, 0, ID('V'), ID('O'), "P <V|O>");  
    dpd_buf4_init(&K, PSIF_LIBTRANS_DPD, 0, ID("[V,O]"), ID("[V,O]"),
                  ID("[V,O]"), ID("[V,O]"), 0, "MO Ints (VO|VO)");
    ...
    dpd_file2_init(&P, PSIF_OMP2_DPD, 0, ID('V'), ID('O'), "Sigma <V|O>");  
    dpd_file2_mat_init(&P);
    dpd_file2_mat_rd(&P);
    idp_idx = 0;
    for(int h = 0; h < nirrep_; ++h){
...
void OMP2Wave::orb_resp_pcg_rhf()
{ 
...
void OMP2Wave::orb_resp_pcg_uhf()
{ 
