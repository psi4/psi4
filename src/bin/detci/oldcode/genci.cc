               ...
               int restart, int restart_vecs)
{
   ...
   if (core_memory > (3*N)) {
     ...
     switch_buf3 = 0;
     psi::fprintf(outfile,"Using himem\n");
     fflush(outfile);
     }
   if (core_memory > (2*N)) {
     buf_size2 = N;
     ...
     fflush(outfile);
     if (!buf_size3) {
       psi::fprintf(outfile,"Using core_memory > (2*N)\n");
       fflush(outfile);
     ...
     psi::fprintf(outfile,"Using lomem\n");
     fflush(outfile);
     }
   ...
   psi::fprintf(outfile,"\n");
   psi::fprintf(outfile,"memory       = %lf\n", memory);
   psi::fprintf(outfile,"num_roots    = %d\n", M);
   psi::fprintf(outfile,"maxiter      = %d\n", maxiter);
   psi::fprintf(outfile,"core_memeory = %d\n", core_memory);
   psi::fprintf(outfile,"buf_size2    = %d\n", buf_size2);
   psi::fprintf(outfile,"num_buf2     = %d\n", num_buf2);
   psi::fprintf(outfile,"extra_buf2   = %d\n", extra_buf2);
   psi::fprintf(outfile,"switch_buf2  = %d\n", switch_buf2);
   psi::fprintf(outfile,"buf_size3    = %d\n", buf_size3);
   psi::fprintf(outfile,"num_buf3     = %d\n", num_buf3);
   psi::fprintf(outfile,"extra_buf3   = %d\n", extra_buf3);
   psi::fprintf(outfile,"switch_buf3  = %d\n", switch_buf3);
   psi::fprintf(outfile,"nucrep       = %lf\n", nucrep);
   psi::fprintf(outfile,"efzc         = %lf\n", efzc);
   psi::fprintf(outfile,"\n");
   psi::fprintf(outfile,"\n");
   fflush(outfile);
 ...
 psi::fprintf(outfile,"Determining initial guess vector(s)....");
 if (restart == 1) 
 ...
 while (!converged && iter-1 <= maxiter) {
  ...
  for (i=0; i<M; i++) { /* loop over number of roots */
     ...
     tval = sqrt(tval); 
     psi::fprintf(outfile, "Iter %3d  Root %d = %13.9lf", iter-1, i+1, 
             (lambda[i] + nucrep + efzc));
     psi::fprintf(outfile, "    Delta_E %.3E   Delta_C %.3E\n",  
             lambda[i] - lastroot[i], tval);
     fflush(outfile); 
     } /* end i loop */

  if (M > 1) psi::fprintf(outfile,"\n");
  if (fabs(lambda[0]-lastroot[0]) <= conv_e && tval <= conv_rms) {
    ...
    for (i=0; i<M; i++) {
       ...
       for (k=0; k<num_buf; k++) { /* for b */
          ...
          imdet = (N > 20) ? 20 : N; 
          if (k == -1) {
            psi::fprintf(outfile,"The %d most important determinants\n",imdet);
            for (I=0; I<imdet; I++)
               psi::fprintf(outfile,"     %d   %7.6f\n", I, buffer[1][I]); 
            }
          } /* end k loop */
       if (extra_buf != 0) {
  ...
  if (L>maxnvect) {
    psi::fprintf(outfile, "(test_sem): L(%2d) > maxnvect(%2d)!",L,maxnvect);
    psi::fprintf(outfile, " Aborting!\n");
    exit(0);
    } 
 ...
 if (converged == 0) {
   ...
   psi::fprintf(outfile,"Maximum number of iterations exceded.\n");
   }
                        ...
                        PSI_FPTR byte, int print_lvl)
{
 ...
 if (!switch_buf3) { /* use large buffer method */
   ...
   for (m=0; m<num_new_vec; m++) { /* loop of number of roots */
      ...
      if (print_lvl > 5) 
        for (I=0; I<buf_size; I++) 
           psi::fprintf(outfile,"sigma[%d] = %15.7f\n", I, buffer[1][I]); 
              ...
              struct stringwr **betlist)
{
 ...
 double max, min, tmp1;
   ...
   i = strings2det(CalcInfo.ref_alp_list, CalcInfo.ref_alp_rel,
                   CalcInfo.ref_bet_list, CalcInfo.ref_bet_rel);
   psi::fprintf(outfile,"reference det = %d\n", i);
   fflush(outfile);
   if (N < h0blocksize) {
     psi::fprintf(outfile, "blocksize larger than number of determinants.\n");
     exit(0);
     } 
   ...
   psi::fprintf(outfile,"num_h0blocksize = %d\n", num_h0blocksize);
   psi::fprintf(outfile,"extra_h0blocksize = %d\n", extra_h0blocksize);
   fflush(outfile);
   ...
   /* for (I=0; I<h0blocksize; I++) {
      psi::fprintf(outfile,"detval[%d] = %lf\t detref[%d] = %d\n",
   ...
   psi::fprintf(outfile,"eigval = %20.8f\n", eigval[0]);
   psi::fprintf(outfile,"efzc   = %20.8f\n", efzc);
   psi::fprintf(outfile,"nucrep = %20.8f\n", nucrep);
   psi::fprintf(outfile,"*** H0 Block Eigenvalue = %20.8f\n\n",
