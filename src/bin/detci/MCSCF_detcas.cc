...
void form_independent_pairs(void)
{
  ...
  IndPairs.set(CalcInfo.nirreps, MAX_RAS_SPACES, CalcInfo.ras_opi,
               ...
               CalcInfo.ci2relpitz, MCSCF_Parameters.ignore_ras_ras, MCSCF_Parameters.ignore_fz);
    ...
    if (MCSCF_Parameters.hessian == "FULL") {
      ...
      if (MCSCF_Parameters.level_shift) {
        while (hess_det < MCSCF_Parameters.determ_min) {
          ...
          for (i=0; i<npairs; i++) {
            CalcInfo.mo_hess[i][i] += MCSCF_Parameters.shift;
            for (j=0; j<npairs; j++) {
              hess_copy[i][j] = CalcInfo.mo_hess[i][j];
            }
          }
...
void scale_gradient(void)
{
  ...
  if (MCSCF_Parameters.scale_step != 1.0) {
    for (pair=0; pair<npairs; pair++) 
      CalcInfo.theta_step[pair] *= MCSCF_Parameters.scale_step;
  }
...
int take_step(void)
{
  ...
  if (MCSCF_Parameters.force_step) {
    CalcInfo.theta_cur[MCSCF_Parameters.force_pair] = MCSCF_Parameters.force_value;
  ...
  if (CalcInfo.mcscf_iter >= MCSCF_Parameters.diis_start) 
