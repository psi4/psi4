...
if (reference_ == "RESTRICTED") {
    ...
    // T(IA,JB) => T_IJ^AB(2)
    dpd_->buf4_sort(&T, PSIF_OCC_DPD , prqs, ID("[O,O]"), ID("[V,V]"), "T2_2 <IJ|AB>");
    dpd_->buf4_close(&T);
    ...
    // T(JA,IB) => T_IJ^AB(2)
    dpd_->buf4_sort(&T, PSIF_OCC_DPD , rpqs, ID("[O,O]"), ID("[V,V]"), "T2_2 (IJ|AB)");
    dpd_->buf4_close(&T);    
    ...
    dpd_->buf4_copy(&T, PSIF_OCC_DPD, "T2_2new <OO|VV>");
    dpd_->buf4_close(&T); 
    ...
    dpd_->buf4_close(&K);
    dpd_->buf4_close(&TAA);
    dpd_->buf4_sort(&T, PSIF_OCC_DPD , rspq, ID("[O,O]"), ID("[V,V]"), "Z2_2 <OO|VV>");
    dpd_->buf4_close(&T);
    dpd_->buf4_init(&T, PSIF_OCC_DPD, 0, ID("[O,O]"), ID("[V,V]"),
                  ID("[O,O]"), ID("[V,V]"), 0, "T2_2new <OO|VV>");
    dpd_->buf4_init(&Tp, PSIF_OCC_DPD, 0, ID("[O,O]"), ID("[V,V]"),
    ...
    dpd_->buf4_close(&D);
    if (print_ > 2) dpd_->buf4_print(&Tnew, outfile, 1);
    ...
    dpd_->buf4_init(&Tnew, PSIF_OCC_DPD, 0, ID("[O,O]"), ID("[V,V]"),
                  ID("[O,O]"), ID("[V,V]"), 0, "T2_2new <OO|VV>");
    dpd_->buf4_copy(&Tnew, PSIF_OCC_DPD, "RT2_2 <OO|VV>");
    ...
    // Reset
    dpd_->buf4_copy(&Tnew, PSIF_OCC_DPD, "T2_2 <OO|VV>");
    if (print_ > 1) dpd_->buf4_print(&Tnew, outfile, 1);
    dpd_->buf4_close(&Tnew);
    ...
    dpd_->buf4_init(&T, PSIF_OCC_DPD, 0, ID("[O,O]"), ID("[V,V]"),
                  ID("[O,O]"), ID("[V,V]"), 0, "T2_1 <OO|VV>");
    dpd_->buf4_copy(&T, PSIF_OCC_DPD, "T2 <OO|VV>");
    dpd_->buf4_close(&T);
    dpd_->buf4_init(&T, PSIF_OCC_DPD, 0, ID("[O,O]"), ID("[V,V]"),
    ...
    dpd_->buf4_close(&Tp);
     ...
     dpd_->buf4_init(&T, PSIF_OCC_DPD, 0, ID("[O,O]"), ID("[V,V]"),
                  ID("[O,O]"), ID("[V,V]"), 0, "T2_2 <OO|VV>");
     dpd_->buf4_copy(&T, PSIF_OCC_DPD, "Tau_2 <OO|VV>");
     dpd_->buf4_copy(&T, PSIF_OCC_DPD, "T2_2AA <OO|VV>");
     dpd_->buf4_sort(&T, PSIF_OCC_DPD, qprs, ID("[O,O]"), ID("[V,V]"), "T2_2jiab <OO|VV>");
     dpd_->buf4_close(&T);
