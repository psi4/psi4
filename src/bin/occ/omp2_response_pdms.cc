...
{  
 ...
 if (reference_ == "RESTRICTED") {
	...
	dpd_->file2_init(&Go, PSIF_OCC_DENSITY, 0, ID('O'), ID('O'), "G <O|O>");  
	dpd_->contract442(&T, &Tau, &Go, 0, 0, 1.0, 0.0);
	dpd_->file2_close(&Go);
	
	// G_ae = -\sum{m,n,f} t_mn^ef * tau_mn^af
	dpd_->file2_init(&Gv, PSIF_OCC_DENSITY, 0, ID('V'), ID('V'), "G <V|V>");  
	dpd_->contract442(&Tau, &T, &Gv, 2, 2, -1.0, 0.0); 
	dpd_->file2_close(&Gv);
	...
	dpd_->file2_init(&Go, PSIF_OCC_DENSITY, 0, ID('O'), ID('O'), "G <O|O>");  
	dpd_->file2_mat_init(&Go);
	dpd_->file2_mat_rd(&Go);
        #pragma omp parallel for
	for(int h = 0; h < nirrep_; ++h){
	  for(int i = 0 ; i < aoccpiA[h]; ++i){
            for(int j = 0 ; j < aoccpiA[h]; ++j){
	...
	dpd_->file2_init(&Gv, PSIF_OCC_DENSITY, 0, ID('V'), ID('V'), "G <V|V>"); 
	dpd_->file2_mat_init(&Gv);
	dpd_->file2_mat_rd(&Gv);
        #pragma omp parallel for
	for(int h = 0; h < nirrep_; ++h){
	...
	dpd_->buf4_init(&TAB, PSIF_OCC_DPD, 0, ID("[O,o]"), ID("[V,v]"),
                  ID("[O,o]"), ID("[V,v]"), 0, "T2_1 <Oo|Vv>");
	dpd_->buf4_init(&TAA2, PSIF_OCC_DPD, 0, ID("[O,O]"), ID("[V,V]"),
                  ID("[O,O]"), ID("[V,V]"), 0, "T2_1 <OO|VV>");
	dpd_->buf4_init(&TBB2, PSIF_OCC_DPD, 0, ID("[o,o]"), ID("[v,v]"),
                  ID("[o,o]"), ID("[v,v]"), 0, "T2_1 <oo|vv>");
	dpd_->buf4_init(&TAB2_, PSIF_OCC_DPD, 0, ID("[O,o]"), ID("[V,v]"),
                  ID("[O,o]"), ID("[V,v]"), 0, "T2_1 <Oo|Vv>");
	...
	dpd_->contract442(&TAA, &TAA2, &G, 0, 0, 0.5, 0.0);
	
	// G_IM += \sum{n,E,f} t_In^Ef * l_Ef^Mn = \sum{N,E,F} t_In^Ef * t_Mn^Ef
	dpd_->contract442(&TAB, &TAB2_, &G, 0, 0, 1.0, 1.0);
	dpd_->file2_close(&G);
	...
	// G_im = 1/2 \sum{n,e,f} t_in^ef * l_ef^mn = 1/2 \sum{n,e,f} t_in^ef * t_mn^ef
	dpd_->file2_init(&G, PSIF_OCC_DENSITY, 0, ID('o'), ID('o'), "G <o|o>");  
	dpd_->contract442(&TBB, &TBB2, &G, 0, 0, 0.5, 0.0);
	
	// G_im  += \sum{N,e,F} t_Ni^Fe * l_Fe^Nm = \sum{N,e,F} t_Ni^Fe * t_Nm^Fe
	dpd_->contract442(&TAB, &TAB2_, &G, 1, 1, 1.0, 1.0);
	dpd_->file2_close(&G);
	...
	dpd_->contract442(&TAA, &TAA2, &G, 2, 2, -0.5, 0.0); 
	
	// G_EA += - \sum{M,n,f} t_Mn^Af * l_Ef^Mn = - \sum{M,n,f} t_Mn^Af * t_Mn^Ef
	dpd_->contract442(&TAB, &TAB2_, &G, 2, 2, -1.0, 1.0); 
	dpd_->file2_close(&G);
	
	// Beta-Beta spin case
	// G_ea = -1/2 \sum{m,n,f} t_mn^af * l_ef^mn = -1/2 \sum{m,n,f} t_mn^af * t_mn^ef
	dpd_->file2_init(&G, PSIF_OCC_DENSITY, 0, ID('v'), ID('v'), "G <v|v>");  
	dpd_->contract442(&TBB, &TBB2, &G, 2, 2, -0.5, 0.0); 
	
	// G_ea += - \sum{M,n,F} t_Mn^Fa * l_Fe^Mn = - \sum{M,n,F} t_Mn^Fa * t_Mn^Fe
	dpd_->contract442(&TAB, &TAB2_, &G, 3, 3, -1.0, 1.0); 
	dpd_->file2_close(&G);
	...
	dpd_->buf4_close(&TBB);
	dpd_->buf4_close(&TAB);
	dpd_->buf4_close(&TAA2);
	dpd_->buf4_close(&TBB2);
	dpd_->buf4_close(&TAB2_);
