...
if (reference_ == "RESTRICTED") {
    ...
    dpd_->buf4_copy(&K, PSIF_OCC_DPD, "W (OV|OV)");
    dpd_->buf4_close(&K);
...
//===========================================================================================
    ...
    */
    ...
    dpd_->buf4_sort(&K, PSIF_OCC_DPD , psqr, ID("[O,v]"), ID("[O,v]"), "W (Ov|Ov)");
    dpd_->buf4_close(&K);
    dpd_->buf4_init(&W, PSIF_OCC_DPD, 0, ID("[O,v]"), ID("[O,v]"),
                  ID("[O,v]"), ID("[O,v]"), 0, "W (Ov|Ov)");
    dpd_->buf4_scm(&W, -1.0);
    ...
    dpd_->buf4_sort(&K, PSIF_OCC_DPD , rqsp, ID("[o,V]"), ID("[o,V]"), "W (oV|oV)");
    dpd_->buf4_close(&K);
    dpd_->buf4_init(&W, PSIF_OCC_DPD, 0, ID("[o,V]"), ID("[o,V]"),
                  ID("[o,V]"), ID("[o,V]"), 0, "W (oV|oV)");
    dpd_->buf4_scm(&W, -1.0);
    ...
    dpd_->buf4_init(&W, PSIF_OCC_DPD, 0, ID("[O,V]"), ID("[o,v]"),
                  ID("[O,V]"), ID("[o,v]"), 0, "W (OV|ov)");
    dpd_->buf4_sort(&W, PSIF_OCC_DPD , rspq, ID("[o,v]"), ID("[O,V]"), "W (ov|OV)");
    dpd_->buf4_close(&W);
