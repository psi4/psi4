...
if (reference_ == "RESTRICTED") {
    ...
    dpd_->buf4_copy(&K, PSIF_OCC_DPD, "T <OO|VV>");
    dpd_->buf4_close(&K);
    ...
    dpd_->buf4_close(&D);
     ...
     dpd_->buf4_copy(&T, PSIF_OCC_DPD, "TAA <OO|VV>");
     dpd_->buf4_sort(&T, PSIF_OCC_DPD, qprs, ID("[O,O]"), ID("[V,V]"), "Tjiab <OO|VV>");
...
if (reference_ == "RESTRICTED") {
    ...
    dpd_->buf4_init(&K, PSIF_LIBTRANS_DPD, 0, ID("[O,O]"), ID("[V,V]"),
                  ID("[O,O]"), ID("[V,V]"), 0, "MO Ints <OO|VV>");
    dpd_->buf4_copy(&K, PSIF_OCC_DPD, "Tnew <OO|VV>");
    dpd_->buf4_close(&K);
    ...
    dpd_->buf4_close(&D);
     ...
     // Build Tau(ij,ab) = 2*T(ij,ab) - T(ji,ab)
     // Build TAA(ij,ab) = T(ij,ab) - T(ji,ab)
     dpd_->buf4_copy(&Tnew, PSIF_OCC_DPD, "Tau <OO|VV>");
     dpd_->buf4_copy(&Tnew, PSIF_OCC_DPD, "TAA <OO|VV>");
     dpd_->buf4_sort(&Tnew, PSIF_OCC_DPD, qprs, ID("[O,O]"), ID("[V,V]"), "Tjiab <OO|VV>");
     dpd_->buf4_init(&Tau, PSIF_OCC_DPD, 0, ID("[O,O]"), ID("[V,V]"),
                  ID("[O,O]"), ID("[V,V]"), 0, "Tau <OO|VV>");
     dpd_->buf4_init(&Tss, PSIF_OCC_DPD, 0, ID("[O,O]"), ID("[V,V]"),
                  ID("[O,O]"), ID("[V,V]"), 0, "TAA <OO|VV>");
    ...
    dpd_->buf4_copy(&Tnew, PSIF_OCC_DPD, "Residual_T <OO|VV>");
    ...
    // Reset
    dpd_->buf4_copy(&Tnew, PSIF_OCC_DPD, "T <OO|VV>");
...
//===========================================================================================
    ...
    dpd_->buf4_init(&K, PSIF_LIBTRANS_DPD, 0, ID("[O,O]"), ID("[V,V]"),
                  ID("[O,O]"), ID("[V,V]"), 0, "MO Ints <OO||VV>");
    dpd_->buf4_copy(&K, PSIF_OCC_DPD, "T2_1new <OO|VV>");
    dpd_->buf4_close(&K);
    ...
    dpd_->buf4_init(&Tnew, PSIF_OCC_DPD, 0, ID("[O,O]"), ID("[V,V]"),
                  ID("[O,O]"), ID("[V,V]"), 0, "T2_1new <OO|VV>");
    dpd_->buf4_copy(&Tnew, PSIF_OCC_DPD, "RT2_1 <OO|VV>");
    dpd_->buf4_init(&R, PSIF_OCC_DPD, 0, ID("[O,O]"), ID("[V,V]"),
    ...
    // Reset
    dpd_->buf4_copy(&Tnew, PSIF_OCC_DPD, "T2_1 <OO|VV>");
    if (print_ > 1) dpd_->buf4_print(&Tnew, outfile, 1);
    dpd_->buf4_close(&Tnew);
