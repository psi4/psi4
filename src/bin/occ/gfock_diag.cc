...
if (reference_ == "RESTRICTED") {
	...
	// Build Fij
	dpd_->file2_init(&GF, PSIF_OCC_DENSITY, 0, ID('O'), ID('O'), "GF <O|O>");  
	...
	// Build Fab
	dpd_->file2_init(&GF, PSIF_OCC_DENSITY, 0, ID('V'), ID('V'), "GF <V|V>"); 
...
if (wfn_type_ != "OMP2") { 
   ...
   if (twopdm_abcd_type == "DIRECT" ) {
 	...
 	// X_MNFA = 2\sum{E,C} [2t_MN^CE(1) - t_MN^EC(1)] * <FA|CE>   
        dpd_->buf4_init(&X, PSIF_OCC_DENSITY, 0, ID("[V,V]"), ID("[O,O]"),
        ...
        dpd_->contract444(&K, &T, &X, 0, 0, 2.0, 0.0);
	dpd_->buf4_close(&K);
	dpd_->buf4_close(&T);
        dpd_->buf4_sort(&X, PSIF_OCC_DENSITY, rspq, ID("[O,O]"), ID("[V,V]"), "X <OO|VV>");
	dpd_->buf4_close(&X);

        // OMP2.5
        if (wfn_type_ == "OMP2.5") { 
        dpd_->buf4_init(&X, PSIF_OCC_DENSITY, 0, ID("[O,O]"), ID("[V,V]"),
...
if (wfn_type_ != "OMP2") { 
   if (twopdm_abcd_type == "DIRECT" ) {
	...
	dpd_->buf4_init(&K, PSIF_LIBTRANS_DPD, 0, ints->DPD_ID("[V,V]"), ints->DPD_ID("[V,V]"),
...
if (wfn_type_ != "OMP2") { 
   if (twopdm_abcd_type == "DIRECT" ) {
 	// X_MNAC = 1/4 \sum{E,F} t_MN^EF * <AC||EF> = 1/2 \sum{E,F} t_MN^EF * <AC|EF>  
        ...
        dpd_->contract444(&K, &T, &X, 0, 0, 0.5, 0.0);
	dpd_->buf4_close(&K);
	dpd_->buf4_close(&T);
        dpd_->buf4_sort(&X, PSIF_OCC_DENSITY, rspq, ID("[O,O]"), ID("[V,V]"), "X <OO|VV>");
	dpd_->buf4_close(&X);
	...
	dpd_->buf4_close(&X);
        ...
        dpd_->contract444(&K, &T, &X, 0, 0, 1.0, 0.0);
	dpd_->buf4_close(&K);
	dpd_->buf4_close(&T);
        dpd_->buf4_sort(&X, PSIF_OCC_DENSITY, rspq, ID("[O,o]"), ID("[V,v]"), "X <Oo|Vv>");
	dpd_->buf4_close(&X);
...
/********************************************************************************************/ 
	...
	dpd_->buf4_init(&G, PSIF_OCC_DENSITY, 0, ID("[O,O]"), ID("[V,V]"),
                  ID("[O,O]"), ID("[V,V]"), 0, "TPDM <OO|VV>");
	dpd_->contract442(&K, &G, &GF, 1, 1, 2.0, 1.0); 
	dpd_->buf4_close(&K);
	dpd_->buf4_close(&G);
...
if (wfn_type_ != "OMP2") { 
	...
	dpd_->buf4_init(&K, PSIF_LIBTRANS_DPD, 0, ID("[O,v]"), ID("[V,o]"),
