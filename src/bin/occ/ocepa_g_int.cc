...
{  
 ...
 if (reference_ == "RESTRICTED") {
	...
	dpd_->contract442(&T, &Tau, &G, 0, 0, 1.0, 0.0);
	dpd_->file2_close(&G);
	...
	dpd_->contract442(&Tau, &T, &G, 2, 2, -1.0, 0.0); 
	dpd_->file2_close(&G);
	...
	dpd_->buf4_init(&TAB, PSIF_OCC_DPD, 0, ID("[O,o]"), ID("[V,v]"),
                  ID("[O,o]"), ID("[V,v]"), 0, "T2 <Oo|Vv>");
	dpd_->buf4_init(&LAA, PSIF_OCC_DPD, 0, ID("[O,O]"), ID("[V,V]"),
                  ID("[O,O]"), ID("[V,V]"), 0, "T2 <OO|VV>");
	dpd_->buf4_init(&LBB, PSIF_OCC_DPD, 0, ID("[o,o]"), ID("[v,v]"),
                  ID("[o,o]"), ID("[v,v]"), 0, "T2 <oo|vv>");
	dpd_->buf4_init(&LAB, PSIF_OCC_DPD, 0, ID("[O,o]"), ID("[V,v]"),
                  ID("[O,o]"), ID("[V,v]"), 0, "T2 <Oo|Vv>");
	...
	dpd_->file2_init(&G, PSIF_OCC_DENSITY, 0, ID('O'), ID('O'), "G <O|O>");  
	dpd_->contract442(&TAA, &LAA, &G, 0, 0, 0.5, 0.0);
	
	// G_IM += \sum{n,E,f} t_In^Ef(1) * l_Ef^Mn(1) = \sum{N,E,F} t_In^Ef(1) * t_Mn^Ef(1)
	dpd_->contract442(&TAB, &LAB, &G, 0, 0, 1.0, 1.0);
	dpd_->file2_close(&G);
	...
	// G_im = 1/2 \sum{n,e,f} t_in^ef(1) * l_ef^mn(1) = 1/2 \sum{n,e,f} t_in^ef(1) * t_mn^ef(1)
	dpd_->file2_init(&G, PSIF_OCC_DENSITY, 0, ID('o'), ID('o'), "G <o|o>");  
	dpd_->contract442(&TBB, &LBB, &G, 0, 0, 0.5, 0.0);
	
	// G_im  += \sum{N,e,F} t_Ni^Fe(1) * l_Fe^Nm(1) = \sum{N,e,F} t_Ni^Fe(1) * t_Nm^Fe(1)
	dpd_->contract442(&TAB, &LAB, &G, 1, 1, 1.0, 1.0);
	dpd_->file2_close(&G);
	...
	dpd_->file2_init(&G, PSIF_OCC_DENSITY, 0, ID('V'), ID('V'), "G <V|V>");  
	dpd_->contract442(&TAA, &LAA, &G, 2, 2, -0.5, 0.0); 
	
	// G_EA += - \sum{M,n,f} t_Mn^Af(1) * l_Ef^Mn(1) = - \sum{M,n,f} t_Mn^Af(1) * t_Mn^Ef(1)
	dpd_->contract442(&TAB, &LAB, &G, 2, 2, -1.0, 1.0); 
	dpd_->file2_close(&G);
	...
	// G_ea = -1/2 \sum{m,n,f} t_mn^af(1) * l_ef^mn(1) = -1/2 \sum{m,n,f} t_mn^af(1) * t_mn^ef(1)
	dpd_->file2_init(&G, PSIF_OCC_DENSITY, 0, ID('v'), ID('v'), "G <v|v>");  
	dpd_->contract442(&TBB, &LBB, &G, 2, 2, -0.5, 0.0); 
	
	// G_ea += - \sum{M,n,F} t_Mn^Fa(1) * l_Fe^Mn(1) = - \sum{M,n,F} t_Mn^Fa(1) * t_Mn^Fe(1)
	dpd_->contract442(&TAB, &LAB, &G, 3, 3, -1.0, 1.0); 
	dpd_->file2_close(&G);
