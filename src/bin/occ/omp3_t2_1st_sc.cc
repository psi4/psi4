...
if (reference_ == "RESTRICTED") {
    ...
    dpd_buf4_copy(&K, PSIF_OCC_DPD, "T2_1 <OO|VV>");
    dpd_buf4_close(&K);
    ...
    dpd_buf4_close(&D);
     ...
     dpd_buf4_copy(&T, PSIF_OCC_DPD, "Tau_1 <OO|VV>");
     dpd_buf4_copy(&T, PSIF_OCC_DPD, "T2_1AA <OO|VV>");
     dpd_buf4_sort(&T, PSIF_OCC_DPD, qprs, ID("[O,O]"), ID("[V,V]"), "T2_1jiab <OO|VV>");
    ...
    dpd_buf4_sort(&T, PSIF_OCC_DPD , qrps, ID("[O,V]"), ID("[O,V]"), "T2_1pp (OV|OV)");
    dpd_buf4_close(&T);

    // Tau(IJ,AB) => Tau'(IA,JB), Tau"(JA,IB)
    dpd_buf4_init(&T, PSIF_OCC_DPD, 0, ID("[O,O]"), ID("[V,V]"),
                  ID("[O,O]"), ID("[V,V]"), 0, "Tau_1 <OO|VV>");
    dpd_buf4_sort(&T, PSIF_OCC_DPD , prqs, ID("[O,V]"), ID("[O,V]"), "Tau_1 (OV|OV)");
    dpd_buf4_sort(&T, PSIF_OCC_DPD , qrps, ID("[O,V]"), ID("[O,V]"), "Tau_1pp (OV|OV)");
    dpd_buf4_close(&T);
...
//===========================================================================================
    ...
    */
    ...
    dpd_buf4_init(&T, PSIF_OCC_DPD, 0, ID("[O,o]"), ID("[V,v]"),
                  ID("[O,o]"), ID("[V,v]"), 0, "T2_1 <Oo|Vv>");
    dpd_buf4_sort(&T, PSIF_OCC_DPD , prqs, ID("[O,V]"), ID("[o,v]"), "T2_1 (OV|ov)");
    dpd_buf4_sort(&T, PSIF_OCC_DPD , qrps, ID("[o,V]"), ID("[O,v]"), "T2_1 (oV|Ov)");
    dpd_buf4_close(&T);    
   
    // T(IA,jb) => T(jb,IA)   
    dpd_buf4_init(&T, PSIF_OCC_DPD, 0, ID("[O,V]"), ID("[o,v]"),
                  ID("[O,V]"), ID("[o,v]"), 0, "T2_1 (OV|ov)");
    dpd_buf4_sort(&T, PSIF_OCC_DPD , rspq, ID("[o,v]"), ID("[O,V]"), "T2_1 (ov|OV)");
    dpd_buf4_close(&T);
