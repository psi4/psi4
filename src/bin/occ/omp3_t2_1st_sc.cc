...
if (reference_ == "RESTRICTED") {
    ...
    dpd_->buf4_copy(&K, PSIF_OCC_DPD, "T2_1 <OO|VV>");
    dpd_->buf4_close(&K);
    ...
    dpd_->buf4_close(&D);
     ...
     dpd_->buf4_copy(&T, PSIF_OCC_DPD, "Tau_1 <OO|VV>");
     dpd_->buf4_copy(&T, PSIF_OCC_DPD, "T2_1AA <OO|VV>");
     dpd_->buf4_sort(&T, PSIF_OCC_DPD, qprs, ID("[O,O]"), ID("[V,V]"), "T2_1jiab <OO|VV>");
     dpd_->buf4_init(&Tau, PSIF_OCC_DPD, 0, ID("[O,O]"), ID("[V,V]"),
                  ID("[O,O]"), ID("[V,V]"), 0, "Tau_1 <OO|VV>");
     dpd_->buf4_init(&Tss, PSIF_OCC_DPD, 0, ID("[O,O]"), ID("[V,V]"),
     ...
     dpd_->buf4_axpy(&Ttemp, &Tss, -1.0); // -1.0*Ttemp + Tss -> Tss
     dpd_->buf4_close(&Ttemp);
     dpd_->buf4_close(&Tau);
     dpd_->buf4_close(&Tss);
     
     if (print_ > 4) dpd_->buf4_print(&T, outfile, 1);
     dpd_->buf4_close(&T);
    ...
    dpd_->buf4_init(&T, PSIF_OCC_DPD, 0, ID("[O,O]"), ID("[V,V]"),
                  ID("[O,O]"), ID("[V,V]"), 0, "T2_1 <OO|VV>");
    dpd_->buf4_sort(&T, PSIF_OCC_DPD , prqs, ID("[O,V]"), ID("[O,V]"), "T2_1 (OV|OV)");
    dpd_->buf4_sort(&T, PSIF_OCC_DPD , qrps, ID("[O,V]"), ID("[O,V]"), "T2_1pp (OV|OV)");
    dpd_->buf4_close(&T);

    // Tau(IJ,AB) => Tau'(IA,JB), Tau"(JA,IB)
    dpd_->buf4_init(&T, PSIF_OCC_DPD, 0, ID("[O,O]"), ID("[V,V]"),
                  ID("[O,O]"), ID("[V,V]"), 0, "Tau_1 <OO|VV>");
    dpd_->buf4_sort(&T, PSIF_OCC_DPD , prqs, ID("[O,V]"), ID("[O,V]"), "Tau_1 (OV|OV)");
    dpd_->buf4_sort(&T, PSIF_OCC_DPD , qrps, ID("[O,V]"), ID("[O,V]"), "Tau_1pp (OV|OV)");
    dpd_->buf4_close(&T);
...
//===========================================================================================
    ...
    dpd_->buf4_dirprd(&D, &T);
    dpd_->buf4_close(&D);
    if (print_ > 1) dpd_->buf4_print(&T, outfile, 1);
    dpd_->buf4_close(&T);
    ...
    */
    ...
    dpd_->buf4_init(&T, PSIF_OCC_DPD, 0, ID("[O,o]"), ID("[V,v]"),
                  ID("[O,o]"), ID("[V,v]"), 0, "T2_1 <Oo|Vv>");
    dpd_->buf4_sort(&T, PSIF_OCC_DPD , prqs, ID("[O,V]"), ID("[o,v]"), "T2_1 (OV|ov)");
    dpd_->buf4_sort(&T, PSIF_OCC_DPD , qrps, ID("[o,V]"), ID("[O,v]"), "T2_1 (oV|Ov)");
    dpd_->buf4_close(&T);    
   
    // T(IA,jb) => T(jb,IA)   
    dpd_->buf4_init(&T, PSIF_OCC_DPD, 0, ID("[O,V]"), ID("[o,v]"),
                  ID("[O,V]"), ID("[o,v]"), 0, "T2_1 (OV|ov)");
    dpd_->buf4_sort(&T, PSIF_OCC_DPD , rspq, ID("[o,v]"), ID("[O,V]"), "T2_1 (ov|OV)");
    dpd_->buf4_close(&T);
