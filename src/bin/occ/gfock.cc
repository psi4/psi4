...
if (wfn_type_ != "OMP2") { 
   ...
   if (twopdm_abcd_type == "DIRECT" ) {
 	...
 	// X_MNIC = 2\sum{E,F} t_MN^EF(1) * <IC|EF>   
        dpd_->buf4_init(&X, PSIF_OCC_DENSITY, 0, ID("[O,O]"), ID("[O,V]"),
                  ID("[O,O]"), ID("[O,V]"), 0, "X <OO|OV>");
        dpd_->buf4_init(&K, PSIF_LIBTRANS_DPD, 0, ID("[O,V]"), ID("[V,V]"),
        ...
        dpd_->contract444(&T, &K, &X, 0, 0, 2.0, 0.0);
	dpd_->buf4_close(&K);
	dpd_->buf4_close(&T);
	dpd_->buf4_close(&X);
...
/************************************************************************************************/
	// Build Fai
	dpd_->file2_init(&GF, PSIF_OCC_DENSITY, 0, ID('V'), ID('O'), "GF <V|O>"); 
	...
	dpd_->contract442(&K, &G, &GF, 3, 3, 4.0, 0.0); 
	dpd_->buf4_close(&K);
	dpd_->buf4_close(&G);
...
if (wfn_type_ == "OMP2" && incore_iabc_ == 1) { 
        ...
        dpd_->file2_init(&G, PSIF_OCC_DENSITY, 0, ID('V'), ID('V'), "CORR OPDM <V|V>");
        ...
        dpd_->file2_mat_wrt(&G);
        dpd_->file2_close(&G);
 
        // Sort
        dpd_->buf4_init(&K, PSIF_LIBTRANS_DPD, 0, ID("[O,V]"), ID("[V,V]"),
                      ID("[O,V]"), ID("[V>=V]+"), 0, "MO Ints (OV|VV)");
        dpd_->buf4_sort(&K, PSIF_LIBTRANS_DPD , qprs, ID("[V,O]"), ID("[V,V]"), "MO Ints (VO|VV)");
	dpd_->buf4_close(&K);
 
	// Fai += 2 * \sum{e,f} (ai|ef) * Gcorr(e,f) 
	dpd_->buf4_init(&K, PSIF_LIBTRANS_DPD, 0, ints->DPD_ID("[V,O]"), ints->DPD_ID("[V,V]"),
                  ints->DPD_ID("[V,O]"), ints->DPD_ID("[V,V]"), 0, "MO Ints (VO|VV)");
        dpd_->file2_init(&G, PSIF_OCC_DENSITY, 0, ID('V'), ID('V'), "CORR OPDM <V|V>");
        dpd_->contract422(&K, &G, &GF, 0, 0, 2.0, 1.0);
	dpd_->buf4_close(&K);
        dpd_->file2_close(&G);
 ...
 do
 {
   ...
   for (int idx=0; idx < nbuf; idx++ )
   {
	...
	dpd_->buf4_init(&K, PSIF_LIBTRANS_DPD, 0, ints->DPD_ID("[O,V]"), ints->DPD_ID("[V,V]"),
	...
	dpd_->contract442(&K, &G, &GF, 2, 2, 4.0, 1.0); 
	dpd_->buf4_close(&K);
	dpd_->buf4_close(&G);
...
if (wfn_type_ == "OMP2" && incore_iabc_ == 0) { 
 ...
 do
 {
   ...
   for (int idx=0; idx < nbuf; idx++ )
   {
	...
	dpd_->buf4_init(&G, PSIF_OCC_DENSITY, 0, ID("[O,O]"), ID("[V,V]"),
                  ID("[O,O]"), ID("[V,V]"), 0, "TPDM <OO|VV>");
	dpd_->contract442(&K, &G, &GF, 1, 1, 8.0, 1.0); 
	dpd_->buf4_close(&K);
	dpd_->buf4_close(&G);
	dpd_->file2_close(&GF);
}// end else
...
/************************************************************************************************/
	// Build Fia
	dpd_->file2_init(&GF, PSIF_OCC_DENSITY, 0, ID('O'), ID('V'), "GF <O|V>");  
	...
	dpd_->buf4_init(&G, PSIF_OCC_DENSITY, 0, ID("[O,V]"), ID("[O,V]"),
                  ID("[O,V]"), ID("[O,V]"), 0, "TPDM <OV|OV>");
	dpd_->contract442(&K, &G, &GF, 1, 1, 4.0, 0.0); 
	dpd_->buf4_close(&K);
	dpd_->buf4_close(&G);
	...
	dpd_->buf4_init(&G, PSIF_OCC_DENSITY, 0, ID("[O,O]"), ID("[V,V]"),
                  ID("[O,O]"), ID("[V,V]"), 0, "TPDM <OO|VV>");
	dpd_->contract442(&K, &G, &GF, 2, 2, 8.0, 1.0); 
	dpd_->buf4_close(&K);
	dpd_->buf4_close(&G);
...
if (wfn_type_ != "OMP2") { 
   if (twopdm_abcd_type == "DIRECT" ) {
	...
	dpd_->contract442(&X, &T, &GF, 2, 2, 1.0, 1.0); 
	dpd_->buf4_close(&X);
	dpd_->buf4_close(&T);
	dpd_->file2_close(&GF);
   }
	...
	dpd_->contract442(&K, &G, &GF, 0, 0, 4.0, 1.0); 
	dpd_->buf4_close(&K);
	dpd_->buf4_close(&G);
	dpd_->file2_close(&GF);
   }
...
/************************************************************************************************/
	// Load Fai
	dpd_->file2_init(&GF, PSIF_OCC_DENSITY, 0, ID('V'), ID('O'), "GF <V|O>");  
	dpd_->file2_mat_init(&GF);
	dpd_->file2_mat_rd(&GF);
	for(int h = 0; h < nirrep_; ++h){
...
if (wfn_type_ != "OMP2") { 
   if (twopdm_abcd_type == "DIRECT" ) {
	...
	dpd_->buf4_close(&X);
        ...
        dpd_->contract444(&T, &K, &X, 0, 0, 1.0, 0.0);
	dpd_->buf4_close(&K);
	dpd_->buf4_close(&T);
	dpd_->buf4_close(&X);
	...
	dpd_->buf4_close(&X);
        ...
        dpd_->buf4_init(&X, PSIF_OCC_DENSITY, 0, ID("[O,o]"), ID("[V,o]"),
                  ID("[O,o]"), ID("[V,o]"), 0, "X <Oo|Vo>");
        dpd_->buf4_init(&K, PSIF_LIBTRANS_DPD, 0, ID("[V,o]"), ID("[V,v]"),
...
/********************************************************************************************/ 
	...
	dpd_->buf4_init(&G, PSIF_OCC_DENSITY, 0, ID("[O,O]"), ID("[O,O]"),
                  ID("[O,O]"), ID("[O,O]"), 0, "TPDM <OO|OO>");
	dpd_->contract442(&K, &G, &GF, 3, 3, 2.0, 0.0); 
	dpd_->buf4_close(&K);
	dpd_->buf4_close(&G);
	...
	dpd_buf4_close(&G);
        */
        ...
        dpd_->buf4_init(&G, PSIF_OCC_DENSITY, 0, ID("[O,O]"), ID("[V,V]"),
                  ID("[O,O]"), ID("[V,V]"), 0, "TPDM <OO|VV>");
        dpd_->contract442(&K, &G, &GF, 1, 1, 4.0, 1.0); 
        dpd_->buf4_close(&G);
	...
	// FAJ += 4 * \sum{E,M,F} <EM||FA> * G_MEJF = 4 * \sum{E,M,F} <ME||AF> * G_MEJF
        ...
        dpd_->contract442(&K, &G, &GF, 3, 3, 4.0, 1.0);
        dpd_->buf4_close(&K);
        dpd_->buf4_close(&G);
...
if (wfn_type_ != "OMP2") { 
	...
	dpd_->buf4_init(&K, PSIF_LIBTRANS_DPD, 0, ID("[V,o]"), ID("[V,v]"),
                  ID("[V,o]"), ID("[V,v]"), 0, "MO Ints <Vo|Vv>");
	dpd_->buf4_init(&G, PSIF_OCC_DENSITY, 0, ID("[V,o]"), ID("[O,v]"),
...
/********************************************************************************************/ 
	...
	dpd_->buf4_init(&G, PSIF_OCC_DENSITY, 0, ID("[O,O]"), ID("[V,V]"),
                  ID("[O,O]"), ID("[V,V]"), 0, "TPDM <OO|VV>");
	dpd_->contract442(&K, &G, &GF, 2, 2, 2.0, 0.0); 
	dpd_->buf4_close(&K);
	dpd_->buf4_close(&G);
...
if (wfn_type_ != "OMP2") { 
   if (twopdm_abcd_type == "DIRECT" ) {
	...
	dpd_->contract442(&X, &T, &GF, 2, 2, 0.25, 1.0); 
	dpd_->buf4_close(&X);
	dpd_->buf4_close(&T);
   }
...
if (wfn_type_ != "OMP2") { 
   if (twopdm_abcd_type == "DIRECT" ) {
	...
	dpd_->contract442(&X, &T, &GF, 3, 3, 1.0, 1.0); 
	dpd_->buf4_close(&X);
	dpd_->buf4_close(&T);
   }
