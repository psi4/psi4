...
{   
 ...
 if (reference_ == "RESTRICTED") {
    ...
    dpd_->contract444(&T, &Tau, &V, 0, 0, 2.0, 0.0);
    dpd_->buf4_close(&V);
    dpd_->buf4_close(&T);
    dpd_->buf4_close(&Tau);
    ...
    dpd_->contract444(&Tau, &T, &V, 0, 0, 1.0, 1.0); 
    dpd_->buf4_close(&T);
    dpd_->buf4_close(&Tau);

    // V(IB,JA) => V(IA,JB)
    dpd_->buf4_sort(&V, PSIF_OCC_DENSITY , psrq, ID("[O,V]"), ID("[O,V]"), "V <OV|OV>");
    dpd_->buf4_close(&V);
    ...
    // V(IB,JA) => V(IA,BJ)
    dpd_->buf4_sort(&V, PSIF_OCC_DENSITY , psqr, ID("[O,V]"), ID("[V,O]"), "V <OV|VO>");
    dpd_->buf4_close(&V);
    ...
    dpd_->buf4_init(&V, PSIF_OCC_DENSITY, 0, ID("[O,O]"), ID("[O,O]"),
                  ID("[O,O]"), ID("[O,O]"), 0, "V <OO|OO>");
    dpd_->contract444(&T, &L, &V, 0, 0, 0.5, 0.0);
    dpd_->buf4_close(&T);
    dpd_->buf4_close(&L);
    dpd_->buf4_close(&V);
    ...
    dpd_->buf4_init(&T, PSIF_OCC_DPD, 0, ID("[O,V]"), ID("[O,V]"),
                  ID("[O,V]"), ID("[O,V]"), 0, "T2_1 (OV|OV)");
    dpd_->buf4_init(&L, PSIF_OCC_DPD, 0, ID("[O,V]"), ID("[O,V]"),
                  ID("[O,V]"), ID("[O,V]"), 0, "T2_1 (OV|OV)");
    dpd_->buf4_init(&V, PSIF_OCC_DENSITY, 0, ID("[O,V]"), ID("[O,V]"),
                  ID("[O,V]"), ID("[O,V]"), 0, "V <IB|JA>");
    dpd_->contract444(&T, &L, &V, 0, 0, 0.5, 0.0); 
    ...
    dpd_->buf4_init(&L, PSIF_OCC_DPD, 0, ID("[O,V]"), ID("[o,v]"),
                  ID("[O,V]"), ID("[o,v]"), 0, "T2_1 (OV|ov)");
    dpd_->contract444(&T, &L, &V, 0, 0, 0.5, 1.0); 
    dpd_->buf4_close(&T);
    dpd_->buf4_close(&L);
    ...
    dpd_->buf4_init(&L, PSIF_OCC_DPD, 0, ID("[O,V]"), ID("[o,v]"),
                  ID("[O,V]"), ID("[o,v]"), 0, "T2_1 (OV|ov)");
    dpd_->contract444(&T, &L, &V, 1, 1, 0.5, 1.0); 
    dpd_->buf4_close(&T);
    dpd_->buf4_close(&L);
    ...
    dpd_->buf4_init(&L, PSIF_OCC_DPD, 0, ID("[o,V]"), ID("[O,v]"),
                  ID("[o,V]"), ID("[O,v]"), 0, "T2_1 (oV|Ov)");
    dpd_->buf4_init(&V1, PSIF_OCC_DENSITY, 0, ID("[O,v]"), ID("[O,v]"),
                  ID("[O,v]"), ID("[O,v]"), 0, "V <Ib|Ja>");
    dpd_->buf4_init(&V2, PSIF_OCC_DENSITY, 0, ID("[o,V]"), ID("[o,V]"),
                  ID("[o,V]"), ID("[o,V]"), 0, "V <iB|jA>");
    // V_IaJb(2) => V(Ib,Ja) = 1/2 \sum_{m,E} T_Im^Eb(1) T_Jm^Ea(1) = 1/2 \sum_{m,E} T(mE,IB) T(mE,Ja) 
    dpd_->contract444(&T, &L, &V1, 1, 1, 0.5, 0.0); 
    // V_iAjB(2) => V(iB,jA) = 1/2 \sum_{M,e} T_Mi^Be(1) T_Mj^Ae(1) = 1/2 \sum_{M,e} T(iB,Me) T(jA,Me) 
    dpd_->contract444(&T, &L, &V2, 0, 0, 0.5, 0.0); 
    dpd_->buf4_close(&T);
    dpd_->buf4_close(&L);
    
    // V(Ib,Ja) => V(Ia,Jb) & V(iB,jA) => V(iA,jB)
    dpd_->buf4_sort(&V1, PSIF_OCC_DENSITY , psrq, ID("[O,v]"), ID("[O,v]"), "V <Ov|Ov>");
    dpd_->buf4_close(&V1);
    dpd_->buf4_sort(&V2, PSIF_OCC_DENSITY , psrq, ID("[o,V]"), ID("[o,V]"), "V <oV|oV>");
    dpd_->buf4_close(&V2);
    ...
    */
    ...
    dpd_->buf4_init(&T, PSIF_OCC_DPD, 0, ID("[o,v]"), ID("[o,v]"),
                  ID("[o,v]"), ID("[o,v]"), 0, "T2_1 (ov|ov)");
    dpd_->contract444(&L, &T, &V, 1, 1, 0.5, 1.0); 
    dpd_->buf4_close(&T);
    dpd_->buf4_close(&L);
