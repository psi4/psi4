...
void OCCWave::omp2_mp2_energy()
{
 ...
 if (reference_ == "RESTRICTED") {
     ...
     Emp2AA = 0.5 * dpd_->buf4_dot(&Tss, &K);     
     dpd_->buf4_close(&Tss);
...
void OCCWave::cepa_energy()
{
 ...
 if (reference_ == "RESTRICTED") {
     ...
     dpd_->buf4_init(&K, PSIF_LIBTRANS_DPD, 0, ID("[O,O]"), ID("[V,V]"),
                  ID("[O,O]"), ID("[V,V]"), 0, "MO Ints <OO||VV>");
     EcepaAA = 0.25 * dpd_->buf4_dot(&T, &K);     
     dpd_->buf4_close(&T);
     dpd_->buf4_close(&K);     
     ...
     dpd_->buf4_init(&K, PSIF_LIBTRANS_DPD, 0, ID("[o,o]"), ID("[v,v]"),
                  ID("[o,o]"), ID("[v,v]"), 0, "MO Ints <oo||vv>");
     EcepaBB = 0.25 * dpd_->buf4_dot(&T, &K);     
     dpd_->buf4_close(&T);
     dpd_->buf4_close(&K);     
