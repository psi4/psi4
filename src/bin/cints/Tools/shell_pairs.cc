...
void init_shell_pairs() {
  ...
  if (memd > UserOptions.memory)
    throw std::domain_error(
        "Not enough memory to store shell pair data. Need more memory.");
  UserOptions.memory -= memd;
  stack = (double *) malloc(sizeof(double) * memd);
  curr_stack_ptr = stack;

  /*--- allocate room for the shell pairs ---*/
  pairs = (struct shell_pair **) malloc(sizeof(struct shell_pair *)
      * BasisSet.num_shells);
  for (i = 0; i < BasisSet.num_shells; i++)
    pairs[i] = (struct shell_pair *) malloc(sizeof(struct shell_pair)
        * BasisSet.num_shells);
  ...
  for (si = 0; si < BasisSet.num_shells; si++) {
    ...
    for (sj = 0; sj < BasisSet.num_shells; sj++) {
      ...
      /* determine and store irreps spanned by orbital products in shell pair */
      sp->span = (bool *) malloc(Symmetry.nirreps*sizeof(bool));
      for (int Gi = 0; Gi < Symmetry.nirreps; ++Gi) {
        sp->span[Gi] = false;
      }
      for (int Gi = 0; Gi < Symmetry.nirreps; Gi++) {
        for (int Gj = 0; Gj < Symmetry.nirreps; Gj++) {
          if(BasisSet.shells[si].span[Gi] && BasisSet.shells[sj].span[Gj]){
            sp->span[Gi^Gj] = true;
          }
        }
      }

      sp->a1 = curr_stack_ptr;
      curr_stack_ptr += np_i;
      sp->a2 = curr_stack_ptr;
      curr_stack_ptr += np_j;
      ...
      sp->inorm = curr_stack_ptr;
      curr_stack_ptr += np_i;
      sp->jnorm = curr_stack_ptr;
      curr_stack_ptr += np_j;

      sp->Sovlp = (double **) malloc(np_i * sizeof(double *));
      for (i = 0; i < np_i; i++) {
        sp->Sovlp[i] = curr_stack_ptr;
        curr_stack_ptr += np_j;
      }
      ...
      for (i = 0; i < np_i; i++) {
        ...
        inorm
            = BasisSet.cgtos[BasisSet.shells[si].fprim + i - 1].ccoeff[BasisSet.shells[si].am
                - 1];
        sp->a1[i] = a1;
        sp->inorm[i] = inorm;
        for (j = 0; j < np_j; j++) {
          ...
          gam = a1 + a2;
          jnorm
              = BasisSet.cgtos[BasisSet.shells[sj].fprim + j - 1].ccoeff[BasisSet.shells[sj].am
                  - 1];
...
void init_unique_shell_pairs(void) {
  ...
  for (us = 0; us < Symmetry.num_unique_shells; us++) {
    ...
    num_bf = BasisSet.puream ? 2 * BasisSet.shells[shell].am - 1
        : ioff[BasisSet.shells[shell].am];
  ...
  pairs = (struct unique_shell_pair **) malloc(
      sizeof(struct unique_shell_pair *) * Symmetry.num_unique_shells);
  for (usi = 0; usi < Symmetry.num_unique_shells; usi++) {
    si = Symmetry.us2s[usi];
    nbfi = BasisSet.puream ? 2 * BasisSet.shells[si].am - 1
        : ioff[BasisSet.shells[si].am];
    pairs[usi] = (struct unique_shell_pair *) malloc(
        sizeof(struct unique_shell_pair) * Symmetry.num_unique_shells);
    for (usj = 0; usj < Symmetry.num_unique_shells; usj++) {
      sj = Symmetry.us2s[usj];
      pairs[usi][usj].SOpair_npi = init_int_array(Symmetry.nirreps);
      nbfj = BasisSet.puream ? 2 * BasisSet.shells[sj].am - 1
          : ioff[BasisSet.shells[sj].am];
      ...
      for (bf_i = 0; bf_i < nbfi; bf_i++) {
        ...
        for (bf_j = 0; bf_j < max_bf_j; bf_j++)
          for (irr_i = 0; irr_i < Symmetry.nirreps; irr_i++) {
            if (bf_so[usi][bf_i][irr_i] == -1)
              continue;
            max_irr_j = (usi == usj && bf_i == bf_j) ? irr_i + 1
                : Symmetry.nirreps;
      ...
      pairs[usi][usj].SOpair_so_i = (int **) malloc(Symmetry.nirreps
          * sizeof(int *));
      pairs[usi][usj].SOpair_so_j = (int **) malloc(Symmetry.nirreps
          * sizeof(int *));
      pairs[usi][usj].SOpair_bf_i = (int **) malloc(Symmetry.nirreps
          * sizeof(int *));
      pairs[usi][usj].SOpair_bf_j = (int **) malloc(Symmetry.nirreps
          * sizeof(int *));
      for (irrep = 0; irrep < Symmetry.nirreps; irrep++)
          ...
          for (irr_i = 0; irr_i < Symmetry.nirreps; irr_i++) {
            ...
            for (irr_j = 0; irr_j < max_irr_j; irr_j++)
              if (bf_so[usj][bf_j][irr_j] != -1) {
                ...
                if (usi == usj && (so_i = bf_so[usi][bf_i][irr_i]) < (so_j
                    = bf_so[usj][bf_j][irr_j])) {
                  ...
                  pairs[usi][usj].SOpair_so_i[irrep][count]
                      = bf_so[usi][bf_i][irr_i];
                  pairs[usi][usj].SOpair_so_j[irrep][count]
                      = bf_so[usj][bf_j][irr_j];
