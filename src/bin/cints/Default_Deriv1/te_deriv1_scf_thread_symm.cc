    ...
    for (usjj = 0; usjj <= usii; usjj++) {
      for (uskk = 0; uskk <= usii; uskk++) {
        for (usll = 0; usll <= ((usii == uskk) ? usjj : uskk); usll++, quartet_index++) {
          ...
          if (quartet_index % UserOptions.num_threads != thread_num) continue;
          ...
          for (plquartet = 0; plquartet < num_unique_quartets; plquartet++) {
            ...
            if (usi == usj) pfac *= 0.5;
            if (usk == usl) pfac *= 0.5;
            if (usi == usk && usj == usl || usi == usl && usj == usk) pfac
                *= 0.5;
                ...
                for (ao_j = sj_fao; ao_j < sj_fao + nj; ao_j++)
                  for (ao_k = sk_fao; ao_k < sk_fao + nk; ao_k++)
                    for (ao_l = sl_fao; ao_l < sl_fao + nl; ao_l++) {
                      FourInd[count] = (4.0 * Dens[ao_i][ao_j]
                          * Dens[ao_k][ao_l] - Dens[ao_i][ao_k]
                          * Dens[ao_j][ao_l] - Dens[ao_i][ao_l]
                          * Dens[ao_k][ao_j]) * GTOs.bf_norm[orig_am[0]][ao_i
                          - si_fao] * GTOs.bf_norm[orig_am[1]][ao_j - sj_fao]
                          * GTOs.bf_norm[orig_am[2]][ao_k - sk_fao]
                          * GTOs.bf_norm[orig_am[3]][ao_l - sl_fao];
                      count++;
                    }
            } else { /*--- ROHF or TCSCF ---*/
              if (am) memset((char *) FourInd, 0, sizeof(double) * quartet_size);
              else FourInd[0] = 0.0;
                ...
                for (mosh_j = 0; mosh_j < MOInfo.num_moshells; mosh_j++) {
                  ...
                  for (ao_i = si_fao; ao_i < si_fao + ni; ao_i++)
                    for (ao_j = sj_fao; ao_j < sj_fao + nj; ao_j++)
                      for (ao_k = sk_fao; ao_k < sk_fao + nk; ao_k++)
                        for (ao_l = sl_fao; ao_l < sl_fao + nl; ao_l++) {
                          FourInd[count] += (alpha * dens_i[ao_i][ao_j]
                              * dens_j[ao_k][ao_l] + beta * (dens_i[ao_i][ao_k]
                              * dens_j[ao_j][ao_l] + dens_i[ao_i][ao_l]
                              * dens_j[ao_k][ao_j]));
                          count++;
                        }
                }
