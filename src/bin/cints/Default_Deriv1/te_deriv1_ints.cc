...
void te_deriv1_ints() {
  ...
  {
    const int max_num_cdsalc_per_quartet = min(max_num_cdsalcs_per_quartet,
        CDSALCs.nsalcs);
  ...
  /*-------------------------
   ...
   -------------------------*/
  max_cart_class_size = (ioff[BasisSet.max_am]) * (ioff[BasisSet.max_am])
      * (ioff[BasisSet.max_am]) * (ioff[BasisSet.max_am]);
  ...
  for (int c = 0; c < num_coords; c++) {
    tot_data[c] = new struct tebuf[max_num_unique_quartets
        * max_cart_class_size];
  }
  ...
  init_libderiv1(&Libderiv, BasisSet.max_am - 1, max_num_prim_comb,
      max_cart_class_size);
    ...
    for (int usjj = 0; usjj <= usii; usjj++)
      for (int uskk = 0; uskk < Symmetry.num_unique_shells; uskk++) {
        ...
        for (int usll = 0; usll <= usll_max; usll++) {
          ...
          if (BasisSet.shells[Symmetry.us2s[usi]].am
              < BasisSet.shells[Symmetry.us2s[usj]].am) {
            dum = usi;
            usi = usj;
            usj = dum;
          }
          if (BasisSet.shells[Symmetry.us2s[usk]].am
              < BasisSet.shells[Symmetry.us2s[usl]].am) {
            dum = usk;
            usk = usl;
            usl = dum;
          }
          ...
          if (BasisSet.shells[Symmetry.us2s[usi]].am
              + BasisSet.shells[Symmetry.us2s[usj]].am
              > BasisSet.shells[Symmetry.us2s[usk]].am
                  + BasisSet.shells[Symmetry.us2s[usl]].am) {
            dum = usi;
            usi = usk;
            usk = dum;
          ...
          if (Symmetry.nirreps > 1) { /*--- Non-C1 symmetry case ---*/
            ...
            ni = (BasisSet.puream ? 2 * BasisSet.shells[si].am - 1
                : ioff[BasisSet.shells[si].am]);
            nj = (BasisSet.puream ? 2 * BasisSet.shells[sjj].am - 1
                : ioff[BasisSet.shells[sjj].am]);
            nk = (BasisSet.puream ? 2 * BasisSet.shells[skk].am - 1
                : ioff[BasisSet.shells[skk].am]);
            nl = (BasisSet.puream ? 2 * BasisSet.shells[sll].am - 1
              ...
              for (dcr_ijkl = 0; dcr_ijkl < Symmetry.dcr_dim[stab_ij][stab_kl]; dcr_ijkl++) {
                ...
                for (dcr_kl = 0; dcr_kl < Symmetry.dcr_dim[stab_k][stab_l]; dcr_kl++) {
                  ...
                  /*-------------------------------------------------------------
                   ...
                   -------------------------------------------------------------*/
                  if (!(total_am % 2) || (BasisSet.shells[si].center
                      != BasisSet.shells[sj].center)
                      || (BasisSet.shells[sj].center
                          != BasisSet.shells[sk].center)
                      || (BasisSet.shells[sk].center
                          != BasisSet.shells[sl].center)) {
                    sj_arr[count] = sj;
                    sk_arr[count] = sk;
                    sl_arr[count] = sl;
            ...
            sk_arr[0] = usk;
            sl_arr[0] = usl;
            ni = (BasisSet.puream ? 2 * BasisSet.shells[usi].am - 1
                : ioff[BasisSet.shells[usi].am]);
            nj = (BasisSet.puream ? 2 * BasisSet.shells[usj].am - 1
                : ioff[BasisSet.shells[usj].am]);
            nk = (BasisSet.puream ? 2 * BasisSet.shells[usk].am - 1
                : ioff[BasisSet.shells[usk].am]);
            nl = (BasisSet.puream ? 2 * BasisSet.shells[usl].am - 1
                : ioff[BasisSet.shells[usl].am]);
          ...
          for (int c = 0; c < 4; c++) {
            ...
            for (int xyz = 0; xyz < 3; xyz++, cart_der++) {
              const CDSALC_t::cd2salc_map_t& cd2salc_map =
                  CDSALCs.cd2salc_map[cart_der];
...
#define NO_PERM_SYMM 0
...
#else
                    deriv1_quartet_data(
                        &(Libderiv.PrimQuartet[num_prim_comb++]), &fjt_table,
            ...
            for (int j = 0; j < size; j++) {
              cart_ints[3][j] = -(cart_ints[0][j] + cart_ints[6][j]
                  + cart_ints[9][j]);
              cart_ints[4][j] = -(cart_ints[1][j] + cart_ints[7][j]
                  + cart_ints[10][j]);
              cart_ints[5][j] = -(cart_ints[2][j] + cart_ints[8][j]
                  + cart_ints[11][j]);
            }
            ...
            for (int uc = 0; uc < num_unique_centers; uc++) {
              ...
              for (int xyz = 0; xyz < 3; xyz++, totderindex++, uniquetotderindex++) {
                ...
                if (am) {
                  const double* data = norm_quartet(source_ints, target_ints,
                      orig_am, BasisSet.puream);
                  if (data != target_ints) memcpy(target_ints, data,
                      sizeof(double) * class_size);
                } else {
            ...
            for (int uc = 0; uc < num_unique_centers; uc++) {
              ...
              for (int xyz = 0; xyz < 3; xyz++, abs_cart_der++, cart_der++) {
                ...
                const CDSALC_t::cd2salc_map_t& cd2salc_map =
                    CDSALCs.cd2salc_map[abs_cart_der];
                const int nsalcs = cd2salc_map.nsalcs;
          ...
          if (Symmetry.nirreps >= 1) { /*--- Non-C1 case ---*/
            ...
            for (int irrep_ij = 0; irrep_ij < Symmetry.nirreps; irrep_ij++) {
              if ((npi_ij = usp_ij->SOpair_npi[irrep_ij]) == 0) continue;

              /* loop over possible irrep of |kl) */
              for (int irrep_kl = 0; irrep_kl < Symmetry.nirreps; irrep_kl++) {
                if ((npi_kl = usp_kl->SOpair_npi[irrep_kl]) == 0) continue;
                ...
                int exists_nonzero_deriv = irreps_of_allowed_derivatives & (1
                    << irrep_deriv);
                if (!exists_nonzero_deriv) continue;
                ...
                for (ij = 0; ij < npi_ij; ij++) { /*--- Loop over SO pairs from usij ---*/
                  ...
                  for (kl = 0; kl < npi_kl; kl++) { /*--- Loop over SO pairs from uskl ---*/
                    ...
                    for (s = 0; s < num_unique_quartets; s++) { /*--- Sum over petite list quartets ---*/
                      ...
                      const cdsalc_elem_vec& salc_elems =
                          plist_salcs[s][irrep_deriv];
                      ...
                      for (int derop = 0; derop < ncontribs; derop++) {
                        ...
                        const double ao_integral =
                            plist_ints[s][elem.cart_der][index];
                        const double so_contribution = so2ao_pfac * elem.coef
                            * ao_integral;
                        ...
                        if (UserOptions.print_lvl > PRINT_DEBUG) {
                          ...
                          fprintf(
                              ...
                              elem.cart_der, bf_i, bf_j, bf_k, bf_l,
                              ao_integral);
                          fprintf(outfile,
                              "   SO pfac = %lf  deriv SALC pfac = %lf\n",
                              so2ao_pfac, elem.coef);
                          fprintf(
                              outfile,
                              "   SO integral -- deriv wrt coord %d ( %d %d | %d %d ) += %20.10lf = %20.10lf\n\n",
                              salc_thisquartet2all[salc], so_i, so_j, so_k,
                              so_l, so_contribution, so_int[salc]);
                        }
                      }
                    }
          ...
          if (have_nonzero_integrals) { /* Let's see if we need to write out something */
            ...
            for (int salc = 0; salc < nsalcders_for_this_quartet; salc++) {
              ...
              if (nints > 0) {
                ...
                iwl_buf_wrt_struct_nocut(&(D1ERIOUT[abs_salc]),
                    tot_data[abs_salc], nints);
  ...
  fprintf(outfile,
      ...
      oss.str().c_str(), IOUnits.itapD1ERI_SO, IOUnits.itapD1ERI_SO
          + num_coords - 1);
  //}
