...
double EaT_RHF(void)
{
  ...
  dpd_->file2_init(&L1, PSIF_CC_LAMBDA, 0, 0, 1, "LIA");
  dpd_->file2_mat_init(&L1);
  dpd_->file2_mat_rd(&L1);

  dpd_->buf4_init(&T2, PSIF_CC_TAMPS, 0, 0, 5, 0, 5, 0, "tIjAb");
  dpd_->buf4_init(&L2, PSIF_CC_LAMBDA, 0, 0, 5, 0, 5, 0, "LIjAb");
    ...
    for(j=0; j < occpi[Gj]; j++) {
      ...
      for(k=0; k < occpi[Gk]; k++) {
        ...
        for(Gab=0; Gab < nirreps; Gab++) {
          ...
          W2[Gab] = dpd_->dpd_block_matrix(Fints->params->coltot[Gab],virtpi[Gc]);
          W3[Gab] = dpd_->dpd_block_matrix(Fints->params->coltot[Gab],virtpi[Gc]);
        }
        // timer_off("malloc");
        ...
        dpd_->sort_3d(W2, W3, nirreps, Gijk, Fints->params->coltot, Fints->params->colidx,
               Fints->params->colorb, Fints->params->rsym, Fints->params->ssym,
               vir_off, vir_off, virtpi, vir_off, Fints->params->colidx, acb, 0);
        ...
        dpd_->sort_3d(W3, W2, nirreps, Gijk, Fints->params->coltot, Fints->params->colidx,
               Fints->params->colorb, Fints->params->rsym, Fints->params->ssym,
               vir_off, vir_off, virtpi, vir_off, Fints->params->colidx, bac, 0);
        ...
        for(Gab=0; Gab < nirreps; Gab++) {
          ...
          dpd_->free_dpd_block(W3[Gab],Fints->params->coltot[Gab],virtpi[Gc]);
        ...
        for(Gab=0; Gab < nirreps; Gab++) {
          ...
          dpd_->free_dpd_block(W2[Gab],Fints->params->coltot[Gab],virtpi[Gc]);
