...
double ET_UHF_ABB(void)
{
  ...
  dpd_->buf4_init(&T2BB, PSIF_CC_TAMPS, 0, 10, 15, 12, 17, 0, "tijab");
  ...
  dpd_->buf4_init(&EBBints, PSIF_CC_EINTS, 0, 10, 30, 12, 30, 0, "E <ij||ka> (i>j,ka)");
  ...
  for(h=0; h < nirreps; h++) {
    dpd_->buf4_mat_irrep_init(&T2BB, h);
    dpd_->buf4_mat_irrep_rd(&T2BB, h);
    ...
    dpd_->buf4_mat_irrep_init(&EBBints, h);
    dpd_->buf4_mat_irrep_rd(&EBBints, h);
    ...
    dpd_->buf4_mat_irrep_init(&DBBints, h);
    dpd_->buf4_mat_irrep_rd(&DBBints, h);
	...
	for(i=0; i < aoccpi[Gi]; i++) {
	  ...
	  for(j=0; j < boccpi[Gj]; j++) {
		...
		for(Gab=0; Gab < nirreps; Gab++) {
		  ...
		  WAbc[Gab] = dpd_->dpd_block_matrix(FABints.params->coltot[Gab], bvirtpi[Gc]);
		}
		...
		for(Gd=0; Gd < nirreps; Gd++) {
		  ...
		  FABints.matrix[Gid] = dpd_->dpd_block_matrix(bvirtpi[Gd], FABints.params->coltot[Gid]);
		  dpd_->buf4_mat_irrep_rd_block(&FABints, Gid, id, bvirtpi[Gd]);
		  ...
		  dpd_->free_dpd_block(FABints.matrix[Gid], bvirtpi[Gd], FABints.params->coltot[Gid]);
		...
		for(Gab=0; Gab < nirreps; Gab++) {
		  ...
		  WAcb[Gab] = dpd_->dpd_block_matrix(FABints.params->coltot[Gab], bvirtpi[Gc]);
		}
        ...
        dpd_->sort_3d(WAcb, WAbc, nirreps, Gijk, FABints.params->coltot, FABints.params->colidx,
		...
		for(Gab=0; Gab < nirreps; Gab++) {
		  ...
		  dpd_->free_dpd_block(WAcb[Gab], FABints.params->coltot[Gab], bvirtpi[Gc]);

		  WbcA[Gab] = dpd_->dpd_block_matrix(FBBints.params->coltot[Gab], avirtpi[Gc]);
		}
		...
		for(Gd=0; Gd < nirreps; Gd++) {
		  ...
		  FBBints.matrix[Gjd] = dpd_->dpd_block_matrix(bvirtpi[Gd], FBBints.params->coltot[Gjd]);
		  dpd_->buf4_mat_irrep_rd_block(&FBBints, Gjd, jd, bvirtpi[Gd]);
		  ...
		  dpd_->free_dpd_block(FBBints.matrix[Gjd], bvirtpi[Gd], FBBints.params->coltot[Gjd]);
		  ...
		  FBBints.matrix[Gkd] = dpd_->dpd_block_matrix(bvirtpi[Gd], FBBints.params->coltot[Gkd]);
		  dpd_->buf4_mat_irrep_rd_block(&FBBints, Gkd, kd, bvirtpi[Gd]);
		  ...
		  dpd_->free_dpd_block(FBBints.matrix[Gkd], bvirtpi[Gd], FBBints.params->coltot[Gkd]);
        ...
        dpd_->sort_3d(WbcA, WAbc, nirreps, Gijk, FBBints.params->coltot, FBBints.params->colidx,
		...
		for(Gab=0; Gab < nirreps; Gab++) {
		  ...
		  dpd_->free_dpd_block(WbcA[Gab], FBBints.params->coltot[Gab], avirtpi[Gc]);

		  WcAb[Gab] = dpd_->dpd_block_matrix(FBAints.params->coltot[Gab], bvirtpi[Gc]);
		}
		...
		for(Gd=0; Gd < nirreps; Gd++) {
		  ...
		  FBAints.matrix[Gjd] = dpd_->dpd_block_matrix(avirtpi[Gd], FBAints.params->coltot[Gjd]);
		  dpd_->buf4_mat_irrep_rd_block(&FBAints, Gjd, jd, avirtpi[Gd]);
		  ...
		  dpd_->free_dpd_block(FBAints.matrix[Gjd], avirtpi[Gd], FBAints.params->coltot[Gjd]);
		  ...
		  FBAints.matrix[Gkd] = dpd_->dpd_block_matrix(avirtpi[Gd], FBAints.params->coltot[Gkd]);
		  dpd_->buf4_mat_irrep_rd_block(&FBAints, Gkd, kd, avirtpi[Gd]);
		  ...
		  dpd_->free_dpd_block(FBAints.matrix[Gkd], avirtpi[Gd], FBAints.params->coltot[Gkd]);
        ...
        dpd_->sort_3d(WcAb, WAbc, nirreps, Gijk, FBAints.params->coltot, FBAints.params->colidx,
		...
		for(Gab=0; Gab < nirreps; Gab++) {
		  ...
		  dpd_->free_dpd_block(WcAb[Gab], FBAints.params->coltot[Gab], bvirtpi[Gc]);

		  WbAc[Gab] = dpd_->dpd_block_matrix(FBAints.params->coltot[Gab], bvirtpi[Gc]);
		}
        ...
        dpd_->sort_3d(WbAc, WAbc, nirreps, Gijk, FBAints.params->coltot, FBAints.params->colidx,
		       FBAints.params->colorb, FBAints.params->rsym, FBAints.params->ssym,
		...
		for(Gab=0; Gab < nirreps; Gab++) {
		  ...
		  dpd_->free_dpd_block(WbAc[Gab], FBAints.params->coltot[Gab], bvirtpi[Gc]);
		}
		...
		for(Gab=0; Gab < nirreps; Gab++) {
		  ...
		  VAbc[Gab] = dpd_->dpd_block_matrix(FABints.params->coltot[Gab], bvirtpi[Gc]);
		...
		for(Gab=0; Gab < nirreps; Gab++) {
		  ...
		  dpd_->free_dpd_block(WAbc[Gab], FABints.params->coltot[Gab], bvirtpi[Gc]);
		  dpd_->free_dpd_block(VAbc[Gab], FABints.params->coltot[Gab], bvirtpi[Gc]);
		}
