...
extern void T3_UHF_AAB_abc(double ***W, double ***V, int disc, int nirreps,
    ...
    double T3_grad_UHF_BBA(void)
    {
      ...
      dpd_->buf4_init(&S2BB, PSIF_CC_MISC, 0, 10, 15, 12, 17, 0, "Sijab");
      dpd_->buf4_init(&S2BA, PSIF_CC_MISC, 0, 23, 29, 23, 29, 0, "SiJaB");
      for(h=0; h < nirreps; h++) {
	dpd_->buf4_mat_irrep_init(&S2BB, h);
  	dpd_->buf4_mat_irrep_rd(&S2BB, h);
	dpd_->buf4_mat_irrep_init(&S2BA, h);
      }
      ...
      dpd_->buf4_init(&Gijab, PSIF_CC_GAMMA, 0, 10, 15, 12, 17, 0, "Gijab");
      dpd_->buf4_init(&GiJaB, PSIF_CC_GAMMA, 0, 23, 29, 23, 29, 0, "GiJaB");
      for(h=0; h < nirreps; h++) {
	...
	dpd_->buf4_mat_irrep_init(&GiJaB, h);
      ...
      dpd_->buf4_init(&Gijka, PSIF_CC_GAMMA, 0, 10, 30, 12, 30, 0, "Gijka");
	...
	dpd_->buf4_mat_irrep_rd(&GiJkA, h);
      ...
      dpd_->buf4_init(&Gidab, PSIF_CC_GAMMA, 0, 30, 15, 30, 17, 0, "Gidab");
	...
	for(Gj=0; Gj < nirreps; Gj++) {
	  for(Gk=0; Gk < nirreps; Gk++) {
	    ...
	    for(Gab=0; Gab < nirreps; Gab++) {
	      ...
	      WabC[Gab] = dpd_->dpd_block_matrix(FBBints.params->coltot[Gab], avirtpi[Gc]);
	      VabC[Gab] = dpd_->dpd_block_matrix(FBBints.params->coltot[Gab], avirtpi[Gc]);
	      XabC[Gab] = dpd_->dpd_block_matrix(FBBints.params->coltot[Gab], avirtpi[Gc]);
	    }

	    for(Ga=0; Ga < nirreps; Ga++) {
	      Gbc = Ga ^ Gijk;
	      Y1[Ga] = dpd_->dpd_block_matrix(bvirtpi[Ga], FABints.params->coltot[Gbc]); /* beta-alpha-beta */
	      Y2[Ga] = dpd_->dpd_block_matrix(bvirtpi[Ga], FBAints.params->coltot[Gbc]); /* beta-beta-alpha */
	    }
		...
		for(k=0; k < aoccpi[Gk]; k++) {
		  ...
		  for(h=0; h < nirreps; h++) {
		    ...
		    dpd_->buf4_mat_irrep_init(&DBAints, h);
		    dpd_->buf4_mat_irrep_rd(&DBAints, h);
		  }
		  ...
		  for(Gd=0; Gd < nirreps; Gd++) {
		    ...
		    if(nrows && ncols && nlinks) {
		      FABints.matrix[Gkd] = dpd_->dpd_block_matrix(nrows, nlinks);
		      dpd_->buf4_mat_irrep_rd_block(&FABints, Gkd, kd, nrows);
			...
			for(a=0; a < bvirtpi[Ga]; a++) {
		      ...
		      dpd_->free_dpd_block(FABints.matrix[Gkd], nrows, nlinks);
		      free_block(Z);
		    } /* nrows && ncols && nlinks */
		  } /* Gd */
		  ...
		  for(Gd=0; Gd < nirreps; Gd++) {
		    ...
		    if(nrows && ncols && nlinks) {
		      ...
		      FBBints.matrix[Gid] = dpd_->dpd_block_matrix(nrows, nlinks);
		      dpd_->buf4_mat_irrep_rd_block(&FBBints, Gid, id, nrows);
		      Z = block_matrix(nrows, ncols);
			...
			for(c=0; c < ncols; c++) {
		      ...
		      dpd_->free_dpd_block(FBBints.matrix[Gid], nrows, nlinks);
		      free_block(Z);
		    } /* nrows && ncols && nlinks */
		  } /* Gd */
		  ...
		  for(Gd=0; Gd < nirreps; Gd++) {
		    ...
		    if(nrows && ncols && nlinks) {
		      ...
		      FBAints.matrix[Gid] = dpd_->dpd_block_matrix(ncols, nlinks);
		      dpd_->buf4_mat_irrep_rd_block(&FBAints, Gid, id, ncols);
		      Z = block_matrix(nrows, ncols);
			...
			for(d=0; d < ncols; d++) {
		      ...
		      dpd_->free_dpd_block(FBAints.matrix[Gid], ncols, nlinks);
		      free_block(Z);
		  ...
		  for(h=0; h < nirreps; h++) {
		    ...
		    dpd_->buf4_mat_irrep_close(&DBAints, h);
		  }
	    ...
	    for(Gab=0; Gab < nirreps; Gab++) {
	      Gc = Gab ^ Gijk;
	      dpd_->free_dpd_block(WabC[Gab], FBBints.params->coltot[Gab], avirtpi[Gc]);
	      dpd_->free_dpd_block(VabC[Gab], FBBints.params->coltot[Gab], avirtpi[Gc]);
	      dpd_->free_dpd_block(XabC[Gab], FBBints.params->coltot[Gab], avirtpi[Gc]);
	    }
	    for(Ga=0; Ga < nirreps; Ga++) {
	      Gbc = Ga ^ Gijk;
	      dpd_->free_dpd_block(Y1[Ga], bvirtpi[Ga], FABints.params->coltot[Gbc]);
	      dpd_->free_dpd_block(Y2[Ga], bvirtpi[Ga], FBAints.params->coltot[Gbc]);
	    }
      ...
      for(h=0; h < nirreps; h++) {
	dpd_->buf4_mat_irrep_wrt(&S2BB, h);
	dpd_->buf4_mat_irrep_close(&S2BB, h);
      }
      for(h=0; h < nirreps; h++) {
	dpd_->buf4_mat_irrep_wrt(&S2BA, h);
	dpd_->buf4_mat_irrep_close(&S2BA, h);
      }
      for(h=0; h < nirreps; h++) {
	...
	dpd_->buf4_mat_irrep_close(&GIdAb, h);
      ...
      dpd_->buf4_close(&S2BB);
      /* Combine SIjAb and SiJaB */
      dpd_->buf4_sort_axpy(&S2BA, PSIF_CC_MISC, qpsr, 22, 28, "SIjAb", 1);
      dpd_->buf4_close(&S2BA);
      dpd_->buf4_close(&Gijab);
      /* Combine GIjAb and GiJaB */
      dpd_->buf4_sort_axpy(&GiJaB, PSIF_CC_GAMMA, qpsr, 22, 28, "GIjAb", 1);
      dpd_->buf4_close(&GiJaB);
      ...
      dpd_->buf4_init(&EBBints, PSIF_CC_EINTS, 0, 31, 10, 31, 12, 0, "E <ak||ij> (ak, i>j)");
              ...
              for (Gc=0; Gc < nirreps; ++Gc) {
                for (c=0; c < avirtpi[Gc]; ++c) {
                  ...
                  for (Gij=0; Gij < nirreps; ++Gij) {
                    Gk = Gij ^ Gabc;
                    WijK[Gij] = dpd_->dpd_block_matrix(T2BB.params->coltot[Gij], aoccpi[Gk]);
                    VijK[Gij] = dpd_->dpd_block_matrix(T2BB.params->coltot[Gij], aoccpi[Gk]);
                  }
