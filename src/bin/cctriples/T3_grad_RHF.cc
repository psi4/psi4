...
namespace psi { namespace cctriples {
    ...
    void T3_grad_RHF(void)
    {
      ...
      dpd_->buf4_init(&S2, PSIF_CC_MISC, 0, 0, 5, 0, 5, 0, "SIjAb");
      dpd_->buf4_init(&Fints, PSIF_CC_FINTS, 0, 10, 5, 10, 5, 0, "F <ia|bc>");
      dpd_->buf4_init(&F2ints, PSIF_CC_FINTS, 0, 10, 5, 10, 5, 0, "F <ia|bc>");
	...
	dpd_->buf4_mat_irrep_init(&F2ints, h);
	dpd_->buf4_mat_irrep_rd(&F2ints, h);
	  ...
	  for(Gk=0; Gk < nirreps; Gk++) {
		...
		for(k=0; k < occpi[Gk]; k++) {
		  ...
		  for(Gab=0; Gab < nirreps; Gab++) {
		    ...
		    M[Gab] = dpd_->dpd_block_matrix(Fints.params->coltot[Gab], virtpi[Gc]);
		  }
		  timer_off("malloc");
...
// 					     - 2 * V[Gcb][cb][a] - 2 * V[Gac][ac][b] - 2 * V[Gba][ba][c])/denom;
		  ...
		  for(Gd=0; Gd < nirreps; Gd++) {
		    ...
		    if(nrows && ncols && nlinks) {
		      ...
		      Fints.matrix[Gid] = dpd_->dpd_block_matrix(virtpi[Gd], Fints.params->coltot[Gid]);
		      dpd_->buf4_mat_irrep_rd_block(&Fints, Gid, id, virtpi[Gd]);
		  ...
		  for(Gab=0; Gab < nirreps; Gab++) {
		    ...
		    dpd_->free_dpd_block(M[Gab],Fints.params->coltot[Gab],virtpi[Gc]);
		  }
		  timer_off("malloc");
