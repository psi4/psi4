...
void T3_UHF_AAA_abc(double ***W, double ***V, int disc, int nirreps, int A,
  ...
  for (h = 0; h < nirreps; h++) {
    dpd_->buf4_mat_irrep_init(F, h);
    dpd_->buf4_mat_irrep_rd(F, h);
  }
  ...
  for (Gij = 0; Gij < nirreps; ++Gij) {
    ...
    W2[Gij] = dpd_->dpd_block_matrix(C2->params->coltot[Gij], occpi[Gk]);
  ...
  for (Gd = 0; Gd < nirreps; Gd++) {
    ...
    if (nrows && ncols && nlinks) {
      C2->matrix[Gcd] = dpd_->dpd_block_matrix(nlinks, nrows);
      dpd_->buf4_mat_irrep_rd_block(C2, Gcd, cd, nlinks);
      C_DGEMM('t', 't', nrows, ncols, nlinks, -1.0, C2->matrix[Gcd][0], nrows,
          &(F->matrix[Gab][ab][kd]), nlinks, 1.0, W[Gij][0], ncols);
      dpd_->free_dpd_block(C2->matrix[Gcd], nlinks, nrows);
    }
    ...
    if (nrows && ncols && nlinks) {
      C2->matrix[Gbd] = dpd_->dpd_block_matrix(nlinks, nrows);
      dpd_->buf4_mat_irrep_rd_block(C2, Gbd, bd, nlinks);
      C_DGEMM('t', 't', nrows, ncols, nlinks, -1.0, C2->matrix[Gbd][0], nrows,
          &(F->matrix[Gca][ca][kd]), nlinks, 1.0, W[Gij][0], ncols);
      dpd_->free_dpd_block(C2->matrix[Gbd], nlinks, nrows);
    }
    ...
    if (nrows && ncols && nlinks) {
      C2->matrix[Gad] = dpd_->dpd_block_matrix(nlinks, nrows);
      dpd_->buf4_mat_irrep_rd_block(C2, Gad, ad, nlinks);
      C_DGEMM('t', 't', nrows, ncols, nlinks, 1.0, C2->matrix[Gad][0], nrows,
          &(F->matrix[Gcb][cb][kd]), nlinks, 1.0, W[Gij][0], ncols);
      dpd_->free_dpd_block(C2->matrix[Gad], nlinks, nrows);
    }
  } /* Gd */
  ...
  for (Gl = 0; Gl < nirreps; ++Gl) {
    ...
    dpd_->buf4_mat_irrep_init(C2, Gab);
    dpd_->buf4_mat_irrep_rd(C2, Gab);
    ...
    if (nrows && ncols && nlinks) {
      E->matrix[Gcl] = dpd_->dpd_block_matrix(nlinks, nrows);
      dpd_->buf4_mat_irrep_rd_block(E, Gcl, cl, nlinks);
      C_DGEMM('t', 't', nrows, ncols, nlinks, -1.0, E->matrix[Gcl][0], nrows,
          &(C2->matrix[Gab][ab][kl]), nlinks, 1.0, W[Gij][0], ncols);
      dpd_->free_dpd_block(E->matrix[Gcl], nlinks, nrows);
    }
    dpd_->buf4_mat_irrep_close(C2, Gab);
    ...
    dpd_->buf4_mat_irrep_init(C2, Gca);
    dpd_->buf4_mat_irrep_rd(C2, Gca);
    ...
    if (nrows && ncols && nlinks) {
      E->matrix[Gbl] = dpd_->dpd_block_matrix(nlinks, nrows);
      dpd_->buf4_mat_irrep_rd_block(E, Gbl, bl, nlinks);
      C_DGEMM('t', 't', nrows, ncols, nlinks, -1.0, E->matrix[Gbl][0], nrows,
          &(C2->matrix[Gca][ca][kl]), nlinks, 1.0, W[Gij][0], ncols);
      dpd_->free_dpd_block(E->matrix[Gbl], nlinks, nrows);
    }
    dpd_->buf4_mat_irrep_close(C2, Gca);
    ...
    dpd_->buf4_mat_irrep_init(C2, Gcb);
    dpd_->buf4_mat_irrep_rd(C2, Gcb);
    ...
    if (nrows && ncols && nlinks) {
      E->matrix[Gal] = dpd_->dpd_block_matrix(nlinks, nrows);
      dpd_->buf4_mat_irrep_rd_block(E, Gal, al, nlinks);
      C_DGEMM('t', 't', nrows, ncols, nlinks, 1.0, E->matrix[Gal][0], nrows,
          &(C2->matrix[Gcb][cb][kl]), nlinks, 1.0, W[Gij][0], ncols);
      dpd_->free_dpd_block(E->matrix[Gal], nlinks, nrows);
    }
    dpd_->buf4_mat_irrep_close(C2, Gcb);
  } /* Gl */
  ...
  //Sort W2[ik][j]->W[ij][k]
  dpd_->sort_3d(W2, W, nirreps, Gabc ^ GX3, C2->params->coltot,
  ...
  for (Gij = 0; Gij < nirreps; ++Gij) {
    Gk = Gij ^ Gabc ^ GX3; /* changed */
    dpd_->free_dpd_block(W2[Gij], C2->params->coltot[Gij], occpi[Gk]);
  } /* Gij */
...
void T3_UHF_AAB_abc(double ***W, double ***V, int disc, int nirreps, int A,
  ...
  for (h=0; h < nirreps; h++) {
    dpd_->buf4_mat_irrep_init(FAA, h);
    dpd_->buf4_mat_irrep_rd(FAA, h);
    dpd_->buf4_mat_irrep_init(FAB, h);
    dpd_->buf4_mat_irrep_rd(FAB, h);
    dpd_->buf4_mat_irrep_init(FBA, h);
    dpd_->buf4_mat_irrep_rd(FBA, h);
  }
  ...
  for (Gd=0; Gd < nirreps; Gd++) {
    ...
    if (nrows && ncols && nlinks) {
      T2AA->matrix[Gad] = dpd_->dpd_block_matrix(nlinks, nrows);
      dpd_->buf4_mat_irrep_rd_block(T2AA, Gad, ad, nlinks);
      C_DGEMM('t', 't', nrows, ncols, nlinks, 1.0, T2AA->matrix[Gad][0], nrows,
          &(FBA->matrix[Gcb][cb][kd]), nlinks, 1.0, W[Gij][0], ncols);
      dpd_->free_dpd_block(T2AA->matrix[Gad], nlinks, nrows);
    }
    ...
    if (nrows && ncols && nlinks) {
      T2AA->matrix[Gbd] = dpd_->dpd_block_matrix(nlinks, nrows);
      dpd_->buf4_mat_irrep_rd_block(T2AA, Gbd, bd, nlinks);
      C_DGEMM('t', 't', nrows, ncols, nlinks, -1.0, T2AA->matrix[Gbd][0],
          nrows, &(FBA->matrix[Gca][ca][kd]), nlinks, 1.0, W[Gij][0], ncols);
      dpd_->free_dpd_block(T2AA->matrix[Gbd], nlinks, nrows);
    }
  } /* Gd */
  ...
  for (Gl=0; Gl < nirreps; ++Gl) {
    ...
    dpd_->buf4_mat_irrep_init(T2BA, Gcb);
    dpd_->buf4_mat_irrep_rd(T2BA, Gcb);
    ...
    if (nrows && ncols && nlinks) {
      EAA->matrix[Gal] = dpd_->dpd_block_matrix(nlinks, nrows);
      dpd_->buf4_mat_irrep_rd_block(EAA, Gal, al, nlinks);
      C_DGEMM('t', 't', nrows, ncols, nlinks, 1.0, EAA->matrix[Gal][0], nrows,
          &(T2BA->matrix[Gcb][cb][kl]), nlinks, 1.0, W[Gij][0], ncols);
      dpd_->free_dpd_block(EAA->matrix[Gal], nlinks, nrows);
    }
    dpd_->buf4_mat_irrep_close(T2BA, Gcb);
    ...
    dpd_->buf4_mat_irrep_init(T2BA, Gca);
    dpd_->buf4_mat_irrep_rd(T2BA, Gca);
    ...
    if (nrows && ncols && nlinks) {
      EAA->matrix[Gbl] = dpd_->dpd_block_matrix(nlinks, nrows);
      dpd_->buf4_mat_irrep_rd_block(EAA, Gbl, bl, nlinks);
      C_DGEMM('t', 't', nrows, ncols, nlinks, -1.0, EAA->matrix[Gbl][0], nrows,
          &(T2BA->matrix[Gca][ca][kl]), nlinks, 1.0, W[Gij][0], ncols);
      dpd_->free_dpd_block(EAA->matrix[Gbl], nlinks, nrows);
    }
    dpd_->buf4_mat_irrep_close(T2BA, Gca);
  } /* Gl */

  /* Allocate the alpha-beta-alpha array */
  for (Gij=0; Gij < nirreps; ++Gij) {
    Gk = Gabc ^ Gij ^GX3; /* assumes totally symmetric! */
    W2[Gij] = dpd_->dpd_block_matrix(T2AB->params->coltot[Gij], aoccpi[Gk]);
  }
  ...
  for (Gd=0; Gd < nirreps; ++Gd) {
    ...
    if (nrows && ncols && nlinks) {
      T2AB->matrix[Gbd] = dpd_->dpd_block_matrix(nlinks, nrows);
      dpd_->buf4_mat_irrep_rd_block(T2AB, Gbd, bd, nlinks);
      C_DGEMM('t', 't', nrows, ncols, nlinks, -1.0, T2AB->matrix[Gbd][0],
          nrows, &(FAB->matrix[Gac][ac][jd]), nlinks, 1.0, W2[Gik][0], ncols);
      dpd_->free_dpd_block(T2AB->matrix[Gbd], nlinks, nrows);
    }
    ...
    if (nrows && ncols && nlinks) {
      T2AB->matrix[Gad] = dpd_->dpd_block_matrix(nlinks, nrows);
      dpd_->buf4_mat_irrep_rd_block(T2AB, Gad, ad, nlinks);
      C_DGEMM('t', 't', nrows, ncols, nlinks, 1.0, T2AB->matrix[Gad][0], nrows,
          &(FAB->matrix[Gbc][bc][jd]), nlinks, 1.0, W2[Gik][0], ncols);
      dpd_->free_dpd_block(T2AB->matrix[Gad], nlinks, nrows);
    }
  } /* Gd */
  ...
  for (Gl=0; Gl < nirreps; ++Gl) {
    ...
    dpd_->buf4_mat_irrep_init(T2AA, Gab);
    dpd_->buf4_mat_irrep_rd(T2AA, Gab);
    ...
    if (nrows && ncols && nlinks) {
      EAB->matrix[Gcl] = dpd_->dpd_block_matrix(nlinks, nrows);
      dpd_->buf4_mat_irrep_rd_block(EAB, Gcl, cl, nlinks);
      C_DGEMM('t', 't', nrows, ncols, nlinks, 1.0, EAB->matrix[Gcl][0], nrows,
          &(T2AA->matrix[Gab][ab][jl]), nlinks, 1.0, W2[Gik][0], ncols);
      dpd_->free_dpd_block(EAB->matrix[Gcl], nlinks, nrows);
    }
    dpd_->buf4_mat_irrep_close(T2AA, Gab);
  } /* Gl */

  /*=== Sort W2[Ik][J] -> W[IJ][k] ===*/
  dpd_->sort_3d(W2, W, nirreps, Gabc^GX3, T2AB->params->coltot,
  ...
  for (Gl=0; Gl < nirreps; ++Gl) {
    ...
    dpd_->buf4_mat_irrep_init(T2AB, Gac);
    dpd_->buf4_mat_irrep_rd(T2AB, Gac);
    ...
    if (nrows && ncols && nlinks) {
      EBA->matrix[Gbl] = dpd_->dpd_block_matrix(nlinks, nrows);
      dpd_->buf4_mat_irrep_rd_block(EBA, Gbl, bl, nlinks);
      C_DGEMM('t', 't', nrows, ncols, nlinks, -1.0, EBA->matrix[Gbl][0], nrows,
          &(T2AB->matrix[Gac][ac][il]), nlinks, 1.0, W2[Gkj][0], ncols);
      dpd_->free_dpd_block(EBA->matrix[Gbl], nlinks, nrows);
    }
    dpd_->buf4_mat_irrep_close(T2AB, Gac);
    ...
    dpd_->buf4_mat_irrep_init(T2AB, Gbc);
    dpd_->buf4_mat_irrep_rd(T2AB, Gbc);
    ...
    if (nrows && ncols && nlinks) {
      EBA->matrix[Gal] = dpd_->dpd_block_matrix(nlinks, nrows);
      dpd_->buf4_mat_irrep_rd_block(EBA, Gal, al, nlinks);
      C_DGEMM('t', 't', nrows, ncols, nlinks, 1.0, EBA->matrix[Gal][0], nrows,
          &(T2AB->matrix[Gbc][bc][il]), nlinks, 1.0, W2[Gkj][0], ncols);
      dpd_->free_dpd_block(EBA->matrix[Gal], nlinks, nrows);
    }
    dpd_->buf4_mat_irrep_close(T2AB, Gbc);
  } /* Gl */

  /*=== Sort W2[kJ][I] -> W[IJ][k] ===*/
  dpd_->sort_3d(W2, W, nirreps, Gabc^GX3, T2BA->params->coltot,
  ...
  for (Gij=0; Gij < nirreps; ++Gij) {
    Gk = Gabc ^ Gij ^GX3; /* assumes totally symmetric! */
    dpd_->free_dpd_block(W2[Gij], T2AB->params->coltot[Gij], aoccpi[Gk]);
    W2[Gij] = dpd_->dpd_block_matrix(T2BA->params->coltot[Gij], aoccpi[Gk]);
  }
  ...
  for (Gd=0; Gd < nirreps; ++Gd) {
    ...
    if (nrows && ncols && nlinks) {
      T2BA->matrix[Gcd] = dpd_->dpd_block_matrix(nlinks, nrows);
      dpd_->buf4_mat_irrep_rd_block(T2BA, Gcd, cd, nlinks);
      C_DGEMM('t', 't', nrows, ncols, nlinks, -1.0, T2BA->matrix[Gcd][0],
          nrows, &(FAA->matrix[Gab][ab][jd]), nlinks, 1.0, W2[Gki][0], ncols);
      dpd_->free_dpd_block(T2BA->matrix[Gcd], nlinks, nrows);
    }
  } /* Gd */
  ...
  for (Gij=0; Gij < nirreps; Gij++) {
    Gk = Gabc ^ Gij ^ GX3;
    dpd_->free_dpd_block(W2[Gij], T2BA->params->coltot[Gij], aoccpi[Gk]);
  }
  ...
  for (h=0; h < nirreps; h++) {
    dpd_->buf4_mat_irrep_close(FAA, h);
    dpd_->buf4_mat_irrep_close(FAB, h);
    dpd_->buf4_mat_irrep_close(FBA, h);
  }
}
