...
void X1_build(const char *pert, int irrep, double omega)
{
  ...
  dpd_->buf4_init(&W, PSIF_CC_HBAR, 0, 10, 10, 10, 10, 0, "2 W(jb,ME) + W(Jb,Me)");
  ...
  for(Gam=0; Gam < moinfo.nirreps; Gam++) {
    ...
    dpd_->buf4_mat_irrep_init(&X2, Gim);
    dpd_->buf4_mat_irrep_rd(&X2, Gim);
    dpd_->buf4_mat_irrep_shift13(&X2, Gim);
    ...
    dpd_->buf4_mat_irrep_close(&X2, Gim);
  }
