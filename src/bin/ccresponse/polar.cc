...
void polar(void)
{
  ...
  for(i=0; i < params.nomega; i++) {
    ...
    if(params.omega[i] != 0.0)
      ...
      psi::fprintf(outfile,   "   Evaluated at omega = %8.6f E_h (Inf nm, %5.3f eV, %8.2f cm-1)\n", 
  ...
  if(params.nomega > 1) {  /* print a summary table for multi-wavelength calcs */
    ...
    psi::fprintf(outfile, "\n\t-------------------------------\n");
    if (params.wfn == "CC2")
      psi::fprintf(outfile,   "\t      CC2 Polarizability\n");
    else
      psi::fprintf(outfile,   "\t      CCSD Polarizability\n");
    psi::fprintf(outfile,   "\t-------------------------------\n");
    psi::fprintf(outfile,   "\t    Omega          alpha\n");
    psi::fprintf(outfile,   "\t E_h      nm        a.u.        \n");
    psi::fprintf(outfile,   "\t-----   ------ ----------------\n");
    for(i=0; i < params.nomega; i++)
