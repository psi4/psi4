	   ...
	   const char *pert_x, int irrep_x, double omega)
{
  ...
  dpd_file2_init(&mu1, PSIF_CC_OEI, irrep_c, 0, 1, lbl);
  ...
  dpd_file2_init(&z1, PSIF_CC_TMP0, 0, 0, 1, "z_IA");

  sprintf(lbl, "%sBAR_MI", pert_c);
  dpd_file2_init(&mu1, PSIF_CC_OEI, irrep_c, 0, 0, lbl);
  sprintf(lbl, "X_%s_IA (%5.3f)", pert_x, omega);
  dpd_file2_init(&X1, PSIF_CC_OEI, irrep_x, 0, 1, lbl);
  ...
  dpd_file2_init(&mu1, PSIF_CC_OEI, irrep_c, 1, 1, lbl);
  sprintf(lbl, "X_%s_IA (%5.3f)", pert_x, omega);
  dpd_file2_init(&X1, PSIF_CC_OEI, irrep_x, 0, 1, lbl);
  ...
  dpd_file2_init(&xc, PSIF_CC_TMP0, 0, 0, 0, "XC_IJ");
  sprintf(lbl, "%s_IA", pert_c);
  dpd_file2_init(&mu, PSIF_CC_OEI, irrep_c, 0, 1, lbl);
  sprintf(lbl, "X_%s_IA (%5.3f)", pert_x, omega);
  dpd_file2_init(&X1, PSIF_CC_OEI, irrep_x, 0, 1, lbl);
  ...
  dpd_buf4_init(&z2, PSIF_CC_TMP0, 0, 0, 5, 0, 5, 0, "Z(Ij,Ab) Final");
  dpd_buf4_scm(&z2, 0);
  dpd_buf4_close(&z2);
  ...
  dpd_buf4_init(&Z, PSIF_CC_TMP1, 0, 0, 5, 0, 5, 0, "Z(Ij,Ab)");
  sprintf(lbl, "%sBAR_MbIj", pert_c);
  dpd_buf4_init(&mu2, PSIF_CC_LR, irrep_c, 10, 0, 10, 0, 0, lbl);
  dpd_contract244(&X1, &mu2, &Z, 0, 0, 1, 1, 0);
  dpd_buf4_close(&mu2);
  dpd_buf4_axpy(&Z, &z2, -1);
  dpd_buf4_sort(&Z, PSIF_CC_TMP1, qpsr, 0, 5, "Z(jI,bA)");
  dpd_buf4_close(&Z);
  dpd_buf4_init(&Z, PSIF_CC_TMP1, 0, 0, 5, 0, 5, 0, "Z(jI,bA)");
  dpd_buf4_axpy(&Z, &z2, -1);
  dpd_buf4_close(&Z);
  ...
  if(params.sekino) {  /* disconnected piece for Sekino-Bartlett modelIII */
    ...
    dpd_file2_init(&z1, PSIF_CC_TMP0, irrep_c, 0, 1, lbl);
