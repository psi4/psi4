...
void init_X(const char *pert, int irrep, double omega)
{
  ...
  dpd_->file2_init(&mu1, PSIF_CC_OEI, irrep, 0, 1, lbl);
  sprintf(lbl, "X_%s_IA (%5.3f)", pert, omega);
  if(!params.restart || !psio_tocscan(PSIF_CC_OEI, lbl)) {
    dpd_->file2_copy(&mu1, PSIF_CC_OEI, lbl);
  ...
  dpd_->buf4_init(&mu2, PSIF_CC_LR, irrep, 0, 5, 0, 5, 0, lbl);
  sprintf(lbl, "X_%s_IjAb (%5.3f)", pert, omega);
  if(!params.restart || !psio_tocscan(PSIF_CC_LR, lbl)) {
    dpd_->buf4_copy(&mu2, PSIF_CC_LR, lbl);
