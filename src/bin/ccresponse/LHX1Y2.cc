...
//  fprintf(outfile, "L(1)HX1Y2 = %20.12f\n", polar);
  ...
  dpd_->file2_init(&z1, PSIF_CC_TMP0, irrep_x, 0, 0, lbl);
  sprintf(lbl, "X_%s_IA (%5.3f)", pert_x, omega_x);
  dpd_->file2_init(&X1, PSIF_CC_OEI, irrep_x, 0, 1, lbl);
  dpd_->file2_init(&F, PSIF_CC_OEI, 0, 0, 1, "FME");
  dpd_->contract222(&F, &X1, &z1, 0, 0, 1, 0);
  dpd_->file2_close(&F);
  dpd_->file2_close(&X1);
  ...
  sprintf(lbl, "X_%s_IjAb (%5.3f)", pert_y, omega_y);
  dpd_->buf4_init(&Y2, PSIF_CC_LR, irrep_y, 0, 5, 0, 5, 0, lbl);
  dpd_->contract244(&z1, &Y2, &Z1, 0, 0, 0, 1, 0);
  dpd_->buf4_close(&Y2);
  dpd_->file2_close(&z1);
  ...
  dpd_->file2_init(&z1, PSIF_CC_TMP0, irrep_x, 1, 1, lbl);
  sprintf(lbl, "X_%s_IA (%5.3f)", pert_x, omega_x);
  dpd_->file2_init(&X1, PSIF_CC_OEI, irrep_x, 0, 1, lbl);
  dpd_->file2_init(&F, PSIF_CC_OEI, 0, 0, 1, "FME");
  dpd_->contract222(&X1, &F, &z1, 1, 1, -1, 0);
  dpd_->file2_close(&F);
  dpd_->file2_close(&X1);
  ...
  sprintf(lbl, "X_%s_IjAb (%5.3f)", pert_y, omega_y);
  dpd_->buf4_init(&Y2, PSIF_CC_LR, irrep_y, 0, 5, 0, 5, 0, lbl);
  dpd_->contract424(&Y2, &z1, &Z1, 3, 1, 0, 1, 0);
  dpd_->buf4_close(&Y2);
  dpd_->file2_close(&z1);
  dpd_->buf4_axpy(&Z1, &Z, 1);
  dpd_->buf4_sort(&Z1, PSIF_CC_TMP1, qpsr, 0, 5, "Z(jI,bA)");
  ...
  dpd_->buf4_init(&Z1, PSIF_CC_TMP0, irrep_x, 11, 10, 11, 10, 0, lbl);
  ...
  for(Gbm=0; Gbm < moinfo.nirreps; Gbm++) {
    ...
    dpd_->buf4_mat_irrep_init(&Z1, Gbm);
    ...
    dpd_->buf4_mat_irrep_wrt(&Z1, Gbm);
    dpd_->buf4_mat_irrep_close(&Z1, Gbm);
  ...
  dpd_->buf4_sort(&Z1, PSIF_CC_TMP0, qsrp, 10, 10, lbl);
  dpd_->buf4_close(&Z1);

  sprintf(lbl, "Z_%s_WMbEj (bM,Ej)", pert_x);
  dpd_->buf4_init(&Z1, PSIF_CC_TMP0, irrep_x, 11, 11, 11, 11, 0, lbl);
  dpd_->buf4_init(&W, PSIF_CC_HBAR, 0, 0, 11, 0, 11, 0, "WMnIe (Mn,eI)");
  dpd_->contract244(&X1, &W, &Z1, 0, 0, 0, -1, 0);
  dpd_->buf4_close(&W);
  sprintf(lbl, "Z_%s_MbEj (ME,jb)", pert_x);
  dpd_->buf4_sort_axpy(&Z1, PSIF_CC_TMP0, qrsp, 10, 10, lbl, 1);
  dpd_->buf4_close(&Z1);

  sprintf(lbl, "Z_%s_MbeJ (Mb,eJ)", pert_x);
  dpd_->buf4_init(&Z1, PSIF_CC_TMP0, irrep_x, 10, 11, 10, 11, 0, lbl);
  dpd_->buf4_init(&W, PSIF_CC_HBAR, 0, 0, 11, 0, 11, 0, "WMnIe (Mn,eI)");
  dpd_->contract424(&W, &X1, &Z1, 1, 0, 1, 1, 0);
  dpd_->buf4_close(&W);
  sprintf(lbl, "Z_%s_MbeJ (Me,Jb)", pert_x);
  dpd_->buf4_sort(&Z1, PSIF_CC_TMP0, prsq, 10, 10, lbl);
  dpd_->buf4_close(&Z1);

  sprintf(lbl, "Z_%s_MbeJ (bM,eJ)", pert_x);
  dpd_->buf4_init(&Z1, PSIF_CC_TMP0, irrep_x, 11, 11, 11, 11, 0, lbl);
  dpd_->buf4_init(&W, PSIF_CC_HBAR, 0, 11, 5, 11, 5, 0, "WAmEf");
  dpd_->contract424(&W, &X1, &Z1, 3, 1, 0, -1, 0);
  dpd_->buf4_close(&W);
  sprintf(lbl, "Z_%s_MbeJ (Me,Jb)", pert_x);
  dpd_->buf4_sort_axpy(&Z1, PSIF_CC_TMP0, qrsp, 10, 10, lbl, 1);
  dpd_->buf4_close(&Z1);
  ...
  dpd_->buf4_scmcopy(&Z1, PSIF_CC_TMP0, lbl, 2);
  dpd_->buf4_close(&Z1);
  sprintf(lbl, "Z_%s_MbeJ (Me,Jb)", pert_x);
  dpd_->buf4_init(&Z1, PSIF_CC_TMP0, irrep_x, 10, 10, 10, 10, 0, lbl);
  sprintf(lbl, "Z_%s_(2MbEj+MbeJ) (ME,JB)", pert_x);
  dpd_->buf4_init(&Z2, PSIF_CC_TMP0, irrep_x, 10, 10, 10, 10, 0, lbl);
  dpd_->buf4_axpy(&Z1, &Z2, 1);
  dpd_->buf4_close(&Z2);
  dpd_->buf4_close(&Z1);
  ...
  dpd_->contract444(&Y2, &Z, &Z1, 0, 1, 0.5, 0);
  dpd_->buf4_close(&Z);
  dpd_->buf4_close(&Y2);
  dpd_->buf4_close(&Z1);
  ...
  dpd_->buf4_init(&Y2, PSIF_CC_LR, irrep_y, 10, 10, 10, 10, 0, lbl);
  dpd_->contract444(&Y2, &Z, &Z1, 0, 1, 1, 0);
  dpd_->buf4_close(&Y2);
  dpd_->buf4_close(&Z);
  dpd_->buf4_close(&Z1);
  ...
  dpd_->buf4_close(&W);
  dpd_->buf4_init(&Z1, PSIF_CC_TMP0, 0, 0, 5, 0, 5, 0, "Z(Ij,Ab)");
  dpd_->contract424(&Y2, &z, &Z1, 1, 0, 1, -1, 0);
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 0, 5, 0, 5, 0, "Z(Ij,Ab) Final");
  dpd_->buf4_axpy(&Z1, &Z, 1);
  dpd_->buf4_close(&Z);
...
/*   dpd_dot24(&X1, &W, &z, 0, 0, 1, 0); */
  ...
  dpd_->buf4_init(&Z1, PSIF_CC_TMP0, 0, 0, 5, 0, 5, 0, "Z(Ij,Ab) temp");
...
/*** Marker ***/
  ...
  dpd_->buf4_init(&W, PSIF_CC_HBAR, 0, 0, 11, 0, 11, 0, "WMnIe (Mn,eI)");
  dpd_->contract244(&X1, &W, &Z1, 1, 2, 1, 1, 0);
  dpd_->buf4_close(&W);
  sprintf(lbl, "Z_%s_MnIj", pert_x);
  dpd_->buf4_sort(&Z1, PSIF_CC_TMP0, pqsr, 0, 0, lbl);
  dpd_->buf4_sort_axpy(&Z1, PSIF_CC_TMP0, qprs, 0, 0, lbl, 1);
  dpd_->buf4_close(&Z1);
  dpd_->file2_close(&X1);
  ...
  dpd_->buf4_init(&Z1, PSIF_CC_TMP0, irrep_y, 11, 0, 11, 0, 0, lbl);
  sprintf(lbl, "X_%s_IjAb (%5.3f)", pert_y, omega_y);
  dpd_->buf4_init(&Y2, PSIF_CC_LR, irrep_y, 0, 5, 0, 5, 0, lbl);
  dpd_->buf4_init(&W, PSIF_CC_HBAR, 0, 11, 5, 11, 5, 0, "WAmEf");
  dpd_->contract444(&W, &Y2, &Z1, 0, 0, 1, 0);
  dpd_->buf4_close(&W);
  dpd_->buf4_close(&Y2);
  ...
  sprintf(lbl, "X_%s_IA (%5.3f)", pert_x, omega_x);
  dpd_->file2_init(&X1, PSIF_CC_OEI, irrep_x, 0, 1, lbl);
  dpd_->contract424(&Z1, &X1, &Z, 1, 0, 0, -2, 1);
  dpd_->file2_close(&X1);
  dpd_->buf4_close(&Z1);
  dpd_->buf4_close(&Z);
