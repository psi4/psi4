...
//  fprintf(outfile, "L(1)HX1Y2 = %20.12f\n", polar);
  ...
  dpd_file2_init(&z1, PSIF_CC_TMP0, irrep_x, 0, 0, lbl);
  sprintf(lbl, "X_%s_IA (%5.3f)", pert_x, omega_x);
  dpd_file2_init(&X1, PSIF_CC_OEI, irrep_x, 0, 1, lbl);
  ...
  dpd_file2_init(&z1, PSIF_CC_TMP0, irrep_x, 1, 1, lbl);
  sprintf(lbl, "X_%s_IA (%5.3f)", pert_x, omega_x);
  dpd_file2_init(&X1, PSIF_CC_OEI, irrep_x, 0, 1, lbl);
  dpd_file2_init(&F, PSIF_CC_OEI, 0, 0, 1, "FME");
  ...
  dpd_buf4_init(&Z1, PSIF_CC_TMP0, irrep_x, 11, 10, 11, 10, 0, lbl);
  ...
  dpd_buf4_sort(&Z1, PSIF_CC_TMP0, qsrp, 10, 10, lbl);
  dpd_buf4_close(&Z1);

  sprintf(lbl, "Z_%s_WMbEj (bM,Ej)", pert_x);
  dpd_buf4_init(&Z1, PSIF_CC_TMP0, irrep_x, 11, 11, 11, 11, 0, lbl);
  dpd_buf4_init(&W, PSIF_CC_HBAR, 0, 0, 11, 0, 11, 0, "WMnIe (Mn,eI)");
  dpd_contract244(&X1, &W, &Z1, 0, 0, 0, -1, 0);
  dpd_buf4_close(&W);
  sprintf(lbl, "Z_%s_MbEj (ME,jb)", pert_x);
  dpd_buf4_sort_axpy(&Z1, PSIF_CC_TMP0, qrsp, 10, 10, lbl, 1);
  dpd_buf4_close(&Z1);
  ...
  dpd_buf4_close(&W);
  sprintf(lbl, "Z_%s_MbeJ (Me,Jb)", pert_x);
  dpd_buf4_sort(&Z1, PSIF_CC_TMP0, prsq, 10, 10, lbl);
  dpd_buf4_close(&Z1);
  ...
  dpd_buf4_scmcopy(&Z1, PSIF_CC_TMP0, lbl, 2);
  dpd_buf4_close(&Z1);
  sprintf(lbl, "Z_%s_MbeJ (Me,Jb)", pert_x);
  dpd_buf4_init(&Z1, PSIF_CC_TMP0, irrep_x, 10, 10, 10, 10, 0, lbl);
  sprintf(lbl, "Z_%s_(2MbEj+MbeJ) (ME,JB)", pert_x);
  dpd_buf4_init(&Z2, PSIF_CC_TMP0, irrep_x, 10, 10, 10, 10, 0, lbl);
  ...
  dpd_buf4_close(&W);
  dpd_buf4_init(&Z1, PSIF_CC_TMP0, 0, 0, 5, 0, 5, 0, "Z(Ij,Ab)");
...
/*   dpd_dot24(&X1, &W, &z, 0, 0, 1, 0); */
  ...
  dpd_buf4_init(&Z1, PSIF_CC_TMP0, 0, 0, 5, 0, 5, 0, "Z(Ij,Ab) temp");
...
/*** Marker ***/
  ...
  dpd_buf4_sort(&Z1, PSIF_CC_TMP0, pqsr, 0, 0, lbl);
  dpd_buf4_sort_axpy(&Z1, PSIF_CC_TMP0, qprs, 0, 0, lbl, 1);
  dpd_buf4_close(&Z1);
  dpd_file2_close(&X1);
  ...
  dpd_buf4_init(&Z1, PSIF_CC_TMP0, irrep_y, 11, 0, 11, 0, 0, lbl);
  sprintf(lbl, "X_%s_IjAb (%5.3f)", pert_y, omega_y);
  dpd_buf4_init(&Y2, PSIF_CC_LR, irrep_y, 0, 5, 0, 5, 0, lbl);
  dpd_buf4_init(&W, PSIF_CC_HBAR, 0, 11, 5, 11, 5, 0, "WAmEf");
