...
void X2_build(const char *pert, int irrep, double omega)
{
  ...
  dpd_->buf4_copy(&X2new, PSIF_CC_LR, lbl);
  ...
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, irrep, 0, 5, 0, 5, 0, lbl);
  dpd_->buf4_init(&W, PSIF_CC_HBAR, 0, 10, 0, 10, 0, 0, "WMbIj");
  dpd_->contract244(&X1, &W, &Z, 0, 0, 1, 1, 0);
  dpd_->buf4_close(&W);
  dpd_->buf4_axpy(&Z, &X2new, -1);
  dpd_->buf4_close(&X2new);  /* Need to close X2new to avoid collisions */
  sprintf(lbl, "New X_%s_IjAb (%5.3f)", pert, omega);
  dpd_->buf4_sort_axpy(&Z, PSIF_CC_LR, qpsr, 0, 5, lbl, -1);
  dpd_->buf4_init(&X2new, PSIF_CC_LR, irrep, 0, 5, 0, 5, 0, lbl); /* re-open X2new here */
  dpd_->buf4_close(&Z);
  ...
  for(Gej=0; Gej < moinfo.nirreps; Gej++) {
    ...
    Gij = Gab ^ irrep;
    dpd_->buf4_mat_irrep_init(&Z, Gij);
    dpd_->buf4_mat_irrep_shift13(&Z, Gij);
    for(Ge=0; Ge < moinfo.nirreps; Ge++) {
      Gj = Ge ^ Gej;
      Gi = Gj ^ Gij;
      nrows = moinfo.occpi[Gj];
      length = nrows * W.params->coltot[Gab];
      dpd_->buf4_mat_irrep_init_block(&W, Gej, nrows);
      for(E=0; E < moinfo.virtpi[Ge]; E++) {
	e = moinfo.vir_off[Ge] + E;
	dpd_->buf4_mat_irrep_rd_block(&W, Gej, W.row_offset[Gej][e], nrows);
	for(II=0; II < moinfo.occpi[Gi]; II++) {
	  if(length)
	    C_DAXPY(length, X1.matrix[Gi][II][E], W.matrix[Gej][0], 1, Z.shift.matrix[Gij][Gi][II], 1);
	}
      }
      dpd_->buf4_mat_irrep_close_block(&W, Gej, nrows);
    }
    dpd_->buf4_mat_irrep_wrt(&Z, Gij);
    dpd_->buf4_mat_irrep_close(&Z, Gij);
  }
  ...
  dpd_->file2_init(&z, PSIF_CC_TMP0, irrep, 0, 0, lbl);
  dpd_->buf4_init(&W, PSIF_CC_HBAR, 0, 0, 11, 0, 11, 0, "2WMnIe - WnMIe (Mn,eI)");
  dpd_->dot23(&X1, &W, &z, 0, 0, 1, 0);
  dpd_->buf4_close(&W);
  sprintf(lbl, "Z(Ij,Ab) %s", pert);
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, irrep, 0, 5, 0, 5, 0, lbl);
  dpd_->buf4_init(&T2, PSIF_CC_TAMPS, 0, 0, 5, 0, 5, 0, "tIjAb");
  dpd_->contract244(&z, &T2, &Z, 0, 0, 0, 1, 0);
  dpd_->buf4_close(&T2);
  dpd_->file2_close(&z);
  dpd_->buf4_axpy(&Z, &X2new, -1);
  dpd_->buf4_close(&X2new);  /* Need to close X2new to avoid collisions */
  ...
  dpd_->file2_init(&z, PSIF_CC_TMP0, irrep, 1, 1, lbl);
  ...
  for(Gbm=0; Gbm < moinfo.nirreps; Gbm++) {
    ...
    free(X);
    dpd_->buf4_mat_irrep_row_close(&W, Gbm);
  }
  dpd_->file2_mat_close(&X1);
  dpd_->file2_mat_wrt(&z);
  dpd_->file2_mat_close(&z);
  /* end ooc additions, 7/28/05, -TDC */
  dpd_->buf4_close(&W);
  sprintf(lbl, "Z(Ij,Ab) %s", pert);
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, irrep, 0, 5, 0, 5, 0, lbl);
  dpd_->buf4_init(&T2, PSIF_CC_TAMPS, 0, 0, 5, 0, 5, 0, "tIjAb");
  dpd_->contract424(&T2, &z, &Z, 3, 1, 0, 1, 0);
  dpd_->buf4_close(&T2);
  dpd_->file2_close(&z);
  dpd_->buf4_axpy(&Z, &X2new, 1);
  dpd_->buf4_close(&X2new);  /* Need to close X2new to avoid collisions */
  ...
  dpd_->buf4_axpy(&X2, &X2new, -omega);

  sprintf(lbl, "Z(Ij,Ab) %s", pert);
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, irrep, 0, 5, 0, 5, 0, lbl);
  dpd_->file2_init(&F, PSIF_CC_OEI, 0, 1, 1, "FAE");
  dpd_->contract424(&X2, &F, &Z, 3, 1, 0, 1, 0);
  dpd_->file2_close(&F);
  dpd_->buf4_axpy(&Z, &X2new, 1);
  dpd_->buf4_close(&X2new);  /* Need to close X2new to avoid collisions */
  sprintf(lbl, "New X_%s_IjAb (%5.3f)", pert, omega);
  ...
  dpd_->contract244(&F, &X2, &Z, 0, 0, 0, 1, 0);
  dpd_->file2_close(&F);
  dpd_->buf4_axpy(&Z, &X2new, -1);
  dpd_->buf4_close(&X2new);  /* Need to close X2new to avoid collisions */
  sprintf(lbl, "New X_%s_IjAb (%5.3f)", pert, omega);
  ...
  dpd_->contract444(&W, &X2, &X2new, 1, 1, 1, 1);
  dpd_->buf4_close(&W);

  if(params.abcd == "OLD") {
    sprintf(lbl, "Z(Ab,Ij) %s", pert);
    dpd_->buf4_init(&Z, PSIF_CC_TMP0, irrep, 5, 0, 5, 0, 0, lbl);
    dpd_->buf4_init(&I, PSIF_CC_BINTS, 0, 5, 5, 5, 5, 0, "B <ab|cd>");
    dpd_->contract444(&I, &X2, &Z, 0, 0, 1, 0);
    dpd_->buf4_close(&I);
    dpd_->buf4_close(&X2new);  /* Need to close X2new to avoid collisions */
    sprintf(lbl, "New X_%s_IjAb (%5.3f)", pert, omega);
    dpd_->buf4_sort_axpy(&Z, PSIF_CC_LR, rspq, 0, 5, lbl, 1);
    dpd_->buf4_init(&X2new, PSIF_CC_LR, irrep, 0, 5, 0, 5, 0, lbl); /* re-open X2new here */
    dpd_->buf4_close(&Z);
  }
    ...
    dpd_->buf4_init(&X2, PSIF_CC_LR, irrep, 3, 8, 3, 8, 0, lbl);
    dpd_->buf4_init(&I, PSIF_CC_BINTS, 0, 8, 8, 8, 8, 0, "B(+) <ab|cd> + <ab|dc>");
    sprintf(lbl, "S_%s_(ab,ij)", pert);
    dpd_->buf4_init(&S, PSIF_CC_TMP0, irrep, 8, 3, 8, 3, 0, lbl);
    dpd_->contract444(&I, &X2, &S, 0, 0, 0.5, 0);
    dpd_->buf4_close(&S);
    dpd_->buf4_close(&I);
    dpd_->buf4_close(&X2);
    timer_off("ABCD:S");
    ...
    sprintf(lbl, "X_%s_(+)(ij,ab) (%5.3f)", pert, omega);
    dpd_->buf4_init(&X2, PSIF_CC_LR, irrep, 3, 8, 3, 8, 0, lbl);
    dpd_->buf4_mat_irrep_init(&X2, irrep);
    dpd_->buf4_mat_irrep_rd(&X2, irrep);
    X_diag = dpd_->dpd_block_matrix(X2.params->rowtot[irrep], moinfo.nvirt);
    ...
    dpd_->buf4_mat_irrep_close(&X2, irrep);
    ...
    dpd_->free_dpd_block(X_diag, X2.params->rowtot[irrep], moinfo.nvirt);
    dpd_->buf4_close(&X2);

    timer_on("ABCD:A");
    sprintf(lbl, "X_%s_(-)(ij,ab) (%5.3f)", pert, omega);
    dpd_->buf4_init(&X2, PSIF_CC_LR, irrep, 4, 9, 4, 9, 0, lbl);
    dpd_->buf4_init(&I, PSIF_CC_BINTS, 0, 9, 9, 9, 9, 0, "B(-) <ab|cd> - <ab|dc>");
    sprintf(lbl, "A_%s_(ab,ij)", pert);
    dpd_->buf4_init(&A, PSIF_CC_TMP0, irrep, 9, 4, 9, 4, 0, lbl);
    dpd_->contract444(&I, &X2, &A, 0, 0, 0.5, 0);
    dpd_->buf4_close(&A);
    dpd_->buf4_close(&I);
    dpd_->buf4_close(&X2);
    timer_off("ABCD:A");
    ...
    dpd_->buf4_close(&X2new);  /* Need to close X2new to avoid collisions */
    sprintf(lbl, "S_%s_(ab,ij)", pert);
    dpd_->buf4_init(&S, PSIF_CC_TMP0, irrep, 5, 0, 8, 3, 0, lbl);
    sprintf(lbl, "New X_%s_IjAb (%5.3f)", pert, omega);
    dpd_->buf4_sort_axpy(&S, PSIF_CC_LR, rspq, 0, 5, lbl, 1);
    dpd_->buf4_close(&S);
    sprintf(lbl, "A_%s_(ab,ij)", pert);
    dpd_->buf4_init(&A, PSIF_CC_TMP0, irrep, 5, 0, 9, 4, 0, lbl);
    sprintf(lbl, "New X_%s_IjAb (%5.3f)", pert, omega);
    dpd_->buf4_sort_axpy(&A, PSIF_CC_LR, rspq, 0, 5, lbl, 1);
    dpd_->buf4_close(&A);
    dpd_->buf4_init(&X2new, PSIF_CC_LR, irrep, 0, 5, 0, 5, 0, lbl); /* re-open X2new here */
  ...
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, irrep, 10, 0, 10, 0, 0, lbl);
  dpd_->buf4_init(&I, PSIF_CC_FINTS, 0, 10, 5, 10, 5, 0, "F <ia|bc>");
  dpd_->contract444(&I, &X2, &Z, 0, 0, 1, 0);
  dpd_->buf4_close(&I);
  sprintf(lbl, "Z(Ij,Ab) %s", pert);
  dpd_->buf4_init(&Z1, PSIF_CC_TMP0, irrep, 0, 5, 0, 5, 0, lbl);
  dpd_->file2_init(&t1, PSIF_CC_OEI, 0, 0, 1, "tIA");
  dpd_->contract244(&t1, &Z, &Z1, 0, 0, 1, 1, 0);
  dpd_->file2_close(&t1);
  dpd_->buf4_close(&Z);
  dpd_->buf4_axpy(&Z1, &X2new, -1);
  dpd_->buf4_close(&X2new);  /* Need to close X2new to avoid collisions */
  sprintf(lbl, "New X_%s_IjAb (%5.3f)", pert, omega);
  dpd_->buf4_sort_axpy(&Z1, PSIF_CC_LR, qpsr, 0, 5, lbl, -1);
  dpd_->buf4_init(&X2new, PSIF_CC_LR, irrep, 0, 5, 0, 5, 0, lbl); /* re-open X2new here */
  dpd_->buf4_close(&Z1);

  sprintf(lbl, "Z(Ij,Mn) %s", pert);
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, irrep, 0, 0, 0, 0, 0, lbl);
  dpd_->buf4_init(&I, PSIF_CC_DINTS, 0, 0, 5, 0, 5, 0, "D <ij|ab>");
  dpd_->contract444(&X2, &I, &Z, 0, 0, 1, 0);
  dpd_->buf4_close(&I);
  dpd_->buf4_init(&T2, PSIF_CC_TAMPS, 0, 0, 5, 0, 5, 0, "tauIjAb");
  dpd_->contract444(&Z, &T2, &X2new, 0, 1, 1, 1);
  dpd_->buf4_close(&T2);
  dpd_->buf4_close(&Z);
  ...
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, irrep, 10, 10, 10, 10, 0, lbl);
  dpd_->buf4_init(&W, PSIF_CC_HBAR, 0, 10, 10, 10, 10, 0, "WMbeJ");
  sprintf(lbl, "X_%s_IbjA (%5.3f)", pert, omega);
  dpd_->buf4_init(&X2, PSIF_CC_LR, irrep, 10, 10, 10, 10, 0, lbl);
  dpd_->contract444(&X2, &W, &Z, 0, 1, 1, 0);
  dpd_->buf4_close(&X2);
  dpd_->buf4_close(&W);
  sprintf(lbl, "X(IA,jb) III %s", pert);
  dpd_->buf4_sort(&Z, PSIF_CC_TMP0, psrq, 10, 10, lbl);
  dpd_->buf4_close(&Z);
  sprintf(lbl, "X(IA,jb) I %s", pert);
  dpd_->buf4_init(&Z1, PSIF_CC_TMP0, irrep, 10, 10, 10, 10, 0, lbl);
  sprintf(lbl, "X_%s_(2IAjb-IbjA) (%5.3f)", pert, omega);
  dpd_->buf4_init(&X2, PSIF_CC_LR, irrep, 10, 10, 10, 10, 0, lbl);
  dpd_->buf4_init(&W, PSIF_CC_HBAR, 0, 10, 10, 10, 10, 0, "2 W(ME,jb) + W(Me,Jb)");
  dpd_->contract444(&X2, &W, &Z1, 0, 1, 0.5, 0);
  dpd_->buf4_close(&W);
  dpd_->buf4_close(&X2);
  sprintf(lbl, "Z(Ib,jA) %s", pert);
  dpd_->buf4_init(&Z2, PSIF_CC_TMP0, irrep, 10, 10, 10, 10, 0, lbl);
  ...
  dpd_->buf4_sort(&Z1, PSIF_CC_TMP0, prqs, 0, 5, lbl);
  dpd_->buf4_close(&Z1);
  dpd_->buf4_init(&Z1, PSIF_CC_TMP0, irrep, 0, 5, 0, 5, 0, lbl);
  dpd_->buf4_axpy(&Z1, &X2new, 1);
  dpd_->buf4_close(&X2new);  /* Need to close X2new to avoid collisions */
  sprintf(lbl, "New X_%s_IjAb (%5.3f)", pert, omega);
  dpd_->buf4_sort_axpy(&Z1, PSIF_CC_LR, qpsr, 0, 5, lbl, 1); /* II+IV */
  dpd_->buf4_init(&X2new, PSIF_CC_LR, irrep, 0, 5, 0, 5, 0, lbl); /* re-open X2new here */
  dpd_->buf4_close(&Z1);
  ...
  dpd_->buf4_init(&X2, PSIF_CC_LR, irrep, 0, 5, 0, 5, 0, lbl);
  dpd_->buf4_init(&I, PSIF_CC_DINTS, 0, 0, 5, 0, 5, 0, "D <ij|ab>");
  dpd_->contract442(&I, &X2, &z, 2, 2, 1, 0);
  dpd_->buf4_close(&I);
  dpd_->buf4_close(&X2);
  sprintf(lbl, "Z(Ij,Ab) %s", pert);
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, irrep, 0, 5, 0, 5, 0, lbl);
  dpd_->buf4_init(&T2, PSIF_CC_TAMPS, 0, 0, 5, 0, 5, 0, "tIjAb");
  dpd_->contract424(&T2, &z, &Z, 3, 0, 0, 1, 0);
  dpd_->buf4_close(&T2);
  dpd_->file2_close(&z);
  dpd_->buf4_axpy(&Z, &X2new, -1);
  dpd_->buf4_close(&X2new);  /* Need to close X2new to avoid collisions */
