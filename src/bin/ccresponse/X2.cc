...
void X2_build(const char *pert, int irrep, double omega)
{
  ...
  dpd_buf4_copy(&X2new, PSIF_CC_LR, lbl);
  ...
  dpd_buf4_init(&Z, PSIF_CC_TMP0, irrep, 0, 5, 0, 5, 0, lbl);
  ...
  sprintf(lbl, "New X_%s_IjAb (%5.3f)", pert, omega);
  dpd_buf4_sort_axpy(&Z, PSIF_CC_LR, qpsr, 0, 5, lbl, -1);
  dpd_buf4_init(&X2new, PSIF_CC_LR, irrep, 0, 5, 0, 5, 0, lbl); /* re-open X2new here */
  dpd_buf4_close(&Z);
  ...
  dpd_buf4_init(&W, PSIF_CC_HBAR, 0, 0, 11, 0, 11, 0, "2WMnIe - WnMIe (Mn,eI)");
  ...
  if(params.abcd == "OLD") {
    ...
    dpd_buf4_init(&Z, PSIF_CC_TMP0, irrep, 5, 0, 5, 0, 0, lbl);
    dpd_buf4_init(&I, PSIF_CC_BINTS, 0, 5, 5, 5, 5, 0, "B <ab|cd>");
    dpd_contract444(&I, &X2, &Z, 0, 0, 1, 0);
    dpd_buf4_close(&I);
    dpd_buf4_close(&X2new);  /* Need to close X2new to avoid collisions */
    sprintf(lbl, "New X_%s_IjAb (%5.3f)", pert, omega);
    dpd_buf4_sort_axpy(&Z, PSIF_CC_LR, rspq, 0, 5, lbl, 1);
    dpd_buf4_init(&X2new, PSIF_CC_LR, irrep, 0, 5, 0, 5, 0, lbl); /* re-open X2new here */
    dpd_buf4_close(&Z);
  }
    ...
    dpd_buf4_init(&X2, PSIF_CC_LR, irrep, 3, 8, 3, 8, 0, lbl);
    dpd_buf4_init(&I, PSIF_CC_BINTS, 0, 8, 8, 8, 8, 0, "B(+) <ab|cd> + <ab|dc>");
    sprintf(lbl, "S_%s_(ab,ij)", pert);
    dpd_buf4_init(&S, PSIF_CC_TMP0, irrep, 8, 3, 8, 3, 0, lbl);
    ...
    timer_on("ABCD:A");
    sprintf(lbl, "X_%s_(-)(ij,ab) (%5.3f)", pert, omega);
    dpd_buf4_init(&X2, PSIF_CC_LR, irrep, 4, 9, 4, 9, 0, lbl);
    dpd_buf4_init(&I, PSIF_CC_BINTS, 0, 9, 9, 9, 9, 0, "B(-) <ab|cd> - <ab|dc>");
    sprintf(lbl, "A_%s_(ab,ij)", pert);
    dpd_buf4_init(&A, PSIF_CC_TMP0, irrep, 9, 4, 9, 4, 0, lbl);
    ...
    dpd_buf4_close(&X2new);  /* Need to close X2new to avoid collisions */
    sprintf(lbl, "S_%s_(ab,ij)", pert);
    dpd_buf4_init(&S, PSIF_CC_TMP0, irrep, 5, 0, 8, 3, 0, lbl);
    sprintf(lbl, "New X_%s_IjAb (%5.3f)", pert, omega);
    dpd_buf4_sort_axpy(&S, PSIF_CC_LR, rspq, 0, 5, lbl, 1);
    dpd_buf4_close(&S);
    sprintf(lbl, "A_%s_(ab,ij)", pert);
    dpd_buf4_init(&A, PSIF_CC_TMP0, irrep, 5, 0, 9, 4, 0, lbl);
    sprintf(lbl, "New X_%s_IjAb (%5.3f)", pert, omega);
    dpd_buf4_sort_axpy(&A, PSIF_CC_LR, rspq, 0, 5, lbl, 1);
    dpd_buf4_close(&A);
    dpd_buf4_init(&X2new, PSIF_CC_LR, irrep, 0, 5, 0, 5, 0, lbl); /* re-open X2new here */
  ...
  dpd_buf4_init(&Z, PSIF_CC_TMP0, irrep, 10, 0, 10, 0, 0, lbl);
  dpd_buf4_init(&I, PSIF_CC_FINTS, 0, 10, 5, 10, 5, 0, "F <ia|bc>");
  dpd_contract444(&I, &X2, &Z, 0, 0, 1, 0);
  dpd_buf4_close(&I);
  sprintf(lbl, "Z(Ij,Ab) %s", pert);
  dpd_buf4_init(&Z1, PSIF_CC_TMP0, irrep, 0, 5, 0, 5, 0, lbl);
  ...
  dpd_buf4_close(&X2new);  /* Need to close X2new to avoid collisions */
  sprintf(lbl, "New X_%s_IjAb (%5.3f)", pert, omega);
  dpd_buf4_sort_axpy(&Z1, PSIF_CC_LR, qpsr, 0, 5, lbl, -1);
  dpd_buf4_init(&X2new, PSIF_CC_LR, irrep, 0, 5, 0, 5, 0, lbl); /* re-open X2new here */
  dpd_buf4_close(&Z1);

  sprintf(lbl, "Z(Ij,Mn) %s", pert);
  dpd_buf4_init(&Z, PSIF_CC_TMP0, irrep, 0, 0, 0, 0, 0, lbl);
  ...
  dpd_buf4_init(&Z, PSIF_CC_TMP0, irrep, 10, 10, 10, 10, 0, lbl);
  dpd_buf4_init(&W, PSIF_CC_HBAR, 0, 10, 10, 10, 10, 0, "WMbeJ");
  sprintf(lbl, "X_%s_IbjA (%5.3f)", pert, omega);
  dpd_buf4_init(&X2, PSIF_CC_LR, irrep, 10, 10, 10, 10, 0, lbl);
  dpd_contract444(&X2, &W, &Z, 0, 1, 1, 0);
  dpd_buf4_close(&X2);
  dpd_buf4_close(&W);
  sprintf(lbl, "X(IA,jb) III %s", pert);
  dpd_buf4_sort(&Z, PSIF_CC_TMP0, psrq, 10, 10, lbl);
  dpd_buf4_close(&Z);
  sprintf(lbl, "X(IA,jb) I %s", pert);
  dpd_buf4_init(&Z1, PSIF_CC_TMP0, irrep, 10, 10, 10, 10, 0, lbl);
  ...
  dpd_buf4_close(&X2);
  sprintf(lbl, "Z(Ib,jA) %s", pert);
  dpd_buf4_init(&Z2, PSIF_CC_TMP0, irrep, 10, 10, 10, 10, 0, lbl);
  ...
  dpd_buf4_sort(&Z1, PSIF_CC_TMP0, prqs, 0, 5, lbl);
  dpd_buf4_close(&Z1);
  dpd_buf4_init(&Z1, PSIF_CC_TMP0, irrep, 0, 5, 0, 5, 0, lbl);
  dpd_buf4_axpy(&Z1, &X2new, 1);
  dpd_buf4_close(&X2new);  /* Need to close X2new to avoid collisions */
  sprintf(lbl, "New X_%s_IjAb (%5.3f)", pert, omega);
  dpd_buf4_sort_axpy(&Z1, PSIF_CC_LR, qpsr, 0, 5, lbl, 1); /* II+IV */
  dpd_buf4_init(&X2new, PSIF_CC_LR, irrep, 0, 5, 0, 5, 0, lbl); /* re-open X2new here */
  dpd_buf4_close(&Z1);
