	          ...
	          const char *pert_y, int irrep_y, double omega_y)
{
  ...
  dpd_file2_init(&z1, PSIF_CC_TMP0, irrep_y, 0, 0, lbl);
  ...
  dpd_file2_init(&z, PSIF_CC_TMP0, 0, 0, 1, "Z(I,A) Final");
  ...
  dpd_file2_init(&z1, PSIF_CC_TMP0, irrep_y, 1, 1, lbl);
  dpd_buf4_init(&I, PSIF_CC_DINTS, 0, 0, 5, 0, 5, 0, "D 2<ij|ab> - <ij|ba>");
  sprintf(lbl, "X_%s_IjAb (%5.3f)", pert_y, omega_y);
  dpd_buf4_init(&Y2, PSIF_CC_LR, irrep_y, 0, 5, 0, 5, 0, lbl);
  ...
  dpd_file2_init(&z1, PSIF_CC_TMP0, irrep_x, 0, 1, lbl);
  sprintf(lbl, "X_%s_IA (%5.3f)", pert_x, omega_x);
  dpd_file2_init(&X1, PSIF_CC_OEI, irrep_x, 0, 1, lbl);
  dpd_buf4_init(&I, PSIF_CC_DINTS, 0, 0, 5, 0, 5, 0, "D 2<ij|ab> - <ij|ba>");
