...
void sort_X(const char *pert, int irrep, double omega)
{
  ...
  dpd_buf4_sort(&X, PSIF_CC_LR, prqs, 10, 10, lbl);
  sprintf(lbl, "X_%s_IbjA (%5.3f)", pert, omega);
  dpd_buf4_sort(&X, PSIF_CC_LR, psqr, 10, 10, lbl);
  ...
  sprintf(lbl, "X_%s_IAjb (%5.3f)", pert, omega);
  dpd_buf4_init(&X, PSIF_CC_LR, irrep, 10, 10, 10, 10, 0, lbl);
  sprintf(lbl, "X_%s_(2IAjb-IbjA) (%5.3f)", pert, omega);
  dpd_buf4_scmcopy(&X, PSIF_CC_LR, lbl, 2);
  dpd_buf4_sort_axpy(&X, PSIF_CC_LR, psrq, 10, 10, lbl, -1);
  dpd_buf4_close(&X);
  ...
  dpd_buf4_scmcopy(&X, PSIF_CC_LR, lbl, 2);
  dpd_buf4_sort_axpy(&X, PSIF_CC_LR, rqps, 10, 10, lbl, -1);
  dpd_buf4_close(&X);

  if(params.ref == 0 && params.abcd == "NEW") {
    /* X(-)(ij,ab) (i>j, a>b) = X(ij,ab) - X(ij,ba) */
    sprintf(lbl, "X_%s_IjAb (%5.3f)", pert, omega);
    dpd_buf4_init(&X, PSIF_CC_LR, irrep, 4, 9, 0, 5, 1, lbl);
    sprintf(lbl, "X_%s_(-)(ij,ab) (%5.3f)", pert, omega);
    dpd_buf4_copy(&X, PSIF_CC_LR, lbl);
    dpd_buf4_close(&X);
    ...
    dpd_buf4_init(&X, PSIF_CC_LR, irrep, 0, 5, 0, 5, 0, lbl);
    sprintf(lbl, "X_%s_(+)(ij,ab) (%5.3f)", pert, omega);
    dpd_buf4_copy(&X, PSIF_CC_TMP0, lbl);
    dpd_buf4_sort_axpy(&X, PSIF_CC_TMP0, pqsr, 0, 5, lbl, 1);
    dpd_buf4_close(&X);
    dpd_buf4_init(&X, PSIF_CC_TMP0, irrep, 3, 8, 0, 5, 0, lbl);
    dpd_buf4_copy(&X, PSIF_CC_LR, lbl);
    dpd_buf4_close(&X);
  }
}
