	      ...
	      const char *pert_y, int irrep_y, double omega_y)
{
  ...
  /* Type-I L2 residual */
  dpd_->buf4_init(&L2, PSIF_CC_LAMPS, 0, 0, 5, 0, 5, 0, "LHX1Y1 I (2 Lijab - Lijba)");
  dpd_->buf4_init(&Z1, PSIF_CC_TMP0, 0, 0, 5, 0, 5, 0, "X*Y(ij,ab)");
  polar_I = 2.0 * dpd_->buf4_dot(&L2, &Z1);
  dpd_->buf4_close(&Z1);
  dpd_->buf4_close(&L2);

  /* Type-II L2 residual */
  dpd_->buf4_init(&L2, PSIF_CC_LAMPS, 0, 10, 10, 10, 10, 0, "LHX1Y1 Residual II");
  dpd_->buf4_init(&Z1, PSIF_CC_TMP0, 0, 10, 10, 10, 10, 0, "(X*Y+Y*X)(ie,ma)");
  polar_II = -2.0 * dpd_->buf4_dot(&L2, &Z1);
  dpd_->buf4_close(&Z1);
  dpd_->buf4_close(&L2);
              ...
              const char *pert_y, int irrep_y, double omega_y)
{
  ...
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 0, 5, 0, 5, 0, "X*Y(ij,ab)");
  dpd_->buf4_scm(&Z, 0.0);
  for(h=0; h< nirreps; h++) {
  ...
  dpd_->buf4_init(&Z, PSIF_CC_TMP0, 0, 10, 10, 10, 10, 0, "(X*Y+Y*X)(ie,ma)");
  dpd_->buf4_scm(&Z, 0.0);
  for(h=0; h< nirreps; h++) {
    dpd_->buf4_mat_irrep_init(&Z, h);
    for(row=0; row < Z.params->rowtot[h]; row++) {
