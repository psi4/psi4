...
if (reference_ == "RESTRICTED") {
    ...
    dpd_buf4_copy(&K, PSIF_OCEPA_DPD, "W (OV|OV)");
    dpd_buf4_close(&K);
...
//===========================================================================================
    ...
    dpd_buf4_copy(&K, PSIF_OCEPA_DPD, "W_1 <OO|OO>");
    dpd_buf4_close(&K);    
    ...
    dpd_buf4_copy(&K, PSIF_OCEPA_DPD, "W_1 <VV|VV>");
    dpd_buf4_close(&K);
    ...
    //Print 
    if (print_ > 3) {
      dpd_buf4_init(&W, PSIF_OCEPA_DPD, 0, ID("[V,V]"), ID("[V,V]"),
    ...
    */
    ...
    dpd_buf4_sort(&K, PSIF_OCEPA_DPD , psqr, ID("[O,v]"), ID("[O,v]"), "W (Ov|Ov)");
    dpd_buf4_close(&K);
    dpd_buf4_init(&W, PSIF_OCEPA_DPD, 0, ID("[O,v]"), ID("[O,v]"),
                  ID("[O,v]"), ID("[O,v]"), 0, "W (Ov|Ov)");
    dpd_buf4_scm(&W, -1.0);
    ...
    dpd_buf4_sort(&K, PSIF_OCEPA_DPD , rqsp, ID("[o,V]"), ID("[o,V]"), "W (oV|oV)");
    dpd_buf4_close(&K);
    dpd_buf4_init(&W, PSIF_OCEPA_DPD, 0, ID("[o,V]"), ID("[o,V]"),
                  ID("[o,V]"), ID("[o,V]"), 0, "W (oV|oV)");
    dpd_buf4_scm(&W, -1.0);
    ...
    dpd_buf4_init(&W, PSIF_OCEPA_DPD, 0, ID("[O,V]"), ID("[o,v]"),
                  ID("[O,V]"), ID("[o,v]"), 0, "W (OV|ov)");
    dpd_buf4_sort(&W, PSIF_OCEPA_DPD , rspq, ID("[o,v]"), ID("[O,V]"), "W (ov|OV)");
    dpd_buf4_close(&W);
