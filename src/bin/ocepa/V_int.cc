...
{   
 ...
 if (reference_ == "RESTRICTED") {
    ...
    dpd_buf4_init(&Tau, PSIF_OCEPA_DPD, 0, ID("[O,V]"), ID("[O,V]"),
    ...
    // V(IB,JA) => V(IA,JB)
    dpd_buf4_sort(&V, PSIF_OCEPA_DENSITY , psrq, ID("[O,V]"), ID("[O,V]"), "V <OV|OV>");
    dpd_buf4_close(&V);
    ...
    // V(IB,JA) => V(IA,BJ)
    dpd_buf4_sort(&V, PSIF_OCEPA_DENSITY , psqr, ID("[O,V]"), ID("[V,O]"), "V <OV|VO>");
    dpd_buf4_close(&V);
    ...
    dpd_buf4_init(&T, PSIF_OCEPA_DPD, 0, ID("[O,V]"), ID("[O,V]"),
                  ID("[O,V]"), ID("[O,V]"), 0, "T2 (OV|OV)");
    dpd_buf4_init(&L, PSIF_OCEPA_DPD, 0, ID("[O,V]"), ID("[O,V]"),
                  ID("[O,V]"), ID("[O,V]"), 0, "T2 (OV|OV)");
    dpd_buf4_init(&V, PSIF_OCEPA_DENSITY, 0, ID("[O,V]"), ID("[O,V]"),
                  ID("[O,V]"), ID("[O,V]"), 0, "V <IB|JA>");
    dpd_contract444(&T, &L, &V, 0, 0, 0.5, 0.0); 
    ...
    dpd_buf4_init(&L, PSIF_OCEPA_DPD, 0, ID("[o,V]"), ID("[O,v]"),
                  ID("[o,V]"), ID("[O,v]"), 0, "T2 (oV|Ov)");
    dpd_buf4_init(&V1, PSIF_OCEPA_DENSITY, 0, ID("[O,v]"), ID("[O,v]"),
                  ID("[O,v]"), ID("[O,v]"), 0, "V <Ib|Ja>");
    dpd_buf4_init(&V2, PSIF_OCEPA_DENSITY, 0, ID("[o,V]"), ID("[o,V]"),
                  ID("[o,V]"), ID("[o,V]"), 0, "V <iB|jA>");
    ...
    dpd_buf4_sort(&V1, PSIF_OCEPA_DENSITY , psrq, ID("[O,v]"), ID("[O,v]"), "V <Ov|Ov>");
    dpd_buf4_close(&V1);
    dpd_buf4_sort(&V2, PSIF_OCEPA_DENSITY , psrq, ID("[o,V]"), ID("[o,V]"), "V <oV|oV>");
    dpd_buf4_close(&V2);
