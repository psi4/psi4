...
//===========================================================================================
   ...
   psio_->close(PSIF_MO_OPDM, 1);
    ...
    dpd_buf4_init(&G, PSIF_OCEPA_DENSITY, 0, ID("[V,v]"), ID("[V,v]"),
              ID("[V,v]"), ID("[V,v]"), 0, "TPDM <Vv|Vv>");
    dpd_buf4_sort(&G, PSIF_OCEPA_DENSITY , prqs, ID("[V,V]"), ID("[v,v]"), "TPDM (VV|vv)");
    dpd_buf4_close(&G);
    dpd_buf4_init(&G, PSIF_OCEPA_DENSITY, 0, ID("[V,V]"), ID("[v,v]"),
              ID("[V,V]"), ID("[v,v]"), 0, "TPDM (VV|vv)");
    dpd_buf4_scm(&G, 4.0);
    dpd_buf4_dump(&G, &AB, avir_qt, avir_qt, bvir_qt, bvir_qt, 0, 0);
    dpd_buf4_close(&G);
