...
if (reference_ == "RESTRICTED") {
	...
	GFock->add(HG1);
	Ecepa_rdm=HG1->trace() + Enuc; // One-electron contribution to MP2L
   ...
   if (twopdm_abcd_type == "DIRECT" ) {
 	...
 	// X_MNIC = 2\sum{E,F} t_MN^EF(1) * <IC|EF>   
        dpd_buf4_init(&X, PSIF_OCEPA_DENSITY, 0, ID("[O,O]"), ID("[O,V]"),
	...
	// Build Fai
	dpd_file2_init(&GF, PSIF_OCEPA_DENSITY, 0, ID('V'), ID('O'), "GF <V|O>"); 
	...
	// Build Fia
	dpd_file2_init(&GF, PSIF_OCEPA_DENSITY, 0, ID('O'), ID('V'), "GF <O|V>");  
...
/********************************************************************************************/ 
	...
	GFockA->add(HG1A);
	GFockB->add(HG1B);
	Ecepa_rdm = HG1A->trace() + HG1B->trace() + Enuc; // One-electron contribution to MP3L
...
/********************************************************************************************/ 
   if (twopdm_abcd_type == "DIRECT" ) {
	...
	dpd_buf4_close(&X);
        ...
        dpd_buf4_init(&X, PSIF_OCEPA_DENSITY, 0, ID("[O,o]"), ID("[V,o]"),
...
/********************************************************************************************/ 
	...
	dpd_buf4_init(&K, PSIF_LIBTRANS_DPD, 0, ID("[V,o]"), ID("[V,v]"),
                  ID("[V,o]"), ID("[V,v]"), 0, "MO Ints <Vo|Vv>");
	dpd_buf4_init(&G, PSIF_OCEPA_DENSITY, 0, ID("[V,o]"), ID("[O,v]"),
