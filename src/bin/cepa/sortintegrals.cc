...
void SortAllIntegrals(iwlbuf *Buf,int nfzc,int nfzv,int norbs,int ndoccact,int nvirt,bool iscim){
  ...
  // 8 bytes for tmp, 16 for integral struct
  ULI maxelem = memory / (sizeof(double) + sizeof(struct integral));
  if (maxelem > 2*v*(v+1)/2*v*(v+1)/2) maxelem = 2*v*(v+1)/2*v*(v+1)/2;
  ...
  struct integral*integralbuffer;
  ULI nelem = maxelem/(5+2*ov3nfiles+2*nfiles) - 20;
  if ((nelem+20)*(5+2*ov3nfiles+2*nfiles)>maxelem)
     integralbuffer= new integral[(nelem+20)*(5+2*ov3nfiles+2*nfiles)];
  else
     integralbuffer= new integral[maxelem];
  ...
  for (ULI k = 0; k < ov3nfiles; k++) {
      ...
      abci3[k] = integralbuffer+(nelem+20L)*(5L+k);
      abci5[k] = integralbuffer+(nelem+20L)*(5L+ov3nfiles+k);
  }
  ...
  for (ULI k = 0; k < nfiles; k++) {
      abcd1[k] = integralbuffer+(nelem+20L)*(5L+2L*ov3nfiles+k);
      abcd2[k] = integralbuffer+(nelem+20L)*(nfiles+5L+2L*ov3nfiles+k);
  }
  ...
  for (ULI k = 0; k < ov3nfiles; k++){ 
      if (nabci5[k]>0){
         ...
         psio->write(PSIF_DCC_SORT_START+k+2*nfiles+2*ov3nfiles,"E2abci5",(char*)&abci5[k][0],nabci5[k]*sizeof(struct integral),abci5_addr[k],&abci5_addr[k]);
         psio->close(PSIF_DCC_SORT_START+k+2*nfiles+2*ov3nfiles,1);
  ...
  fprintf(outfile,"done.\n");fflush(outfile);
  fprintf(outfile,"        Sort (IA|BC) 2/2....");fflush(outfile);
  SortBlockNewNew(totalnabci5,ov3,integralbuffer2,tmp,PSIF_DCC_ABCI5,"E2abci5",maxelem,PSIF_DCC_SORT_START+2*nfiles+2*ov3nfiles,ov3nfiles);
  fprintf(outfile,"done.\n");fflush(outfile);
  ...
  ULI nbins,binsize,lastbin;
  /**
    *  Combine ABCD1 and ABCD2 integrals for SJS packing
    */
...
void abci5_terms_new(double val,ULI p,ULI q,ULI r,ULI s,ULI o,ULI v,ULI*nabci5,ULI*totalnabci5,struct integral**abci5,ULI binsize,ULI bucketsize,psio_address*addr,ULI filestart,ULI nfiles){
  ...
  for (k = 0; k < nfiles; k++) {
      if (nabci5[k] >= bucketsize) {
         ...
         //write
         psio->open(filestart+k,PSIO_OPEN_OLD);
         psio->write(filestart+k,"E2abci5",(char*)&abci5[k][0],nabci5[k]*sizeof(struct integral),addr[k],&addr[k]);
         psio->close(filestart+k,1);
         totalnabci5[k] += nabci5[k];
         nabci5[k] = 0;
      }
