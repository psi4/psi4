...
void optrot(void)
{
        ...
        for(Ga=0,ai=0; Ga < moinfo.nirreps; Ga++) {
          ...
          for(a=0; a < moinfo.virtpi[Ga]; a++) {
            ...
            for(i=0; i < moinfo.occpi[Gi]; i++,ai++) {
              ...
              T[alpha][h][ai] = -2 * moinfo.L[alpha][aa][ii];
              T[alpha][h][ai+dim] = -2 * moinfo.L[alpha][aa][ii];
            }
          }
        }

        C_DGEMV('n', 2*dim, 2*dim, 1, &(C[h][0][0]), 2*dim, &(T[alpha][h][0]), 1, 0, &(S[alpha][h][0]), 1);
      } /* if(dim) */
    }
  }
  ...
  for(alpha=0; alpha < 3; alpha++) {
    for(h=0; h < moinfo.nirreps; h++) {
      free(R[alpha][h]); free(S[alpha][h]); free(T[alpha][h]);
    }
  }
  for(alpha=0; alpha < 3; alpha++) {
    free(R[alpha]); free(S[alpha]); free(T[alpha]);
  }
  free(R);
  free(S);
  free(T);
