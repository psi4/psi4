...
void calc_orb_step(int npairs, double *grad, double *hess_diag, double *theta)
{
  ...
  for (pair=0; pair<npairs; pair++) {
    ...
    if (denom < 0.0) {
      psi::fprintf(outfile, "Warning: MO Hessian denominator negative\n");
      denom = -denom;
    }
  ...
  */
  if (Params.level_shift) {
    while (hess_det < Params.determ_min) {
      psi::fprintf(outfile,"Level shifting the hessian by %8.3E\n",Params.shift);
      for (i=0;i<npairs;i++) {
    ...
    psi::fprintf(outfile,"Determinant of the hessian is greater than %8.3E\n",
      Params.determ_min);
  }
  ...
  if (!Params.invert_hessian) { /* solve H delta = - g */
    psi::fprintf(outfile,"Solving system of linear equations for orbital step...");
    ...
    solved = C_DGESV(npairs,1,&(hess_copy[0][0]),npairs,pivots,
      BVector,npairs);
    if (solved == 0) {
      psi::fprintf(outfile,"equations solved!\n");
      for(i=0;i<npairs;i++) {
        theta[i] = BVector[i];
      }
    }
    else {
      psi::fprintf(outfile,"FAILED TO SOLVE FOR THETA VALUES\n");
      psi::fprintf(outfile,"DGESV returned error %5d \n",solved);
    ...
    psi::fprintf(outfile,"Attempting to directly invert the Hessian matrix\n");
    ...
    psi::fprintf(outfile, "Hessian * Hessian inverse = \n");
  ...
  if (biggest_step > Params.step_max) {
    psi::fprintf(outfile, "Scaling the step\n");
    for (i=0;i<npairs;i++) {
...
void calc_orb_step_bfgs(int npairs, double *grad, double **hess, double *theta)
{
  ...
  psi::fprintf(outfile,"\nLargest step in theta space is %12.6lf \n", biggest_step);
  if (biggest_step > Params.step_max) {
    psi::fprintf(outfile, "Largest allowed step %12.6lf --- scaling the step\n",
      Params.step_max);
    for (i=0;i<npairs;i++) {
      theta[i] = theta[i] * Params.step_max / biggest_step;
    }
...
void print_step(int npairs, int steptype)
{
  ...
  if (steptype == 0) 
    ...
    psi::fprintf(outfile, "(print_step): Unrecognized steptype %d\n", steptype);
  ...
  if (sumfile == NULL) {
    psi::fprintf(outfile, "(print_step): Unable to open file %s\n", sumfile_name);
  }
  else {
    entries++;
    psi::fprintf(sumfile, "%5d\n", entries);
    for (i=0; i<entries; i++) {
      psi::fprintf(sumfile, "%5d %5d %14.9lf %14.9lf %20.12lf %9s\n", i+1, nind[i], 
              scaled_rmsgrad[i], rmsgrad[i], energies[i], comments[i]);
    }
    fclose(sumfile);
  }
