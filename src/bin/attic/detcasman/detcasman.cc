...
int main(int argc, char *argv[])
{
  ...
  if (converged) {
    outfile->Printf("                  ORBITALS CONVERGED\n");
    outfile->Printf("\n  * %s total energy = %17.12lf\n", wfn, energy_last);
  }
  else
    outfile->Printf("               ORBITALS DID NOT CONVERGE\n");
...
void title(void)
{
  outfile->Printf("\n");
  outfile->Printf("*******************************************************\n");
  outfile->Printf("                   D E T C A S M A N\n");
  outfile->Printf("\n");
  outfile->Printf("                   C. David Sherrill\n") ;
  outfile->Printf("                    October 7 1998\n") ;
  outfile->Printf("*******************************************************\n");
  outfile->Printf("\n\n\n");
...
{
  outfile->Printf("\n");
  outfile->Printf("                DETCAS MANAGER EXITING\n");
  outfile->Printf("*******************************************************\n");
  outfile->Printf("\n\n\n");
...
double calc_ci_conv(double scale_conv, double *energy_last)
{
  ...
  if (fscanf(sumfile, "%d", &entries) != 1) {
    outfile->Printf("detcasman: Trouble reading num entries in file %s\n",
            sumfile_name);
    fclose(sumfile);
    return(scale_conv * 0.1);
  }
...
void print_mos_aobasis(void) {
  outfile->Printf("In print_mos_aobasis.\n");
