  ...
  if(params.ref == 0) { /** RHF **/
    ...
    */
    ...
    for(iter=0; iter < params.maxiter; iter++) {
      ...
      sprintf(lbl, "New UIjAb[%d] Increment", irrep);
      dpd_->buf4_init(&Unew, PSIF_CC_MISC, irrep, 0, 5, 0, 5, 0, lbl);
      sprintf(lbl, "UIjAb[%d]", irrep);
      dpd_->buf4_init(&U, PSIF_CC_MISC, irrep, 0, 5, 0, 5, 0, lbl);

      dpd_->buf4_axpy(&U, &Unew, -lambda);

      dpd_->file2_init(&F, PSIF_CC_OEI, 0, 0, 0, "fIJ");
      dpd_->contract424(&U, &F, &Unew, 1, 0, 1, -1, 1);
      dpd_->contract244(&F, &U, &Unew, 0, 0, 0, -1, 1);
      dpd_->file2_close(&F);

      dpd_->file2_init(&F, PSIF_CC_OEI, 0, 1, 1, "fAB");
      dpd_->contract244(&F, &U, &Unew, 1, 2, 1, 1, 1);
      dpd_->contract424(&U, &F, &Unew, 3, 1, 0, 1, 1);
      dpd_->file2_close(&F);
	...
	sprintf(lbl, "dIjAb[%d]", irrep);
	dpd_->buf4_init(&D, PSIF_CC_MISC, irrep, 0, 5, 0, 5, 0, lbl);
	dpd_->buf4_dirprd(&D, &Unew);
	dpd_->buf4_close(&D);
      }

      rms = 0.0;
      for(h=0; h < nirreps; h++) {
	dpd_->buf4_mat_irrep_init(&Unew, h);
	dpd_->buf4_mat_irrep_rd(&Unew, h);
	...
	dpd_->buf4_mat_irrep_close(&Unew, h);
      }
      dpd_->buf4_close(&Unew);
      rms = sqrt(rms);
    ...
    sprintf(lbl, "ZIJAB[%d]", irrep);
    dpd_->buf4_init(&Z, PSIF_CC_MISC, irrep, 2, 7, 2, 7, 0, lbl);
    sprintf(lbl, "UIJAB[%d]", irrep);
    dpd_->buf4_copy(&Z, PSIF_CC_MISC, lbl);
    dpd_->buf4_close(&Z);

    sprintf(lbl, "UIJAB[%d]", irrep);
    dpd_->buf4_init(&U, PSIF_CC_MISC, irrep, 2, 7, 2, 7, 0, lbl);
    sprintf(lbl, "dIJAB[%d]", irrep);
    ...
    /* U(ij,ab) <-- Z(ij,ab) * D(ij,ab) */
    sprintf(lbl, "Zijab[%d]", irrep);
    dpd_->buf4_init(&Z, PSIF_CC_MISC, irrep, 12, 17, 12, 17, 0, lbl);
    sprintf(lbl, "Uijab[%d]", irrep);
    dpd_->buf4_copy(&Z, PSIF_CC_MISC, lbl);
    dpd_->buf4_close(&Z);

    sprintf(lbl, "Uijab[%d]", irrep);
    dpd_->buf4_init(&U, PSIF_CC_MISC, irrep, 12, 17, 12, 17, 0, lbl);
    sprintf(lbl, "dijab[%d]", irrep);
    ...
    sprintf(lbl, "UIjAb[%d]", irrep);
    dpd_->buf4_init(&U, PSIF_CC_MISC, irrep, 22, 28, 22, 28, 0, lbl);
    sprintf(lbl, "dIjAb[%d]", irrep);
