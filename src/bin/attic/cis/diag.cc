...
void diag(void)
{
  ...
  if(params.ref == 0) { /**RHF **/
	...
	if(nroot != params.rpi[h])
	  psi::fprintf(outfile, "\tDavidson algorithm converged only %d roots in irrep %d.\n", 
		  nroot, h);
      }
      ...
      /*
	psi::fprintf(outfile, "RHF-CIS Singlet Eigenvectors for irrep %d:\n", h);
	...
	if(nroot != params.rpi[h])
      ...
      /*
	psi::fprintf(outfile, "UHF-CIS Eigenvectors for irrep %d:\n", h);
	print_mat(v, dim, params.rpi[h], outfile);
      */
