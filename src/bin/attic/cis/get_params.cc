...
/* start pre- porting
  ...
  for(h=0; h < moinfo.nirreps; h++) {
    ...
    if(params.rpi[h] > max_dim) {
      ...
      psi::fprintf(outfile, "\tSetting rpi[%d] = %d.\n", h, max_dim);
...
//  fndcor(&(params.memory),infile,outfile);
  ...
  psi::fprintf(outfile, "\tRoots sought per irrep =  ");
  for (i=0;i<moinfo.nirreps;++i) psi::fprintf(outfile, "%3d", params.rpi[i]);
  psi::fprintf(outfile,"\n");
  psi::fprintf(outfile, "\tDiagonalization        = %s\n", params.diag_method.c_str());
  ...
  if(params.local) {
    ...
    psi::fprintf(outfile, "\tAmp Print Cutoff =   %f\n", local.amp_print_cutoff);
  }
  psi::fprintf(outfile, "\n");
