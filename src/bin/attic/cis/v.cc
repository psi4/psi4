...
void v_build(int irrep, int root, enum Spin spin)
{
  ...
  if(params.ref == 0) { /** RHF **/
    ...
    dpd_->file2_init(&V, PSIF_CC_MISC, irrep, 0, 1, lbl);

    dpd_->file2_init(&F, PSIF_CC_MISC, 0, 1, 1, "FAB");
    dpd_->contract222(&B, &F, &V, 0, 0, 1, 0);
    dpd_->file2_close(&F);

    dpd_->file2_init(&F, PSIF_CC_MISC, 0, 0, 0, "FIJ");
    dpd_->contract222(&F, &B, &V, 0, 1, 1, 1);
    dpd_->file2_close(&F);
    ...
    sprintf(lbl, "VIA[%d]", irrep);
    dpd_->file2_init(&V_A, PSIF_CC_MISC, irrep, 0, 1, lbl);
    sprintf(lbl, "Via[%d]", irrep);
    dpd_->file2_init(&V_B, PSIF_CC_MISC, irrep, 2, 3, lbl);

    dpd_->file2_init(&F, PSIF_CC_MISC, 0, 1, 1, "FAB");
    dpd_->contract222(&B_A, &F, &V_A, 0, 0, 1, 0);
    dpd_->file2_close(&F);

    dpd_->file2_init(&F, PSIF_CC_MISC, 0, 0, 0, "FIJ");
    dpd_->contract222(&F, &B_A, &V_A, 0, 1, 1, 1);
    dpd_->file2_close(&F);

    sprintf(lbl, "FKC(%d)[%d]", root, irrep);
    dpd_->file2_init(&F, PSIF_CC_MISC, irrep, 0, 1, lbl);
    dpd_->buf4_init(&T2, PSIF_CC_MISC, 0, 0, 5, 2, 7, 0, "MP2 tIJAB");
    dpd_->dot24(&F, &T2, &V_A, 0, 0, 1, 1);
    dpd_->buf4_close(&T2);
    dpd_->file2_close(&F);
    ...
    dpd_->contract222(&B_B, &F, &V_B, 0, 0, 1, 0);
    dpd_->file2_close(&F);

    dpd_->file2_init(&F, PSIF_CC_MISC, 0, 2, 2, "Fij");
    dpd_->contract222(&F, &B_B, &V_B, 0, 1, 1, 1);
    dpd_->file2_close(&F);

    sprintf(lbl, "Fkc(%d)[%d]", root, irrep);
    dpd_->file2_init(&F, PSIF_CC_MISC, irrep, 2, 3, lbl);
    dpd_->buf4_init(&T2, PSIF_CC_MISC, 0, 10, 15, 12, 17, 0, "MP2 tijab");
    dpd_->dot24(&F, &T2, &V_B, 0, 0, 1, 1);
    dpd_->buf4_close(&T2);
    dpd_->file2_close(&F);
