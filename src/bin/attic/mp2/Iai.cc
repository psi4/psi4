...
void rhf_sf_Iai(void)
{
  ...
  dpd_->buf4_init(&Eints, PSIF_CC_EINTS, 0, 11, 0, 11, 0, 1, "E <ai|jk>");
  dpd_->dot24(&D, &Eints, &I, 0, 0, 1.0, 0.0);
  dpd_->dot24(&D, &Eints, &I, 1, 0, 1.0, 1.0);
  dpd_->buf4_close(&Eints);
  dpd_->file2_close(&D);

  dpd_->file2_init(&D, PSIF_CC_OEI, 0, 0, 0, "Dij");
  dpd_->buf4_init(&Eints, PSIF_CC_EINTS, 0, 11, 0, 11, 0, 0, "E <ai|jk>");
  ...
  dpd_->buf4_init(&Fints, PSIF_CC_FINTS, 0, 10, 5, 10, 5, 1, "F <ia|bc>");
  dpd_->dot24(&D, &Fints, &I, 1, 1, 1.0, 1.0);
  dpd_->dot24(&D, &Fints, &I, 0, 1, 1.0, 1.0);
  ...
  dpd_->contract442(&Fints, &G, &I, 1, 1, 2.0, 1.0);
  dpd_->buf4_close(&G);
  dpd_->buf4_close(&Fints);

  dpd_->buf4_init(&Fints, PSIF_CC_FINTS, 0, 10, 5, 10, 5, 0, "F <ia|bc>");
  dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 0, 5, 0, 5, 0, "GIjAb");
  dpd_->buf4_sort(&G, PSIF_CC_TMP0, qpsr, 0, 5, "GiJaB");
  dpd_->buf4_close(&G);
  dpd_->buf4_init(&G, PSIF_CC_TMP0, 0, 0, 5, 0, 5, 0, "GiJaB");
  dpd_->contract442(&Fints, &G, &I, 1, 1, 2.0, 1.0);
  dpd_->buf4_close(&G);
  dpd_->buf4_close(&Fints);
