...
void rhf_sort_twopdm(void)
{
  ...
  dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 11, 0, 11, 0, 0, "GAiJk");
  for(h=0; h < nirreps; h++) {
    dpd_->buf4_mat_irrep_init(&G, h);
    for(Gm=0; Gm < nirreps; Gm++) {
      Gi = Ga = h^Gm;
  ...
  dpd_->buf4_sort(&G1, PSIF_CC_GAMMA, pqsr, 0, 0, "GIjlK");
  dpd_->buf4_scm(&G1, 2.0);
  dpd_->buf4_init(&G2, PSIF_CC_GAMMA, 0, 0, 0, 0, 0, 0, "GIjlK");
  dpd_->buf4_axpy(&G2, &G1, -1.0);
  dpd_->buf4_close(&G2);
  dpd_->buf4_close(&G1);

  dpd_->buf4_init(&G1, PSIF_CC_GAMMA, 0, 11, 0, 11, 0, 0, "GAiJk");
  dpd_->buf4_sort(&G1, PSIF_CC_GAMMA, pqsr, 11, 0, "GAikJ");
  dpd_->buf4_scm(&G1, 2.0);
  dpd_->buf4_init(&G2, PSIF_CC_GAMMA, 0, 11, 0, 11, 0, 0, "GAikJ");
  dpd_->buf4_axpy(&G2, &G1, -1.0);
  dpd_->buf4_close(&G2);
  dpd_->buf4_close(&G1);

  dpd_->buf4_init(&G1, PSIF_CC_GAMMA, 0, 0, 5, 0, 5, 0, "GIjAb");
  dpd_->buf4_sort(&G1, PSIF_CC_GAMMA, pqsr, 0, 5, "GIjbA");
  dpd_->buf4_scm(&G1, 2.0);
  dpd_->buf4_init(&G2, PSIF_CC_GAMMA, 0, 0, 5, 0, 5, 0, "GIjbA");
  dpd_->buf4_axpy(&G2, &G1, -1.0);
  dpd_->buf4_close(&G2);
  dpd_->buf4_init(&G2, PSIF_CC_GAMMA, 0, 10, 10, 10, 10, 0, "GIbJa");
  dpd_->buf4_sort(&G2, PSIF_CC_GAMMA, prsq, 0, 5, "GIbJa (IJ,ab)");
  dpd_->buf4_close(&G2);
  dpd_->buf4_init(&G2, PSIF_CC_GAMMA, 0, 0, 5, 0, 5, 0, "GIbJa (IJ,ab)");
  dpd_->buf4_axpy(&G2, &G1, -0.5);
  dpd_->buf4_close(&G2);
  dpd_->buf4_close(&G1);
  ...
  dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 11, 0, 11, 0, 0, "GAiJk");
  dpd_->buf4_sort(&G, PSIF_CC_TMP0, qsrp, 0, 10, "G(IK,JA)");
  dpd_->buf4_close(&G);
  ...
  dpd_->buf4_sort(&G, PSIF_CC_TMP0, prqs, 10, 10, "G(IA,JB)");
  dpd_->buf4_close(&G);
  dpd_->buf4_init(&G, PSIF_CC_TMP0, 0, 10, 10, 10, 10, 0, "G(IA,JB)");
  dpd_->buf4_symm(&G);
