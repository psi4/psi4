...
namespace psi { namespace ccdensity {
    ...
    */
    ...
    void energy_UHF(struct RHO_Params rho_params)
    {
      ...
      this_energy = dpd_->file2_dot(&D, &F);
      dpd_->file2_close(&F);
      dpd_->file2_close(&D);
      ...
      dpd_->file2_init(&D, PSIF_CC_OEI, 0, 2, 2, rho_params.Dij_lbl);
      ...
      two_energy += dpd_->buf4_dot(&G, &A);
      dpd_->buf4_close(&G);
      dpd_->buf4_close(&A);

      dpd_->buf4_init(&A, PSIF_CC_AINTS, 0, 12, 12, 10, 10, 1, "A <ij|kl>");
      dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 12, 12, 12, 12, 0, "Gijkl");
      two_energy += dpd_->buf4_dot(&G, &A);
      dpd_->buf4_close(&G);
      dpd_->buf4_close(&A);

      dpd_->buf4_init(&A, PSIF_CC_AINTS, 0, 22, 22, 22, 22, 0, "A <Ij|Kl>");
      dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 22, 22, 22, 22, 0, "GIjKl");
      two_energy += dpd_->buf4_dot(&G, &A);
      dpd_->buf4_close(&G);
      dpd_->buf4_close(&A);
      ...
      dpd_->buf4_init(&DInts, PSIF_CC_DINTS, 0, 2, 7, 2, 7, 0, "D <IJ||AB> (I>J,A>B)");
      dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 2, 7, 2, 7, 0, "GIJAB");
      two_energy += dpd_->buf4_dot(&G, &DInts);
      dpd_->buf4_close(&G);
      dpd_->buf4_close(&DInts);

      dpd_->buf4_init(&DInts, PSIF_CC_DINTS, 0, 12, 17, 12, 17, 0, "D <ij||ab> (i>j,a>b)");
      ...
      */
      ...
      dpd_->buf4_init(&DInts, PSIF_CC_DINTS, 0, 24, 27, 24, 27, 0, "D <Ij|Ab> (Ib,jA)");
      dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 24, 27, 24, 27, 0, "GIbjA");
      two_energy -= dpd_->buf4_dot(&G, &DInts);
      dpd_->buf4_close(&G);
      dpd_->buf4_close(&DInts);

      dpd_->buf4_init(&DInts, PSIF_CC_DINTS, 0, 27, 24, 27, 24, 0, "D <iJ|aB> (iB,Ja)");
      ...
      dpd_->buf4_init(&FInts, PSIF_CC_FINTS, 0, 21, 7, 21, 5, 1, "F <AI|BC>");
      dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 21, 7, 21, 7, 0, "GCIAB");
      two_energy += dpd_->buf4_dot(&G, &FInts);
      dpd_->buf4_close(&G);
      dpd_->buf4_close(&FInts);

      dpd_->buf4_init(&FInts, PSIF_CC_FINTS, 0, 31, 17, 31, 15, 1, "F <ai|bc>");
      dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 31, 17, 31, 17, 0, "Gciab");
      two_energy += dpd_->buf4_dot(&G, &FInts);
      dpd_->buf4_close(&G);
      dpd_->buf4_close(&FInts);

      dpd_->buf4_init(&FInts, PSIF_CC_FINTS, 0, 25, 29, 25, 29, 0, "F <aI|bC>");
      dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 25, 29, 25, 29, 0, "GcIaB");
      two_energy += dpd_->buf4_dot(&G, &FInts);
      dpd_->buf4_close(&G);
      dpd_->buf4_close(&FInts);

      dpd_->buf4_init(&FInts, PSIF_CC_FINTS, 0, 26, 28, 26, 28, 0, "F <Ai|Bc>");
      ...
      two_energy += dpd_->buf4_dot(&G, &B);
      dpd_->buf4_close(&G);
      dpd_->buf4_close(&B);

      dpd_->buf4_init(&B, PSIF_CC_BINTS, 0, 17, 17, 15, 15, 1, "B <ab|cd>");
      dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 17, 17, 17, 17, 0, "Gabcd");
      two_energy += dpd_->buf4_dot(&G, &B);
      dpd_->buf4_close(&G);
      dpd_->buf4_close(&B);
