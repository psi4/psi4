...
void sort_ltd_uhf(struct TD_Params S)
{
  ...
  dpd_->file2_init(&D, PSIF_CC_TMP, S.irrep, 2, 2, "LTDij");
  dpd_->file2_mat_init(&D); 
  dpd_->file2_mat_rd(&D);
  for(h=0; h < nirreps; h++) {
  ...
  dpd_->file2_init(&D, PSIF_CC_TMP, S.irrep, 3, 3, "LTDab");
  dpd_->file2_mat_init(&D);
  dpd_->file2_mat_rd(&D);
  for(h=0; h < nirreps; h++) {
  ...
  /* Note that this component of the density is stored occ-vir */
  dpd_->file2_init(&D, PSIF_CC_TMP, S.irrep, 2, 3, "LTDai");
  dpd_->file2_mat_init(&D);
  dpd_->file2_mat_rd(&D);
  for(h=0; h < nirreps; h++) {
    for(i=0; i < boccpi[h]; i++) {
  ...
  dpd_->file2_init(&D, PSIF_CC_TMP, S.irrep, 2, 3, "LTDia");
  dpd_->file2_mat_init(&D);
  dpd_->file2_mat_rd(&D);
  for(h=0; h < nirreps; h++) {
    for(i=0; i < boccpi[h]; i++) {
