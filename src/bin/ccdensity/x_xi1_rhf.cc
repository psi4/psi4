...
x_xi_check("begin xi1");
  ...
  if ((R_irr == 0)  && (!params.connect_xi)) {
    ...
    params.RD_overlap = dpd_->file2_trace(&I1);
    dpd_->file2_close(&I1);
    dpd_->file2_init(&L1, PSIF_CC_GL, L_irr, 0, 1, "LIA");
    dpd_->file2_copy(&L1, PSIF_EOM_XI, "XIA");
    dpd_->file2_close(&L1);
    dpd_->file2_init(&L1, PSIF_EOM_XI, G_irr, 0, 1, "XIA");
    dpd_->file2_scm(&L1, params.RD_overlap);
    dpd_->file2_close(&L1);
  }
#ifdef DEBUG_XI
x_xi_check("term 1");
...
x_xi_check("term 4");
  ...
  dpd_->file2_init(&L1, PSIF_CC_GL, L_irr, 0, 1, "LIA");
  params.overlap1 = 2.0 * dpd_->file2_dot(&R1, &L1);
  dpd_->file2_close(&R1);
  dpd_->file2_close(&L1);
...
x_xi_check("term 8");
  ...
  dpd_->file2_init(&XIA, PSIF_EOM_XI, G_irr, 0, 1, "XIA");
  dpd_->file2_init(&I1, PSIF_EOM_TMP, G_irr, 0, 0, "LR2_OO");
  dpd_->buf4_init(&H2, PSIF_CC_HBAR, 0, 0, 11, 0, 11, 0, "2WMnIe - WnMIe (Mn,eI)");
...
/*  term 11 XIA -= Rmnef Lmoea Winof */
  dpd_->buf4_init(&H2, PSIF_CC_HBAR, 0, 0, 11, 0, 11, 0, "WMnIe - 2WnMIe (Mn,eI)");
  dpd_->buf4_sort(&H2, PSIF_EOM_TMP1, qrsp, 10, 0, "WMnIe - 2WnMIe qrsp");
  dpd_->buf4_close(&H2);
  dpd_->buf4_init(&H2, PSIF_CC_HBAR, 0, 0, 11, 0, 11, 0, "2WMnIe - WnMIe (Mn,eI)");
  dpd_->buf4_sort(&H2, PSIF_EOM_TMP1, qrsp, 10, 0, "2WMnIe - WnMIe qrsp");
  dpd_->buf4_close(&H2);

  dpd_->file2_init(&XIA, PSIF_EOM_XI, G_irr, 0, 1, "XIA");
  dpd_->buf4_init(&H2, PSIF_EOM_TMP1, 0, 10, 0, 10, 0, 0, "2WMnIe - WnMIe qrsp");
  dpd_->buf4_init(&I2, PSIF_EOM_TMP, G_irr, 10, 10, 10, 10, 0, "R2L2_OVov");
  dpd_->contract442(&H2, &I2, &XIA, 3, 3, -1.0, 1.0);
  dpd_->buf4_close(&I2);
  dpd_->buf4_close(&H2);
  dpd_->buf4_init(&H2, PSIF_EOM_TMP1, 0, 10, 0, 10, 0, 0, "WMnIe - 2WnMIe qrsp");
  dpd_->buf4_init(&I2, PSIF_EOM_TMP, G_irr, 10, 10, 10, 10, 0, "R2L2_OvOv");
  dpd_->contract442(&H2, &I2, &XIA, 3, 3, -1.0, 1.0);
  dpd_->buf4_close(&I2);
  dpd_->buf4_close(&H2);
...
/* term 12, + (Rmnef Lmieg) Wgnaf */
  dpd_->buf4_init(&I2, PSIF_EOM_TMP, G_irr, 10, 10, 10, 10, 0, "R2L2_OvOv");
  dpd_->buf4_copy(&I2, PSIF_EOM_TMP1, "R2L2 2OVov + OvOv");
  dpd_->buf4_close(&I2);
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 10, 10, 10, 10, 0, "R2L2 2OVov + OvOv");
  dpd_->buf4_init(&I2, PSIF_EOM_TMP, G_irr, 10, 10, 10, 10, 0, "R2L2_OVov");
  dpd_->buf4_axpy(&I2, &Z2, 2.0);
  dpd_->buf4_close(&I2);
  dpd_->buf4_sort(&Z2, PSIF_EOM_TMP1, sprq, 11, 10, "2OVov + OvOv (Gn,If)"); 
  dpd_->buf4_close(&Z2);

  dpd_->buf4_init(&I2, PSIF_EOM_TMP, G_irr, 10, 10, 10, 10, 0, "R2L2_OVov");
  dpd_->buf4_copy(&I2, PSIF_EOM_TMP1, "R2L2 OVov + 2OvOv");
  dpd_->buf4_close(&I2);
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 10, 10, 10, 10, 0, "R2L2 OVov + 2OvOv");
  dpd_->buf4_init(&I2, PSIF_EOM_TMP, G_irr, 10, 10, 10, 10, 0, "R2L2_OvOv");
  dpd_->buf4_axpy(&I2, &Z2, 2.0);
  dpd_->buf4_close(&I2);
  dpd_->buf4_sort(&Z2, PSIF_EOM_TMP1, spqr, 11, 11, "OVov + 2OvOv (Gn,fI)"); 
  dpd_->buf4_close(&Z2);

  dpd_->file2_init(&XIA, PSIF_EOM_XI, G_irr, 0, 1, "XIA");
  dpd_->buf4_init(&I2, PSIF_EOM_TMP1, G_irr, 11, 10, 11, 10, 0, "2OVov + OvOv (Gn,If)");
  dpd_->buf4_init(&H2, PSIF_CC_HBAR, 0, 11, 5, 11, 5, 0, "WAmEf");
  dpd_->contract442(&I2, &H2, &XIA, 2, 2, 1.0, 1.0);
  dpd_->buf4_close(&I2);
  dpd_->buf4_init(&I2, PSIF_EOM_TMP1, G_irr, 11, 11, 11, 11, 0, "OVov + 2OvOv (Gn,fI)");
