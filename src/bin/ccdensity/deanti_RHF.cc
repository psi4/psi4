...
namespace psi { namespace ccdensity {
    ...
    ** */
	...
	fflush(outfile);
      ...
      dpd_buf4_scmcopy(&G1, PSIF_CC_GAMMA, "2 Gijkl - Gijlk", 2);
      dpd_buf4_sort_axpy(&G1, PSIF_CC_GAMMA, pqsr, 0, 0, "2 Gijkl - Gijlk", -1);
      dpd_buf4_close(&G1);

      dpd_buf4_init(&G1, PSIF_CC_GAMMA, 0, 0, 0, 0, 0, 0, "2 Gijkl - Gijlk");
      dpd_buf4_copy(&G1, PSIF_CC_GAMMA, "GIjKl");
      if(!params.aobasis) {
	...
	total_two_energy += two_energy;
      ...
      dpd_buf4_scmcopy(&G1, PSIF_CC_GAMMA, "2 Gijka - Gjika", 2);
      dpd_buf4_sort_axpy(&G1, PSIF_CC_GAMMA, qprs, 0, 10, "2 Gijka - Gjika", -1);
      dpd_buf4_close(&G1);

      dpd_buf4_init(&G1, PSIF_CC_GAMMA, 0, 0, 10, 0, 10, 0, "2 Gijka - Gjika");
      /* The factor of 2 here is necessary because Gijka is multiplied by 1/2 in Gijka.cc */
      dpd_buf4_scmcopy(&G1, PSIF_CC_GAMMA, "GIjKa", 2);
      if(!params.aobasis) {
	...
	total_two_energy += 4*two_energy;
      ...
      dpd_buf4_scmcopy(&G1, PSIF_CC_GAMMA, "2 Gijab - Gijba", 2);
      dpd_buf4_sort_axpy(&G1, PSIF_CC_GAMMA, pqsr, 0, 5, "2 Gijab - Gijba", -1);
      dpd_buf4_close(&G1);

      dpd_buf4_init(&G2, PSIF_CC_GAMMA, 0, 10, 10, 10, 10, 0, "GIBJA");
      dpd_buf4_sort_axpy(&G2, PSIF_CC_GAMMA, prsq, 0, 5, "2 Gijab - Gijba", -1);
      dpd_buf4_close(&G2);
      dpd_buf4_init(&G2, PSIF_CC_GAMMA, 0, 10, 10, 10, 10, 0, "GIbjA");
      dpd_buf4_sort_axpy(&G2, PSIF_CC_GAMMA, prsq, 0, 5, "2 Gijab - Gijba", -1);
      dpd_buf4_close(&G2);  

      dpd_buf4_init(&G1, PSIF_CC_GAMMA, 0, 0, 5, 0, 5, 0, "2 Gijab - Gijba");
      dpd_buf4_scmcopy(&G1, PSIF_CC_GAMMA, "GIjAb", 2);
      if(!params.aobasis) {
	...
	total_two_energy += two_energy;
      ...
      dpd_buf4_scmcopy(&G1, PSIF_CC_GAMMA, "2 Gciab - Gciba", 2);
      dpd_buf4_sort_axpy(&G1, PSIF_CC_GAMMA, pqsr, 11, 5, "2 Gciab - Gciba", -1);
      dpd_buf4_close(&G1);

      dpd_buf4_init(&G1, PSIF_CC_GAMMA, 0, 11, 5, 11, 5, 0, "2 Gciab - Gciba");
      /* The factor of 2 here is necessary because Gciab is multiplied by 1/2 in Gciab.cc */
      dpd_buf4_scmcopy(&G1, PSIF_CC_GAMMA, "GCiAb", 2);
      if(!params.aobasis) {
	...
	total_two_energy += 4*two_energy;
      ...
      dpd_buf4_scmcopy(&G1, PSIF_CC_GAMMA, "2 Gabcd - Gabdc", 2);
      dpd_buf4_sort_axpy(&G1, PSIF_CC_GAMMA, pqsr, 5, 5, "2 Gabcd - Gabdc", -1);
      dpd_buf4_close(&G1);

      dpd_buf4_init(&G1, PSIF_CC_GAMMA, 0, 5, 5, 5, 5, 0, "2 Gabcd - Gabdc");
      dpd_buf4_copy(&G1, PSIF_CC_GAMMA, "GAbCd");
      if(!params.aobasis) {  
