...
void x_xi2_rohf(void)
{
  ...
  dpd_->buf4_scmcopy(&D2, PSIF_EOM_XI, "Xijab", params.overlap1+params.overlap2);
  dpd_->buf4_close(&D2);
  dpd_->buf4_init(&D2, PSIF_CC_DINTS, 0, 0, 5, 0, 5, 0, "D <ij|ab>");
  dpd_->buf4_scmcopy(&D2, PSIF_EOM_XI, "XIjAb", params.overlap1+params.overlap2);
  dpd_->buf4_close(&D2);
#ifdef DEBUG_XI
...
x_xi_check("terms 4 and 6");
  ...
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, R_irr, 0, 0, 0, 0, 0, "Z (Ij,Mn)");
  dpd_->contract444(&D2, &R2, &Z2, 0, 0, 1.0, 0.0); 
  dpd_->buf4_close(&R2);
  dpd_->buf4_close(&D2);
  dpd_->buf4_init(&XIjAb, PSIF_EOM_XI, G_irr, 0, 5, 0, 5, 0, "XIjAb");
...
x_xi_check("term 8");
  ...
  dpd_->file2_init(&I1, PSIF_EOM_TMP1, R_irr, 1, 1, "Z (f,a)");
      /* for term 20: */
  dpd_->file2_init(&Z1B, PSIF_EOM_TMP_XI, R_irr, 1, 1, "R1Wamef_vv");
...
x_xi_check("terms 11, 17 and 20");
  ...
  dpd_->file2_init(&I1, PSIF_EOM_TMP1, R_irr, 0, 0, "Z (m,i)");
      /* for term 21 */
  dpd_->file2_init(&Z1B, PSIF_EOM_TMP_XI, R_irr, 0, 0, "R1Wmnie_oo");
...
x_xi_check("term 14 (Fme)");
  ...
  if (!params.connect_xi) {
    ...
    dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "Z (jI,bA)");
    dpd_->file2_init(&I1, PSIF_EOM_TMP, G_irr, 0, 1, "L2R1_ov");
    dpd_->buf4_init(&H2, PSIF_CC_HBAR, 0, 11, 5, 11, 5, 0, "WaMeF");
    dpd_->contract244(&I1, &H2, &Z2, 1, 0, 0, 1.0, 0.0);
    dpd_->buf4_close(&H2);
    dpd_->file2_close(&I1);
    dpd_->buf4_sort_axpy(&Z2, PSIF_EOM_XI, qpsr, 0, 5, "XIjAb", 1.0);
    dpd_->buf4_close(&Z2);
#ifdef DEBUG_XI
x_xi_check("term 22 (Wamef)");
  ...
  if (!params.connect_xi) {
    ...
    dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "Z (jI,Ab)");
    dpd_->file2_init(&I1, PSIF_EOM_TMP, G_irr, 0, 1, "L2R1_ov");
    dpd_->buf4_init(&H2, PSIF_CC_HBAR, 0, 0, 11, 0, 11, 0, "WmNiE (mN,Ei)");
    dpd_->contract424(&H2, &I1, &Z2, 3, 0, 0, 1.0, 0.0);
    dpd_->buf4_close(&H2);
    dpd_->file2_close(&I1);
    dpd_->buf4_sort_axpy(&Z2, PSIF_EOM_XI, qprs, 0, 5, "XIjAb", -1.0);
    dpd_->buf4_close(&Z2);
#ifdef DEBUG_XI
...
x_xi_check("term 25 (Wmnie)");
  ...
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "Z (Ij,bA)");
  dpd_->buf4_init(&I2, PSIF_EOM_TMP, G_irr, 0, 11, 0, 11, 0, "L2R1_OovO");
  dpd_->buf4_init(&H2, PSIF_CC_HBAR, 0, 11, 5, 11, 5, 0, "WaMeF");
  dpd_->contract444(&I2, &H2, &Z2, 0, 1, 1.0, 0.0); 
  dpd_->buf4_close(&H2);
  dpd_->buf4_close(&I2);
  dpd_->buf4_sort_axpy(&Z2, PSIF_EOM_XI, pqsr, 0, 5, "XIjAb", -1.0);
  dpd_->buf4_close(&Z2);
#ifdef DEBUG_XI
x_xi_check("term 24 (Wamef)");
  ...
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, R_irr, 11, 11, 11, 11, 0, "Z (EJ,BN)");
  ...
  dpd_->buf4_close(&H2);
  dpd_->file2_close(&R1);
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP1, qpsr, 10, 10, "Z (JE,NB)");
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, R_irr, 10, 10, 10, 10, 0, "Z (JE,NB)");
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP1, psrq, 10, 10, "Z (JB,NE)");
  dpd_->buf4_close(&Z);

  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, R_irr, 11, 10, 11, 10, 0, "Z (eJ,nB)");
  dpd_->buf4_init(&H2, PSIF_CC_HBAR, 0, 0, 10, 0, 10, 0, "WmNiE");
  dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "Ria");
  dpd_->contract244(&R1, &H2, &Z, 0, 0, 0, 1.0, 0.0);
  dpd_->buf4_close(&H2);
  ...
  dpd_->buf4_close(&H2);
  dpd_->file2_close(&R1);
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP1, qprs, 10, 10, "Z (Je,nB)");
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, R_irr, 10, 10, 10, 10, 0, "Z (Je,nB)");
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP1, psrq, 10, 10, "Z (JB,ne)");
  dpd_->buf4_close(&Z);
  ...
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, R_irr, 10, 11, 10, 11, 0, "Z (Je,bN)");
  dpd_->buf4_init(&H2, PSIF_CC_HBAR, 0, 0, 11, 0, 11, 0, "WMnIe (Mn,eI)");
  dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "Ria");
  dpd_->contract424(&H2, &R1, &Z, 1, 0, 1, -1.0, 0.0);
  dpd_->file2_close(&R1);
  dpd_->buf4_close(&H2);
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP1, prsq, 10, 10, "Z (Jb,Ne)");
  dpd_->buf4_close(&Z);

  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, R_irr, 11, 11, 11, 11, 0, "Z2 (eJ,bN)");
  dpd_->buf4_init(&H2, PSIF_CC_HBAR, 0, 11, 5, 11, 5, 0, "WaMeF");
  dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  dpd_->contract424(&H2, &R1, &Z, 3, 1, 0, 1.0, 0.0);
  dpd_->file2_close(&R1);
  dpd_->buf4_close(&H2);
  dpd_->buf4_sort_axpy(&Z, PSIF_EOM_TMP1, qrsp, 10, 10, "Z (Jb,Ne)", 1.0);
  dpd_->buf4_close(&Z);
  ...
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 10, 10, 10, 10, 0, "Z2 (IA,JB)");
  dpd_->buf4_init(&L2, PSIF_CC_GL, L_irr, 10, 10, 10, 10, 0, "LIAJB");
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, R_irr, 10, 10, 10, 10, 0, "Z (JB,NE)");
...
x_xi_check("terms 18, 19 (Wmnie, Wamef)");
  ...
  dpd_->buf4_close(&L2);
  dpd_->buf4_close(&Z);
  dpd_->buf4_sort_axpy(&Z2, PSIF_EOM_XI, prqs, 0, 5, "XIjAb", -1.0);
  dpd_->buf4_close(&Z2);

  /* XIjAb += + L(jA,Ne) Z(Ib,Ne) + L(Ib,nE) Z(jA,nE)  */
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 10, 10, 10, 10, 0, "Z2 (Ib,jA)");
  dpd_->buf4_init(&L2, PSIF_CC_GL, L_irr, 10, 10, 10, 10, 0, "LjAIb");
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, R_irr, 10, 10, 10, 10, 0, "Z (Jb,Ne)");
  dpd_->contract444(&Z, &L2, &Z2, 0, 0, -1.0, 0.0);
  dpd_->buf4_close(&Z);
  dpd_->buf4_close(&L2);

  dpd_->buf4_init(&L2, PSIF_CC_GL, L_irr, 10, 10, 10, 10, 0, "LIbjA");
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, R_irr, 10, 10, 10, 10, 0, "Z (jB,nE)");
  dpd_->contract444(&L2, &Z, &Z2, 0, 0, -1.0, 1.0);
  dpd_->buf4_close(&Z);
  dpd_->buf4_close(&L2);
  dpd_->buf4_sort_axpy(&Z2, PSIF_EOM_XI, prsq, 0, 5, "XIjAb", 1.0);
  dpd_->buf4_close(&Z2);
#ifdef DEBUG_XI
x_xi_check("terms 18, 19 (Wmnie, Wamef)");
#endif
...
void x_xi2_4_rohf(void)
{
  ...
  dpd_->file2_mat_init(&RIA);
  dpd_->file2_mat_init(&Ria);
  dpd_->file2_mat_init(&LIA);
  dpd_->file2_mat_init(&Lia);
  dpd_->file2_mat_rd(&RIA);
  dpd_->file2_mat_rd(&Ria);
  dpd_->file2_mat_rd(&LIA);
  dpd_->file2_mat_rd(&Lia);

  dpd_->buf4_init(&I2, PSIF_EOM_TMP1, G_irr, 10, 10, 10, 10, 0, "RL_OVOV");
  for(h=0; h < nirreps; h++) {
  ...
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 10, 10, 10, 0, "RL_OVOV");
...
void x_xi2_14(void)
{
  ...
  dpd_->file2_init(&IIA, PSIF_EOM_TMP, G_irr, 0, 1, "L2R1_OV");
  dpd_->file2_init(&FME, PSIF_CC_OEI, 0, 0, 1, "FME");
  dpd_->file2_mat_init(&IIA);
  dpd_->file2_mat_init(&FME);
  dpd_->file2_mat_rd(&IIA);
  ...
  if (params.ref == 1) {
    ...
    dpd_->file2_init(&Iia, PSIF_EOM_TMP, G_irr, 2, 3, "L2R1_ov");
  ...
  if (params.ref != 0) { /* ROHF or UHF for XIJAB */
    ...
    dpd_->buf4_init(&XIJAB, PSIF_EOM_XI, G_irr, 0, 5, 2, 7, 0, "XIJAB");
    dpd_->buf4_axpy(&Z, &XIJAB, 1.0);
    dpd_->buf4_close(&XIJAB);
    dpd_->buf4_sort_axpy(&Z, PSIF_EOM_XI, qprs, 2, 7, "XIJAB", -1.0);
    dpd_->buf4_sort_axpy(&Z, PSIF_EOM_XI, pqsr, 2, 7, "XIJAB", -1.0);
    dpd_->buf4_sort_axpy(&Z, PSIF_EOM_XI, qpsr, 2, 7, "XIJAB", 1.0);
    dpd_->buf4_close(&Z);
  }
  ...
  if (params.ref != 0) { /* ROHF or UHF for Xijab */
    if (params.ref == 1)
      ...
      dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 15, 10, 15, 0, "Z (ij,ab)");
    for(h=0; h < nirreps; h++) {
      dpd_->buf4_mat_irrep_init(&Z, h);
      for(row=0; row < Z.params->rowtot[h]; row++) {
        i = Z.params->roworb[h][row][0];
    ...
    if (params.ref == 1) {
      ...
      dpd_->buf4_axpy(&Z, &Xijab, 1.0);
      dpd_->buf4_close(&Xijab);
      dpd_->buf4_sort_axpy(&Z, PSIF_EOM_XI, qprs, 12, 17, "Xijab", -1.0);
      dpd_->buf4_sort_axpy(&Z, PSIF_EOM_XI, pqsr, 12, 17, "Xijab", -1.0);
      dpd_->buf4_sort_axpy(&Z, PSIF_EOM_XI, qpsr, 12, 17, "Xijab", 1.0);
      dpd_->buf4_close(&Z);
    }
  }
  ...
  if (params.ref == 0) { /* RHF XIjAb */
    dpd_->buf4_init(&XIjAb, PSIF_EOM_XI, G_irr, 0, 5, 0, 5, 0, "XIjAb");
    for(h=0; h < nirreps; h++) {
      dpd_->buf4_mat_irrep_init(&XIjAb, h);
      dpd_->buf4_mat_irrep_rd(&XIjAb, h);
      ...
      dpd_->buf4_mat_irrep_wrt(&XIjAb, h);
      dpd_->buf4_mat_irrep_close(&XIjAb, h);
    }
    dpd_->buf4_close(&XIjAb);
    dpd_->file2_mat_close(&IIA);
    dpd_->file2_close(&IIA);
    ...
    dpd_->buf4_close(&XIjAb);
    dpd_->file2_mat_close(&IIA); dpd_->file2_mat_close(&Iia);
    dpd_->file2_close(&IIA); dpd_->file2_close(&Iia);
    dpd_->file2_mat_close(&FME); dpd_->file2_mat_close(&Fme);
