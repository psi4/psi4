...
void x_xi2_rohf(void)
{
  ...
  dpd_buf4_scmcopy(&D2, PSIF_EOM_XI, "Xijab", params.overlap1+params.overlap2);
  dpd_buf4_close(&D2);
  dpd_buf4_init(&D2, PSIF_CC_DINTS, 0, 0, 5, 0, 5, 0, "D <ij|ab>");
  dpd_buf4_scmcopy(&D2, PSIF_EOM_XI, "XIjAb", params.overlap1+params.overlap2);
  dpd_buf4_close(&D2);
#ifdef DEBUG_XI
x_xi_check("terms 1 and 5");
  ...
  dpd_buf4_init(&XIjAb, PSIF_EOM_XI, G_irr, 0, 5, 0, 5, 0, "XIjAb");
  dpd_file2_init(&I1, PSIF_EOM_TMP, G_irr, 1, 1, "LR_VV");
  dpd_buf4_init(&D2, PSIF_CC_DINTS, 0, 0, 5, 0, 5, 0, "D <ij|ab>");
...
x_xi_check("terms 4 and 6");
  ...
  dpd_buf4_init(&D2, PSIF_CC_DINTS, 0, 0, 5, 0, 5, 0, "D <ij|ab>");
  dpd_buf4_init(&R2, PSIF_CC_GR, R_irr, 0, 5, 0, 5, 0, "RIjAb");
  dpd_buf4_init(&Z2, PSIF_EOM_TMP1, R_irr, 0, 0, 0, 0, 0, "Z (Ij,Mn)");
  dpd_contract444(&D2, &R2, &Z2, 0, 0, 1.0, 0.0); 
  dpd_buf4_close(&R2);
  dpd_buf4_close(&D2);
  dpd_buf4_init(&XIjAb, PSIF_EOM_XI, G_irr, 0, 5, 0, 5, 0, "XIjAb");
  dpd_buf4_init(&L2, PSIF_CC_GL, L_irr, 0, 5, 0, 5, 0, "LIjAb");
  dpd_contract444(&Z2, &L2, &XIjAb, 0, 1, 1.0, 1.0); 
  dpd_buf4_close(&L2);
  dpd_buf4_close(&Z2);
  dpd_buf4_close(&XIjAb);
...
x_xi_check("term 7");
  ...
  dpd_buf4_close(&Xijab);
  dpd_buf4_init(&XIjAb, PSIF_EOM_XI, G_irr, 0, 5, 0, 5, 0, "XIjAb");
  dpd_buf4_init(&I2, PSIF_EOM_TMP, G_irr, 0, 0, 0, 0, 0, "R2L2_OoOo");
  dpd_buf4_init(&D2, PSIF_CC_DINTS, 0, 0, 5, 0, 5, 0, "D <ij|ab>");
  dpd_contract444(&I2, &D2, &XIjAb, 1, 1, 1.0, 1.0); 
  dpd_buf4_close(&D2);
  dpd_buf4_close(&I2);
...
x_xi_check("term 8");
  ...
  dpd_file2_init(&I1, PSIF_EOM_TMP1, R_irr, 1, 1, "Z (f,a)");
      /* for term 20: */
  dpd_file2_init(&Z1B, PSIF_EOM_TMP_XI, R_irr, 1, 1, "R1Wamef_vv");
...
x_xi_check("terms 11, 17 and 20");
  ...
  dpd_file2_init(&I1, PSIF_EOM_TMP1, R_irr, 0, 0, "Z (m,i)");
      /* for term 21 */
  dpd_file2_init(&Z1B, PSIF_EOM_TMP_XI, R_irr, 0, 0, "R1Wmnie_oo");
...
x_xi_check("term 14 (Fme)");
  ...
  if (!params.connect_xi) {
    ...
    dpd_buf4_init(&H2, PSIF_CC_HBAR, 0, 11, 7, 11, 7, 0, "WAMEF");
    ...
    dpd_buf4_init(&XIjAb, PSIF_EOM_XI, G_irr, 0, 5, 0, 5, 0, "XIjAb");
    dpd_file2_init(&I1, PSIF_EOM_TMP, G_irr, 0, 1, "L2R1_OV");
    dpd_buf4_init(&H2, PSIF_CC_HBAR, 0, 11, 5, 11, 5, 0, "WAmEf");
    dpd_contract244(&I1, &H2, &XIjAb, 1, 0, 0, 1.0, 1.0);
    dpd_buf4_close(&H2);
    dpd_file2_close(&I1);
    dpd_buf4_close(&XIjAb);

    dpd_buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "Z (jI,bA)");
    dpd_file2_init(&I1, PSIF_EOM_TMP, G_irr, 0, 1, "L2R1_ov");
    dpd_buf4_init(&H2, PSIF_CC_HBAR, 0, 11, 5, 11, 5, 0, "WaMeF");
    dpd_contract244(&I1, &H2, &Z2, 1, 0, 0, 1.0, 0.0);
    dpd_buf4_close(&H2);
    dpd_file2_close(&I1);
    dpd_buf4_sort_axpy(&Z2, PSIF_EOM_XI, qpsr, 0, 5, "XIjAb", 1.0);
    dpd_buf4_close(&Z2);
#ifdef DEBUG_XI
x_xi_check("term 22 (Wamef)");
  ...
  if (!params.connect_xi) {
    dpd_buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 2, 5, 2, 5, 0, "Z (I>J,AB)");
    dpd_file2_init(&I1, PSIF_EOM_TMP, G_irr, 0, 1, "L2R1_OV");
    dpd_buf4_init(&H2, PSIF_CC_HBAR, 0, 2, 10, 2, 10, 0, "WMNIE");
    ...
    dpd_buf4_init(&XIjAb, PSIF_EOM_XI, G_irr, 0, 5, 0, 5, 0, "XIjAb");
    dpd_file2_init(&I1, PSIF_EOM_TMP, G_irr, 0, 1, "L2R1_OV");
    dpd_buf4_init(&H2, PSIF_CC_HBAR, 0, 0, 10, 0, 10, 0, "WMnIe");
    dpd_contract244(&I1, &H2, &XIjAb, 0, 2, 1, -1.0, 1.0);
    dpd_buf4_close(&H2);
    dpd_file2_close(&I1);
    dpd_buf4_close(&XIjAb);

    dpd_buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "Z (jI,Ab)");
    dpd_file2_init(&I1, PSIF_EOM_TMP, G_irr, 0, 1, "L2R1_ov");
    dpd_buf4_init(&H2, PSIF_CC_HBAR, 0, 0, 11, 0, 11, 0, "WmNiE (mN,Ei)");
    dpd_contract424(&H2, &I1, &Z2, 3, 0, 0, 1.0, 0.0);
    dpd_buf4_close(&H2);
    dpd_file2_close(&I1);
    dpd_buf4_sort_axpy(&Z2, PSIF_EOM_XI, qprs, 0, 5, "XIjAb", -1.0);
    dpd_buf4_close(&Z2);
#ifdef DEBUG_XI
x_xi_check("term 23 (Wmnie)");
  ...
  dpd_buf4_init(&XIJAB, PSIF_EOM_XI, G_irr, 2, 7, 2, 7, 0, "XIJAB");
  dpd_buf4_init(&I2, PSIF_EOM_TMP, G_irr, 7, 10, 7, 10, 0, "L2R1_VVOV");
  dpd_buf4_init(&H2, PSIF_CC_HBAR, 0, 2, 10, 2, 10, 0, "WMNIE");
  ...
  dpd_buf4_init(&XIjAb, PSIF_EOM_XI, G_irr, 0, 5, 0, 5, 0, "XIjAb");
  dpd_buf4_init(&I2, PSIF_EOM_TMP, G_irr, 5, 10, 5, 10, 0, "L2R1_VvOv");
  dpd_buf4_init(&H2, PSIF_CC_HBAR, 0, 0, 10, 0, 10, 0, "WMnIe");
  dpd_contract444(&H2, &I2, &XIjAb, 0, 0, 1.0, 1.0); 
  dpd_buf4_close(&H2);
  dpd_buf4_close(&I2);
...
x_xi_check("term 25 (Wmnie)");
  ...
  dpd_buf4_init(&XIJAB, PSIF_EOM_XI, G_irr, 2, 7, 2, 7, 0, "XIJAB");
  dpd_buf4_init(&I2, PSIF_EOM_TMP, G_irr, 2, 11, 2, 11, 0, "L2R1_OOVO");
  dpd_buf4_init(&H2, PSIF_CC_HBAR, 0, 11, 7, 11, 7, 0, "WAMEF");
  ...
  dpd_buf4_init(&XIjAb, PSIF_EOM_XI, G_irr, 0, 5, 0, 5, 0, "XIjAb");
  dpd_buf4_init(&I2, PSIF_EOM_TMP, G_irr, 0, 11, 0, 11, 0, "L2R1_OoVo");
  dpd_buf4_init(&H2, PSIF_CC_HBAR, 0, 11, 5, 11, 5, 0, "WAmEf");
  dpd_contract444(&I2, &H2, &XIjAb, 0, 1, -1.0, 1.0); 
  dpd_buf4_close(&H2);
  dpd_buf4_close(&I2);
  ...
  dpd_buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "Z (Ij,bA)");
  dpd_buf4_init(&I2, PSIF_EOM_TMP, G_irr, 0, 11, 0, 11, 0, "L2R1_OovO");
  dpd_buf4_init(&H2, PSIF_CC_HBAR, 0, 11, 5, 11, 5, 0, "WaMeF");
  dpd_contract444(&I2, &H2, &Z2, 0, 1, 1.0, 0.0); 
  dpd_buf4_close(&H2);
  dpd_buf4_close(&I2);
  dpd_buf4_sort_axpy(&Z2, PSIF_EOM_XI, pqsr, 0, 5, "XIjAb", -1.0);
  dpd_buf4_close(&Z2);
#ifdef DEBUG_XI
x_xi_check("term 24 (Wamef)");
  ...
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, R_irr, 11, 11, 11, 11, 0, "Z (EJ,BN)");
  dpd_buf4_init(&H2, PSIF_CC_HBAR, 0, 0, 11, 2, 11, 0, "WMNIE (M>N,EI)");
  dpd_file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  dpd_contract244(&R1, &H2, &Z, 0, 0, 0, 1.0, 0.0);
  dpd_buf4_close(&H2);
  dpd_buf4_init(&H2, PSIF_CC_HBAR, 0, 11, 5, 11, 7, 0, "WAMEF");
  dpd_contract424(&H2, &R1, &Z, 3, 1, 0, 1.0, 1.0);
  dpd_buf4_close(&H2);
  dpd_file2_close(&R1);
  dpd_buf4_sort(&Z, PSIF_EOM_TMP1, qpsr, 10, 10, "Z (JE,NB)");
  dpd_buf4_close(&Z);
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, R_irr, 10, 10, 10, 10, 0, "Z (JE,NB)");
  dpd_buf4_sort(&Z, PSIF_EOM_TMP1, psrq, 10, 10, "Z (JB,NE)");
  dpd_buf4_close(&Z);

  dpd_buf4_init(&Z, PSIF_EOM_TMP1, R_irr, 11, 10, 11, 10, 0, "Z (eJ,nB)");
  dpd_buf4_init(&H2, PSIF_CC_HBAR, 0, 0, 10, 0, 10, 0, "WmNiE");
  dpd_file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "Ria");
  dpd_contract244(&R1, &H2, &Z, 0, 0, 0, 1.0, 0.0);
  dpd_buf4_close(&H2);
  ...
  dpd_buf4_close(&H2);
  dpd_file2_close(&R1);
  dpd_buf4_sort(&Z, PSIF_EOM_TMP1, qprs, 10, 10, "Z (Je,nB)");
  dpd_buf4_close(&Z);
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, R_irr, 10, 10, 10, 10, 0, "Z (Je,nB)");
  dpd_buf4_sort(&Z, PSIF_EOM_TMP1, psrq, 10, 10, "Z (JB,ne)");
  dpd_buf4_close(&Z);
  ...
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, R_irr, 10, 11, 10, 11, 0, "Z (Je,bN)");
  dpd_buf4_init(&H2, PSIF_CC_HBAR, 0, 0, 11, 0, 11, 0, "WMnIe (Mn,eI)");
  dpd_file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "Ria");
  dpd_contract424(&H2, &R1, &Z, 1, 0, 1, -1.0, 0.0);
  dpd_file2_close(&R1);
  dpd_buf4_close(&H2);
  dpd_buf4_sort(&Z, PSIF_EOM_TMP1, prsq, 10, 10, "Z (Jb,Ne)");
  dpd_buf4_close(&Z);

  dpd_buf4_init(&Z, PSIF_EOM_TMP1, R_irr, 11, 11, 11, 11, 0, "Z2 (eJ,bN)");
  dpd_buf4_init(&H2, PSIF_CC_HBAR, 0, 11, 5, 11, 5, 0, "WaMeF");
  dpd_file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  dpd_contract424(&H2, &R1, &Z, 3, 1, 0, 1.0, 0.0);
  dpd_file2_close(&R1);
  dpd_buf4_close(&H2);
  dpd_buf4_sort_axpy(&Z, PSIF_EOM_TMP1, qrsp, 10, 10, "Z (Jb,Ne)", 1.0);
  dpd_buf4_close(&Z);
  ...
  dpd_buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 10, 10, 10, 10, 0, "Z2 (IA,JB)");
  dpd_buf4_init(&L2, PSIF_CC_GL, L_irr, 10, 10, 10, 10, 0, "LIAJB");
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, R_irr, 10, 10, 10, 10, 0, "Z (JB,NE)");
...
x_xi_check("terms 18, 19 (Wmnie, Wamef)");
  ...
  dpd_buf4_close(&L2);
  dpd_buf4_close(&Z);
  dpd_buf4_sort_axpy(&Z2, PSIF_EOM_XI, prqs, 0, 5, "XIjAb", -1.0);
  dpd_buf4_close(&Z2);

  /* XIjAb += + L(jA,Ne) Z(Ib,Ne) + L(Ib,nE) Z(jA,nE)  */
  dpd_buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 10, 10, 10, 10, 0, "Z2 (Ib,jA)");
  dpd_buf4_init(&L2, PSIF_CC_GL, L_irr, 10, 10, 10, 10, 0, "LjAIb");
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, R_irr, 10, 10, 10, 10, 0, "Z (Jb,Ne)");
  dpd_contract444(&Z, &L2, &Z2, 0, 0, -1.0, 0.0);
  dpd_buf4_close(&Z);
  dpd_buf4_close(&L2);

  dpd_buf4_init(&L2, PSIF_CC_GL, L_irr, 10, 10, 10, 10, 0, "LIbjA");
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, R_irr, 10, 10, 10, 10, 0, "Z (jB,nE)");
  dpd_contract444(&L2, &Z, &Z2, 0, 0, -1.0, 1.0);
  dpd_buf4_close(&Z);
  dpd_buf4_close(&L2);
  dpd_buf4_sort_axpy(&Z2, PSIF_EOM_XI, prsq, 0, 5, "XIjAb", 1.0);
  dpd_buf4_close(&Z2);
#ifdef DEBUG_XI
x_xi_check("terms 18, 19 (Wmnie, Wamef)");
#endif
...
void x_xi2_4_rohf(void)
{
  ...
  dpd_buf4_init(&I2, PSIF_EOM_TMP, G_irr, 10, 10, 10, 10, 0, "R2L2_OVOV");
  ...
  dpd_buf4_init(&I2, PSIF_EOM_TMP1, G_irr, 10, 10, 10, 10, 0, "RL_OVOV");
  for(h=0; h < nirreps; h++) {
  ...
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 10, 10, 10, 0, "RL_OVOV");
...
void x_xi2_14(void)
{
  ...
  dpd_file2_init(&IIA, PSIF_EOM_TMP, G_irr, 0, 1, "L2R1_OV");
  ...
  if (params.ref == 1) {
    ...
    dpd_file2_init(&Iia, PSIF_EOM_TMP, G_irr, 2, 3, "L2R1_ov");
  ...
  if (params.ref != 0) { /* ROHF or UHF for XIJAB */
    ...
    dpd_buf4_close(&XIJAB);
    dpd_buf4_sort_axpy(&Z, PSIF_EOM_XI, qprs, 2, 7, "XIJAB", -1.0);
    dpd_buf4_sort_axpy(&Z, PSIF_EOM_XI, pqsr, 2, 7, "XIJAB", -1.0);
    dpd_buf4_sort_axpy(&Z, PSIF_EOM_XI, qpsr, 2, 7, "XIJAB", 1.0);
    dpd_buf4_close(&Z);
  }
  ...
  if (params.ref != 0) { /* ROHF or UHF for Xijab */
    if (params.ref == 1)
      ...
      dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 15, 10, 15, 0, "Z (ij,ab)");
    for(h=0; h < nirreps; h++) {
      dpd_buf4_mat_irrep_init(&Z, h);
      for(row=0; row < Z.params->rowtot[h]; row++) {
        i = Z.params->roworb[h][row][0];
    ...
    if (params.ref == 1) {
      ...
      dpd_buf4_axpy(&Z, &Xijab, 1.0);
      dpd_buf4_close(&Xijab);
      dpd_buf4_sort_axpy(&Z, PSIF_EOM_XI, qprs, 12, 17, "Xijab", -1.0);
      dpd_buf4_sort_axpy(&Z, PSIF_EOM_XI, pqsr, 12, 17, "Xijab", -1.0);
      dpd_buf4_sort_axpy(&Z, PSIF_EOM_XI, qpsr, 12, 17, "Xijab", 1.0);
      dpd_buf4_close(&Z);
    }
  }
