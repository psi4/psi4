...
x_xi_check("terms 1 and 5");
  ...
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "X (Ij,Ab)");
  dpd_->file2_init(&I1, PSIF_EOM_TMP, G_irr, 1, 1, "LR_VV");
  dpd_->buf4_init(&D2, PSIF_CC_DINTS, 0, 0, 5, 0, 5, 0, "D <ij|ab>");
  dpd_->contract424(&D2, &I1, &Z2, 3, 1, 0, 1.0, 0.0);
  dpd_->buf4_close(&D2);
  dpd_->file2_close(&I1);
  dpd_->buf4_init(&XIjAb, PSIF_EOM_XI, G_irr, 0, 5, 0, 5, 0, "XIjAb");
  dpd_->buf4_axpy(&Z2, &XIjAb, -1.0);
  dpd_->buf4_sort(&Z2, PSIF_EOM_TMP1, qpsr, 0, 5, "X (jI,bA)");
  dpd_->buf4_close(&Z2);
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "X (jI,bA)");
  dpd_->buf4_axpy(&Z2, &XIjAb, -1.0);
  dpd_->buf4_close(&Z2);
...
x_xi_check("term 8");
  ...
  dpd_->file2_copy(&I1, PSIF_EOM_TMP1, "X1 (F,A)");
  dpd_->file2_close(&I1);
  dpd_->file2_init(&I1, PSIF_EOM_TMP1, R_irr, 1, 1, "X1 (F,A)");
      /* for term 20: */
  ...
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "XIjAb");
  dpd_->file2_init(&I1, PSIF_EOM_TMP1, R_irr, 1, 1, "X1 (F,A)");
  dpd_->buf4_init(&L2, PSIF_CC_GL, L_irr, 0, 5, 0, 5, 0, "LIjAb");
...
x_xi_check("terms 11, 17 and 20");
  ...
  dpd_->file2_copy(&I1, PSIF_EOM_TMP1, "X1 (M,I)");
  dpd_->file2_close(&I1);
  dpd_->file2_init(&I1, PSIF_EOM_TMP1, R_irr, 0, 0, "X1 (M,I)");
       /* for term 21 */
...
x_xi_check("term 23 (Wmnie)");
  ...
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, R_irr, 0, 0, 0, 0, 0, "X (Ij,Nm)");
  dpd_->buf4_init(&H2, PSIF_CC_HBAR, 0, 0, 10, 0, 10, 0, "WMnIe");
  dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  dpd_->contract424(&H2, &R1, &Z2, 3, 1, 0, 1.0, 0.0); 
  dpd_->file2_close(&R1);
  dpd_->buf4_close(&H2);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "XIjAb");
  dpd_->buf4_init(&L2, PSIF_CC_GL, L_irr, 0, 5, 0, 5, 0, "LIjAb");
  dpd_->contract444(&Z2, &L2, &Z, 0, 1, 1.0, 0.0);
  dpd_->buf4_close(&L2);
  dpd_->buf4_close(&Z2);
  dpd_->buf4_init(&XIjAb, PSIF_EOM_XI, G_irr, 0, 5, 0, 5, 0, "XIjAb");
  dpd_->buf4_axpy(&Z, &XIjAb, 1.0);
  dpd_->buf4_close(&XIjAb);
  dpd_->buf4_sort_axpy(&Z, PSIF_EOM_XI, qpsr, 0, 5, "XIjAb", 1.0);
  dpd_->buf4_close(&Z);
#ifdef DEBUG_XI
...
x_xi_check("term 24 (Wamef)");
  ...
  dpd_->file2_close(&R1);
  dpd_->buf4_close(&H2);
  dpd_->buf4_sort_axpy(&Z, PSIF_EOM_TMP1, qpsr, 10, 10, "Z (Je,Nb)", 1.0);
  dpd_->buf4_close(&Z);
  ...
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 10, 10, 10, 10, 0, "XIjAb (IA,jb)");
  dpd_->buf4_init(&L2, PSIF_CC_GL, L_irr, 10, 10, 10, 10, 0, "2LIjAb - LIjbA (IA,jb)");
  ...
  dpd_->buf4_sort(&Z2, PSIF_EOM_TMP1, rspq, 10, 10, "XIjAb (jb,IA)");
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 10, 10, 10, 0, "XIjAb (jb,IA)");
  dpd_->buf4_axpy(&Z, &Z2, 1.0);
  dpd_->buf4_close(&Z);
  ...
  dpd_->buf4_sort(&Z2, PSIF_EOM_TMP1, rspq, 10, 10, "Z2 (jA,Ib)");
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 10, 10, 10, 0, "Z2 (jA,Ib)");
  dpd_->buf4_axpy(&Z, &Z2, 1.0);
  dpd_->buf4_close(&Z);
...
x_xi_check("terms 18, 19 (Wmnie, Wamef)");
  ...
  tval = 2.0 * dpd_->file2_dot_self(&XIA);
  dpd_->file2_close(&XIA);
  fprintf(outfile,"XI_IA amplitudes: Norm=%15.10lf, Dot=%15.10lf\n", sqrt(tval), tval );

  dpd_->buf4_init(&XIjAb, PSIF_EOM_XI, G_irr, 0, 5, 0, 5, 0, "XIjAb");
  dpd_->buf4_sort(&XIjAb, PSIF_EOM_TMP1, pqsr, 0, 5, "XIjbA");
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "XIjbA");
  dpd_->file2_init(&XIA, PSIF_EOM_XI, G_irr, 0, 1, "XIA");
...
void x_xi2_4_rhf(void)
{
  ...
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 10, 10, 10, 10, 0, "X2 (IA,jb)");
  ...
  dpd_->buf4_init(&Z, PSIF_EOM_TMP, G_irr, 10, 10, 10, 10, 0, "R2L2_OvOv");
  dpd_->buf4_init(&D, PSIF_CC_DINTS, 0, 10, 10, 10, 10, 0, "D <ij|ab> (ia,jb)");
  dpd_->contract444(&D, &Z, &Z2, 1, 1, 1.0, 1.0);
  dpd_->buf4_close(&D);
  dpd_->buf4_close(&Z);

  dpd_->buf4_sort(&Z2, PSIF_EOM_TMP1, prqs, 0, 5, "XIjAb");
  dpd_->buf4_close(&Z2);
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "XIjAb");
  dpd_->buf4_init(&XIjAb, PSIF_EOM_XI, G_irr, 0, 5, 0, 5, 0, "XIjAb");
  dpd_->buf4_axpy(&Z2, &XIjAb, 1.0);
  ...
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, 0, 10, 10, 10, 10, 0, "X2 (Ib,jA)");
  dpd_->buf4_init(&Z, PSIF_EOM_TMP, G_irr, 10, 10, 10, 10, 0, "R2L2_OvOv");
  ...
  dpd_->buf4_sort(&Z2, PSIF_EOM_TMP1, prsq, 0, 5, "XIjAb");
  dpd_->buf4_close(&Z2);
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "XIjAb");
  dpd_->buf4_init(&XIjAb, PSIF_EOM_XI, G_irr, 0, 5, 0, 5, 0, "XIjAb");
  dpd_->buf4_axpy(&Z2, &XIjAb, 1.0);
