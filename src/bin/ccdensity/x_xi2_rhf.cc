...
x_xi_check("terms 1 and 5");
  ...
  dpd_buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "X (Ij,Ab)");
  ...
  dpd_buf4_sort(&Z2, PSIF_EOM_TMP1, qpsr, 0, 5, "X (jI,bA)");
  dpd_buf4_close(&Z2);
  dpd_buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "X (jI,bA)");
  dpd_buf4_axpy(&Z2, &XIjAb, -1.0);
  dpd_buf4_close(&Z2);
...
x_xi_check("term 8");
  ...
  dpd_file2_copy(&I1, PSIF_EOM_TMP1, "X1 (F,A)");
  dpd_file2_close(&I1);
  dpd_file2_init(&I1, PSIF_EOM_TMP1, R_irr, 1, 1, "X1 (F,A)");
      /* for term 20: */
  ...
  dpd_buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "XIjAb");
  dpd_file2_init(&I1, PSIF_EOM_TMP1, R_irr, 1, 1, "X1 (F,A)");
  dpd_buf4_init(&L2, PSIF_CC_GL, L_irr, 0, 5, 0, 5, 0, "LIjAb");
...
x_xi_check("terms 11, 17 and 20");
  ...
  dpd_file2_copy(&I1, PSIF_EOM_TMP1, "X1 (M,I)");
  dpd_file2_close(&I1);
  dpd_file2_init(&I1, PSIF_EOM_TMP1, R_irr, 0, 0, "X1 (M,I)");
       /* for term 21 */
...
x_xi_check("term 23 (Wmnie)");
  ...
  dpd_buf4_init(&Z2, PSIF_EOM_TMP1, R_irr, 0, 0, 0, 0, 0, "X (Ij,Nm)");
  dpd_buf4_init(&H2, PSIF_CC_HBAR, 0, 0, 10, 0, 10, 0, "WMnIe");
  dpd_file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  dpd_contract424(&H2, &R1, &Z2, 3, 1, 0, 1.0, 0.0); 
  dpd_file2_close(&R1);
  dpd_buf4_close(&H2);
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "XIjAb");
  dpd_buf4_init(&L2, PSIF_CC_GL, L_irr, 0, 5, 0, 5, 0, "LIjAb");
  ...
  dpd_buf4_close(&XIjAb);
  dpd_buf4_sort_axpy(&Z, PSIF_EOM_XI, qpsr, 0, 5, "XIjAb", 1.0);
  dpd_buf4_close(&Z);
#ifdef DEBUG_XI
...
x_xi_check("term 24 (Wamef)");
  ...
  dpd_file2_close(&R1);
  dpd_buf4_close(&H2);
  dpd_buf4_sort_axpy(&Z, PSIF_EOM_TMP1, qpsr, 10, 10, "Z (Je,Nb)", 1.0);
  dpd_buf4_close(&Z);
  ...
  dpd_buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 10, 10, 10, 10, 0, "XIjAb (IA,jb)");
  dpd_buf4_init(&L2, PSIF_CC_GL, L_irr, 10, 10, 10, 10, 0, "2LIjAb - LIjbA (IA,jb)");
  ...
  dpd_buf4_sort(&Z2, PSIF_EOM_TMP1, rspq, 10, 10, "XIjAb (jb,IA)");
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 10, 10, 10, 0, "XIjAb (jb,IA)");
  ...
  dpd_buf4_sort(&Z2, PSIF_EOM_TMP1, rspq, 10, 10, "Z2 (jA,Ib)");
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 10, 10, 10, 0, "Z2 (jA,Ib)");
  dpd_buf4_axpy(&Z, &Z2, 1.0);
  dpd_buf4_close(&Z);
...
x_xi_check("terms 18, 19 (Wmnie, Wamef)");
  ...
  dpd_buf4_init(&XIjAb, PSIF_EOM_XI, G_irr, 0, 5, 0, 5, 0, "XIjAb");
  dpd_buf4_sort(&XIjAb, PSIF_EOM_TMP1, pqsr, 0, 5, "XIjbA");
  dpd_buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "XIjbA");
  dpd_file2_init(&XIA, PSIF_EOM_XI, G_irr, 0, 1, "XIA");
...
void x_xi2_4_rhf(void)
{
  ...
  dpd_buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 10, 10, 10, 10, 0, "X2 (IA,jb)");
  ...
  dpd_buf4_init(&Z, PSIF_EOM_TMP, G_irr, 10, 10, 10, 10, 0, "R2L2_OvOv");
  dpd_buf4_init(&D, PSIF_CC_DINTS, 0, 10, 10, 10, 10, 0, "D <ij|ab> (ia,jb)");
  dpd_contract444(&D, &Z, &Z2, 1, 1, 1.0, 1.0);
  dpd_buf4_close(&D);
  dpd_buf4_close(&Z);

  dpd_buf4_sort(&Z2, PSIF_EOM_TMP1, prqs, 0, 5, "XIjAb");
  dpd_buf4_close(&Z2);
  dpd_buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "XIjAb");
  dpd_buf4_init(&XIjAb, PSIF_EOM_XI, G_irr, 0, 5, 0, 5, 0, "XIjAb");
  dpd_buf4_axpy(&Z2, &XIjAb, 1.0);
  ...
  dpd_buf4_init(&Z2, PSIF_EOM_TMP1, 0, 10, 10, 10, 10, 0, "X2 (Ib,jA)");
  dpd_buf4_init(&Z, PSIF_EOM_TMP, G_irr, 10, 10, 10, 10, 0, "R2L2_OvOv");
  ...
  dpd_buf4_sort(&Z2, PSIF_EOM_TMP1, prsq, 0, 5, "XIjAb");
  dpd_buf4_close(&Z2);
  dpd_buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "XIjAb");
  dpd_buf4_init(&XIjAb, PSIF_EOM_XI, G_irr, 0, 5, 0, 5, 0, "XIjAb");
  dpd_buf4_axpy(&Z2, &XIjAb, 1.0);
