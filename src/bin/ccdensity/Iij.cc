...
namespace psi { namespace ccdensity {
    ...
    */
	...
	dpd_->file2_init(&D, PSIF_CC_OEI, 0, 2, 2, rho_params.Dij_lbl);
	dpd_->buf4_init(&Aints, PSIF_CC_AINTS, 0, 10, 10, 10, 10, 1, "A <ij|kl>");
	dpd_->dot24(&D, &Aints, &I, 0, 0, 1.0, 1.0);
	dpd_->dot24(&D, &Aints, &I, 1, 0, 1.0, 1.0);
	dpd_->buf4_close(&Aints);
	dpd_->file2_close(&D);
	...
	dpd_->file2_init(&D, PSIF_CC_OEI, 0, 0, 0, rho_params.DIJ_lbl);
	dpd_->buf4_init(&Aints, PSIF_CC_AINTS, 0, 22, 22, 22, 22, 0, "A <Ij|Kl>");
	dpd_->dot13(&D, &Aints, &I, 0, 0, 1.0, 1.0);
	dpd_->dot13(&D, &Aints, &I, 1, 0, 1.0, 1.0);
	dpd_->buf4_close(&Aints);
	dpd_->file2_close(&D);
	...
	dpd_->buf4_init(&Aints, PSIF_CC_AINTS, 0, 0, 0, 0, 0, 0, "A <ij|kl>");
	dpd_->contract442(&Aints, &G, &I, 0, 0, 2.0, 1.0); 
	dpd_->buf4_close(&Aints);
	dpd_->buf4_close(&G);
	dpd_->file2_close(&I);
      }
	...
	dpd_->buf4_init(&Aints, PSIF_CC_AINTS, 0, 0, 2, 0, 0, 1, "A <ij|kl>");
	...
	dpd_->buf4_init(&Aints, PSIF_CC_AINTS, 0, 0, 0, 0, 0, 0, "A <ij|kl>");
	dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 0, 0, 0, 0, 0, "GIjKl");
	dpd_->contract442(&Aints, &G, &I, 1, 1, 2.0, 1.0);
	dpd_->buf4_close(&G);
	dpd_->buf4_close(&Aints);
	...
	dpd_->buf4_init(&Aints, PSIF_CC_AINTS, 0, 10, 12, 10, 10, 1, "A <ij|kl>");
	...
	dpd_->contract442(&Cints, &G, &I, 0, 0, 2.0, 1.0);
	dpd_->buf4_close(&G);
	dpd_->buf4_close(&Cints);
	...
	dpd_->buf4_init(&Dints, PSIF_CC_DINTS, 0, 0, 5, 0, 5, 0, "D <ij|ab>");
	dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 10, 10, 10, 10, 0, "GIbjA");
	dpd_->buf4_sort(&G, PSIF_CC_TMP0, prsq, 0, 5, "GIbjA (Ij,Ab)");
	dpd_->buf4_close(&G);
	dpd_->buf4_init(&G, PSIF_CC_TMP0, 0, 0, 5, 0, 5, 0, "GIbjA (Ij,Ab)");
	dpd_->contract442(&Dints, &G, &I, 0, 0, -2.0, 1.0);
	dpd_->buf4_close(&G);
	dpd_->buf4_close(&Dints);
	...
	dpd_->buf4_sort(&G, PSIF_CC_TMP0, prsq, 22, 28, "GIbjA (Ij,Ab)");
	dpd_->buf4_close(&G);
	dpd_->buf4_init(&G, PSIF_CC_TMP0, 0, 22, 28, 22, 28, 0, "GIbjA (Ij,Ab)");
	dpd_->contract442(&Dints, &G, &I, 0, 0, -2.0, 1.0);
	dpd_->buf4_close(&G);
	dpd_->buf4_close(&Dints);
	...
	dpd_->buf4_sort(&G, PSIF_CC_TMP0, prsq, 23, 29, "GiBJa (iJ,aB)");
	dpd_->buf4_close(&G);
	dpd_->buf4_init(&G, PSIF_CC_TMP0, 0, 23, 29, 23, 29, 0, "GiBJa (iJ,aB)");
	dpd_->contract442(&Dints, &G, &I, 0, 0, -2.0, 1.0);
	dpd_->buf4_close(&G);
	dpd_->buf4_close(&Dints);
