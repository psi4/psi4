...
void deanti_UHF(struct RHO_Params rho_params)
{
  ...
  /* G(ij,kl) = 1/2 G(ij,kl) */
  dpd_buf4_init(&G, PSIF_CC_GAMMA, 0, 10, 10, 12, 12, 0, "Gijkl");
  dpd_buf4_scm(&G, 0.5);
  dpd_buf4_init(&A, PSIF_CC_AINTS, 0, 10, 10, 10, 10, 0, "A <ij|kl>");
  two_energy = dpd_buf4_dot(&A, &G);
  dpd_buf4_close(&A);
  dpd_buf4_close(&G);
  fprintf(outfile, "\tijkl energy                = %20.15f\n", two_energy);
  ...
  dpd_buf4_init(&E, PSIF_CC_EINTS, 0, 0, 20, 0, 20, 0, "E <IJ|KA>");
  two_energy = 2*dpd_buf4_dot(&E, &G);
  dpd_buf4_close(&E);
  dpd_buf4_close(&G);
  ...
  dpd_buf4_init(&E, PSIF_CC_EINTS, 0, 10, 30, 10, 30, 0, "E <ij|ka>");
  two_energy = 2*dpd_buf4_dot(&E, &G);
  dpd_buf4_close(&E);
  dpd_buf4_close(&G);
  fprintf(outfile, "\tijka energy                = %20.15f\n", two_energy);
  ...
  dpd_buf4_init(&G2, PSIF_CC_GAMMA, 0, 24, 27, 24, 27, 0, "GIbjA");
  dpd_buf4_sort(&G2, PSIF_CC_TMP0, prsq, 22, 28, "GIbjA (Ij,Ab)");
  dpd_buf4_close(&G2);
  dpd_buf4_init(&G2, PSIF_CC_TMP0, 0, 22, 28, 22, 28, 0, "GIbjA (Ij,Ab)");
  ...
  dpd_buf4_copy(&G, PSIF_CC_GAMMA, "G(IJ,AB)");
  dpd_buf4_close(&G);
  dpd_buf4_init(&G, PSIF_CC_GAMMA, 0, 0, 5, 0, 5, 0, "G(IJ,AB)");
  dpd_buf4_init(&G2, PSIF_CC_GAMMA, 0, 20, 20, 20, 20, 0, "GIBJA");
  ...
  dpd_buf4_copy(&G, PSIF_CC_GAMMA, "G(ij,ab)");
  dpd_buf4_close(&G);
  dpd_buf4_init(&G, PSIF_CC_GAMMA, 0, 10, 15, 10, 15, 0, "G(ij,ab)");
  dpd_buf4_init(&G2, PSIF_CC_GAMMA, 0, 30, 30, 30, 30, 0, "Gibja");
  dpd_buf4_sort(&G2, PSIF_CC_TMP0, prsq, 10, 15, "Gibja (ij,ab)");
  dpd_buf4_close(&G2);
  dpd_buf4_init(&G2, PSIF_CC_TMP0, 0, 10, 15, 10, 15, 0, "Gibja (ij,ab)");
  dpd_buf4_axpy(&G2, &G, -1.0);
  dpd_buf4_close(&G2);
  ...
  dpd_buf4_init(&F, PSIF_CC_FINTS, 0, 21, 5, 21, 5, 0, "F <AI|BC>");
  two_energy = 2*dpd_buf4_dot(&F, &G);
  dpd_buf4_close(&F);
  dpd_buf4_close(&G); 
  ...
  dpd_buf4_init(&F, PSIF_CC_FINTS, 0, 31, 15, 31, 15, 0, "F <ai|bc>");
  two_energy = 2*dpd_buf4_dot(&F, &G);
  dpd_buf4_close(&F);
  dpd_buf4_close(&G);
  fprintf(outfile, "\tciab energy                = %20.15f\n", two_energy);
  ...
  /* G(AB,CD) = 1/2 G(AB,CD) */
  dpd_buf4_init(&G, PSIF_CC_GAMMA, 0, 5, 5, 7, 7, 0, "GABCD");
  dpd_buf4_scm(&G, 0.5);
  ...
  /* G(ab,cd) = 1/2 G(ab,cd) */
  dpd_buf4_init(&G, PSIF_CC_GAMMA, 0, 15, 15, 17, 17, 0, "Gabcd");
  dpd_buf4_scm(&G, 0.5);
  dpd_buf4_init(&B, PSIF_CC_BINTS, 0, 15, 15, 15, 15, 0, "B <ab|cd>");
  two_energy = dpd_buf4_dot(&B, &G);
  dpd_buf4_close(&B);
  dpd_buf4_close(&G);
  fprintf(outfile, "\tabcd energy                = %20.15f\n", two_energy);
