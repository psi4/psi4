...
void deanti_UHF(struct RHO_Params rho_params)
{
  ...
  dpd_->file2_init(&h, PSIF_CC_OEI, 0, 0, 0, "h(I,J)");
  one_energy += dpd_->file2_dot(&d, &h);
  dpd_->file2_close(&h);
  dpd_->file2_close(&d);

  dpd_->file2_init(&d, PSIF_CC_OEI, 0, 2, 2, rho_params.Dij_lbl);
  dpd_->file2_init(&h, PSIF_CC_OEI, 0, 2, 2, "h(i,j)");
  one_energy += dpd_->file2_dot(&d, &h);
  dpd_->file2_close(&h);
  dpd_->file2_close(&d);

  dpd_->file2_init(&d, PSIF_CC_OEI, 0, 1, 1, rho_params.DAB_lbl);
  dpd_->file2_init(&h, PSIF_CC_OEI, 0, 1, 1, "h(A,B)");
  one_energy += dpd_->file2_dot(&d, &h);
  dpd_->file2_close(&h);
  dpd_->file2_close(&d);

  dpd_->file2_init(&d, PSIF_CC_OEI, 0, 3, 3, rho_params.Dab_lbl);
  dpd_->file2_init(&h, PSIF_CC_OEI, 0, 3, 3, "h(a,b)");
  one_energy += dpd_->file2_dot(&d, &h);
  dpd_->file2_close(&h);
  dpd_->file2_close(&d);

  dpd_->file2_init(&d, PSIF_CC_OEI, 0, 0, 1, rho_params.DIA_lbl);
  dpd_->file2_init(&h, PSIF_CC_OEI, 0, 0, 1, "h(I,A)");
  one_energy += dpd_->file2_dot(&d, &h);
  dpd_->file2_close(&h);
  dpd_->file2_close(&d);

  dpd_->file2_init(&d, PSIF_CC_OEI, 0, 2, 3, rho_params.Dia_lbl);
  dpd_->file2_init(&h, PSIF_CC_OEI, 0, 2, 3, "h(i,a)");
  one_energy += dpd_->file2_dot(&d, &h);
  dpd_->file2_close(&h);
  dpd_->file2_close(&d);
  ...
  two_energy = 2*dpd_->buf4_dot(&A, &G);
  dpd_->buf4_close(&A);
  dpd_->buf4_close(&G);
  ...
  /* G(ij,kl) = 1/2 G(ij,kl) */
  dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 10, 10, 12, 12, 0, "Gijkl");
  dpd_->buf4_scm(&G, 0.5);
  dpd_->buf4_init(&A, PSIF_CC_AINTS, 0, 10, 10, 10, 10, 0, "A <ij|kl>");
  two_energy = dpd_->buf4_dot(&A, &G);
  dpd_->buf4_close(&A);
  dpd_->buf4_close(&G);
  fprintf(outfile, "\tijkl energy                = %20.15f\n", two_energy);
  ...
  dpd_->buf4_init(&E, PSIF_CC_EINTS, 0, 0, 20, 0, 20, 0, "E <IJ|KA>");
  two_energy = 2*dpd_->buf4_dot(&E, &G);
  dpd_->buf4_close(&E);
  dpd_->buf4_close(&G);
  ...
  dpd_->buf4_init(&E, PSIF_CC_EINTS, 0, 10, 30, 10, 30, 0, "E <ij|ka>");
  two_energy = 2*dpd_->buf4_dot(&E, &G);
  dpd_->buf4_close(&E);
  dpd_->buf4_close(&G);
  fprintf(outfile, "\tijka energy                = %20.15f\n", two_energy);
  ...
  dpd_->buf4_init(&G2, PSIF_CC_GAMMA, 0, 24, 27, 24, 27, 0, "GIbjA");
  dpd_->buf4_sort(&G2, PSIF_CC_TMP0, prsq, 22, 28, "GIbjA (Ij,Ab)");
  dpd_->buf4_close(&G2);
  dpd_->buf4_init(&G2, PSIF_CC_TMP0, 0, 22, 28, 22, 28, 0, "GIbjA (Ij,Ab)");
  dpd_->buf4_axpy(&G2, &G, -1.0);
  dpd_->buf4_close(&G2);
  dpd_->buf4_init(&D, PSIF_CC_DINTS, 0, 22, 28, 22, 28, 0, "D <Ij|Ab>");
  two_energy = 2 * dpd_->buf4_dot(&D, &G);
  dpd_->buf4_close(&D);
  dpd_->buf4_close(&G);
  ...
  dpd_->buf4_copy(&G, PSIF_CC_GAMMA, "G(IJ,AB)");
  dpd_->buf4_close(&G);
  dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 0, 5, 0, 5, 0, "G(IJ,AB)");
  dpd_->buf4_init(&G2, PSIF_CC_GAMMA, 0, 20, 20, 20, 20, 0, "GIBJA");
  ...
  dpd_->buf4_copy(&G, PSIF_CC_GAMMA, "G(ij,ab)");
  dpd_->buf4_close(&G);
  dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 10, 15, 10, 15, 0, "G(ij,ab)");
  dpd_->buf4_init(&G2, PSIF_CC_GAMMA, 0, 30, 30, 30, 30, 0, "Gibja");
  dpd_->buf4_sort(&G2, PSIF_CC_TMP0, prsq, 10, 15, "Gibja (ij,ab)");
  dpd_->buf4_close(&G2);
  dpd_->buf4_init(&G2, PSIF_CC_TMP0, 0, 10, 15, 10, 15, 0, "Gibja (ij,ab)");
  dpd_->buf4_axpy(&G2, &G, -1.0);
  dpd_->buf4_close(&G2);
  ...
  two_energy = 2*dpd_->buf4_dot(&F, &G);
  dpd_->buf4_close(&F);
  dpd_->buf4_close(&G);
  ...
  dpd_->buf4_init(&F, PSIF_CC_FINTS, 0, 21, 5, 21, 5, 0, "F <AI|BC>");
  two_energy = 2*dpd_->buf4_dot(&F, &G);
  dpd_->buf4_close(&F);
  dpd_->buf4_close(&G); 
  ...
  dpd_->buf4_init(&F, PSIF_CC_FINTS, 0, 31, 15, 31, 15, 0, "F <ai|bc>");
  two_energy = 2*dpd_->buf4_dot(&F, &G);
  dpd_->buf4_close(&F);
  dpd_->buf4_close(&G);
  fprintf(outfile, "\tciab energy                = %20.15f\n", two_energy);
  ...
  two_energy = 2*dpd_->buf4_dot(&B, &G);
  dpd_->buf4_close(&B);
  dpd_->buf4_close(&G);
  ...
  /* G(AB,CD) = 1/2 G(AB,CD) */
  dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 5, 5, 7, 7, 0, "GABCD");
  dpd_->buf4_scm(&G, 0.5);
  dpd_->buf4_init(&B, PSIF_CC_BINTS, 0, 5, 5, 5, 5, 0, "B <AB|CD>");
  two_energy = dpd_->buf4_dot(&B, &G);
  dpd_->buf4_close(&B);
  dpd_->buf4_close(&G);
  fprintf(outfile, "\tABCD energy                = %20.15f\n", two_energy);
  total_two_energy += two_energy;
  ...
  /* G(ab,cd) = 1/2 G(ab,cd) */
  dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 15, 15, 17, 17, 0, "Gabcd");
  dpd_->buf4_scm(&G, 0.5);
  dpd_->buf4_init(&B, PSIF_CC_BINTS, 0, 15, 15, 15, 15, 0, "B <ab|cd>");
  two_energy = dpd_->buf4_dot(&B, &G);
  dpd_->buf4_close(&B);
  dpd_->buf4_close(&G);
  fprintf(outfile, "\tabcd energy                = %20.15f\n", two_energy);
