...
void x_Gijab_uhf(void)
{
  ...
  if (G_irr == 0) {
    ...
    dpd_->buf4_scm(&G,0.0);
    dpd_->buf4_close(&G);
    dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 12, 17, 12, 17, 0, "Gijab");
    dpd_->buf4_scm(&G,0.0);
    dpd_->buf4_close(&G);
    dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 22, 28, 22, 28, 0, "GIjAb");
    dpd_->buf4_scm(&G,0.0);
    dpd_->buf4_close(&G);
  }
  ...
  /* -P(ij) LR_oo(m,i) Tau(mj,ab); terms 4,5,8,9 */
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 17, 10, 17, 0, "Z(ij,a>b)");
  dpd_->file2_init(&I1, PSIF_EOM_TMP, G_irr, 2, 2, "LR_oo");
  dpd_->file2_init(&Z1, PSIF_EOM_TMP1, G_irr, 2, 2, "Z(n,i)");
  dpd_->file2_axpy(&I1, &Z1, 1.0, 0);
  dpd_->file2_close(&I1);
  /* -P(ij) L2R1_ov(m,f) T(i,f) Tau(mj,ab); terms 35, 36 */
  if (!params.connect_xi) {
  ...
  dpd_->file2_close(&Z1);
  dpd_->buf4_close(&T);
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 10, 17, 12, 17, 0, "Gijab");
  dpd_->buf4_axpy(&Z, &G, -1.0);
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP1, qprs, 10, 17, "Z(ji,a>b)");
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 17, 10, 17, 0, "Z(ji,a>b)");
  dpd_->buf4_axpy(&Z, &G, 1.0);
  dpd_->buf4_close(&Z);
  dpd_->buf4_close(&G);
  ...
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 23, 29, 23, 29, 0, "Z(jI,bA)");
  dpd_->file2_init(&I1, PSIF_EOM_TMP, G_irr, 2, 2, "LR_oo");
  dpd_->file2_init(&Z1, PSIF_EOM_TMP1, G_irr, 2, 2, "Z(n,i)");
  dpd_->file2_axpy(&I1, &Z1, 1.0, 0);
  dpd_->file2_close(&I1);
  ...
  dpd_->file2_close(&Z1);
  dpd_->buf4_close(&T);
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP1, qpsr, 22, 28, "Z(Ij,Ab)");
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 22, 28, 22, 28, 0, "Z(Ij,Ab)");
  dpd_->buf4_axpy(&Z, &G, -1.0);
  dpd_->buf4_close(&G);
  ...
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 12, 15, 12, 15, 0, "Z(i>j,ab)");
  dpd_->buf4_init(&T, PSIF_CC_TAMPS, 0, 12, 15, 12, 17, 0, "tauijab");
  dpd_->file2_init(&I1, PSIF_EOM_TMP, G_irr, 3, 3, "LR_vv");
  dpd_->contract244(&I1, &T, &Z, 0, 2, 1, 1.0, 0.0);
  dpd_->file2_close(&I1);
  dpd_->buf4_close(&T);
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 12, 15, 12, 17, 0, "Gijab");
  dpd_->buf4_axpy(&Z, &G, -1.0);
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP1, pqsr, 12, 15, "Z(i>j,ba)");
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 12, 15, 12, 15, 0, "Z(i>j,ba)");
  dpd_->buf4_axpy(&Z, &G, 1.0);
  dpd_->buf4_close(&Z);
  dpd_->buf4_close(&G);
  ...
  dpd_->file2_close(&I1);
  dpd_->buf4_close(&T);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 22, 29, 22, 29, 0, "Z(Ij,bA)");
  dpd_->buf4_init(&T, PSIF_CC_TAMPS, 0, 22, 29, 22, 29, 0, "tauIjbA");
  dpd_->file2_init(&I1, PSIF_EOM_TMP, G_irr, 3, 3, "LR_vv");
  dpd_->contract244(&I1, &T, &Z, 0, 2, 1, 1.0, 0.0);
  dpd_->file2_close(&I1);
  dpd_->buf4_close(&T);
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP1, pqsr, 22, 28, "Z(Ij,Ab)");
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 22, 28, 22, 28, 0, "Z(Ij,Ab)");
  dpd_->buf4_axpy(&Z, &G, -1.0);
  dpd_->buf4_close(&G);
  ...
  dpd_->buf4_close(&G);
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 12, 17, 12, 17, 0, "Gijab");
  dpd_->buf4_init(&I, PSIF_EOM_TMP, L_irr, 12, 12, 12, 12, 0, "Tau2L2_oooo");
  ...
  dpd_->buf4_close(&G);
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 22, 28, 22, 28, 0, "GIjAb");
  dpd_->buf4_init(&I, PSIF_EOM_TMP, L_irr, 22, 22, 22, 22, 0, "Tau2L2_OoOo");
  ...
  dpd_->buf4_init(&I, PSIF_EOM_TMP, G_irr, 2, 20, 2, 20, 0, "L2R1_OOVO(pqsr)");
  dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tIA");
  ...
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 12, 10, 12, 0, "Z(ij,m>n)");
  dpd_->buf4_init(&I, PSIF_EOM_TMP, G_irr, 12, 30, 12, 30, 0, "L2R1_oovo(pqsr)");
  dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 2, 3, "tia");
  dpd_->contract424(&I, &T1, &Z, 3, 1, 1, 1.0, 0.0);
  dpd_->file2_close(&T1);
  dpd_->buf4_close(&I);
  /* add terms 12, 14 */
  dpd_->buf4_init(&I, PSIF_EOM_TMP, G_irr, 10, 12, 12, 12, 0, "R2L2_oooo");
  dpd_->buf4_axpy(&I, &Z, -0.5);
  dpd_->buf4_close(&I);
  ...
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 10, 17, 12, 17, 0, "Gijab");
  dpd_->buf4_axpy(&Z2, &G, -1.0);
  dpd_->buf4_sort(&Z2, PSIF_EOM_TMP1, qprs, 10, 17, "Z(ji,a>b)");
  dpd_->buf4_close(&Z2);
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 10, 17, 10, 17, 0, "Z(ji,a>b)");
  dpd_->buf4_axpy(&Z2, &G, 1.0);
  dpd_->buf4_close(&Z2);
  dpd_->buf4_close(&G);

  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 22, 22, 22, 22, 0, "Z(Ij,Mn)");
  dpd_->buf4_init(&I, PSIF_EOM_TMP, G_irr, 22, 24, 22, 24, 0, "L2R1_OovO(pqsr)");
  dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 2, 3, "tia");
  dpd_->contract424(&I, &T1, &Z, 3, 1, 1, 1.0, 0.0);
  dpd_->file2_close(&T1);
  dpd_->buf4_close(&I);
  dpd_->buf4_init(&I, PSIF_EOM_TMP, G_irr, 22, 26, 22, 26, 0, "L2R1_OoVo");
  dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tIA");
  dpd_->contract244(&T1, &I, &Z, 1, 2, 0, 1.0, 1.0);
  dpd_->file2_close(&T1);
  dpd_->buf4_close(&I);
  /* add terms 12, 14 */
  dpd_->buf4_init(&I, PSIF_EOM_TMP, G_irr, 22, 22, 22, 22, 0, "R2L2_OoOo");
  ...
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 2, 21, 2, 21, 0, "Z(I>J,AN)");
  ...
  dpd_->buf4_close(&G);
  /* + 0.5 P(ab) (tau_ijef Lmnef) Rma Tnb ; terms 25, 27 */
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 12, 31, 12, 31, 0, "Z(i>j,an)");
  dpd_->buf4_init(&I, PSIF_EOM_TMP, L_irr, 12, 10, 12, 12, 0, "Tau2L2_oooo");
  ...
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 12, 15, 12, 17, 0, "Gijab");
  dpd_->buf4_axpy(&Z2, &G, 1.0);
  dpd_->buf4_sort(&Z2, PSIF_EOM_TMP1, pqsr, 12, 15, "Z(i>j,ba)");
  dpd_->buf4_close(&Z2);
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 12, 15, 12, 15, 0, "Z(i>j,ba)");
  dpd_->buf4_axpy(&Z2, &G, -1.0);
  dpd_->buf4_close(&Z2);
  dpd_->buf4_close(&G);
  /* + tau_IjEf LMnEf RMA Tnb ; terms 25, 27 */
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 22, 26, 22, 26, 0, "Z(Ij,An)");
  dpd_->buf4_init(&I, PSIF_EOM_TMP, L_irr, 22, 22, 22, 22, 0, "Tau2L2_OoOo");
  dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  dpd_->contract244(&R1, &I, &Z, 0, 2, 1, 1.0, 0.0);
  dpd_->file2_close(&R1);
  ...
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 22, 26, 22, 26, 0, "Z2(Ij,An)");
  dpd_->buf4_init(&I, PSIF_EOM_TMP, L_irr, 22, 22, 22, 22, 0, "Tau2L2_OoOo");
  dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tIA");
  ...
  dpd_->file2_init(&I1, PSIF_EOM_TMP, L_irr, 2, 2, "LT_oo");
  dpd_->contract244(&I1, &R, &Z, 0, 0, 0, 1.0, 0.0);
  dpd_->file2_close(&I1);
  dpd_->buf4_close(&R);
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 10, 17, 12, 17, 0, "Gijab");
  ...
  dpd_->file2_close(&I1);
  dpd_->buf4_close(&R);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 23, 28, 23, 28, 0, "Z(jI,Ab)");
  dpd_->buf4_init(&R, PSIF_CC_GR, R_irr, 23, 28, 23, 28, 0, "RiJAb");
  dpd_->file2_init(&I1, PSIF_EOM_TMP, L_irr, 2, 2, "LT_oo");
  dpd_->contract244(&I1, &R, &Z, 0, 0, 0, 1.0, 0.0);
  dpd_->file2_close(&I1);
  dpd_->buf4_close(&R);
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP1, qprs, 22, 28, "Z(Ij,Ab)");
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 22, 28, 22, 28, 0, "Z(Ij,Ab)");
  dpd_->buf4_axpy(&Z, &G, -1.0);
  dpd_->buf4_close(&Z);
  ...
  dpd_->file2_init(&I1, PSIF_EOM_TMP, L_irr, 3, 3, "LT_vv");
  dpd_->contract424(&R, &I1, &Z, 3, 0, 0, 1.0, 0.0);
  dpd_->file2_close(&I1);
  dpd_->buf4_close(&R);
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 12, 15, 12, 17, 0, "Gijab");
  ...
  if (!params.connect_xi) {
    ...
    dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 12, 31, 12, 31, 0, "Z(i>j,an)");
    dpd_->buf4_init(&T, PSIF_CC_TAMPS, 0, 12, 15, 12, 17, 0, "tijab");
    dpd_->file2_init(&I1, PSIF_EOM_TMP, G_irr, 2, 3, "L2R1_ov");
    dpd_->contract424(&T, &I1, &Z, 3, 1, 0, 1.0, 0.0);
    dpd_->file2_close(&I1);
    dpd_->buf4_close(&T);
    ...
    dpd_->buf4_close(&Z);
    dpd_->buf4_close(&G);
    dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 22, 24, 22, 24, 0, "Z(Ij,Nb)");
    dpd_->buf4_init(&T, PSIF_CC_TAMPS, 0, 22, 28, 22, 28, 0, "tIjAb");
    dpd_->file2_init(&I1, PSIF_EOM_TMP, G_irr, 0, 1, "L2R1_OV");
  ...
  dpd_->buf4_init(&Z, PSIF_EOM_TMP0, G_irr, 20, 20, 20, 20, 0, "Z(IA,JB)");
  dpd_->buf4_init(&I, PSIF_EOM_TMP, G_irr, 20, 20, 20, 20, 0, "R2L2_OVOV");
  dpd_->buf4_init(&T, PSIF_CC_TAMPS, 0, 20, 20, 20, 20, 0, "tIAJB");
  dpd_->contract444(&I, &T, &Z, 0, 1, 1.0, 0.0);
  dpd_->buf4_close(&I);
  dpd_->buf4_close(&T);
  dpd_->buf4_init(&I, PSIF_EOM_TMP, G_irr, 20, 30, 20, 30, 0, "R2L2_OVov");
  dpd_->buf4_init(&T, PSIF_CC_TAMPS, 0, 30, 20, 30, 20, 0, "tiaJB");
  dpd_->contract444(&I, &T, &Z, 0, 1, 1.0, 1.0);
  dpd_->buf4_close(&I);
  dpd_->buf4_close(&T);

  dpd_->buf4_init(&Z, PSIF_EOM_TMP0, G_irr, 30, 30, 30, 30, 0, "Z(ia,jb)");
  dpd_->buf4_init(&I, PSIF_EOM_TMP, G_irr, 30, 30, 30, 30, 0, "R2L2_ovov");
  dpd_->buf4_init(&T, PSIF_CC_TAMPS, 0, 30, 30, 30, 30, 0, "tiajb");
  dpd_->contract444(&I, &T, &Z, 0, 1, 1.0, 0.0);
  dpd_->buf4_close(&I);
  dpd_->buf4_close(&T);
  dpd_->buf4_init(&I, PSIF_EOM_TMP, G_irr, 30, 20, 30, 20, 0, "R2L2_ovOV");
  dpd_->buf4_init(&T, PSIF_CC_TAMPS, 0, 20, 30, 20, 30, 0, "tIAjb");
  dpd_->contract444(&I, &T, &Z, 0, 1, 1.0, 1.0);
  dpd_->buf4_close(&I);
  dpd_->buf4_close(&T);

  dpd_->buf4_init(&Z, PSIF_EOM_TMP0, G_irr, 20, 30, 20, 30, 0, "Z(IA,jb)");
  dpd_->buf4_init(&I, PSIF_EOM_TMP, G_irr, 20, 20, 20, 20, 0, "R2L2_OVOV");
  dpd_->buf4_init(&T, PSIF_CC_TAMPS, 0, 20, 30, 20, 30, 0, "tIAjb");
  dpd_->contract444(&I, &T, &Z, 0, 1, 1.0, 0.0);
  dpd_->buf4_close(&I);
  ...
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 24, 27, 24, 27, 0, "Z(Ib,jA)");
  dpd_->buf4_init(&I, PSIF_EOM_TMP, G_irr, 27, 27, 27, 27, 0, "R2L2_oVoV");
  dpd_->buf4_init(&T, PSIF_CC_TAMPS, 0, 24, 27, 24, 27, 0, "tIbjA");
  dpd_->contract444(&T, &I, &Z, 0, 0, 1.0, 0.0);
  dpd_->buf4_close(&I);
  dpd_->buf4_close(&T);
  dpd_->buf4_init(&I, PSIF_EOM_TMP, G_irr, 24, 24, 24, 24, 0, "R2L2_OvOv");
  dpd_->buf4_init(&T, PSIF_CC_TAMPS, 0, 24, 27, 24, 27, 0, "tIbjA");
  dpd_->contract444(&I, &T, &Z, 0, 1, 1.0, 1.0);
  dpd_->buf4_close(&I);
  dpd_->buf4_close(&T);
  dpd_->buf4_sort_axpy(&Z, PSIF_EOM_TMP0, psrq, 20, 30, "Z(IA,jb)", 1.0);
  dpd_->buf4_close(&Z);
  ...
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 20, 0, 20, 0, 0, "Z(JB,NI)");
  dpd_->buf4_init(&I, PSIF_EOM_TMP, L_irr, 20, 20, 20, 20, 0, "VIAJB");
  dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tIA");
  dpd_->contract424(&I, &T1, &Z, 3, 1, 0, 1.0, 0.0);
  dpd_->file2_close(&T1);
  dpd_->buf4_close(&I);
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 21, 20, 21, 20, 0, "Z(AI,JB)");
  dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  dpd_->contract244(&R1, &Z, &Z2, 0, 2, 0, 1.0, 0.0);
  dpd_->file2_close(&R1);
  dpd_->buf4_close(&Z);
  dpd_->buf4_sort_axpy(&Z2, PSIF_EOM_TMP0, qprs, 20, 20, "Z(IA,JB)", -1.0);
  dpd_->buf4_close(&Z2);
  
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 30, 10, 30, 10, 0, "Z(jb,ni)");
  dpd_->buf4_init(&I, PSIF_EOM_TMP, L_irr, 30, 30, 30, 30, 0, "Viajb");
  dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 2, 3, "tia");
  dpd_->contract424(&I, &T1, &Z, 3, 1, 0, 1.0, 0.0);
  dpd_->file2_close(&T1);
  dpd_->buf4_close(&I);
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 31, 30, 31, 30, 0, "Z(ai,jb)");
  dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 2, 3, "Ria");
  dpd_->contract244(&R1, &Z, &Z2, 0, 2, 0, 1.0, 0.0);
  dpd_->file2_close(&R1);
  dpd_->buf4_close(&Z);
  dpd_->buf4_sort_axpy(&Z2, PSIF_EOM_TMP0, qprs, 30, 30, "Z(ia,jb)", -1.0);
  dpd_->buf4_close(&Z2);

  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 30, 0, 30, 0, 0, "Z(jb,NI)");
  dpd_->buf4_init(&I, PSIF_EOM_TMP, L_irr, 30, 20, 30, 20, 0, "ViaJB");
  dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tIA");
  dpd_->contract424(&I, &T1, &Z, 3, 1, 0, 1.0, 0.0);
  dpd_->file2_close(&T1);
  dpd_->buf4_close(&I);
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 21, 30, 21, 30, 0, "Z(AI,jb)");
  dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  dpd_->contract244(&R1, &Z, &Z2, 0, 2, 0, 1.0, 0.0);
  dpd_->file2_close(&R1);
  dpd_->buf4_close(&Z);
  dpd_->buf4_sort_axpy(&Z2, PSIF_EOM_TMP0, qprs, 20, 30, "Z(IA,jb)", -1.0);
  dpd_->buf4_close(&Z2);

  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 24, 22, 24, 22, 0, "Z(Ib,Nj)");
  dpd_->buf4_init(&I, PSIF_EOM_TMP, L_irr, 24, 24, 24, 24, 0, "VIaJb");
  dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 2, 3, "tia");
  dpd_->contract424(&I, &T1, &Z, 3, 1, 0, 1.0, 0.0);
  dpd_->file2_close(&T1);
  dpd_->buf4_close(&I);
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 24, 26, 24, 26, 0, "Z(Ib,Aj)");
  dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  dpd_->contract244(&R1, &Z, &Z2, 0, 2, 1, 1.0, 0.0);
  dpd_->file2_close(&R1);
  dpd_->buf4_close(&Z);
  dpd_->buf4_sort_axpy(&Z2, PSIF_EOM_TMP0, prsq, 20, 30, "Z(IA,jb)", +1.0);
  dpd_->buf4_close(&Z2);

  psio_close(PSIF_EOM_TMP1,0);
  psio_open(PSIF_EOM_TMP1,PSIO_OPEN_NEW);

  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 27, 23, 27, 23, 0, "Z(jA,nI)");
  dpd_->buf4_init(&I, PSIF_EOM_TMP, L_irr, 27, 27, 27, 27, 0, "ViAjB");
  dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tIA");
  dpd_->contract424(&I, &T1, &Z, 3, 1, 0, 1.0, 0.0);
  dpd_->file2_close(&T1);
  dpd_->buf4_close(&I);
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 25, 27, 25, 27, 0, "Z(bI,jA)");
  dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 2, 3, "Ria");
  dpd_->contract244(&R1, &Z, &Z2, 0, 2, 0, 1.0, 0.0);
  dpd_->file2_close(&R1);
  dpd_->buf4_close(&Z);
  dpd_->buf4_sort(&Z2, PSIF_EOM_TMP1, sqrp, 21, 30, "Z(AI,jb)");
  dpd_->buf4_close(&Z2);
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 21, 30, 21, 30, 0, "Z(AI,jb)");
  dpd_->buf4_sort_axpy(&Z2, PSIF_EOM_TMP0, qprs, 20, 30, "Z(IA,jb)", +1.0);
  dpd_->buf4_close(&Z2);

  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 20, 10, 20, 10, 0, "Z(IA,nj)");
  dpd_->buf4_init(&I, PSIF_EOM_TMP, L_irr, 20, 30, 20, 30, 0, "VIAjb");
  dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 2, 3, "tia");
  dpd_->contract424(&I, &T1, &Z, 3, 1, 0, 1.0, 0.0);
  dpd_->file2_close(&T1);
  dpd_->buf4_close(&I);
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 20, 31, 20, 31, 0, "Z(IA,bj)");
  dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 2, 3, "Ria");
  dpd_->contract244(&R1, &Z, &Z2, 0, 2, 1, 1.0, 0.0);
  dpd_->file2_close(&R1);
  dpd_->buf4_close(&Z);
  dpd_->buf4_sort_axpy(&Z2, PSIF_EOM_TMP0, pqsr, 20, 30, "Z(IA,jb)", -1.0);
  dpd_->buf4_close(&Z2);
  ...
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 20, 0, 20, 0, 0, "Z(JB,NI)");
  dpd_->buf4_init(&I, PSIF_EOM_TMP, L_irr, 20, 20, 20, 20, 0, "VIAJB");
  dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  ...
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 30, 10, 30, 10, 0, "Z(jb,ni)");
  dpd_->buf4_init(&I, PSIF_EOM_TMP, L_irr, 30, 30, 30, 30, 0, "Viajb");
  dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 2, 3, "Ria");
  dpd_->contract424(&I, &R1, &Z, 3, 1, 0, 1.0, 0.0);
  dpd_->file2_close(&R1);
  ...
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 30, 0, 30, 0, 0, "Z(jb,NI)");
  dpd_->buf4_init(&I, PSIF_EOM_TMP, L_irr, 30, 20, 30, 20, 0, "ViaJB");
  dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  dpd_->contract424(&I, &R1, &Z, 3, 1, 0, 1.0, 0.0);
  dpd_->file2_close(&R1);
  ...
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 24, 22, 24, 22, 0, "Z(Ib,Nj)");
  dpd_->buf4_init(&I, PSIF_EOM_TMP, L_irr, 24, 24, 24, 24, 0, "VIaJb");
  dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 2, 3, "Ria");
  dpd_->contract424(&I, &R1, &Z, 3, 1, 0, 1.0, 0.0);
  dpd_->file2_close(&R1);
  ...
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 27, 23, 27, 23, 0, "Z(jA,nI)");
  dpd_->buf4_init(&I, PSIF_EOM_TMP, L_irr, 27, 27, 27, 27, 0, "ViAjB");
  dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  dpd_->contract424(&I, &R1, &Z, 3, 1, 0, 1.0, 0.0);
  dpd_->file2_close(&R1);
  ...
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 20, 10, 20, 10, 0, "Z(IA,nj)");
  dpd_->buf4_init(&I, PSIF_EOM_TMP, L_irr, 20, 30, 20, 30, 0, "VIAjb");
  dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 2, 3, "Ria");
  dpd_->contract424(&I, &R1, &Z, 3, 1, 0, 1.0, 0.0);
  dpd_->file2_close(&R1);
  ...
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP0, G_irr, 20, 20, 20, 20, 0, "Z(IA,JB)");
  ...
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP0, G_irr, 30, 30, 30, 30, 0, "Z(ia,jb)");
  dpd_->buf4_sort(&Z2, PSIF_EOM_TMP1, prqs, 10, 15, "Z(ij,ab)");
  dpd_->buf4_close(&Z2);
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 10, 15, 12, 17, 0, "Gijab");
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 15, 10, 15, 0, "Z(ij,ab)");
  dpd_->buf4_axpy(&Z, &G, 1.0);
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP1, qprs, 10, 15, "Z(ji,ab)");
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP1, pqsr, 10, 15, "Z(ij,ba)");
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 15, 10, 15, 0, "Z(ji,ab)");
  dpd_->buf4_axpy(&Z, &G, -1.0);
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 15, 10, 15, 0, "Z(ij,ba)");
  dpd_->buf4_axpy(&Z, &G, -1.0);
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP1, qprs, 10, 15, "Z(ji,ba)");
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 15, 10, 15, 0, "Z(ji,ba)");
  dpd_->buf4_axpy(&Z, &G, 1.0);
  dpd_->buf4_close(&Z);
  dpd_->buf4_close(&G);

  dpd_->buf4_init(&Z, PSIF_EOM_TMP0, G_irr, 20, 30, 20, 30, 0, "Z(IA,jb)");
  dpd_->buf4_sort_axpy(&Z, PSIF_EOM_TMP0, prqs, 22, 28, "GIjAb", 1.0);
  dpd_->buf4_close(&Z);
  ...
  dpd_->buf4_close(&G);
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 12, 17, 12, 17, 0, "Gijab");
  dpd_->buf4_init(&V, PSIF_CC_GAMMA, G_irr, 12, 17, 12, 17, 0, "Gijab");
  dpd_->buf4_axpy(&G, &V, 0.5);
  dpd_->buf4_close(&V);
  dpd_->buf4_close(&G);
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 22, 28, 22, 28, 0, "GIjAb");
  dpd_->buf4_init(&V, PSIF_CC_GAMMA, G_irr, 22, 28, 22, 28, 0, "GIjAb");
  dpd_->buf4_axpy(&G, &V, 0.5);
  dpd_->buf4_close(&V);
  dpd_->buf4_close(&G);
...
void x_Gijab_uhf_2(void) { 
  ...
  dpd_->file2_init(&L1R2B, PSIF_EOM_TMP, G_irr, 2, 3, "L1R2_ov");
  ...
  dpd_->file2_init(&Z1B, PSIF_EOM_TMP1, G_irr, 2, 3, "Zia");
  ...
  dpd_->file2_init(&I1B, PSIF_EOM_TMP, L_irr, 3, 3, "LT2_vv");
  ...
  dpd_->file2_close(&R1A);
  dpd_->file2_close(&I1A);
  dpd_->file2_init(&I1B, PSIF_EOM_TMP, L_irr, 2, 2, "LT2_oo");
  dpd_->file2_init(&R1B, PSIF_CC_GR, R_irr, 2, 3, "Ria");
  ...
  if (!params.connect_xi) {
    ...
    dpd_->buf4_close(&T);
    dpd_->buf4_init(&T, PSIF_CC_TAMPS, 0, 23, 29, 23, 29, 0, "tiJaB");
...
void x_Gijab_uhf_3(void) { 
  ...
  dpd_->file2_init(&L1T2B, PSIF_EOM_TMP, L_irr, 2, 3, "L1T2_ov");
  ...
  dpd_->file2_init(&Z1B, PSIF_EOM_TMP1, L_irr, 2, 3, "Zia");
  ...
  dpd_->file2_close(&T1A);
  dpd_->file2_close(&I1A);
  dpd_->file2_init(&I1B, PSIF_EOM_TMP, L_irr, 3, 3, "LT1_vv");
  dpd_->file2_init(&T1B, PSIF_CC_OEI, 0, 2, 3, "tia");
  dpd_->contract222(&T1B, &I1B, &Z1B, 0, 1, -1.0, 1.0);
  dpd_->file2_close(&T1B);
  dpd_->file2_close(&I1B);
