...
void deanti_ROHF(struct RHO_Params rho_params)
{
  ...
  dpd_->buf4_init(&G1, PSIF_CC_GAMMA, 0, 0, 0, 0, 0, 0, "GIjKl");
  dpd_->buf4_sort(&G1, PSIF_CC_TMP0, qprs, 0, 0, "GjIKl");
  dpd_->buf4_init(&G2, PSIF_CC_TMP0, 0, 0, 0, 0, 0, 0, "GjIKl");
  dpd_->buf4_sort(&G2, PSIF_CC_TMP0, pqsr, 0, 0, "GiJkL");
  dpd_->buf4_close(&G2);
  dpd_->buf4_init(&G2, PSIF_CC_TMP0, 0, 0, 0, 0, 0, 0, "GiJkL");
  dpd_->buf4_axpy(&G2, &G1, 1.0);
  dpd_->buf4_close(&G2);
  dpd_->buf4_init(&G2, PSIF_CC_GAMMA, 0, 0, 0, 2, 2, 0, "GIJKL");
  dpd_->buf4_axpy(&G2, &G1, 1.0);
  dpd_->buf4_close(&G2);
  dpd_->buf4_init(&G2, PSIF_CC_GAMMA, 0, 0, 0, 2, 2, 0, "Gijkl");
  dpd_->buf4_axpy(&G2, &G1, 1.0);
  dpd_->buf4_close(&G2);
  dpd_->buf4_scm(&G1, 0.5);
  
  if(!params.aobasis) {
    dpd_->buf4_init(&A, PSIF_CC_AINTS, 0, 0, 0, 0, 0, 0, "A <ij|kl>");
    two_energy = dpd_->buf4_dot(&A, &G1);
    dpd_->buf4_close(&A);
  ...
  dpd_->buf4_init(&G1, PSIF_CC_GAMMA, 0, 0, 10, 0, 10, 0, "GIjKa");
  dpd_->buf4_init(&G2, PSIF_CC_GAMMA, 0, 0, 10, 0, 10, 0, "GiJkA");
  dpd_->buf4_axpy(&G2, &G1, 1.0);
  dpd_->buf4_close(&G2);
  dpd_->buf4_init(&G2, PSIF_CC_GAMMA, 0, 0, 10, 2, 10, 0, "GIJKA");
  dpd_->buf4_axpy(&G2, &G1, 1.0);
  dpd_->buf4_close(&G2);
  dpd_->buf4_init(&G2, PSIF_CC_GAMMA, 0, 0, 10, 2, 10, 0, "Gijka");
  dpd_->buf4_axpy(&G2, &G1, 1.0);
  ...
  dpd_->buf4_init(&G1, PSIF_CC_GAMMA, 0, 0, 5, 0, 5, 0, "GIjAb");
  dpd_->buf4_init(&G2, PSIF_CC_GAMMA, 0, 10, 10, 10, 10, 0, "GIbjA");
  dpd_->buf4_sort(&G2, PSIF_CC_TMP0, prsq, 0, 5, "GIbjA (Ij,Ab)");
  dpd_->buf4_close(&G2);
  dpd_->buf4_init(&G2, PSIF_CC_TMP0, 0, 0, 5, 0, 5, 0, "GIbjA (Ij,Ab)");
  dpd_->buf4_axpy(&G2, &G1, -1.0);
  dpd_->buf4_close(&G2);
  dpd_->buf4_close(&G1);
  
  /* G(IJ,AB) <-- G(IJ,AB) - G(IB,JA) */
  dpd_->buf4_init(&G1, PSIF_CC_GAMMA, 0, 0, 5, 2, 7, 0, "GIJAB");
  dpd_->buf4_copy(&G1, PSIF_CC_TMP0, "G(IJ,AB)");
  dpd_->buf4_close(&G1);
  dpd_->buf4_init(&G1, PSIF_CC_TMP0, 0, 0, 5, 0, 5, 0, "G(IJ,AB)");
  dpd_->buf4_init(&G2, PSIF_CC_GAMMA, 0, 10, 10, 10, 10, 0, "GIBJA");
  dpd_->buf4_sort(&G2, PSIF_CC_TMP0, prsq, 0, 5, "GIBJA (IJ,AB)");
  dpd_->buf4_close(&G2);
  dpd_->buf4_init(&G2, PSIF_CC_TMP0, 0, 0, 5, 0, 5, 0, "GIBJA (IJ,AB)");
  ...
  dpd_->buf4_init(&G1, PSIF_CC_GAMMA, 0, 0, 5, 0, 5, 0, "GIjAb");
  dpd_->buf4_sort(&G1, PSIF_CC_TMP0, qpsr, 0, 5, "G(jI,bA)");
  dpd_->buf4_init(&G2, PSIF_CC_TMP0, 0, 0, 5, 0, 5, 0, "G(jI,bA)");
  dpd_->buf4_axpy(&G2, &G1, 1.0);
  dpd_->buf4_close(&G2);
  dpd_->buf4_init(&G2, PSIF_CC_TMP0, 0, 0, 5, 0, 5, 0, "G(IJ,AB)");
  dpd_->buf4_axpy(&G2, &G1, 1.0);
  dpd_->buf4_close(&G2);
  dpd_->buf4_init(&G2, PSIF_CC_TMP0, 0, 0, 5, 0, 5, 0, "G(ij,ab)");
  dpd_->buf4_axpy(&G2, &G1, 1.0);
  ...
  if(!params.aobasis) {
    ...
    two_energy = dpd_->buf4_dot(&DInts, &G1);
  ...
  dpd_->buf4_init(&G1, PSIF_CC_GAMMA, 0, 11, 5, 11, 5, 0, "GCiAb");
  dpd_->buf4_init(&G2, PSIF_CC_GAMMA, 0, 11, 5, 11, 5, 0, "GcIaB");
  dpd_->buf4_axpy(&G2, &G1, 1.0);
  dpd_->buf4_close(&G2); 
  dpd_->buf4_init(&G2, PSIF_CC_GAMMA, 0, 11, 5, 11, 7, 0, "GCIAB");
  dpd_->buf4_axpy(&G2, &G1, 1.0);  
  dpd_->buf4_close(&G2); 
  dpd_->buf4_init(&G2, PSIF_CC_GAMMA, 0, 11, 5, 11, 7, 0, "Gciab");
  dpd_->buf4_axpy(&G2, &G1, 1.0);
  ...
  if(!params.aobasis) {
    ...
    dpd_->buf4_sort(&FInts, PSIF_CC_TMP0, qprs, 11, 5, "F(CI,BA)");
    dpd_->buf4_close(&FInts);
    dpd_->buf4_init(&FInts, PSIF_CC_TMP0, 0, 11, 5, 11, 5, 0, "F(CI,BA)");
    dpd_->buf4_sort(&FInts, PSIF_CC_TMP1, pqsr, 11, 5, "F(CI,AB)");
    dpd_->buf4_close(&FInts);
    dpd_->buf4_init(&FInts, PSIF_CC_TMP1, 0, 11, 5, 11, 5, 0, "F(CI,AB)");
    two_energy = dpd_->buf4_dot(&FInts, &G1);
    dpd_->buf4_close(&FInts);
  ...
  dpd_->buf4_init(&G1, PSIF_CC_GAMMA, 0, 5, 5, 5, 5, 0, "GAbCd");
  dpd_->buf4_sort(&G1, PSIF_CC_TMP0, qprs, 5, 5, "G(bA,Cd)");
  dpd_->buf4_init(&G2, PSIF_CC_TMP0, 0, 5, 5, 5, 5, 0, "G(bA,Cd)");
  dpd_->buf4_sort(&G2, PSIF_CC_TMP1, pqsr, 5, 5, "G(aB,cD)");
  dpd_->buf4_close(&G2);
  dpd_->buf4_init(&G2, PSIF_CC_TMP1, 0, 5, 5, 5, 5, 0, "G(aB,cD)");
  dpd_->buf4_axpy(&G2, &G1, 1.0);
  dpd_->buf4_close(&G2);
  dpd_->buf4_init(&G2, PSIF_CC_GAMMA, 0, 5, 5, 7, 7, 0, "GABCD");
  dpd_->buf4_axpy(&G2, &G1, 1.0);
  dpd_->buf4_close(&G2);
  dpd_->buf4_init(&G2, PSIF_CC_GAMMA, 0, 5, 5, 7, 7, 0, "Gabcd");
  dpd_->buf4_axpy(&G2, &G1, 1.0);
