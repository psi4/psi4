...
namespace psi { namespace ccdensity {
    ...
    */
	...
	dpd_->contract222(&F, &D, &I, 1, 0, 1.0, 0.0);
	dpd_->contract222(&F, &D, &I, 1, 1, 1.0, 1.0);
	dpd_->file2_close(&D);

	/* Add reference contribution: I'AI <-- 2 fAI */
	dpd_->file2_axpy(&F, &I, 2.0, 1);
	dpd_->file2_close(&F);
	...
	dpd_->file2_init(&D, PSIF_CC_OEI, 0, 0, 0, rho_params.DIJ_lbl);
	dpd_->buf4_init(&Eints, PSIF_CC_EINTS, 0, 21, 0, 21, 0, 1, "E <AI|JK>");
	dpd_->dot24(&D, &Eints, &I, 0, 0, 1.0, 1.0);
	dpd_->dot24(&D, &Eints, &I, 1, 0, 1.0, 1.0);
	dpd_->buf4_close(&Eints);
	dpd_->file2_close(&D);
	...
	dpd_->file2_init(&D, PSIF_CC_OEI, 0, 2, 2, rho_params.Dij_lbl);
	dpd_->buf4_init(&Eints, PSIF_CC_EINTS, 0, 26, 22, 26, 22, 0, "E <Ai|Jk>");
	dpd_->dot24(&D, &Eints, &I, 0, 0, 1.0, 1.0);
	dpd_->dot24(&D, &Eints, &I, 1, 0, 1.0, 1.0);
	dpd_->buf4_close(&Eints);
	dpd_->file2_close(&D);
	...
	dpd_->file2_init(&D, PSIF_CC_OEI, 0, 2, 2, rho_params.Dij_lbl);
	dpd_->buf4_init(&Eints, PSIF_CC_EINTS, 0, 31, 10, 31, 10, 1, "E <ai|jk>");
	dpd_->dot24(&D, &Eints, &I, 0, 0, 1.0, 1.0);
	dpd_->dot24(&D, &Eints, &I, 1, 0, 1.0, 1.0);
	dpd_->buf4_close(&Eints);
	dpd_->file2_close(&D);
	...
	dpd_->file2_init(&D, PSIF_CC_OEI, 0, 0, 0, rho_params.DIJ_lbl);
	dpd_->buf4_init(&Eints, PSIF_CC_EINTS, 0, 24, 22, 24, 22, 0, "E <Ia|Jk>");
	dpd_->dot13(&D, &Eints, &I, 0, 0, 1.0, 1.0);
	dpd_->dot13(&D, &Eints, &I, 1, 0, 1.0, 1.0);
	dpd_->buf4_close(&Eints);
	dpd_->file2_close(&D);
	...
	dpd_->file2_init(&D, PSIF_CC_OEI, 0, 0, 1, rho_params.DIA_lbl);
	dpd_->dot14(&D, &Cints, &I, 0, 0, -1.0, 1.0);
	dpd_->file2_close(&D);
	dpd_->file2_init(&D, PSIF_CC_OEI, 0, 0, 1, rho_params.DAI_lbl);
	dpd_->dot14(&D, &Cints, &I, 0, 0, -1.0, 1.0);
	dpd_->file2_close(&D);
	...
	dpd_->file2_init(&D, PSIF_CC_OEI, 0, 0, 1, rho_params.Dia_lbl);
	dpd_->dot24(&D, &Dints, &I, 0, 1, 1.0, 1.0);
	dpd_->file2_close(&D);
	dpd_->file2_init(&D, PSIF_CC_OEI, 0, 0, 1, rho_params.Dai_lbl);
	dpd_->dot24(&D, &Dints, &I, 0, 1, 1.0, 1.0);
	dpd_->file2_close(&D);
	...
	dpd_->buf4_init(&Dints, PSIF_CC_DINTS, 0, 10, 15, 10, 15, 0, "D <ij||ab>");
	dpd_->file2_init(&D, PSIF_CC_OEI, 0, 2, 3, rho_params.Dai_lbl);
	dpd_->dot24(&D, &Dints, &I, 0, 1, 1.0, 1.0);
	dpd_->file2_close(&D);
	dpd_->file2_init(&D, PSIF_CC_OEI, 0, 2, 3, rho_params.Dia_lbl);
	...
	dpd_->file2_init(&D, PSIF_CC_OEI, 0, 3, 3, rho_params.Dab_lbl);
	dpd_->buf4_init(&Fints, PSIF_CC_FINTS, 0, 30, 15, 30, 15, 1, "F <ia|bc>");
	dpd_->dot24(&D, &Fints, &I, 1, 1, 1.0, 1.0);
	dpd_->dot24(&D, &Fints, &I, 0, 1, 1.0, 1.0);
	dpd_->buf4_close(&Fints);
	dpd_->file2_close(&D);
	...
	dpd_->buf4_init(&Eints, PSIF_CC_EINTS, 0, 11, 2, 11, 0, 1, "E <ai|jk>");
	dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 0, 2, 2, 2, 0, "GIJKL");
	dpd_->contract442(&Eints, &G, &I, 0, 0, 2.0, 1.0);
	dpd_->buf4_close(&G);
	dpd_->buf4_close(&Eints);
	...
	dpd_->buf4_init(&Eints, PSIF_CC_EINTS, 0, 21, 2, 21, 0, 1, "E <AI|JK>");
	dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 0, 2, 2, 2, 0, "GIJKL");
	dpd_->contract442(&Eints, &G, &I, 0, 0, 2.0, 1.0);
	dpd_->buf4_close(&G);
	dpd_->buf4_close(&Eints);
	...
	dpd_->buf4_init(&Eints, PSIF_CC_EINTS, 0, 31, 12, 31, 10, 1, "E <ai|jk>");
	dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 10, 12, 12, 12, 0, "Gijkl");
	dpd_->contract442(&Eints, &G, &I, 0, 0, 2.0, 1.0);
	dpd_->buf4_close(&G);
	dpd_->buf4_close(&Eints);
	...
	dpd_->buf4_init(&Cints, PSIF_CC_CINTS, 0, 10, 10, 10, 10, 0, "C <ia|jb>");
	dpd_->contract442(&Cints, &G, &I, 1, 1, 2.0, 1.0);
	dpd_->buf4_close(&Cints);
	...
	dpd_->buf4_init(&Dints, PSIF_CC_DINTS, 0, 0, 5, 0, 5, 0, "D <ij|ab>");
	dpd_->buf4_sort(&Dints, PSIF_CC_TMP0, rqps, 11, 10, "D <ij|ab> (aj,ib)");
	dpd_->buf4_close(&Dints);
	dpd_->buf4_init(&Dints, PSIF_CC_TMP0, 0, 11, 10, 11, 10, 0, "D <ij|ab> (aj,ib)");
	dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 0, 10, 0, 10, 0, "GIjKa");
	dpd_->contract442(&Dints, &G, &I, 0, 0, 2.0, 1.0);
	dpd_->buf4_close(&G);
	dpd_->buf4_close(&Dints);
	...
	dpd_->buf4_init(&Dints, PSIF_CC_DINTS, 0, 12, 15, 12, 15, 0, "D <ij||ab> (i>j,ab)");
	...
	dpd_->buf4_sort(&G, PSIF_CC_TMP0, qpsr, 23, 29, "GiJaB");
	dpd_->buf4_close(&G);
	dpd_->buf4_init(&G, PSIF_CC_TMP0, 0, 23, 29, 23, 29, 0, "GiJaB");
	dpd_->contract442(&Fints, &G, &I, 1, 1, 2.0, 1.0);
	dpd_->buf4_close(&G);
	dpd_->buf4_close(&Fints);
	...
	dpd_->buf4_init(&Fints, PSIF_CC_FINTS, 0, 10, 5, 10, 5, 1, "F <ia|bc>");
	dpd_->contract442(&Fints, &G, &I, 2, 2, 2.0, 1.0);
	dpd_->buf4_close(&Fints);
	dpd_->buf4_close(&G);
	...
	dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 10, 10, 10, 10, 0, "GIbjA");
	dpd_->buf4_sort(&G, PSIF_CC_TMP0, rsqp, 10, 11, "GIbjA (jA,bI)");
	dpd_->buf4_close(&G);
	dpd_->buf4_init(&G, PSIF_CC_TMP0, 0, 10, 11, 10, 11, 0, "GIbjA (jA,bI)");
	dpd_->buf4_init(&Fints, PSIF_CC_FINTS, 0, 10, 5, 10, 5, 0, "F <ia|bc>");
	...
	dpd_->buf4_sort(&G, PSIF_CC_TMP0, rsqp, 27, 25, "GIbjA (jA,bI)");
	dpd_->buf4_close(&G);
	dpd_->buf4_init(&G, PSIF_CC_TMP0, 0, 27, 25, 27, 25, 0, "GIbjA (jA,bI)");
	dpd_->buf4_init(&Fints, PSIF_CC_FINTS, 0, 27, 29, 27, 29, 0, "F <iA|bC>");
	dpd_->contract442(&Fints, &G, &I, 3, 3, -2.0, 1.0);
	dpd_->buf4_close(&Fints);
	dpd_->buf4_close(&G);
	...
	dpd_->buf4_sort(&G, PSIF_CC_TMP0, rsqp, 24, 26, "GiBJa (Ja,Bi)");
	dpd_->buf4_close(&G);
	dpd_->buf4_init(&G, PSIF_CC_TMP0, 0, 24, 26, 24, 26, 0, "GiBJa (Ja,Bi)");
	dpd_->buf4_init(&Fints, PSIF_CC_FINTS, 0, 24, 28, 24, 28, 0, "F <Ia|Bc>");
	dpd_->contract442(&Fints, &G, &I, 3, 3, -2.0, 1.0);
	dpd_->buf4_close(&Fints);
	dpd_->buf4_close(&G);
	...
	dpd_->buf4_sort(&G, PSIF_CC_GAMMA, qpsr, 10, 5, "2 Giabc - Giacb");
	dpd_->buf4_close(&G);

	dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 10, 5, 10, 5, 0, "2 Giabc - Giacb");
	dpd_->buf4_init(&Bints, PSIF_CC_BINTS, 0, 5, 5, 5, 5, 0, "B <ab|cd>");
	dpd_->contract442(&Bints, &G, &I, 0, 0, 2.0, 1.0);
	...
	dpd_->buf4_sort(&G, PSIF_CC_TMP0, qprs, 10, 7, "GICAB");
	dpd_->buf4_close(&G);
	dpd_->buf4_init(&G, PSIF_CC_TMP0, 0, 10, 7, 10, 7, 0, "GICAB");
	dpd_->buf4_init(&Bints, PSIF_CC_BINTS, 0, 5, 7, 5, 5, 1, "B <ab|cd>");
	dpd_->contract442(&Bints, &G, &I, 0, 0, -2.0, 1.0);
	dpd_->buf4_close(&Bints);
	dpd_->buf4_close(&G);

	dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 11, 5, 11, 5, 0, "GcIaB");
	dpd_->buf4_sort(&G, PSIF_CC_TMP0, qpsr, 10, 5, "GIcAb");
	dpd_->buf4_close(&G);
	dpd_->buf4_init(&G, PSIF_CC_TMP0, 0, 10, 5, 10, 5, 0, "GIcAb");
	dpd_->buf4_init(&Bints, PSIF_CC_BINTS, 0, 5, 5, 5, 5, 0, "B <ab|cd>");
	dpd_->contract442(&Bints, &G, &I, 0, 0, 2.0, 1.0);
	dpd_->buf4_close(&Bints);
	dpd_->buf4_close(&G);
	...
	dpd_->buf4_sort(&G, PSIF_CC_TMP0, qprs, 20, 7, "GICAB");
	dpd_->buf4_close(&G);
	dpd_->buf4_init(&G, PSIF_CC_TMP0, 0, 20, 7, 20, 7, 0, "GICAB");
	dpd_->buf4_init(&Bints, PSIF_CC_BINTS, 0, 5, 7, 5, 5, 1, "B <AB|CD>");
	dpd_->contract442(&Bints, &G, &I, 0, 0, -2.0, 1.0);
	dpd_->buf4_close(&Bints);
	dpd_->buf4_close(&G);
	...
	dpd_->buf4_sort(&G, PSIF_CC_TMP0, qpsr, 24, 28, "GIcAb");
	dpd_->buf4_close(&G);
	dpd_->buf4_init(&G, PSIF_CC_TMP0, 0, 24, 28, 24, 28, 0, "GIcAb");
	dpd_->buf4_init(&Bints, PSIF_CC_BINTS, 0, 28, 28, 28, 28, 0, "B <Ab|Cd>");
	dpd_->contract442(&Bints, &G, &I, 0, 0, 2.0, 1.0);
	dpd_->buf4_close(&Bints);
	dpd_->buf4_close(&G);
	...
	dpd_->buf4_sort(&G, PSIF_CC_TMP0, qprs, 30, 17, "Gicab");
	dpd_->buf4_close(&G);
	dpd_->buf4_init(&G, PSIF_CC_TMP0, 0, 30, 17, 30, 17, 0, "Gicab");
	dpd_->buf4_init(&Bints, PSIF_CC_BINTS, 0, 15, 17, 15, 15, 1, "B <ab|cd>");
	dpd_->contract442(&Bints, &G, &I, 0, 0, -2.0, 1.0);
	dpd_->buf4_close(&Bints);
	dpd_->buf4_close(&G);

	dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 26, 28, 26, 28, 0, "GCiAb");
	dpd_->buf4_sort(&G, PSIF_CC_TMP0, rspq, 28, 26, "GAbCi");
	dpd_->buf4_close(&G);
	dpd_->buf4_init(&G, PSIF_CC_TMP0, 0, 28, 26, 28, 26, 0, "GAbCi");
	dpd_->buf4_init(&Bints, PSIF_CC_BINTS, 0, 28, 28, 28, 28, 0, "B <Ab|Cd>");
	dpd_->contract442(&Bints, &G, &I, 3, 3, 2.0, 1.0);
	dpd_->buf4_close(&Bints);
	dpd_->buf4_close(&G);
