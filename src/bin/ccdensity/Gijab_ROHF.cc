...
void Gijab_ROHF(void)
{
  ...
  dpd_file2_close(&g);
  dpd_file2_init(&ZZ, PSIF_CC_TMP1, 0, 0, 0, "Z(i,m)");
  dpd_file2_init(&L1, PSIF_CC_GLG, 0, 0, 1, "Lia");
  dpd_file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tia");
  dpd_contract222(&T1, &L1, &ZZ, 0, 0, 1.0, 1.0);
  dpd_file2_close(&T1);
  ...
  dpd_file2_close(&g);
  dpd_file2_init(&ZZ, PSIF_CC_TMP3, 0, 1, 1, "Z(e,a)");
  dpd_file2_init(&L1, PSIF_CC_GLG, 0, 0, 1, "Lia");
  dpd_file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tia");
  dpd_contract222(&L1, &T1, &ZZ, 1, 1, -1.0, 1.0);
  dpd_file2_close(&T1);
  ...
  dpd_buf4_close(&T);
  dpd_buf4_init(&Z, PSIF_CC_TMP4, 0, 10, 10, 10, 10, 0, "Z(IA,ME)");
  dpd_buf4_scm(&Z, -1.0);
  for(h=0; h < nirreps; h++) {
  ...
  dpd_buf4_close(&T);
  dpd_buf4_init(&Z, PSIF_CC_TMP5, 0, 10, 10, 10, 10, 0, "Z(ia,me)");
  dpd_buf4_scm(&Z, -1.0);
  for(h=0; h < nirreps; h++) {
    dpd_buf4_mat_irrep_init(&Z, h); 0,
				      dpd_buf4_mat_irrep_rd(&Z, h);
  ...
  /* ( - T(iA,Me) + 2 * T(i,e) T(M,A) ) --> Z(iA,Me) */
  dpd_buf4_init(&T, PSIF_CC_TAMPS, 0, 10, 10, 10, 10, 0, "tjAIb");
  dpd_buf4_copy(&T, PSIF_CC_TMP6, "Z(iA,Me)");
  dpd_buf4_close(&T);
  dpd_buf4_init(&Z, PSIF_CC_TMP6, 0, 10, 10, 10, 10, 0, "Z(iA,Me)");
  for(h=0; h < nirreps; h++) {
    dpd_buf4_mat_irrep_init(&Z, h); 0,
				      dpd_buf4_mat_irrep_rd(&Z, h);
    for(row=0; row < Z.params->rowtot[h]; row++) {
  ...
  dpd_buf4_close(&T);
  dpd_buf4_init(&Z, PSIF_CC_TMP7, 0, 10, 10, 10, 10, 0, "Z(Ia,mE)");
  for(h=0; h < nirreps; h++) {
    dpd_buf4_mat_irrep_init(&Z, h); 0,
				      dpd_buf4_mat_irrep_rd(&Z, h);
    for(row=0; row < Z.params->rowtot[h]; row++) {
  ...
  dpd_buf4_init(&Z, PSIF_CC_TMP8, 0, 10, 10, 10, 10, 0, "Z(IA,JB)");
  dpd_buf4_init(&T, PSIF_CC_TMP4, 0, 10, 10, 10, 10, 0, "Z(IA,ME)");
  ...
  dpd_buf4_close(&V);
  dpd_buf4_close(&T);
  dpd_buf4_sort(&Z, PSIF_CC_TMP9, rqps, 10, 10, "Z(JA,IB)");
  dpd_buf4_sort(&Z, PSIF_CC_TMP10, psrq, 10, 10, "Z(IB,JA)");
  dpd_buf4_sort(&Z, PSIF_CC_TMP11, rspq, 10, 10, "Z(JB,IA)");
  dpd_buf4_init(&Z1, PSIF_CC_TMP9, 0, 10, 10, 10, 10, 0, "Z(JA,IB)");
  dpd_buf4_axpy(&Z1, &Z, -1.0);
  dpd_buf4_close(&Z1);
  dpd_buf4_init(&Z1, PSIF_CC_TMP10, 0, 10, 10, 10, 10, 0, "Z(IB,JA)");
  dpd_buf4_axpy(&Z1, &Z, -1.0);
  dpd_buf4_close(&Z1);
  dpd_buf4_init(&Z1, PSIF_CC_TMP11, 0, 10, 10, 10, 10, 0, "Z(JB,IA)");
  dpd_buf4_axpy(&Z1, &Z, 1.0);
  dpd_buf4_close(&Z1);
  ...
  dpd_buf4_init(&Z, PSIF_CC_TMP8, 0, 10, 10, 10, 10, 0, "Z(ia,jb)");
  dpd_buf4_init(&T, PSIF_CC_TMP5, 0, 10, 10, 10, 10, 0, "Z(ia,me)");
  dpd_buf4_init(&V, PSIF_CC_MISC, 0, 10, 10, 10, 10, 0, "Viajb");
  dpd_contract444(&T, &V, &Z, 0, 0, 1.0, 0.0);
  dpd_buf4_close(&V);
  dpd_buf4_close(&T);
  ...
  dpd_buf4_init(&Z1, PSIF_CC_TMP8, 0, 0, 5, 0, 5, 0, "Z1(Ij,Ab)");
  dpd_buf4_init(&T, PSIF_CC_TAMPS, 0, 0, 5, 0, 5, 0, "tauIjAb");
  dpd_file2_init(&ZZ, PSIF_CC_TMP0, 0, 0, 0, "Z(I,M)");
  dpd_contract244(&ZZ, &T, &Z1, 1, 0, 0, 1.0, 0.0);
  dpd_file2_close(&ZZ);
  dpd_buf4_close(&T);
  dpd_buf4_init(&Z2, PSIF_CC_TMP9, 0, 0, 5, 0, 5, 0, "Z2(jI,bA)");
  dpd_buf4_init(&T, PSIF_CC_TAMPS, 0, 0, 5, 0, 5, 0, "tauiJaB");
  dpd_file2_init(&ZZ, PSIF_CC_TMP1, 0, 0, 0, "Z(i,m)");
  dpd_contract244(&ZZ, &T, &Z2, 1, 0, 0, 1.0, 0.0);
  dpd_file2_close(&ZZ);
  dpd_buf4_close(&T);
  dpd_buf4_sort(&Z2, PSIF_CC_TMP10, qprs, 0, 5, "Z2(Ij,bA)");
  dpd_buf4_close(&Z2);
  dpd_buf4_init(&Z2, PSIF_CC_TMP10, 0, 0, 5, 0, 5, 0, "Z2(Ij,bA)");
  dpd_buf4_sort(&Z2, PSIF_CC_TMP9, pqsr, 0, 5, "Z2(Ij,Ab)");
  dpd_buf4_close(&Z2);
  dpd_buf4_init(&Z2, PSIF_CC_TMP9, 0, 0, 5, 0, 5, 0, "Z2(Ij,Ab)");
  dpd_buf4_axpy(&Z2, &Z1, 1.0);
  dpd_buf4_close(&Z2);
  dpd_buf4_init(&G, PSIF_CC_GAMMA, 0, 0, 5, 0, 5, 0, "GIjAb");
  dpd_buf4_axpy(&Z1, &G, -1.0);
  ...
  dpd_buf4_close(&G);
  /* - ( Z(E,A) Tau(Ij,bE) - Z(e,b) Tau(Ij,Ae) ) */
  dpd_buf4_init(&Z1, PSIF_CC_TMP8, 0, 0, 5, 0, 5, 0, "ZZ1(Ij,Ab)");
  dpd_buf4_init(&T, PSIF_CC_TAMPS, 0, 0, 5, 0, 5, 0, "tauIjAb");
  dpd_file2_init(&ZZ, PSIF_CC_TMP3, 0, 1, 1, "Z(e,a)");
  dpd_contract424(&T, &ZZ, &Z1, 3, 0, 0, 1.0, 0.0);
  dpd_file2_close(&ZZ);
  ...
  /* T'(jA,Me) V(Ib,Me) */
  dpd_buf4_init(&Z1, PSIF_CC_TMP9, 0, 10, 10, 10, 10, 0, "Z(jA,Ib)");
  dpd_buf4_init(&T, PSIF_CC_TMP6, 0, 10, 10, 10, 10, 0, "Z(iA,Me)");
  dpd_buf4_init(&V, PSIF_CC_MISC, 0, 10, 10, 10, 10, 0, "VIaJb");
  dpd_contract444(&T, &V, &Z1, 0, 0, 1.0, 0.0);
  dpd_buf4_close(&V);
  dpd_buf4_close(&T);
  dpd_buf4_sort(&Z1, PSIF_CC_TMP10, rqps, 10, 10, "Z(IA,jb)");
  dpd_buf4_close(&Z1);
  dpd_buf4_init(&Z1, PSIF_CC_TMP10, 0, 10, 10, 10, 10, 0, "Z(IA,jb)");
  dpd_buf4_axpy(&Z1, &Z, -1.0);
  dpd_buf4_close(&Z1);
  /* T'(Ib,mE) V(jA,mE) */
  dpd_buf4_init(&Z1, PSIF_CC_TMP9, 0, 10, 10, 10, 10, 0, "Z(Ib,jA)");
  dpd_buf4_init(&T, PSIF_CC_TMP7, 0, 10, 10, 10, 10, 0, "Z(Ia,mE)");
  dpd_buf4_init(&V, PSIF_CC_MISC, 0, 10, 10, 10, 10, 0, "ViAjB");
  dpd_contract444(&T, &V, &Z1, 0, 0, 1.0, 0.0);
  dpd_buf4_close(&V);
  dpd_buf4_close(&T);
  dpd_buf4_sort(&Z1, PSIF_CC_TMP10, psrq, 10, 10, "Z(IA,jb)");
  dpd_buf4_close(&Z1);
  dpd_buf4_init(&Z1, PSIF_CC_TMP10, 0, 10, 10, 10, 10, 0, "Z(IA,jb)");
  dpd_buf4_axpy(&Z1, &Z, -1.0);
  dpd_buf4_close(&Z1);
  /* T'(jb,ME) (T2) V(IA,ME) + T'(jb,me) (TMP5) V(IA,me) */
  dpd_buf4_init(&Z1, PSIF_CC_TMP9, 0, 10, 10, 10, 10, 0, "Z(IA,jb)");
  dpd_buf4_init(&T, PSIF_CC_TMP5, 0, 10, 10, 10, 10, 0, "Z(ia,me)");
  dpd_buf4_init(&V, PSIF_CC_MISC, 0, 10, 10, 10, 10, 0, "VIAjb");
  ...
  dpd_file2_init(&ZZ, PSIF_CC_TMP8, 0, 0, 1, "ZZ(j,b)");
  dpd_file2_init(&L1, PSIF_CC_GLG, 0, 0, 1, "LIA");
  dpd_buf4_init(&T, PSIF_CC_TAMPS, 0, 10, 10, 10, 10, 0, "tiaJB");
  dpd_contract422(&T, &L1, &ZZ, 0, 0, -1.0, 0.0);
  dpd_buf4_close(&T);
