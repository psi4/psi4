...
void ltdensity_rohf(struct TD_Params S)
{
  ...
  if(S.irrep == 0) {
    dpd_file2_init(&D, PSIF_CC_TMP, 0, 0, 0, "LTDIJ");
    ...
    dpd_file2_init(&D, PSIF_CC_TMP, 0, 1, 1, "LTDAB");
  ...
  dpd_file2_copy(&R1, PSIF_CC_TMP, "LTDIA");
  dpd_file2_close(&R1);

  if(S.irrep == 0) {
    dpd_file2_init(&D, PSIF_CC_TMP, 0, 0, 1, "LTDIA");
  ...
  if(S.irrep == 0) {
    ...
    dpd_file2_init(&L1, PSIF_CC_GLG, 0, 0, 1, "LIA");
    dpd_file2_copy(&L1, PSIF_CC_TMP, "LTDAI");
    dpd_file2_close(&L1);
  ...
  dpd_file2_init(&Tia, PSIF_CC_OEI, 0, 0, 1, "tia");
  dpd_file2_init(&RIA, PSIF_CC_GR, S.irrep, 0, 1, "RIA");
  dpd_file2_init(&Ria, PSIF_CC_GR, S.irrep, 0, 1, "Ria");
  dpd_file2_init(&LIA, PSIF_CC_GLG, 0, 0, 1, "LIA");
  dpd_file2_init(&Lia, PSIF_CC_GLG, 0, 0, 1, "Lia");

  /* D[i][j] = -LR_oo[j][i] - t1[i][f] * L2R1_ov[j][f] */

  dpd_file2_init(&DIJ, PSIF_CC_TMP, S.irrep, 0, 0, "LTDIJ");
  dpd_file2_init(&Int, PSIF_EOM_TMP, S.irrep, 0, 0, "LR_OO");
  dpd_file2_axpy(&Int, &DIJ, -1.0, 1);
  dpd_file2_close(&Int);

  dpd_file2_init(&Int, PSIF_EOM_TMP, S.irrep, 0, 1, "L2R1_OV");
  ...
  dpd_file2_init(&DAB, PSIF_CC_TMP, S.irrep, 1, 1, "LTDAB");
  dpd_file2_init(&Int, PSIF_EOM_TMP, S.irrep, 1, 1, "LR_VV");
  ...
  dpd_file2_init(&DAI, PSIF_CC_TMP, S.irrep, 0, 1, "LTDAI");
  dpd_file2_init(&Int, PSIF_EOM_TMP, S.irrep, 0, 1, "L2R1_OV");
  ...
  dpd_file2_init(&DIA, PSIF_CC_TMP, S.irrep, 0, 1, "LTDIA");
  dpd_file2_init(&Dia, PSIF_CC_TMP, S.irrep, 0, 1, "LTDia");

  /* D[i][a] = L1R2_ov[i][a] */

  dpd_file2_init(&Int, PSIF_EOM_TMP, S.irrep, 0, 1, "L1R2_OV");
  ...
  dpd_file2_init(&Int, PSIF_EOM_TMP, 0, 0, 0, "LT2_OO");
  ...
  dpd_file2_init(&Int, PSIF_EOM_TMP, 0, 1, 1, "LT2_VV");
  ...
  dpd_file2_init(&Int, PSIF_EOM_TMP, S.irrep, 0, 1, "L2R1_OV");
  dpd_file2_init(&XIJ, PSIF_EOM_TMP, S.irrep, 0, 0, "XIJ");
...
void ltdensity_uhf(struct TD_Params S)
{
  ...
  if(S.irrep == 0 ) { /* Symmetric Transition */
    ...
    dpd_file2_init(&D, PSIF_CC_TMP, 0, 2, 2, "LTDij");
    ...
    dpd_file2_init(&D, PSIF_CC_TMP, 0, 3, 3, "LTDab");
  ...
  if(S.irrep == 0) { /* Symmetric Transitions */
    ...
    dpd_file2_init(&D, PSIF_CC_TMP, 0, 2, 3, "LTDia");
  ...
  dpd_file2_init(&RIA, PSIF_CC_GR, S.irrep, 0, 1, "RIA");
  dpd_file2_init(&Ria, PSIF_CC_GR, S.irrep, 2, 3, "Ria");
  dpd_file2_init(&LIA, PSIF_CC_GL, S.irrep, 0, 1, "LIA");
  dpd_file2_init(&Lia, PSIF_CC_GL, S.irrep, 2, 3, "Lia");
  ...
  dpd_file2_init(&Dij, PSIF_CC_TMP, S.irrep, 2, 2, "LTDij");
  dpd_file2_init(&Int, PSIF_EOM_TMP, S.irrep, 2, 2, "LR_oo");
  dpd_file2_axpy(&Int, &Dij, -1.0, 1);
  dpd_file2_close(&Int);

  dpd_file2_init(&Int, PSIF_EOM_TMP, S.irrep, 2, 3, "L2R1_ov");
  dpd_contract222(&Tia, &Int, &Dij, 0, 0, -1.0, 1.0);
  dpd_file2_close(&Int);
  dpd_file2_close(&Dij);
  ...
  dpd_file2_init(&Dab, PSIF_CC_TMP, S.irrep, 3, 3, "LTDab");
  dpd_file2_init(&Int, PSIF_EOM_TMP, S.irrep, 3, 3, "LR_vv");
  dpd_file2_axpy(&Int, &Dab, 1.0, 0);
  dpd_file2_close(&Int);
  ...
  dpd_file2_init(&Dai, PSIF_CC_TMP, S.irrep, 2, 3, "LTDai");
  dpd_file2_init(&Int, PSIF_EOM_TMP, S.irrep, 2, 3, "L2R1_ov");
  dpd_file2_axpy(&Int, &Dai, 1.0, 0);
  dpd_file2_close(&Int);
  dpd_file2_close(&Dai);
  ...
  dpd_file2_init(&Dia, PSIF_CC_TMP, S.irrep, 2, 3, "LTDia");
  ...
  dpd_file2_init(&Int, PSIF_EOM_TMP, S.irrep, 2, 3, "L1R2_ov");
  dpd_file2_axpy(&Int, &Dia, 1.0, 0);
  dpd_file2_close(&Int);
  ...
  dpd_file2_init(&Int, PSIF_EOM_TMP, 0, 2, 2, "LT2_oo");
  dpd_contract222(&Int, &Ria, &Dia, 1, 1, -1.0, 1.0);
  dpd_file2_close(&Int);
  ...
  dpd_file2_init(&Int, PSIF_EOM_TMP, 0, 3, 3, "LT2_vv");
  dpd_contract222(&Ria, &Int, &Dia, 0, 1, -1.0, 1.0);
  dpd_file2_close(&Int);
  ...
  dpd_file2_init(&Int, PSIF_EOM_TMP, S.irrep, 2, 3, "L2R1_ov");
  dpd_file2_init(&Xij, PSIF_EOM_TMP, S.irrep, 2, 2, "Xij");
  dpd_contract222(&Tia, &Int, &Xij, 0, 0, 1.0, 0.0);
  dpd_file2_close(&Int);
