...
void x_Gibja_rohf(void)
{
  ...
  dpd_buf4_init(&V2, PSIF_EOM_TMP, G_irr, 10, 10, 10, 10, 0, "R2L2_OVOV");
  dpd_buf4_sort(&V2, PSIF_EOM_TMP0, rspq, 10, 10, "GIAJB");
  dpd_buf4_close(&V2);
  dpd_buf4_init(&V2, PSIF_EOM_TMP, G_irr, 10, 10, 10, 10, 0, "R2L2_ovov");
  dpd_buf4_sort(&V2, PSIF_EOM_TMP0, rspq, 10, 10, "Giajb");
  dpd_buf4_close(&V2);
  ...
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 0, 11, 0, 11, 0, "Z(IM,AJ)");
  ...
  dpd_file2_close(&T1A);
  dpd_buf4_close(&L2);
  dpd_buf4_init(&Z1, PSIF_EOM_TMP1, G_irr, 10, 11, 10, 11, 0, "Z(IB,AJ)");
  dpd_file2_init(&R1A, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  dpd_contract424(&Z, &R1A, &Z1, 1, 0, 1, 1.0, 0.0);
  dpd_file2_close(&R1A);
  dpd_buf4_close(&Z);
  dpd_buf4_sort(&Z1, PSIF_EOM_TMP1, prqs, 10, 11, "Z(IA,BJ)");
  dpd_buf4_close(&Z1);
  dpd_buf4_init(&Z1, PSIF_EOM_TMP1, G_irr, 10, 11, 10, 11, 0, "Z(IA,BJ)");
  dpd_buf4_sort(&Z1, PSIF_EOM_TMP1, pqsr, 10, 10, "Z(IA,JB)");
  dpd_buf4_close(&Z1);
  dpd_buf4_init(&Z1, PSIF_EOM_TMP1, G_irr, 10, 10, 10, 10, 0, "Z(IA,JB)");
  dpd_buf4_init(&G, PSIF_EOM_TMP0, G_irr, 10, 10, 10, 10, 0, "GIAJB");
  ...
  dpd_buf4_close(&Z1);
  dpd_buf4_init(&Z1, PSIF_EOM_TMP1, G_irr, 10, 10, 10, 10, 0, "Z(IA,jb)");
  dpd_buf4_init(&G, PSIF_EOM_TMP0, 0, 10, 10, 10, 10, 0, "GIAjb");
  dpd_buf4_axpy(&Z1, &G, -1.0);
  dpd_buf4_close(&Z1);
  dpd_buf4_close(&G);
  ...
  dpd_buf4_init(&Z, PSIF_EOM_TMP, G_irr, 0, 11, 2, 11, 0, "L2R1_OOVO");
  dpd_buf4_init(&Z1, PSIF_EOM_TMP1, G_irr, 10, 11, 10, 11, 0, "Z(IB,AJ)");
  dpd_file2_init(&T1A, PSIF_CC_OEI, 0, 0, 1, "tIA");
  ...
  dpd_buf4_init(&Z, PSIF_EOM_TMP, G_irr, 0, 11, 0, 11, 0, "L2R1_OovO");
  dpd_buf4_init(&Z1, PSIF_EOM_TMP1, G_irr, 10, 11, 10, 11, 0, "Z(Ib,aJ)");
  dpd_file2_init(&T1B, PSIF_CC_OEI, 0, 0, 1, "tia");
  dpd_contract424(&Z, &T1B, &Z1, 1, 0, 1, 1.0, 0.0);
  dpd_file2_close(&T1B);
  ...
  /* term 5, G(iA,jB) <-- - (L2(iM,eA)*R(j,e)) * T(M,B) */
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 11, 0, 11, 0, "Z(iM,Aj)");
  dpd_buf4_init(&L2, PSIF_CC_GL, L_irr, 0, 5, 0, 5, 0, "LiJAb");
  dpd_file2_init(&R1A, PSIF_CC_GR, R_irr, 0, 1, "Ria");
  ...
  dpd_file2_init(&L1B, PSIF_CC_GL, L_irr, 0, 1, "Lia");
  ...
  dpd_buf4_init(&G, PSIF_EOM_TMP0, G_irr, 10, 10, 10, 10, 0, "GIAJB");
  dpd_buf4_sort(&G, PSIF_EOM_TMP0, psrq, 10, 10, "GIBJA");
  dpd_buf4_close(&G);
  dpd_buf4_init(&G, PSIF_EOM_TMP0, G_irr, 10, 10, 10, 10, 0, "Giajb");
  dpd_buf4_sort(&G, PSIF_EOM_TMP0, psrq, 10, 10, "Gibja");
  dpd_buf4_close(&G);
  ...
  dpd_buf4_init(&GIBJA, PSIF_EOM_TMP0, G_irr, 10, 10, 10, 10, 0, "GIBJA");
  ...
  dpd_buf4_symm(&G);
  dpd_buf4_close(&G);
  dpd_buf4_init(&GIbjA, PSIF_CC_GAMMA, G_irr, 10, 10, 10, 10, 0, "GIbjA");
  dpd_buf4_init(&GiBJa, PSIF_CC_GAMMA, G_irr, 10, 10, 10, 10, 0, "GiBJa");
  dpd_buf4_symm2(&GIbjA, &GiBJa);
  dpd_buf4_close(&GiBJa);
  dpd_buf4_sort(&GIbjA, PSIF_CC_GAMMA, rspq, 10, 10, "GiBJa");
  dpd_buf4_close(&GIbjA);
