...
void x_Gciab_rohf(void) { 
  ...
  dpd_->buf4_init(&Z, PSIF_EOM_TMP, G_irr, 7, 11, 7, 11, 0, "L2R1_VVOV(pqsr)");
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP0, rspq, 11, 7, "GCIAB");
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP, G_irr, 7, 11, 7, 11, 0, "L2R1_vvov(pqsr)");
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP0, rspq, 11, 7, "Gciab");
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP, G_irr, 5, 11, 5, 11, 0, "L2R1_VvoV(pqsr)");
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP0, rspq, 11, 5, "GCiAb");
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP, G_irr, 5, 11, 5, 11, 0, "L2R1_VvOv(pqsr)");
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP0, rsqp, 11, 5, "GcIaB");
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP0, G_irr, 11, 7, 11, 7, 0, "GCIAB");
  dpd_->buf4_scm(&Z, -1.0);
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP0, G_irr, 11, 7, 11, 7, 0, "Gciab");
  dpd_->buf4_scm(&Z, -1.0);
  ...
  dpd_->buf4_init(&Z, PSIF_EOM_TMP, G_irr, 7, 11, 7, 11, 0, "R2L1_VVOV(pqsr)");
  dpd_->buf4_sort_axpy(&Z, PSIF_EOM_TMP0, rspq, 11, 7, "GCIAB",-1.0);
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP, G_irr, 7, 11, 7, 11, 0, "R2L1_vvov(pqsr)");
  dpd_->buf4_sort_axpy(&Z, PSIF_EOM_TMP0, rspq, 11, 7, "Gciab",-1.0);
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP, G_irr, 5, 11, 5, 11, 0, "R2L1_VvoV(pqsr)");
  dpd_->buf4_sort_axpy(&Z, PSIF_EOM_TMP0, rspq, 11, 5, "GCiAb", 1.0);
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP, G_irr, 5, 11, 5, 11, 0, "R2L1_VvOv(pqsr)");
  dpd_->buf4_sort_axpy(&Z, PSIF_EOM_TMP0, rsqp, 11, 5, "GcIaB", 1.0);
  dpd_->buf4_close(&Z);
  ...
  */
  ...
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 11, 2, 11, 2, 0, "Z(CI,MN)");
  dpd_->buf4_init(&L, PSIF_CC_GL, L_irr, 2, 5, 2, 7, 0, "LIJAB");
  dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tIA");
  dpd_->contract424(&L, &T1, &Z, 3, 1, 1, 1.0, 0.0);
  dpd_->file2_close(&T1);
  dpd_->buf4_close(&L);
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 11, 7, 11, 7, 0, "GCIAB");
  dpd_->buf4_init(&R, PSIF_CC_GR, R_irr, 2, 7, 2, 7, 0, "RIJAB");
  dpd_->contract444(&Z, &R, &G, 0, 1, -1.0, 1.0);
  dpd_->buf4_close(&R);
  dpd_->buf4_close(&G);
  dpd_->buf4_close(&Z);
  /* term 3, rho_ciab -= 0.5 Lmnce Rmnab tie */
  ...
  dpd_->buf4_close(&Z);
  /* term 3, rho_CiAb -= 0.5 LMnCe RMnAb tie */
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 11, 0, 11, 0, 0, "Z(Ci,Mn)");
  dpd_->buf4_init(&L, PSIF_CC_GL, L_irr, 0, 5, 0, 5, 0, "LIjAb");
  dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tia");
  dpd_->contract424(&L, &T1, &Z, 3, 1, 1, 1.0, 0.0);
  dpd_->file2_close(&T1);
  dpd_->buf4_close(&L);
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 11, 5, 11, 5, 0, "GCiAb");
  ...
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 11, 2, 11, 2, 0, "Z(CI,MN)");
  dpd_->buf4_init(&L, PSIF_CC_GL, L_irr, 2, 5, 2, 7, 0, "LIJAB");
  dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  dpd_->contract424(&L, &R1, &Z, 3, 1, 1, 1.0, 0.0);
  dpd_->file2_close(&R1);
  dpd_->buf4_close(&L);
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 11, 7, 11, 7, 0, "GCIAB");
  ...
  dpd_->buf4_close(&Z);
  /* term 4, rho_CiAb -= 0.5 LMnCe TauMnAb Rie */
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 11, 0, 11, 0, 0, "Z(Ci,Mn)");
  dpd_->buf4_init(&L, PSIF_CC_GL, L_irr, 0, 5, 0, 5, 0, "LIjAb");
  dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "Ria");
  dpd_->contract424(&L, &R1, &Z, 3, 1, 1, 1.0, 0.0);
  dpd_->file2_close(&R1);
  ...
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 11, 7, 11, 7, 0, "GCIAB");
  dpd_->buf4_init(&Tau, PSIF_CC_TAMPS, 0, 0, 7, 2, 7, 0, "tauIJAB");
  dpd_->file2_init(&I1, PSIF_EOM_TMP, G_irr, 0, 1, "L2R1_OV");
  dpd_->contract244(&I1, &Tau, &G, 0, 0, 0, 1.0, 1.0);
  dpd_->file2_close(&I1);
  dpd_->buf4_close(&Tau);
  dpd_->buf4_close(&G);
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 11, 7, 11, 7, 0, "Gciab");
  ...
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 2, 11, 2, 11, 0, "Z(NM,CI)");
  dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tIA");
  dpd_->buf4_init(&L, PSIF_CC_GL, L_irr, 2, 5, 2, 7, 0, "LIJAB");
  dpd_->contract424(&L, &T1, &Z, 3, 1, 0, 1.0, 0.0);
  dpd_->buf4_close(&L);
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 0, 11, 2, 11, 0, "Z(NM,CI)");
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, L_irr, 11, 11, 11, 11, 0, "Z(CI,AM)");
  dpd_->contract244(&T1, &Z, &Z2, 0, 0, 1, 1.0, 0.0);
  dpd_->file2_close(&T1);
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 11, 5, 11, 5, 0, "Z(CI,AB)");
  dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  dpd_->contract424(&Z2, &R1, &Z, 3, 0, 0, 1.0, 0.0);
  dpd_->file2_close(&R1);
  dpd_->buf4_close(&Z2);
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 11, 5, 11, 7, 0, "GCIAB");
  dpd_->buf4_axpy(&Z, &G, -1.0);
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP1, pqsr, 11, 5, "Z(CI,BA)");
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 11, 5, 11, 5, 0, "Z(CI,BA)");
  dpd_->buf4_axpy(&Z, &G, 1.0);
  dpd_->buf4_close(&Z);
  dpd_->buf4_close(&G);
  ...
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 0, 11, 0, 11, 0, "Z(Nm,Ci)");
  dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tia");
  dpd_->buf4_init(&L, PSIF_CC_GL, L_irr, 0, 5, 0, 5, 0, "LIjAb");
  dpd_->contract424(&L, &T1, &Z, 3, 1, 0, 1.0, 0.0);
  dpd_->buf4_close(&L);
  ...
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 11, 5, 11, 5, 0, "GCiAb");
  dpd_->file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "Ria");
  dpd_->contract424(&Z2, &R1, &G, 3, 0, 0, -1.0, 1.0);
  dpd_->file2_close(&R1);
  dpd_->buf4_close(&Z2);
  dpd_->buf4_close(&G);
  ...
  dpd_->buf4_close(&L);
  dpd_->file2_close(&T1);
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, L_irr, 11, 11, 11, 11, 0, "Z(Ci,bM)");
  dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tia");
  dpd_->contract244(&T1, &Z, &Z2, 0, 0, 1, 1.0, 0.0);
  dpd_->file2_close(&T1);
  dpd_->buf4_close(&Z);
  ...
  dpd_->file2_close(&R1);
  dpd_->buf4_close(&Z2);
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP1, pqsr, 11, 5, "Z(Ci,Ab)");
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 11, 5, 11, 5, 0, "Z(Ci,Ab)");
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 11, 5, 11, 5, 0, "GCiAb");
  dpd_->buf4_axpy(&Z, &G, -1.0);
  ...
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 11, 7, 11, 7, 0, "GCIAB");
  dpd_->buf4_init(&V, PSIF_CC_GAMMA, G_irr, 11, 7, 11, 7, 0, "GCIAB");
  ...
  dpd_->buf4_close(&G);
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 11, 5, 11, 5, 0, "GCiAb");
  dpd_->buf4_init(&V, PSIF_CC_GAMMA, G_irr, 11, 5, 11, 5, 0, "GCiAb");
  dpd_->buf4_axpy(&G, &V, 0.5);
  dpd_->buf4_close(&V);
  dpd_->buf4_close(&G);
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 11, 5, 11, 5, 0, "GcIaB");
...
void x_Gciab_6(void) { 
  ...
  dpd_->file2_init(&LR1A, PSIF_EOM_TMP, G_irr, 1, 1, "LR_VV");
  dpd_->file2_init(&T1A, PSIF_CC_OEI, 0, 0, 1, "tIA");
  if (params.ref == 0 || params.ref == 1) {
    dpd_->file2_init(&LR1B, PSIF_EOM_TMP, G_irr, 1, 1, "LR_vv");
    dpd_->file2_init(&T1B, PSIF_CC_OEI, 0, 0, 1, "tia");
  }
  else {
    dpd_->file2_init(&LR1B, PSIF_EOM_TMP, G_irr, 3, 3, "LR_vv");
    dpd_->file2_init(&T1B, PSIF_CC_OEI, 0, 2, 3, "tia");
  }
  dpd_->file2_mat_init(&T1A);   dpd_->file2_mat_init(&T1B);
  dpd_->file2_mat_init(&LR1A);   dpd_->file2_mat_init(&LR1B);
  dpd_->file2_mat_rd(&T1A);     dpd_->file2_mat_rd(&T1B);
  dpd_->file2_mat_rd(&LR1A);     dpd_->file2_mat_rd(&LR1B);

  /* rho_CIAB += LR1_VV(C,A) T(I,B) - LR1_VV(C,B) T(I,A) */
  if (params.ref == 0 || params.ref == 1)
    dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 11, 5, 11, 7, 0, "GCIAB");
  else
    dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 21, 5, 21, 7, 0, "GCIAB");
  for(h=0; h < nirreps; h++) {
  ...
  if (params.ref == 0 || params.ref == 1)
    ...
    dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 31, 15, 31, 17, 0, "Gciab");
  for(h=0; h < nirreps; h++) {
    dpd_->buf4_mat_irrep_init(&G, h);
    dpd_->buf4_mat_irrep_rd(&G, h);
    for(row=0; row < G.params->rowtot[h]; row++) {
  ...
  if (params.ref == 0 || params.ref == 1)
    ...
    dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 26, 28, 26, 28, 0, "GCiAb");
  for(h=0; h < nirreps; h++) {
    dpd_->buf4_mat_irrep_init(&G, h);
    dpd_->buf4_mat_irrep_rd(&G, h);
    for(row=0; row < G.params->rowtot[h]; row++) {
  ...
  if (params.ref == 0 || params.ref == 1)
    ...
    dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 25, 29, 25, 29, 0, "GcIaB");
  for(h=0; h < nirreps; h++) {
    dpd_->buf4_mat_irrep_init(&G, h);
    dpd_->buf4_mat_irrep_rd(&G, h);
    for(row=0; row < G.params->rowtot[h]; row++) {
  ...
  dpd_->file2_mat_close(&LR1A);
  dpd_->file2_mat_close(&LR1B);
  dpd_->file2_close(&LR1A);
  dpd_->file2_close(&LR1B);
...
void x_Gciab_7(void) { 
  ...
  /* open one-electron files for the nasty terms */
  dpd_->file2_init(&LT1A, PSIF_EOM_TMP, L_irr, 1, 1, "LT_VV");
  dpd_->file2_init(&R1A, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  if (params.ref == 0 || params.ref == 1) {
    dpd_->file2_init(&LT1B, PSIF_EOM_TMP, L_irr, 1, 1, "LT_vv");
    dpd_->file2_init(&R1B, PSIF_CC_GR, R_irr, 0, 1, "Ria");
  }
  else {
    dpd_->file2_init(&LT1B, PSIF_EOM_TMP, L_irr, 3, 3, "LT_vv");
    dpd_->file2_init(&R1B, PSIF_CC_GR, R_irr, 2, 3, "Ria");
  }
  dpd_->file2_mat_init(&R1A);   dpd_->file2_mat_init(&R1B);
  dpd_->file2_mat_init(&LT1A);   dpd_->file2_mat_init(&LT1B);
  dpd_->file2_mat_rd(&R1A);     dpd_->file2_mat_rd(&R1B);
  dpd_->file2_mat_rd(&LT1A);     dpd_->file2_mat_rd(&LT1B);
  ...
  dpd_->file2_mat_close(&LT1A);
  dpd_->file2_mat_close(&LT1B);
  dpd_->file2_close(&LT1A);
  dpd_->file2_close(&LT1B);
...
void x_Gciab_8_rohf(void) { 
  ...
  /* term 8, +P(AB) LMNCE RINAE TMB */
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 5, 10, 5, 0, "Z(IA,BC)");
  dpd_->buf4_init(&V, PSIF_EOM_TMP, G_irr, 10, 10, 10, 10, 0, "R2L2_OVOV");
  dpd_->file2_init(&T1A, PSIF_CC_OEI, 0, 0, 1, "tIA");
  dpd_->contract244(&T1A, &V, &Z, 0, 2, 1, 1.0, 0.0); 
  dpd_->file2_close(&T1A);
  dpd_->buf4_close(&V);
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP1, sprq, 11, 5, "Z(CI,BA)");
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 11, 5, 11, 5, 0, "Z(CI,BA)");
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 11, 5, 11, 7, 0, "GCIAB");
  dpd_->buf4_axpy(&Z, &G, 1.0);
  ...
  dpd_->buf4_init(&V, PSIF_EOM_TMP, G_irr, 10, 10, 10, 10, 0, "R2L2_oVoV");
  dpd_->buf4_sort(&V, PSIF_EOM_TMP1, spqr, 11, 11, "Z(Ci,Am)");
  dpd_->buf4_close(&V);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 11, 11, 11, 11, 0, "Z(Ci,Am)");
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 11, 5, 11, 5, 0, "GCiAb");
  dpd_->file2_init(&T1B, PSIF_CC_OEI, 0, 0, 1, "tia");
  ...
  dpd_->buf4_init(&V, PSIF_EOM_TMP, G_irr, 10, 10, 10, 10, 0, "R2L2_ovOV");
  dpd_->buf4_sort(&V, PSIF_EOM_TMP1, sprq, 11, 10, "Z(Ci,Mb)");
  dpd_->buf4_close(&V);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 11, 10, 11, 10, 0, "Z(Ci,Mb)");
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 11, 5, 11, 5, 0, "GCiAb");
  dpd_->file2_init(&T1A, PSIF_CC_OEI, 0, 0, 1, "tIA");
  dpd_->contract244(&T1A, &Z, &G, 0, 2, 1, 1.0, 1.0); 
  dpd_->file2_close(&T1A);
  dpd_->buf4_close(&G);
  dpd_->buf4_close(&Z);
  ...
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 11, 5, 11, 5, 0, "GcIaB");
  dpd_->file2_init(&T1B, PSIF_CC_OEI, 0, 0, 1, "tia");
  dpd_->contract244(&T1B, &Z, &G, 0, 2, 1, 1.0, 1.0); 
  dpd_->file2_close(&T1B);
  dpd_->buf4_close(&G);
  dpd_->buf4_close(&Z);
  ...
  dpd_->file2_init(&R1A, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  dpd_->contract244(&R1A, &V, &Z, 0, 2, 1, 1.0, 0.0); 
  dpd_->file2_close(&R1A);
  dpd_->buf4_close(&V);
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP1, sprq, 11, 5, "Z(CI,BA)");
  dpd_->buf4_close(&Z);
  ...
  dpd_->buf4_init(&V, PSIF_EOM_TMP, L_irr, 10, 10, 10, 10, 0, "Viajb");
  dpd_->file2_init(&R1B, PSIF_CC_GR, R_irr, 0, 1, "Ria");
  dpd_->contract244(&R1B, &V, &Z, 0, 2, 1, 1.0, 0.0); 
  dpd_->file2_close(&R1B);
  dpd_->buf4_close(&V);
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP1, sprq, 11, 5, "Z(ci,ba)");
  dpd_->buf4_close(&Z);
  ...
  dpd_->buf4_sort(&V, PSIF_EOM_TMP1, spqr, 11, 11, "Z(Ci,Am)");
  dpd_->buf4_close(&V);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 11, 11, 11, 11, 0, "Z(Ci,Am)");
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 11, 5, 11, 5, 0, "GCiAb");
  dpd_->file2_init(&R1B, PSIF_CC_GR, R_irr, 0, 1, "Ria");
  ...
  dpd_->buf4_sort(&V, PSIF_EOM_TMP1, sprq, 11, 10, "Z(Ci,Mb)");
  dpd_->buf4_close(&V);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 11, 10, 11, 10, 0, "Z(Ci,Mb)");
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 11, 5, 11, 5, 0, "GCiAb");
  dpd_->file2_init(&R1A, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  dpd_->contract244(&R1A, &Z, &G, 0, 2, 1, 1.0, 1.0); 
  dpd_->file2_close(&R1A);
  dpd_->buf4_close(&G);
  dpd_->buf4_close(&Z);
  ...
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 11, 5, 11, 5, 0, "GcIaB");
  dpd_->file2_init(&R1B, PSIF_CC_GR, R_irr, 0, 1, "Ria");
  dpd_->contract244(&R1B, &Z, &G, 0, 2, 1, 1.0, 1.0); 
  dpd_->file2_close(&R1B);
  dpd_->buf4_close(&G);
  dpd_->buf4_close(&Z);
