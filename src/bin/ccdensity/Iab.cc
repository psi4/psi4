...
namespace psi { namespace ccdensity {
    ...
    */
	...
	dpd_->contract222(&F, &D, &I, 1, 1, 1.0, 0.0);
	dpd_->file2_close(&D);
	...
	for(Gac=0; Gac < nirreps; Gac++) {
	  ...
	  for(Ga=0; Ga < nirreps; Ga++) {
	    Gb = Ga;
	    Gc = Ga ^ Gac;
	    Bints.matrix[Gac] = dpd_->dpd_block_matrix(virtpi[Gc], Bints.params->coltot[Gac]);
	    G.matrix[Gbc] = dpd_->dpd_block_matrix(virtpi[Gc], G.params->coltot[Gbc]);
	    for(a=0; a < virtpi[Ga]; a++) {
	      A = vir_off[Ga] + a;
	      for(b=0; b < virtpi[Gb]; b++) {
		B = vir_off[Gb] + b;
		dpd_->buf4_mat_irrep_rd_block(&Bints, Gac, Bints.row_offset[Gac][A], virtpi[Gc]);
		dpd_->buf4_mat_irrep_rd_block(&G, Gbc, G.row_offset[Gbc][B], virtpi[Gc]);
		length = virtpi[Gc] * Bints.params->coltot[Gac];
		if(length)
		  I.matrix[Ga][a][b] += 2.0 * C_DDOT(length, Bints.matrix[Gac][0], 1, G.matrix[Gbc][0], 1);
	      }
	    }
	    dpd_->free_dpd_block(Bints.matrix[Gac], virtpi[Gc], Bints.params->coltot[Gac]);
	    dpd_->free_dpd_block(G.matrix[Gbc], virtpi[Gc], G.params->coltot[Gbc]);
	  }
	}
	...
	for(Gac=0; Gac < nirreps; Gac++) {
	  ...
	  for(Gc=0; Gc < nirreps; Gc++) {
	    Ga = Gc ^ Gac;
	    Gb = Ga;
	    Bints.matrix[Gac] = dpd_->dpd_block_matrix(virtpi[Ga], Bints.params->coltot[Gac]);
	    G.matrix[Gbc] = dpd_->dpd_block_matrix(virtpi[Gb], G.params->coltot[Gbc]);
	    for(c=0; c < virtpi[Gc]; c++) {
	      C = vir_off[Gc] + c;
	      dpd_->buf4_mat_irrep_rd_block(&Bints, Gac, Bints.row_offset[Gac][C], virtpi[Ga]);
	      dpd_->buf4_mat_irrep_rd_block(&G, Gbc, G.row_offset[Gbc][C], virtpi[Gb]);
	      for(a=0; a < virtpi[Ga]; a++) {
		for(b=0; b < virtpi[Gb]; b++) {
	    ...
	    dpd_->free_dpd_block(Bints.matrix[Gac], virtpi[Ga], Bints.params->coltot[Gac]);
	    dpd_->free_dpd_block(G.matrix[Gbc], virtpi[Gb], G.params->coltot[Gbc]);
	  }
	}
	dpd_->file2_mat_wrt(&I);
	dpd_->file2_mat_close(&I);
	...
	dpd_->buf4_init(&Fints, PSIF_CC_FINTS, 0, 10, 5, 10, 5, 0, "F <ia|bc>");
	dpd_->buf4_sort(&Fints, PSIF_CC_TMP0, qpsr, 11, 5, "F(Ai,Cd)");
	dpd_->buf4_close(&Fints);
	dpd_->buf4_init(&Fints, PSIF_CC_TMP0, 0, 11, 5, 11, 5, 0, "F(Ai,Cd)");
	dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 11, 5, 11, 5, 0, "GCiAb");
	dpd_->contract442(&Fints, &G, &I, 0, 0, 2.0, 1.0);
	dpd_->buf4_close(&G);
	dpd_->buf4_close(&Fints);
	...
	dpd_->buf4_sort(&Fints, PSIF_CC_TMP0, qprs, 11, 5, "F (DI,CA)");
	dpd_->buf4_close(&Fints);
	dpd_->buf4_init(&Fints, PSIF_CC_TMP0, 0, 11, 5, 11, 5, 0, "F (DI,CA)");
	...
	dpd_->buf4_init(&Fints, PSIF_CC_FINTS, 0, 10, 5, 10, 5, 0, "F <ia|bc>");
	dpd_->buf4_sort(&Fints, PSIF_CC_TMP0, qpsr, 11, 5, "F (Di,Ac)");
	dpd_->buf4_close(&Fints);
	dpd_->buf4_init(&Fints, PSIF_CC_TMP0, 0, 11, 5, 11, 5, 0, "F (Di,Ac)");
	dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 11, 5, 11, 5, 0, "GCiAb");
	dpd_->contract442(&Fints, &G, &I, 2, 2, 2.0, 1.0);
	dpd_->buf4_close(&G);
	dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 11, 5, 11, 5, 0, "GcIaB");
	...
	dpd_->buf4_init(&Fints, PSIF_CC_FINTS, 0, 21, 5, 21, 5, 1, "F <AI|BC>");
	...
	dpd_->buf4_init(&Fints, PSIF_CC_FINTS, 0, 31, 15, 31, 15, 1, "F <ai|bc>");
	...
	dpd_->buf4_init(&G, PSIF_CC_GAMMA, 0, 10, 10, 10, 10, 0, "GIbjA");
	dpd_->buf4_sort(&G, PSIF_CC_TMP0, rpqs, 0, 5, "GIbjA (jI,bA)");
	dpd_->buf4_close(&G);
	dpd_->buf4_init(&G, PSIF_CC_TMP0, 0, 0, 5, 0, 5, 0, "GIbjA (jI,bA)");
	...
	dpd_->buf4_sort(&G, PSIF_CC_TMP0, rpqs, 23, 29, "GIbjA (jI,bA)");
	dpd_->buf4_close(&G);
	dpd_->buf4_init(&G, PSIF_CC_TMP0, 0, 23, 29, 23, 29, 0, "GIbjA (jI,bA)");
	dpd_->contract442(&Dints, &G, &I, 3, 3, -2.0, 1.0);
	dpd_->buf4_close(&G);
	dpd_->buf4_close(&Dints);
	...
	dpd_->buf4_sort(&G, PSIF_CC_TMP0, rpqs, 22, 28, "GiBJa (Ji,Ba)");
	dpd_->buf4_close(&G);
	dpd_->buf4_init(&G, PSIF_CC_TMP0, 0, 22, 28, 22, 28, 0, "GiBJa (Ji,Ba)");
	dpd_->contract442(&Dints, &G, &I, 3, 3, -2.0, 1.0);
	dpd_->buf4_close(&G);
	dpd_->buf4_close(&Dints);
