...
void x_Gijab_ROHF(void)
{
  ...
  if (G_irr == 0) {
    ...
    dpd_buf4_copy(&T, PSIF_EOM_TMP0, "GIJAB");
    dpd_buf4_close(&T);
    dpd_buf4_init(&T, PSIF_CC_TAMPS, 0, 2, 7, 2, 7, 0, "tauijab");
    dpd_buf4_copy(&T, PSIF_EOM_TMP0, "Gijab");
    dpd_buf4_close(&T);
    ...
    dpd_buf4_init(&G, PSIF_EOM_TMP0, G_irr, 2, 7, 2, 7, 0, "GIJAB");
    dpd_buf4_scm(&G,0.0);
    dpd_buf4_close(&G);
    dpd_buf4_init(&G, PSIF_EOM_TMP0, G_irr, 2, 7, 2, 7, 0, "Gijab");
    dpd_buf4_scm(&G,0.0);
    dpd_buf4_close(&G);
    dpd_buf4_init(&G, PSIF_EOM_TMP0, G_irr, 0, 5, 0, 5, 0, "GIjAb");
    dpd_buf4_scm(&G,0.0);
    dpd_buf4_close(&G);
  }

  /* -P(ij) LR_OO(M,I) Tau(MJ,AB); terms 4,5,8,9 */
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 7, 0, 7, 0, "Z(IJ,A>B)");
  dpd_file2_init(&I1, PSIF_EOM_TMP, G_irr, 0, 0, "LR_OO");
  dpd_file2_init(&Z1, PSIF_EOM_TMP1, G_irr, 0, 0, "Z(N,I)");
  ...
  dpd_file2_close(&Z1);
  dpd_buf4_close(&T);
  dpd_buf4_init(&G, PSIF_EOM_TMP0, G_irr, 0, 7, 2, 7, 0, "GIJAB");
  dpd_buf4_axpy(&Z, &G, -1.0);
  dpd_buf4_sort(&Z, PSIF_EOM_TMP1, qprs, 0, 7, "Z(JI,A>B)");
  dpd_buf4_close(&Z);
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 7, 0, 7, 0, "Z(JI,A>B)");
  dpd_buf4_axpy(&Z, &G, 1.0);
  dpd_buf4_close(&Z);
  dpd_buf4_close(&G);
  ...
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "Z(jI,bA)");
  dpd_file2_init(&I1, PSIF_EOM_TMP, G_irr, 0, 0, "LR_oo");
  dpd_file2_init(&Z1, PSIF_EOM_TMP1, G_irr, 0, 0, "Z(n,i)");
  dpd_file2_axpy(&I1, &Z1, 1.0, 0);
  dpd_file2_close(&I1);
  ...
  dpd_file2_close(&Z1);
  dpd_buf4_close(&T);
  dpd_buf4_sort(&Z, PSIF_EOM_TMP1, qpsr, 0, 5, "Z(Ij,Ab)");
  dpd_buf4_close(&Z);
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "Z(Ij,Ab)");
  dpd_buf4_axpy(&Z, &G, -1.0);
  dpd_buf4_close(&G);
  ...
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 2, 5, 2, 5, 0, "Z(I>J,AB)");
  ...
  dpd_file2_close(&I1);
  dpd_buf4_close(&T);
  dpd_buf4_init(&G, PSIF_EOM_TMP0, G_irr, 2, 5, 2, 7, 0, "GIJAB");
  dpd_buf4_axpy(&Z, &G, -1.0);
  dpd_buf4_sort(&Z, PSIF_EOM_TMP1, pqsr, 2, 5, "Z(I>J,BA)");
  dpd_buf4_close(&Z);
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 2, 5, 2, 5, 0, "Z(I>J,BA)");
  dpd_buf4_axpy(&Z, &G, 1.0);
  dpd_buf4_close(&Z);
  dpd_buf4_close(&G);
  ...
  dpd_file2_close(&I1);
  dpd_buf4_close(&T);
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "Z(Ij,bA)");
  dpd_buf4_init(&T, PSIF_CC_TAMPS, 0, 0, 5, 0, 5, 0, "tauIjbA");
  dpd_file2_init(&I1, PSIF_EOM_TMP, G_irr, 1, 1, "LR_vv");
  dpd_contract244(&I1, &T, &Z, 0, 2, 1, 1.0, 0.0);
  dpd_file2_close(&I1);
  dpd_buf4_close(&T);
  dpd_buf4_sort(&Z, PSIF_EOM_TMP1, pqsr, 0, 5, "Z(Ij,Ab)");
  dpd_buf4_close(&Z);
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "Z(Ij,Ab)");
  dpd_buf4_axpy(&Z, &G, -1.0);
  dpd_buf4_close(&G);
  ...
  dpd_buf4_init(&G, PSIF_EOM_TMP0, G_irr, 2, 7, 2, 7, 0, "GIJAB");
  dpd_buf4_init(&I, PSIF_EOM_TMP, L_irr, 2, 2, 2, 2, 0, "Tau2L2_OOOO");
  ...
  dpd_buf4_close(&G);
  dpd_buf4_init(&G, PSIF_EOM_TMP0, G_irr, 0, 5, 0, 5, 0, "GIjAb");
  dpd_buf4_init(&I, PSIF_EOM_TMP, L_irr, 0, 0, 0, 0, 0, "Tau2L2_OoOo");
  ...
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 2, 0, 2, 0, "Z(IJ,M>N)");
  dpd_buf4_init(&I, PSIF_EOM_TMP, G_irr, 2, 10, 2, 10, 0, "L2R1_OOVO(pqsr)");
  dpd_file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tIA");
  dpd_contract424(&I, &T1, &Z, 3, 1, 1, 1.0, 0.0);
  dpd_file2_close(&T1);
  dpd_buf4_close(&I);
  /* add terms 12, 14 */
  dpd_buf4_init(&I, PSIF_EOM_TMP, G_irr, 0, 2, 2, 2, 0, "R2L2_OOOO");
  ...
  dpd_buf4_sort(&Z2, PSIF_EOM_TMP1, qprs, 0, 7, "Z(JI,A>B)");
  dpd_buf4_close(&Z2);
  dpd_buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 0, 7, 0, 7, 0, "Z(JI,A>B)");
  dpd_buf4_axpy(&Z2, &G, 1.0);
  dpd_buf4_close(&Z2);
  dpd_buf4_close(&G);
  ...
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 0, 0, 0, 0, "Z(Ij,Mn)");
  dpd_buf4_init(&I, PSIF_EOM_TMP, G_irr, 0, 10, 0, 10, 0, "L2R1_OovO(pqsr)");
  dpd_file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tia");
  dpd_contract424(&I, &T1, &Z, 3, 1, 1, 1.0, 0.0);
  dpd_file2_close(&T1);
  dpd_buf4_close(&I);
  dpd_buf4_init(&I, PSIF_EOM_TMP, G_irr, 0, 11, 0, 11, 0, "L2R1_OoVo");
  dpd_file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tIA");
  dpd_contract244(&T1, &I, &Z, 1, 2, 0, 1.0, 1.0);
  dpd_file2_close(&T1);
  dpd_buf4_close(&I);
  /* add terms 12, 14 */
  dpd_buf4_init(&I, PSIF_EOM_TMP, G_irr, 0, 0, 0, 0, 0, "R2L2_OoOo");
  ...
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 2, 11, 2, 11, 0, "Z(I>J,AN)");
  dpd_buf4_init(&I, PSIF_EOM_TMP, L_irr, 2, 0, 2, 2, 0, "Tau2L2_OOOO");
  ...
  dpd_buf4_init(&G, PSIF_EOM_TMP0, G_irr, 2, 5, 2, 7, 0, "GIJAB");
  dpd_buf4_axpy(&Z2, &G, 1.0);
  dpd_buf4_sort(&Z2, PSIF_EOM_TMP1, pqsr, 2, 5, "Z(I>J,BA)");
  dpd_buf4_close(&Z2);
  dpd_buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 2, 5, 2, 5, 0, "Z(I>J,BA)");
  dpd_buf4_axpy(&Z2, &G, -1.0);
  dpd_buf4_close(&Z2);
  dpd_buf4_close(&G);
  /* + 0.5 P(ab) (tau_ijef Lmnef) Rma Tnb ; terms 25, 27 */
  ...
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 11, 0, 11, 0, "Z(Ij,An)");
  dpd_buf4_init(&I, PSIF_EOM_TMP, L_irr, 0, 0, 0, 0, 0, "Tau2L2_OoOo");
  dpd_file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  dpd_contract244(&R1, &I, &Z, 0, 2, 1, 1.0, 0.0);
  dpd_file2_close(&R1);
  ...
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 0, 11, 0, 11, 0, "Z2(Ij,An)");
  dpd_buf4_init(&I, PSIF_EOM_TMP, L_irr, 0, 0, 0, 0, 0, "Tau2L2_OoOo");
  dpd_file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tIA");
  ...
  dpd_file2_init(&I1, PSIF_EOM_TMP, L_irr, 0, 0, "LT_OO");
  ...
  dpd_file2_close(&I1);
  dpd_buf4_close(&R);
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "Z(jI,Ab)");
  dpd_buf4_init(&R, PSIF_CC_GR, R_irr, 0, 5, 0, 5, 0, "RiJAb");
  dpd_file2_init(&I1, PSIF_EOM_TMP, L_irr, 0, 0, "LT_oo");
  dpd_contract244(&I1, &R, &Z, 0, 0, 0, 1.0, 0.0);
  dpd_file2_close(&I1);
  dpd_buf4_close(&R);
  dpd_buf4_sort(&Z, PSIF_EOM_TMP1, qprs, 0, 5, "Z(Ij,Ab)");
  dpd_buf4_close(&Z);
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "Z(Ij,Ab)");
  dpd_buf4_axpy(&Z, &G, -1.0);
  dpd_buf4_close(&Z);
  ...
  dpd_file2_init(&I1, PSIF_EOM_TMP, L_irr, 1, 1, "LT_VV");
  ...
  if (!params.connect_xi) {
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 2, 11, 2, 11, 0, "Z(I>J,AN)");
  dpd_buf4_init(&T, PSIF_CC_TAMPS, 0, 2, 5, 2, 7, 0, "tIJAB");
  dpd_file2_init(&I1, PSIF_EOM_TMP, G_irr, 0, 1, "L2R1_OV");
  ...
  dpd_buf4_close(&Z2);
  dpd_buf4_init(&G, PSIF_EOM_TMP0, G_irr, 2, 5, 2, 7, 0, "GIJAB");
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, 0, 2, 5, 2, 5, 0, "Z(I>J,AB)");
  dpd_buf4_axpy(&Z, &G, -1.0);
  dpd_buf4_sort(&Z, PSIF_EOM_TMP1, pqsr, 2, 5, "Z(I>J,BA)");
  dpd_buf4_close(&Z);
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, 0, 2, 5, 2, 5, 0, "Z(I>J,BA)");
  dpd_buf4_axpy(&Z, &G, 1.0);
  dpd_buf4_close(&G);
  ...
  dpd_buf4_close(&Z);
  dpd_buf4_close(&G);
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 10, 0, 10, 0, "Z(Ij,Nb)");
  dpd_buf4_init(&T, PSIF_CC_TAMPS, 0, 0, 5, 0, 5, 0, "tIjAb");
  dpd_file2_init(&I1, PSIF_EOM_TMP, G_irr, 0, 1, "L2R1_OV");
  ...
  dpd_buf4_init(&Z, PSIF_EOM_TMP0, G_irr, 10, 10, 10, 10, 0, "Z(IA,JB)");
  dpd_buf4_init(&I, PSIF_EOM_TMP, G_irr, 10, 10, 10, 10, 0, "R2L2_OVOV");
  ...
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 10, 10, 10, 0, "Z(Ib,jA)");
  dpd_buf4_init(&I, PSIF_EOM_TMP, G_irr, 10, 10, 10, 10, 0, "R2L2_oVoV");
  ...
  dpd_buf4_close(&I);
  dpd_buf4_close(&T);
  dpd_buf4_sort_axpy(&Z, PSIF_EOM_TMP0, psrq, 10, 10, "Z(IA,jb)", 1.0);
  dpd_buf4_close(&Z);
  ...
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 10, 0, 10, 0, 0, "Z(JB,NI)");
  dpd_buf4_init(&I, PSIF_EOM_TMP, L_irr, 10, 10, 10, 10, 0, "VIAJB");
  dpd_file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tIA");
  dpd_contract424(&I, &T1, &Z, 3, 1, 0, 1.0, 0.0);
  dpd_file2_close(&T1);
  dpd_buf4_close(&I);
  dpd_buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 11, 10, 11, 10, 0, "Z(AI,JB)");
  dpd_file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  dpd_contract244(&R1, &Z, &Z2, 0, 2, 0, 1.0, 0.0);
  dpd_file2_close(&R1);
  dpd_buf4_close(&Z);
  dpd_buf4_sort_axpy(&Z2, PSIF_EOM_TMP0, qprs, 10, 10, "Z(IA,JB)", -1.0);
  dpd_buf4_close(&Z2);
  ...
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 10, 0, 10, 0, 0, "Z(Ib,Nj)");
  dpd_buf4_init(&I, PSIF_EOM_TMP, L_irr, 10, 10, 10, 10, 0, "VIaJb");
  dpd_file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tia");
  dpd_contract424(&I, &T1, &Z, 3, 1, 0, 1.0, 0.0);
  dpd_file2_close(&T1);
  dpd_buf4_close(&I);
  dpd_buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 10, 11, 10, 11, 0, "Z(Ib,Aj)");
  dpd_file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  dpd_contract244(&R1, &Z, &Z2, 0, 2, 1, 1.0, 0.0);
  dpd_file2_close(&R1);
  dpd_buf4_close(&Z);
  dpd_buf4_sort_axpy(&Z2, PSIF_EOM_TMP0, prsq, 10, 10, "Z(IA,jb)", +1.0);
  dpd_buf4_close(&Z2);

  psio_close(PSIF_EOM_TMP1,0);
  psio_open(PSIF_EOM_TMP1,PSIO_OPEN_NEW);

  dpd_buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 10, 0, 10, 0, 0, "Z(jA,nI)");
  dpd_buf4_init(&I, PSIF_EOM_TMP, L_irr, 10, 10, 10, 10, 0, "ViAjB");
  dpd_file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tIA");
  dpd_contract424(&I, &T1, &Z, 3, 1, 0, 1.0, 0.0);
  dpd_file2_close(&T1);
  dpd_buf4_close(&I);
  dpd_buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 11, 10, 11, 10, 0, "Z(bI,jA)");
  dpd_file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "Ria");
  dpd_contract244(&R1, &Z, &Z2, 0, 2, 0, 1.0, 0.0);
  dpd_file2_close(&R1);
  dpd_buf4_close(&Z);
  dpd_buf4_sort(&Z2, PSIF_EOM_TMP1, sqrp, 11, 10, "Z(AI,jb)");
  dpd_buf4_close(&Z2);
  dpd_buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 11, 10, 11, 10, 0, "Z(AI,jb)");
  dpd_buf4_sort_axpy(&Z2, PSIF_EOM_TMP0, qprs, 10, 10, "Z(IA,jb)", +1.0);
  dpd_buf4_close(&Z2);

  dpd_buf4_init(&Z, PSIF_EOM_TMP1, L_irr, 10, 0, 10, 0, 0, "Z(IA,nj)");
  dpd_buf4_init(&I, PSIF_EOM_TMP, L_irr, 10, 10, 10, 10, 0, "VIAjb");
  dpd_file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tia");
  dpd_contract424(&I, &T1, &Z, 3, 1, 0, 1.0, 0.0);
  dpd_file2_close(&T1);
  dpd_buf4_close(&I);
  dpd_buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 10, 11, 10, 11, 0, "Z(IA,bj)");
  dpd_file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "Ria");
  dpd_contract244(&R1, &Z, &Z2, 0, 2, 1, 1.0, 0.0);
  dpd_file2_close(&R1);
  dpd_buf4_close(&Z);
  dpd_buf4_sort_axpy(&Z2, PSIF_EOM_TMP0, pqsr, 10, 10, "Z(IA,jb)", -1.0);
  dpd_buf4_close(&Z2);
  ...
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 0, 10, 0, 0, "Z(JB,NI)");
  dpd_buf4_init(&I, PSIF_EOM_TMP, L_irr, 10, 10, 10, 10, 0, "VIAJB");
  dpd_file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  ...
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 0, 10, 0, 0, "Z(Ib,Nj)");
  dpd_buf4_init(&I, PSIF_EOM_TMP, L_irr, 10, 10, 10, 10, 0, "VIaJb");
  dpd_file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "Ria");
  dpd_contract424(&I, &R1, &Z, 3, 1, 0, 1.0, 0.0);
  dpd_file2_close(&R1);
  ...
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 0, 10, 0, 0, "Z(jA,nI)");
  dpd_buf4_init(&I, PSIF_EOM_TMP, L_irr, 10, 10, 10, 10, 0, "ViAjB");
  dpd_file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  dpd_contract424(&I, &R1, &Z, 3, 1, 0, 1.0, 0.0);
  dpd_file2_close(&R1);
  ...
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 0, 10, 0, 0, "Z(IA,nj)");
  dpd_buf4_init(&I, PSIF_EOM_TMP, L_irr, 10, 10, 10, 10, 0, "VIAjb");
  dpd_file2_init(&R1, PSIF_CC_GR, R_irr, 0, 1, "Ria");
  dpd_contract424(&I, &R1, &Z, 3, 1, 0, 1.0, 0.0);
  dpd_file2_close(&R1);
  ...
  dpd_buf4_init(&Z2, PSIF_EOM_TMP0, G_irr, 10, 10, 10, 10, 0, "Z(IA,JB)");
  dpd_buf4_sort(&Z2, PSIF_EOM_TMP1, prqs, 0, 5, "Z(IJ,AB)");
  dpd_buf4_close(&Z2);
  dpd_buf4_init(&G, PSIF_EOM_TMP0, G_irr, 0, 5, 2, 7, 0, "GIJAB");
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "Z(IJ,AB)");
  dpd_buf4_axpy(&Z, &G, 1.0);
  dpd_buf4_sort(&Z, PSIF_EOM_TMP1, qprs, 0, 5, "Z(JI,AB)");
  dpd_buf4_sort(&Z, PSIF_EOM_TMP1, pqsr, 0, 5, "Z(IJ,BA)");
  dpd_buf4_close(&Z);
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "Z(JI,AB)");
  dpd_buf4_axpy(&Z, &G, -1.0);
  dpd_buf4_close(&Z);
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "Z(IJ,BA)");
  dpd_buf4_axpy(&Z, &G, -1.0);
  dpd_buf4_sort(&Z, PSIF_EOM_TMP1, qprs, 0, 5, "Z(JI,BA)");
  dpd_buf4_close(&Z);
  dpd_buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 5, 0, 5, 0, "Z(JI,BA)");
  dpd_buf4_axpy(&Z, &G, 1.0);
  dpd_buf4_close(&Z);
  ...
  dpd_buf4_init(&Z, PSIF_EOM_TMP0, G_irr, 10, 10, 10, 10, 0, "Z(IA,jb)");
  dpd_buf4_sort_axpy(&Z, PSIF_EOM_TMP0, prqs, 0, 5, "GIjAb", 1.0);
  dpd_buf4_close(&Z);

  /* add to ground state parts */
  dpd_buf4_init(&G, PSIF_EOM_TMP0, G_irr, 2, 7, 2, 7, 0, "GIJAB");
  dpd_buf4_init(&V, PSIF_CC_GAMMA, G_irr, 2, 7, 2, 7, 0, "GIJAB");
  ...
  dpd_buf4_close(&G);
  dpd_buf4_init(&G, PSIF_EOM_TMP0, G_irr, 0, 5, 0, 5, 0, "GIjAb");
  dpd_buf4_init(&V, PSIF_CC_GAMMA, G_irr, 0, 5, 0, 5, 0, "GIjAb");
  dpd_buf4_axpy(&G, &V, 0.5);
  dpd_buf4_close(&V);
  dpd_buf4_close(&G);
...
void x_Gijab_ROHF_2(void) { 
  ...
  dpd_file2_init(&L1R2A, PSIF_EOM_TMP, G_irr, 0, 1, "L1R2_OV");
  dpd_file2_init(&L1R2B, PSIF_EOM_TMP, G_irr, 0, 1, "L1R2_ov");
  dpd_file2_copy(&L1R2A, PSIF_EOM_TMP1, "ZIA");
  dpd_file2_copy(&L1R2B, PSIF_EOM_TMP1, "Zia");
  dpd_file2_close(&L1R2A);
  dpd_file2_close(&L1R2B);

  dpd_file2_init(&Z1A, PSIF_EOM_TMP1, G_irr, 0, 1, "ZIA");
  dpd_file2_init(&Z1B, PSIF_EOM_TMP1, G_irr, 0, 1, "Zia");

  /* Z(I,A) += 0.5 (lmnef tmnea) rif, term 22 */
  dpd_file2_init(&I1A, PSIF_EOM_TMP, L_irr, 1, 1, "LT2_VV");
  dpd_file2_init(&R1A, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  dpd_contract222(&R1A, &I1A, &Z1A, 0, 1, -1.0, 1.0);
  dpd_file2_close(&R1A);
  dpd_file2_close(&I1A);
  dpd_file2_init(&I1B, PSIF_EOM_TMP, L_irr, 1, 1, "LT2_vv");
  ...
  dpd_file2_init(&I1A, PSIF_EOM_TMP, L_irr, 0, 0, "LT2_OO");
  dpd_file2_init(&R1A, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  dpd_contract222(&I1A, &R1A, &Z1A, 1, 1, -1.0, 1.0);
  dpd_file2_close(&R1A);
  dpd_file2_close(&I1A);
  dpd_file2_init(&I1B, PSIF_EOM_TMP, L_irr, 0, 0, "LT2_oo");
  dpd_file2_init(&R1B, PSIF_CC_GR, R_irr, 0, 1, "Ria");
  ...
  dpd_file2_init(&I1B, PSIF_EOM_TMP, G_irr, 0, 1, "L2R1_ov");
...
void x_Gijab_ROHF_3(void) { 
  ...
  dpd_file2_init(&L1T2A, PSIF_EOM_TMP, L_irr, 0, 1, "L1T2_OV");
  dpd_file2_init(&L1T2B, PSIF_EOM_TMP, L_irr, 0, 1, "L1T2_ov");
  dpd_file2_copy(&L1T2A, PSIF_EOM_TMP1, "ZIA");
  dpd_file2_copy(&L1T2B, PSIF_EOM_TMP1, "Zia");
  dpd_file2_close(&L1T2A);
  dpd_file2_close(&L1T2B);

  dpd_file2_init(&Z1A, PSIF_EOM_TMP1, L_irr, 0, 1, "ZIA");
  dpd_file2_init(&Z1B, PSIF_EOM_TMP1, L_irr, 0, 1, "Zia");
  ...
  /* Z(i,a) += Lme Tma Tie ; term 19  */
  dpd_file2_init(&I1A, PSIF_EOM_TMP, L_irr, 1, 1, "LT1_VV");
  dpd_file2_init(&T1A, PSIF_CC_OEI, 0, 0, 1, "tIA");
  dpd_contract222(&T1A, &I1A, &Z1A, 0, 1, -1.0, 1.0);
  dpd_file2_close(&T1A);
  dpd_file2_close(&I1A);
  dpd_file2_init(&I1B, PSIF_EOM_TMP, L_irr, 1, 1, "LT1_vv");
  dpd_file2_init(&T1B, PSIF_CC_OEI, 0, 0, 1, "tia");
  dpd_contract222(&T1B, &I1B, &Z1B, 0, 1, -1.0, 1.0);
  dpd_file2_close(&T1B);
  dpd_file2_close(&I1B);
