...
void x_Gijka_rohf(void) { 
  ...
  dpd_->buf4_init(&Z, PSIF_EOM_TMP, G_irr, 2, 10, 2, 10, 0, "L2R1_OOVO(pqsr)");
  dpd_->buf4_copy(&Z, PSIF_EOM_TMP0, "GIJKA");
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP, G_irr, 2, 10, 2, 10, 0, "L2R1_oovo(pqsr)");
  dpd_->buf4_copy(&Z, PSIF_EOM_TMP0, "Gijka");
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP, G_irr, 0, 10, 0, 10, 0, "L2R1_OovO(pqsr)");
  dpd_->buf4_copy(&Z, PSIF_EOM_TMP0, "GIjKa");
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP, G_irr, 0, 10, 0, 10, 0, "L2R1_OoVo(qpsr)");
  dpd_->buf4_copy(&Z, PSIF_EOM_TMP0, "GiJkA");
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP0, G_irr, 0, 10, 0, 10, 0, "GIjKa");
  dpd_->buf4_scm(&Z, -1.0);
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP0, G_irr, 0, 10, 0, 10, 0, "GiJkA");
  dpd_->buf4_scm(&Z, -1.0);
  ...
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 2, 10, 2, 10, 0, "GIJKA");
  dpd_->buf4_init(&Z, PSIF_EOM_TMP, G_irr, 2, 10, 2, 10, 0, "L1R2_OOVO(pqsr)");
  ...
  dpd_->buf4_close(&Z);
  dpd_->buf4_close(&G);
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 0, 10, 0, 10, 0, "GIjKa");
  dpd_->buf4_init(&Z, PSIF_EOM_TMP, G_irr, 0, 10, 0, 10, 0, "L1R2_OovO(pqsr)");
  dpd_->buf4_axpy(&Z, &G, -1.0);
  dpd_->buf4_close(&Z);
  dpd_->buf4_close(&G);
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 0, 10, 0, 10, 0, "GiJkA");
  dpd_->buf4_init(&Z, PSIF_EOM_TMP, G_irr, 0, 10, 0, 10, 0, "L1R2_OoVo(qpsr)");
  dpd_->buf4_axpy(&Z, &G, -1.0);
  dpd_->buf4_close(&Z);
  dpd_->buf4_close(&G);
  ...
  dpd_->buf4_close(&G);
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 0, 10, 0, 10, 0, "GIjKa");
  dpd_->buf4_init(&Z, PSIF_EOM_TMP, G_irr, 0, 0, 0, 0, 0, "R2L2_OoOo");
  dpd_->file2_init(&T1B, PSIF_CC_OEI, 0, 0, 1, "tia");
  dpd_->contract424(&Z, &T1B, &G, 3, 0, 0, 1.0, 1.0);
  dpd_->file2_close(&T1B);
  dpd_->buf4_close(&Z);
  ...
  dpd_->buf4_close(&G);
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 0, 10, 0, 10, 0, "GIjKa");
  dpd_->buf4_init(&Z, PSIF_EOM_TMP, L_irr, 0, 0, 0, 0, 0, "Tau2L2_OoOo");
  dpd_->file2_init(&R1B, PSIF_CC_GR, R_irr, 0, 1, "Ria");
  dpd_->contract424(&Z, &R1B, &G, 3, 0, 0, 1.0, 1.0);
  dpd_->file2_close(&R1B);
  dpd_->buf4_close(&Z);
  ...
  dpd_->buf4_init(&V, PSIF_EOM_TMP, G_irr, 2, 10, 2, 10, 0, "L2R1_OOVO(pqsr)");
  dpd_->file2_init(&T1A, PSIF_CC_OEI, 0, 0, 1, "tIA");
  dpd_->contract424(&V, &T1A, &Z, 3, 1, 1, 1.0, 0.0);
  dpd_->buf4_close(&V);
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 0, 0, 2, 0, "Z5(JI,KM)");
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 0, 10, 0, 10, 0, "Z5(JI,KA)");
  dpd_->contract424(&Z, &T1A, &Z2, 3, 0, 0, 1.0, 0.0);
  dpd_->file2_close(&T1A);
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 0, 10, 2, 10, 0, "GIJKA");
  dpd_->buf4_axpy(&Z2, &G, -1.0);
  dpd_->buf4_sort(&Z2, PSIF_EOM_TMP1, qprs, 0, 10, "Z5(IJ,KA)");
  dpd_->buf4_close(&Z2);
  dpd_->buf4_init(&Z2, PSIF_EOM_TMP1, G_irr, 0, 10, 0, 10, 0, "Z5(IJ,KA)");
  dpd_->buf4_axpy(&Z2, &G, 1.0);
  dpd_->buf4_close(&Z2);
  dpd_->buf4_close(&G);
  /* term 10, +P(ij) lkmef rjf tma tie */
  ...
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 0, 0, 0, 0, "Z(Ij,Km)");
  ...
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 0, 0, 0, 0, "Z(iJ,kM)");
  dpd_->buf4_init(&V, PSIF_EOM_TMP, G_irr, 0, 11, 0, 11, 0, "L2R1_OovO(qprs)");
  dpd_->file2_init(&T1B, PSIF_CC_OEI, 0, 0, 1, "tia");
  dpd_->contract244(&T1B, &V, &Z, 1, 2, 0, 1.0, 0.0);
  dpd_->file2_close(&T1B);
  dpd_->buf4_close(&V);
  dpd_->buf4_init(&V, PSIF_EOM_TMP, G_irr, 0, 10, 0, 10, 0, "L2R1_OoVo(qpsr)");
  dpd_->file2_init(&T1A, PSIF_CC_OEI, 0, 0, 1, "tIA");
  ...
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 2, 10, 2, 10, 0, "GIJKA");
  dpd_->buf4_init(&V, PSIF_CC_GAMMA, G_irr, 2, 10, 2, 10, 0, "GIJKA");
  ...
  dpd_->buf4_close(&G);
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 0, 10, 0, 10, 0, "GIjKa");
  dpd_->buf4_init(&V, PSIF_CC_GAMMA, G_irr, 0, 10, 0, 10, 0, "GIjKa");
  dpd_->buf4_axpy(&G, &V, 0.5);
  dpd_->buf4_close(&V);
  dpd_->buf4_close(&G);
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 0, 10, 0, 10, 0, "GiJkA");
...
void x_Gijka_6_rohf(void) { 
  ...
  dpd_->file2_init(&LR1A, PSIF_EOM_TMP, G_irr, 0, 0, "LR_OO");
  dpd_->file2_init(&LR1B, PSIF_EOM_TMP, G_irr, 0, 0, "LR_oo");
  dpd_->file2_init(&T1A, PSIF_CC_OEI, 0, 0, 1, "tIA");
  dpd_->file2_init(&T1B, PSIF_CC_OEI, 0, 0, 1, "tia");
...
void x_Gijka_7_rohf(void) { 
  ...
  dpd_->file2_init(&LTB, PSIF_EOM_TMP, L_irr, 0, 0, "LT_oo");
...
void x_Gijka_8_rohf(void) { 
  ...
  /* term 8, +P(ij) Lkmfe rimae tjf */
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 0, 10, 0, 0, "Z(IA,KJ)");
  dpd_->buf4_init(&V, PSIF_EOM_TMP, G_irr, 10, 10, 10, 10, 0, "R2L2_OVOV");
  dpd_->file2_init(&T1A, PSIF_CC_OEI, 0, 0, 1, "tIA");
  dpd_->contract424(&V, &T1A, &Z, 3, 1, 0, 1.0, 0.0); 
  dpd_->file2_close(&T1A);
  dpd_->buf4_close(&V);
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP1, psrq, 0, 10, "Z(IJ,KA)");
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 10, 0, 10, 0, "Z(IJ,KA)");
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 0, 10, 2, 10, 0, "GIJKA");
  dpd_->buf4_axpy(&Z, &G, 1.0);
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP1, qprs, 0, 10, "Z(JI,KA)");
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 10, 0, 10, 0, "Z(JI,KA)");
  dpd_->buf4_axpy(&Z, &G, -1.0);
  dpd_->buf4_close(&G);
  dpd_->buf4_close(&Z);
  ...
  dpd_->buf4_close(&Z);
  dpd_->buf4_close(&G);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 0, 10, 0, 0, "Z2(ja,KI)");
  dpd_->buf4_init(&V, PSIF_EOM_TMP, G_irr, 10, 10, 10, 10, 0, "R2L2_ovOV");
  dpd_->file2_init(&T1A, PSIF_CC_OEI, 0, 0, 1, "tIA");
  dpd_->contract424(&V, &T1A, &Z, 3, 1, 0, 1.0, 0.0); 
  dpd_->file2_close(&T1A);
  dpd_->buf4_close(&V);
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP1, psrq, 0, 10, "Z2(jI,Ka)");
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 10, 0, 10, 0, "Z2(jI,Ka)");
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP1, qprs, 0, 10, "Z2(Ij,Ka)");
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 10, 0, 10, 0, "Z2(Ij,Ka)");
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 0, 10, 0, 10, 0, "GIjKa");
  dpd_->buf4_axpy(&Z, &G, -1.0);
  dpd_->buf4_close(&Z);
  dpd_->buf4_close(&G);
  ...
  /* term 9, +P(ij) Lkmfe rimae tjf */
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 0, 10, 0, 0, "Z3(IA,KJ)");
  dpd_->buf4_init(&V, PSIF_EOM_TMP, L_irr, 10, 10, 10, 10, 0, "VIAJB");
  dpd_->file2_init(&R1A, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  dpd_->contract424(&V, &R1A, &Z, 3, 1, 0, 1.0, 0.0); 
  dpd_->file2_close(&R1A);
  dpd_->buf4_close(&V);
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP1, psrq, 0, 10, "Z3(IJ,KA)");
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 10, 0, 10, 0, "Z3(IJ,KA)");
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 0, 10, 2, 10, 0, "GIJKA");
  dpd_->buf4_axpy(&Z, &G, 1.0);
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP1, qprs, 0, 10, "Z3(JI,KA)");
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 10, 0, 10, 0, "Z3(JI,KA)");
  dpd_->buf4_axpy(&Z, &G, -1.0);
  dpd_->buf4_close(&G);
  dpd_->buf4_close(&Z);
  ...
  dpd_->buf4_close(&Z);
  dpd_->buf4_close(&G);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 10, 0, 10, 0, 0, "Z4(ja,KI)");
  dpd_->buf4_init(&V, PSIF_EOM_TMP, L_irr, 10, 10, 10, 10, 0, "ViaJB");
  dpd_->file2_init(&R1A, PSIF_CC_GR, R_irr, 0, 1, "RIA");
  dpd_->contract424(&V, &R1A, &Z, 3, 1, 0, 1.0, 0.0); 
  dpd_->file2_close(&R1A);
  dpd_->buf4_close(&V);
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP1, psrq, 0, 10, "Z4(jI,Ka)");
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 10, 0, 10, 0, "Z4(jI,Ka)");
  dpd_->buf4_sort(&Z, PSIF_EOM_TMP1, qprs, 0, 10, "Z4(Ij,Ka)");
  dpd_->buf4_close(&Z);
  dpd_->buf4_init(&Z, PSIF_EOM_TMP1, G_irr, 0, 10, 0, 10, 0, "Z4(Ij,Ka)");
  dpd_->buf4_init(&G, PSIF_EOM_TMP0, G_irr, 0, 10, 0, 10, 0, "GIjKa");
  dpd_->buf4_axpy(&Z, &G, -1.0);
  dpd_->buf4_close(&Z);
  dpd_->buf4_close(&G);
