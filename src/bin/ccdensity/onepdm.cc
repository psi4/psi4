...
void onepdm(struct RHO_Params rho_params)
{
  ...
  if(params.ref == 0 || params.ref == 1) { /** RHF/ROHF **/
    ...
    dpd_buf4_init(&L2, PSIF_CC_GLG, 0, 0, 7, 2, 7, 0, "LIJAB");
    ...
    dpd_buf4_init(&L2, PSIF_CC_GLG, 0, 2, 5, 2, 7, 0, "LIJAB");
    ...
    if (rho_params.L_ground) {
      dpd_file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tIA");
      dpd_file2_copy(&T1, PSIF_CC_OEI, rho_params.DIA_lbl);
      dpd_file2_close(&T1);
    }
    ...
    dpd_file2_init(&L1, PSIF_CC_GLG, 0, 0, 1, "LIA");
    dpd_file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tIA");
    dpd_file2_init(&Z, PSIF_CC_TMP0, 0, 0, 0, "Z(I,M)");
    ...
    dpd_file2_init(&Z, PSIF_CC_TMP0, 0, 1, 1, "Z(A,E)");
    dpd_buf4_init(&T2, PSIF_CC_TAMPS, 0, 2, 5, 2, 7, 0, "tIJAB");
    dpd_buf4_init(&L2, PSIF_CC_GLG, 0, 2, 5, 2, 7, 0, "LIJAB");
    ...
    dpd_file2_init(&L1, PSIF_CC_GLG, 0, 0, 1, "LIA");
    dpd_file2_copy(&L1, PSIF_CC_OEI, rho_params.DAI_lbl);
    dpd_file2_close(&L1);
    ...
    */
    ...
    if(params.wfn == "CCSD_T" && params.dertype == 1) {
	 ...
	 calculated in cctriples */
      ...
      dpd_file2_copy(&D, PSIF_CC_OEI, rho_params.DIJ_lbl);
      dpd_file2_close(&D);
      dpd_file2_init(&D, PSIF_CC_OEI, 0, 2, 2, "Dij");
      dpd_file2_copy(&D, PSIF_CC_OEI, rho_params.Dij_lbl);
      dpd_file2_close(&D);
      dpd_file2_init(&D, PSIF_CC_OEI, 0, 1, 1, "DAB");
      dpd_file2_copy(&D, PSIF_CC_OEI, rho_params.DAB_lbl);
      dpd_file2_close(&D);
    ...
    dpd_buf4_init(&L2, PSIF_CC_GLG, 0, 10, 17, 12, 17, 0, "Lijab");
    dpd_contract442(&T2, &L2, &D, 0, 0, -1.0, factor);
    dpd_buf4_close(&L2);
    dpd_buf4_close(&T2);
    ...
    dpd_buf4_init(&L2, PSIF_CC_GLG, 0, 12, 15, 12, 17, 0, "Lijab");
    ...
    dpd_file2_init(&L1, PSIF_CC_GLG, 0, 2, 3, "Lia");
    dpd_file2_init(&T1, PSIF_CC_OEI, 0, 2, 3, "tia");
    dpd_file2_init(&Z, PSIF_CC_TMP0, 0, 2, 2, "Z(i,m)");
    dpd_contract222(&T1, &L1, &Z, 0, 0, 1.0, 0.0);
    dpd_file2_close(&L1);
    dpd_contract222(&Z, &T1, &D, 0, 1, -1.0, 1.0);
    dpd_file2_close(&T1);
    ...
    dpd_file2_close(&Z);
    dpd_file2_close(&T1);
    dpd_file2_init(&Z, PSIF_CC_TMP0, 0, 3, 3, "Z(a,e)");
    dpd_buf4_init(&T2, PSIF_CC_TAMPS, 0, 12, 15, 12, 17, 0, "tijab");
    dpd_buf4_init(&L2, PSIF_CC_GLG, 0, 12, 15, 12, 17, 0, "Lijab");
    dpd_contract442(&T2, &L2, &Z, 2, 2, 1.0, 0.0);
    dpd_buf4_close(&L2);
