...
void onepdm(struct RHO_Params rho_params)
{
  ...
  if(params.ref == 0 || params.ref == 1) { /** RHF/ROHF **/
    ...
    if (rho_params.L_ground) {
      dpd_->file2_init(&T1, PSIF_CC_OEI, 0, 0, 1, "tIA");
      dpd_->file2_copy(&T1, PSIF_CC_OEI, rho_params.DIA_lbl);
      dpd_->file2_close(&T1);
    }
    ...
    dpd_->file2_init(&L1, PSIF_CC_GLG, 0, 0, 1, "LIA");
    dpd_->file2_copy(&L1, PSIF_CC_OEI, rho_params.DAI_lbl);
    dpd_->file2_close(&L1);
    ...
    */
    ...
    if(params.wfn == "CCSD_T" && params.dertype == 1) {
	 ...
	 calculated in cctriples */
      ...
      dpd_->file2_copy(&D, PSIF_CC_OEI, rho_params.DIJ_lbl);
      dpd_->file2_close(&D);
      dpd_->file2_init(&D, PSIF_CC_OEI, 0, 2, 2, "Dij");
      dpd_->file2_copy(&D, PSIF_CC_OEI, rho_params.Dij_lbl);
      dpd_->file2_close(&D);
      dpd_->file2_init(&D, PSIF_CC_OEI, 0, 1, 1, "DAB");
      dpd_->file2_copy(&D, PSIF_CC_OEI, rho_params.DAB_lbl);
      dpd_->file2_close(&D);
    ...
    dpd_->buf4_init(&T2, PSIF_CC_TAMPS, 0, 0, 7, 2, 7, 0, "tIJAB");
    dpd_->buf4_init(&L2, PSIF_CC_GLG, 0, 0, 7, 2, 7, 0, "LIJAB");
    dpd_->contract442(&T2, &L2, &D, 0, 0, -1.0, factor);
    dpd_->buf4_close(&L2);
    dpd_->buf4_close(&T2); 
    ...
    dpd_->buf4_init(&L2, PSIF_CC_GLG, 0, 2, 5, 2, 7, 0, "LIJAB");
    dpd_->buf4_init(&T2, PSIF_CC_TAMPS, 0, 2, 5, 2, 7, 0, "tIJAB");
    dpd_->contract442(&L2, &T2, &D, 3, 3, 1.0, factor);
    dpd_->buf4_close(&T2);
    dpd_->buf4_close(&L2);
    dpd_->buf4_init(&L2, PSIF_CC_GLG, 0, 23, 29, 23, 29, 0, "LiJaB");
    dpd_->buf4_init(&T2, PSIF_CC_TAMPS, 0, 23, 29, 23, 29, 0, "tiJaB");
