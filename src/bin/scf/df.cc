...
{   
    ...
    else if (df_storage_ == flip_B_core )
    {
    	...
    	psio_open(PSIF_DFSCF_BJI,PSIO_OPEN_NEW);
    	...
    	for (int ij = 0 ; ij < norbs*(norbs+1)/2; ij++)
    	{
    		...
    		int errcode = psio_write(PSIF_DFSCF_BJI,"BJ Three-Index Integrals",(char *) &(buffer[0][0]),sizeof(double)*ri_nbf_,next_PSIF_DFSCF_BJI,&next_PSIF_DFSCF_BJI);
    	}
    	psio_close(PSIF_DFSCF_BJI,1);
    	free(ao_p_ia);
    	free(temp);
    	
    	B_ia_P_ = block_matrix(ri_nbf_,norbs*(norbs+1)/2);
    	psio_open(PSIF_DFSCF_BJI,PSIO_OPEN_OLD);
    	next_PSIF_DFSCF_BJI = PSIO_ZERO;
    	for (int ij = 0 ; ij < norbs*(norbs+1)/2; ij++)
    	{
    		int errcode = psio_read(PSIF_DFSCF_BJI,"BJ Three-Index Integrals",(char *) &(buffer[0][0]),sizeof(double)*ri_nbf_,next_PSIF_DFSCF_BJI,&next_PSIF_DFSCF_BJI);
    		
    		for (int Q = 0; Q<ri_nbf_; Q++)
    			B_ia_P_[Q][ij] = buffer[Q][0];
    	}
    	psio_close(PSIF_DFSCF_BJI,0);
    	free(buffer);
    }
    ...
    else if (df_storage_ == flip_B_disk )
    {
    	...
    	for (int Q = 0; Q<ri_nbf_; Q++)
    	{
    		int errcode = psio_write(PSIF_DFSCF_BJ,"BJ Three-Index Integrals",(char *) &(B_ia_P_[Q][0]),sizeof(double)*norbs*(norbs+1)/2,next_PSIF_DFSCF_BJ,&next_PSIF_DFSCF_BJ);
    	}
    ...
    else 
    {
    	...
    	for (int ij = 0; ij<norbs*(norbs+1)/2; ij++)
    	{
    		...
    		errcode = psio_write(PSIF_DFSCF_BJI,"BJ Three-Index Integrals",(char *) &(out_buffer[0][0]),sizeof(double)*ri_nbf_,next_PSIF_DFSCF_BJI,&next_PSIF_DFSCF_BJI);
    	}
    	free(out_buffer);
    	...
    	for (int ij = 0; ij < norbs*(norbs+1)/2; ij++)
    	{
    		int errcode = psio_read(PSIF_DFSCF_BJI,"BJ Three-Index Integrals",(char *) &(in_buffer[0][0]),sizeof(double)*ri_nbf_,next_PSIF_DFSCF_BJI,&next_PSIF_DFSCF_BJI);
    		...
    		if (buf_ind == max_cols || ij == norbs*(norbs+1)/2-1)
    		{
    			...
    			{	
    			  //fprintf(outfile,"\n  Working on Q %d",Q); fflush(outfile);
    				...
    				errcode = psio_write(PSIF_DFSCF_BJ,"BJ Three-Index Integrals",(char *) &(buffer[Q][0]),sizeof(double)*buf_ind,next_PSIF_DFSCF_BJ,&next_PSIF_DFSCF_BJ);
...
void RHF::form_G_from_RI()
{
    ...
    //B_ia_P in disk, B_im_Q in disk
    	...
    	for (int Q = 0; Q<ri_nbf_; Q++)
    	{
    		...
    		errcode = psio_write(PSIF_DFSCF_K,"Exchange Tensor",(char *) &(out_buffer[0]),sizeof(double)*norbs*ndocc,next_PSIF_DFSCF_K,&next_PSIF_DFSCF_K);
    	}
    ...
    free_block(Cocc);
    	...
    	for (int Q = 0; Q<ri_nbf_; Q++)
    	{
    		int errcode = psio_read(PSIF_DFSCF_K,"Exchange Tensor",(char *) &(in_buffer[0]),sizeof(double)*norbs*ndocc,next_PSIF_DFSCF_K,&next_PSIF_DFSCF_K);
