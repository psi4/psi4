...
void read_basis(const char *basiskey)
{
   ...
   /*-----------------------
    ...
    -----------------------*/
     ...
     depth = 0;
     errcod = ip_count(const_cast<char *>(basiskey),&depth,0);
     if (depth == 0) { /* the same basis for all atoms */
       errcod = ip_string(const_cast<char *>(basiskey),&basis_type,0);
       if (errcod != IPE_OK)
	 ...
	 atom_basis[i] = basis_type;
       ...
       errcod = ip_count(const_cast<char *>(basiskey),&j,1,0);
	  ...
	  ----------*/
         if (depth == num_atoms)
	   for(i=0;i<num_atoms;i++) {
	     errcod = ip_string(const_cast<char *>(basiskey),&basis_type,1,i);
	     if (errcod != IPE_OK)
	   ...
	   /*----------
	    ...
	    ----------*/
	 for(i=0;i<depth;i++) {
	   errcod = ip_count(const_cast<char *>(basiskey),&j,1,i);
	   if (errcod != IPE_OK || j != 2) {
	     fprintf(outfile,"  There is a problem with line %d of the BASIS array!\n",i+1);
	     punt("Invalid basis set file");;
	   }
	   errcod = ip_string(const_cast<char *>(basiskey),&elem_label,2,i,0);
	   atom_num(elem_label,&Z);
	   free(elem_label);
	   errcod = ip_string(const_cast<char *>(basiskey),&basis_type,2,i,1);
