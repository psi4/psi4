...
void denom_uhf(void)
{
  ...
  dpd_->file4_init(&dIJAB, PSIF_CC_DENOM, 0, 1, 6, "dIJAB");
  for(h=0; h < nirreps; h++) {
  ...
  dpd_->file4_init(&dIJAB, PSIF_CC_DENOM, 0, 11, 16, "dijab");
  for(h=0; h < nirreps; h++) {
      dpd_->file4_mat_irrep_init(&dIJAB, h);
      for(ij=0; ij < dIJAB.params->rowtot[h]; ij++) {
	  i = dIJAB.params->roworb[h][ij][0];
  ...
  dpd_->file4_init(&dIJAB, PSIF_CC_DENOM, 0, 22, 28, "dIjAb");
  for(h=0; h < nirreps; h++) {
      dpd_->file4_mat_irrep_init(&dIJAB, h);
      for(ij=0; ij < dIJAB.params->rowtot[h]; ij++) {
	  i = dIJAB.params->roworb[h][ij][0];
...
void denom_rhf(void)
{
  ...
  dpd_->file4_init(&dIjAb, PSIF_CC_DENOM, 0, 0, 5, "dIjAb");
