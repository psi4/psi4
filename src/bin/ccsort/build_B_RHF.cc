...
void build_B_RHF(double omega)
{
  ...
  dpd_->buf4_sort(&D, PSIF_MO_HESS, sprq, 11, 11, "B(AI,BJ)");
  dpd_->buf4_close(&D);
	
  dpd_->buf4_init(&C, PSIF_CC_CINTS, 0, 10, 10, 10, 10, 0, "C <ia|jb>");
  dpd_->buf4_sort_axpy(&C, PSIF_MO_HESS, qpsr, 11, 11, "B(AI,BJ)", -1);
  dpd_->buf4_close(&C);
  ...
  dpd_->buf4_init(&Bmat, PSIF_MO_HESS, 0, 11, 11, 11, 11, 0, "B(AI,BJ)");
  for(h=0; h < nirreps; h++) {
    dpd_->buf4_mat_irrep_init(&Bmat, h);
    dpd_->buf4_mat_irrep_rd(&Bmat, h);
    ...
    dpd_->buf4_mat_irrep_wrt(&Bmat, h);
    dpd_->buf4_mat_irrep_close(&Bmat, h);
  }
  dpd_->buf4_close(&Bmat);
