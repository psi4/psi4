...
void ffile(FILE **fptr, const char *suffix, int code)
{
  ...
  case 2:
    *fptr = fopen(name,"r+");
    break;
  default:
    psi::fprintf(stderr,"error in ffile: invalid code %d\n",code);
  }
  if (*fptr == NULL) {
    psi::fprintf(stderr,"error in ffile: cannot open file %s\n", suffix);
...
void ffile_noexit(FILE **fptr, char *suffix, int code)
{
  ...
  case 2:
    *fptr = fopen(name,"r+");
    break;
  default:
    psi::fprintf(stderr,"error in ffile_noexit: invalid code %d\n",code);
  }
}
...
void ffileb(FILE **fptr, char *suffix, int code)
{
  ...
  case 2:
    *fptr = fopen(name,"rb");
    break;
  default:
    psi::fprintf(stderr,"error in ffileb: invalid code %d\n",code);
  }
  free(name);

  if (*fptr == NULL) {
    psi::fprintf(stderr,"error in ffileb: cannot open file %s\n", suffix);
    exit(PSI_RETURN_FAILURE);
  }
}
...
void ffileb_noexit(FILE **fptr, char *suffix, int code)
{
  ...
  case 2:
    *fptr = fopen(name,"rb");
    break;
  default:
    psi::fprintf(stderr,"error in ffileb_noexit: invalid code %d\n",code);
  }
  free(name);
}
