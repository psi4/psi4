...
X_B3_Functional::X_B3_Functional(int _block_size): Functional(_block_size)
{
...
X_B3_Functional::~X_B3_Functional()
{
    free(value_);
    free(gradA_);
    free(gradB_);
...
void X_B3_Functional::computeFunctionalRKS(shared_ptr<Properties> prop)
{
        double ao = 0.2;
        double ax = 0.72;	
	const double *rho = prop->getDensity();
        ...
        for (int grid_index = 0; grid_index<ntrue; grid_index++) {
            ...
            value_[grid_index] = -(1-ao)*c*rho13*rho1;
	    gradA_[grid_index] = -(1-ao)*4.0/3.0*c*rho13;
            ...
            value_[grid_index] += -ax*d*rho13*rho1*xs*xs/den;
            gradA_[grid_index] += ax*4.0/3.0*d*drho2/(rho1*rho1*rho13*den);
            gradA_[grid_index] += ax*d*drho2/(rho1*rho13*den*den)*(-8.0*d*sqrt(drho2)*arcsinhxs/(rho1*rho1*rho13)-8.0*d*drho2/(rho1*rho83*sqrt_deal));

	    gradB_[grid_index] = gradA_[grid_index];

            gradAA_[grid_index] = -ax*d/(rho1*rho13*den);
            gradAA_[grid_index] += ax*d*drho2/(rho1*rho13*den*den)*(3.0*d*arcsinhxs/(sqrt(drho2)*rho13*rho1)+3.0*d/(rho83*sqrt_deal));
...
void X_B3_Functional::computeFunctionalUKS(shared_ptr<Properties> prop)
{
    //TODO
}
string X_B3_Functional::getName()
{
	string s("");
	s+="B3";
	return s;
}
string X_B3_Functional::getDescription()
{
	string s("  ");//Leave the spaces
	s+="Becke Exchange Functional with exact exchange contribution";
	return s;
}
string X_B3_Functional::getCitation()
{
	string s("  "); //Leave the spaces
	s+="A. D. Becke, J. Chem. Phys., 98(7), 1993, pp. 5648-5652";
	return s;
}
