...
class MBEFrag: public Set{
	private:
		...
		int MBEOrder;
		void Copy(const MBEFrag& other){this->Parents=other.Parents;this->MBEOrder=other.MBEOrder;}
	public:
		int GetMBEOrder(){return MBEOrder;}
		int ParentI(const int I){return Parents[I];}
		void SetMBEOrder(const int N){MBEOrder=N;}
		void SetParents(const int* Ps){for(int i=0;i<MBEOrder;i++)Parents.push_back(Ps[i]);}
		...
		MBEFrag& operator=(const MBEFrag& other){if(this!=&other){Set::operator=(other);Copy(other);}return *this;}
};
