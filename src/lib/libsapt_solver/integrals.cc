...
void SAPT3B::df_ints()
{
  psio_->open(PSIF_3B_SAPT_AA_DF_INTS,PSIO_OPEN_NEW);
  psio_->open(PSIF_3B_SAPT_BB_DF_INTS,PSIO_OPEN_NEW);
  psio_->open(PSIF_3B_SAPT_CC_DF_INTS,PSIO_OPEN_NEW);
...
#pragma omp for schedule(dynamic)
  ...
  zero_disk(PSIF_3B_SAPT_AA_DF_INTS,"AA RI Integrals",(char *) &(zeros[0]),
            calc_info_.nrio,calc_info_.noccA*calc_info_.noccA);
  zero_disk(PSIF_3B_SAPT_AA_DF_INTS,"AR RI Integrals",(char *) &(zeros[0]),
            calc_info_.nrio,calc_info_.nvirA*calc_info_.noccA);
  zero_disk(PSIF_3B_SAPT_AA_DF_INTS,"RR RI Integrals",(char *) &(zeros[0]),
            calc_info_.nrio,calc_info_.nvirA*calc_info_.nvirA);
  ...
  for (int i=0,oP=0; oP < nmo*nmo; ++i, oP += numP) {
    ...
    for (int ij=0; ij<numP; ij++) {
      ...
      int j = (ij+oP)%nmo;
      if (i < calc_info_.noccA && j < calc_info_.noccA) {
        psio_->write(PSIF_3B_SAPT_AA_DF_INTS,"AA RI Integrals",(char *)
          &(temp_J[ij][0]),sizeof(double)*(ULI) calc_info_.nrio,next_DF_AA,
          &next_DF_AA);
      }
    }
    ...
    for (int ij=0; ij<numP; ij++) {
      ...
      int j = (ij+oP)%nmo;
      if (i < calc_info_.noccA && j >= calc_info_.noccA) {
        psio_->write(PSIF_3B_SAPT_AA_DF_INTS,"AR RI Integrals",(char *)
          &(temp_J[ij][0]),sizeof(double)*(ULI) calc_info_.nrio,next_DF_AR,
          &next_DF_AR);
      }
    }
    ...
    for (int ij=0; ij<numP; ij++) {
      ...
      int j = (ij+oP)%nmo;
      if (i >= calc_info_.noccA && j >= calc_info_.noccA) {
        psio_->write(PSIF_3B_SAPT_AA_DF_INTS,"RR RI Integrals",(char *)
          &(temp_J[ij][0]),sizeof(double)*(ULI) calc_info_.nrio,next_DF_RR,
          &next_DF_RR);
      }
    }
  ...
  zero_disk(PSIF_3B_SAPT_BB_DF_INTS,"BB RI Integrals",(char *) &(zeros[0]),
            calc_info_.nrio,calc_info_.noccB*calc_info_.noccB);
  zero_disk(PSIF_3B_SAPT_BB_DF_INTS,"BS RI Integrals",(char *) &(zeros[0]),
            calc_info_.nrio,calc_info_.nvirB*calc_info_.noccB);
  zero_disk(PSIF_3B_SAPT_BB_DF_INTS,"SS RI Integrals",(char *) &(zeros[0]),
            calc_info_.nrio,calc_info_.nvirB*calc_info_.nvirB);
  ...
  for (int i=0,oP=0; oP < nmo*nmo; ++i, oP += numP) {
    ...
    for (int ij=0; ij<numP; ij++) {
      ...
      int j = (ij+oP)%nmo;
      if (i < calc_info_.noccB && j < calc_info_.noccB) {
        psio_->write(PSIF_3B_SAPT_BB_DF_INTS,"BB RI Integrals",(char *)
          &(temp_J[ij][0]),sizeof(double)*(ULI) calc_info_.nrio,next_DF_BB,
          &next_DF_BB);
      }
    }
    ...
    for (int ij=0; ij<numP; ij++) {
      ...
      int j = (ij+oP)%nmo;
      if (i < calc_info_.noccB && j >= calc_info_.noccB) {
        psio_->write(PSIF_3B_SAPT_BB_DF_INTS,"BS RI Integrals",(char *)
          &(temp_J[ij][0]),sizeof(double)*(ULI) calc_info_.nrio,next_DF_BS,
          &next_DF_BS);
      }
    }
    ...
    for (int ij=0; ij<numP; ij++) {
      ...
      int j = (ij+oP)%nmo;
      if (i >= calc_info_.noccB && j >= calc_info_.noccB) {
        psio_->write(PSIF_3B_SAPT_BB_DF_INTS,"SS RI Integrals",(char *)
          &(temp_J[ij][0]),sizeof(double)*(ULI) calc_info_.nrio,next_DF_SS,
          &next_DF_SS);
      }
    }
  ...
  zero_disk(PSIF_3B_SAPT_CC_DF_INTS,"CC RI Integrals",(char *) &(zeros[0]),
            calc_info_.nrio,calc_info_.noccC*calc_info_.noccC);
  zero_disk(PSIF_3B_SAPT_CC_DF_INTS,"CT RI Integrals",(char *) &(zeros[0]),
            calc_info_.nrio,calc_info_.noccC*calc_info_.nvirC);
  zero_disk(PSIF_3B_SAPT_CC_DF_INTS,"TT RI Integrals",(char *) &(zeros[0]),
            calc_info_.nrio,calc_info_.nvirC*calc_info_.nvirC);

  psio_address next_DF_CC = PSIO_ZERO;
  psio_address next_DF_CT = PSIO_ZERO;
  psio_address next_DF_TT = PSIO_ZERO;
  ...
  for (int i=0,oP=0; oP < nmo*nmo; ++i, oP += numP) {
    ...
    for (int ij=0; ij<numP; ij++) {
      int i = (ij+oP)/nmo;
      int j = (ij+oP)%nmo;
      if (i < calc_info_.noccC && j < calc_info_.noccC) {
        psio_->write(PSIF_3B_SAPT_CC_DF_INTS,"CC RI Integrals",(char *)
          &(temp_J[ij][0]),sizeof(double)*(ULI) calc_info_.nrio,next_DF_CC,
          &next_DF_CC);
      }
    }
    ...
    for (int ij=0; ij<numP; ij++) {
      ...
      int j = (ij+oP)%nmo;
      if (i < calc_info_.noccC && j >= calc_info_.noccC) {
        psio_->write(PSIF_3B_SAPT_CC_DF_INTS,"CT RI Integrals",(char *)
          &(temp_J[ij][0]),sizeof(double)*(ULI) calc_info_.nrio,next_DF_CT,
          &next_DF_CT);
      }
    }
    ...
    for (int ij=0; ij<numP; ij++) {
      ...
      int j = (ij+oP)%nmo;
      if (i >= calc_info_.noccC && j >= calc_info_.noccC) {
        psio_->write(PSIF_3B_SAPT_CC_DF_INTS,"TT RI Integrals",(char *)
          &(temp_J[ij][0]),sizeof(double)*(ULI) calc_info_.nrio,next_DF_TT,
          &next_DF_TT);
      }
    }
...
void SAPT3B::oetrans()
{
    ...
    calc_info_.S_AB = block_matrix(calc_info_.nmo,calc_info_.nmo);
    calc_info_.S_AC = block_matrix(calc_info_.nmo,calc_info_.nmo);
    calc_info_.S_BA = block_matrix(calc_info_.nmo,calc_info_.nmo);
    ...
    double **VAij = block_matrix(calc_info_.nso,calc_info_.nso);
    ...
    double **VAiB = block_matrix(calc_info_.nso,calc_info_.nmo);
    double **VAiC = block_matrix(calc_info_.nso,calc_info_.nmo);
    ...
    C_DGEMM('N','N',calc_info_.nso,calc_info_.nmo,calc_info_.nso,1.0,
            &(VAij[0][0]),calc_info_.nso,&(calc_info_.CC[0][0]),calc_info_.nmo,
            0.0,&(VAiC[0][0]),calc_info_.nmo);

    free_block(VAij);

    calc_info_.VABB = block_matrix(calc_info_.nmo,calc_info_.nmo);
    calc_info_.VACC = block_matrix(calc_info_.nmo,calc_info_.nmo);
    ...
    double **VBij = block_matrix(calc_info_.nso,calc_info_.nso);
    ...
    double **VBiA = block_matrix(calc_info_.nso,calc_info_.nmo);
    double **VBiC = block_matrix(calc_info_.nso,calc_info_.nmo);

    C_DGEMM('N','N',calc_info_.nso,calc_info_.nmo,calc_info_.nso,1.0,
            &(VBij[0][0]),calc_info_.nso,&(calc_info_.CA[0][0]),calc_info_.nmo,
            0.0,&(VBiA[0][0]),calc_info_.nmo);

    C_DGEMM('N','N',calc_info_.nso,calc_info_.nmo,calc_info_.nso,1.0,
            &(VBij[0][0]),calc_info_.nso,&(calc_info_.CC[0][0]),calc_info_.nmo,
            0.0,&(VBiC[0][0]),calc_info_.nmo);

    free_block(VBij);

    calc_info_.VBAA = block_matrix(calc_info_.nmo,calc_info_.nmo);
    calc_info_.VBCC = block_matrix(calc_info_.nmo,calc_info_.nmo);

    C_DGEMM('T','N',calc_info_.nmo,calc_info_.nmo,calc_info_.nso,1.0,
            &(calc_info_.CA[0][0]),calc_info_.nmo,&(VBiA[0][0]),calc_info_.nmo,
            0.0,&(calc_info_.VBAA[0][0]),calc_info_.nmo);

    C_DGEMM('T','N',calc_info_.nmo,calc_info_.nmo,calc_info_.nso,1.0,
            &(calc_info_.CC[0][0]),calc_info_.nmo,&(VBiC[0][0]),calc_info_.nmo,
            0.0,&(calc_info_.VBCC[0][0]),calc_info_.nmo);

    free_block(VBiA);
    free_block(VBiC);

    double **VCij = block_matrix(calc_info_.nso,calc_info_.nso);

    for (int i=0,ij=0; i<calc_info_.nso; i++) {
      for (int j=0; j<=i; j++,ij++) {
        VCij[i][j] = calc_info_.VC[ij];
        VCij[j][i] = calc_info_.VC[ij];
        }}

    free(calc_info_.VC);

    double **VCiA = block_matrix(calc_info_.nso,calc_info_.nmo);
    double **VCiB = block_matrix(calc_info_.nso,calc_info_.nmo);

    C_DGEMM('N','N',calc_info_.nso,calc_info_.nmo,calc_info_.nso,1.0,
            &(VCij[0][0]),calc_info_.nso,&(calc_info_.CA[0][0]),calc_info_.nmo,
            0.0,&(VCiA[0][0]),calc_info_.nmo);

    C_DGEMM('N','N',calc_info_.nso,calc_info_.nmo,calc_info_.nso,1.0,
            &(VCij[0][0]),calc_info_.nso,&(calc_info_.CB[0][0]),calc_info_.nmo,
            0.0,&(VCiB[0][0]),calc_info_.nmo);

    free_block(VCij);

    calc_info_.VCAA = block_matrix(calc_info_.nmo,calc_info_.nmo);
    calc_info_.VCBB = block_matrix(calc_info_.nmo,calc_info_.nmo);

    C_DGEMM('T','N',calc_info_.nmo,calc_info_.nmo,calc_info_.nso,1.0,
            &(calc_info_.CA[0][0]),calc_info_.nmo,&(VCiA[0][0]),calc_info_.nmo,
            0.0,&(calc_info_.VCAA[0][0]),calc_info_.nmo);

    C_DGEMM('T','N',calc_info_.nmo,calc_info_.nmo,calc_info_.nso,1.0,
            &(calc_info_.CB[0][0]),calc_info_.nmo,&(VCiB[0][0]),calc_info_.nmo,
            0.0,&(calc_info_.VCBB[0][0]),calc_info_.nmo);
...
void SAPT3B::w_ints()
{
    ...
    calc_info_.WBAR = block_matrix(calc_info_.noccA,calc_info_.nvirA);
    calc_info_.WBRR = block_matrix(calc_info_.nvirA,calc_info_.nvirA);
    build_w(calc_info_.WBAA,calc_info_.WBAR,calc_info_.WBRR,calc_info_.VBAA,
      PSIF_3B_SAPT_AA_DF_INTS,"AA RI Integrals","AR RI Integrals",
      "RR RI Integrals",PSIF_3B_SAPT_BB_DF_INTS,"BB RI Integrals",
      calc_info_.noccA,calc_info_.nvirA,calc_info_.noccB);

    calc_info_.WCAA = block_matrix(calc_info_.noccA,calc_info_.noccA);
    calc_info_.WCAR = block_matrix(calc_info_.noccA,calc_info_.nvirA);
    calc_info_.WCRR = block_matrix(calc_info_.nvirA,calc_info_.nvirA);
    build_w(calc_info_.WCAA,calc_info_.WCAR,calc_info_.WCRR,calc_info_.VCAA,
      PSIF_3B_SAPT_AA_DF_INTS,"AA RI Integrals","AR RI Integrals",
      "RR RI Integrals",PSIF_3B_SAPT_CC_DF_INTS,"CC RI Integrals",
      calc_info_.noccA,calc_info_.nvirA,calc_info_.noccC);

    calc_info_.WABB = block_matrix(calc_info_.noccB,calc_info_.noccB);
    calc_info_.WABS = block_matrix(calc_info_.noccB,calc_info_.nvirB);
    calc_info_.WASS = block_matrix(calc_info_.nvirB,calc_info_.nvirB);
    build_w(calc_info_.WABB,calc_info_.WABS,calc_info_.WASS,calc_info_.VABB,
      PSIF_3B_SAPT_BB_DF_INTS,"BB RI Integrals","BS RI Integrals",
      "SS RI Integrals",PSIF_3B_SAPT_AA_DF_INTS,"AA RI Integrals",
      calc_info_.noccB,calc_info_.nvirB,calc_info_.noccA);

    calc_info_.WCBB = block_matrix(calc_info_.noccB,calc_info_.noccB);
    calc_info_.WCBS = block_matrix(calc_info_.noccB,calc_info_.nvirB);
    calc_info_.WCSS = block_matrix(calc_info_.nvirB,calc_info_.nvirB);
    build_w(calc_info_.WCBB,calc_info_.WCBS,calc_info_.WCSS,calc_info_.VCBB,
      PSIF_3B_SAPT_BB_DF_INTS,"BB RI Integrals","BS RI Integrals",
      "SS RI Integrals",PSIF_3B_SAPT_CC_DF_INTS,"CC RI Integrals",
      calc_info_.noccB,calc_info_.nvirB,calc_info_.noccC);

    calc_info_.WACC = block_matrix(calc_info_.noccC,calc_info_.noccC);
    calc_info_.WACT = block_matrix(calc_info_.noccC,calc_info_.nvirC);
    calc_info_.WATT = block_matrix(calc_info_.nvirC,calc_info_.nvirC);
    build_w(calc_info_.WACC,calc_info_.WACT,calc_info_.WATT,calc_info_.VACC,
      PSIF_3B_SAPT_CC_DF_INTS,"CC RI Integrals","CT RI Integrals",
      "TT RI Integrals",PSIF_3B_SAPT_AA_DF_INTS,"AA RI Integrals",
      calc_info_.noccC,calc_info_.nvirC,calc_info_.noccA);

    calc_info_.WBCC = block_matrix(calc_info_.noccC,calc_info_.noccC);
    calc_info_.WBCT = block_matrix(calc_info_.noccC,calc_info_.nvirC);
    calc_info_.WBTT = block_matrix(calc_info_.nvirC,calc_info_.nvirC);
    build_w(calc_info_.WBCC,calc_info_.WBCT,calc_info_.WBTT,calc_info_.VBCC,
      PSIF_3B_SAPT_CC_DF_INTS,"CC RI Integrals","CT RI Integrals",
      "TT RI Integrals",PSIF_3B_SAPT_BB_DF_INTS,"BB RI Integrals",
  ...
  int B_file, char *BB_ints, int noccA, int nvirA, int noccB)
{
    ...
    double **B_p_AA = get_DF_ints(A_file,AA_ints,noccA*noccA);

    C_DGEMV('n',noccA*noccA,calc_info_.nrio,2.0,&(B_p_AA[0][0]),
      calc_info_.nrio,diagBB,1,1.0,&(WBAA[0][0]),1);

    free_block(B_p_AA);

    double **B_p_AR = get_DF_ints(A_file,AR_ints,noccA*nvirA);

    C_DGEMV('n',noccA*nvirA,calc_info_.nrio,2.0,&(B_p_AR[0][0]),
      calc_info_.nrio,diagBB,1,1.0,&(WBAR[0][0]),1);

    free_block(B_p_AR);

    double **B_p_RR = get_DF_ints(A_file,RR_ints,nvirA*nvirA);

    C_DGEMV('n',nvirA*nvirA,calc_info_.nrio,2.0,&(B_p_RR[0][0]),
      calc_info_.nrio,diagBB,1,1.0,&(WBRR[0][0]),1);
