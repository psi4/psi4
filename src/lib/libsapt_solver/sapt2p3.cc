...
SAPT2p3::SAPT2p3(SharedWavefunction Dimer, SharedWavefunction MonomerA,
                 SharedWavefunction MonomerB, Options& options,
                 boost::shared_ptr<PSIO>psio)
                  : SAPT2p(Dimer, MonomerA, MonomerB, options, psio),
  e_elst13_(0.0),
  e_ind30_(0.0),
  e_exch_ind30_(0.0),
  e_ind30r_(0.0),
...
void SAPT2p3::print_results()
{
  ...
  for(scal_it = Xscal.begin(); scal_it != Xscal.end(); ++scal_it) {
    ...
    outfile->Printf("      Elst12,r                %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
      e_elst12_ * 1000.0,
      e_elst12_ * pc_hartree2kcalmol,
      e_elst12_ * pc_hartree2kJmol);
    outfile->Printf("      Elst13,r                %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
      e_elst13_ * 1000.0,
    ...
    outfile->Printf("      Exch11(S^2) %3s         %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
      scaled.c_str(),
      *scal_it * e_exch11_ * 1000.0,
      *scal_it * e_exch11_ * pc_hartree2kcalmol,
      *scal_it * e_exch11_ * pc_hartree2kJmol);
    outfile->Printf("      Exch12(S^2) %3s         %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
      scaled.c_str(),
    ...
    if (third_order_)
      outfile->Printf("      Ind30,r                 %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
        e_ind30r_ * 1000.0,
        e_ind30r_ * pc_hartree2kcalmol,
        e_ind30r_ * pc_hartree2kJmol);
    outfile->Printf("      Ind22                   %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
      e_ind22_ * 1000.0,
    ...
    if (third_order_)
      outfile->Printf("      Exch-Ind30,r %3s        %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
        scaled.c_str(),
        *scal_it * e_exch_ind30r_ * 1000.0,
        *scal_it * e_exch_ind30r_ * pc_hartree2kcalmol,
        *scal_it * e_exch_ind30r_ * pc_hartree2kJmol);
    outfile->Printf("      Exch-Ind22 %3s          %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
      scaled.c_str(),
    ...
    if (third_order_)
      outfile->Printf("      delta HF,r (3) %3s      %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
        scaled.c_str(),
        dHF3 * 1000.0,
        dHF3 * pc_hartree2kcalmol,
        dHF3 * pc_hartree2kJmol);
    if(e_MP2 != 0.0) {
        outfile->Printf("      delta MP2,r (2) %3s     %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
          scaled.c_str(),
          dMP2_2 * 1000.0,
          dMP2_2 * pc_hartree2kcalmol,
          dMP2_2 * pc_hartree2kJmol);
        if (third_order_)
          outfile->Printf("      delta MP2,r (3) %3s     %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
            scaled.c_str(),
            dMP2_3 * 1000.0,
    ...
    outfile->Printf("      Disp30                  %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
      e_disp30_ * 1000.0,
      e_disp30_ * pc_hartree2kcalmol,
      e_disp30_ * pc_hartree2kJmol);
    outfile->Printf("      Disp21                  %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
      e_disp21_ * 1000.0,
      e_disp21_ * pc_hartree2kcalmol,
      e_disp21_ * pc_hartree2kJmol);
    if (mbpt_disp_) {
      outfile->Printf("      Disp22 (SDQ)            %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
        e_disp22sdq_ * 1000.0,
        e_disp22sdq_ * pc_hartree2kcalmol,
        e_disp22sdq_ * pc_hartree2kJmol);
      outfile->Printf("      Disp22 (T)              %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
        e_disp22t_ * 1000.0,
        e_disp22t_ * pc_hartree2kcalmol,
        e_disp22t_ * pc_hartree2kJmol);
      if (nat_orbs_t3_)
        outfile->Printf("      Est. Disp22 (T)         %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
          e_est_disp22t_ * 1000.0,
          e_est_disp22t_ * pc_hartree2kcalmol,
          e_est_disp22t_ * pc_hartree2kJmol);
    }
    if (ccd_disp_) {
      outfile->Printf("      Disp2 (CCD)             %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
        e_disp2d_ccd_ * 1000.0,
        e_disp2d_ccd_ * pc_hartree2kcalmol,
        e_disp2d_ccd_ * pc_hartree2kJmol);
      outfile->Printf("      Disp22 (S) (CCD)        %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
        e_disp22s_ccd_ * 1000.0,
        e_disp22s_ccd_ * pc_hartree2kcalmol,
        e_disp22s_ccd_ * pc_hartree2kJmol);
      outfile->Printf("      Disp22 (T) (CCD)        %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
        e_disp22t_ccd_ * 1000.0,
        e_disp22t_ccd_ * pc_hartree2kcalmol,
        e_disp22t_ccd_ * pc_hartree2kJmol);
      if (nat_orbs_t3_)
        outfile->Printf("      Est. Disp22 (T) (CCD)   %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
    ...
    if (third_order_) {
      outfile->Printf("      Exch-Disp30 %3s         %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
        scaled.c_str(),
        *scal_it * e_exch_disp30_ * 1000.0,
        *scal_it * e_exch_disp30_ * pc_hartree2kcalmol,
        *scal_it * e_exch_disp30_ * pc_hartree2kJmol);
      outfile->Printf("      Ind-Disp30              %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
        e_ind_disp30_ * 1000.0,
        e_ind_disp30_ * pc_hartree2kcalmol,
        e_ind_disp30_ * pc_hartree2kJmol);
      outfile->Printf("      Exch-Ind-Disp30 %3s     %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
        scaled.c_str(),
    ...
    outfile->Printf("  Total SAPT2 %3s             %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
      scaled.c_str(),
      e_sapt2_ * 1000.0,
    ...
    if (mbpt_disp_) {
      ...
      outfile->Printf("  Total SAPT2+(3) %3s         %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
        scaled.c_str(),
        e_sapt2pp3_ * 1000.0,
      ...
      if(e_MP2 != 0.0) {
          outfile->Printf("  Total SAPT2+dMP2 %3s        %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
            scaled.c_str(),
            e_sapt2p_dmp2 * 1000.0,
            e_sapt2p_dmp2 * pc_hartree2kcalmol,
            e_sapt2p_dmp2 * pc_hartree2kJmol);
          outfile->Printf("  Total SAPT2+(3)dMP2 %3s     %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
            scaled.c_str(),
            e_sapt_2pp3_dmp2 * 1000.0,
            e_sapt_2pp3_dmp2 * pc_hartree2kcalmol,
            e_sapt_2pp3_dmp2 * pc_hartree2kJmol);
          if (third_order_)
            outfile->Printf("  Total SAPT2+3dMP2   %3s     %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
              scaled.c_str(),
    ...
    if (ccd_disp_) {
      outfile->Printf("  Total SAPT2+(CCD) %3s       %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
        scaled.c_str(),
        e_sapt2p_ccd_ * 1000.0,
        e_sapt2p_ccd_ * pc_hartree2kcalmol,
        e_sapt2p_ccd_ * pc_hartree2kJmol);
      outfile->Printf("  Total SAPT2+(3)(CCD) %3s    %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
        scaled.c_str(),
      ...
      if(e_MP2 != 0.0) {
          outfile->Printf("  Total SAPT2+(CCD)dMP2 %3s   %16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
            scaled.c_str(),
            e_sapt2p_ccd_dmp2 * 1000.0,
            e_sapt2p_ccd_dmp2 * pc_hartree2kcalmol,
            e_sapt2p_ccd_dmp2 * pc_hartree2kJmol);
          outfile->Printf("  Total SAPT2+(3)(CCD)dMP2 %3s%16.8lf mH %16.8lf kcal mol^-1 %16.8lf kJ mol^-1\n",
            scaled.c_str(),
