...
void XFunctional::compute_sigma_functional(const std::map<std::string,SharedVector>& in, const std::map<std::string,SharedVector>& out, int npoints, int deriv, double alpha, bool spin)
{
    ...
    for (int Q = 0; Q < npoints; Q++) {
        ...
        switch (gga_type_) {
            ...
            case RPBE: {
                double kk0 = (4.0 * _k0_ * _k0_);
                Fs = 1.0 + _PBE_kp_ * (1.0 - exp(- _PBE_mu_ * s * s / (_PBE_kp_ * kk0))); 
                Fs_s = 2.0 * _PBE_mu_ * s * exp(- _PBE_mu_ * s * s / (_PBE_kp_ * kk0)); 
                break;
            }
            case SOGGA: {
                double mus2 = 1.0 + _PBE_mu_ * s * s / (4.0 * _k0_ * _k0_ * _PBE_kp_);
                ...
                Fs_s = 2.0 * _PBE_mu_ * s * exp(- _PBE_mu_ * s * s / (_PBE_kp_ * kk0)) + 2.0 / (mus2 * mus2) * _PBE_mu_ * s / (4.0 * _k0_ * _k0_); 
                break;
            }
            case PW91: {
