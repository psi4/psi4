...
boost::shared_ptr<Functional> Functional::createFunctional(const std::string & name, int npoints, int deriv)
{
    ...
    if (boost::to_upper_copy(name) == boost::to_upper_copy(std::string("wB_X")))
        return boost::shared_ptr<Functional> (new wB_X_Functional(npoints,deriv));
