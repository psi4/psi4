function data = getTauHCTHFunctional()
...
data.param_vals = [0.0, 3.0/5.0*(6.0*pi*pi)^(2.0/3.0),...
    ...
    0.006, 0.2, 0.004,...
    0.65262, 6.3638, -14.080, -3.3755,...
    0.41385,-0.9086, -0.0549,  1.7480,...
    1.10734,-1.0534,  6.3491, -2.5531,...
    0.00110,-0.3041,  6.9543, -0.7235];  
...
% KE part
syms C_k real
t_a = heaviside(1E20*tau_a - C_k*rho_a^(5/3))*C_k*rho_a^(5/3)/tau_a + heaviside(C_k*rho_a^(5/3) - 1E20*tau_a)*1E20;
t_b = heaviside(1E20*tau_b - C_k*rho_b^(5/3))*C_k*rho_b^(5/3)/tau_b + heaviside(C_k*rho_b^(5/3) - 1E20*tau_a)*1E20;
w_a = (t_a - 1) / (t_a + 1);
w_b = (t_b - 1) / (t_b + 1);
...
data.description = 'TauHCTH Power Series Pure Meta-GGA';
