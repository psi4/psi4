...
namespace globalFutures {
  ...
  typedef std::list<CommInfo> ProcList;
  ...
  void GFFinalize()
  {
    ...
    for (ProcList::const_iterator iter = procList.begin(); iter != procList.end(); iter++)
      ofs << iter->target << ", " << iter->g_a << ", " << iter->tid << std::endl;
  ...
  void GF_Get(int g_a, int lo[], int hi[], void *buf, int ld[])
  {
    ...
    for (int i = 0; i < np; i++) {
      ...
      cinfo.target = procs[i];
      cinfo.g_a = g_a;
      cinfo.tid = this_tbb_thread::get_id();

      procList.push_back(cinfo);
    }
