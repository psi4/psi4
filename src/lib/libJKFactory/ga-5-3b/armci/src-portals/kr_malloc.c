...
static Header *morecore(size_t nu, context_t *ctx, size_t *last_size, char **last_ptr) {
    char *cp;
    Header *up;
...
char *_kr_last_ptr;
size_t _kr_last_size;

char *kr_malloc(size_t nbytes, context_t *ctx, int record_allocation, char **new_base, size_t *new_size) {
    Header *p, *prevp;
    size_t nunits;
    char *return_ptr;
    ...
    for (p=prevp->s.ptr; ; prevp = p, p = p->s.ptr) {
      ...
      if (p == ctx->freep) {       /* wrapped around the free list */
	      if ((p = morecore(nunits, ctx, &_kr_last_size,&_kr_last_ptr)) == (Header *) NULL) {
    ...
    if(record_allocation){
      *((char **)new_base)=_kr_last_ptr;
      *new_size=_kr_last_size;
    }
    return return_ptr;
}
...
static char *kr_malloc_shmem(size_t nbytes, context_t *ctx) {
	...
	kr_error("kr_malloc_shmem: Alignment is not valid",
      ...
      ctx->base.s.valid1 = VALID1;
      ctx->base.s.valid2 = VALID2;
      if ((p = morecore(nunits, ctx,NULL,NULL)) == (Header *) NULL) return NULL;
	...
	break;
      ...
      if (p == ctx->freep)	{	/* wrapped around the free list */
	if ((p = morecore(nunits, ctx,NULL,NULL)) == (Header *) NULL) {
	  return_ptr = (char *) NULL;
	  break;
	}
	prev_shmid     = ctx->shmid;
