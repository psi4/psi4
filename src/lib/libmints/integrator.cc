...
RadialQuadrature Integrator::getBeckeRadial(int n, double xi)
{
    ...
    for (int tau = 1; tau<=n; tau++) {
        ...
        //$w *= r^2$ accounts for spherical integration on R^3
                ...
                rad.w[tau-1] = M_PI/(n+1.0)*temp*temp*xi*2.0/((1.0+x)*(1.0+x)*sqrt(1.0-x*x));
        rad.w[tau-1] *= rad.r[tau-1]*rad.r[tau-1];
    }
    return rad;
}
...
RadialQuadrature Integrator::getEMRadial(int n, double xi)
{
    ...
    rad.r = init_array(n);
    for (int tau = 1; tau<=n; tau++) {
        //$r = \xi*\tau^2*(n_\tau+1-i)^-2$
        rad.r[tau-1] = xi*tau*tau/((n+1.0-tau)*(n+1.0-tau));
        //$w = 2*\xi^3*(n_\tau+1)*\tau^5*(n_\tau+1-i)^{-7}$
        rad.w[tau-1] = 2.0*xi*xi*xi*(n+1.0)*pow(tau,5.0)*pow(n+1.0-tau,-7.0);
    }
    return rad;
}
RadialQuadrature Integrator::getTreutlerRadial(int n, double xi, double alpha)
{
    ...
    for (int tau = 1; tau<=n; tau++) {
        ...
        //$r = \frac{\xi}{\ln(2)}(1+x)^{\alpha}\ln\left(\frac{2.0}{1-x}\right)$
                x = cos(tau/(n+1.0)*M_PI);
        rad.r[tau-1] = xi*INVLN2*pow(1.0+x,alpha)*log(2.0/(1.0-x));
        //$w = \frac{\pi}{n_tau+1.0}\sin^2\left(\frac{\tau}{n_\tau+1}\pi\right)$
        //$w *= \frac{2\xi}{(1+x)^2}$ Accounts for change of variable
        //$w *= \frac{1}{\sqrt{1-x^2}}$ Accounts for integral type
        //$w *= r^2$ accounts for spherical integration on R^3
                temp = sin(tau/(n+1.0)*M_PI);
                rad.w[tau-1] = M_PI/(n+1.0)*temp*temp;
        rad.w[tau-1] *= xi*INVLN2*(alpha*pow(1.0+x,alpha-1.0)*log(2.0/(1.0-x))+pow(1.0+x,alpha)/(1.0-x));
        rad.w[tau-1] *= 1.0/sqrt(1.0-x*x);
        rad.w[tau-1] *= rad.r[tau-1]*rad.r[tau-1];
    }
...
SphericalQuadrature Integrator::getLebedevSpherical(int degree)
{
    ...
    start = getLebedevReccurencePoints(1,start,a,b,v,leb_tmp);
    break;
    ...
    start = getLebedevReccurencePoints(3,start,a,b,v,leb_tmp);
    break;
    ...
    start = getLebedevReccurencePoints(2,start,a,b,v,leb_tmp);
    ...
    start = getLebedevReccurencePoints(5,start,a,b,v,leb_tmp);
    break;
    ...
    start = getLebedevReccurencePoints(4,start,a,b,v,leb_tmp);
    break;
    ...
    start = getLebedevReccurencePoints(6,start,a,b,v,leb_tmp);
    break;
...
int Integrator::getLebedevReccurencePoints(int type, int start, double a, double b, double v, const SphericalQuadrature & leb)
{
    ...
    leb.y[start] = 0.0;
    leb.z[start] = 0.0;
        leb.w[start] = 4.0*M_PI*v;

    leb.x[start+1] = -a;
    leb.y[start+1] = 0.0;
    leb.z[start+1] = 0.0;
        leb.w[start+1] = 4.0*M_PI*v;

    leb.x[start+2] = 0.0;
    leb.y[start+2] = a;
    leb.z[start+2] = 0.0;
        leb.w[start+2] = 4.0*M_PI*v;

    leb.x[start+3] = 0.0;
    leb.y[start+3] = -a;
    leb.z[start+3] = 0.0;
        leb.w[start+3] = 4.0*M_PI*v;

    leb.x[start+4] = 0.0;
    leb.y[start+4] = 0.0;
    leb.z[start+4] = a;
        leb.w[start+4] = 4.0*M_PI*v;

    leb.x[start+5] = 0.0;
    leb.y[start+5] = 0.0;
    leb.z[start+5] = -a;
        leb.w[start+5] = 4.0*M_PI*v;
    end = start+6;
    break;
    ...
    leb.y[start+6] = 0.0;
    leb.z[start+6] = a;
        leb.w[start+6] = 4.0*M_PI*v;

    leb.x[start+7] = -a;
    leb.y[start+7] = 0.0;
    leb.z[start+7] = -a;
        leb.w[start+7] = 4.0*M_PI*v;

    leb.x[start+8] = a;
    leb.y[start+8] = a;
    leb.z[start+8] = 0.0;
        leb.w[start+8] = 4.0*M_PI*v;

    leb.x[start+9] = -a;
    leb.y[start+9] = a;
    leb.z[start+9] = 0.0;
        leb.w[start+9] = 4.0*M_PI*v;

    leb.x[start+10] = a;
    leb.y[start+10] = -a;
    leb.z[start+10] = 0.0;
        leb.w[start+10] = 4.0*M_PI*v;

    leb.x[start+11] = -a;
    leb.y[start+11] = -a;
    leb.z[start+11] = 0.0;
        leb.w[start+11] = 4.0*M_PI*v;
    end = start+12;
    break;
    ...
    leb.y[start+12] = b;
    leb.z[start+12] = -a;
        leb.w[start+12] = 4.0*M_PI*v;

    leb.x[start+13] = a;
    leb.y[start+13] = -b;
    leb.z[start+13] = -a;
        leb.w[start+13] = 4.0*M_PI*v;

    leb.x[start+14] = -a;
    leb.y[start+14] = -b;
    leb.z[start+14] = -a;
        leb.w[start+14] = 4.0*M_PI*v;

    leb.x[start+15] = a;
    leb.y[start+15] = b;
    leb.z[start+15] = a;
        leb.w[start+15] = 4.0*M_PI*v;

    leb.x[start+16] = b;
    leb.y[start+16] = a;
    leb.z[start+16] = a;
        leb.w[start+16] = 4.0*M_PI*v;

    leb.x[start+17] = -b;
    leb.y[start+17] = a;
    leb.z[start+17] = a;
        leb.w[start+17] = 4.0*M_PI*v;

    leb.x[start+18] = b;
    leb.y[start+18] = -a;
    leb.z[start+18] = a;
        leb.w[start+18] = 4.0*M_PI*v;

    leb.x[start+19] = b;
    leb.y[start+19] = a;
    leb.z[start+19] = -a;
        leb.w[start+19] = 4.0*M_PI*v;

    leb.x[start+20] = -b;
    leb.y[start+20] = -a;
    leb.z[start+20] = a;
        leb.w[start+20] = 4.0*M_PI*v;

    leb.x[start+21] = -b;
    leb.y[start+21] = a;
    leb.z[start+21] = -a;
        leb.w[start+21] = 4.0*M_PI*v;

    leb.x[start+22] = b;
    leb.y[start+22] = -a;
    leb.z[start+22] = -a;
        leb.w[start+22] = 4.0*M_PI*v;

    leb.x[start+23] = -b;
    leb.y[start+23] = -a;
    leb.z[start+23] = -a;
        leb.w[start+23] = 4.0*M_PI*v;
    end = start + 24;
    break;
    ...
    leb.y[start+24] = b;
    leb.z[start+24] = a;
        leb.w[start+24] = 4.0*M_PI*v;

    leb.x[start+25] = -c;
    leb.y[start+25] = b;
    leb.z[start+25] = a;
        leb.w[start+25] = 4.0*M_PI*v;

    leb.x[start+26] = c;
    leb.y[start+26] = -b;
    leb.z[start+26] = a;
        leb.w[start+26] = 4.0*M_PI*v;

    leb.x[start+27] = c;
    leb.y[start+27] = b;
    leb.z[start+27] = -a;
        leb.w[start+27] = 4.0*M_PI*v;

    leb.x[start+28] = -c;
    leb.y[start+28] = -b;
    leb.z[start+28] = a;
        leb.w[start+28] = 4.0*M_PI*v;

    leb.x[start+29] = c;
    leb.y[start+29] = -b;
    leb.z[start+29] = -a;
        leb.w[start+29] = 4.0*M_PI*v;

    leb.x[start+30] = -c;
    leb.y[start+30] = b;
    leb.z[start+30] = -a;
        leb.w[start+30] = 4.0*M_PI*v;

    leb.x[start+31] = -c;
    leb.y[start+31] = -b;
    leb.z[start+31] = -a;
        leb.w[start+31] = 4.0*M_PI*v;

    leb.x[start+32] = a;
    leb.y[start+32] = c;
    leb.z[start+32] = b;
        leb.w[start+32] = 4.0*M_PI*v;

    leb.x[start+33] = -a;
    leb.y[start+33] = c;
    leb.z[start+33] = b;
        leb.w[start+33] = 4.0*M_PI*v;

    leb.x[start+34] = a;
    leb.y[start+34] = -c;
    leb.z[start+34] = b;
        leb.w[start+34] = 4.0*M_PI*v;

    leb.x[start+35] = a;
    leb.y[start+35] = c;
    leb.z[start+35] = -b;
        leb.w[start+35] = 4.0*M_PI*v;

    leb.x[start+36] = -a;
    leb.y[start+36] = -c;
    leb.z[start+36] = b;
        leb.w[start+36] = 4.0*M_PI*v;

    leb.x[start+37] = a;
    leb.y[start+37] = -c;
    leb.z[start+37] = -b;
        leb.w[start+37] = 4.0*M_PI*v;

    leb.x[start+38] = -a;
    leb.y[start+38] = c;
    leb.z[start+38] = -b;
        leb.w[start+38] = 4.0*M_PI*v;

    leb.x[start+39] = -a;
    leb.y[start+39] = -c;
    leb.z[start+39] = -b;
        leb.w[start+39] = 4.0*M_PI*v;

    leb.x[start+40] = b;
    leb.y[start+40] = c;
    leb.z[start+40] = a;
        leb.w[start+40] = 4.0*M_PI*v;

    leb.x[start+41] = -b;
    leb.y[start+41] = c;
    leb.z[start+41] = a;
        leb.w[start+41] = 4.0*M_PI*v;

    leb.x[start+42] = b;
    leb.y[start+42] = -c;
    leb.z[start+42] = a;
        leb.w[start+42] = 4.0*M_PI*v;

    leb.x[start+43] = b;
    leb.y[start+43] = c;
    leb.z[start+43] = -a;
        leb.w[start+43] = 4.0*M_PI*v;

    leb.x[start+44] = -b;
    leb.y[start+44] = -c;
    leb.z[start+44] = a;
        leb.w[start+44] = 4.0*M_PI*v;

    leb.x[start+45] = b;
    leb.y[start+45] = -c;
    leb.z[start+45] = -a;
        leb.w[start+45] = 4.0*M_PI*v;

    leb.x[start+46] = -b;
    leb.y[start+46] = c;
    leb.z[start+46] = -a;
        leb.w[start+46] = 4.0*M_PI*v;

    leb.x[start+47] = -b;
    leb.y[start+47] = -c;
    leb.z[start+47] = -a;
        leb.w[start+47] = 4.0*M_PI*v;
    end = start + 48;
    break;
