    ...
    buffer_offset_(0L)
{
    ...
    try {
        ...
        psi::fprintf(stderr, "Error allocating dscratch_.\n%s\n", e.what());
        exit(EXIT_FAILURE);
    }
    ...
    try {
        ...
        psi::fprintf(stderr, "Error allocating iscratch.\n%s\n", e.what());
        exit(EXIT_FAILURE);
    }
...
#if DEBUG
    psi::fprintf(outfile, "\timin %d iopt %d zmin %d zopt %d\n", imin, iopt, zmin, zopt);
#endif
    i_buffer_size_ = iopt;
    d_buffer_size_ = zopt;
...
size_t ERDTwoElectronInt::compute_shell(int shell_i, int shell_j, int shell_k, int shell_l)
{
    ...
    psi::fprintf(outfile, "\n\nShell (%2d %2d | %2d %2d) - center (%2d %2d | %2d %2d) - angular momentum (%d %d | %d %d)\n",
                    ...
                    am1, am2, am3, am4);
    psi::fprintf(outfile, "XYZ1: %16.10f %16.10f %16.10f\n", x1, y1, z1);
    psi::fprintf(outfile, "XYZ2: %16.10f %16.10f %16.10f\n", x2, y2, z2);
    psi::fprintf(outfile, "XYZ3: %16.10f %16.10f %16.10f\n", x3, y3, z3);
    psi::fprintf(outfile, "XYZ4: %16.10f %16.10f %16.10f\n", x4, y4, z4);
    psi::fprintf(outfile, "Indices  -> %d, %d\n", ccbeg_[0], ccend_[0]);

    psi::fprintf(outfile, "Number of primitives: %d %d %d %d\n", npgto1, npgto2, npgto3, npgto4);
    psi::fprintf(outfile, "Coefficients: ");
    for(int n = 0; n < npgto; ++n)
        psi::fprintf(outfile, "%14.10f ", cc_[n]);
    psi::fprintf(outfile, "\n");
    psi::fprintf(outfile, "Exponents:    ");
    for(int n = 0; n < npgto; ++n)
        psi::fprintf(outfile, "%14.10f ", alpha_[n]);
    psi::fprintf(outfile, "\n");
#endif
...
#if DEBUG
    psi::fprintf(outfile, "Buffer offset is %d\n", buffer_offset_-1);
    psi::fprintf(outfile, "%d integrals were computed\n", nbatch);
#endif
