...
CubeFile::process_density()
{
    ...
    // Two header lines
    fprintf(cubefile, "Generated by Psi4. Bounding box: X %.4f to %.4f, Y%.4f to %.4f, Z %.4f to %.4f\n",
                        minx_, maxx_, miny_, maxy_, minz_, maxz_);
    fprintf(cubefile, "Density from the %s wavefunction.\n", wfn_->name().c_str());
    // Natoms, origin
