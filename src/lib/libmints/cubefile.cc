...
CubeFile::process_density()
{
    ...
    FILE *cubefile = fopen(filename_.c_str(), "w");
    // Two header lines
    psi::fprintf(cubefile, "Generated by Psi4. Bounding box: X %.4f to %.4f, Y%.4f to %.4f, Z %.4f to %.4f\n",
                        minx_, maxx_, miny_, maxy_, minz_, maxz_);
    psi::fprintf(cubefile, "Density from the %s wavefunction.\n", wfn_->name().c_str());
    // Natoms, origin
    psi::fprintf(cubefile, "%5d%12.6f%12.6f%12.6f\n", natoms, minx_, miny_, minz_);
    ...
    for(int atom = 0; atom < natoms; ++atom){
        ...
        psi::fprintf(cubefile, "%5d%12.6f%12.6f%12.6f%12.6f\n", 
    ...
    //        psi::fprintf(cubefile, "%f %f\n", x, y);
            for(int zvox = 0; zvox < nptsz_; ++zvox){
                ...
                psi::fprintf(cubefile, "%13.5E", val);
                if((count++ % 6) == 5 && zvox!=nptsz_-1)
                    psi::fprintf(cubefile, "\n");
            }
            psi::fprintf(cubefile, "\n");
        }
    }
