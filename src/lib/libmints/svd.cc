...
L10:
    psi::fprintf(stderr,"ERROR: INPUT PARAMETER IQ FOR SUBROUTINE SING\n");
    psi::fprintf(stderr,"EITHER LESS THAN 0 OR GREATER THAN 3\n");
    abort();
L20:
    if (*iq > 3) {
	goto L10;
...
L40:
    psi::fprintf(stderr,"ERROR: INPUT PARAMETER IP FOR SUBROUTINE SING\n");
    psi::fprintf(stderr,"EITHER LESS THAN 0 OR GREATER THAN 3\n");
    abort();
L50:
    if (*ip > 3) {
...
L10:
    psi::fprintf(stderr,"ERROR: INPUT PARAMETER IQ FOR SUBROUTINE BIDAG2\n");
    psi::fprintf(stderr,"EITHER LESS THAN 0 OR GREATER THAN 3\n");
    abort();
L20:
    if (*iq > 3) {
...
L40:
    psi::fprintf(stderr,"ERROR: INPUT PARAMETER IP FOR SUBROUTINE BIDAG2\n");
    psi::fprintf(stderr,"EITHER LESS THAN 0 OR GREATER THAN 3\n");
    abort();
L50:
    if (*ip > 3) {
...
hsr3_(double *a, int *la, int *m, int *n)
{
    ...
    psi::fprintf(stderr,"ERROR: ARGUMENT M MUST BE .GE. N IN SUBROUTINE HSR3\n");
    abort();
L10:
    s = -(double)a[k + k * a_dim1];
    i__1 = *m;
...
hsr4_(double *a, int *la, int *m, int *n)
{
    ...
    psi::fprintf(stderr,"ERROR: ARGUMENT M MUST BE .GE. N IN SUBROUTINE HSR4\n");
    abort();
L10:
...
hsr5_(double *a, int *la, int *m, int *n)
{
    ...
    psi::fprintf(stderr,"ERROR: ARGUMENT M MUST BE .GE. N IN SUBROUTINE HSR5\n");
    abort();
L10:
    if (*m == *n) {
	goto L90;
...
L530:
    k = *n - k + 1;
    psi::fprintf(stderr,"SINCE THE STOPPING CRITERION NOT SATISFIED\n");
    psi::fprintf(stderr,"AFTER %d ITERATIONS, WE STOP WHILE COMPUTING\n", ns);
    psi::fprintf(stderr,"EIGENVALUE NUMBER %d", k);
    e[1] = (real) k;
    return 0;
} /* singb_ */
