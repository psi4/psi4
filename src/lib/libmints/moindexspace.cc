...
{
    boost::shared_ptr<Matrix> SBA = overlap(B, A.basis());
    SBA->set_name("Sba");
    boost::shared_ptr<Matrix> SBB = overlap(B, B);
    SBB->set_name("SBB");
    ...
    // 2. Form T
    boost::shared_ptr<Matrix> I = Matrix::create("I = SAB SBB SBA", SBA->colspi(), SBA->colspi());
    I->transform(SBB, SBA);

    boost::shared_ptr<Matrix> T = Matrix::create("T", A.dim(), A.dim());
    T->transform(I, A.C());
    ...
    // 4a. Ca T^{-1/2}
    boost::shared_ptr<Matrix> CaT = Matrix::create("Ca*T^{-1/2}", A.C()->rowspi(), A.C()->colspi());
    CaT->gemm(false, false, 1.0, A.C(), T, 0.0);

    // 4b. Sba * 4a
    boost::shared_ptr<Matrix> SbaCaT = Matrix::create("SbaCaT", SBB->rowspi(), A.C()->colspi());
    SbaCaT->gemm(false, false, 1.0, SBA, CaT, 0.0);

    // 4c. [Sbb]^-1 * 4b
    boost::shared_ptr<Matrix> Cb = Matrix::create("Cb", SBB->rowspi(), A.C()->colspi());
...
boost::shared_ptr<Matrix> MOIndexSpace::overlap(const MOIndexSpace &space1, const MOIndexSpace &space2)
{
...
boost::shared_ptr<Matrix> MOIndexSpace::overlap(const boost::shared_ptr<BasisSet>& basis1,
