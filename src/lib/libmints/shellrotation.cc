...
//    }
    ...
    for (I.first(); !I.is_done(); I.next()) {
        for (J.first(); !J.is_done(); J.next()) {
            ...
            double tmp = 0.0;
//            psi::fprintf(outfile, "J.a = %d J.b = %d J.c = %d\n", J.a(), J.b(), J.c());
//            psi::fprintf(outfile, "I.coef = %lf, J.coef = %lf\n", I.coef(), J.coef());
            for (K.start(J.a(), J.b(), J.c()); K; K.next()) {
//                psi::fprintf(outfile, "T(%d,%d) += %6.4f", I.pureindex(), J.pureindex(), coef);
                double tmp2 = coef;
                for (m=0; m<3; ++m)
                    lI[m] = I.l(m);
                ...
                for (m=0; m<am_; ++m) {
                    ...
                    tmp2 *= so(K.axis(m), iI);
//                    psi::fprintf(outfile, " * so(%d,%d) [=%4.2f]",
//                            iI,K.axis(m),so(iI,K.axis(m)));
                }
//                psi::fprintf(outfile, " = %8.6f\n", tmp2);
                tmp += tmp2;
            }
...
{
    psi::fprintf(outfile, "ShellRotation\n");
