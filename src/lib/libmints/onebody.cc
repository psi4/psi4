...
//        psi::fprintf(outfile, "1e_1: cart = %d pure = %d coef = %8.5f\n", sti.cartindex(), sti.pureindex(), sti.coef());

        for(int l=0; l<nl; l++) {
//            psi::fprintf(outfile, "\ttptr = %8.5f coef = %8.5f sptr = %8.5f\n", *tptr, coef, *sptr);
            *(tptr++) += coef * *(sptr++);
        }
    }
}
...
//        psi::fprintf(outfile, "1e_2: cart = %d pure = %d coef = %8.5f\n", sti.cartindex(), sti.pureindex(), sti.coef());
...
void OneBodyAOInt::compute(std::vector<SharedMatrix > &result)
{
    ...
    if (result.size() != nchunk_) {
        psi::fprintf(stderr, "result length = %ld, nchunk = %d\n", result.size(), nchunk_);
