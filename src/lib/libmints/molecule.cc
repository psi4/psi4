...
double Molecule::mass(int atom) const
{
    ...
    if (fabs(atoms_[atom]->Z() - static_cast<int>(atoms_[atom]->Z())) > 0.0)
        psi::fprintf(outfile, "WARNING: Obtaining masses from atom with fractional charge...may be incorrect!!!\n");
...
void Molecule::print_in_angstrom() const
{
    // Sometimes one just wants angstroms regardless of input units
        ...
        if (natom()) {
            if (pg_) psi::fprintf(outfile,"    Molecular point group: %s\n", pg_->symbol().c_str());
            if (full_pg_) psi::fprintf(outfile,"    Full point group: %s\n\n", full_point_group().c_str());
            psi::fprintf(outfile,"    Geometry (in %s), charge = %d, multiplicity = %d:\n\n",
                    "Angstrom", molecular_charge_, multiplicity_);
            psi::fprintf(outfile,"       Center              X                  Y                   Z       \n");
            psi::fprintf(outfile,"    ------------   -----------------  -----------------  -----------------\n");

            for(int i = 0; i < natom(); ++i){
                psi::fprintf(outfile, "    %8s%4s ",symbol(i).c_str(),Z(i) ? "" : "(Gh)"); fflush(outfile);
                for(int j = 0; j < 3; j++)
                    psi::fprintf(outfile, "  %17.12f", xyz(i, j) * pc_bohr2angstroms);
                psi::fprintf(outfile,"\n");
            }
            psi::fprintf(outfile,"\n");
            fflush(outfile);
            ...
            psi::fprintf(outfile, "  No atoms in this molecule.\n");
    ...
    // and psi3 using bohr.
        ...
        if (natom()) {
            ...
            for(int i = 0; i < natom(); ++i){
                psi::fprintf(outfile, "    %8s%4s ",symbol(i).c_str(),Z(i) ? "" : "(Gh)"); fflush(outfile);
                for(int j = 0; j < 3; j++)
                    psi::fprintf(outfile, "  %17.12f", xyz(i, j));
                psi::fprintf(outfile,"\n");
            }
            psi::fprintf(outfile,"\n");
            fflush(outfile);
...
void Molecule::print_in_input_format() const
{
        if (nallatom()) {
            ...
            for(int i = 0; i < nallatom(); ++i){
                if (fZ(i) || (fsymbol(i) == "X")) {
                    psi::fprintf(outfile, "    %-8s", fsymbol(i).c_str());
                } else {
                    std::string stmp = std::string("Gh(") + fsymbol(i) + ")";
                    psi::fprintf(outfile, "    %-8s", stmp.c_str());
                }
            ...
            if(geometry_variables_.size()){
                std::map<std::string, double>::const_iterator iter;
                for(iter = geometry_variables_.begin(); iter!=geometry_variables_.end(); ++iter){
                    psi::fprintf(outfile, "    %-10s=%16.10f\n", iter->first.c_str(), iter->second);
                }
                psi::fprintf(outfile, "\n");
            }
        }
...
void Molecule::print() const
{
        ...
        if (natom()) {
            ...
            for(int i = 0; i < natom(); ++i){
                ...
                psi::fprintf(outfile, "    %8s%4s ",symbol(i).c_str(),Z(i) ? "" : "(Gh)"); fflush(outfile);
                for(int j = 0; j < 3; j++)
                    psi::fprintf(outfile, "  %17.12f", geom[j]);
                psi::fprintf(outfile,"\n");
            }
            psi::fprintf(outfile,"\n");
            fflush(outfile);
...
void Molecule::print_full() const
{
        ...
        if (natom()) {
            ...
            for(int i = 0; i < full_atoms_.size(); ++i){
                ...
                psi::fprintf(outfile, "    %8s%4s ",fsymbol(i).c_str(),fZ(i) ? "" : "(Gh)"); fflush(outfile);
                for(int j = 0; j < 3; j++)
                    psi::fprintf(outfile, "  %17.12f", geom[j]);
                psi::fprintf(outfile,"\n");
            }
...
{
    psi::fprintf(outfile, "        Interatomic Distances (Angstroms)\n\n");
    for(int i=0;i<natom();i++) {
        for(int j=i+1;j<natom();j++) {
            Vector3 eij = xyz(j)-xyz(i);
            double distance=eij.norm();
            psi::fprintf(outfile, "        Distance %d to %d %-8.3lf\n",i+1,j+1,distance*pc_bohr2angstroms);
        }
    }
    psi::fprintf(outfile, "\n\n");
}
...
{
    psi::fprintf(outfile, "        Bond Angles (degrees)\n\n");
    for(int j=0;j<natom();j++) {
        for(int i=0;i<natom();i++){
            if(i==j)
                continue;
            for(int k=i+1;k<natom();k++) {
                ...
                psi::fprintf(outfile, "        Angle %d-%d-%d: %8.3lf\n", i+1, j+1, k+1, phi);
            }
        }
    }
    psi::fprintf(outfile, "\n\n");
...
{
    psi::fprintf(outfile, "        Dihedral Angles (Degrees)\n\n");
    for(int i=0; i<natom(); i++) {
        for(int j=0; j<natom(); j++) {
            if(i==j)
                continue;
            for(int k=0; k<natom(); k++) {
                ...
                for(int l=0; l<natom(); l++) {
                    ...
                    psi::fprintf(outfile, "        Dihedral %d-%d-%d-%d: %8.3lf\n", i+1, j+1, k+1, l+1, tau);
                }
            }
        }
    }
...
{
    psi::fprintf(outfile, "        Out-Of-Plane Angles (Degrees)\n\n");
    for(int i=0; i<natom(); i++) {
        for(int j=0; j<natom(); j++) {
            if(i==j)
                continue;
            for(int k=0; k<natom(); k++) {
                ...
                for(int l=0; l<natom(); l++) {
                    ...
                    psi::fprintf(outfile, "        Out-of-plane %d-%d-%d-%d: %8.3lf\n", i+1, j+1, k+1, l+1, theta);
                }
            }
        }
    }
...
/*
  ...
  if (rot_const[0] == 0) // linear
    psi::fprintf(outfile,"\tA = **********  ");
  else               // non-linear
    psi::fprintf(outfile,"\tA = %10.5lf  ", rot_const[0]);

  if (rot_const[1] == 0) // atom
    psi::fprintf(outfile,"  B = **********    C = **********  \n");
  else               // molecule
    psi::fprintf(outfile,"  B = %10.5lf   C = %10.5lf\n", rot_const[1], rot_const[2]);
*/
  return rot_const;
}
...
void Molecule::print_rotational_constants(void) const {
  ...
  if (rot_const[0] == 0.0) // linear
    ...
    psi::fprintf(outfile,"\tA = %10.5lf  ", rot_const[0]*pc_c/10000);
  if (rot_const[1] == 0.0) // atom
    psi::fprintf(outfile,"  B = **********    C = **********  \n");
  else               // molecule
...
RotorType Molecule::rotor_type(double zero_tol) const {
  ...
  //psi::fprintf(outfile, "\tDegeneracy is %d\n", degen);
...
//    psi::fprintf(outfile, "xaxis %20.14lf %20.14lf %20.14lf\n", xaxis[0], xaxis[1], xaxis[2]);
//    psi::fprintf(outfile, "yaxis %20.14lf %20.14lf %20.14lf\n", yaxis[0], yaxis[1], yaxis[2]);
//    psi::fprintf(outfile, "zaxis %20.14lf %20.14lf %20.14lf\n", zaxis[0], zaxis[1], zaxis[2]);
...
void Molecule::form_symmetry_information(double tol)
{
    ...
    if (natom() == 0) {
        ...
        nequiv_  = 0;
        atom_to_unique_ = 0;
        //psi::fprintf(outfile, "No atoms detected, returning\n");fflush(outfile);
        return;
    }
...
void Molecule::set_variable(const std::string &str, double val)
{
    ...
    geometry_variables_[str] = val;
        ...
        psi::fprintf(outfile, "Setting geometry variable %s to %f\n", str.c_str(), val);
    try {
        update_geometry();
    }
    catch (...) {
...
void Molecule::set_full_point_group(double zero_tol) {
  ...
  //psi::fprintf(outfile,"\t\tRotor type        : %s\n", RotorTypeList[rotor].c_str());
  ...
  //psi::fprintf(outfile,"d2h_subgroup %s \n", d2h_subgroup.c_str());

  // Check inversion
  Vector3 v3_zero(0, 0, 0);
  bool op_i = has_inversion(v3_zero, zero_tol);
  //psi::fprintf(outfile,"\t\tInversion symmetry: %s\n", (op_i ? "yes" : "no"));
  ...
  if (rotor == RT_ATOM) { // atoms
    ...
    if (d2h_subgroup == "c1") {
      ...
      psi::fprintf(outfile,"\t\tWarning: Cannot determine point group.\n");
  }
    ...
    // I_evects->print_out();
    // psi::fprintf(outfile,"I_evals %15.10lf %15.10lf %15.10lf\n", I_evals[0], I_evals[1], I_evals[2]);
    ...
    if (fabs(phi) > 1.0e-14) {
      ...
      //psi::fprintf(outfile, "Rotating by %lf to get principal axis on z-axis.\n", phi);
      geom.copy(test_mat);
    }

    //psi::fprintf(outfile,"Geometry to analyze - principal axis on z-axis:\n");
    //for (i=0; i<natom(); ++i)
      //psi::fprintf(outfile,"%20.15lf %20.15lf %20.15lf\n", geom(i,0), geom(i,1), geom(i,2));
    //psi::fprintf(outfile,"\n");

    // Determine order Cn and Sn of principal axis.
    int Cn_z = matrix_3d_rotation_Cn(geom, z_axis, false, zero_tol);
    //psi::fprintf(outfile,"\t\tHighest rotation axis (Cn_z) : %d\n", Cn_z);

    int Sn_z = matrix_3d_rotation_Cn(geom, z_axis, true, zero_tol);
    //psi::fprintf(outfile,"\t\tHighest rotation axis (Sn_z) : %d\n", Sn_z);
    ...
    if (fabs(phi) > 1.0e-14) {
      ...
      //psi::fprintf(outfile, "Rotating by %8.3e to get atom %d in yz-plane.\n", phi, pivot_atom_i+1);
      geom.copy(test_mat);
    }
    ...
    if (i == natom())
      op_sigma_v = true;
    //psi::fprintf(outfile,"\t\tsigma_v : %s\n", (op_sigma_v ? "yes" : "no"));

    //psi::fprintf(outfile,"geom to analyze - one atom in yz plane\n");
    //for (i=0; i<natom(); ++i)
      //psi::fprintf(outfile,"%20.15lf %20.15lf %20.15lf\n", geom(i,0), geom(i,1), geom(i,2));
    //psi::fprintf(outfile,"\n");
    ...
    //psi::fprintf(outfile,"\t\tperp. C2's :  %s\n", (is_D ? "yes" : "no"));
