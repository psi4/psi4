...
/**
* This will read in the ROHF effective Fock matrix, which is written to checkpoint
* by the Hartree-Fock code, and diagonalize the occ-occ and vir-vir blocks in the
* alpha and beta spaces.  This is called automatically if the TransformationType
...
IntegralTransform::semicanonicalize(shared_ptr<PSIO> psio, shared_ptr<Chkpt> chkpt)
{
    ...
    for(int h = 0, moOffset = 0; h < _nirreps; ++h){
        ...
        if(nOccOrbs){
            ...
            if(int stat = C_DSYEV('v','u', nOccOrbs, F[0], nOccOrbs, &(evals[moOffset]), work, 3*_nmo)) {
            ...
            C_DGEMM('n', 't', _sopi[h], nOccOrbs, nOccOrbs, 1.0, C[h][0],
                    _mopi[h], F[0], nOccOrbs, 0.0, _Ca[0][0], _mopi[h]);
        }
        // Update the Fock Matrix
        for(int i = 0; i < nOccOrbs; ++i){
            for(int j = 0; j <= i; ++j){
                int ij = INDEX((i + moOffset), (j + moOffset));
                temp[ij] = i == j ? evals[i] : 0.0;
            }
        }
        free_block(F);
        ...
        F = block_matrix(nVirOrbs, nVirOrbs);
        ...
        if(nVirOrbs){
            // Diagonalize the Fock matrix; on exit F is the rotation matrix to diagonalize F
            if(int stat = C_DSYEV('v','u', nVirOrbs, F[0], nVirOrbs, &(evals[moOffset + nOccOrbs]), work, 3*_nmo)) {
...
//    iwl_wrtone(PSIF_OEI, PSIF_MO_A_FOCK, _nTriMo, temp);
    ...
    for(int h = 0, moOffset = 0; h < _nirreps; ++h){
        ...
        if(nOccOrbs){
            // Diagonalize the Fock matrix; on exit F is the rotation matrix to diagonalize F
            if(int stat = C_DSYEV('v','u', nOccOrbs, F[0], nOccOrbs, &(evals[moOffset]), work, 3*_nmo)) {
                fprintf(outfile, "rotate(): Error in beta Foo[%1d] diagonalization. stat = %d\n", h, stat);
                throw PsiException("Semicanonicalization error", __FILE__, __LINE__);
            }
            // Now rotate the orbitals using the eigenvectors of F
            C_DGEMM('n', 't', _sopi[h], nOccOrbs, nOccOrbs, 1.0, C[h][0],
                    _mopi[h], F[0], nOccOrbs, 0.0, _Cb[0][0], _mopi[h]);
        }
        // Update the Fock Matrix
        for(int i = 0; i < nOccOrbs; ++i){
            for(int j = 0; j <= i; ++j){
        ...
        free_block(C[h]);
        if(_print > 3){
            fprintf(outfile, "Beta orbitals for irrep %d\n", h);
