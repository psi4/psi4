...
RBase::RBase(bool flag) :
    Wavefunction(Process::environment.options,_default_psio_lib_) 
{
    psi::fprintf(outfile, "Dirty hack %s\n\n", (flag ? "true" : "false"));
}
RBase::~RBase()
{
    postiterations();
...
void RBase::common_init()
{
    ...
    if (!ref) {
        ...
        psi::fprintf(outfile, "    Attempting to build reference from Chkpt.\n\n");
...
void RBase::preiterations()
{
    if (!jk_) {
        if (options_.get_bool("SAVE_JK")) {
            ...
            psi::fprintf(outfile,"    Reusing JK object from SCF.\n\n");
        } else {
            jk_ = JK::build_JK();
...
void RCPHF::print_header()
{
    ...
    if (tasks_.size()) {
        psi::fprintf(outfile, "  ==> Named Tasks <==\n\n");
    ...
    if (debug_ > 1) {
        psi::fprintf(outfile, "  ==> Fock Matrix (MO Basis) <==\n\n");
        eps_aocc_->print();
        eps_avir_->print();
    }
}
...
double RCPHF::compute_energy()
{
    ...
    if (print_) {
        psi::fprintf(outfile, "  ==> CPHF Iterations <==\n\n");
    }
...
{
    psi::fprintf(outfile, "  ==> Excitation Energies <==\n\n");

    psi::fprintf(outfile,"  -----------------------------------------------\n");
    psi::fprintf(outfile,"  %5s %11s %14s %14s\n",
    ...
    for (int i = 0; i < states_.size(); i++) {
        ...
        psi::fprintf(outfile,"  %-5d %1s%-5d(%3s) %14.6E %14.6E\n",
    ...
    if (debug_ > 1) {
        if (singlets_.size()) {
            psi::fprintf(outfile, "  ==> Singlet States <==\n\n");
        ...
        if (triplets_.size()) {
            psi::fprintf(outfile, "  ==> Triplet States <==\n\n");
...
void RCIS::print_amplitudes()
{
    ...
    psi::fprintf(outfile, "  ==> Significant Amplitudes <==\n\n");

    psi::fprintf(outfile,"  --------------------------------------------------\n");
    psi::fprintf(outfile,"  %5s %11s %20s %11s\n",
    ...
    for (int i = 0; i < states_.size(); i++) {
        ...
        if (amps.size()) {
            ...
            psi::fprintf(outfile,"  %-5d %1s%-5d(%3s) %5d%-3s -> %5d%-3s %11.3E\n",
...
void RCIS::print_transitions()
{
    ...
    psi::fprintf(outfile, "  ==> GS->XS Oscillator Strengths <==\n\n");

    psi::fprintf(outfile,"  --------------------------------------------------------------------\n");
    psi::fprintf(outfile,"  %5s %11s %11s %11s %11s %14s\n",
    ...
    for (int i = 0; i < states_.size(); i++) {
        ...
        psi::fprintf(outfile,"  %-5d %1s%-5d(%3s) %11.3E %11.3E %11.3E %14.6E\n",
