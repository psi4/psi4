...
//                madness::Future<SharedMatrix> F_sum = task(me_, &LMP2::build_F_sum, ij, iter);
    ...
    if (iter > 0) {
        ...
        if (fabs(Delta_Elmp2_) < econv_ && fabs(Drms_T2_) < rmsconv_ || iter >= maxiter_) {
          ...
          if (iter >= maxiter_)
            if (me_ == 0)
              psi::fprintf(outfile, "LMP2 has not converged in the maximum number of iterations.\n maxiter = %d\n", maxiter_);
        }
        else conv = 0;
    }
    return conv;
...
//                psi::fprintf(outfile, "\tF_sum ij: %d\n", ij);
