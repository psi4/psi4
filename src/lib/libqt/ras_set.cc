...
{
    psi::fprintf(outfile, "libqt::ras_set: Not converted to liboptions yet...beware!");
    return 1;
#if 0
  ...
  if (errbad == 1) {
    psi::fprintf(stderr, "(ras_set): trouble parsing RAS keyword\n");
    return(0);
  }
    ...
    if (ras_type==0) ras_opi[1][irrep] = socc[irrep];
      ...
      for (irrep = 0; irrep<nirreps; irrep++) {
         ...
         if (ras_opi[1][irrep] < 0) {
           psi::fprintf(stderr, "(ras_set): val_orb must be larger than RAS I\n");
           return(0);
         }
      }
    }
  ...
  for (irrep=0; irrep<nirreps; irrep++) {
    ...
    if (tmpi > orbspi[irrep]) {
      psi::fprintf(stderr, "(ras_set): orbitals don't add up for irrep %d\n",
          irrep);
      return(0);
    }
  ...
  for (irrep=0; irrep<nirreps; irrep++) {
    if (used[irrep] > orbspi[irrep]) {
      psi::fprintf(stderr, "(ras_set): on final check, used more orbitals");
      psi::fprintf(stderr, "   than were available (%d vs %d) for irrep %d\n",
          used[irrep], orbspi[irrep], irrep);
      errbad = 1;
    }
    if (used[irrep] < orbspi[irrep]) {
      psi::fprintf(stderr, "(ras_set): on final check, used fewer orbitals");
      psi::fprintf(stderr, "   than were available (%d vs %d) for irrep %d\n",
          used[irrep], orbspi[irrep], irrep);
      errbad = 1;
    }
  ...
  */
  ...
  if (!parsed_ras2) {
    if (options["ACTIVE"].has_changed()) {
      ...
      if (parsed_ras1)
        psi::fprintf(outfile, "ras_set(): Warning: ACTIVE overrides RAS 1\n");
