...
X = zeros([numatoms],Float)
Y = zeros([numatoms],Float)
Z = zeros([numatoms],Float)
...
while( (len(White)>0) or (len(Queue)>0) ): #Iterates to the next fragment
   while(len(Queue)>0): #BFS within a fragment
      for u in Queue:   #find all nearest Neighbors (still coloured white) to vertex u
            for i in White:
               Distance = sqrt((X[i]-X[u])*(X[i]-X[u]) + (Y[i]-Y[u])*(Y[i]-Y[u]) + (Z[i]-Z[u])*(Z[i]-Z[u]))
      ...
      Black.append(u)
      Fragment[frag].append(int(u+1)) #and add to group (adding 1 to start list at one instead of zero)
